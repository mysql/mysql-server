...
{
  if ((strlen(name) > sizeof(LOG_SERVICES_PREFIX)) &&
      (name[11]=='.') &&
      (0 == strncmp(name, LOG_SERVICES_PREFIX, 11)))
    name+= (sizeof(LOG_SERVICES_PREFIX));

  if (0 == strncmp(name, LOG_BUILTINS_FILTER,
                   sizeof(LOG_BUILTINS_FILTER)))
    return LOG_SERVICE_BUILTIN_TYPE_FILTER;
  if (0 == strncmp(name, LOG_BUILTINS_SINK,
                   sizeof(LOG_BUILTINS_SINK)))
...
static const log_item_wellknown_key log_item_wellknown_keys[] =
{
  ...
  { C_STRING_WITH_LEN("table"),        LOG_CSTRING,
  ...
  { C_STRING_WITH_LEN("and_n_more"),   LOG_INTEGER,
...
int log_item_wellknown_by_type(log_item_type t)
{
  ...
  for (c= 0; (c < log_item_wellknown_keys_count); c++)
  {
    if (log_item_wellknown_keys[c].item_type == t)
      return c;
  }
...
int log_item_inconsistent(log_item *li)
{
  ...
  if (c != li->item_class)
    return LOG_ITEM_CLASS_MISMATCH;

  // no key set
  if (li->key == nullptr)
    return LOG_ITEM_KEY_NULL;
...
{
  while (ll->count > 0)
    log_item_free(&(ll->item[--ll->count]));
...
int log_line_index_by_item(log_line *ll, log_item *ref)
{
  ...
  else
  {
    while (count > 0)
    {
      if (ll->item[--count].type == ref->type)
        return count;
    }
  }
...
static int log_sink_trad(void *instance MY_ATTRIBUTE((unused)), log_line *ll)
{
  ...
  if (ll->count > 0)
  {
    for (c= 0; c < ll->count; c++)
    {
      ...
      if (log_item_inconsistent(&ll->item[c]))
        continue;
    ...
    {
      msg= "No error message, or error message of non-string type. " \
  ...
  */
  if (ll->count > 0)
  {
             ...
             (ll->seen & LOG_ITEM_SQL_ERRSYMBOL))
    {
      ...
      int            n=        log_line_index_by_type(ll,
...
*/
static size_t
log_builtins_stack_get_service_from_var(const char **s, const char **e)
{
                                            ...
                                            char *buf, size_t bufsiz)
{
      ...
      (service != nullptr))
  {
    ...
    if ((ls= reinterpret_cast<SERVICE_TYPE(log_service)*>(service)) == nullptr)
    {
      imp_mysql_server_registry.release(service);
      service= nullptr;
    }
...
static log_service_cache_entry *log_service_cache_entry_new(const char *name,
                                                            ...
                                                            my_h_service srv)
{
  char                    *n=   my_strndup(key_memory_log_error_stack,
                                           name, name_len, MYF(0));
  ...
  if (n != nullptr)
  {
    ...
    if ((sce= (log_service_cache_entry *)
         my_malloc(key_memory_log_error_stack,
                   sizeof(log_service_cache_entry),
                   MYF(0))) == nullptr)
      my_free(n);
    else
    {
                                               ...
                                               log_line *ll)
{
  ...
  if ((lsi= (log_service_instance *)
       my_malloc(key_memory_log_error_stack,
                 sizeof(log_service_instance),
                 MYF(0))) != nullptr)
  {
...
int log_builtins_error_stack_flush()
{
  ...
  while ((lsi != nullptr) && ((sce= lsi->sce) != nullptr))
  {
    if (sce->type == LOG_SERVICE_BUILTIN_TYPE_NONE)
    {
      ...
      if (ls != nullptr)
      {
        if ((ls->flush != nullptr) && (ls->flush(&lsi->instance) < 0))
          rr--;
      }
      else
        rr--;
...
int log_builtins_init()
{
  ...
  if (mysql_rwlock_init(0, &THR_LOCK_log_stack))
    return -1;
...
/**
  ...
  @retval        >0:                      index in array of wellknowns
*/
DEFINE_METHOD(int, log_builtins_imp::wellknown_by_name, (const char *key,
                                                         size_t length))
{
...
*/
DEFINE_METHOD(int,           log_builtins_imp::item_inconsistent,
                                                     (log_item *li))
{
...
/**
  ...
  @retval  false    if wellknown type
*/
DEFINE_METHOD(bool,             log_builtins_imp::item_generic_type,
                                                           (log_item_type t))
{
...
/**
  ...
  @retval   false   if not of a string class
*/
DEFINE_METHOD(bool,             log_builtins_imp::item_string_class,
                                                          (log_item_class c))
{
...
/**
  ...
  @retval   false  if not of a numeric class
*/
DEFINE_METHOD(bool,             log_builtins_imp::item_numeric_class,
                                                          (log_item_class c))
{
...
/**
  ...
  @retval         the number of items set
*/
DEFINE_METHOD(int,              log_builtins_imp::line_item_count,
                                              (log_line *ll))
{
                                                       ...
                                                       void **my_errstream))
{
  ...
  *my_errstream= nullptr;
  ...
  if ((file == nullptr) ||
      (log_error_dest == nullptr) || (!strcmp(log_error_dest, "stderr")))
  {
  ...
  else
  {
    ...
    if (file[0] == '.')
    {
      fn_format(errorlog_filename_buff, log_error_dest, path,
                file, MY_APPEND_EXT | MY_REPLACE_DIR);

    }
    else
    {
      fn_format(errorlog_filename_buff, file, path,
                ".les", MY_REPLACE_DIR);
...
/**
  ...
  @retval !0                 failure
*/
DEFINE_METHOD(int, log_builtins_imp::write_errstream, (void *my_errstream,
                                                       const char *buffer,
                                                       size_t length))
{
...
/**
  ...
  @retval  1                 dedicated
*/
DEFINE_METHOD(int, log_builtins_imp::dedicated_errstream, (void *my_errstream))
{
  log_errstream *les= (log_errstream *) my_errstream;
...
/**
  ...
  @param len  length of string to copy
*/
DEFINE_METHOD(char *, log_builtins_string_imp::strndup, (const char *fm,
                                                         size_t len))
{
...
*/
DEFINE_METHOD(int, log_builtins_syseventlog_imp::open,  (const char *name,
                                                         int option,
                                                         int facility))
{
  int ret= 0;
...
/**
  ...
  @retval <>0  Error
*/
DEFINE_METHOD(int, log_builtins_syseventlog_imp::write, (enum loglevel level,
                                                         const char *msg))
{
  int      ret= 0;
  mysql_mutex_lock(&THR_LOCK_log_syseventlog);
  ...
  if (hEventLog)
  {
    nchars= my_convert((char *) buff, sizeof(buff) - sizeof(buff[0]),
                       ...
                       MAX_SYSLOG_MESSAGE_SIZE, &my_charset_utf8_bin,
                       &dummy_errors);
