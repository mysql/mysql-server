...
static void dictionary_activate(set_type *dict_words)
{
  ...
  strncpy(validate_password_dictionary_file_last_parsed,
          ss.str().c_str(), ss.str().length() + 1);
...
static int validate_dictionary_check(my_h_string password)
{
  ...
  if (!(buffer= (char*) my_malloc(PSI_NOT_INSTRUMENTED,
                                  MAX_PASSWORD_LENGTH, MYF(0))))
    return (0);

  if (mysql_service_mysql_string_converter->convert_to_buffer(
                                            lower_string_handle, buffer,
                                            MAX_PASSWORD_LENGTH, "utf8"))
  {
    LogEvent().type(LOG_TYPE_ERROR)
              .prio(ERROR_LEVEL)
              .subsys("validate_password")
                          ...
                          const char *field_name)
{
  ...
  if (mysql_service_mysql_security_context_options->get(ctx, field_name, &user))
  {
...
static bool is_valid_password_by_user_name(void *thd, my_h_string password)
{
  ...
  return
    is_valid_user(ctx, buffer, length, "user")
...
static int validate_password_policy_strength(void *thd,
                                             my_h_string password, int policy)
{
  int has_digit= 0;
  int has_lower= 0;
  int has_upper= 0;
  ...
  {
    if (!is_valid_password_by_user_name(thd, password))
      return (0);
...
int register_system_variables()
{
  ...
  if (mysql_service_component_sys_variable_register->register_variable(
            "validate_password",
            "length", PLUGIN_VAR_INT | PLUGIN_VAR_RQCMDARG,
            "Password validate length to check for minimum password_length",
            NULL, length_update, (void *)&length,
  ...
  if (mysql_service_component_sys_variable_register->register_variable(
            "validate_password",
            "number_count", PLUGIN_VAR_INT | PLUGIN_VAR_RQCMDARG,
            "password validate digit to ensure minimum numeric character in "
            "password", NULL, length_update, (void *)&num_count,
  ...
  if (mysql_service_component_sys_variable_register->register_variable(
            "validate_password",
            "mixed_case_count", PLUGIN_VAR_INT | PLUGIN_VAR_RQCMDARG,
            "Password validate mixed case to ensure minimum upper/lower case "
            "in password", NULL, length_update, (void *)&mixed_case_count,
  ...
  if (mysql_service_component_sys_variable_register->register_variable(
            "validate_password",
            "special_char_count", PLUGIN_VAR_INT | PLUGIN_VAR_RQCMDARG,
  ...
  if (mysql_service_component_sys_variable_register->register_variable(
            ...
            "password", NULL, dictionary_update,
            (void *)&str_arg, (void *)&validate_password_dictionary_file))
  {
    LogEvent().type(LOG_TYPE_ERROR)
              .prio(ERROR_LEVEL)
              .subsys("validate_password")
...
static mysql_service_status_t validate_password_deinit()
{
  ...
  if (unregister_system_variables()  ||
...
BEGIN_SERVICE_IMPLEMENTATION(validate_password, validate_password)
  ...
  validate_password_imp::get_strength
...
DECLARE_LIBRARY_COMPONENTS
  &COMPONENT_REF(validate_password)
