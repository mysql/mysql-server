...
void open_log()
{
  ...
  if (!log_outfile.is_open())
    log_outfile.open("test_pfs_notification.log",
...
*/
void
thread_create_callback(int handle, const PSI_thread_attrs *thread_attrs)
{
  callback_print_log(handle, "thread_create", thread_attrs, 0);
}

void
thread_create_cb1(const PSI_thread_attrs *thread_attrs)
{
  int handle = 1;
...
void
thread_create_cb2(const PSI_thread_attrs *thread_attrs)
{
  int handle = 2;
  thread_create_callback(handle, thread_attrs);
}
...
void
thread_create_cb3(const PSI_thread_attrs *thread_attrs)
{
  int handle = 3;
  thread_create_callback(handle, thread_attrs);
}
...
*/
void
thread_destroy_callback(int handle, const PSI_thread_attrs *thread_attrs)
{
  callback_print_log(handle, "thread_destroy", thread_attrs, 0);
}

void
thread_destroy_cb1(const PSI_thread_attrs *thread_attrs)
{
  int handle = 1;
...
void
thread_destroy_cb2(const PSI_thread_attrs *thread_attrs)
{
  int handle = 2;
  thread_destroy_callback(handle, thread_attrs);
}
...
void
thread_destroy_cb3(const PSI_thread_attrs *thread_attrs)
{
  int handle = 3;
  thread_destroy_callback(handle, thread_attrs);
}
...
{
  return (user == "PFS_MTR_MODE_ENABLE"  ||
          user == "PFS_MTR_MODE_DISABLE" ||
          user == "PFS_MTR_REGISTER_INTERNAL"   ||
          user == "PFS_MTR_UNREGISTER_INTERNAL" ||
          user == "PFS_MTR_NEGAIVE_TEST_CASES"  ||
          user == "PFS_USER1" ||
          user == "PFS_USER2" ||
          user == "PFS_USER3");
}
...
*/
void
session_connect_callback(int handle, const PSI_thread_attrs *thread_attrs)
{
  ...
  */
  ...
  if (log_enabled)
  {
    ...
    */
    if (user == "PFS_MTR_REGISTER_INTERNAL")
    {
      if (!internal_registration)
      {
        ...
        internal_handle = mysql_service_pfs_notification->
                                      register_notification(&callbacks, false);
        callback_print_log(handle, "register_notification_internal", thread_attrs,
                           internal_handle);
    ...
    /*
      Test unregistering of an 'internal' registration that does not use a ref count.
      Callbacks are disabled but not completely unregistered.
    */
    if (user == "PFS_MTR_UNREGISTER_INTERNAL")
    {
      if (internal_registration)
      {
        int ret = mysql_service_pfs_notification->unregister_notification(internal_handle);
    ...
    if (user == "PFS_MTR_NEGATIVE_TEST_CASES")
    {
      ...
      std::memset(&callbacks, 0, sizeof(callbacks));
      int ret = mysql_service_pfs_notification->
                                       register_notification(&callbacks, true);
      callback_print_log(handle, "register_notification(bad_cb)",
                         thread_attrs, ret);

      /* Register w/ null callbacks. */
      ret = mysql_service_pfs_notification->
                                       register_notification(nullptr, true);
      callback_print_log(handle, "register_notification(nullptr)",
                         thread_attrs, ret);
    ...
    */
    ...
    if (mysql_service_pfs_resource_group->get_thread_system_attrs_by_id(NULL,
    ...
    std::string group (my_thread_attrs.m_groupname, my_thread_attrs.m_groupname_length);
    if (group.empty())
...
void
session_connect_cb1(const PSI_thread_attrs *thread_attrs)
{
  int handle = 1;
...
void
session_connect_cb2(const PSI_thread_attrs *thread_attrs)
{
  int handle = 2;
  session_connect_callback(handle, thread_attrs);
}
...
void
session_connect_cb3(const PSI_thread_attrs *thread_attrs)
{
  int handle = 3;
  session_connect_callback(handle, thread_attrs);
}
...
void
session_connect_internal(const PSI_thread_attrs *thread_attrs)
{
...
*/
void
session_disconnect_callback(int handle, const PSI_thread_attrs *thread_attrs)
{
  callback_print_log(handle, "session_disconnect", thread_attrs, 0);
}

void
session_disconnect_cb1(const PSI_thread_attrs *thread_attrs)
{
  int handle = 1;
...
void
session_disconnect_cb2(const PSI_thread_attrs *thread_attrs)
{
  int handle = 2;
  session_disconnect_callback(handle, thread_attrs);
}
...
void
session_disconnect_cb3(const PSI_thread_attrs *thread_attrs)
{
  int handle = 3;
  session_disconnect_callback(handle, thread_attrs);
}
...
*/
void
session_change_user_callback(int handle, const PSI_thread_attrs *thread_attrs)
{
  callback_print_log(handle, "session_change_user", thread_attrs, 0);
}

void
session_change_user_cb1(const PSI_thread_attrs *thread_attrs)
{
  int handle = 1;
...
void
session_change_user_cb2(const PSI_thread_attrs *thread_attrs)
{
  int handle = 2;
  session_change_user_callback(handle, thread_attrs);
}
...
void
session_change_user_cb3(const PSI_thread_attrs *thread_attrs)
{
  int handle = 3;
  session_change_user_callback(handle, thread_attrs);
}
