...
/**
  This file contains a test (example) component, which tests the service "test_service",
  provided by the component "service_component".
*/
...
/*      if (mysql_service_mysql_string_converter->convert_from_buffer(&out_string,
                                            ...
                                            "utf8"))
      {
        WRITE_LOG ("Length is negative for buffer in convert from buffer: failed.\n");
      }
      else
      {
        WRITE_LOG ("Length is negative for buffer in convert from buffer: passed.\n");
      }
*/
// Empty string in convert from buffer
    if (mysql_service_mysql_string_converter->convert_from_buffer(&out_string,
                                          empty_test_text, // its a input buffer
                                          strlen(empty_test_text),
                                          "utf8"))
    {
...
// NULL as in_string in get number of chars
      if (mysql_service_mysql_string_character_access->get_char_length (NULL,
          &out_length))
      {
        WRITE_LOG ("NULL as input spring in get_char_length passed.\n");
      }
// valid get number of chars
      if (mysql_service_mysql_string_character_access->get_char_length (out_string,
          &out_length))
      {
...
// NULL as in_string in get number of bytes
      if (mysql_service_mysql_string_byte_access->get_byte_length (NULL,
          &out_length))
      {
        WRITE_LOG ("NULL as input buffer in get_byte_length passed.\n");
      }
// valid get number of bytes
      if (mysql_service_mysql_string_byte_access->get_byte_length (out_string,
          &out_length))
      {
...
// NULL as input buffer in tolower
        if (mysql_service_mysql_string_case->tolower(&low_string,
            NULL))
        {
...
// NULL as input buffer in Convert string to buffer
          if (mysql_service_mysql_string_converter->convert_to_buffer(NULL,
                                             low_test_text,
                                             MAX_BUFFER_LENGTH,
                                             "utf8"))
          {
...
// Convert low string to buffer
          if (mysql_service_mysql_string_converter->convert_to_buffer(low_string,
                                             low_test_text,
                                             MAX_BUFFER_LENGTH,
                                             "utf8"))
          {
...
// NULL as input buffer in toupper
        ...
        else
        {
          ...
          if (mysql_service_mysql_string_converter->convert_to_buffer(upper_string,
                                             upper_test_text,
                                             MAX_BUFFER_LENGTH,
                                             "utf8"))
          {
...
// Get char with index 1
      ulong out_char;
      if (mysql_service_mysql_string_character_access->get_char(out_string,
                                              1,
                                              &out_char))
      {
...
// Get byte with index strlen
      uint out_byte;
      if (mysql_service_mysql_string_byte_access->get_byte(out_string,
                                              strlen(test_text)-1,
                                              &out_byte))
      {
...
// NULL as iterator in get_next
        if (mysql_service_mysql_string_iterator->iterator_get_next(NULL,
                                                        &out_iter_char))
        {
...
// NULL as iterator in is_digit
        ...
        while (mysql_service_mysql_string_iterator->iterator_get_next(out_iterator,
                                                        &out_iter_char)
              != true)
        {
...
DECLARE_LIBRARY_COMPONENTS
  &COMPONENT_REF(test_string_service)
