 ...
 */
PSI_table_handle *
continent_open_table(PSI_pos **pos)
{
 ...
 */
void
continent_close_table(PSI_table_handle *handle)
{
...
static void
copy_record(Continent_record *dest, Continent_record *source)
{
...
int
continent_rnd_next(PSI_table_handle *handle)
{
...
int
continent_rnd_init(PSI_table_handle *, bool)
{
  return 0;
}
...
int
continent_rnd_pos(PSI_table_handle *handle)
{
  Continent_Table_Handle *h = (Continent_Table_Handle *)handle;
  Continent_record *record = &continent_records_array[h->m_pos.get_index()];
...
int
continent_index_init(PSI_table_handle *handle,
                     unsigned int idx,
                     bool,
...
int
continent_index_read(PSI_index_handle *index,
                     ...
                     int find_flag)
{
  ...
  case 0:
  {
    ...
    mysql_service_pfs_plugin_table->read_key_string(reader, &i->m_name, find_flag);
  }
  break;
  default:
    DBUG_ASSERT(0);
...
int
continent_index_next(PSI_table_handle *handle)
{
  Continent_Table_Handle *h = (Continent_Table_Handle *)handle;
...
void
continent_reset_position(PSI_table_handle *handle)
{
  Continent_Table_Handle *h = (Continent_Table_Handle *)handle;
  h->m_pos.reset();
  h->m_next_pos.reset();
...
int
continent_read_column_value(PSI_table_handle *handle,
                            PSI_field *field,
                            unsigned int index)
{
  Continent_Table_Handle *h = (Continent_Table_Handle *)handle;
...
unsigned long long
continent_get_row_count(void)
{
  return continent_rows_in_table;
}

void
init_continent_share(PFS_engine_table_share_proxy *share)
{
  ...
  share->m_proxy_engine_table = {continent_rnd_next,
                                 continent_rnd_init,
                                 continent_rnd_pos,
