...
class Sasl_mechanism_kerberos : public Sasl_mechanism {
  ...
  */
  ...
  /**
    ...
    @retval false failure
   */
  virtual bool preauthenticate(const char *user, const char *password) override;
  /**
   Get LDAP host.

   @return LDAP host URL or nullptr on failure
  */
  virtual const char *get_ldap_host() override;
  ...
  /**
   ...
   @param name [out] default user name
    ...
    @retval false failure
  */
  virtual bool get_default_user(std::string &name) override;
  /**
   Gets array of SASL callbacks supported by the mechanism.

   @return array of callbacks
  */
  virtual const sasl_callback_t *get_callbacks() override { return callbacks; }
  /**
    GSSAPI authentication must be concluded by MySQL server.

    @return true
  */
  virtual bool require_conclude_by_server() override { return true; }
