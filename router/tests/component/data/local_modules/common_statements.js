 ...
 * @param {object} [options] options replacement values for statement text and
 *     response
...
exports.get =
  ...
  var statements = {
    ...
    select_port: {
      stmt: "select @@port",
      result:
          {columns: [{name: "@@port", type: "LONG"}], rows: [[options["port"]]]}
    },
    select_repeat_4097: {
    ...
    router_select_schema_version: {
      ...
      result: {
        columns: [
          {type: "LONGLONG", name: "major"}, {type: "LONGLONG", name: "minor"},
          {type: "LONGLONG", name: "patch"}
        ],
    ...
    router_select_group_membership_with_primary_mode: {
      ...
      result: {
        ...
        rows:
            options["group_replication_membership"].map(function(currentValue) {
              return [
    ...
    router_start_transaction: {"stmt": "START TRANSACTION", "ok": {}},
    ...
    router_commit: {stmt: "COMMIT", ok: {}},

    router_rollback: {stmt: "ROLLBACK", ok: {}},
    ...
    router_drop_users:
        {stmt_regex: "^DROP USER IF EXISTS 'mysql_router.*", ok: {}},
    ...
    router_count_clusters_v1: {
      ...
      result: {columns: [{"type": "LONGLONG", "name": "count(*)"}], rows: [[1]]}
    },
    ...
    router_create_user_if_not_exists: {
      "stmt_regex": "^CREATE USER IF NOT EXISTS '" +
          options.account_user_pattern + "'@'" + options.account_host_pattern +
          "' IDENTIFIED WITH mysql_native_password AS '" +
          options.account_pass_pattern + "'",
      "ok": {warning_count: options.create_user_warning_count}
    },
    // CREATE USER (without IF NOT EXISTS) is triggered by --account-create
    // always
    router_create_user: {
    ...
    // this query will only be issued if CREATE USER [IF NOT EXISTS] returned
    // warning_count > 0
