...
({
  stmts: function (stmt) {
    ...
    var metadata_version = (mysqld.global.upgrade_in_progress === 1) ? [0, 0, 0] : [1, 0, 2];
    var options = {
      ...
      if (mysqld.global.perm_error_on_version_update === 1) {
        return {
          error: {
            code: 1142,
            sql_state: "HY001",
            message: "UPDATE command denied to user 'user'@'localhost' for table 'routers'"
          }
        }
      }
      else return router_update_version_strict_v1;
    }
