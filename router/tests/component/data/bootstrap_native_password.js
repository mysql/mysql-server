...
if (mysqld.global.router_version === undefined) {
  // version is needed, but its value doesn't matter for this test.
  mysqld.global.router_version = "9.1.0";
}
...
/* check that bootstrap loads the native-password client auth plugin.
 */

({
  handshake: {
    auth: {
      username: "root",                           // as in the test
      password: "fake-pass",                      // from kRootPassword
      auth_method_name: "mysql_native_password",  // enforce native-password
    }
  },
  stmts: function(stmt) {
    var res;
