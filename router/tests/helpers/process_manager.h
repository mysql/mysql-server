...
class ProcessManager {
 ...
 protected:
  ...
  class MysqlServerMockSpawner {
   public:
    MysqlServerMockSpawner &filename(std::string fn) {
      filename_ = std::move(fn);
      return *this;
    }

    MysqlServerMockSpawner &classic_port(uint16_t port) {
      classic_port_ = port;
      return *this;
    }

    MysqlServerMockSpawner &x_port(uint16_t port) {
      x_port_ = port;
      return *this;
    }

    MysqlServerMockSpawner &http_port(uint16_t port) {
      http_port_ = port;
      return *this;
    }

    MysqlServerMockSpawner &with_ssl(bool ssl) {
      enable_ssl_ = ssl;
      return *this;
    }
    ...
    Spawner &spawner() { return spwnr_; }

    friend class ProcessManager;

   private:
    MysqlServerMockSpawner(Spawner &&spwnr) : spwnr_(std::move(spwnr)) {}

    MysqlServerMockSpawner &logging_folder(std::string logging_folder) {
      logging_folder_ = std::move(logging_folder);
    ...
    std::chrono::milliseconds wait_for_notify_ready_{std::chrono::seconds(30)};
    bool enable_ssl_{false};

    std::string logging_folder_;
  };

  MysqlServerMockSpawner mysql_server_mock_spawner() {
    MysqlServerMockSpawner spwnr = spawner(mysqlserver_mock_exec_.str());

    spwnr.logging_folder(get_test_temp_dir_name());
