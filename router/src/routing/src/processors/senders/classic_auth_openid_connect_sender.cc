...
AuthOpenidConnectSender::process() {
  switch (stage()) {
    case Stage::Init:
      return init();
    case Stage::Response:
...
AuthOpenidConnectSender::init() {
  ...
  if (auto &tr = tracer()) {
    tr.trace(Tracer::Event().stage("openid_connect::sender::password"));
  }
...
stdx::expected<Processor::Result, std::error_code>
AuthOpenidConnectSender::response() {
  // ERR|OK|EOF|other
...
stdx::expected<Processor::Result, std::error_code>
AuthOpenidConnectSender::ok() {
  stage(Stage::Done);

  if (auto &tr = tracer()) {
    tr.trace(Tracer::Event().stage("openid_connect::sender::ok"));
  }
...
stdx::expected<Processor::Result, std::error_code>
AuthOpenidConnectSender::error() {
  stage(Stage::Done);

  if (auto &tr = tracer()) {
    tr.trace(Tracer::Event().stage("openid_connect::sender::error"));
  }
