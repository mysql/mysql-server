...
TEST_F(FilterObjectsTest, complex_or_two_elements) {
  ...
  ASSERT_EQ("(( v1=1) OR( v2='a'))", sut_.get_result());
}
...
TEST_F(FilterObjectsTest, match_expression_without_modifiers) {
  sut_.parse(
      json("{\"$match\":{\"$params\":[\"c1\"], "
           "\"$against\":{\"$expr\":\"q1\"}}}"));
  EXPECT_EQ("(MATCH (`c1`) AGAINST('q1' ) )", sut_.get_result());

  sut_.parse(
      json("{\"$match\":{\"$params\":[\"c1\", \"c2\"], "
           "\"$against\":{\"$expr\":\"q1\"}}}"));
  EXPECT_EQ("(MATCH (`c1`,`c2`) AGAINST('q1' ) )", sut_.get_result());
}
