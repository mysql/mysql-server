...
QueryChangesAuthApp::QueryChangesAuthApp(const uint64_t last_audit_log_id) {
  audit_log_id_ = last_audit_log_id;
}

void QueryChangesAuthApp::query_entries(MySQLSession *session) {
...
void QueryChangesAuthApp::query_auth_entries(MySQLSession *session,
                                             Entries *out,
  ...
  if (entries.empty() && table_name == "auth_app") {
    entry::AuthApp pe;
    pe.id = id;
    pe.deleted = true;
...
std::string QueryChangesAuthApp::build_query(const std::string &table_name,
                                             const entry::UniversalId &id) {
