...
void QueryChangesDbSchema::query_entries(MySQLSession *session) {
  ...
  for (const auto &audit_entry : audit_entries.entries) {
    if (audit_entry.old_table_id.has_value())
      query_schema_entries(session, &local_entries, audit_entry.table,
...
void QueryChangesDbSchema::query_schema_entries(MySQLSession *session,
                                                VectorOfEntries *out,
  ...
  if (entries.empty() && table_name == "db_schema") {
    entry::DbSchema se;
    se.id = id;
    se.deleted = true;
...
std::string QueryChangesDbSchema::build_query(const std::string &table_name,
                                              const entry::UniversalId id) {
