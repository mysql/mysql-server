...
QueryChangesUrlHost::QueryChangesUrlHost(const uint64_t last_audit_log_id) {
  audit_log_id_ = last_audit_log_id;
}

void QueryChangesUrlHost::query_entries(MySQLSession *session) {
  ...
  for (const auto &audit_entry : audit_entries.entries) {
    if (audit_entry.old_table_id.has_value())
      query_host_entries(session, &local_entries, audit_entry.table,
...
void QueryChangesUrlHost::query_host_entries(MySQLSession *session,
                                             VectorOfEntries *out,
  ...
  if (entries.empty() && table_name == "url_host") {
    entry::UrlHost h;
    h.id = id;
    h.deleted = true;
...
std::string QueryChangesUrlHost::build_query(const std::string &table_name,
                                             const entry::UniversalId id) {
