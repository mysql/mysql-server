...
SaslData SaslHandler::get_authorize_data(RequestContext &ctxt) {
  ...
  bool has_auth_data = url.parameters_.count(kParameterAuthData);
  auto state = get_authentication_state(url.parameters_, has_auth_data);

  auto auth_data = helper::container::get_value_default(url.parameters_,
                                                        kParameterAuthData, "");
