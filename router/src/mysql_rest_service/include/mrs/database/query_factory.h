...
class QueryFactory : public mrs::interface::QueryFactory {
 public:
  std::shared_ptr<QueryChangesAuthApp> create_query_changes_auth_app(
      const uint64_t last_audit_log_id) override;
  std::shared_ptr<QueryChangesContentFile> create_query_changes_content_file(
      const uint64_t last_audit_log_id) override;
  std::shared_ptr<QueryChangesDbObject> create_query_changes_db_object(
      const uint64_t last_audit_log_id) override;
  std::shared_ptr<QueryAuditLogEntries> create_query_audit_log() override;
  std::shared_ptr<QueryEntriesAuthApp> create_query_auth_all() override;
