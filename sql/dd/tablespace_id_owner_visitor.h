...
namespace dd {
/**
   Vistor function which invokes its visitor argument on any subobject
   which holds a tablespace, that is, the table itself, its m_indexes,
   its m_leaf_partitions, and m_indexes (Partition_index) of each
   leaf_partition. Visitation is terminated when the vistor closure
   returns true.
...
bool visit_tablespace_id_owners(const Table &t, VISITOR_TYPE &&visitor) {
  ...
  for (const Partition *lp : t.leaf_partitions()) {
    if (visitor(*lp)) {
      return true;
    }
    for (const Partition_index *lpi : lp->indexes()) {
      if (visitor(*lpi)) {
        return true;
      }
    }
