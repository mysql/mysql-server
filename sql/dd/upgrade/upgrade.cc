...
{ dd_upgrade_flag= param; }

bool allow_sdi_creation()
{ return create_sdi_flag; }

void set_allow_sdi_creation(bool param)
{ create_sdi_flag= param; }
...
static void rename_stats_tables()
{
  ...
  */
  ...
  build_table_filename(to_path, sizeof(to_path) - 1, MYSQL_SCHEMA_NAME.str,
                       index_stats_backup.c_str(), IBD_EXT.c_str(),
                       0, &not_used);
  ...
  build_table_filename(to_path, sizeof(to_path) - 1, MYSQL_SCHEMA_NAME.str,
                       table_stats_backup.c_str(), IBD_EXT.c_str(),
                       0, &not_used);
...
*/
static bool ha_finish_upgrade(THD *thd,
                              plugin_ref plugin,
                              void *failed_upgrade)
{
...
{
  return (plugin_foreach(thd, ha_finish_upgrade,
...
void rename_back_stats_tables(THD *thd)
{
  ...
  /*
    ...
    error handling code.
  */
  (void) execute_query(thd, "SET GLOBAL INNODB_FAST_SHUTDOWN= 0");
...
Upgrade_status::Upgrade_status() :
  m_file(nullptr),
  m_filename("mysql_dd_upgrade_info")
{}
...
bool do_pre_checks_and_initialize_dd(THD *thd)
{
  ...
  char path[FN_REFLEN+1];
  bool not_used;
  build_table_filename(path, sizeof(path) - 1, "", "mysql",
                       ".ibd", 0, &not_used);
  bool exists_mysql_tablespace= (!my_access(path, F_OK));

  // Check existence of mysql/plugin.frm
  build_table_filename(path, sizeof(path) - 1, "mysql", "plugin",
                       ".frm", 0, &not_used);
