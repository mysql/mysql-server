...
int pathncmp(const LEX_CSTRING &a, const LEX_CSTRING &b, size_t n)
{
  ...
  return files_charset_info->coll->
...
Dir_pat_tuple
make_dir_pattern_tuple(const LEX_STRING &path, const LEX_CSTRING &schema_name)
{
  ...
  if (test_if_hard_path(path.str))
  {
    ...
    return { dd::String_type{dirname, dirname_len},
        std::move(fpat), is_in_datadir };
  }
                        ...
                        dd::sdi_file::Paths_type *paths)
{
  ...
  for (uint i= 0; i < dir->number_off_files; ++i)
  {
    bool match= false;
    if (!lower_case_file_system)
      match= (my_wildcmp_mb_bin(files_charset_info,
                   dir->dir_entry[i].name,
                   dir->dir_entry[i].name + strlen(dir->dir_entry[i].name),
                   pat_beg, pat_end, '\\', '?', '*') == 0);
    else
      match= (my_wildcmp(files_charset_info,
                   dir->dir_entry[i].name,
                   dir->dir_entry[i].name + strlen(dir->dir_entry[i].name),
                   pat_beg, pat_end, '\\', '?', '*') == 0);
    if (match)
...
{
  return checked_return(write_sdi_file(sdi_filename(table.id(), table.name(),
                                                    schema.name()), sdi));
}
          ...
          dd::String_type *buf)
{
  ...
  if (mysql_file_read(sdi_fd, sdi_buf, buf->size(), MYF(MY_FAE|MY_WME|MY_NABP)))
  {
    return true;
  }
  return false;
                            ...
                            const dd::String_type &table_name)
{
  char path[FN_REFLEN+1];
  bool was_truncated= false;
  size_t plen= build_table_filename(path, sizeof(path) - 1, schema_name.c_str(),
                                    table_name.c_str(),
                                    ".MYD", 0, &was_truncated);
