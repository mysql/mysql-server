...
*/
bool DD_properties::unchecked_get(THD *thd,
                                  const String_type &key,
                                  String_type *value,
                                  bool *exists)
{
  if (init_cached_properties(thd))
    return true;

  *exists= m_properties->exists(key);
  if (*exists)
    m_properties->get(key, *value);
...
*/
bool DD_properties::unchecked_set(THD *thd,
                                  const String_type &key,
...
bool DD_properties::get(THD *thd,
                        const String_type &key,
                        uint *value,
                        bool *exists)
{
...
bool DD_properties::set(THD *thd,
                        const String_type &key,
                        uint value)
{
  DBUG_ASSERT(m_property_desc.find(key) != m_property_desc.end() &&
                        ...
                        bool *exists)
{
  ...
  if (unchecked_get(thd, key, &property_string, exists))
    return true;
