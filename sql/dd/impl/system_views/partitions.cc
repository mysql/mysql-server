...
#include "dd/impl/system_views/partitions.h"
...
Partitions::Partitions()
{
  ...
  m_target_def.add_field(FIELD_PARTITION_NAME, "PARTITION_NAME",
                         "part.name");
  ...
  m_target_def.add_field(FIELD_PARTITION_DESCRIPTION,
                         "PARTITION_DESCRIPTION", "part.description_utf8");
  ...
  m_target_def.add_field(FIELD_NODEGROUP, "NODEGROUP",
  ...
  m_target_def.add_from("JOIN mysql.catalogs cat "
                        "ON cat.id=sch.catalog_id");
  m_target_def.add_from("LEFT JOIN mysql.table_partitions part ON "
                        "part.table_id=tbl.id");
  m_target_def.add_from("LEFT JOIN mysql.table_partitions sub_part "
                        "ON sub_part.parent_partition_id=part.id");
  m_target_def.add_from("LEFT JOIN mysql.tablespaces part_ts "
                        "ON part_ts.id=part.tablespace_id");
  m_target_def.add_from("LEFT JOIN mysql.tablespaces sub_part_ts "
