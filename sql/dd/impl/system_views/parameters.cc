...
#include "dd/impl/system_views/parameters.h"
...
Parameters::Parameters()
{
  ...
  m_target_def.add_field(FIELD_SPECIFIC_CATALOG,
                         "SPECIFIC_CATALOG", "cat.name");
  ...
  m_target_def.add_field(FIELD_ORDINAL_POSITION, "ORDINAL_POSITION",
   "IF (rtn.type = 'FUNCTION', prm.ordinal_position-1, prm.ordinal_position)");
  m_target_def.add_field(FIELD_PARAMETER_MODE, "PARAMETER_MODE",
    "IF (rtn.type = 'FUNCTION' AND prm.ordinal_position = 1, NULL, prm.mode)");
  m_target_def.add_field(FIELD_PARAMETER_NAME, "PARAMETER_NAME",
    "IF (rtn.type = 'FUNCTION' AND prm.ordinal_position = 1, NULL, prm.name)");
  ...
  m_target_def.add_from("mysql.parameters prm JOIN mysql.routines rtn "
