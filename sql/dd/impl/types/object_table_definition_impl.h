...
private:
  ...
  static const char* key(Label label)
  {
    switch (label)
    {
    case Label::NAME:         return "name";
    case Label::FIELDS:       return "fields";
    case Label::INDEXES:      return "indexes";
    case Label::FOREIGN_KEYS: return "foreign_keys";
    case Label::OPTIONS:      return "options";
    case Label::LABEL:        return "lbl";
    case Label::POSITION:     return "pos";
    case Label::DEFINITION:   return "def";
    case Label::ELEMENT:      return "elem";
    default:
      DBUG_ASSERT(false);
      return "";
    }
  ...
  {
    DBUG_ASSERT(
      element_numbers != nullptr && element_definitions != nullptr &&
      element_numbers->find(element_name) == element_numbers->end() &&
      element_definitions->find(element_number) == element_definitions->end());
...
public:
  ...
  {
    add_element(field_number, field_name, field_definition,
                &m_field_numbers, &m_field_definitions);
  }

  virtual void add_index(int index_number, const String_type &index_name,
                         const String_type &index_definition)
  {
    add_element(index_number, index_name, index_definition,
                &m_index_numbers, &m_index_definitions);
  }
  ...
  virtual String_type get_ddl() const
  {
    ...
    */
    if (!m_ddl_statement.empty())
      return m_ddl_statement;
    ...
    // Output indexes
    for (auto index: m_index_definitions)
      ss << ",\n  " << index.second;

    // Output foreign keys
    for (auto key: m_foreign_key_definitions)
      ss << ",\n  " << key.second;

    ss << "\n)";

    // Output options
    for (auto option: m_option_definitions)
      ss << " " << option.second;
  ...
  virtual void store_into_properties(Properties *table_def_properties) const
  {
    ...
    table_def_properties->set(key(Label::FOREIGN_KEYS),
                              fk_props->raw_string());
    delete fk_props;
  ...
  virtual bool restore_from_properties(const Properties &table_def_properties)
  {
    ...
    if (table_def_properties.get(key(Label::NAME), m_table_name) ||
        ...
        set_element_properties(property_str,
                               &m_field_numbers, &m_field_definitions) ||
