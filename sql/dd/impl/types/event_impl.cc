...
bool Event_impl::restore_attributes(const Raw_record &r)
{
  ...
  m_event_status=
    (enum_event_status) r.read_int(Events::FIELD_STATUS);
  m_is_event_status_null= r.is_null(Events::FIELD_STATUS);

  m_on_completion=
    (enum_on_completion) r.read_int(Events::FIELD_ON_COMPLETION);
...
bool Event_impl::store_attributes(Raw_record *r)
{
  ...
  return store_id(r, Events::FIELD_ID) ||
         store_name(r, Events::FIELD_NAME) ||
         r->store(Events::FIELD_NAME, name()) ||
         r->store(Events::FIELD_INTERVAL_FIELD,
                  m_interval_field,
         ...
         r->store(Events::FIELD_INTERVAL_VALUE,
                  m_interval_value,
                  m_is_interval_value_null) ||
         r->store(Events::FIELD_TIME_ZONE, m_time_zone) ||
         r->store_time(Events::FIELD_EXECUTE_AT,
                       m_execute_at,
                       m_is_execute_at_null) ||
         r->store_time(Events::FIELD_STARTS,
                       m_starts,
                       m_is_starts_null) ||
         r->store_time(Events::FIELD_ENDS,
                       m_ends,
                       m_is_ends_null) ||
         r->store_time(Events::FIELD_LAST_EXECUTED,
                       m_last_executed,
         ...
         r->store(Events::FIELD_CONNECTION_COLLATION_ID, m_connection_collation_id) ||
