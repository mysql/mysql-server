...
{
  DBUG_EXECUTE_IF("sdi_import_commit_fail",
    return (m_in_datadir &&
            mysql_file_delete(key_file_sdi, "no_such_file",
                              MYF(MY_FAE)));
  );
  return (m_in_datadir &&
          mysql_file_delete(key_file_sdi, m_tmp_sdi_filename.c_str(),
                            MYF(MY_FAE)));
}
...
bool Import_target::load(THD *thd, String_type *shared_buffer)
{
  ...
  {
    m_table_object->set_name(casedn(dd_charset_info,
                                    m_table_object->name()));
    m_schema_name_in_sdi= casedn(dd_charset_info,
                                 std::move(m_schema_name_in_sdi));
  }

  if (lower_case_table_names == 2)
  {
    m_lc_tname.reset(new dd::String_type(casedn(dd_charset_info,
                                               m_table_object->name())));
    m_lc_sname.reset(new dd::String_type(casedn(dd_charset_info,
                                                m_schema_name_in_sdi)));
  }
  ...
  if (mysql_file_rename(key_file_sdi, m_path.c_str(),
                        tmpname.c_str(), MYF(MY_FAE)))
  {
    return true;
  }
...
//                     TL_IGNORE);
  ...
  if (update_referencing_views_metadata(thd, &tl, false,
                                        &uncommitted_tables))
  {
    return true;
  }
...
//                     table_name, TL_IGNORE);
  ...
  // the table.
  bool create_access_denied= check_access(thd, CREATE_ACL, tl.db,
                                          &tl.grant.privilege,
  ...
  bool create_grant_denied= check_grant(thd, CREATE_ACL, &tl,
                                        false /* any_combination_will_do */,
                                        1/* number */,
                                        false /* no_errors */);
