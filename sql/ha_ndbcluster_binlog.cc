                             ...
                             const char *new_db, const char *new_table_name)
{
  ...
  NDB_SCHEMA_OBJECT *ndb_schema_object;
  {
    ...
    {
      /**
       * Try to trigger a race between late incomming slock ack for schema operations
       * having its coordinator on another node, which we would otherwise have discarded
       * as no matching ndb_schema_object existed,
       * and another schema op with same 'key', coordinated by this node.
       * Thus causing a mixup betweeen these, and the schema distribution getting
  ...
  */
           ...
           ndb_latest_handled_binlog_epoch < ndb_get_latest_trans_gci()))
  {
    ...
    *root_ptr= &mem_root;
    ...
    if (unlikely(s_pOp != NULL && s_pOp->getGCI() == current_epoch))
    {
      ...
      while (s_pOp != NULL && s_pOp->getGCI() == current_epoch)
      {
        if (!s_pOp->hasError())
        {
     ...
     */
    else if ((i_pOp != NULL && i_pOp->getGCI() == current_epoch) ||
              ...
              current_epoch > ndb_latest_handled_binlog_epoch))
    {
      ...
      if (i_pOp == NULL || i_pOp->getGCI() != current_epoch)
      {
        /*
      ...
      {
        assert(i_pOp != NULL && i_pOp->getGCI() == current_epoch);
        rows= &_row;
        ...
        do
        {
          ...
          i_pOp = i_ndb->nextEvent();
        } while (i_pOp && i_pOp->getGCI() == current_epoch);
