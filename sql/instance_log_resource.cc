...
int MY_ATTRIBUTE((visibility("default"))) Instance_log_resource::
...
bool Instance_log_resource_mi_wrapper::collect_info() {
  ...
  if (!error) json_channels->append_clone(&json_channel);
...
bool Instance_log_resource_binlog_wrapper::collect_info() {
  ...
  if (binlog->is_open()) {
    ...
    if (!error) json_master->add_clone("binary_log_position", &json_log_pos);
  }
  return error;
};
...
Instance_log_resource *Instance_log_resource_factory::get_wrapper(
    Master_info *mi, Json_dom *json) {
...
Instance_log_resource *Instance_log_resource_factory::get_wrapper(
    Gtid_state *gtid_state, Json_dom *json) {
