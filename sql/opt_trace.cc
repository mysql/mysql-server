...
class Opt_trace_stmt {
  ...
  */
  ...
  void set_missing_unstructured_bytes(size_t num_bytes) {
    m_missing_unstructured_bytes = num_bytes;
  }
  ...
  */
  ...
  size_t indentation() const { return stack_of_current_structs.size() * 2; }
  ...
  */
  ...
  /// Trace bytes that were discarded by UnstructuredTrace because they
  /// would exceed optimizer_trace_max_mem_size.
...
/// Add an escape sequence to 'buffer' if 'c' is a character that should be
/// escaped in optimizer trace.
/// @param c The character to escape.
/// @param buffer The buffer in which to put the escape sequence.
/// @returns The next free position in in 'buffer'.
static char *Escape(char c, char *buffer) {
  ...
  if (esc != 0)  // Escaping with backslash.
  {
    ...
    *pbuf++ = esc;
    ...
    uint ascii_code = static_cast<uint>(c);
...
void Opt_trace_stmt::next_line() {
  ...
  size_t to_be_printed = indentation();
...
void Opt_trace_stmt::fill_info(Opt_trace_info *info) const {
  if (unlikely(info->missing_priv = missing_priv)) {
    ...
    info->missing_bytes =
        ...
        // Multiply with a fudge factor to compensate for indentation,
        // adding quotes etc..
        static_cast<size_t>(1.08 * m_missing_unstructured_bytes);
  }
}
...
void Opt_trace_context::ConsumeUnstructuredTrace(THD *thd) {
  ...
  const Opt_trace_object trace_wrapper{this};
  Opt_trace_array join_optimizer{this, "join_optimizer"};
  Mem_root_array<char> line{thd->mem_root};

  unstructured_trace()->contents().Consume([&](char ch) {
    if (ch == '\n') {
  ...
  pimpl->current_stmt_in_gen->set_missing_unstructured_bytes(
      unstructured_trace()->contents().excess_bytes());
}
