                                    ...
                                    my_off_t log_pos)
{
  ...
  if (m_rli->is_mts_in_group() || m_rli->is_in_group())
    DBUG_RETURN(false);
  ...
  if (m_log_names_cmp_result == LOG_NAMES_CMP_UNKNOWN)
  {
    ...
    */
    if (log_name == NULL || strcmp("", log_name) == 0)
      DBUG_RETURN(false);
    ...
    if (strncmp(basename, m_until_log_name, (int)(q-basename)) == 0)
    {
      ...
      if (log_name_extension < m_until_log_name_extension)
        ...
        m_log_names_cmp_result=
          ...
          LOG_NAMES_CMP_GREATER : LOG_NAMES_CMP_EQUAL ;
    }
    else
    {
...
bool Until_before_gtids::check_after_dispatching_event()
{
  return false;
}
...
bool Until_mts_gap::check_at_start_slave()
{
  return false;
}
...
bool Until_mts_gap::check_after_dispatching_event()
{
  return false;
}
