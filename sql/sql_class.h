...
public:
  Semijoin_mat_exec(bool is_scan, TABLE_LIST *const emb_sj_nest,
                    uint table_count, uint field_count)
    :is_scan(is_scan), emb_sj_nest(emb_sj_nest),
    table_count(table_count), field_count(field_count), 
    table_index(-1), subq_exprs(), table_param(), table(NULL), mat_fields(NULL)
  {}
private:
...
public:
  ...
  const uint field_count;       ///< Number of fields in temporary table
  int table_index;              ///< Index into join_tab array for inner tables
  List<Item> subq_exprs;        ///< List of expressions describing temp. table
