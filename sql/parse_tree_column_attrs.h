...
public:
  explicit
  PT_comment_column_attr(const LEX_STRING &comment) : comment(comment) {}
  ...
  virtual bool contextualize(Column_parse_context *pc)
  {
    if (pc->is_generated)
    {
      my_error(ER_WRONG_USAGE, MYF(0),
               "SERIAL DEFAULT VALUE", "generated column");
      return true;
    }
    return super::contextualize(pc);
  }
...
public:
  explicit
  PT_column_format_column_attr(column_format_type format) : format(format) {}
...
public:
  explicit
  PT_storage_media_column_attr(ha_storage_media media) : media(media) {}
...
public:
  ...
  explicit
  PT_bit_type(const char *length) : PT_type(MYSQL_TYPE_BIT), length(length) {}
...
public:
  ...
  {}
  virtual ulong get_type_flags() const { return force_binary ? BINCMP_FLAG : 0;}
  virtual const char *get_length() const { return length; }
...
public:
  virtual bool contextualize(Parse_context *pc)
  {
    if (super::contextualize(pc) || type_node->contextualize(pc))
      return true;
