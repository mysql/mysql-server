...
bool initialize_monitor_log()
{
  ...
  DWORD dw_types = (EVENTLOG_ERROR_TYPE | EVENTLOG_WARNING_TYPE |
...
{
  return getenv("MYSQLD_PARENT_PID");
}
...
static HANDLE get_service_status_pipe_in_mysqld()
{
  ...
  while (1)
  {
    client_service_status_pipe = CreateFile(service_status_pipe_name,
                                            GENERIC_WRITE, 0, nullptr,
                                            OPEN_EXISTING,
                                            FILE_READ_ATTRIBUTES, nullptr);
    ...
    if (!WaitNamedPipe(service_status_pipe_name, 20000))
      return nullptr;
  }
}
...
static bool setup_service_status_pipe_in_monitor()
{
  ...
  service_status_pipe = CreateNamedPipe(service_status_pipe_name,
...
static void close_service_status_pipe_in_monitor()
{
  ...
  if (service_status_cmd_event)
    CloseHandle(service_status_cmd_event);
}
...
bool send_service_status(const Service_status_msg &msg)
{
    ...
    if (get_service_status_pipe_in_mysqld() == nullptr)
        return true;
...
static bool get_and_set_service_status()
{
    ...
    ReadFile(service_status_pipe, &service_msg, sizeof(service_msg),
             &bytes_read, nullptr);
    ...
    if (bytes_read != 0)
    {
        ...
        case  'T':
            ...
            ulong slow_start_timeout= strtoul(&service_msg.service_msg()[3],
                                              nullptr, 0);
...
static int monitor_mysqld(LPSTR cmd_line)
{
  ...
  */
  ...
  res= CreateProcess(path_name, cmd_line, nullptr, nullptr, FALSE,
                     0, env_block.get(), cwd, &si, &pi);
  ...
  if (is_windows_service())
  {
    while (true)
    {
      DWORD rv = WaitForMultipleObjects(NUM_WAIT_HANDLES,
                                        (HANDLE *)event_handles, FALSE,
                                        INFINITE);
      if (rv == WAIT_FAILED)
      {
        monitor_log_msg(Monitor_log_msg_type::MONITOR_LOG_ERROR,
...
bool is_early_option(int argc, char **argv)
{
  for (int index= 1; index < argc; index++)
  {
    ...
    {
      if (is_early_short_option(cur_arg[1]))
        return true;
      cur_arg+= 2; // Skip --
      if (*cur_arg != '\0' && is_early_option(cur_arg))
        return true;
    }
  }
  return false;
...
static void set_event_names()
{
  ...
  snprintf(service_status_cmd_processed_event_name,
           sizeof(service_status_cmd_processed_event_name),
           "mysqld%d_srvp", my_monitor_pid);
...
bool setup_service_status_cmd_processed_handle()
{
  ...
  if (my_security_attr_create(&service_status_cmd_processed_sec_attr,
                              &errmsg, GENERIC_ALL,
...
static bool initialize_events()
{
  ...
  if (my_security_attr_create(&service_status_cmd_sec_attr,
                              &errmsg, GENERIC_ALL,
                              SYNCHRONIZE | EVENT_MODIFY_STATE))
  {
    monitor_log_msg(Monitor_log_msg_type::MONITOR_LOG_ERROR,
  ...
  service_status_cmd_event=
    CreateEvent(service_status_cmd_sec_attr, FALSE, FALSE,
...
int start_monitor()
{
  ...
  for (int i = 0; i < argc_tmp; i++)
  {
    ...
    if ((i == 2 || i == 1) &&
        get_win_service_ptr()->IsService(arg.c_str()))
      break;
  ...
  if (is_windows_service())
    get_win_service_ptr()->SetExitEvent();
