...
#include "sql/gis/discrete_frechet_distance_functor.h"
...
namespace gis {
Discrete_frechet_distance::Discrete_frechet_distance(double major, double minor)
    : m_geographic_strategy(
          new bgsd::geographic<boost::geometry::strategy::andoyer,
                               bgs::spheroid<double>>(
              bgs::spheroid<double>(major, minor))) {}

double Discrete_frechet_distance::operator()(const Geometry *g1,
                                             const Geometry *g2) const {
  return apply(*this, g1, g2);
}

double Discrete_frechet_distance::eval(const Geometry *g1,
                                       const Geometry *g2) const {
  throw not_implemented_exception::for_non_projected(*g1, *g2);
}

double Discrete_frechet_distance::eval(const Geographic_linestring *g1,
                                       const Geographic_linestring *g2) const {
  return bg::discrete_frechet_distance(*g1, *g2, *m_geographic_strategy);
}

double Discrete_frechet_distance::eval(const Cartesian_linestring *g1,
...
bool discrete_frechet_distance(const dd::Spatial_reference_system *srs,
  ...
  try {
    ...
    Discrete_frechet_distance dfd(srs ? srs->semi_major_axis() : 0.0,
                                  srs ? srs->semi_minor_axis() : 0.0);
    *discrete_frechet_distance = dfd(g1, g2);
  } catch (...) {
    handle_gis_exception("st_discrete_frechet_distance");
    return true;
  }

  if (!std::isfinite(*discrete_frechet_distance) ||
      *discrete_frechet_distance < 0.0) {
    my_error(ER_DATA_OUT_OF_RANGE, MYF(0), "Discrete_frechet_distance",
