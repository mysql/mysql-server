...
static bool write_length_and_string(String *to, const String &from)
{
  ...
  DBUG_EXECUTE_IF("binlog_corrupt_write_length_and_string_truncate_before_string",
                  {
                    DBUG_SET("-d,binlog_corrupt_write_length_and_string_truncate_before_string");
                    return false;
                  });
...
bool Json_diff::write_binary(String *to) const
{
  ...
  */
  ...
  if (return_early)
    DBUG_RETURN(false);
#endif
  DBUG_PRINT("info", ("wrote JSON path '%s' of length %lu",
                      buf.ptr(), (unsigned long) buf.length()));
  ...
  if (m_operation != enum_json_diff_operation::REMOVE)
  {
    ...
    if (value().to_binary(current_thd, &buf) ||
      ...
      DBUG_RETURN(true); /* purecov: inspected */ // OOM, error is reported
    DBUG_PRINT("info", ("wrote JSON value of length %lu",
...
bool Json_diff_vector::read_binary(const char **from,
                                   const TABLE *table, const char *field_name)
{
  ...
  while (length)
  {
    ...
    if (operation_number >= JSON_DIFF_OPERATION_COUNT)
      goto corrupted;
    enum_json_diff_operation operation=
    ...
    if (parse_path(false, path_length, pointer_cast<const char *>(p),
                   &path, &bad_index))
      goto corrupted;
    p+= path_length;
    length-= path_length;
    ...
    if (operation != enum_json_diff_operation::REMOVE)
    {
      ...
      json_binary::Value value=
        json_binary::parse_binary(pointer_cast<const char *>(p), value_length);
