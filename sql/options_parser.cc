...
struct result
{
  ...
  */
  ...
  /**
    Information about the specific error that occured.
  */
  std::string msg;
  /**
                            ...
                            std::map<std::string, std::string> *options)
{
  ...
  size_t delimiter_count= std::count(str_copy.begin(),
                                     str_copy.end(),
                                     delimiter);
  ...
  for (size_t i= 0; i <= delimiter_count; i++)
  {
    ...
    else
    {
      ...
      substring_endpos-= 1; // Decrement to set end position at position before
                            // delimiter.
    }
    ...
    if (key_value_separator_count == 0)
    {
      ...
      parser_result.msg= str_copy.substr(substring_startpos,
                                        substring_endpos-substring_startpos+1);
      parser_result.c= key_value_separator;
      return parser_result;
    }

    key_value_separator_pos= str_copy.find(key_value_separator,
                                           substring_startpos);
    ...
    std::string key= str_copy.substr(key_startpos,
                                     key_endpos - key_startpos + 1);
    std::string value= str_copy.substr(value_startpos,
                                       value_endpos-value_startpos + 1);
