...
static PasswdValue check_user(const char *user) {
  ...
  if (user_id) {
    if (user) {
      ...
      tmp_user_info = my_getpwnam(user);
      if ((tmp_user_info.is_void() || user_id != tmp_user_info.pw_uid))
  ...
  if (!strcmp(user, "root"))
    return tmp_user_info;  // Avoid problem with dynamic libraries

  tmp_user_info = my_getpwnam(user);
  if (tmp_user_info.is_void()) {
    ...
    if (tmp_user_info.is_void()) goto err;
  }
  return tmp_user_info;
  /* purecov: end */
...
#ifndef _WIN32
    if (!user_info.is_void()) set_user(mysqld_user, user_info);
#endif
  } else
#endif
    locked_in_memory = 0;
