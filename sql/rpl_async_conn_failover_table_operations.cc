...
Rpl_async_conn_failover_table_operations::insert_row(
  ...
  std::string err_msg{
      "Error executing asynchronous_connection_failover_add_primary() UDF "
      "function."};
  ...
  fields = table->field;
  std::tie(weight, channel, host, port, network_namespace) = master_conn_detail;

  /* Store channel */
  if (store_field(fields[0], channel)) {
    err_msg.assign(
        "Error saving channel field of "
        "asynchronous_connection_failover_add_primary().");
    return std::make_tuple(true, err_msg);
  }
  ...
  err_msg.assign(
      "The UDF asynchronous_connection_failover_add_primary()"
...
Rpl_async_conn_failover_table_operations::delete_row(
  ...
  std::string err_msg{
      "Error executing asynchronous_connection_failover_delete_primary() UDF "
      "function."};
  ...
  */
  ...
  if (store_field(fields[0], channel)) {
    err_msg.assign(
        "Error saving channel field of "
        "asynchronous_connection_failover_delete_primary().");
    return std::make_tuple(true, err_msg);
  }
  ...
  err_msg.assign(
      "The UDF asynchronous_connection_failover_delete_primary() executed "
      "successfully.");
  return std::make_tuple(false, err_msg);
}
...
Rpl_async_conn_failover_table_operations::read_rows(std::string channel_name) {
  ...
  Rpl_sys_table_access table_op;
  if (table_op.init(m_db, m_table, m_num_field, m_lock_type)) {
    return std::make_tuple(true, master_conn_detail);
  }
  ...
  if (!key_access.init(table)) {
    do {
      /* get master connection detail */
      master_conn_detail.push_back(get_data(table));
    } while (!key_access.next());
  }

  auto error = key_access.deinit() || table_op.deinit();
  if (!error) {
    std::sort(master_conn_detail.begin(), master_conn_detail.end(),
              [](auto const &t1, auto const &t2) { return t1 > t2; });
  }

  return std::make_tuple(error, master_conn_detail);
}
...
Rpl_async_conn_failover_table_operations::read_random_rows_pos(
  ...
  if (!key_access.init(table, pos)) {
    if (!key_access.next()) {
      /* get master connection detail */
      master_conn_detail = get_data(table);
    }
  }
