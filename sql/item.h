...
private:
  char *m_str;
  uint m_length;
  bool m_is_autogenerated; /* indicate was name of this Item
...
public:
  ...
  */
  ...
  {
    m_is_autogenerated= is_autogenerated; // Must be done before set()
    copy(str, length, cs);
  }
...
public:
  ...
  {
    return item_name.length() ? item_name.ptr() : "???";
  }
...
inline void Item_sp_variable::make_field(Send_field *field)
{
  ...
  if (item_name.length())
    it->item_name.copy(item_name.ptr(), (uint) /*strlen(name)*/ item_name.length());
  else
...
public:
  ...
  int save_in_field(Field *field, bool no_conversions);
  bool basic_const_item() const { return 1; }
  Item *clone_item() { return new Item_int(item_name.ptr(),value,max_length); }
  virtual void print(String *str, enum_query_type query_type);
