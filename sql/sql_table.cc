...
public:
    ...
    : m_thd(thd)
  {
    ...
    if (if_exists)
      m_built_query.append("DROP TABLE IF EXISTS ");
    else
  ...
  void add_array(const Prealloced_array<TABLE_LIST *, 1> &tables)
  {
    ...
    {
      append_table_ident(m_thd, &m_built_query, table, false);
      ...
      if (thd->locked_tables_mode)
      {
        ...
        */
        ...
        /*
          ...
          Note that we won't try to reopen tables those removal will be
          later rolled back thanks to some error. Such situations should
          be fairly rare.
        */
        table->table= 0;
      }
  ...
  if (tmp_non_trans_tables.size() != 0)
  {
    ...
    */
    ...
    if (mysql_bin_log.is_open())
    {
      ...
      /*
        The single purpose of the below hack is to generate GTID for the DROP
       ...
       nonexistent_tables.size() != 0))
  {
    ...
    */
    ...
    if (mysql_bin_log.is_open())
    {
      ...
      /*
        See the rationale for the below hack with "commit" above.
      */
      if (gtid_and_table_groups_state != GTID_SINGLE_TABLE_GROUP)
      {
        /*
