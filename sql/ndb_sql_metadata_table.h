...
#include <cstdint>
#include <string>
#include "sql/ndb_record_layout.h"
...
public:
  ...
  void getType(const char *buf,
               unsigned short *a)           { layout().getValue(buf, 0, a);    }
  void getName(const char *buf,
               size_t *a, const char **b)   { layout().getValue(buf, 1, a, b); }
  void getSeq(const char *buf,
              unsigned short *a)            { layout().getValue(buf, 2, a);    }
  /* Getter for nullable column returns bool; true = NOT NULL */
  bool getNote(const char *buf,
               uint32_t *a)          { return layout().getValue(buf, 3, a);    }
  void getSql(const char *buf,
              size_t *a, const char **b)    { layout().getValue(buf, 4, a, b); }
