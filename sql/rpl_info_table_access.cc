...
*/
bool Rpl_info_table_access::open_table(THD* thd, const char *dbstr, const char *tbstr,
...
bool Rpl_info_table_access::find_info_id(uint idx, LEX_STRING id, TABLE *table)
{
  ...
  if (!table->key_info ||
      DBUG_EVALUATE_IF("fault_injection_Rpl_info_table_access::find_id_info", 1, 0))
    DBUG_RETURN(TRUE);
