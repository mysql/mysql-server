...
std::vector<const Edge*> Dijkstra::operator()(const int& start_point_id, const int& end_point_id, double& total_cost, const std::function<bool()>& stop) {
    m_point_map.clear();
    point_heap.clear();
    int point = start_point_id; // node id
    Point& node = m_point_map[point] = Point{ 0, /*m_heu(point)*/ 0, nullptr };
    // #iterations, for callback func stop().
    // incremented once for every node popped.
    ...
    while (point != end_point_id) {
      ...
      if (++iters % iters_per_callback == 0 && stop())
          return {};
    }
