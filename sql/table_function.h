...
public:
  ...
  /**
    ...
    @returns
      ...
      false on success
  */
  bool create_result_table(ulonglong options,
...
enum class enum_jt_column { JTC_ORDINALITY, JTC_PATH, JTC_EXISTS,
                            JTC_NESTED_PATH };
...
enum class enum_jtc_on : uint16 {
    JTO_ERROR, JTO_NULL, JTO_DEFAULT, JTO_IMPLICIT };
...
public:
  ...
  */
  ...
  JT_data_source():
    v(key_memory_JSON), producing_records(false)
  {}
  ~JT_data_source() {};
...
public:
  Json_table_column(enum_jt_column type)
    : m_jtc_type(type), m_jds_elt(nullptr), m_child_jds_elt(nullptr),
    m_next_nested(nullptr), m_prev_nested(nullptr), m_field_idx(-1)
  {}
  Json_table_column(enum_jt_column col_type,
  ...
  Json_table_column(LEX_STRING path, List<Json_table_column> *cols)
    ...
    m_nested_columns(cols), m_path_str(path)
  {}
  void cleanup();
