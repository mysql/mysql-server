  ...
  */
  ...
  /* @todo: Print it for all the channels */
  {
    ...
    if (default_mi && default_mi->rli) {
      DBUG_PRINT("info",
                 ("init group source %s %lu  group relay %s %lu event %s %lu\n",
...
err:
  ...
  channel_map.unlock();
  if (error) LogErr(INFORMATION_LEVEL, ER_REPLICA_NOT_STARTED_ON_SOME_CHANNELS);
  ...
  for (uint i = 0;
    ...
    if ((versions_for_all_bugs[i].bug_id == bug_id) &&
      ...
      my_printf_error(ER_UNKNOWN_ERROR,
                      "source may suffer from"
                      " http://bugs.mysql.com/bug.php?id=%u"
                      " so replica stops; check error log on replica"
                      " for more info",
                      MYF(0), bug_id);
      ...
      if (report_level != INFORMATION_LEVEL)
        rli->report(report_level, ER_SERVER_UNKNOWN_ERROR,
                    "According to the source's version ('%s'),"
                    " it is probable that source suffers from this bug:"
                    " http://bugs.mysql.com/bug.php?id=%u"
                    " and thus replicating the current binary log event"
                    " may make the replica's data become different from the"
                    " source's data."
                    " To take no risk, replica refuses to replicate"
                    " this event and stops."
                    " We recommend that all updates be stopped on the"
                    " source and replica, that the data of both be"
                    " manually synchronized,"
                    " that source's binary logs be deleted,"
                    " that source be upgraded to a version at least"
