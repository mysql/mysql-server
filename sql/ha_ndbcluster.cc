                                 ...
                                 key_part_map keypart_map)
{
  ...
  if (result == NdbQuery::NextResult_gotRow)
  {
    ...
    assert(key_cmp_if_same(table, key, active_index,
  ...
  */
  ...
  if (cursor)
  {
    ...
    */
    ...
    {
       m_blobs_pending |= ndb_table_has_blobs(m_table);
...
void ha_ndbcluster::update_comment_info(THD *thd, HA_CREATE_INFO *create_info,
  ...
  if (create_info->comment.str == NULL) {
...
int ndbcluster_init(void* p)
{
  ...
  if (unlikely(opt_initialize))
  {
    ...
    assert(!ha_storage_engine_is_enabled(static_cast<handlerton*>(p)));
...
int
Ndb_util_thread::do_init()
{
  mysql_mutex_init(PSI_INSTRUMENT_ME, &LOCK, MY_MUTEX_INIT_FAST);
