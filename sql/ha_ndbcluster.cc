...
int ha_ndbcluster::update_row(const byte *old_data, byte *new_data)
{
  ...
  */
  ...
  if (cursor)
  {
    /* For a scan, we unly need to execute() if the batch buffer is full. */
                       ...
                       MY_BITMAP *defined, byte *buf)
{
  ...
  /*
    ToDo: Changed this to set unused null bits to 1 (used to set them to 0).
    Otherwise replication breaks, as it uses memcmp() on the null bytes to
    compare records, and NdbRecord (correctly) sets unused bits to 1.
    I think this is the same bug that recently caused replication between NDB
    and myisam to fail. So merging that fix should be the proper fix.
    See BUG#26591.
  */
                                      ...
                                      HANDLER_BUFFER *buffer)
{
  ...
  */
  ...
  if (num_scan_ranges > 0)
  {
    ...
    if (generate_scan_filter(m_cond_stack, scanOp))
