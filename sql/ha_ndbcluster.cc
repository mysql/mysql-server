...
int ha_ndbcluster::write_row(uchar *record)
{
  ...
  if (unlikely(m_slow_path))
  {
    ...
    */
    ...
    else if (m_thd_ndb->trans_options & TNTO_NO_LOGGING)
...
int ha_ndbcluster::init_handler_for_statement(THD *thd, Thd_ndb *thd_ndb)
{
  /*
    ...
    pointer to point to the NDB transaction context.
   */
                                      ...
                                      HANDLER_BUFFER *buffer)
{
       ...
       multi_range_curr++)
  {
    ...
    case PRIMARY_KEY_INDEX:
    {
      ...
      if ((op= m_thd_ndb->trans->getNdbOperation(tab)) && 
          !op->readTuple(lm) && 
        ...
        ERR_RETURN(op ? op->getNdbError() : m_thd_ndb->trans->getNdbError());
      break;
    }
    break;
    case UNIQUE_ORDERED_INDEX:
    ...
    {
      multi_range_curr->range_flag |= UNIQUE_RANGE;
      if ((op= m_thd_ndb->trans->getNdbIndexOperation(unique_idx, tab)) && 
          !op->readTuple(lm) && 
  ...
  range:
      ...
      if (scanOp == 0)
      {
        ...
        else if ((scanOp= m_thd_ndb->trans->getNdbIndexScanOperation(idx, tab)) 
  ...
  if (!(res= execute_no_commit_ie(this, m_thd_ndb->trans, true)))
  {
...
ha_ndbcluster::read_multi_range_next(KEY_MULTI_RANGE ** multi_range_found_p)
{
  ...
  for (;multi_range_curr < m_multi_range_defined; multi_range_curr++)
  {
    ...
    if (multi_range_curr->range_flag & UNIQUE_RANGE)
    {
      ...
      op= m_thd_ndb->trans->getNextCompletedOperation(op);
