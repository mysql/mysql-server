...
void get_date_from_daynr(long daynr, uint *ret_year, uint *ret_month,
  ...
  if (daynr <= 365L || daynr >= 3652500) { /* Fix if wrong daynr */
    ...
    *ret_month = 1;
    for (month_pos = days_in_month; day_of_year > (uint)*month_pos;
...
static uint to_ascii(const CHARSET_INFO *cs, const char *src, size_t src_length,
  ...
  while (dst < dstend &&
         (cnvres = (cs->cset->mb_wc)(cs, &wc, (const uchar *)src,
  ...
  switch (int_type) {
    ...
    case INTERVAL_DAY_HOUR: {
      ...
      if (interval.second > MAX_DAY_NUMBER * 24ULL * 60ULL * 60ULL)
        goto invalid_date;
      sec = ((ltime->day - 1) * 3600LL * 24LL + ltime->hour * 3600LL +
      ...
      if ((ulonglong)daynr > MAX_DAY_NUMBER) goto invalid_date;
    ...
    case INTERVAL_WEEK: {
      ...
      get_date_from_daynr((long)period, &ltime->year, &ltime->month,
                          &ltime->day);
    } break;
    case INTERVAL_YEAR:
      if (interval.year > 10000UL) goto invalid_date;
      ltime->year += sign * (long)interval.year;
      if ((ulong)ltime->year >= 10000L) goto invalid_date;
    ...
    case INTERVAL_MONTH: {
      ...
      period =
          (ltime->year * 12ULL +
           sign * (unsigned long long)interval.year * 12ULL + ltime->month -
           1ULL + sign * (unsigned long long)interval.month);
  ...
  */
  if (l_time1->time_type == MYSQL_TIMESTAMP_TIME)  // Time value
    ...
    if (l_time2->time_type == MYSQL_TIMESTAMP_TIME)
      ...
      days -= l_sign * calc_daynr((uint)l_time2->year, (uint)l_time2->month,
                                  (uint)l_time2->day);
  }

  microseconds =
      ((longlong)days * SECONDS_IN_24H +
       (longlong)(l_time1->hour * 3600L + l_time1->minute * 60L +
                  l_time1->second) -
       l_sign * (longlong)(l_time2->hour * 3600L + l_time2->minute * 60L +
                           l_time2->second)) *
          1000000LL +
      (longlong)l_time1->second_part - l_sign * (longlong)l_time2->second_part;
...
int my_time_compare(const MYSQL_TIME *a, const MYSQL_TIME *b) {
  ...
  if (a->second_part < b->second_part) return -1;
  if (a->second_part > b->second_part) return 1;
...
/* Rounding functions */
static uint msec_round_add[7] = {500000000, 50000000, 5000000, 500000,
                                 50000,     5000,     0};
