...
public:
  ...
  ~Table_upgrade_guard()
  {
    ...
    if (m_table->s->m_part_info)
      free_items(m_table->s->m_part_info->item_list);
    ...
    */
    if (m_table->s->field)
    {
      ...
      {
        if ((*ptr)->gcol_info)
          free_items((*ptr)->gcol_info->item_list);
      }
    }
                         ...
                         bool is_fix_view_cols_and_deps)
{
  ...
  */
  ...
  if (error)
  {
    if (error == HA_ADMIN_NEEDS_DUMP_UPGRADE)
      ndb_log_error("Table upgrade required for "
  ...
  */
  ...
  if (mysql_prepare_create_table(thd, schema_name.c_str(), table_name.c_str(),
                                 ...
                                 file, true, // NDB tables are auto-partitoned.
