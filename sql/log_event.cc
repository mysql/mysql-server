  ...
  switch (binlog_version) {
    ...
    case 4:
      ...
      */
      ...
      /*
         Now the older formats; in that case load_tmpdir is cleaned up by the
         I/O thread.
      */
    case 1:
      if (strncmp(rli->get_rli_description_event()->server_version, "3.23.57",
                  7) >= 0 &&
          created) {
        /*
...
uint Load_log_event::get_query_buffer_length() {
  return
      ...
      5 + 2 * db_len + 3 + 18 + fname_len * 4 +
      2 +                        // "LOAD DATA INFILE 'file''"
      11 +                       // "CONCURRENT "
      7 +                        // LOCAL
      9 +                        // " REPLACE or IGNORE "
...
void Load_log_event::print_query(bool need_db, const char *cs, char *buf,
...
#ifdef MYSQL_SERVER
    quoted_id_len =
        my_strmov_quoted_identifier(this->thd, (char *)quoted_id, db, 0);
#else
  ...
  if (is_concurrent) pos = my_stpcpy(pos, "CONCURRENT ");

  if (fn_start) *fn_start = pos;

  if (check_fname_outside_temp_buf()) pos = my_stpcpy(pos, "LOCAL ");
  ...
  if (num_fields) {
    ...
    for (i = 0; i < num_fields; i++) {
      ...
      quoted_id_len =
          my_strmov_quoted_identifier(this->thd, quoted_id, field, 0);
