...
Rpl_info_table::Rpl_info_table(uint nparam, uint param_field_id,
                               const char* param_schema,
                               const char *param_table)
:Rpl_info_handler(nparam), field_id(param_field_id), use_default(FALSE)
{
...
int Rpl_info_table::do_init_info()
{
  ...
  */
  longlong2str(server_id, field_values->field[field_id].use.str, 10);
  field_values->field[field_id].use.length= strlen(field_values->field[field_id].use.str);
  if (!access->find_info_id(field_id, field_values->field[field_id].use, table))
  {
    /*
...
int Rpl_info_table::do_flush_info(const bool force)
{
  ...
  /*
    We only update the rpl_info table if we are in a top-level statement.
    If we are inside a stored function, we do not do it.
  */
...
int Rpl_info_table::do_prepare_info_for_write()
{
  ...
  cursor= 1;
  for (int pos= 0; field_values && pos < ninfo; pos++)
    field_values->field[pos].use.str= field_values->field[pos].saved.str;
...
bool Rpl_info_table::do_set_info(const int pos, const float value)
{
  ...
  int needed_size= sizeof(value) * 4;
  /*
