...
String *Item_func_get_dd_table_private_data::val_str(String *str)
{
  ...
  if ((option_ptr=args[0]->val_str(&option)) != nullptr)
  {
	...
	if (strcmp(args[1]->val_str(&option)->ptr(), "autoinc") == 0)
	{
      if (p->exists("autoinc"))
      {
	...
	if (strcmp(args[1]->val_str(&option)->ptr(), "data_directory") == 0)
	{
      if (p->exists("data_directory"))
      {
        dd::String_type opt_value;
        p->get("data_directory", opt_value);
        if (!opt_value.empty())
        {
          if (opt_value.size() > 7)
            opt_value.erase(7, dd::String_type::npos);
	...
	if (strcmp(args[1]->val_str(&option)->ptr(), "version") == 0)
	{
      if (p->exists("version"))
      {
        p->get_uint32("version", &opt_value);
        ptr = longlong10_to_str(opt_value, ptr, 10);
      }
    }
