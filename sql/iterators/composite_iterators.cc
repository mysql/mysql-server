...
bool MaterializeIterator<Profiler>::MaterializeQueryBlock(
  ...
  uchar *const set_counter_0 =
      t->m_set_counter != nullptr ? t->m_set_counter->field_ptr() : nullptr;
  uchar *const set_counter_1 = t->m_set_counter != nullptr
  ...
  auto read_counter = [t, set_counter_0, set_counter_1]() -> ulonglong {
    ...
    t->m_set_counter->set_field_ptr(set_counter_1);
    ulonglong cnt = static_cast<ulonglong>(t->m_set_counter->val_int());
    t->m_set_counter->set_field_ptr(set_counter_0);
    return cnt;
  };
  ...
  const bool is_union = table()->is_union();

  while (true) {
    // For EXCEPT and INTERSECT we test LIMIT in ScanTableIterator
    if (is_union && *stored_rows >= m_limit_rows) break;
    ...
    if (is_union) {
    ...
    } else if (query_block.block_no == 0) {
      //
      ...
      if (t->m_except) {
        ...
        if (check_unique_constraint(t)) {
          // counter := 1
          t->m_set_counter->store(1, true);
          // next, go on to write the row
        } else {
          // counter := counter + 1
          t->m_set_counter->store(read_counter() + 1, true);
        ...
        if (t->m_last_operation_is_distinct) {
          //
          ...
          if (check_unique_constraint(t)) {
            ...
            t->m_set_counter->store(query_block.total_blocks - 1, true);
            // next, go on to write the row
          } else {
          ...
          if (check_unique_constraint(t)) {
            ...
            // left side counter := 1
            c[0] = 1;
            t->m_set_counter->store(c.value(), true);
            // go on to write the row
          } else {
            HalfCounter c(read_counter());
            if (static_cast<uint64_t>(c[0]) + 1 >
                std::numeric_limits<uint32_t>::max()) {
              my_error(ER_INTERSECT_ALL_MAX_DUPS_EXCEEDED, MYF(0));
              return true;
            }
      ...
      if (t->m_except) {
        ...
        if (cnt > 0) {
          if (query_block.block_no < query_block.first_distinct)
            // counter := counter - 1
            t->m_set_counter->store(cnt - 1, true);
          else
            t->m_set_counter->store(0, true);
          error = t->file->ha_update_row(t->record[1], t->record[0]);
          if (!t->file->is_ignorable_error(error) &&
              spill_to_disk_and_retry_update_row(thd(), error))
            return true;
        ...
        if (t->m_last_operation_is_distinct) {
          ...
          // leave it behind.
          ulonglong cnt = read_counter();
          if (cnt == query_block.total_blocks - query_block.block_no) {
            // counter -:= 1
            cnt = cnt - 1;
            t->m_set_counter->store(cnt, true);
            error = t->file->ha_update_row(t->record[1], t->record[0]);
            if (!t->file->is_ignorable_error(error) &&
                spill_to_disk_and_retry_update_row(thd(), error))
              return true;
    ...
    if (!t->file->is_ignorable_error(error)) {
      ...
      if (!is_duplicate && (t->is_union() || query_block.block_no == 0))
        ++*stored_rows;
