...
uint Item::time_precision()
{
  if (const_item() && result_type() == STRING_RESULT && !is_temporal())
  {
    ...
    if ((tmp= val_str(&buf)) &&
      ...
      return min(status.fractional_digits, DATETIME_MAX_DECIMALS);
  }
  return min(decimals, DATETIME_MAX_DECIMALS);
}
...
{ 
  ...
  case TIME_VALUE:
  {
    ...
    str->length((uint) my_TIME_to_str(&value.time, (char *) str->ptr(),
                                      min(decimals, DATETIME_MAX_DECIMALS)));
...
Field *Item::tmp_table_field_from_field_type(TABLE *table, bool fixed_length)
{
  ...
  */
  ...
  case MYSQL_TYPE_TIME:
    ...
    break;
  case MYSQL_TYPE_TIMESTAMP:
    field= new Field_timestampf(maybe_null, name, decimals);
    break;
  case MYSQL_TYPE_DATETIME:
