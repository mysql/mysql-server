...
bool Secondary_engine::run_load_statements(char *statement, MYSQL *mysql,
                                           Errors expected_errors) {
  // Check if the statement is a CREATE TABLE statement or a DDL statement
  if ((expected_errors.size() == 0 &&
       match_create_statement(statement, &m_table_name)) ||
      match_ddl_statement(statement, &m_table_name)) {
    ...
    if (match_truncate_statement(statement, &m_table_name) ||
        (!m_change_propagation &&
...
bool Secondary_engine::run_unload_statements(char *statement, MYSQL *mysql,
                                             Errors expected_errors) {
