...
static const std::string c_name("thread_pool plugin"),
    opt_name("mysql_option_tracker_option"),
    c_option_name("Enterprise Thread Pool");
typedef weak_service_reference<SERVICE_TYPE(mysql_option_tracker_option),
                               c_name, opt_name>
    weak_option;
unsigned long long opt_option_tracker_usage_thread_pool_plugin = 0;
static bool cb(unsigned long long new_value) {
  opt_option_tracker_usage_thread_pool_plugin = new_value;
  return false;
}
static bool cb_define_failed = false;

bool thread_pool_plugin_option_usage_init(SERVICE_TYPE(registry) * reg,
                                          SERVICE_TYPE(registry_registration) *
                                              reg_reg) {
  /*
    Thread pool is not supposed to be dynamically loadable.
    Do not keep a reference to the option tracker from it.
  */
  return weak_option::init(
      reg, reg_reg,
      [&](SERVICE_TYPE(mysql_option_tracker_option) * opt) {
        bool const ret =
            0 != opt->define(c_option_name.c_str(), c_name.c_str(), 1) ||
            option_usage_read_counter(
                c_option_name.c_str(),
                &opt_option_tracker_usage_thread_pool_plugin, reg_srv) ||
            (cb_define_failed = option_usage_register_callback(
                 c_option_name.c_str(), cb, reg_srv));
        // thread pool is immeditately being used directly after loading it.
        ++opt_option_tracker_usage_thread_pool_plugin;
        return ret;
      },
      false);
}
bool thread_pool_plugin_option_usage_deinit(
    SERVICE_TYPE(registry) * reg,
    SERVICE_TYPE(registry_registration) * reg_reg) {
  return weak_option::deinit(
      reg, reg_reg, [&](SERVICE_TYPE(mysql_option_tracker_option) * opt) {
