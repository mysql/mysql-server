...
Client::Client(std::shared_ptr<xpl::iface::Vio> connection,
    ...
    : m_client_id(client_id),
      m_server(server),
      m_idle_reporting(std::make_unique<Client_idle_reporting>(this, nullptr)),
      m_connection(connection),
      m_config(std::make_shared<ngs::Protocol_config>(m_server->get_config())),
      m_dispatcher(this),
...
xpl::Capabilities_configurator *Client::capabilities_configurator() {
  ...
  handlers.push_back(ngs::allocate_shared<xpl::Capability_tls>(this));
  handlers.push_back(ngs::allocate_shared<xpl::Capability_auth_mech>(this));

  handlers.push_back(ngs::allocate_shared<xpl::Capability_readonly_value>(
      "doc.formats", "text"));

  handlers.push_back(
      ngs::allocate_shared<xpl::Capability_client_interactive>(this));
  ...
  handlers.push_back(ngs::allocate_shared<Cap_handles_expired_passwords>(this));
...
void Client::get_capabilities(const Mysqlx::Connection::CapabilitiesGet &) {
  ngs::Memory_instrumented<xpl::Capabilities_configurator>::Unique_ptr
      configurator(capabilities_configurator());
...
void Client::set_capabilities(
  ...
  ngs::Memory_instrumented<xpl::Capabilities_configurator>::Unique_ptr
      configurator(capabilities_configurator());
  ngs::Error_code error_code = configurator->prepare_set(setcap.capabilities());
...
void Client::handle_message(ngs::Message_request *request) {
  auto s(session());
...
void Client::on_accept() {
  ...
  set_encoder(ngs::allocate_object<ngs::Protocol_encoder>(
      m_connection,
...
void Client::on_session_auth_success(xpl::iface::Session *) {
  ...
  if (ngs::Compression_algorithm::k_none != m_cached_compression_algorithm) {
    ngs::Compression_style style = m_cached_combine_msg
                                       ? ngs::Compression_style::k_group
                                       : ngs::Compression_style::k_multiple;

    if (m_cached_max_msg == 1) {
      style = ngs::Compression_style::k_single;
    }
...
ngs::Error_code Client::read_one_message_and_dispatch() {
  DBUG_TRACE;
...
void Client::run() {
  try {
    ...
    while (m_state != State::k_closing && m_session) {
      ngs::Error_code error = read_one_message_and_dispatch();
...
ngs::Protocol_encoder_compression *
Client::get_protocol_compression_or_install_it() {
  if (!m_is_compression_encoder_injected) {
    m_is_compression_encoder_injected = true;
    auto encoder = ngs::allocate_object<ngs::Protocol_encoder_compression>(
  ...
  return reinterpret_cast<ngs::Protocol_encoder_compression *>(m_encoder.get());
}

void Client::configure_compression_opts(
    const ngs::Compression_algorithm algo, const int64_t max_msg,
    const bool combine, const xpl::Optional_value<int64_t> &level) {
...
bool Client::create_session() {
  ...
  if (error) {
    log_warning(ER_XPLUGIN_FAILED_TO_INITIALIZE_SESSION, client_id(),
                error.message.c_str());
    error.severity = ngs::Error_code::FATAL;
...
int32_t Client::get_adjusted_compression_level(
  ...
  switch (algo) {
    ...
    case ngs::Compression_algorithm::k_none:  // fall-through
    default: {
    }
  }
  return 1;
...
void Client::queue_up_disconnection_notice(const ngs::Error_code &error) {
  auto notice = std::make_shared<ngs::Notice_descriptor>(
...
void Client::queue_up_disconnection_notice_if_necessary() {
  if (State::k_running == m_state_when_reason_changed) {
    switch (m_close_reason) {
      case Close_reason::k_server_shutdown:
        queue_up_disconnection_notice(ngs::SQLError(ER_SERVER_SHUTDOWN));
        break;
      case Close_reason::k_kill:
        if (m_session->get_status_variables().m_fatal_errors_sent.load() == 0)
          queue_up_disconnection_notice(ngs::SQLError(ER_SESSION_WAS_KILLED));
        break;
      default: {
      }
    }
...
std::string Client::resolve_hostname() const {
  std::string result;
...
bool Client::is_localhost(const char *hostname) const {
