...
my_socket Connection_vio::get_socket_id() { return m_vio->get_fd(); }

ssize_t Connection_vio::write(const Const_buffer_sequence &data,
                              long write_timeout) {
  ssize_t c = 0;

  for (Const_buffer_sequence::const_iterator it = data.begin();
       it != data.end(); ++it) {
...
Connection_type Connection_vio::connection_type() {
  if (options()->active_tls()) return Connection_tls;
...
bool Ssl_context::activate_tls(Connection_vio &conn,
  ...
  conn.m_options_session =
      IOptions_session_ptr(ngs::allocate_shared<Options_session_ssl>(vio));
