...
int test_snprintf()
{
  ...
  my_snprintf(buffer, sizeof(buffer),
              "Starting test of my_snprintf in test services.\n");
  ...
  my_snprintf(buffer, sizeof(buffer),
              "Shutting down test of my_snprintf in test services.\n");
  my_write(outfile, (uchar*) buffer, strlen(buffer), MYF(0));
...
/* Test of service: my_plugin_log_message */
  /* Log the value of the switch in mysqld.err. */
  ret = my_plugin_log_message(&p, MY_INFORMATION_LEVEL, "Test services with_log_message_val: %d", 
