...
{
public:
  Stats() : ptr_group_name(NULL), view_id(0), node_id(0),
            number_of_nodes(0),
    ...
    {};
  void reset(char *new_group_name)
  {
    set_group_name(new_group_name);
    view_id= 0;
    node_id= 0;
    number_of_nodes= 0;
  ...
  void      set_group_name(char * arg) { ptr_group_name= arg; };
  char*     get_group_name() { return ptr_group_name; };
  longlong get_view_id() { return view_id; };
  // Todo: to index the requested node
  longlong get_node_id() { return node_id; };
  // Todo: to define formally node address
  // <type> get_node_address(uint index) { return node_id; };
  longlong get_number_of_nodes() { return number_of_nodes; };
  ...
  ulonglong get_min_message_length() { return min_message_length; };
  ulonglong get_max_message_length() { return max_message_length; };
...
private:
  ...
  */
  ...
  ulonglong node_id;      // to mean the local node
  ulonglong number_of_nodes;
  ulonglong total_messages_sent;
  ulonglong total_bytes_sent;
  ulonglong total_messages_received;
  ulonglong total_bytes_received;
  ulonglong min_message_length;
  ulonglong max_message_length;
