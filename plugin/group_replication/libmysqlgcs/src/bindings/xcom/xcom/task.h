...
#define ADD_T_EV(when, file, state, what) \
  add_task_event(when, file, state, what)
#define ADD_WAIT_EV(when, file, state, what, milli) \
  add_wait_event(when, file, state, what, milli)
#else
#define ADD_EVENTS(x)
...
#define TASK_BEGIN                                                             \
  ...
  ADD_EVENTS(if (stack->sp->state) {                                           \
    ...
    add_event(int_arg(stack->sp->state));                                      \
             ...
             add_event(void_arg(stack)); add_event(string_arg(stack->name));); \
