 ...
 */
PSI_table_handle*
machine_open_table(PSI_pos** pos)
{
 ...
 */
void
machine_close_table(PSI_table_handle* handle)
{
...
static void
copy_record(Machine_Record* dest, Machine_Record* source)
{
...
int
machine_rnd_next(PSI_table_handle* handle)
{
...
int
machine_rnd_init(PSI_table_handle* h MY_ATTRIBUTE((unused)),
...
int
machine_rnd_pos(PSI_table_handle* handle)
{
  Machine_Table_Handle* h = (Machine_Table_Handle*)handle;
...
int
machine_index_init(PSI_table_handle* handle MY_ATTRIBUTE((unused)),
...
int
machine_index_read(PSI_index_handle* index MY_ATTRIBUTE((unused)),
...
int
machine_index_next(PSI_table_handle* handle MY_ATTRIBUTE((unused)))
{
  /* Do nothing as there are no index */
  return PFS_HA_ERR_END_OF_FILE;
}
...
void
machine_reset_position(PSI_table_handle* handle)
{
  Machine_Table_Handle* h = (Machine_Table_Handle*)handle;
  h->m_pos.reset();
  h->m_next_pos.reset();
...
int
machine_read_column_value(PSI_table_handle* handle,
                          PSI_field* field,
                          uint index)
{
  Machine_Table_Handle* h = (Machine_Table_Handle*)handle;
  ...
  case 2: /* MACHINE_MADE */
    table_svc->set_field_char_utf8(
      field, h->current_row.machine_made, h->current_row.machine_made_length);
    break;
...
int
machine_write_row_values(PSI_table_handle* handle)
{
  Machine_Table_Handle* h = (Machine_Table_Handle*)handle;
  bool found = false;
  ...
  if (!found)
    machine_records_vector.push_back(h->current_row);
...
int
machine_write_column_value(PSI_table_handle* handle,
                           PSI_field* field,
                           unsigned int index)
{
  Machine_Table_Handle* h = (Machine_Table_Handle*)handle;
...
int
machine_update_row_values(PSI_table_handle* handle)
{
  Machine_Table_Handle* h = (Machine_Table_Handle*)handle;
...
int
machine_update_column_value(PSI_table_handle* handle,
                            PSI_field* field,
                            unsigned int index)
{
  Machine_Table_Handle* h = (Machine_Table_Handle*)handle;
...
int
machine_delete_row_values(PSI_table_handle* handle)
{
  Machine_Table_Handle* h = (Machine_Table_Handle*)handle;
...
unsigned long long
machine_get_row_count(void)
{
  return machine_rows_in_table;
}

void
init_machine_share(PFS_engine_table_share_proxy* share)
{
  ...
  share->m_table_definition =
    ...
    "enum('LAPTOP','DESKTOP','MOBILE'), MACHINE_MADE char(20), EMPLOYEE_NUMBER "
    "INTEGER";
