...
/**
  ...
  his/her own search to make sure duplicate values are not inserted for
...
*/
static bool
is_duplicate(Ename_Record *record, int skip_index)
{
  for (int i = 0; i < EMPLOYEEE_NAME_MAX_ROWS; i++)
  {
    ...
    if (!temp->m_exist || i == skip_index)
      continue;
    if (temp->e_number.val == record->e_number.val)
      return true;
  }
 ...
 */
void
ename_close_table(PSI_table_handle *handle)
{
...
static void
copy_record(Ename_Record *dest, Ename_Record *source)
{
...
int
ename_rnd_next(PSI_table_handle *handle)
{
...
int
ename_rnd_init(PSI_table_handle *h MY_ATTRIBUTE((unused)),
...
int
ename_rnd_pos(PSI_table_handle *handle)
{
  Ename_Table_Handle *h = (Ename_Table_Handle *)handle;
  Ename_Record *record = &ename_records_array[h->m_pos.get_index()];
...
int
ename_index_init(PSI_table_handle *handle,
                 uint idx,
...
int
ename_index_read(PSI_index_handle *index,
...
int
ename_index_next(PSI_table_handle *handle)
{
  Ename_Table_Handle *h = (Ename_Table_Handle *)handle;
  Ename_index *i = NULL;
...
void
ename_reset_position(PSI_table_handle *handle)
{
  Ename_Table_Handle *h = (Ename_Table_Handle *)handle;
  h->m_pos.reset();
  h->m_next_pos.reset();
...
int
ename_read_column_value(PSI_table_handle *handle, PSI_field *field, uint index)
{
  Ename_Table_Handle *h = (Ename_Table_Handle *)handle;
  ...
  case 1: /* FIRST_NAME */
    ...
    break;
  case 2: /* LAST_NAME */
    table_svc->set_field_varchar_utf8_len(
...
int
ename_write_row_values(PSI_table_handle *handle)
{
  Ename_Table_Handle *h = (Ename_Table_Handle *)handle;
...
int
ename_write_column_value(PSI_table_handle *handle,
                         PSI_field *field,
                         unsigned int index)
{
  Ename_Table_Handle *h = (Ename_Table_Handle *)handle;
...
int
ename_update_row_values(PSI_table_handle *handle)
{
  int result = 0;
  Ename_Table_Handle *h = (Ename_Table_Handle *)handle;
...
int
ename_update_column_value(PSI_table_handle *handle,
                          PSI_field *field,
                          unsigned int index)
{
  Ename_Table_Handle *h = (Ename_Table_Handle *)handle;
...
int
ename_delete_row_values(PSI_table_handle *handle)
{
  Ename_Table_Handle *h = (Ename_Table_Handle *)handle;
...
unsigned long long
ename_get_row_count(void)
{
  return ename_rows_in_table;
}

void
init_ename_share(PFS_engine_table_share_proxy *share)
{
  share->m_table_name = "pfs_example_employee_name";
  share->m_table_name_length = 25;
  share->m_table_definition =
    "EMPLOYEE_NUMBER INTEGER, FIRST_NAME char(20), LAST_NAME varchar(20), PRIMARY "
