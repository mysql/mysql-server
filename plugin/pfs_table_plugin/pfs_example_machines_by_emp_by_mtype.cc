 ...
 */
PSI_table_handle*
m_by_emp_by_mtype_open_table(PSI_pos** pos)
{
 ...
 */
void
m_by_emp_by_mtype_close_table(PSI_table_handle* handle)
{
...
static void
reset_record(M_by_emp_by_mtype_record* record)
{
...
static void
make_record(M_by_emp_by_mtype_record* record,
            Ename_Record* e_record,
            Machine_Record* m_record)
{
  record->count.val = 1;
  record->count.is_null = false;
...
int
m_by_emp_by_mtype_rnd_next(PSI_table_handle* handle)
{
...
int
m_by_emp_by_mtype_rnd_init(PSI_table_handle* h MY_ATTRIBUTE((unused)),
...
int
m_by_emp_by_mtype_rnd_pos(PSI_table_handle* handle)
{
  M_by_emp_by_mtype_Table_Handle* h = (M_by_emp_by_mtype_Table_Handle*)handle;
  Ename_Record* e_record = &ename_records_array[h->m_pos.m_index_1];
...
int
m_by_emp_by_mtype_index_init(PSI_table_handle* handle MY_ATTRIBUTE((unused)),
...
int
m_by_emp_by_mtype_index_read(PSI_index_handle* index MY_ATTRIBUTE((unused)),
...
int
m_by_emp_by_mtype_index_next(PSI_table_handle* handle MY_ATTRIBUTE((unused)))
{
  /* No Index */
  return 0;
}
...
void
m_by_emp_by_mtype_reset_position(PSI_table_handle* handle)
{
  M_by_emp_by_mtype_Table_Handle* h = (M_by_emp_by_mtype_Table_Handle*)handle;
  h->m_pos.reset();
  h->m_next_pos.reset();
...
int
m_by_emp_by_mtype_read_column_value(PSI_table_handle* handle,
                                    PSI_field* field,
                                    uint index)
{
  M_by_emp_by_mtype_Table_Handle* h = (M_by_emp_by_mtype_Table_Handle*)handle;
  ...
  {
  case 0: /* FIRST_NAME */
    table_svc->set_field_char_utf8(
      field, h->current_row.f_name, h->current_row.f_name_length);
    break;
  case 1: /* LAST_NAME */
    table_svc->set_field_char_utf8(
      field, h->current_row.l_name, h->current_row.l_name_length);
    break;
  case 2: /* MACHINE_TYPE */
...
unsigned long long
m_by_emp_by_mtype_get_row_count(void)
{
  return 0;
}

void
init_m_by_emp_by_mtype_share(PFS_engine_table_share_proxy* share)
{
