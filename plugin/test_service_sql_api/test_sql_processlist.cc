...
static void exec_test_cmd(MYSQL_SESSION session, const char *test_cmd, void *p, void * ctx)
{
  ...
  if (fail)
    my_plugin_log_message(&p, MY_ERROR_LEVEL, "test_sql_processlist: ret code: %d", fail);
  else
  {
    if (pctx->num_cols)
      get_data_str(pctx);
...
static void test_sql(void *p)
{
  ...
  WRITE_STR("Session 1 : \n");
  WRITE_STR("-----------------------------------------------------------------\n");
  exec_test_cmd(session_1, "/*Session_1*/ SELECT user,db,command,info FROM information_schema.processlist WHERE info LIKE 'PLUGIN%' order by id", p, plugin_ctx);
  ...
  WRITE_STR("Session 2 : \n");
  WRITE_STR("-----------------------------------------------------------------\n");
  exec_test_cmd(session_2, "/*Session_2*/ SELECT user,db,command,info FROM information_schema.processlist WHERE info LIKE 'PLUGIN%' order by id", p, plugin_ctx);
