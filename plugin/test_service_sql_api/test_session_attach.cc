...
{
public:
  Plugin_context(void *plugin_handle)
  ...
  {
    m_logger.print_to_file(
        Conversions::to_string(args...));
  }

  void separator()
  {
    log_test_line(m_separator);
  }
...
static MYSQL_THDVAR_INT(var_int, PLUGIN_VAR_OPCMDARG,
      "Test variable",
      nullptr,
      nullptr,
      0, 0, 2147483, 0);
...
static void handle_log_error(void *, uint sql_errno, const char * err_msg,
                             const char *)
{
  plugin_context->log_test_line("SQL execution failed with ", sql_errno, " error and message: ", err_msg);
}
...
static void exec_test_cmd(MYSQL_SESSION session, const char *test_cmd)
{
  ...
  const bool failed = 0 != command_service_run_command(
      session,
      COM_QUERY,
      &cmd,
...
static void test_sql()
{
  ...
  for (uint i = 0; i < number_of_sessions; i++)
  {
    ...
    if (1 == session_id_text.length())
      session_id_text.insert(0, "0");
  ...
  for (int i = 0; i < number_of_sessions; ++i)
  {
    ...
    const int session_index = number_of_sessions -
        ((i + session_offset) % number_of_sessions) - 1;
    ...
    if (expected_session_variable_value(session_index) != THDVAR(session_thd, var_int))
    {
      plugin_context->log_test_line("Verify Session ", i + 1, " variable failed, actual value is ", THDVAR(session_thd, var_int));
...
{
  struct test_thread_context *thread_context = (struct test_thread_context*) param;
...
{
  plugin_context->separator();
  plugin_context->log_test_line("Test in a server thread. Attach must fail on non srv_session thread.");
...
static struct st_mysql_sys_var* plugin_system_variables[] = {
  MYSQL_SYSVAR(var_int),
  nullptr
};
