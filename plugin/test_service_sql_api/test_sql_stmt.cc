...
public:
  Column(const char *db_name, const char *table_name,
         ...
         const char *org_col_name, unsigned long length,
         unsigned int charsetnr, unsigned int flags, unsigned int decimals,
         enum_field_types type)
  : db_name(db_name), table_name(table_name), org_table_name(org_table_name),
    col_name(col_name), org_col_name(org_col_name), length(length),
    charsetnr(charsetnr), flags(flags), decimals(decimals), type(type)
  {}
  ...
  void dump_column_meta()
  {
    ...
    if (flags)
      WRITE_VAL(" (%s)", fieldflags2str(flags));
    WRITE_STR("\n");
...
public:
  ...
  Server_context()
    : current_col(0), current_row(0), server_status(0),
      warn_count(0), affected_rows(0), last_insert_id(0), sql_errno(0)
  {
    err_msg.clear();
    sqlstate.clear();
    message.clear();
...
static int
handle_end_column_metadata(void *pctx, uint server_status, uint warn_count)
{
  char buffer[STRING_BUFFER_SIZE];
  Server_context *ctx= (Server_context*) pctx;
...
static int handle_end_row(void *pctx)
{
  ...
  if (ctx->cmd == COM_STMT_PREPARE &&
      ctx->current_row == 0 &&
      ctx->tables.size() == 1 &&
      ctx->tables[0].columns.size() == 4 &&
...
static const char*
test_decimal_as_string(char *buff, const decimal_t *val, int *length)
{
  if (!val)
    return "NULL";
...
static void
handle_ok(void *pctx, uint server_status, uint statement_warn_count,
...
static void handle_error(void *pctx, uint sql_errno,
...
static void test_6(MYSQL_SESSION session, void *p)
{
  ...
  set_query_in_com_data(&cmd, "CREATE TABLE t3(a1 INT, a2 CHAR(32), "
                              "a3 DOUBLE(4, 2), a4 DECIMAL(3, 1))");
  run_cmd(session, COM_QUERY, &cmd, &ctx, false, p);

  set_query_in_com_data(&cmd, "CREATE TABLE t4(b0 INT, b1 INT, b2 CHAR(32), "
                              "b3 DOUBLE(4, 2), b4 DECIMAL(3, 1))");
  run_cmd(session, COM_QUERY, &cmd, &ctx, false, p);

  set_query_in_com_data(&cmd, "INSERT INTO t3 VALUES"
                        "(1, '11', 12.34, 56.7), "
                        "(2, '12', 56.78, 90.1), "
                        "(3, '13', 23.45, 67.8)");
  run_cmd(session, COM_QUERY, &cmd, &ctx, false, p);

  set_query_in_com_data(&cmd, "INSERT INTO t4 VALUES"
