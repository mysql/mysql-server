...
static const char *log_filename= "test_sql_views_triggers";
...
static struct st_test_statement test_query_plan[]=
{
  ...
  /* View */
  {"test", true , "SELECT * FROM v1" },
  { NULL , true , "SELECT VIEW_DEFINITION FROM INFORMATION_SCHEMA.VIEWS WHERE TABLE_SCHEMA = 'test' AND TABLE_NAME = 'v1'" },
  {"test", false, "DROP VIEW v1" },
  {"test", false, "CREATE VIEW v1 AS SELECT * FROM t1" },
  {"test", true , "SELECT * FROM v1" },
  /* Trigger */
  {"test", false, "UPDATE t1 SET c2 = 'abcde' WHERE c1 = 1" },
  {"test", true , "SELECT * FROM t2" },
  { NULL , true , "SELECT TRIGGER_NAME FROM INFORMATION_SCHEMA.TRIGGERS WHERE TRIGGER_SCHEMA = 'test' AND TRIGGER_NAME = 'trig'" },
  {"test", false, "DROP TRIGGER trig" },
  {"test", false, "CREATE TRIGGER trig AFTER UPDATE ON t1 FOR EACH ROW INSERT INTO t2 VALUES (1, 'aaa')" },
  {"test", false, "UPDATE t1 SET c2 = 'abcde' WHERE c1 = 1" },
  {"test", true , "SELECT * FROM t2" }
};
...
mysql_declare_plugin(test_daemon)
{
  ...
  "test_sql_views_triggers",
  "Pavan Naik, Andrey Hristov",
  "Test sql views triggers",
  PLUGIN_LICENSE_GPL,
