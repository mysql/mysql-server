...
and being used here */
extern int write_bin_log(THD *thd, bool clear_error, const char *query,
...
extern int binlog_log_row(TABLE *table, const uchar *before_record,
    ...
    char *table_name) /*!< in: table name */
{
  ...
  snprintf(query_str, MAX_FULL_NAME_LEN + 16, "%s %s", "truncate table",
           table_name);
    ...
    int idx_to_use)         /*!< in: index to use */
{
  ...
  for (unsigned int i = 0; i < key_info->key_parts; i++) {
    ...
    if (i < srch_args->num_arg) {
      ...
      if (srch_len != 0) {
        ...
        key_part->field->store(srch_value, srch_len, &my_charset_bin);
  ...
  result = handle->index_read_map(my_table->record[0], srch_buf, part_map,
                                  HA_READ_KEY_EXACT);
