...
struct Test_element {
  ...
  Test_element &operator=(size_t i) { return *this = static_cast<int>(i); }
...
struct Key_container {
  Key_container() {
    for (int ix = 0; ix <= sz; ++ix) key_ptrs[ix] = &key_data[ix];
  }
 ...
 protected:
  ...
  virtual void SetUp() {
    for (size_t ix = 0; ix < array_elements(m_test_data); ++ix)
      m_test_data[ix] = ix;
    std::random_shuffle(&m_test_data[0],
                        &m_test_data[array_elements(m_test_data)]);
  }
  ...
  Bounded_queue<Test_element *, Test_key *, Test_keymaker, Test_key_compare>
      m_heap;
...
TEST_F(BoundedQueueTest, TooManyElements) {
  EXPECT_EQ(1, m_queue.init(UINT_MAX, true, test_key_compare, &m_keymaker,
                            m_keys.key_ptrs));
  EXPECT_EQ(1, m_queue.init(UINT_MAX - 1, true, test_key_compare, &m_keymaker,
                            m_keys.key_ptrs));
}
...
static void my_string_ptr_sort(Test_key **base, uint items, size_t size) {
  if (size && items) {
    std::sort(base, base + items, [](const Test_key *a, const Test_key *b) {
...
TEST(CostEstimationTest, MergeManyBuff) {
  ...
  while (num_rows <= MAX_FILE_SIZE / 4) {
    const double merge_cost = get_merge_many_buffs_cost_fast(
        num_rows, num_keys, row_lenght, &cost_model_table);
...
class PerfTest : public ::testing::Test {
 public:
  ...
  virtual void TearDown() {
    std::cout << "C-compare " << count_int_ptr_compare << " Cpp-compare "
              << count_operator << std::endl
              << std::endl;
  }
  /*
...
void insert_into_queue() {
  typedef Key_container<row_limit, int> Container;
  std::cout << "insert " << num_rows << " rows into queue size " << row_limit
            << std::endl;
  for (int it = 0; it < num_iterations; ++it) {
    ...
    EXPECT_EQ(0, queue.init(row_limit, false, int_ptr_compare, &int_keymaker,
                            keys->key_ptrs));
...
void insert_into_heap() {
  typedef Key_container<row_limit, int> Container;
  std::cout << "insert " << num_rows << " rows into heap size " << row_limit
            << std::endl;
  for (int it = 0; it < num_iterations; ++it) {
    Container *keys = new Container;
    srand(0);
    Int_keymaker int_keymaker;
    Queue_t queue;
    EXPECT_EQ(0, queue.init(row_limit, &int_keymaker, keys->key_ptrs));
    for (ha_rows ix = 0; ix < num_rows; ++ix) {
      int data = test_data(ix);
      queue.push(&data);
    }
...
TEST_F(PerfTest, InsertIntoQUEUE) { insert_into_queue(); }

TEST_F(PerfTest, InsertIntoPriorityQueue) {
  insert_into_heap<
      Bounded_queue<int *, int *, Int_keymaker, Int_ptr_compare>>();
}

TEST_F(PerfTest, InsertIntoStdQueue) {
  insert_into_heap<
      Bounded_queue_std<int *, int *, Int_keymaker, Int_ptr_compare>>();
}

TEST_F(PerfTest, InsertIntoBoostQueue) {
  insert_into_heap<
      Bounded_queue_boost<int *, int *, Int_keymaker, Int_ptr_compare>>();
}
