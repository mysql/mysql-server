  ...
  for (int top_parens= 0; top_parens <= max_parens; ++top_parens)
    for (int u1parens= 0; u1parens <= max_parens; ++u1parens)
      for (int u2parens= 0; u2parens <= max_parens; ++u2parens)
        ...
        {
          string query=
            parenthesize("SELECT 1", u1parens) +
            " UNION " +
            parenthesize("SELECT 2", u2parens) +
            " UNION " +
...
//  EXPECT_STREQ("union", ((Item_field*)fake->order_list.first->item[0])->context->first_name_resolution_table->alias);
}
