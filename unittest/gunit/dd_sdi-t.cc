...
void mock_properties(Properties &p, uint64 size)
{
...
void mock_dd_obj(Column_type_element *cte)
{
  if (cte)
  {
...
void mock_dd_obj(Column *c)
{
  static Object_id curid= 10000;
  dynamic_cast<Entity_object_impl*>(c)->set_id(curid++);
  c->set_type(enum_column_types::ENUM);
  ...
  {
    dynamic_cast<Column_impl*>(c)->set_ordinal_position(1);
  }
}


void mock_dd_obj(Index_element *ie)
{
  ie->set_length(42);
  ie->set_order(Index_element::ORDER_DESC);
}


void mock_dd_obj(Index *i, Column *c= NULL)
{
  static Object_id curid= 10000;
  dynamic_cast<Entity_object_impl*>(i)->set_id(curid++);
  ...
  i->set_type(Index::IT_MULTIPLE);
  i->set_algorithm(Index::IA_HASH);

  mock_dd_obj(i->add_element(c));

  if (i->ordinal_position() == 0)
  {
    dynamic_cast<Index_impl*>(i)->set_ordinal_position(1);
  }
}


void mock_dd_obj(Foreign_key_element *fke)
{
  fke->referenced_column_name("mocked referenced column name");
}


void mock_dd_obj(Foreign_key *fk)
{
...
void mock_dd_obj(Partition_index *pi)
{
...
void mock_dd_obj(Partition_value *pv)
{
  pv->set_list_num(42);
...
void mock_dd_obj(Partition *p, Index *ix= NULL)
{
  p->set_level(42);
  p->set_number(42);
...
void mock_dd_obj(Table *t)
{
  ...
  t->set_partition_type(Table::PT_RANGE);
  t->set_default_partitioning(Table::DP_NUMBER);
  t->set_partition_expression("mocked partition expression");
  t->set_subpartition_type(Table::ST_LINEAR_HASH);
  t->set_default_subpartitioning(Table::DP_YES);
  t->set_subpartition_expression("mocked subpartition expression");

  for (int i= 0; i < FANOUT; ++i)
  {
    mock_dd_obj(t->add_foreign_key());
    Column *c= t->add_column();
    mock_dd_obj(c);
    Index *ix= t->add_index();
    mock_dd_obj(ix, c);
    Partition *p= t->add_partition();
    mock_dd_obj(p, ix);
  }
}


void mock_dd_obj(Tablespace_file *f)
{
...
void mock_dd_obj(Tablespace *ts)
{
...
void simple_test()
{
  std::unique_ptr<T> dd_obj(create_object<T>());
...
{
  std::unique_ptr<Schema> s(dd::create_object<Schema>());
  mock_dd_obj(s.get());
  api_test(s);
}
...
{
  std::unique_ptr<Tablespace> ts(dd::create_object<Tablespace>());
  mock_dd_obj(ts.get());
