...
TEST_F(Aligned_atomic_test, aligned_allocation) {
  ...
  EXPECT_EQ((unsigned long)atm1.m_underlying % memory::cache_line_size(), 0);

  memory::Aligned_atomic<bool> atm2{true};
  EXPECT_EQ((unsigned long)atm2.m_underlying % memory::cache_line_size(), 0);

  memory::Aligned_atomic<short> atm3{0};
  EXPECT_EQ((unsigned long)atm3.m_underlying % memory::cache_line_size(), 0);
}
...
TEST_F(Aligned_atomic_test, aligned_allocation_array) {
  ...
  for (int i = 0; i < array_size; i++)
    EXPECT_EQ((unsigned long)atm[i].m_underlying % memory::cache_line_size(),
              0);
}
