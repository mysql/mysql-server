...
namespace sha2_password_unittest
{
  ...
  TEST_F(SHA256_digestTest, DigestSingleStage)
  {
    ...
    size_t plaintext1_length= strlen((char *)plaintext1);
    unsigned char expected_digest1[] = {0x5c, 0xa1, 0xa2, 0x08,
                                        0xc4, 0x61, 0x95, 0x3f,
                                        0x7c, 0x12, 0xd2, 0x79,
                                        0xc7, 0xa8, 0x62, 0x0f,
                                        0x3f, 0x83, 0x87, 0xf4,
    ...
    ASSERT_TRUE(sha256_digest.all_ok() == true);
    ASSERT_TRUE(sha256_digest.update_digest(plaintext1, plaintext1_length) == false);
    ASSERT_TRUE(sha256_digest.retrieve_digest(digest_output, sizeof(digest_output)) == false);
    ASSERT_TRUE(memcmp(expected_digest1, digest_output, CACHING_SHA2_DIGEST_LENGTH) == 0);
    sha256_digest.scrub();

    const char * plaintext2= "ABCD1234%^&*";
    size_t plaintext2_length= strlen((char *) plaintext2);

    unsigned char expected_digest2[] = {0xdd, 0xaf, 0x2b, 0xa0,
                                        0x31, 0x73, 0x45, 0x3d,
                                        0x72, 0x34, 0x51, 0x27,
                                        0xeb, 0x32, 0xb5, 0x24,
                                        0x00, 0xc8, 0xf0, 0x89,
                                        ...
                                        0x01, 0x45, 0x84, 0x09};
    ASSERT_TRUE(sha256_digest.all_ok() == true);
    ASSERT_TRUE(sha256_digest.update_digest(plaintext2, plaintext2_length) == false);
    ASSERT_TRUE(sha256_digest.retrieve_digest(digest_output, sizeof(digest_output)) == false);
    ASSERT_TRUE(memcmp(expected_digest2, digest_output, CACHING_SHA2_DIGEST_LENGTH) == 0);
  ...
  TEST_F(SHA256_digestTest, DigestMultiStage)
  {
    ...
    size_t plaintext_input1_length= strlen((char *)plaintext_input1);
    const char * plaintext_input2= "ABCD1234%^&*";
    size_t plaintext_input2_length= strlen((char *) plaintext_input2);
    const char * plaintext_input3= "Hello World";
    size_t plaintext_input3_length= strlen((char *) plaintext_input3);

    unsigned char digest_output[CACHING_SHA2_DIGEST_LENGTH];

    unsigned char expected_digest1[] = {0xd5, 0x66, 0x19, 0xff,
                                        0x7c, 0xdb, 0x13, 0x00,
                                        0x0b, 0x57, 0x1c, 0x33,
                                        0x8f, 0xe3, 0x33, 0xaf,
                                        0x41, 0x87, 0xa6, 0x83,
    ...
    ASSERT_TRUE(sha256_digest.all_ok() == true);
    ASSERT_TRUE(sha256_digest.update_digest(plaintext_input1, plaintext_input1_length) == false);
    ASSERT_TRUE(sha256_digest.update_digest(plaintext_input2, plaintext_input2_length) == false);
    ASSERT_TRUE(sha256_digest.update_digest(plaintext_input3, plaintext_input3_length) == false);
    ASSERT_TRUE(sha256_digest.retrieve_digest(digest_output, sizeof(digest_output)) == false);
    ASSERT_TRUE(memcmp(expected_digest1, digest_output, CACHING_SHA2_DIGEST_LENGTH) == 0);
    sha256_digest.scrub();

    unsigned char expected_digest2[]= {0xd0, 0xcf, 0xa1, 0xd2,
                                       0x9f, 0xb6, 0xfe, 0x8f,
                                       0x3c, 0xff, 0x2c, 0xa8,
                                       0x2a, 0xe2, 0xc6, 0x6b,
                                       0x8b, 0x2b, 0x33, 0xf9,
  ...
  TEST_F(SHA256_digestTest, GenerateScramble)
  {
    ...
    unsigned char expected_scramble1[]= {0x6a, 0x45, 0x37, 0x96,
                                         0x6b, 0x29, 0x63, 0x59,
                                         0x24, 0x8d, 0x64, 0x86,
                                         0x0a, 0xd6, 0xcc, 0x2a,
                                         0x06, 0x47, 0x8c, 0x26,
    ...
    ASSERT_TRUE(memcmp(expected_scramble1, scramble, CACHING_SHA2_DIGEST_LENGTH) == 0);
  }
  ...
  TEST_F(SHA256_digestTest, ValidateScramble)
  {
    ...
    unsigned char received_scramble1[]= {0x6a, 0x45, 0x37, 0x96,
                                         0x6b, 0x29, 0x63, 0x59,
                                         0x24, 0x8d, 0x64, 0x86,
                                         0x0a, 0xd6, 0xcc, 0x2a,
                                         0x06, 0x47, 0x8c, 0x26,
    ...
    size_t source_length= strlen((char*) source);
    std::string rnd="eF!@34gH%^78";

    unsigned char digest_stage2[CACHING_SHA2_DIGEST_LENGTH];
    ASSERT_TRUE(sha256_digest.all_ok() == true);
    ASSERT_TRUE(sha256_digest.update_digest(source, source_length) == false);
    ASSERT_TRUE(sha256_digest.retrieve_digest(digest_stage2, sizeof(digest_stage2)) == false);
    sha256_digest.scrub();
    ASSERT_TRUE(sha256_digest.all_ok() == true);
    ASSERT_TRUE(sha256_digest.update_digest(digest_stage2, sizeof(digest_stage2)) == false);
    ASSERT_TRUE(sha256_digest.retrieve_digest(digest_stage2, sizeof(digest_stage2)) == false);
    sha256_digest.scrub();

    Validate_scramble validate_scramble(received_scramble1, digest_stage2,
  ...
  TEST_F(SHA256_digestTest, validate_sha256_scramble)
  {
    ...
    size_t rnd_length= strlen((char *) rnd);

    unsigned char digest_stage2[CACHING_SHA2_DIGEST_LENGTH];

    ASSERT_TRUE(sha256_digest.all_ok() == true);
    ASSERT_TRUE(sha256_digest.update_digest(source, source_length) == false);
    ASSERT_TRUE(sha256_digest.retrieve_digest(digest_stage2, CACHING_SHA2_DIGEST_LENGTH) == false);
    sha256_digest.scrub();
    ASSERT_TRUE(sha256_digest.all_ok() == true);
    ASSERT_TRUE(sha256_digest.update_digest(digest_stage2, CACHING_SHA2_DIGEST_LENGTH) == false);
    ASSERT_TRUE(sha256_digest.retrieve_digest(digest_stage2, CACHING_SHA2_DIGEST_LENGTH) == false);
    sha256_digest.scrub();

    ASSERT_TRUE(validate_sha256_scramble(received_scramble1, sizeof(received_scramble1),
                                  digest_stage2, sizeof(digest_stage2),
                                  rnd, rnd_length) == false);
  }
  ...
  TEST_F(SHA256_digestTest, SHA2_password_cache)
  {
    ...
    sha2_cache_entry retrieved_password;
    {
      ...
      ASSERT_TRUE(sha256_digest.all_ok() == true);
      ASSERT_TRUE(sha256_digest.update_digest(source.c_str(), source.length()) == false);
      ASSERT_TRUE(sha256_digest.retrieve_digest(stored_hash.digest_buffer, CACHING_SHA2_DIGEST_LENGTH) == false);
      sha256_digest.scrub();
      ASSERT_TRUE(sha256_digest.update_digest(stored_hash.digest_buffer, CACHING_SHA2_DIGEST_LENGTH) == false);
      ASSERT_TRUE(sha256_digest.retrieve_digest(stored_hash.digest_buffer, CACHING_SHA2_DIGEST_LENGTH) == false);
      sha256_digest.scrub();
      ASSERT_TRUE(sha256_digest.all_ok() == true);
    }
    ...
    ASSERT_TRUE(sha2_password_cache.search(auth_id1, retrieved_password) == false);
    ...
    ASSERT_TRUE(sha2_password_cache.search(auth_no_user, retrieved_password) == true);
    ASSERT_TRUE(sha2_password_cache.search(auth_no_host, retrieved_password) == true);
    ASSERT_TRUE(sha2_password_cache.search(auth_empty, retrieved_password) == true);
    ...
    ASSERT_TRUE(sha2_password_cache.search(auth_id3, retrieved_password) == false);
    ASSERT_TRUE(memcmp((void*)stored_hash.digest_buffer,
                       (void*)retrieved_password.digest_buffer,
                       CACHING_SHA2_DIGEST_LENGTH) == 0);
    ...
    ASSERT_TRUE(sha2_password_cache.search(auth_empty, retrieved_password) == false);
    ASSERT_TRUE(memcmp((void*)stored_hash.digest_buffer,
                       (void*)retrieved_password.digest_buffer,
                       CACHING_SHA2_DIGEST_LENGTH) == 0);
  ...
  TEST_F(SHA256_digestTest, Caching_sha2_password_Serialize_Deserialize)
  {
    ...
    ASSERT_TRUE(caching_sha2_password.deserialize(sha256_valid_auth_string1,
                                                 digest_type, salt,
                                                 sha256_digest, iterations)
                == false);
    ...
    ASSERT_TRUE(caching_sha2_password.serialize(sha256_valid_auth_string1_serialized,
                                               digest_type, salt,
                                               sha256_digest, iterations)
                == false);
    ASSERT_TRUE(sha256_valid_auth_string1 == sha256_valid_auth_string1_serialized);
    ...
    ASSERT_TRUE(caching_sha2_password.deserialize(sha256_valid_auth_string2,
                                                 digest_type, salt,
                                                 sha256_digest, iterations)
                == false);
    ASSERT_TRUE(digest_type == Digest_info::SHA256_DIGEST);
    ...
    ASSERT_TRUE(caching_sha2_password.serialize(sha256_valid_auth_string2_serialized,
                                               digest_type, salt,
                                               sha256_digest, iterations)
                == false);
    ASSERT_TRUE(sha256_valid_auth_string2 == sha256_valid_auth_string2_serialized);
    ...
    ASSERT_TRUE(caching_sha2_password.deserialize(sha256_invalid_digest_type,
                                                 digest_type, salt,
                                                 sha256_digest, iterations)
                == true);
    sha256_invalid_digest_type.assign("$HOHO$005$01234567ABCDEFGH!@#$abcdefgh01234567ijklm890!@#$%^&*a;3-6;2[6-4");
    ASSERT_TRUE(caching_sha2_password.deserialize(sha256_invalid_digest_type,
                                                 digest_type, salt,
                                                 sha256_digest, iterations)
                == true);
    ...
    ASSERT_TRUE(caching_sha2_password.deserialize(sha256_invalid_iteration,
                                                 digest_type, salt,
                                                 sha256_digest, iterations)
                == true);
    sha256_invalid_iteration.assign("$A$000$01234567ABCDEFGH!@#$abcdefgh01234567ijklm890!@#$%^&*a;3-6;2[6-4");
    ASSERT_TRUE(caching_sha2_password.deserialize(sha256_invalid_iteration,
                                                 digest_type, salt,
                                                 sha256_digest, iterations)
                == true);
    sha256_invalid_iteration.assign("$A$01234567ABCDEFGH!@#$abcdefgh01234567ijklm890!@#$%^&*a;3-6;2[6-4");
    ASSERT_TRUE(caching_sha2_password.deserialize(sha256_invalid_iteration,
                                                 digest_type, salt,
                                                 sha256_digest, iterations)
                == true);
    ...
    ASSERT_TRUE(caching_sha2_password.deserialize(sha256_invalid_salt,
                                                 digest_type, salt,
                                                 sha256_digest, iterations)
                == true);
    ...
    ASSERT_TRUE(caching_sha2_password.deserialize(sha256_invalid_digest,
                                                 digest_type, salt,
                                                 sha256_digest, iterations)
                == true);
  }
  ...
  {
    Caching_sha2_password caching_sha2_password(0);
    Caching_sha2_password caching_sha2_password_4(0, DEFAULT_STORED_DIGEST_ROUNDS, 4);

    std::string plaintext("HahaH0hO1234#$@#%");
    sha2_cache_entry digest;

    unsigned char expected_digest[]= {0x00, 0x3c, 0x20, 0x68,
                                      0xb3, 0x6b, 0xc9, 0xb7,
                                      0x18, 0x75, 0xb3, 0x38,
                                      0x7d, 0x2f, 0x86, 0xf9,
                                      0xe3, 0x3a, 0xb3, 0x83,
    ...
    unsigned char expected_digest_4[]= {0x5c, 0x28, 0x83, 0xc3,
                                        0x7a, 0xf0, 0xe1, 0x69,
                                        0x2a, 0x03, 0xe2, 0x91,
                                        0x06, 0x4a, 0xf2, 0xa5,
                                        0x05, 0x8b, 0x4a, 0xcd,
    ...
    ASSERT_TRUE(caching_sha2_password.generate_fast_digest(plaintext, digest) == false);
    ASSERT_TRUE(memcmp(digest.digest_buffer, expected_digest, CACHING_SHA2_DIGEST_LENGTH) == 0);

    ASSERT_TRUE(caching_sha2_password_4.generate_fast_digest(plaintext, digest) == false);
    ASSERT_TRUE(memcmp(digest.digest_buffer, expected_digest_4, CACHING_SHA2_DIGEST_LENGTH) == 0);

    plaintext.assign(";bCdEF34^i9&*\":({\\56\"");

    unsigned char expected_digest2[]= {0x36, 0x87, 0x98, 0x1f,
                                       0xd5, 0x1f, 0xcb, 0x29,
                                       0x66, 0x12, 0x71, 0x51,
                                       0x2c, 0x84, 0xc5, 0x7e,
                                       0x9f, 0xca, 0x6a, 0xf1,
    ...
    unsigned char expected_digest2_4[] = {0x8f, 0xb6, 0xbe, 0x4e,
                                          0x03, 0xae, 0x07, 0xa2,
                                          0x0c, 0x1d, 0x34, 0x4c,
                                          0x65, 0x8c, 0x98, 0xfb,
                                          0x72, 0x0a, 0x10, 0xd3,
    ...
    ASSERT_TRUE(caching_sha2_password.generate_fast_digest(plaintext, digest) == false);
    ASSERT_TRUE(memcmp(digest.digest_buffer, expected_digest2, CACHING_SHA2_DIGEST_LENGTH) == 0);

    ASSERT_TRUE(caching_sha2_password_4.generate_fast_digest(plaintext, digest) == false);
    ASSERT_TRUE(memcmp(digest.digest_buffer, expected_digest2_4, CACHING_SHA2_DIGEST_LENGTH) == 0);
  }
  ...
  TEST_F(SHA256_digestTest, Caching_sha2_password_generate_sha2_multi_hash)
  {
    ...
    my_crypt_genhash(stage2, CRYPT_MAX_PASSWORD_SIZE,
                     plaintext_buffer_arthur.c_str(),
                     plaintext_buffer_arthur.length(),
                     salt_buffer_arthur.c_str(), 0);

    std::string digest_string1(stage2);
    digest_string1= digest_string1.substr(digest_string1.find('$', 3+CRYPT_SALT_LENGTH)+1, std::string::npos);

    Caching_sha2_password caching_sha2_password(0);
    std::string digest;
    ASSERT_TRUE(caching_sha2_password.
                  generate_sha2_multi_hash(plaintext_buffer_arthur,
                                           salt_buffer_arthur,
                                           &digest, 5000) == false);
  ...
  TEST_F(SHA256_digestTest, Caching_sha2_password_authenticate_fast_authenticate)
  {
    Caching_sha2_password caching_sha2_password(0);
    ...
    const char digest_buffer_arthur[]= {0x64, 0x4e, 0x46, 0x65,
                                        0x67, 0x45, 0x61, 0x34,
                                        0x4f, 0x47, 0x44, 0x72,
                                        0x6e, 0x45, 0x75, 0x37,
                                        0x4d, 0x34, 0x54, 0x2e,
    ...
    const char digest_buffer_arthur_ic[]= {0x64, 0x4e, 0x46, 0x65,
                                           0x67, 0x45, 0x61, 0x34,
                                           0x4f, 0x47, 0x44, 0x72,
                                           0x6e, 0x45, 0x75, 0x37,
                                           0x4d, 0x34, 0x54, 0x2e,
    ...
    const char digest_buffer_marvin[]= {0x53, 0x50, 0x4d, 0x4e,
                                        0x2f, 0x4c, 0x38, 0x2e,
                                        0x6e, 0x6f, 0x4b, 0x71,
                                        0x65, 0x38, 0x49, 0x58,
                                        0x56, 0x5a, 0x32, 0x44,
    ...
    ASSERT_TRUE(caching_sha2_password.serialize(serialized_string, digest_type,
                                               salt, digest, iterations) == false);
    ASSERT_TRUE(caching_sha2_password.authenticate(auth_id_marvin,
                                                  serialized_string,
                                                  plaintext) == false);
    // Attempt again, it should pass
    ASSERT_TRUE(caching_sha2_password.authenticate(auth_id_marvin,
    ...
    const char digest_buffer_zaphod[]= {0x72, 0x50, 0x38, 0x4a,
                                        0x6e, 0x38, 0x4d, 0x74,
                                        0x70, 0x36, 0x58, 0x43,
                                        0x4d, 0x78, 0x31, 0x51,
                                        0x6a, 0x53, 0x44, 0x41,
    ...
    ASSERT_TRUE(caching_sha2_password.serialize(serialized_string, digest_type,
                                               salt, digest, iterations) == false);
    ASSERT_TRUE(caching_sha2_password.authenticate(auth_id_zaphod,
                                                  serialized_string,
                                                  plaintext) == false);
    // Attempt again, it should pass
    ASSERT_TRUE(caching_sha2_password.authenticate(auth_id_zaphod,
    ...
    ASSERT_TRUE(caching_sha2_password.fast_authenticate(auth_id_zaphod,
                  reinterpret_cast<const unsigned char *>(scramble_random_zaphod.c_str()),
    ...
    ASSERT_TRUE(caching_sha2_password.fast_authenticate(auth_id_zaphod,
                  reinterpret_cast<const unsigned char *>(scramble_random_zaphod.c_str()),
                  scramble_random_zaphod.length(),
                  scramble_zaphod) == true);

    plaintext.assign(plaintext_buffer_arthur);
    char random_salt_buffer[CRYPT_SALT_LENGTH + 1];
    generate_user_salt(random_salt_buffer, CRYPT_SALT_LENGTH + 1);
    std::string scramble_random_arthur(random_salt_buffer);
    Generate_scramble generate_scramble_arthur(plaintext,
                                               scramble_random_arthur);
    ...
    ASSERT_TRUE(caching_sha2_password.fast_authenticate(auth_id_arthur,
                  reinterpret_cast<const unsigned char *>(scramble_random_arthur.c_str()),
