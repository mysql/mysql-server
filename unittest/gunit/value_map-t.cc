...
TEST_F(ValueMapTest, ValueMapWithLongStrings)
{
  ...
  */
  ...
  value_map.add_values(String("abcdefghijklmnopqrstuvwxyz1234567890abcdef",
                              &my_charset_latin1), 1);
  EXPECT_EQ(value_map.size(), 1U);

  // Exactly 42 characters, with a small difference from the first string
  value_map.add_values(String("abcdefghijklmnopqrstuvwxyz1234567890abcdeg",
                              &my_charset_latin1), 1);
  EXPECT_EQ(value_map.size(), 2U);
  ...
  */
  value_map.add_values(String("abcdefghijklmnopqrstuvwxyz1234567890abcdef1",
                              &my_charset_latin1), 1);
  EXPECT_EQ(value_map.size(), 2U);
  ...
  EXPECT_EQ(strcmp(value_map.begin()->first.ptr(),
                   "abcdefghijklmnopqrstuvwxyz1234567890abcdef"), 0);
  EXPECT_EQ(strcmp(std::next(value_map.begin(), 1)->first.ptr(),
                   "abcdefghijklmnopqrstuvwxyz1234567890abcdeg"), 0);
...
TEST_F(ValueMapTest, LongLongValueMapExtended)
{
  ...
  for (int i= 0; i < 10000; ++i)
    value_map.add_values(dis(gen), 1);
