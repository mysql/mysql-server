...
#define create_item_check_filter(er, op, f1, v1, v2, ut, fti)            \
                                    ...
                                    TestFailLinePrinter called_from_line)
  {
    ...
    const float filter= item->get_filtering_effect(m_table->map,
                                                   used_tables,
                                                   fields_to_ignore,
                                                   rows_in_table);
...
#define create_anditem_check_filter(er, lst, ut, fti)  \
                                 ...
                                 TestFailLinePrinter called_from_line)
  {
    ...
    const float filter= and_item->get_filtering_effect(m_table->map,
                                                       used_tables,
                                                       fields_to_ignore,
                                                       rows_in_table);
    EXPECT_FLOAT_EQ(expected_result, filter);
...
#define create_oritem_check_filter(er, lst, ut, fti)  \
                                ...
                                TestFailLinePrinter called_from_line)
  {
    ...
    const float filter= or_item->get_filtering_effect(m_table->map,
                                                      used_tables,
                                                      fields_to_ignore,
                                                      rows_in_table);
    EXPECT_FLOAT_EQ(expected_result, filter);
...
#define create_initem_check_filter(er, lst, ut, fti)  \
                                ...
                                TestFailLinePrinter called_from_line)
  {
    ...
    const float filter= in_item->get_filtering_effect(m_table->map,
                                                      used_tables,
                                                      fields_to_ignore,
                                                      rows_in_table);
    EXPECT_FLOAT_EQ(expected_result, filter);
...
TEST_F(ItemFilterTest, BasicIgnoreField)
{
  ...
  bitmap_init(&ignore_fld0, 0, m_table->s->fields, false);
...
TEST_F(ItemFilterTest, BasicConstAnd)
{
  ...
  */
  ...
  float filter= and_it->get_filtering_effect(m_table->map,
                                             used_tables,
                                             &ignore_flds,
                                             rows_in_table);
  ...
  */
  ...
  filter= and_it->get_filtering_effect(m_table->map,
                                       used_tables,
                                       &ignore_flds,
                                       rows_in_table);
...
TEST_F(ItemFilterTest, BasicConstOr)
{
  ...
  */
  bitmap_set_bit(&ignore_flds, m_field[0]->field_index);
  const float filt_ign0= or_it->get_filtering_effect(m_table->map,
                                                     used_tables,
                                                     &ignore_flds,
                                                     rows_in_table);
  ...
  */
  bitmap_set_bit(&ignore_flds, m_field[1]->field_index);
  const float filt_ign1= or_it->get_filtering_effect(m_table->map,
                                                     used_tables,
                                                     &ignore_flds,
                                                     rows_in_table);
  ...
  */
  ...
  bitmap_set_bit(&ignore_flds, m_field[2]->field_index);
  const float filt_ign2= or_it->get_filtering_effect(m_table->map,
                                                     used_tables,
                                                     &ignore_flds,
                                                     rows_in_table);
...
TEST_F(ItemFilterTest, InPredicate)
{
  ...
  */
  bitmap_set_bit(&ignore_flds, m_field[0]->field_index);
  const float filt_ign= in_it->get_filtering_effect(m_table->map,
                                                    used_tables,
                                                    &ignore_flds,
                                                    rows_in_table);
