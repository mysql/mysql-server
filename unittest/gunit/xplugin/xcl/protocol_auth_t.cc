...
struct Auth_test_messages {
  ...
  const Message_from_str<Auth_details::Authenticate_continue>
      msg_auth_cont_mysql41 {"auth_data: \"schema\\0user\\0\
*ACFC0C3FA7F3C1F39849B44177D8B82C7F75E0D1\""
  };

  const Message_from_str<Auth_details::Authenticate_continue>
      msg_auth_cont_sha256_memory {"auth_data: \"schema\\0user\\0\
 ...
 public:
  void assert_authenticate(const std::string &mech,
    ...
    auto error = m_sut->execute_authenticate(m_messages.expected_user,
        m_messages.expected_pass, m_messages.expected_schema, mech);
...
TEST_F(Xcl_protocol_impl_tests_auth,
  ...
  expect_write_message_without_payload(
      Auth_test_messages().msg_auth_start_plain,
      expected_error_code);
...
INSTANTIATE_TEST_CASE_P(InstantiationAuthDetails, Xcl_protocol_impl_tests_auth,
    ::testing::Values(
      Auth_details{"MYSQL41",
         Auth_test_messages().msg_auth_start_mysql41,
