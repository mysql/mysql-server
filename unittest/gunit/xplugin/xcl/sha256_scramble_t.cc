...
TEST(SHA256_digest_test, digest_single_stage) {
  ...
  ASSERT_FALSE(sha256_digest.retrieve_digest(
      digest_output, sizeof(digest_output)));
  ASSERT_TRUE(
  ...
  ASSERT_FALSE(sha256_digest.retrieve_digest(digest_output,
      sizeof(digest_output)));
  ASSERT_TRUE(memcmp(expected_digest2, digest_output,
...
TEST(SHA256_digest_test, digest_multi_stage) {
  ...
  ASSERT_TRUE(sha256_digest.all_ok());
  ASSERT_FALSE(sha256_digest.update_digest(plaintext_input1,
      plaintext_input1_length));
  ASSERT_FALSE(sha256_digest.update_digest(plaintext_input2,
      plaintext_input2_length));
  ASSERT_FALSE(sha256_digest.update_digest(plaintext_input3,
      plaintext_input3_length));
  ASSERT_FALSE(sha256_digest.retrieve_digest(digest_output,
      sizeof(digest_output)));
  ASSERT_TRUE(memcmp(expected_digest1, digest_output,
      CACHING_SHA2_DIGEST_LENGTH) == 0);
  sha256_digest.scrub();
...
TEST(SHA256_digest_test, generate_scramble) {
  ...
  ASSERT_TRUE(memcmp(expected_scramble1, scramble,
      CACHING_SHA2_DIGEST_LENGTH) == 0);
}
...
TEST(SHA256_digest_test, generate_sha256_scramble) {
  ...
  ASSERT_FALSE(generate_sha256_scramble(scramble, CACHING_SHA2_DIGEST_LENGTH,
      source.c_str(), source.length(), rnd.c_str(), rnd.length()));
