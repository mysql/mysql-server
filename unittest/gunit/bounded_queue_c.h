...
class Bounded_QUEUE {
 public:
   ...
   */
  int init(ha_rows max_elements, bool max_at_top, compare_function compare,
           Key_generator *sort_param, Key_type *sort_keys);
  ...
  Key_type *pop() {
    ...
    if (queue_is_full((&m_queue))) queue_remove(&m_queue, 0);
    DBUG_ASSERT(m_queue.elements > 0);
    if (m_queue.elements == 0) return NULL;
...
template <typename Element_type, typename Key_type, typename Key_generator>
int Bounded_QUEUE<Element_type, Key_type, Key_generator>::init(
    ha_rows max_elements, bool max_at_top, compare_function compare,
    Key_generator *sort_param, Key_type *sort_keys) {
  ...
  if (max_elements >= (UINT_MAX - 1)) return 1;
  if (compare == NULL)
    compare = reinterpret_cast<compare_function>(
  ...
  return init_queue(&m_queue, PSI_INSTRUMENT_ME, (uint)max_elements + 1, 0,
                    max_at_top, reinterpret_cast<queue_compare>(compare),
                    &m_compare_length);
}

template <typename Element_type, typename Key_type, typename Key_generator>
void Bounded_QUEUE<Element_type, Key_type, Key_generator>::push(
