#ifndef _spw_api_serial_h_
#define _spw_api_serial_h_
...
// Endianness must match Sparrow's
...
public:
	...
	ByteBuffer();
	ByteBuffer(const uint8_t* data, const uint32_t limit);
	ByteBuffer(uint8_t* data, const uint32_t limit, ByteBufferOverflow* overflow = 0);
	...
	void position(const uint32_t pos);
	uint32_t position() const;
	uint32_t limit() const;
	void limit(const uint32_t newLimit);
	bool end() const;
...
inline void ByteBuffer::overflow() _THROW_(SparrowException) {
	if (overflow_ == 0) {
		throw SparrowException::create(false, SPW_API_FAILED, "Buffer overflow (limit=%u bytes)", limit_);
	} else {
		overflow_->overflow();
	}
}
...
inline ByteBuffer::ByteBuffer()
	: data_(NULL), limit_(0), pos_(0), overflow_(0) {
}

inline ByteBuffer::ByteBuffer(const uint8_t* data, const uint32_t limit)
	: data_(const_cast<uint8_t*>(data)), limit_(limit), pos_(0), overflow_(0) {
}

inline ByteBuffer::ByteBuffer(uint8_t* data, const uint32_t limit, ByteBufferOverflow* overflow /* = 0 */)
	: data_(data), limit_(limit), pos_(0), overflow_(overflow) {
}
...
inline void ByteBuffer::position(const uint32_t pos) {
	pos_ = pos;
}

inline uint32_t ByteBuffer::position() const {
	return pos_;
}

inline uint32_t ByteBuffer::limit() const {
	return limit_;
}

inline void ByteBuffer::limit(const uint32_t newLimit) {
	limit_ = newLimit;
}
...
inline void ByteBuffer::advance(uint32_t offset) {
	while (offset > 0) {
		if (pos_ >= limit_) {
			overflow();
		}
		const uint32_t length = std::min(limit_ - pos_, offset);
		pos_ += length;
		offset -= length;
	}
}

inline uint32_t ByteBuffer::remaining() const {
	return limit_ - pos_;
}
...
inline ByteBuffer& ByteBuffer::operator << (const ByteBuffer& v) {
	const uint32_t limit = v.limit_;
	uint32_t pos = v.pos_;
	while (pos < limit) {
		if (pos_ >= limit_) {
			overflow();
		}
		const uint32_t length = std::min(limit_ - pos_, limit - pos);
...
inline ByteBuffer& ByteBuffer::operator >> (ByteBuffer& v) {
	while (v.pos_ < v.limit_) {
		...
		const uint32_t length = std::min(limit_ - pos_, v.limit_ - v.pos_);
...
typedef RefPtr<ByteBuffer>	RefByteBuffer;
...
public:
	...
	ByteBufferGuard(const uint32_t size) : size_(size), buffer_(ByteBuffer::mmap(size), size) {
	}

	~ByteBufferGuard() {
		ByteBuffer::munmap(buffer_.getData(), size_);
	}
...
public:
	IOBuffer(const uint32_t size) : ByteBuffer( ByteBuffer::mmap(size), size ) {
	}
...
class HeapBuffer : public ByteBuffer, public ByteBufferOverflow {
public:

	HeapBuffer(const uint32_t limit=1024);
	...
	HeapBuffer& operator = (const HeapBuffer& buffer);
...
public:
	...
	SocketReader(my_socket socketId, ByteBuffer& buffer) _THROW_(SparrowException);
...
#endif /* #ifndef _spw_api_serial_h_ */
