...
public class JdbcLoad extends Driver {
    ...
    protected void initOperations() throws SQLException {
        ...
        ops.add(
            new JdbcOp("getAllA_asc",
                       "SELECT id, cint, clong, cfloat, cdouble FROM a ORDER BY id ASC") {
                public void run(int countA, int countB) throws SQLException {
        ...
        ops.add(
            new JdbcOp("getAllB0_asc",
                       "SELECT id, cint, clong, cfloat, cdouble FROM b0 ORDER BY id ASC") {
                public void run(int countA, int countB) throws SQLException {
                    ResultSet rs = stmt.executeQuery();
                    for (int i = 1; i <= countB; i++) {
                        rs.next();
        ...
        final JdbcOp setB0ToA = (JdbcOp)ops.get(ops.size() - 6);
        ...
        ops.add(
            new JdbcOp("nullA->B0",
                       "UPDATE b0 b0 SET b0.a_id = NULL WHERE (b0.a_id = ?)") {
                public void run(int countA, int countB) throws SQLException {
                    int cnt = 0;
                    for (int i = 1; i <= countA; i++) {
                        stmt.setInt(1, i);
