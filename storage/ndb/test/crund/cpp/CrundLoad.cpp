...
bool CrundLoad::excludedOperation(const string &xname) {
  ...
  for (vector<string>::const_iterator r = ex.begin(); r != ex.end(); ++r) {
    if (startsWith(xname, *r)) {  // if (xname.matches(*r))
      // cout << "* exclude " << xname << ": match (" << *r << ")" << endl;
      return true;
    }
  }

  if (in.empty()) {
    // cout << "* include " << xname << ": empty includes" << endl;
    return false;
  }

  for (vector<string>::const_iterator r = in.begin(); r != in.end(); ++r) {
    if (startsWith(xname, *r)) {  // if (xname.matches(*r))
      // cout << "* include " << xname << ": match (" << *r << ")" << endl;
      return false;
    }
  }

  // cout << "* exclude " << xname << ": non-match includes" << endl;
  return true;
}
