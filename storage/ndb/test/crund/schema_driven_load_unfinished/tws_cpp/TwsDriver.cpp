...
void TwsDriver::init() { Driver::init(); }

void TwsDriver::close() { Driver::close(); }

void TwsDriver::initProperties() {
  ...
  if (nRows < 1) {
    msg << "[ignored] nRows:            '" << toString(props[L"nRows"]) << "'"
        << endl;
    nRows = 256;
  }
  ...
  if (nRuns < 0) {
    msg << "[ignored] nRuns:             '" << toString(props[L"nRuns"]) << "'"
        << endl;
    nRuns = 1;
  }
...
void TwsDriver::printProperties() {
...
void TwsDriver::runLoads() {
  // anticipating multiple loads to be run here
  runSeries();
}

void TwsDriver::runSeries() {
...
void TwsDriver::verify(int exp, int act) {
  if (doVerify) {
...
void TwsDriver::verify(long exp, long act) {
  if (doVerify) {
    // cout << "XXX exp=" << exp << ", act=" << act << endl;
    if (exp != act) {
      ABORT_VERIFICATION_ERROR();
...
void TwsDriver::verify(long long exp, long long act) {
  if (doVerify) {
    // cout << "XXX exp=" << exp << ", act=" << act << endl;
    if (exp != act) {
      ABORT_VERIFICATION_ERROR();
...
void TwsDriver::verify(const char *exp, const char *act) {
  if (doVerify) {
    // cout << "XXX exp='" << exp << "', act='" << act << "'" << endl;
    if (strcmp(exp, act) != 0) {
      ABORT_VERIFICATION_ERROR();
...
const char *TwsDriver::toStr(XMode mode) {
  switch (mode) {
    case SINGLE:
      return "single";
    case BULK:
...
const char *TwsDriver::toStr(LockMode mode) {
  switch (mode) {
    case SINGLE:
