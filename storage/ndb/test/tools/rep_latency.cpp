...
static int prepare_master_or_slave(Ndb &myNdb, const char *table,
                                   const char *pk, Uint32 pk_val,
                                   const char *col, struct Xxx &xxx,
                                   struct XxxR &xxxr);
...
static int prepare_master_or_slave(Ndb &myNdb, const char *table,
  ...
  if (myPkCol == NULL) APIERROR(myDict->getNdbError());
  ...
  if (myCol == NULL) APIERROR(myDict->getNdbError());
...
static void run_master_update(struct Xxx &xxx, struct XxxR &xxxr) {
  ...
  while (1) {
    ...
    if (trans == NULL) goto err;
    {
      NdbOperation *op = trans->getNdbOperation(myTable);
      if (op == NULL) APIERROR(trans->getNdbError());
    ...
    if (trans->execute(NdbTransaction::Commit)) goto err;
...
static void run_slave_wait(struct Xxx &xxx, struct XxxR &xxxr) {
  ...
  xxxr.latency = ((elapsed_usec1 - elapsed_usec2 / 2) + 999) / 1000;
}
