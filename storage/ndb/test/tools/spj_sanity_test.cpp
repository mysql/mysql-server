...
class IntField {
 public:
  ...
  Uint64 getValue() const { return m_val; }

  Uint32 getSize() const { return sizeof m_val; }
...
class StrField {
 public:
  static const char *getType() { return "VARCHAR(10)"; }

  StrField(int i = 0) : m_len(6) {
...
class GenericRow {
 public:
  ...
  void makeLessThanCond(NdbScanFilter &scanFilter) {
    ...
    require(scanFilter.cmp(NdbScanFilter::COND_LT, 0, m_values,
                           m_values[0].getSize()) == 0);
  }
...
template <typename FieldType>
static bool operator==(const GenericRow<FieldType> &a,
                       const GenericRow<FieldType> &b) {
...
template <typename FieldType>
static bool operator==(const GenericKey<FieldType> &a,
                       const GenericKey<FieldType> &b) {
  for (int i = 0; i < GenericKey<FieldType>::size; i++) {
    if (a.m_values[i].compare(b.m_values[i]) != 0) {
      return false;
    }
...
template <typename FieldType>
static bool lessOrEqual(const GenericRow<FieldType> &a,
                        const GenericRow<FieldType> &b) {
  for (int i = 0; i < GenericKey<FieldType>::size; i++) {
...
static const char *colName(int colNo) {
  static const char *names[] = {"c1", "c2", "c3", "c4", "c5",
                                "c6", "c7", "c8", "c9", "c10"};
...
class Operation {
 ...
 public:
  ...
  void compareRows(const char *text, const Row *expected,
...
class Query {
 public:
  ...
  void verifyRow() const { m_root->verifyRow(); }
  ...
  void close(bool forceSend = false) { m_query->close(forceSend); }
...
class LookupOperation : public Operation {
 public:
  explicit LookupOperation(Query &query, Operation *parent = NULL);
...
class IndexLookupOperation : public Operation {
 public:
  explicit IndexLookupOperation(Query &query, const char *indexName,
...
class TableScanOperation : public Operation {
 public:
  ...
  virtual ~TableScanOperation() { delete[] m_rowFound; }
...
class IndexScanOperation : public Operation {
 public:
  explicit IndexScanOperation(Query &query, const char *indexName,
                              int lowerBoundRowNo, int upperBoundRowNo,
                              NdbQueryOptions::ScanOrdering ordering);

  virtual ~IndexScanOperation() { delete[] m_rowFound; }
...
// Operation methods.
Operation::Operation(class Query &query, Operation *parent)
    : m_query(query),
      m_parent(parent),
      m_operationDef(NULL),
      m_resultPtr(NULL),
...
void Operation::compareRows(const char *text, const Row *expected,
                            const Row *actual) const {
  if (expected == NULL) {
    if (actual == NULL) {
      ...
      ndbout << text << " operationId=" << m_operationId
             << " expected NULL but got." << *actual << endl;
      require(false);
    }
  } else {
    if (actual == NULL) {
      ndbout << text << " operationId=" << m_operationId
             << " expected: " << *expected << " but got NULL." << endl;
      require(false);
    } else {
      ndbout << text << " operationId=" << m_operationId
             << " expected: " << *expected;
...
LookupOperation ::LookupOperation(Query &query, Operation *parent)
...
void LookupOperation::buildThis(NdbQueryBuilder &builder,
  ...
  if (m_parent == NULL) {
    ...
    // Negative testing
    require(builder.linkedValue(m_parent->m_operationDef, "unknown_col") ==
            NULL);
    ...
    for (int i = 0; i < Key::size; i++) {
      keyOperands[i] =
          builder.linkedValue(m_parent->m_operationDef,
                              colName(Row::getForeignKeyColNo(m_childNo, i)));
...
void LookupOperation::submit() {
  NdbQueryOperation *queryOp = m_query.getOperation(m_operationId);
  // Negative testing
  require(queryOp->setResultRowRef(NULL, m_resultCharPtr, NULL) == -1);
  require(queryOp->getQuery().getNdbError().code == QRY_REQ_ARG_IS_NULL);
  require(queryOp->setOrdering(NdbQueryOptions::ScanOrdering_ascending) == -1);
  require(queryOp->getQuery().getNdbError().code == QRY_WRONG_OPERATION_TYPE);

  require(queryOp->setResultRowRef(m_query.getNdbRecord(), m_resultCharPtr,
                                   NULL) == 0);
  // Negative testing
  require(queryOp->setResultRowRef(m_query.getNdbRecord(), m_resultCharPtr,
                                   NULL) == -1);
...
void LookupOperation::verifyOwnRow() {
  if (m_parent == NULL) {
    ...
    compareRows("lookup root operation", &expected, m_resultPtr);
  } else {
    NdbQueryOperation *queryOp = m_query.getOperation(m_operationId);
    if (!queryOp->getParentOperation(0)->isRowNULL()) {
      const Key key = m_parent->m_resultPtr->getForeignKey(m_childNo);
      bool found = false;
      for (int i = 0; i < m_query.getTableSize(); i++) {
        const Row row(i);
        if (row.getPrimaryKey() == key) {
          found = true;
          compareRows("lookup child operation", &row, m_resultPtr);
        }
      }
      if (!found && !queryOp->isRowNULL()) {
        compareRows("lookup child operation", NULL, m_resultPtr);
      }
    }
  }
}
...
IndexLookupOperation ::IndexLookupOperation(Query &query, const char *indexName,
                                            Operation *parent)
    : Operation(query, parent), m_indexName(indexName) {}

void IndexLookupOperation ::buildThis(NdbQueryBuilder &builder,
                                      const NdbDictionary::Table &tab) {
  const NdbDictionary::Dictionary *const dict = m_query.getDictionary();
  char fullName[200];
...
void IndexLookupOperation::submit() {
  NdbQueryOperation *queryOp = m_query.getOperation(m_operationId);
  queryOp->setResultRowRef(m_query.getNdbRecord(), m_resultCharPtr, NULL);
}

void IndexLookupOperation::verifyOwnRow() {
  if (m_parent == NULL) {
    const Row expected(0);
    compareRows("index lookup root operation", &expected, m_resultPtr);
  } else {
    NdbQueryOperation *queryOp = m_query.getOperation(m_operationId);
    if (!queryOp->getParentOperation(0)->isRowNULL()) {
      const Key key = m_parent->m_resultPtr->getForeignKey(m_childNo);
      ...
      for (int i = 0; i < m_query.getTableSize(); i++) {
        ...
        if (row.getIndexKey() == key) {
          found = true;
          compareRows("index lookup child operation", &row, m_resultPtr);
        }
      }
      if (!found && !queryOp->isRowNULL()) {
        compareRows("index lookup child operation", NULL, m_resultPtr);
      }
    }
  }
}
...
TableScanOperation ::TableScanOperation(Query &query, int lessThanRow)
    : Operation(query, NULL), m_rowFound(NULL), m_lessThanRow(lessThanRow) {}
...
void TableScanOperation::verifyOwnRow() {
  ...
  const int upperBound =
      m_lessThanRow == -1 ? m_query.getTableSize() : m_lessThanRow;
  for (int i = 0; i < upperBound; i++) {
    // const Row row(i);
    if (Row(i) == *m_resultPtr) {
      found = true;
      if (m_rowFound[i]) {
        ndbout << "Root table scan operation: " << *m_resultPtr
               << "appeared twice." << endl;
        require(false);
      }
      m_rowFound[i] = true;
  ...
  if (!found) {
    ndbout << "Root table scan operation. Unexpected row: " << *m_resultPtr
           << endl;
    require(false);
  } else {
    ndbout << "Root table scan operation. Got row: " << *m_resultPtr
           << " as expected." << endl;
  }
}
...
IndexScanOperation ::IndexScanOperation(Query &query, const char *indexName,
                                        int lowerBoundRowNo,
                                        int upperBoundRowNo,
                                        NdbQueryOptions::ScanOrdering ordering)
...
void IndexScanOperation::submit() {
  ...
  if (m_ordering != NdbQueryOptions::ScanOrdering_unordered) {
    require(queryOp->setOrdering(NdbQueryOptions::ScanOrdering_ascending) != 0);
    require(queryOp->getQuery().getNdbError().code ==
...
void IndexScanOperation::verifyOwnRow() {
  ...
  for (int i = m_lowerBoundRowNo; i <= m_upperBoundRowNo; i++) {
    ...
    if (row == *m_resultPtr) {
      found = true;
      if (m_rowFound[i]) {
        ndbout << "Root index scan operation: " << *m_resultPtr
               << "appeared twice." << endl;
        require(false);
      }
      m_rowFound[i] = true;
  ...
  if (!found) {
    ndbout << "Root index scan operation. Unexpected row: " << *m_resultPtr
           << endl;
    require(false);
  } else {
    if (m_hasPreviousRow) {
    ...
    ndbout << "Root index scan operation. Got row: " << *m_resultPtr
           << " as expected." << endl;
  }
}
...
void runCase(MYSQL &mysql, Ndb &ndb, Query &query, const char *tabName,
             int tabSize, int rowCount) {
  // Populate test table.
...
int main(int argc, char *argv[]) {
  ...
  if (!mysql_real_connect(&mysql, host, "root", "", "", port, NULL, 0)) {
