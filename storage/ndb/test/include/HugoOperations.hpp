...
class HugoOperations : public UtilTransactions {
 public:
  ...
  int pkInsertRecord(Ndb *, int recordNo, int numRecords = 1,
                     int updatesValue = 0, int row_step = 1);

  int pkWriteRecord(Ndb *, int recordNo, int numRecords = 1,
                    int updatesValue = 0);

  int pkWritePartialRecord(Ndb *, int recordNo, int numRecords = 1);

  int pkReadRecord(Ndb *, int record, int numRecords = 1,
                   NdbOperation::LockMode lm = NdbOperation::LM_Read,
                   NdbOperation::LockMode *lmused = 0, bool noWait = false);

  int pkReadRandRecord(Ndb *, int records, int numRecords = 1,
                       NdbOperation::LockMode lm = NdbOperation::LM_Read,
                       NdbOperation::LockMode *lmused = 0);

  int pkReadRecordLockHandle(Ndb *, Vector<const NdbLockHandle *> &lockHandles,
                             int record, int numRecords = 1,
                             NdbOperation::LockMode lm = NdbOperation::LM_Read,
                             NdbOperation::LockMode *lmused = 0);

  int pkUnlockRecord(Ndb *, Vector<const NdbLockHandle *> &lockHandles,
                     int offset = 0, int numRecords = ~(0),
                     NdbOperation::AbortOption ao = NdbOperation::AbortOnError);

  int pkUpdateRecord(Ndb *, int recordNo, int numRecords = 1,
                     int updatesValue = 0);

  int pkDeleteRecord(Ndb *, int recordNo, int numRecords = 1, int step = 1);

  int pkRefreshRecord(Ndb *, int recordNo, int numRecords = 1,
  ...
  int indexReadRecords(Ndb *, const char *idxName, int recordNo,
                       bool exclusive = false, int records = 1);

  int indexUpdateRecord(Ndb *, const char *idxName, int recordNo,
                        int numRecords = 1, int updatesValue = 0);

  int scanReadRecords(
      Ndb *, NdbScanOperation::LockMode = NdbScanOperation::LM_CommittedRead,
      int numRecords = 1);
  ...
  int execute_async(Ndb *, NdbTransaction::ExecType,
                    NdbOperation::AbortOption = NdbOperation::AbortOnError);
  int execute_async_prepare(
      Ndb *, NdbTransaction::ExecType,
      NdbOperation::AbortOption = NdbOperation::AbortOnError);

  int wait_async(Ndb *, int timeout = -1);

  int releaseLockHandles(Ndb *, Vector<const NdbLockHandle *> &lockHandles,
                         int offset = 0, int numRecords = ~(0));

  const NdbError &getNdbError() const;
  void setQuiet() { m_quiet = true; }

  typedef Uint32 (*AnyValueCallback)(Ndb *, NdbTransaction *, int rowid,
                                     int updVal);
