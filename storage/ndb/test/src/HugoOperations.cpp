...
int HugoOperations::startTransaction(Ndb *pNdb, Uint32 node_id,
                                     Uint32 instance_id) {
  if (pTrans != NULL) {
    ndbout << "HugoOperations::startTransaction, pTrans != NULL" << endl;
    return NDBT_FAILED;
...
int HugoOperations::startTransaction(Ndb *pNdb, int recordNo) {
  if (pTrans != NULL) {
    ndbout << "HugoOperations::startTransaction, pTrans != NULL" << endl;
    return NDBT_FAILED;
  }
...
void HugoOperations::setTransactionId(Uint64 id) {
  if (pTrans != NULL) {
...
NdbConnection *HugoOperations::getTransaction() { return pTrans; }
...
int HugoOperations::pkReadRecord(Ndb *pNdb, int recordNo, int numRecords,
  ...
  rand_lock_mode:
    ...
    if (pIndexScanOp) pIndexScanOp->end_of_bound(r);

    if (r == 0 || pIndexScanOp == 0) {
      // Define attributes to read
      for (a = 0; a < tab.getNoOfColumns(); a++) {
        if ((rows[r]->attributeStore(a) = pOp->getValue(tab.getColumn(a))) ==
            0) {
          NDB_ERR(pTrans->getNdbError());
          setNdbError(pTrans->getNdbError());
          return NDBT_FAILED;
...
int HugoOperations::pkReadRandRecord(Ndb *pNdb, int records, int numRecords,
                                     NdbOperation::LockMode lm,
...
int HugoOperations::pkReadRecordLockHandle(
    Ndb *pNdb, Vector<const NdbLockHandle *> &lockHandles, int recordNo,
    int numRecords, NdbOperation::LockMode lm, NdbOperation::LockMode *lmused) {
  if (idx) {
    g_err << "ERROR : Cannot call pkReadRecordLockHandle on an index" << endl;
    return NDBT_FAILED;
  }
   ...
   * will update lm_used
   */
  while (lm != NdbOperation::LM_Read && lm != NdbOperation::LM_Exclusive) {
    lm = (NdbOperation::LockMode)((rand() >> 16) & 1);
  }

  const NdbOperation *prevOp = pTrans->getLastDefinedOperation();

  int readRc = pkReadRecord(pNdb, recordNo, numRecords, lm, lmused);

  if (readRc == NDBT_OK) {
    /* Now traverse operations added, requesting
     * LockHandles
     */
    const NdbOperation *definedOp =
        (prevOp) ? prevOp->next() : pTrans->getFirstDefinedOperation();
...
int HugoOperations::pkUnlockRecord(Ndb *pNdb,
  ...
  if (lockHandles.size() < (unsigned)(offset + numRecords)) {
    g_err << "ERROR : LockHandles size is " << lockHandles.size() << " offset ("
          << offset << ") and/or numRecords (" << numRecords << ") too large."
          << endl;
    return NDBT_FAILED;
  }
  ...
  for (int i = 0; i < numRecords; i++) {
    ...
    if (lh != NULL) {
      ...
      g_err << "ERROR : LockHandle number " << i + offset << " is NULL. "
...
int HugoOperations::pkUpdateRecord(Ndb *pNdb, int recordNo, int numRecords,
                                   int updatesValue) {
  allocRows(numRecords);
  int check;
  for (int r = 0; r < numRecords; r++) {
    ...
    Uint32 partId;
    if (getPartIdForRow(pOp, r + recordNo, partId)) pOp->setPartitionId(partId);
...
int HugoOperations::setValues(NdbOperation *pOp, int rowId, int updateId) {
  // Define primary keys
  if (equalForRow(pOp, rowId) != 0) {
    g_err << __LINE__ << " equal for row failed" << endl;
    return NDBT_FAILED;
...
int HugoOperations::setNonPkValues(NdbOperation *pOp, int rowId, int updateId) {
...
int HugoOperations::pkInsertRecord(Ndb *pNdb, int recordNo, int numRecords,
                                   int updatesValue, int row_step) {
  int check;
  for (int r = 0; r < numRecords; r++) {
    ...
    if (setValues(pOp, (r * row_step) + recordNo, updatesValue) != NDBT_OK) {
    ...
    Uint32 partId;
    if (getPartIdForRow(pOp, (r * row_step) + recordNo, partId))
      pOp->setPartitionId(partId);
  }
  return NDBT_OK;
}

int HugoOperations::pkWriteRecord(Ndb *pNdb, int recordNo, int numRecords,
                                  int updatesValue) {
  int a, check;
  for (int r = 0; r < numRecords; r++) {
...
int HugoOperations::pkWritePartialRecord(Ndb *pNdb, int recordNo,
                                         int numRecords) {
  int check;
  for (int r = 0; r < numRecords; r++) {
    NdbOperation *pOp = pTrans->getNdbOperation(tab.getName());
...
int HugoOperations::pkDeleteRecord(Ndb *pNdb, int recordNo, int numRecords,
                                   int step) {
  int check;
...
int HugoOperations::pkRefreshRecord(Ndb *pNdb, int recordNo, int numRecords,
                                    int anyValueInfo) {
  ...
  for (int r = 0; r < numRecords; r++) {
    ...
    opts.anyValue = anyValueInfo ? (anyValueInfo << 16) | (r + recordNo) : 0;

    const NdbOperation *pOp =
        pTrans->refreshTuple(record, buffer, &opts, sizeof(opts));
    if (pOp == NULL) {
      NDB_ERR(pTrans->getNdbError());
      setNdbError(pTrans->getNdbError());
      return NDBT_FAILED;
...
int HugoOperations::execute_Commit(Ndb *pNdb, AbortOption eao) {
  int check = 0;
...
void HugoOperations_async_callback(int res, NdbTransaction *pCon, void *ho) {
  ((HugoOperations *)ho)->callback(res, pCon);
}

void HugoOperations::callback(int res, NdbTransaction *pCon) {
  require(pCon == pTrans);
  m_async_reply = 1;
  if (res) {
...
int HugoOperations::execute_async(Ndb *pNdb, NdbTransaction::ExecType et,
                                  NdbOperation::AbortOption eao) {
  m_async_reply = 0;
  pTrans->executeAsynchPrepare(et, HugoOperations_async_callback, this, eao);
...
int HugoOperations::execute_async_prepare(Ndb *pNdb,
...
int HugoOperations::wait_async(Ndb *pNdb, int timeout) {
...
int HugoOperations::equalForRow(NdbOperation *pOp, int row) {
  for (int a = 0; a < tab.getNoOfColumns(); a++) {
...
bool HugoOperations::getPartIdForRow(const NdbOperation *pOp, int rowid,
                                     Uint32 &partId) {
...
int HugoOperations::equalForAttr(NdbOperation *pOp, int attrId, int rowId) {
...
int HugoOperations::setValueForAttr(NdbOperation *pOp, int attrId, int rowId,
                                    int updateId) {
  const NdbDictionary::Column *attr = tab.getColumn(attrId);
  ...
  if (!(attr->getType() == NdbDictionary::Column::Blob)) {
    ...
    Uint32 real_len;
    const char *value =
        calc.calcValue(rowId, attrId, updateId, buf, len, &real_len);
...
int HugoOperations::verifyUpdatesValue(int updatesValue, int _numRows) {
  ...
  for (int i = 0; i < _numRows; i++) {
    ...
    if (calc.getUpdatesValue(rows[i]) != updatesValue) {
      ...
      g_err << "Invalid updates value for row " << i << endl
            ...
            << " calc.getUpdatesValue: " << calc.getUpdatesValue(rows[i])
            << endl
            << rows[i]->c_str().c_str() << endl;
      continue;
    }
...
BaseString HugoOperations::getRecordStr(int recordNum) {
  if (recordNum > (int)rows.size()) return NULL;
  return rows[recordNum]->c_str();
}

int HugoOperations::getRecordGci(int recordNum) { return pTrans->getGCI(); }
...
int HugoOperations::indexReadRecords(Ndb *, const char *idxName, int recordNo,
  ...
  for (int r = 0; r < numRecords; r++) {
    ...
    // Define attributes to read
    for (a = 0; a < tab.getNoOfColumns(); a++) {
      if ((rows[r]->attributeStore(a) = pOp->getValue(tab.getColumn(a))) == 0) {
        NDB_ERR(pTrans->getNdbError());
        setNdbError(pTrans->getNdbError());
        return NDBT_FAILED;
      }
...
int HugoOperations::indexUpdateRecord(Ndb *, const char *idxName, int recordNo,
                                      int numRecords, int updatesValue) {
  int a;
  allocRows(numRecords);
  int check;
  for (int r = 0; r < numRecords; r++) {
...
int HugoOperations::scanReadRecords(Ndb *pNdb, NdbScanOperation::LockMode lm,
                                    int records) {
  allocRows(records);
  ...
  for (int a = 0; a < tab.getNoOfColumns(); a++) {
    if ((rows[0]->attributeStore(a) = pOp->getValue(tab.getColumn(a))) == 0) {
      NDB_ERR(pTrans->getNdbError());
      setNdbError(pTrans->getNdbError());
      return NDBT_FAILED;
    }
...
int HugoOperations::releaseLockHandles(
    Ndb *pNdb, Vector<const NdbLockHandle *> &lockHandles, int offset,
    int numRecords) {
...
const NdbError &HugoOperations::getNdbError() const {
  update(m_error);
  return m_error;
}

void HugoOperations::setNdbError(const NdbError &error) {
  m_error.code = error.code ? error.code : 1;
}

void HugoOperations::setAnyValueCallback(AnyValueCallback avc) {
  avCallback = avc;
}

Uint32 HugoOperations::getAnyValueForRowUpd(int row, int update) {
  if (avCallback == NULL) return 0;

  return (avCallback)(pTrans->getNdb(), pTrans, row, update);
}
