...
HugoTransactions::HugoTransactions(const NdbDictionary::Table &table,
    ...
    : HugoOperations(table, index), row(table) {
...
int HugoTransactions::scanReadRecords(Ndb *pNdb, int records, int abortPercent,
                                      int parallelism,
                                      NdbOperation::LockMode lm, int scan_flags,
  ...
  while (true) {
    if (retryAttempt >= m_retryMax) {
      g_err << __LINE__ << " ERROR: has retried this operation " << retryAttempt
    ...
    if (abortPercent > 0) {
      ...
      if (abortCount < abortPercent) abortTrans = true;
    }
...
int HugoTransactions::scanReadRecords(
    Ndb *pNdb, const NdbDictionary::Index *pIdx, int records, int abortPercent,
    int parallelism, NdbOperation::LockMode lm, int scan_flags, int bound_cnt,
    const HugoBound *bound_arr) {
  int retryAttempt = 0;
  int check, a;
  NdbScanOperation *pOp;
  ...
  while (true) {
    ...
    if (pIdx != NULL) {
      pOp = pIxOp =
          pTrans->getNdbIndexScanOperation(pIdx->getName(), tab.getName());
    } else {
        ...
        switch (err.code) {
          ...
          default:
            if (err.classification == NdbError::TimeoutExpired) {
              ...
              } else if (retryAttempt >= (m_retryMax / 4) && (pIdx != 0)) {
                pIdx = NULL;
                bound_cnt = 0;
...
int HugoTransactions::scanUpdateRecords(Ndb *pNdb,
                                        NdbScanOperation::ScanFlag flags,
                                        int records, int abortPercent,
                                        int parallelism) {
  int retryAttempt = 0;
  int check, a;
  NdbScanOperation *pOp;
  ...
  restart:
    ...
    if (pOp->readTuples(NdbOperation::LM_Exclusive, flags, parallelism)) {
    ...
    for (a = 0; a < tab.getNoOfColumns(); a++) {
      if ((row.attributeStore(a) =
               pOp->getValue(tab.getColumn(a)->getName())) == NULL) {
        NDB_ERR(pTrans->getNdbError());
        setNdbError(pTrans->getNdbError());
        closeTransaction(pNdb);
        return NDBT_FAILED;
    ...
    while ((check = pOp->nextResult(true)) == 0) {
      do {
        ...
        const int updates =
            calc.getUpdatesValue(&row) + (m_empty_update ? 0 : 1);
      ...
      if (check != -1) {
        ...
        if (check != -1) m_latest_gci = pTrans->getGCI();
        pTrans->restart();
      }
...
int HugoTransactions::scanUpdateRecords(Ndb *pNdb, int records,
                                        int abortPercent, int parallelism) {
  return scanUpdateRecords(pNdb, (NdbScanOperation::ScanFlag)0, records,
                           abortPercent, parallelism);
}

// Scan all records exclusive and update
// them one by one
int HugoTransactions::scanUpdateRecords1(Ndb *pNdb, int records,
                                         int abortPercent, int parallelism) {
  return scanUpdateRecords(pNdb, (NdbScanOperation::ScanFlag)0, records,
                           abortPercent, 1);
}
...
// records from db
int HugoTransactions::scanUpdateRecords2(Ndb *pNdb, int records,
                                         int abortPercent, int parallelism) {
  return scanUpdateRecords(pNdb, (NdbScanOperation::ScanFlag)0, records,
                           abortPercent, parallelism);
}

int HugoTransactions::scanUpdateRecords3(Ndb *pNdb, int records,
                                         int abortPercent, int parallelism) {
  return scanUpdateRecords(pNdb, (NdbScanOperation::ScanFlag)0, records,
                           abortPercent, parallelism);
}

int HugoTransactions::loadTable(Ndb *pNdb, int records, int batch,
                                bool allowConstraintViolation, int doSleep,
                                bool oneTrans, int value, bool abort,
                                bool abort_on_first_error, int row_step) {
...
int HugoTransactions::loadTableStartFrom(Ndb *pNdb, int startFrom, int records,
                                         int batch,
                                         bool allowConstraintViolation,
                                         int doSleep, bool oneTrans, int value,
                                         bool abort, bool abort_on_first_error,
                                         int row_step) {
  int check;
  int retryAttempt = 0;
  int retryMax = 5;
  ...
  for (int c = 0; c < records;) {
    ...
    if (c + batch > records) batch = records - c;
    key = c * row_step;
    if (retryAttempt >= retryMax) {
      g_err << "Record " << c << " could not be inserted, has retried "
    ...
    if (doSleep > 0) NdbSleep_MilliSleep(doSleep);
    ...
    if (pkInsertRecord(pNdb, key + startFrom, batch, value, row_step) !=
        NDBT_OK) {
      NDB_ERR(pTrans->getNdbError());
      setNdbError(pTrans->getNdbError());
      closeTransaction(pNdb);
    ...
    if (!oneTrans || (c + batch) >= records) {
      ...
      if (!abort) {
        check = pTrans->execute(Commit, AbortOnError);
        if (check != -1) pTrans->getGCI(&m_latest_gci);
        pTrans->restart();
      } else {
        check = pTrans->execute(NoCommit, AbortOnError);
        if (check != -1) {
    ...
    if (check == -1) {
      ...
      switch (err.status) {
        case NdbError::Success:
          NDB_ERR(err);
          g_err << "ERROR: NdbError reports success when transaction failed"
                << endl;
          setNdbError(err);
          return NDBT_FAILED;
  ...
  if (pTrans) closeTransaction(pNdb);
  return NDBT_OK;
}

int HugoTransactions::fillTable(Ndb *pNdb, int batch) {
  return fillTableStartFrom(pNdb, 0, batch);
}

int HugoTransactions::fillTableStartFrom(Ndb *pNdb, int startFrom, int batch) {
  int check;
  int retryFull = 0;
  int retryAttempt = 0;
  int retryMax = 5;
...
int HugoTransactions::pkReadRecords(Ndb *pNdb, int records, int batch,
                                    NdbOperation::LockMode lm, int _rand) {
  int reads = 0;
  int r = 0;
  int retryAttempt = 0;
  int check;
  ...
  while (r < records) {
    if (r + batch > records) batch = records - r;
    ...
    bool timer_active = m_stats_latency != 0 &&
                        r >= batch &&          // first batch is "warmup"
                        r + batch != records;  // last batch is usually partial

    if (timer_active) timer_start = NdbTick_getCurrentTicks();
    ...
    if (check == -1) {
      ...
      if (indexScans.size() > 0) {
        ...
        if (check != 1 || rows_found > batch) {
          ...
          if (batch == 1) {
            g_info << r << ": not found" << endl;
            abort();
          } else
            g_info << "Found " << rows_found << " of " << batch << " rows"
                   << endl;
        }
        r += batch;
        reads += rows_found;
        ...
        for (int b = 0; (b < batch) && (r + b < records); b++) {
          if (calc.verifyRowValues(rows[b]) != 0) {
            closeTransaction(pNdb);
            g_err << "Line: " << __LINE__ << " verify row failed"
                  << ", record: " << r << " of: " << records << ", row: " << b
                  << " in a batch of: " << batch << endl;
            return NDBT_FAILED;
          }
          reads++;
          r++;
...
int HugoTransactions::pkUpdateRecords(Ndb *pNdb, int records, int batch,
                                      int doSleep) {
  int updated = 0;
  int r = 0;
  int retryAttempt = 0;
  ...
  while (r < records) {
    ...
    if (indexScans.size() > 0) {
      ...
      for (Uint32 scanOp = 0; scanOp < indexScans.size(); scanOp++) {
        while ((check = indexScans[scanOp]->nextResult(true)) == 0) {
          do {
            ...
            int updates =
                calc.getUpdatesValue(rows[0]) + (m_empty_update ? 0 : 1);
          ...
          if (check != 2) break;
          if ((check = pTrans->execute(NoCommit, AbortOnError)) != 0) break;
      ...
      if (rows_found != batch) {
        g_err << "Incorrect num of rows found.  Expected " << batch
...
int HugoTransactions::pkInterpretedUpdateRecords(Ndb *pNdb, int records,
                                                 int batch) {
  int updated = 0;
  int r = 0;
  int retryAttempt = 0;
...
int HugoTransactions::pkDelRecords(Ndb *pNdb, int records, int batch,
                                   bool allowConstraintViolation, int doSleep,
                                   int start_record, int step) {
  int deleted = 0;
  int r = 0;
  int retryAttempt = 0;
  int check;
  ...
  while (r < records) {
    ...
    if (pkDeleteRecord(pNdb, row, batch, step) != NDBT_OK) {
      const NdbError err = pTrans->getNdbError();
      NDB_ERR(err);
      setNdbError(err);
      g_err << r << ": " << err.code << " " << err.message << endl;
...
int HugoTransactions::pkRefreshRecords(Ndb *pNdb, int startFrom, int count,
                                       int batch) {
  int r = 0;
  int retryAttempt = 0;

  g_info << "|- Refreshing records..." << startFrom << "-"
         << (startFrom + count) << " (batch=" << batch << ")" << endl;

  while (r < count) {
    if (r + batch > count) batch = count - r;
...
int HugoTransactions::pkReadUnlockRecords(Ndb *pNdb, int records, int batch,
  ...
  while (r < records) {
    ...
    NdbOperation::LockMode lmused;
    if (pkReadRecordLockHandle(pNdb, lockHandles, r, batch, lm, &lmused) !=
        NDBT_OK) {
      NDB_ERR(pTrans->getNdbError());
      closeTransaction(pNdb);
      return NDBT_FAILED;
    ...
    if (check == -1) {
      ...
      if (pkUnlockRecord(pNdb, lockHandles) != NDBT_OK) {
        closeTransaction(pNdb);
...
int HugoTransactions::lockRecords(Ndb *pNdb, int records, int percentToLock,
                                  int lockTime) {
  ...
  if (percentToLock <= 0) percentToLock = 1;
  double percentVal = (double)percentToLock / 100;
  int lockBatch = (int)(records * percentVal);
  if (lockBatch <= 0) lockBatch = 1;

  allocRows(lockBatch);

  while (r < records) {
    if (r + lockBatch > records) lockBatch = records - r;
    ...
    if (tempErr) continue; /* Retry lock attempt */
...
int HugoTransactions::indexReadRecords(Ndb *pNdb, const char *idxName,
  ...
  const bool ordered =
      (pIndex->getType() == NdbDictionary::Index::OrderedIndex);
...
int HugoTransactions::indexUpdateRecords(Ndb *pNdb, const char *idxName,
                                         int records, int batch) {
  int updated = 0;
  int r = 0;
  int retryAttempt = 0;
