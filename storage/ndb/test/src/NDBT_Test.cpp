...
int NDBT_Context::getNumRecords() const { return records; }

int NDBT_Context::getNumLoops() const { return loops; }
...
Uint32 NDBT_Context::getProperty(const char *_name, Uint32 _default) {
  ...
  if (!props.get(_name, &val)) val = _default;
...
bool NDBT_Context::getPropertyWait(const char *_name, Uint32 _waitVal) {
  ...
  while ((!props.get(_name, &val) ||
          (props.get(_name, &val) && val != _waitVal)) &&
         !stopped)
    NdbCondition_Wait(propertyCondPtr, propertyMutexPtr);
  result = (val == _waitVal);
  NdbMutex_Unlock(propertyMutexPtr);
  return stopped;
}
...
const char *NDBT_Context::getPropertyWait(const char *_name,
                                          const char *_waitVal) {
  const char *val;
  NdbMutex_Lock(propertyMutexPtr);
...
void NDBT_Context::decProperty(const char *name) {
  NdbMutex_Lock(propertyMutexPtr);
  Uint32 val = 0;
...
void NDBT_Context::incProperty(const char *name) {
  NdbMutex_Lock(propertyMutexPtr);
  Uint32 val = 0;
  props.get(name, &val);
  props.put(name, (val + 1), true);
...
Uint32 NDBT_Context::casProperty(const char *name, Uint32 oldValue,
                                 Uint32 newValue) {
  NdbMutex_Lock(propertyMutexPtr);
  Uint32 val = 0;
  props.get(name, &val);
...
void NDBT_Context::wait_timeout(int msec) {
  NdbMutex_Lock(propertyMutexPtr);
  NdbCondition_WaitTimeout(propertyCondPtr, propertyMutexPtr, msec);
  NdbMutex_Unlock(propertyMutexPtr);
}
...
const NdbDictionary::Table **NDBT_Context::getTables() {
...
void NDBT_Context::setNumRecords(int _records) { records = _records; }

void NDBT_Context::setNumLoops(int _loops) { loops = _loops; }

void NDBT_Context::getRecordSubRange(int records, int rangeCount, int rangeId,
...
int NDBT_Step::setUp(Ndb_cluster_connection &con) {
  switch (testcase->getDriverType()) {
    ...
    case NdbApiDriver: {
      ...
      Ndb_internal::setForceShortRequests(m_ndb, m_ctx->suite->getForceShort());
...
int NDBT_Step::execute(NDBT_Context *ctx) {
  ...
  g_info << "  |- " << name << " started ["
         << ctx->suite->getDate(buf, sizeof(buf)) << "]" << endl;
...
NDBT_ParallelStep::NDBT_ParallelStep(NDBT_TestCase *ptest, const char *pname,
                                     NDBT_TESTFUNC *pfunc, int num, int count)
    : NDBT_Step(ptest, pname, pfunc) {
  require(num < count);
  step_type_no = num;
  step_type_count = count;
}
NDBT_Verifier::NDBT_Verifier(NDBT_TestCase *ptest, const char *pname,
                             NDBT_TESTFUNC *pfunc)
    : NDBT_Step(ptest, pname, pfunc) {}
NDBT_Initializer::NDBT_Initializer(NDBT_TestCase *ptest, const char *pname,
                                   NDBT_TESTFUNC *pfunc)
    : NDBT_Step(ptest, pname, pfunc) {}
NDBT_Finalizer::NDBT_Finalizer(NDBT_TestCase *ptest, const char *pname,
                               NDBT_TESTFUNC *pfunc)
    : NDBT_Step(ptest, pname, pfunc) {}
...
NDBT_TestCaseImpl1::~NDBT_TestCaseImpl1() {
  ...
  unsigned i;
  for (i = 0; i < initializers.size(); i++) delete initializers[i];
  initializers.clear();
  for (i = 0; i < verifiers.size(); i++) delete verifiers[i];
  verifiers.clear();
  for (i = 0; i < finalizers.size(); i++) delete finalizers[i];
  finalizers.clear();
  for (i = 0; i < steps.size(); i++) delete steps[i];
  steps.clear();
  results.clear();
  for (i = 0; i < testTables.size(); i++) delete testTables[i];
  testTables.clear();
  for (i = 0; i < testResults.size(); i++) delete testResults[i];
  testResults.clear();
}
...
extern "C" void *runStep_C(void *s) {
  runStep(s);
  return NULL;
}
...
void NDBT_TestCaseImpl1::waitSteps() {
  ...
  while (numStepsCompleted != steps.size())
    NdbCondition_Wait(waitThreadsCondPtr, waitThreadsMutexPtr);
...
int NDBT_TestCaseImpl1::getNoOfRunningSteps() const {
  return steps.size() - getNoOfCompletedSteps();
}

int NDBT_TestCaseImpl1::getNoOfCompletedSteps() const {
...
void NDBT_TestCase::startTimer(NDBT_Context *ctx) { timer.doStart(); }

void NDBT_TestCase::stopTimer(NDBT_Context *ctx) { timer.doStop(); }
...
void NDBT_TestSuite::setCreateTable(bool _flag) { m_createTable = _flag; }

void NDBT_TestSuite::setRunAllTables(bool _flag) { runonce = _flag; }
void NDBT_TestSuite::setCreateAllTables(bool _flag) { m_createAll = _flag; }
void NDBT_TestSuite::setConnectCluster(bool _flag) {
  require(m_createTable == false);
  m_connect_cluster = _flag;
}

void NDBT_TestSuite::setTemporaryTables(bool val) { temporaryTables = val; }

bool NDBT_TestSuite::getTemporaryTables() const { return temporaryTables; }

void NDBT_TestSuite::setLogging(bool val) { m_logging = val; }

bool NDBT_TestSuite::getLogging() const { return m_logging; }

bool NDBT_TestSuite::getForceShort() const { return m_forceShort; }
...
NDBT_TestCase *NDBT_TestSuite::findTest(const char *testname, bool explicitOK) {
  for (unsigned i = 0; i < tests.size(); i++) {
    if (native_strcasecmp(tests[i]->getName(), testname) == 0) return tests[i];
  }

  if (explicitOK == false) return 0;
...
int NDBT_TestSuite::executeAll(Ndb_cluster_connection &con,
  ...
  if (!runonce) {
    ...
    if (_testname == NULL) {
      for (unsigned i = 0; i < tests.size(); i++) {
        if (opt_stop_on_error != 0 && numTestsFail > 0) break;

        execute(con, tests[i], pTab);
      }
    } else {
      NDBT_TestCase *pt = findTest(_testname);
      if (pt != 0) execute(con, pt, pTab);
    }
  }
...
void NDBT_TestSuite::execute(Ndb_cluster_connection &con, NDBT_TestCase *pTest,
...
int NDBT_TestSuite::executeOne(Ndb_cluster_connection &con,
...
int NDBT_TestSuite::executeOneCtx(Ndb_cluster_connection &con,
                                  const NdbDictionary::Table *ptab,
                                  const char *_testname) {
  testSuiteTimer.doStart();
  do {
...
int NDBT_TestSuite::createHook(Ndb *ndb, NdbDictionary::Table &tab, int when) {
  if (when == 0) {
    if (diskbased) {
...
void NDBT_TestSuite::execute(Ndb_cluster_connection &con,
  ...
  if (_testname == NULL) {
    ...
    do {
      ...
      if (pt->m_all_tables && pt->m_has_run) break;

      if (pt->isVerify(pTab) == false) break;
...
static int runCreateTables(NDBT_Context *ctx, NDBT_Step *step) {
  NDBT_TestSuite *suite = ctx->getSuite();
  return suite->createTables(ctx->m_cluster_connection);
}

static int runCreateTable(NDBT_Context *ctx, NDBT_Step *step) {
...
static int runEnsureIndexStatTables(NDBT_Context *ctx, NDBT_Step *step) {
...
int NDBT_TestSuite::dropTables(Ndb_cluster_connection &con) const {
  Ndb ndb(&con, "TEST_DB");
  ndb.init(1);
...
static int runDropTables(NDBT_Context *ctx, NDBT_Step *step) {
  NDBT_TestSuite *suite = ctx->getSuite();
  return suite->dropTables(ctx->m_cluster_connection);
}

static int runDropTable(NDBT_Context *ctx, NDBT_Step *step) {
...
static int runCheckTableExists(NDBT_Context *ctx, NDBT_Step *step) {
  Ndb ndb(&ctx->m_cluster_connection, "TEST_DB");
  ndb.init(1);
  ...
  if (pDictTab == NULL) {
    g_err << "runCheckTableExists : Failed to find table " << tab_name << endl;
...
static int runEmptyDropTable(NDBT_Context *ctx, NDBT_Step *step) {
  return NDBT_OK;
}

int NDBT_TestSuite::report(const char *_tcname) {
  int result;
  char buf[64];  // For timestamp string
  ndbout << "Completed " << name << " [" << getDate(buf, sizeof(buf)) << "]"
         << endl;
  printTestCaseSummary(_tcname);
  ndbout << numTestsExecuted << " test(s) executed" << endl;
  ndbout << numTestsOk << " test(s) OK" << endl;
  if (numTestsSkipped > 0) {
    ndbout << numTestsSkipped << " test(s) skipped" << endl;
  }
  if (numTestsFail > 0) ndbout << numTestsFail << " test(s) failed" << endl;
...
void NDBT_TestSuite::printTestCaseSummary(const char *_tcname) {
  ...
  for (unsigned t = 0; t < tests.size(); t++) {
    if (_tcname != NULL && native_strcasecmp(tests[t]->getName(), _tcname) != 0)
      continue;
...
int NDBT_TestSuite::reportAllTables(const char *_testname) {
  int result;
  char buf[64];  // For timestamp string
  ndbout << "Completed running test [" << getDate(buf, sizeof(buf)) << "]"
         << endl;
  const int totalNumTests = numTestsExecuted;
  printTestCaseSummary(_testname);
  ndbout << numTestsExecuted << " test(s) executed" << endl;
  ndbout << numTestsOk << " test(s) OK("
         << (int)(((float)numTestsOk / totalNumTests) * 100.0) << "%)" << endl;
  if (numTestsSkipped > 0) {
...
static const char *load_default_groups[] = {
    "mysql_cluster", "NDBT", nullptr /* placeholder for program name */,
    nullptr};
...
static struct my_option my_long_options[] = {
    ...
    {"backup-password-from-stdin", NDB_OPT_NOSHORT,
     ...
     &opt_backup_password_from_stdin.opt_value, NULL, 0, GET_BOOL, NO_ARG, 0, 0,
     0, NULL, 0, &opt_backup_password_from_stdin},
    {"print", NDB_OPT_NOSHORT, "Print execution tree", &opt_print, &opt_print,
     0, GET_BOOL, NO_ARG, 0, 0, 0, 0, 0, 0},
    {"print_html", NDB_OPT_NOSHORT, "Print execution tree in html table format",
     &opt_print_html, &opt_print_html, 0, GET_BOOL, NO_ARG, 0, 0, 0, 0, 0, 0},
    {"print_cases", NDB_OPT_NOSHORT, "Print list of test cases",
     &opt_print_cases, &opt_print_cases, 0, GET_BOOL, NO_ARG, 0, 0, 0, 0, 0, 0},
    {"records", 'r', "Number of records", &opt_records, &opt_records, 0,
     GET_INT, REQUIRED_ARG, 1000, 0, 0, 0, 0, 0},
    {"loops", 'l', "Number of loops", &opt_loops, &opt_loops, 0, GET_INT,
     REQUIRED_ARG, 5, 0, 0, 0, 0, 0},
    {"seed", NDB_OPT_NOSHORT, "Random seed", &opt_seed, &opt_seed, 0, GET_UINT,
     REQUIRED_ARG, 0, 0, 0, 0, 0, 0},
    {"testname", 'n', "Name of test to run", &opt_testname, &opt_testname, 0,
     GET_STR, REQUIRED_ARG, 0, 0, 0, 0, 0, 0},
    {"timer", 't', "Print execution time", &opt_timer, &opt_timer, 0, GET_BOOL,
     NO_ARG, 0, 0, 0, 0, 0, 0},
    {"verbose", 'v', "Print verbose status", &opt_verbose, &opt_verbose, 0,
     GET_BOOL, NO_ARG, 0, 0, 0, 0, 0, 0},
    {"temporary-tables", 'T', "Create temporary table(s)", &opt_temporary,
     &opt_temporary, 0, GET_BOOL, NO_ARG, 0, 0, 0, 0, 0, 0},
    {"nologging", NDB_OPT_NOSHORT, "Create table(s) wo/ logging",
     &opt_nologging, &opt_nologging, 0, GET_BOOL, NO_ARG, 0, 0, 0, 0, 0, 0},
    {"noddl", NDB_OPT_NOSHORT,
     "Don't create/drop tables as part of running tests", &opt_noddl,
     &opt_noddl, 0, GET_BOOL, NO_ARG, 0, 0, 0, 0, 0, 0},
    {"forceshortreqs", NDB_OPT_NOSHORT, "Use short signals for NdbApi requests",
     &opt_forceShort, &opt_forceShort, 0, GET_BOOL, NO_ARG, 0, 0, 0, 0, 0, 0},
    {"stop-on-error", NDB_OPT_NOSHORT,
     "Don't run any more tests after one has failed", &opt_stop_on_error,
     &opt_stop_on_error, 0, GET_BOOL, NO_ARG, 0, 0, 0, 0, 0, 0},
...
int NDBT_TestSuite::execute(int argc, const char **argv) {
  ...
  */
  ...
  loops = opt_loops;
  timer = opt_timer;
  if (opt_nologging) setLogging(false);
  ...
  int num_tables = argc;
  if (argc == 0) num_tables = NDBT_Tables::getNumTables();
  ...
  if (m_createTable) {
    for (unsigned t = 0; t < tests.size(); t++) {
      ...
      NDBT_Initializer *pti =
          new NDBT_Initializer(pt, createFuncName, *createFunc);
      pt->addInitializer(pti, true);
      NDBT_Finalizer *ptf = new NDBT_Finalizer(pt, dropFuncName, *dropFunc);
      pt->addFinalizer(ptf);
    }
  ...
  if (m_ensureIndexStatTables && !m_noddl) {
    ...
    for (unsigned t = 0; t < tests.size(); t++) {
      NDBT_TestCaseImpl1 *pt = (NDBT_TestCaseImpl1 *)tests[t];
      NDBT_Initializer *pti = new NDBT_Initializer(
          pt, "runEnsureIndexStatTables", &runEnsureIndexStatTables);
      pt->addInitializer(pti, true);
    }
...
void NDBT_TestCaseImpl1::printHTML() {
  ...
  ndbout << "<td name=tc>" << endl
         << _name << "</td><td width=70%>" << _comment << "</td></tr>" << endl;
}
...
void NDBT_Step::print() { ndbout << "      " << name << endl; }
...
void NDBT_Context::sync_up_and_wait(const char *key, Uint32 value) {
  setProperty(key, value);
  getPropertyWait(key, (unsigned)0);
}

bool NDBT_Context::closeToTimeout(int safety) {
  ...
  if (m_env_timeout < 0) return false;

  Uint64 to = (1000 * m_env_timeout * (100 - safety)) / 100;
  Uint64 now = NdbTick_CurrentMillisecond();
  if (now >= m_test_start_time + to) {
    ndbout_c("closeToTimeout(%d) => true env(timeout): %d", safety,
             m_env_timeout);
    return true;
  }
...
NdbApiConfig const &NDBT_Context::getConfig() const {
