...
NDBT_Thread::NDBT_Thread() { create(0, -1); }

NDBT_Thread::NDBT_Thread(NDBT_ThreadSet *thread_set, int thread_no) {
  create(thread_set, thread_no);
}

void NDBT_Thread::create(NDBT_ThreadSet *thread_set, int thread_no) {
...
void NDBT_Thread::join() {
  NdbThread_WaitFor(m_thread, &m_status);
  m_thread = 0;
}

int NDBT_Thread::connect(class Ndb_cluster_connection *ncc, const char *db) {
  m_ndb = new Ndb(ncc, db);
  if (m_ndb->init() == -1 || m_ndb->waitUntilReady() == -1) {
...
void NDBT_ThreadSet::set_func(NDBT_ThreadFunc *func) {
  for (int n = 0; n < m_count; n++) {
    NDBT_Thread &thr = *m_thread[n];
    thr.set_func(func);
  }
...
void NDBT_ThreadSet::set_input(const void *input) {
  for (int n = 0; n < m_count; n++) {
    NDBT_Thread &thr = *m_thread[n];
    thr.set_input(input);
  }
...
void NDBT_ThreadSet::delete_output() {
  for (int n = 0; n < m_count; n++) {
    if (m_thread[n] != 0) {
      // NDBT_Thread& thr = *m_thread[n];
      // thr.delete_output();
...
int NDBT_ThreadSet::connect(class Ndb_cluster_connection *ncc, const char *db) {
  for (int n = 0; n < m_count; n++) {
    require(m_thread[n] != 0);
    NDBT_Thread &thr = *m_thread[n];
    if (thr.connect(ncc, db) == -1) return -1;
  }
  return 0;
}

void NDBT_ThreadSet::disconnect() {
  for (int n = 0; n < m_count; n++) {
    if (m_thread[n] != 0) {
      NDBT_Thread &thr = *m_thread[n];
      thr.disconnect();
...
int NDBT_ThreadSet::get_err() const {
  for (int n = 0; n < m_count; n++) {
    if (m_thread[n] != 0) {
      NDBT_Thread &thr = *m_thread[n];
      int err = thr.get_err();
