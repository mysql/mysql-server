...
static Uint32 myRand(Uint64 *seed) {
...
Int32 HugoCalculator::calcValue(int record, int attrib, int updates) const {
  Int32 i;
  Uint32 j;
...
static Uint32 calc_len(Uint32 rvalue, int maxlen) {
  Uint32 minlen = 25;
  ...
  if ((Uint32)maxlen <= minlen) return maxlen;

  /* Ensure coverage of maxlen */
  if ((rvalue & 64) == 0) return maxlen;

  return minlen + (rvalue % (maxlen - minlen));
}

static Uint32 calc_blobLen(Uint32 rvalue, int maxlen) {
  int minlen = 1000;
...
const char *HugoCalculator::calcValue(int record, int attrib, int updates,
                                      char *buf, int len,
                                      Uint32 *outlen) const {
  Uint64 seed;
  ...
  *outlen = len;
  ...
  switch (attr->getType()) {
    ...
    case NdbDictionary::Column::Char: {
      ...
      for (; len; len--, pos++) dst[pos] = base64_table[ptr[len] & 0x3f];
...
int HugoCalculator::verifyRowValues(NDBT_ResultRow *const pRow) const {
    ...
    if (i != m_updatesCol && i != m_idCol) {
      ...
      if (res == NULL) {
        if (!pRow->attributeStore(i)->isNULL()) {
          g_err << "|- NULL ERROR: expected a NULL but the column was not null"
                << endl;
        ...
        if (real_len != pRow->attributeStore(i)->get_size_in_bytes()) {
          ...
          result = -1;
        } else if (memcmp(res, pRow->attributeStore(i)->aRef(), real_len) !=
                   0) {
          g_err << "Column: " << attr->getName() << endl;
          const char *buf2 = pRow->attributeStore(i)->aRef();
          for (Uint32 j = 0; j < len; j++) {
            g_err << j << ":" << hex << (Uint32)(Uint8)buf[j] << "[" << hex
                  << (Uint32)(Uint8)buf2[j] << "]";
            if (buf[j] != buf2[j]) {
              g_err << "==>Match failed!";
            }
          ...
          g_err << endl;
          g_err << "|- Invalid data found in attribute " << i << ": \""
                << pRow->attributeStore(i)->aRef() << "\" != \"" << res << "\""
                << endl
...
int HugoCalculator::verifyRecAttr(int record, int updates,
                                  const NdbRecAttr *recAttr) {
  const char *valPtr = NULL;
  int attrib = recAttr->getColumn()->getAttrId();
  Uint32 valLen = recAttr->get_size_in_bytes();
  if (!recAttr->isNULL()) valPtr = (const char *)recAttr->aRef();

  return verifyColValue(record, attrib, updates, valPtr, valLen);
}

int HugoCalculator::verifyColValue(int record, int attrib, int updates,
    ...
    if (res == NULL) {
      ...
      if (real_len != valLen) {
        g_err << "|- Invalid data found in attribute " << attrib << ": \""
              << "Length of expected=" << real_len << endl
              << "Length of passed=" << valLen << endl;
        result = -1;
      } else if (memcmp(res, valPtr, real_len) != 0) {
        g_err << "|- Expected data mismatch on column " << attr->getName()
              << " length " << real_len << " bytes " << endl;
        g_err << "|- Bytewise comparison follows :" << endl;
        for (Uint32 j = 0; j < real_len; j++) {
          g_err << j << ":" << hex << (Uint32)(Uint8)buf[j] << "[" << hex
                << (Uint32)(Uint8)valPtr[j] << "]";
          if (buf[j] != valPtr[j]) {
            g_err << "==>Match failed!";
          }
          g_err << endl;
...
int HugoCalculator::getIdValue(NDBT_ResultRow *const pRow) const {
  return pRow->attributeStore(m_idCol)->u_32_value();
}

int HugoCalculator::getUpdatesValue(NDBT_ResultRow *const pRow) const {
  return pRow->attributeStore(m_updatesCol)->u_32_value();
}

int HugoCalculator::equalForRow(Uint8 *pRow, const NdbRecord *pRecord,
                                int rowId) {
  for (int attrId = 0; attrId < m_tab.getNoOfColumns(); attrId++) {
    ...
    if (attr->getPrimaryKey() == true) {
      ...
      Uint32 real_len;
      const char *value = calcValue(rowId, attrId, 0, buf, len, &real_len);
...
int HugoCalculator::setValues(Uint8 *pRow, const NdbRecord *pRecord, int rowId,
                              int updateVal) {
  ...
  for (int attrId = 0; attrId < m_tab.getNoOfColumns(); attrId++) {
    ...
    if (attr->getPrimaryKey() == false) {
      ...
      Uint32 real_len;
      const char *value =
          calcValue(rowId, attrId, updateVal, buf, len, &real_len);
      if (value != 0) {
        Uint32 off = 0;
        bool ret = NdbDictionary::getOffset(pRecord, attrId, off);
        if (!ret) abort();
...
int HugoCalculator::setKeyParts(HugoCalculator::KeyParts *keyParts, int rowid) {
