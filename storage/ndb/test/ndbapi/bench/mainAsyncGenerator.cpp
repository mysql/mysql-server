...
static int parse_args(int argc, char **argv) {
  ...
  while (i < argc) {
    if (strcmp("-proc", argv[i]) == 0) {
      ...
      if (sscanf(argv[i + 1], "%d", &numProcesses) == -1 || numProcesses <= 0 ||
          numProcesses > 127) {
      ...
      if (sscanf(argv[i + 1], "%d", &parallellism) == -1 || parallellism <= 0) {
      ...
      if (sscanf(argv[i + 1], "%d", &numSeconds) == -1 || numSeconds < 0) {
  ...
  if (minEventSendPoll > parallellism) {
    ndbout_c("minEventSendPoll(%d) > parallellism(%d)", minEventSendPoll,
             parallellism);
...
static void print_transaction(const char *header, unsigned long totalCount,
  ...
  ndbout_c(
      ...
      header, trans->count, (double)trans->count / (double)totalCount * 100.0,
      (int)trans->latency.getMean(), (int)trans->latency.getMin(),
      (int)trans->latency.getMax(), (int)trans->latency.getStddev(),
  ...
  if (printRollback) {
    ...
    ndbout_c("      Rollback Executed: %d (%.2f%%)", trans->rollbackExecuted,
             f);
  }
}

void print_stats(const char *title, unsigned int length,
                 unsigned int transactionFlag, GeneratorStatistics *gen,
                 int numProc, int parallellism) {
  int i;
  char buf[10];

  ndbout_c("\n------ %s ------", title);
  ndbout_c("Length        : %d %s", length,
  ...
  if (gen->totalTransactions == 0) {
    ...
    for (i = 0; i < 5; i++) {
      sprintf(buf, "T%d", i + 1);
      print_transaction(buf, gen->totalTransactions, &gen->transactions[i],
                        i >= 2, i >= 3);
    }
...
static void *threadRoutine(void *arg) {
  ...
  asyncGenerator(data, parallellism, millisSendPoll, minEventSendPoll,
                 forceSendPoll);
...
int main(int argc, char **argv) {
  ...
  ndbout_c("\nStarting Test with %d process(es) for %d %s parallellism %d",
           numProcesses, numSeconds, "sec", parallellism);
  ...
  data =
      (ThreadData *)malloc((numProcesses * parallellism) * sizeof(ThreadData));
  ...
  if (useNdbRecord) {
    ...
    ndbRecordSharedDataPtr =
        (NdbRecordSharedData *)malloc(sizeof(NdbRecordSharedData));
    ...
    if (prog1->add_val(IND_SERVER_READS, (Uint32)1) ||
        prog1->interpret_exit_ok() || prog1->finalise()) {
    ...
    if (prog2->add_val(IND_SERVER_INSERTS, (Uint32)1) ||
        prog2->interpret_exit_ok() || prog2->finalise()) {
    ...
    if (prog3->add_val(IND_SERVER_DELETES, (Uint32)1) ||
        prog3->interpret_exit_ok() || prog3->finalise()) {
  ...
  for (i = 0; i < numProcesses; i++) {
    ...
    pThread = NdbThread_Create(threadRoutine, (void **)&data[i * parallellism],
                               65535, threadName, NDB_THREAD_PRIO_LOW);
    if (pThread) {
  ...
  for (i = 0; i < numProcesses; i++) {
    for (k = 0; k < parallellism; k++) {
      ...
      for (j = 0; j < NUM_TRANSACTION_TYPES; j++) {
        ...
        stats.transactions[j].latency += p->transactions[j].latency;
      }
    }
  }

  print_stats("Test Results", numSeconds, 0, &stats, numProcesses,
              parallellism);
...
void asyncDbDisconnect(Ndb *pNDB) { delete pNDB; }
...
void showTime() {
  ...
  BaseString::snprintf(buf, 128, "%d-%.2d-%.2d %.2d:%.2d:%.2d",
