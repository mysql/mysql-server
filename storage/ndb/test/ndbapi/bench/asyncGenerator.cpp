...
static void doOneTransaction(ThreadData *td, int parallellism,
                             int millisSendPoll, int minEventSendPoll,
                             int forceSendPoll);
...
static SequenceValues transactionDefinition[] = {{25, 1}, {25, 2}, {20, 3},
                                                 {15, 4}, {15, 5}, {0, 0}};

static SequenceValues rollbackDefinition[] = {{98, 0}, {2, 1}, {0, 0}};
...
static int listEmpty(SessionList *list) { return (list->numberInList == 0); }

static void insertSession(SessionList *list, SubscriberNumber number,
                          ServerId serverId) {
  SessionElement *e;
  ...
  if (list->numberInList > maxsize) maxsize = list->numberInList;
}
...
static void initGeneratorStatistics(GeneratorStatistics *gen) {
  ...
  if (initSequence(&gen->transactionSequence, transactionDefinition) != 0) {
    ndbout_c("could not set the transaction types");
    exit(0);
  }

  if (initSequence(&gen->rollbackSequenceT4, rollbackDefinition) != 0) {
    ndbout_c("could not set the rollback sequence");
    exit(0);
  }

  if (initSequence(&gen->rollbackSequenceT5, rollbackDefinition) != 0) {
    ndbout_c("could not set the rollback sequence");
    exit(0);
  }
...
static void doOneTransaction(ThreadData *td, int p, int millis, int minEvents,
                             int force) {
  int i;
...
static void doTransaction_T1(Ndb *pNDB, ThreadData *td, int async) {
  ...
  BaseString::snprintf(td->transactionData.changed_time,
                       sizeof(td->transactionData.changed_time), "%ld - %u",
                       td->changedTime++, (unsigned)myRandom48(65536 * 1024));
...
static void doTransaction_T4(Ndb *pNDB, ThreadData *td, int async) {
  ...
  memset(td->transactionData.session_details + 2, myRandom48(26) + 'A',
         SESSION_DETAILS_LENGTH - 3);
...
void complete_T1(ThreadData *data) {
...
void complete_T2(ThreadData *data) {
...
void complete_T3(ThreadData *data) {
...
void complete_T4(ThreadData *data) {
  ...
  if (data->transactionData.branchExecuted &&
    ...
    insertSession(&data->generator.activeSessions, data->transactionData.number,
...
void complete_T5(ThreadData *data) {
...
****************************************************************
***************************************************************/
void asyncGenerator(ThreadData *data, int parallellism, int millisSendPoll,
  ...
  while (userGetTime() < periodStop) {
    doOneTransaction(startUp, parallellism, millisSendPoll, minEventSendPoll,
                     forceSendPoll);
  }
  ...
  while (userGetTime() < periodStop)
    doOneTransaction(data, parallellism, millisSendPoll, minEventSendPoll,
                     forceSendPoll);
