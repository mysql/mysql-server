...
static Parameter g_paramters[] = {
    ...
    // For range==3, Multiple index scans are used with a number of ranges
    // specified per scan (Number is defined by multi read range.
    ...
    {"multi read range", 1000, 1,
     UINT_MAX}  // Number of ranges to use in MRR access (range=3)
};
...
int main(int argc, const char **argv) {
  ...
  for (i = optind; i < argc; i++) {
    ...
    if (create_table()) goto error;
    if (g_paramters[P_CREATE].value != 2 && run_scan()) goto error;
  }
...
int create_table() {
  ...
  /* Obtain NdbRecord instances for the table and index */
  {
    ...
    g_table_record = dict->createRecord(
        g_table, &spec[0], cols, sizeof(NdbDictionary::RecordSpecification));
  ...
  }
  {
    ...
    g_index_record = dict->createRecord(
        g_index, &spec[0], cols, sizeof(NdbDictionary::RecordSpecification));
...
inline void err(NdbError e) { ndbout << e << endl; }

int setEqBound(NdbIndexScanOperation *isop, const NdbRecord *key_record,
               Uint32 value, Uint32 rangeNum) {
  Uint32 space[2];
  space[0] = value;
...
int run_scan() {
  ...
  for (int i = 0; i < iter; i++) {
    ...
    bool mrr =
        (g_paramters[P_ACCESS].value != 0) && (g_paramters[P_BOUND].value == 3);

    options.scan_flags |= (ord ? NdbScanOperation::SF_OrderBy : 0) |
    ...
    if (g_paramters[P_ACCESS].value == 0) {
      pOp = pTrans->scanTable(g_table_record, lm,
                              NULL,  // Mask
                              &options, sizeof(NdbScanOperation::ScanOptions));
      require(pOp);
    } else {
      pOp = pIOp =
          pTrans->scanIndex(g_index_record, g_table_record, lm,
                            NULL,  // Mask
    ...
    while ((check = pOp->nextResult(&result_row_ptr, true, false)) == 0) {
      do {
        rows++;
               ...
               ((check = pOp->nextResult(&result_row_ptr, false, false)) == 0));
      if (check == -1) {
        err(pTrans->getNdbError());
        return -1;
      }
