...
struct my_option my_long_options[] = {
    NdbStdOpt::ndb_connectstring, NdbStdOpt::connectstring,
    NdbStdOpt::ndb_nodeid, NdbStdOpt::end_of_options};
...
int main(int argc, char **argv) {
  ...
  for (int i = 0; i < (10 * g_loops); i++) {
    if (NDBT_OK != (res = testBitmask())) return NDBT_ProgramExit(res);
  }
  ...
  for (int i = 0; i < (argc ? argc : g_loops); i++) {
    ...
    if (transactions(pNdb, pTab)) break;

    if (unique_indexes(pNdb, pTab)) break;

    if (ordered_indexes(pNdb, pTab)) break;

    if (node_restart(pNdb, pTab)) break;

    if (system_restart(pNdb, pTab)) break;
...
static const NdbDictionary::Table *create_random_table(Ndb *pNdb) {
  do {
    ...
    for (Uint32 i = 0; i < cols && length > 2; i++) {
      ...
      for (Uint32 j = 0; j < len / 8; j++) defbuf[j] = 0x63;
...
static int transactions(Ndb *pNdb, const NdbDictionary::Table *tab) {
  int i = 0;
...
static int unique_indexes(Ndb *pNdb, const NdbDictionary::Table *tab) {
  return 0;
}

static int ordered_indexes(Ndb *pNdb, const NdbDictionary::Table *tab) {
  return 0;
}

static int node_restart(Ndb *pNdb, const NdbDictionary::Table *tab) {
  return 0;
}

static int system_restart(Ndb *pNdb, const NdbDictionary::Table *tab) {
  return 0;
}

/* Note : following classes test functionality of
 * storage/ndb/src/common/util/Bitmask.cpp and were originally defined there.
 * Set BITMASK_DEBUG to 1 to get more test debugging info.
 */
#define BITMASK_DEBUG 0

static bool cmp(const Uint32 b1[], const Uint32 b2[], Uint32 len) {
...
static void print(const Uint32 src[], Uint32 len, Uint32 pos = 0) {
  printf("b'");
...
static int lrand() { return rand(); }

static void rand(Uint32 dst[], Uint32 len) {
  for (Uint32 i = 0; i < len; i++)
    BitmaskImpl::set((len + 31) >> 5, dst, i, (lrand() % 1000) > 500);
}

static int checkCopyField(const Uint32 totalTests) {
  ndbout << "Testing : Checking Bitmaskimpl::copyField" << endl;
  ...
  for (Uint32 test = 0; test < totalTests; test++) {
    ...
    for (Uint32 i = 0; i < maxBitsToCopy; i++) {
      if (BitmaskImpl::get(numWords, targetCopy, i) !=
          BitmaskImpl::get(numWords, targetTest, i)) {
        ndbout_c("copyField :: Mismatch at bit %u, should be %u but is %u", i,
                 BitmaskImpl::get(numWords, targetTest, i),
...
static int checkNoTramplingGetSetField(const Uint32 totalTests) {
  const Uint32 numWords = 67;
  const Uint32 maxBitsToCopy = (numWords * 32);
  Uint32 sourceBuf[numWords];
  Uint32 targetBuf[numWords];

  ndbout << "Testing : Bitmask NoTrampling" << endl;
  ...
  for (Uint32 test = 0; test < totalTests; test++) {
    ...
    for (Uint32 word = 0; word < numWords; word++) {
      ...
      if (BITMASK_DEBUG)
        ndbout << "word=%d, targetWord=%u, firstUntrampledWord..=%u" << word
               << targetWord << firstUntrampledWord;

      if (!(word < firstUntrampledWord) ? (targetWord == 0)
                                        : (targetWord == 0xffffffff)) {
        ndbout << "Notrampling getField failed for srcStart " << srcStart
    ...
    for (Uint32 word = 0; word < numWords; word++) {
      ...
      for (Uint32 bit = 0; bit < 32; bit++) {
        ...
        bool expectedValue =
            !((bitNum >= srcStart) && (bitNum < (srcStart + length)));
        bool actualValue = (((targetWord >> bit) & 1) == 1);
        if (BITMASK_DEBUG)
          ndbout << "bitNum=%u expectedValue=%u, actual value=%u" << bitNum
                 << expectedValue << actualValue;

        if (actualValue != expectedValue) {
          ndbout << "Notrampling setField failed for srcStart " << srcStart
                 << " length " << length << " at word " << word << " bit "
                 << bit << "\n";
          ndbout << "bitNum=%u expectedValue=%u, actual value=%u" << bitNum
                 << expectedValue << actualValue;
          return -1;
...
static int simple(int pos, int size) {
...
static int testRanges(Uint32 bitmask_size) {
  ...
  for (int i = 0; i < 5000; i++) {
    ...
    if (BitmaskImpl::get(sz32, alloc_mask.getBase(), pos)) {
      ...
      if (BITMASK_DEBUG) {
        ...
        for (k = 0; k < a.data.size(); k++) printf("%.8x ", a.data[k]);
        printf("\n");
      }
      if (!cmp(tmp.getBase(), alloc_list[j].data.getBase(), max - min)) {
        return -1;
      }
      while (min < max) BitmaskImpl::clear(sz32, alloc_mask.getBase(), min++);
      alloc_list.erase(j);
    } else {
      Vector<Uint32> tmp;
      tmp.fill(sz32, zero);
...
#define NDB_BM_SUPPORT_RANGE
  ...
  for (Uint32 i = 0; i < 1000; i++) {
    ...
    for (Uint32 j = 0; j < sz; j++) {
      ...
      if (expect) BitmaskImpl::set(sz32, check.getBase(), j);
    }
    ...
    if (!BitmaskImpl::equal(sz32, map.getBase(), check.getBase())) {
      ...
      printf("check: ");
      for (Uint32 j = 0; j < sz32; j++) printf("%.8x ", check[j]);
      printf("\n");

      printf("map  : ");
      for (Uint32 j = 0; j < sz32; j++) printf("%.8x ", map[j]);
      printf("\n");
      return -1;
    }
    ...
    for (Uint32 j = 0; j < sz; j++) {
      bool expect = (j >= start && j < stop);
      if (expect) BitmaskImpl::clear(sz32, check.getBase(), j);
    }
  }
#endif
...
static int testBitmask() {
  ...
  if ((res = checkCopyField(1000)) != 0) return res;
