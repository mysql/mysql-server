...
int runTestApiConnectTimeout(NDBT_Context *ctx, NDBT_Step *step) {
  ...
  if (!mgmd.disconnect()) return NDBT_FAILED;
...
int runTestApiGetStatusTimeout(NDBT_Context *ctx, NDBT_Step *step) {
  ...
  for (int error_ins_no = 0; errs[error_ins_no] != -1; error_ins_no++) {
    ...
    if (error_ins != 0 && error_ins != 9 &&
        ndb_mgm_get_latest_error(h) != ETIMEDOUT) {
      ndbout << "FAILED: Incorrect error code (" << ndb_mgm_get_latest_error(h)
             << " != expected " << ETIMEDOUT
             << ") desc: " << ndb_mgm_get_latest_error_desc(h)
             << " line: " << ndb_mgm_get_latest_error_line(h)
...
int runTestMgmApiReadErrorRestart(NDBT_Context *ctx, NDBT_Step *step) {
  ...
  for (int i = 0; i < 100; i++) {
    ...
    if (i <= 6 && i > 2) {
      if (ndb_mgm_report_event(h, theData, 6))
        ndbout << "failed reporting event" << endl;
    ...
    if (i == 10) {
      ...
      if (need_disconnect) {
        ndbout << "Disconnecting to allow management server to restart." << endl
               << endl;
        ndb_mgm_disconnect(h);
      }
    }
...
static bool get_nodeid_of_type(NdbMgmd &mgmd, ndb_mgm_node_type type,
                               int *nodeId) {
  ndb_mgm_node_type node_types[2] = {type, NDB_MGM_NODE_TYPE_UNKNOWN};
...
static bool get_config_from_illegal_node(NdbMgmd &mgmd, int nodeId) {
  ...
  if (ndb_mgm_get_latest_error(mgmd.handle()) != NDB_MGM_GET_CONFIG_FAILED) {
    g_err << "ndb_mgm_get_configuration from illegal node " << nodeId
          << " not failed, error: " << ndb_mgm_get_latest_error(mgmd.handle())
          << " " << ndb_mgm_get_latest_error_msg(mgmd.handle()) << endl;
    return false;
  }
...
static bool check_get_config_illegal_node(NdbMgmd &mgmd) {
  ...
  if (nodeId == 0) return true;  // All nodes probably defined
...
static bool check_get_config_wrong_type(NdbMgmd &mgmd) {
  int myChoice = myRandom48(2);
  ndb_mgm_node_type randomAllowedType =
      (myChoice) ? NDB_MGM_NODE_TYPE_API : NDB_MGM_NODE_TYPE_MGM;
  int nodeId = 0;
  ...
  for (int l = 0; l < loops; l++) {
    ...
    if (get_nodeid_of_type(mgmd, NDB_MGM_NODE_TYPE_NDB, &nodeId)) {
      ndb_mgm::config_ptr conf(
          ndb_mgm_get_configuration_from_node(mgmd.handle(), nodeId));
      if (!conf) {
        g_err << "ndb_mgm_get_configuration_from_node " << nodeId
              << " failed, error: " << ndb_mgm_get_latest_error(mgmd.handle())
              << " " << ndb_mgm_get_latest_error_msg(mgmd.handle()) << endl;
        return NDBT_FAILED;
      }
    } else {
...
static bool get_nodeid(NdbMgmd &mgmd, const Properties &args,
  ...
  if (!call_args.contains("password")) call_args.put("password", "mysqld");
  if (!call_args.contains("public key"))
    call_args.put("public key", "a public key");
  if (!call_args.contains("name")) call_args.put("name", "testMgm");
  if (!call_args.contains("log_event")) call_args.put("log_event", 1);
  if (!call_args.contains("timeout")) call_args.put("timeout", 100);
  ...
  if (!call_args.contains("endian")) {
    ...
    endian_check.l = 1;
    call_args.put("endian",
                  (endian_check.c[sizeof(long) - 1]) ? "big" : "little");
  }

  if (!mgmd.call("get nodeid", call_args, "get nodeid reply", reply)) {
...
static const char *get_result(const Properties &reply) {
  const char *result;
...
static const char *get_message(const Properties &reply) {
  const char *message;
...
static bool get_nodeid_result_contains(NdbMgmd &mgmd, const Properties &args,
                                       const char *expected_result) {
  Properties reply;
  if (!get_nodeid(mgmd, args, reply)) return false;
  return result_contains(reply, expected_result);
}

static bool check_get_nodeid_invalid_endian1(NdbMgmd &mgmd) {
  union {
    long l;
    char c[sizeof(long)];
  } endian_check;
...
static bool check_get_nodeid_invalid_endian2(NdbMgmd &mgmd) {
  Properties args;
...
static bool check_get_nodeid_invalid_nodetype1(NdbMgmd &mgmd) {
  Properties args;
  args.put("nodetype", 37);
  return get_nodeid_result_contains(mgmd, args, "unknown nodetype 37");
}
...
static bool check_get_nodeid_invalid_nodeid(NdbMgmd &mgmd) {
  for (int nodeId = MAX_NODES; nodeId < MAX_NODES + 2; nodeId++) {
    ...
    if (!get_nodeid_result_contains(mgmd, args, expected.c_str())) return false;
  }
  return true;
}

static bool check_get_nodeid_dynamic_nodeid(NdbMgmd &mgmd) {
  bool result = true;
  Uint32 nodeId = 0;  // Get dynamic node id
  for (int nodeType = NDB_MGM_NODE_TYPE_MIN; nodeType < NDB_MGM_NODE_TYPE_MAX;
       nodeType++) {
    while (true) {
...
static bool check_get_nodeid_nonode(NdbMgmd &mgmd) {
  // Find a node that does not exist
  Config conf;
  if (!mgmd.get_config(conf)) return false;
...
static bool check_get_nodeid_wrong_nodetype(NdbMgmd &mgmd) {
  // Find a node that does exist
  Config conf;
  if (!mgmd.get_config(conf)) return false;
...
int runTestGetNodeId(NDBT_Context *ctx, NDBT_Step *step) {
  ...
  int result = NDBT_FAILED;
  if (check_get_nodeid_invalid_endian1(mgmd) &&
      check_get_nodeid_invalid_endian2(mgmd) &&
      check_get_nodeid_invalid_nodetype1(mgmd) &&
      check_get_nodeid_invalid_nodeid(mgmd) &&
      check_get_nodeid_dynamic_nodeid(mgmd) && check_get_nodeid_nonode(mgmd) &&
      //      check_get_nodeid_nodeid1(mgmd) &&
      check_get_nodeid_wrong_nodetype(mgmd) && true)
    result = NDBT_OK;

  if (!mgmd.end_session()) result = NDBT_FAILED;
...
int runSleepAndStop(NDBT_Context *ctx, NDBT_Step *step) {
  ...
  while (!ctx->isTestStopped() && counter--) NdbSleep_SecSleep(1);
  ;
  ctx->stopTest();
  return NDBT_OK;
}

static bool check_connection(NdbMgmd &mgmd) {
  Properties args, reply;
  mgmd.verbose(false);  // Verbose off
  bool result =
      mgmd.call("check connection", args, "check connection reply", reply);
  mgmd.verbose();  // Verbose on
  return result;
}

static bool check_transporter_connect(NdbMgmd &mgmd, const char *hello) {
  ...
  if (!mgmd.disconnect()) return false;
  if (!mgmd.connect()) return false;
...
static bool show_config(NdbMgmd &mgmd, const Properties &args,
                        Properties &reply) {
  if (!mgmd.call("show config", args, "show config reply", reply, NULL,
                 false)) {
...
int runCheckConfig(NDBT_Context *ctx, NDBT_Step *step) {
  ...
  Properties args1;
  Properties config1;
  if (!show_config(mgmd, args1, config1)) return NDBT_FAILED;
  ...
  for (unsigned i = 0; i < mgmds.size(); i++) {
    ...
    Properties args2;
    Properties config2;
    if (!show_config(mgmd, args2, config2)) return NDBT_FAILED;
    ...
    Config conf2;
    if (!mgmd.get_config(conf2)) return NDBT_FAILED;
...
static bool reload_config(NdbMgmd &mgmd, const Properties &args,
                          Properties &reply) {
  if (!mgmd.call("reload config", args, "reload config reply", reply)) {
...
static bool reload_config_result_contains(NdbMgmd &mgmd, const Properties &args,
                                          const char *expected_result) {
  Properties reply;
  if (!reload_config(mgmd, args, reply)) return false;
  return result_contains(reply, expected_result);
}

static bool check_reload_config_both_config_and_mycnf(NdbMgmd &mgmd) {
  Properties args;
...
static bool show_variables(NdbMgmd &mgmd, Properties &reply) {
  if (!mgmd.call("show variables", "", "show variables reply", reply)) {
...
static bool check_reload_config_invalid_config_filename(NdbMgmd &mgmd,
                                                        bool mycnf) {
...
int runTestReloadConfig(NDBT_Context *ctx, NDBT_Step *step) {
  ...
  if (!show_variables(mgmd, variables)) return NDBT_FAILED;

  variables.print();

  const char *mycnf_str;
  if (!variables.get("mycnf", &mycnf_str)) abort();
  bool uses_mycnf = (strcmp(mycnf_str, "yes") == 0);

  int result = NDBT_FAILED;
  if (check_reload_config_both_config_and_mycnf(mgmd) &&
      check_reload_config_invalid_config_filename(mgmd, uses_mycnf) && true)
    result = NDBT_OK;
...
static bool set_config(NdbMgmd &mgmd, const Properties &args,
  ...
  if (!mgmd.call(cmd_str, call_args, "set config reply", reply,
...
static bool set_config_result_contains(NdbMgmd &mgmd, const Properties &args,
                                       const BaseString &encoded_config,
                                       const char *expected_result) {
  Properties reply;
  if (!set_config(mgmd, args, encoded_config, reply)) return false;
  return result_contains(reply, expected_result);
}

static bool set_config_result_contains(NdbMgmd &mgmd, const Config &conf,
                                       const char *expected_result) {
  Properties reply;
  Properties args;
  ...
  bool v2 = ndb_config_version_v2(mgmd.get_version());
  bool ret =
      v2 ? conf.pack64_v2(encoded_config) : conf.pack64_v1(encoded_config);
  if (!ret) return false;
...
static bool check_set_config_invalid_content_type(NdbMgmd &mgmd) {
...
static bool check_set_config_invalid_content_encoding(NdbMgmd &mgmd) {
  g_info << __func__ << endl;
  Properties args;
...
static bool check_set_config_wrong_config_length(NdbMgmd &mgmd) {
  g_info << __func__ << endl;
...
static bool check_set_config_any_node(NDBT_Context *ctx, NDBT_Step *step,
                                      NdbMgmd &mgmd) {
  g_info << __func__ << endl;
  ...
  for (unsigned i = 0; i < mgmds.size(); i++) {
    ...
    // Get the binary config
    Config conf2;
    if (!mgmd2.get_config(conf2)) return false;

    // Set the modified config
    if (!mgmd2.set_config(conf2)) return false;

    // Check that all mgmds now have the new config
    if (runCheckConfig(ctx, step) != NDBT_OK) return false;
  }

  return true;
}

static bool check_set_config_fail_wrong_generation(NdbMgmd &mgmd) {
  g_info << __func__ << endl;
  // Get the binary config
  Config conf;
  if (!mgmd.get_config(conf)) return false;
  ...
  // Change generation
  if (!conf.setGeneration(conf.getGeneration() + 10)) return false;

  // Set the modified config
  return set_config_result_contains(mgmd, conf, "Invalid generation in");
}

static bool check_set_config_fail_wrong_name(NdbMgmd &mgmd) {
  g_info << __func__ << endl;
  // Get the binary config
  Config conf;
  if (!mgmd.get_config(conf)) return false;
  ...
  // Change name
  if (!conf.setName("NEWNAME")) return false;

  // Set the modified config
  return set_config_result_contains(mgmd, conf, "Invalid configuration name");
}

static bool check_set_config_fail_wrong_primary(NdbMgmd &mgmd) {
  g_info << __func__ << endl;
  // Get the binary config
  Config conf;
  if (!mgmd.get_config(conf)) return false;
  ...
  if (!conf.setPrimaryMgmNode(conf.getPrimaryMgmNode() + 10)) return false;

  // Set the modified config
  return set_config_result_contains(mgmd, conf, "Not primary mgm node");
}
...
int runTestSetConfig(NDBT_Context *ctx, NDBT_Step *step) {
  ...
  int result = NDBT_FAILED;
  if (check_set_config_invalid_content_type(mgmd) &&
      ...
      check_set_config_fail_wrong_primary(mgmd) && true)
    result = NDBT_OK;
...
int runTestSetConfigParallel(NDBT_Context *ctx, NDBT_Step *step) {
  ...
  */
  while (sucessful < loops && !ctx->isTestStopped() && result == NDBT_OK) {
    // Get the binary config
    Config conf;
    if (!mgmd.get_config(conf)) return NDBT_FAILED;
    ...
    if (mgmd.set_config(conf)) {
      ...
      if (error == "Invalid generation in configuration")
        invalid_generation++;
      else if (error == "Config change ongoing")
        config_change_ongoing++;
      else {
        g_err << "Config change failed with unexpected error: '" << error << "'"
              << endl;
        result = NDBT_FAILED;
      }
    }
  ...
  ndbout << "Thread " << step->getStepNo() << ", sucess: " << sucessful
...
static bool get_connection_parameter(NdbMgmd &mgmd, const Properties &args,
                                     Properties &reply) {
  // Fill in default values of other args
  Properties call_args(args);
  if (!call_args.contains("node1")) call_args.put("node1", 1);
  if (!call_args.contains("node2")) call_args.put("node2", 1);
...
static bool set_connection_parameter(NdbMgmd &mgmd, const Properties &args,
                                     Properties &reply) {
  // Fill in default values of other args
  Properties call_args(args);
  if (!call_args.contains("node1")) call_args.put("node1", 1);
  ...
  if (!call_args.contains("param"))
    call_args.put("param", CFG_CONNECTION_SERVER_PORT);
  if (!call_args.contains("value")) call_args.put("value", 37);
...
static bool check_connection_parameter_invalid_nodeid(NdbMgmd &mgmd) {
  for (int nodeId = MAX_NODES; nodeId < MAX_NODES + 2; nodeId++) {
    ...
    if (!get_connection_parameter(mgmd, args, get_result)) return false;

    if (!result_contains(get_result, "Unable to find connection between nodes"))
      return false;

    Properties set_result;
    if (!set_connection_parameter(mgmd, args, set_result)) return false;
...
static bool check_connection_parameter(NdbMgmd &mgmd) {
  ...
  for (int nodeId = 1; nodeId < MAX_NODES; nodeId++) {
    ...
    Properties result;
    if (!get_connection_parameter(mgmd, args, result)) return false;
...

  {
    ...
    Properties set_result;
    if (!set_connection_parameter(mgmd, set_args, set_result)) return false;

    if (!ok(set_result)) return false;
  }

  {
    ...
    Properties get_result;
    if (!get_connection_parameter(mgmd, get_args, get_result)) return false;

    if (!ok(get_result)) return false;
    ...
    if (new_value != "37") {
      g_err << "New value was not correct, expected 37, got " << new_value
            << endl;
      return false;
    }
  }
...

  {
    ...
    if (restored_value != original_value) {
      g_err << "Restored value was not correct, expected " << original_value
            << ", got " << restored_value << endl;
      return false;
    }
...
int runTestConnectionParameter(NDBT_Context *ctx, NDBT_Step *step) {
  ...
  int result = NDBT_FAILED;
  if (check_connection_parameter(mgmd) &&
      check_connection_parameter_invalid_nodeid(mgmd) && true)
    result = NDBT_OK;
...
static bool set_ports(NdbMgmd &mgmd, const Properties &args,
                      const char *bulk_arg, Properties &reply) {
  if (!mgmd.call("set ports", args, "set ports reply", reply, bulk_arg)) {
...
static bool check_set_ports_invalid_num_ports(NdbMgmd &mgmd) {
...
static bool check_set_ports_invalid_mismatch_num_port_1(NdbMgmd &mgmd) {
  ...
  Properties set_result;
  if (!set_ports(mgmd, args, "1=-37\n2=-38\n", set_result)) return false;

  if (ok(set_result)) return false;
  set_result.print();

  if (!result_contains(set_result, "expected empty line")) return false;

  return true;
}

static bool check_set_ports_invalid_mismatch_num_port_2(NdbMgmd &mgmd) {
  ...
  if (!result_contains(set_result, "expected name=value pair")) return false;

  return true;
}

static bool check_set_ports_invalid_port_list(NdbMgmd &mgmd) {
...
static bool check_mgmapi_err(NdbMgmd &mgmd, int return_code, int expected_error,
                             const char *expected_message) {
  if (return_code != -1) {
  ...
  if (mgmd.last_error() != expected_error) {
    ndbout_c(
        "check_mgmapi_error: unexpected error code: %d "
        "expected %d",
        mgmd.last_error(), expected_error);
    return false;
  }
  if (strstr(mgmd.last_error_message(), expected_message) == NULL) {
    ndbout_c(
        "check_mgmapi_error: last_error_message '%s' "
        "didn't contain expected message '%s'",
...
static bool check_set_ports_mgmapi(NdbMgmd &mgmd) {
...

  {
    ...
    NdbMgmd no_handle;
    ret =
        ndb_mgm_set_dynamic_ports(no_handle.handle(), nodeid, ports, num_ports);
  ...
  }
  {
    ...
    ret = ndb_mgm_set_dynamic_ports(no_con.handle(), nodeid, ports, num_ports);
  ...
  num_ports = 0;  // <<
  ret = ndb_mgm_set_dynamic_ports(mgmd.handle(), nodeid, ports, num_ports);
...
static bool check_set_ports(NdbMgmd &mgmd) {
  // Find a NDB node with dynamic port
  Config conf;
  if (!mgmd.get_config(conf)) return false;
  ...
  BaseString new_values;
  {
    ...
    for (unsigned i = 0; i < port_pairs.size(); i++) {
      ...
      if (sscanf(port_pairs[i].c_str(), "%d=%d", &nodeid, &port) != 2) {
        g_err << "Failed to parse port_pairs[" << i << "]: '" << port_pairs[i]
              << "'" << endl;
        return false;
      }
    ...
    Properties set_result;
    if (!set_ports(mgmd, args, new_values.c_str(), set_result)) return false;
  ...
  g_err << "Compare new values of dynamic ports" << endl;
  {
    ...
    if (current_values != new_values) {
      g_err << "Set values was not correct, expected " << new_values << ", got "
            << current_values << endl;
      return false;
    }
  }
  ...
  g_err << "Check restored values" << endl;
  {
    ...
    if (current_values != original_values) {
      g_err << "Restored values was not correct, expected " << original_values
            << ", got " << current_values << endl;
      return false;
    }
  }
...
int runTestSetPorts(NDBT_Context *ctx, NDBT_Step *step) {
  ...
  int result = NDBT_FAILED;
  if (check_set_ports(mgmd) && check_set_ports_invalid_nodeid(mgmd) &&
      ...
      check_set_ports_invalid_port_list(mgmd) && check_set_ports_mgmapi(mgmd) &&
      true)
    result = NDBT_OK;
...
#ifdef NOT_YET
static bool check_restart_connected(NdbMgmd &mgmd) {
  if (!mgmd.restart()) return false;
  return true;
}
...
int runTestRestartMgmd(NDBT_Context *ctx, NDBT_Step *step) {
  ...
  int result = NDBT_FAILED;
  if (check_restart_connected(mgmd) && true) result = NDBT_OK;
...
static bool set_logfilter(NdbMgmd &mgmd, enum ndb_mgm_event_severity severity,
                          int enable) {
  struct ndb_mgm_reply reply;
  if (ndb_mgm_set_clusterlog_severity_filter(mgmd.handle(), severity, enable,
                                             &reply) == -1) {
...
static bool get_logfilter(NdbMgmd &mgmd, enum ndb_mgm_event_severity severity,
                          unsigned int *value) {
  struct ndb_mgm_severity severity_struct;
  severity_struct.category = severity;
  if (ndb_mgm_get_clusterlog_severity_filter(mgmd.handle(), &severity_struct,
                                             1) != 1) {
...
int runTestSetLogFilter(NDBT_Context *ctx, NDBT_Step *step) {
  ...
  for (int i = 0; i < (int)NDB_MGM_EVENT_SEVERITY_ALL; i++) {
    ...
    if (!get_logfilter(mgmd, severity, &initial_value)) return NDBT_FAILED;

    // Turn level off
    if (!set_logfilter(mgmd, severity, 0)) return NDBT_FAILED;

    // Check it's off
    unsigned int curr_value;
    if (!get_logfilter(mgmd, severity, &curr_value)) return NDBT_FAILED;
    ...
    // Turn level on
    if (!set_logfilter(mgmd, severity, 1)) return NDBT_FAILED;
    ...
    if (!set_logfilter(mgmd, severity, initial_value)) return NDBT_FAILED;
  }
...
int runTestBug40922(NDBT_Context *ctx, NDBT_Step *step) {
  ...
  int r = ndb_logevent_get_next(le_handle, &le_event, 2000);
  ...
  if (r == 0) {
    ...
    if (r > 0)
      g_err << "ERROR: Receieved unexpected event: " << le_event.type << endl;
    if (r < 0)
...
int runTestBug45497(NDBT_Context *ctx, NDBT_Step *step) {
  ...
  while (true) {
    ...
    g_err << "Failed to make another connection, connections: " << mgmds.size()
          << endl;
...
int runTestBug16723708(NDBT_Context *ctx, NDBT_Step *step) {
  ...
  NdbLogEventHandle le_handle2 =
      ndb_mgm_create_logevent_handle(mgmd.handle(), filter);
  if (!le_handle2) return NDBT_FAILED;
  ...
  for (int l = 0; l < loops; l++) {
    ...
    int r2 = ndb_logevent_get_next2(le_handle2, &le_event2, 2000);
    ...
    if ((r == 0) || (r2 == 0)) {
      ...
      if (r > 0) {
        g_info << "next() ndb_logevent type : " << le_event.type
               ...
               << ndb_mgm_get_event_category_string(le_event.category) << endl;
      ...
      if (r2 > 0) {
        g_info << "next2() ndb_logevent type : " << le_event2.type
               ...
               << ndb_mgm_get_event_category_string(le_event2.category) << endl;
        ...
        g_err << "ERROR: ndb_logevent_get_next2 returned error: " << r << endl;
        result = NDBT_FAILED;
      }
    }
...
static int runTestGetVersion(NDBT_Context *ctx, NDBT_Step *step) {
  ...
  if (ndb_mgm_get_version(mgmd.handle(), &major, &minor, &build, sizeof(verStr),
                          verStr) != 1) {
  ...
  g_info << "Using major: " << major << " minor: " << minor
         << " build: " << build << " string: " << verStr << endl;
  ...
  while (l < loops) {
    ...
    if (ndb_mgm_get_version(mgmd.handle(), &major2, &minor2, &build2,
    ...
    if (major != major2) {
      g_err << "Got different major: " << major2 << " excpected: " << major
            << endl;
      return NDBT_FAILED;
    }

    if (minor != minor2) {
      g_err << "Got different minor: " << minor2 << " excpected: " << minor
            << endl;
      return NDBT_FAILED;
    }

    if (build != build2) {
      g_err << "Got different build: " << build2 << " excpected: " << build
            << endl;
      return NDBT_FAILED;
    }

    if (strcmp(verStr, verStr2) != 0) {
      g_err << "Got different verStr: " << verStr2 << " excpected: " << verStr
            << endl;
      return NDBT_FAILED;
    }
...
static int runTestGetVersionUntilStopped(NDBT_Context *ctx, NDBT_Step *step) {
  int result = NDBT_OK;
  while (!ctx->isTestStopped() &&
...
int runTestDumpEvents(NDBT_Context *ctx, NDBT_Step *step) {
  ...
  while (l < loops) {
    const Ndb_logevent_type supported[] = {
        NDB_LE_MemoryUsage, NDB_LE_BackupStatus, (Ndb_logevent_type)0};
...
int runTestStatusAfterStop(NDBT_Context *ctx, NDBT_Step *step) {
  ...
  int nodes[1] = {nodeId};
  int stopped =
      ndb_mgm_restart2(mgmd.handle(), NDB_ARRAY_SIZE(nodes), nodes, 0, 0, 1);
  if (stopped < 0) {
...
int sort_ng(const void *_e0, const void *_e1) {
  ...
  if (e0->node_group != e1->node_group) return e0->node_group - e1->node_group;
...
int runBug12928429(NDBT_Context *ctx, NDBT_Step *step) {
   ...
   */
  qsort(cs->node_states, cs->no_of_nodes, sizeof(cs->node_states[0]), sort_ng);
   ...
   */
  int res = ndb_mgm_restart4(mgmd.handle(), cnt, nodes, initial, nostart, abort,
                             force, &disconnnect);

  if (res == -1) {
    ndbout_c("%u res: %u ndb_mgm_get_latest_error: %u line: %u msg: %s",
             __LINE__, res, ndb_mgm_get_latest_error(mgmd.handle()),
...

  {
    ...
    for (int i = 0; i < cs2->no_of_nodes; i++) {
      ...
      if (cs2->node_states[i].node_status != expect) {
        ndbout_c("%u node %u expect: %u found: %u", __LINE__,
                 cs2->node_states[i].node_id, expect,
  ...
  res = ndb_mgm_restart4(mgmd.handle(), cnt, nodes, initial, nostart, abort,
                         force, &disconnnect);
...
int runTestNdbApiConfig(NDBT_Context *ctx, NDBT_Step *step) {
  ...
  struct test_parameter {
    ...
    Uint32 values[2];
      ...
      // Skip test of m_waitfor_timeout since it is not configurable in
      // API-section
      {CFG_DEFAULT_OPERATION_REDO_PROBLEM_ACTION,
       &NdbApiConfig::m_default_queue_option,
       {OPERATION_REDO_PROBLEM_ACTION_ABORT,
        OPERATION_REDO_PROBLEM_ACTION_QUEUE}},
  ...
  for (size_t i = 0; i < NDB_ARRAY_SIZE(parameters[0].values); i++) {
    ...
    // Set the modified config
    if (!mgmd.set_config(conf)) return NDBT_FAILED;
    ...
    for (size_t param = 0; param < NDB_ARRAY_SIZE(parameters); param++) {
      ...
      if (got != expected) {
        ...
        if (j < ConfigInfo::m_NoOfParams)
          g_err << ConfigInfo::m_ParamInfo[j]._fname << " ("
      ...
      if (failures > 0) return NDBT_FAILED;
    }
  }
...
static int runTestCreateLogEvent(NDBT_Context *ctx, NDBT_Step *step) {
  NdbMgmd mgmd;
  int loops = ctx->getNumLoops();
...
TESTCASE("ApiConnectTimeout", "Connect timeout tests for MGMAPI") {
  INITIALIZER(runTestApiConnectTimeout);
}
TESTCASE("ApiTimeoutBasic", "Basic timeout tests for MGMAPI") {
  INITIALIZER(runTestApiTimeoutBasic);
}
TESTCASE("ApiGetStatusTimeout", "Test timeout for MGMAPI getStatus") {
  INITIALIZER(runTestApiGetStatusTimeout);
}
TESTCASE("ApiGetConfigTimeout", "Test timeouts for mgmapi get_configuration") {
  INITIALIZER(runTestMgmApiGetConfigTimeout);
}
TESTCASE("ApiMgmEventTimeout", "Test timeouts for mgmapi get_configuration") {
...
TESTCASE("SetConfig", "Tests the ndb_mgm_set_configuration function") {
...
TESTCASE("TestReloadConfig", "Test of 'reload config'") {
  INITIALIZER(runTestReloadConfig);
}
TESTCASE("TestSetConfig", "Test of 'set config'") {
  INITIALIZER(runTestSetConfig);
}
TESTCASE("TestSetConfigParallel", "Test of 'set config' from 5 threads") {
  STEPS(runTestSetConfigParallel, 5);
}
TESTCASE("GetConfig", "Run ndb_mgm_get_configuration in parallel") {
  STEPS(runGetConfig, 64);
}
TESTCASE("TestStatus", "Test status and status2") {
  INITIALIZER(runTestStatus);
}
TESTCASE("TestStatusMultiple", "Test status and status2 with 64 threads") {
  /**
...
TESTCASE("TestGetNodeId", "Test 'get nodeid'") {
  INITIALIZER(runTestGetNodeId);
}
TESTCASE("TestGetVersion", "Test 'get version' and 'ndb_mgm_get_version'") {
  STEPS(runTestGetVersion, 20);
}
TESTCASE("TestTransporterConnect", "Test 'transporter connect'") {
  INITIALIZER(runTestTransporterConnect);
}
TESTCASE("TestConnectionParameter", "Test 'get/set connection parameter'") {
...
TESTCASE("Stress", "Run everything while changing config") {
...
TESTCASE("Stress2", "Run everything while changing config in parallel") {
  STEP(runTestGetNodeIdUntilStopped);
...
TESTCASE("TestDumpEvents", "Test 'dump events'") {
  STEPS(runTestDumpEvents, 1);
}
TESTCASE("TestStatusAfterStop", "Test get status after stop ") {
  STEPS(runTestStatusAfterStop, 1);
}
TESTCASE("Bug12928429", "") { STEP(runBug12928429); }
TESTCASE("TestNdbApiConfig", "") { STEP(runTestNdbApiConfig); }
TESTCASE("TestSetPorts", "Test 'set ports'") { INITIALIZER(runTestSetPorts); }
