...
static int runGracefulStopRestartNodesInNG0(NDBT_Context *ctx,
  ...
  if (numReplicas != 2) {
    ndbout_c("Incorrect replicas for test");
    return NDBT_SKIPPED;
  }

  const int stepNo = stepTypeNo;
  ...
  const int myNodeId = ng0members[stepTypeNo];
  ndbout_c("Step %u/%u restarting node id %u", stepNo, 2,
           myNodeId);
...
/* 7.6 specific workaround funcs for no getStepTypeNo et al */
static int runGracefulStopRestartNodesInNG0_A(NDBT_Context *ctx,
                                              NDBT_Step *step) {
  return runGracefulStopRestartNodesInNG0(ctx,
                                          step,
                                          0);
}

static int runGracefulStopRestartNodesInNG0_B(NDBT_Context *ctx,
                                              NDBT_Step *step) {
  return runGracefulStopRestartNodesInNG0(ctx,
                                          step,
                                          1);
...
TESTCASE("TestConcurrentGracefulStop",
  ...
  STEP(runGracefulStopRestartNodesInNG0_A);
  STEP(runGracefulStopRestartNodesInNG0_B);
