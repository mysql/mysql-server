...
/* ***************************************************
       ...
       Arguments:
        ...
        -sleep Sleep a number of seconds before running the test, this
               can be used so that another flexProgram have tome to create
tables
...
enum StartType { stIdle, stHammer, stStop, stLast };

enum MyOpType { otInsert, otRead, otDelete, otUpdate, otLast };
...
enum ReadyType { stReady, stRunning };
...
void resetThreads(ThreadNdb *threadArrayP) {
...
void waitForThreads(ThreadNdb *threadArrayP) {
  int cont = 1;
...
void tellThreads(ThreadNdb *threadArrayP, const StartType what) {
  for (int i = 0; i < tNoOfThreads; i++) {
  ...
  if (pMyNdb->waitUntilReady(10000) != 0) {
    ...
    if (check != 0) {
      ...
      for (int i = 0; i < tNoOfThreads; i++) {
        ...
        pThreads[i].threadLife =
            NdbThread_Create(flexHammerThread, (void **)&pThreads[i], 65535,
                             "flexHammerThread", NDB_THREAD_PRIO_LOW);
      }  // for
      ...
      if (returnValue == NDBT_OK) {
        ...
        for (;;) {
          ...
          if ((tNoOfLoops != 0) && (tNoOfLoops <= tLoops)) break;
          ...
          PRINT_TIMER("hammer", tNoOfOperations * tNoOfThreads,
                      tNoOfTables * 6);
...
extern "C" void *flexHammerThread(void *pArg) {
  ...
  int pkValue = 0;
  int readValue[MAXATTR][MAXATTRSIZE];
  std::memset(readValue, 0, sizeof(readValue));
  int attrValue[MAXATTRSIZE];
  NdbRecAttr *tTmp = NULL;
  int tNoOfAttempts = 0;

  for (i = 0; i < MAXATTRSIZE; i++) attrValue[i] = 0;
      ...
      for (count_round = 0; count_round < 5;) {
        ...
        for (count_tables = 0; count_tables < loop_count_tables;
          ...
          switch (tMyOpType) {
            case otInsert:  // Insert case
            case otUpdate:  // Update Case
              for (count_attributes = 1;
                   count_attributes < loop_count_attributes;
                   count_attributes++) {
                check = pMyOperation[count_tables]->setValue(
                    (char *)attrName[count_attributes], (char *)&attrValue[0]);
              }  // for
              break;
            case otRead:  // Read Case
              for (count_attributes = 1;
                   count_attributes < loop_count_attributes;
                   count_attributes++) {
                tTmp = pMyOperation[count_tables]->getValue(
        ...
        if (tThreadResult != 0) {
          ...
          if (check == -1) {
            ...
            int retCode = flexHammerErrorData->handleErrorCommon(
                pMyTransaction->getNdbError());
            if (retCode == 1) {
              // if (strcmp(pMyTransaction->getNdbError().message, "Tuple did
              // not exist") != 0 &&
              // strcmp(pMyTransaction->getNdbError().message,"Tuple already
              // existed when attempting to insert") != 0) ndbout_c("execute:
              // %s", pMyTransaction->getNdbError().message);

              if (pMyTransaction->getNdbError().code != 626 &&
                  pMyTransaction->getNdbError().code != 630) {
...
int readArguments(int argc, char **argv) {
  ...
  while (argc > 1) {
    if (strcmp(argv[i], "-t") == 0) {
      ...
      if ((tSleepTime < 1) || (tSleepTime > 3600)) exit(-1);
  ...
  ndbout << "Hammer ndb with read, insert, update and delete transactions"
         << endl
         << endl;
...
static int createTables(Ndb *pMyNdb) {
  ...
  if (theTableCreateFlag == 0) {
    for (i = 0; i < tNoOfTables; i++) {
      ...
      check = MySchemaOp->createAttribute((char *)attrName[0], TupleKey, 32, 1,
                                          NdbSchemaOp::UnSigned, MMBased,
                                          NotNullAttribute);
      if (check == -1) {
        // Clean up opened schema transaction
        NdbSchemaCon::closeSchemaTrans(MySchemaTransaction);
