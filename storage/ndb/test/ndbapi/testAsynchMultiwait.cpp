...
class NdbPool : private NdbLockable {
 public:
  NdbPool(Ndb_cluster_connection *_conn)
      : conn(_conn), list(0), size(0), created(0) {}
  Ndb *getNdb();
...
int runSetup(NDBT_Context *ctx, NDBT_Step *step) {
  ...
  if (hugoTrans.loadTableAsynch(pNdb, records, batchSize, transactions,
                                operations) != 0) {
    return NDBT_FAILED;
  }
...
int runCleanup(NDBT_Context *ctx, NDBT_Step *step) {
  ...
  HugoAsynchTransactions hugoTrans(*ctx->getTab());
  if (hugoTrans.pkDelRecordsAsynch(pNdb, records, batchSize, transactions,
                                   operations) != 0) {
    return NDBT_FAILED;
  }
...
int runPkReadMultiBasic(NDBT_Context *ctx, NDBT_Step *step) {
  ...
  while (i < loops) {
    ...
    do {
      ...
      while (lumpsize && recordsLeft && ndbcnt < MAX_NDBS) {
        ...
        /* Now send em off */
        trans->executeAsynchPrepare(NdbTransaction::Commit, NULL, NULL,
...
int runPkReadMultiWakeupT1(NDBT_Context *ctx, NDBT_Step *step) {
  ...
  if (phase != 0) {
    ndbout << "Thread 1 : Error, initial phase should be 0 not " << phase
           << endl;
    return NDBT_FAILED;
  };
...
int runPkReadMultiWakeupT2(NDBT_Context *ctx, NDBT_Step *step) {
  ...
  check(hugoOps.pkReadRecord(ndb, 0, 1, NdbOperation::LM_Exclusive) == 0,
        hugoOps);
  /* Prepare, Send */
  check(hugoOps.execute_async(ndb, NdbTransaction::Commit,
...
/* Producer thread */
int runV2MultiWait_Producer(NDBT_Context *ctx, NDBT_Step *step, int thd_id,
                            int nthreads) {
  int records = ctx->getNumRecords();
  HugoOperations hugoOps(*ctx->getTab());
  ...
  while (ctx->isTestStopped() == false) {
    ...
    switch (i % 5) {
      case 0:  // batch size = 2, random = 1
        r = hugoTrans.pkReadRecords(ndb, records / 20, 2, NdbOperation::LM_Read,
                                    1);
        break;
      case 1:
...
TESTCASE("AsynchMultiwaitPkRead", "Verify NdbWaitGroup API (1 thread)") {
  INITIALIZER(runSetup);
  STEP(runPkReadMultiBasic);
  FINALIZER(runCleanup);
}
TESTCASE("AsynchMultiwaitWakeup", "Verify wait-multi-ndb wakeup Api code") {
  INITIALIZER(runSetup);
...
TESTCASE("AsynchMultiwait_Version2", "Verify NdbWaitGroup API version 2") {
  INITIALIZER(runSetup);
