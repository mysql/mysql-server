...
const NdbDictionary::Table *getTable(Ndb *pNdb, int i) {
...
int createOrderedPkIndex_Drop(NDBT_Context *ctx, NDBT_Step *step) {
  ...
  // Drop index
  if (pNdb->getDictionary()->dropIndex(orderedPkIdxName, pTab->getName()) !=
      0) {
...
int runVerifyTable(NDBT_Context *ctx, NDBT_Step *step) { return NDBT_OK; }
  ...
  while (i < loops && !ctx->isTestStopped()) {
    ...
    if (tupscan == 1) {
      ...
      if (hugoTrans.scanReadRecords(GETNDB(step), records, abort, parallelism,
                                    ...
                                    scan_flags) != 0)
        return NDBT_FAILED;
    } else if (hugoTrans.scanReadRecords(
                   GETNDB(step), records, abort, parallelism,
...
int runRandScanRead(NDBT_Context *ctx, NDBT_Step *step) {
  ...
  while (i < loops && !ctx->isTestStopped()) {
    ...
    if (lmarg != ~0) lm = (NdbOperation::LockMode)lmarg;
    int scan_flags = 0;
    ...
    if (hugoTrans.scanReadRecords(GETNDB(step), records, abort, parallelism, lm,
                                  scan_flags) != 0) {
      return NDBT_FAILED;
    }
    i++;
...
int runScanReadIndex(NDBT_Context *ctx, NDBT_Step *step) {
  ...
  const NdbDictionary::Index *pIdx = GETNDB(step)->getDictionary()->getIndex(
  ...
  while (pIdx && i < loops && !ctx->isTestStopped()) {
    ...
    int scan_flags = (NdbScanOperation::SF_OrderBy & -(int)sort) |
                     (NdbScanOperation::SF_Descending & -(int)desc);
    if (hugoTrans.scanReadRecords(GETNDB(step), pIdx, records, abort,
                                  parallelism, lm, scan_flags) != 0) {
      return NDBT_FAILED;
    }
    i++;
  }
...
int runScanReadErrorOneNode(NDBT_Context *ctx, NDBT_Step *step) {
  ...
  while (i < loops && result == NDBT_OK) {
    ...
    for (int j = 0; j < 10; j++) {
      if (hugoTrans.scanReadRecords(GETNDB(step), records, 0, parallelism) !=
          0) {
...
int runScanReadUntilStopped(NDBT_Context *ctx, NDBT_Step *step) {
  ...
  HugoTransactions hugoTrans(*ctx->getTab());
  while (ctx->isTestStopped() == false) {
    if (parallelism == RANDOM_PARALLELISM) para = myRandom48(239) + 1;
...
int runScanReadUntilStoppedPrintTime(NDBT_Context *ctx, NDBT_Step *step) {
  ...
  while (ctx->isTestStopped() == false) {
    ...
    timer.doStart();
    g_info << i << ": ";
    if (ndb->waitUntilReady() != 0) return NDBT_FAILED;
    if (hugoTrans.scanReadRecords(GETNDB(step), records, 0, parallelism) != 0)
      return NDBT_FAILED;
    timer.doStop();
    if ((timer.elapsedTime() / 1000) > 1) timer.printTotalTime();
    i++;
  }
  return NDBT_OK;
}
...
int runScanUpdate2(NDBT_Context *ctx, NDBT_Step *step) {
  ...
  while (i < loops) {
    g_info << i << ": ";
    if (hugoTrans.scanUpdateRecords2(GETNDB(step), records, abort,
                                     parallelism) != 0) {
      return NDBT_FAILED;
    }
    i++;
...
int runCheckGetValue(NDBT_Context *ctx, NDBT_Step *step) {
  ...
  for (unsigned i = 0; i < alist.attriblist.size(); i++) {
    g_info << i << endl;
    if (utilTrans.scanReadRecords(GETNDB(step), parallelism,
                                  NdbOperation::LM_Read, records,
...
int runCloseWithoutStop(NDBT_Context *ctx, NDBT_Step *step) {
  ...
  for (int p = 1; p < 240; p++) {
    ...
    if (scanF.scanReadFunctions(GETNDB(step), records, p,
                                ScanFunctions::CloseWithoutStop, false) != 0) {
      numFailed++;
    }
    g_info << p << " CloseWithoutStop openScanExclusive" << endl;
    if (scanF.scanReadFunctions(GETNDB(step), records, p,
                                ScanFunctions::CloseWithoutStop, true) != 0) {
      numFailed++;
    }
  }
...
int runNextScanWhenNoMore(NDBT_Context *ctx, NDBT_Step *step) {
  ...
  int numFailed = 0;
  ScanFunctions scanF(*pTab);
  if (scanF.scanReadFunctions(GETNDB(step), records, 6,
                              ScanFunctions::NextScanWhenNoMore, false) != 0) {
    numFailed++;
  }
  if (scanF.scanReadFunctions(GETNDB(step), records, 6,
                              ScanFunctions::NextScanWhenNoMore, true) != 0) {
    numFailed++;
  }
...
int runOnlyOpenScanOnce(NDBT_Context *ctx, NDBT_Step *step) {
  ...
  if (scanF.scanReadFunctions(GETNDB(step), records, 6,
                              ScanFunctions::OnlyOpenScanOnce, false) == 0) {
    numFailed++;
  }
  g_info << "OnlyOpenScanOnce openScanExclusive" << endl;
  if (scanF.scanReadFunctions(GETNDB(step), records, 6,
                              ScanFunctions::OnlyOpenScanOnce, true) == 0) {
    numFailed++;
  }
...
int runNoCloseTransaction(NDBT_Context *ctx, NDBT_Step *step) {
  ...
  while (l < loops) {
    ...
    if (scanF.scanReadFunctions(GETNDB(step), records, 6,
                                ScanFunctions::NoCloseTransaction, true) != 0) {
      numFailed++;
    }
    l++;
  }
...
int runCheckInactivityTimeOut(NDBT_Context *ctx, NDBT_Step *step) {
  ...
  ScanFunctions scanF(*pTab);
  if (scanF.scanReadFunctions(GETNDB(step), records, 1,
                              ScanFunctions::CheckInactivityTimeOut,
                              false) != NDBT_OK) {
    numFailed++;
  }
  if (scanF.scanReadFunctions(GETNDB(step), records, 240,
                              ScanFunctions::CheckInactivityTimeOut,
                              true) != NDBT_OK) {
    numFailed++;
  }
...
int runCheckInactivityBeforeClose(NDBT_Context *ctx, NDBT_Step *step) {
  ...
  ScanFunctions scanF(*pTab);
  if (scanF.scanReadFunctions(GETNDB(step), records, 16,
...
int runScanParallelism(NDBT_Context *ctx, NDBT_Step *step) {
...
int runScanVariants(NDBT_Context *ctx, NDBT_Step *step) {
  // int loops = ctx->getNumLoops();
  // int records = ctx->getNumRecords();
  Ndb *pNdb = GETNDB(step);
  const NdbDictionary::Table *pTab = ctx->getTab();
    ...
    for (int flags = 0; flags < 4; flags++) {
      for (int batch = 0; batch < 100; batch += (1 + batch + (batch >> 3))) {
        for (int par = 0; par < 16; par += 1 + (rand() % 3)) {
          bool disk = flags & 1;
          bool tups = flags & 2;
          g_info << "lm: " << lm << " disk: " << disk << " tup scan: " << tups
                 << " par: " << par << " batch: " << batch << endl;
          ...
          if (pOp->readTuples((NdbOperation::LockMode)lm,
                              tups ? NdbScanOperation::SF_TupScan : 0, par,
                              batch) != 0) {
            NDB_ERR(pCon->getNdbError());
            return NDBT_FAILED;
          }
...
int runBug36124(NDBT_Context *ctx, NDBT_Step *step) {
  Ndb *pNdb = GETNDB(step);
  const NdbDictionary::Table *pTab = ctx->getTab();
...
int runBug24447(NDBT_Context *ctx, NDBT_Step *step) {
  ...
  while (i < loops && !ctx->isTestStopped()) {
    ...
    if (nodeId == -1) nodeId = restarter.getMasterNodeId();
  ...
  while (pIdx && i++ < loops && !ctx->isTestStopped()) {
...

    {
      ...
      while (cnt < 3) {
        ...
        if (p2->getConnectedNodeId() == (Uint32)nodeId) cnt++;
      }

      for (unsigned t = 0; t < translist.size(); t++) translist[t]->close();
      translist.clear();
    }

    NdbIndexScanOperation *pOp =
        pTrans->getNdbIndexScanOperation(pIdx, ctx->getTab());
...
int initBug42559(NDBT_Context *ctx, NDBT_Step *step) {
...
int finalizeBug42559(NDBT_Context *ctx, NDBT_Step *step) {
...
int runScanReadIndexWithBounds(NDBT_Context *ctx, NDBT_Step *step) {
   ...
   * If we run many of these in parallel we may exhaust the available KeyInfo
   * storage, which may expose some bugs.
   */
  while (pIdx && i < loops && !ctx->isTestStopped() &&
         NdbTick_CurrentMillisecond() < end) {
    g_info << "Step " << step->getStepNo() << "Loop : " << i << ": ";
    ...
    NdbIndexScanOperation *iso = trans->getNdbIndexScanOperation(
        pIdx->getName(), ctx->getTab()->getName());
    if (iso == NULL) {
      g_err << "Error obtaining IndexScanOperation : " << trans->getNdbError()
            << endl;
      trans->close();
      return NDBT_FAILED;
    }
    ...
    if (iso->readTuples(
            NdbOperation::LM_CommittedRead,
            (NdbScanOperation::SF_OrderBy | NdbScanOperation::SF_ReadRangeNo |
    ...
    for (int range = 0; range < numRanges; range++) {
      ...
      for (Uint32 k = 0; k < pIdx->getNoOfColumns(); k++) {
        ...
        if (iso->setBound(colName, NdbIndexScanOperation::BoundLE, NULL) != 0) {
          g_err << "Error setting bound for column %s. " << iso->getNdbError()
                << endl;
          trans->close();
          return NDBT_FAILED;
        }
    ...
    while ((eof = iso->nextResult(true)) == 0) {
      ...
      if (iso->get_range_no() != rangeNum) {
        g_err << "Expected row " << rows << " to be in range " << rangeNum
    ...
    if (eof != 1) {
      ...
      if (iso->getNdbError().status == NdbError::TemporaryError) {
        if (retries++ < maxRetries) {
          g_err << "Step " << step->getStepNo()
                << "  Temporary, retrying on iteration " << i
                << " rows so far : " << rows << endl;
          trans->close();
...
int runBug54945(NDBT_Context *ctx, NDBT_Step *step) {
  int loops = ctx->getNumLoops();
  const NdbDictionary::Table *pTab = ctx->getTab();
...
int runCloseRefresh(NDBT_Context *ctx, NDBT_Step *step) {
  Ndb *pNdb = GETNDB(step);
...
int runBug13394788(NDBT_Context *ctx, NDBT_Step *step) {
...
static int populateTable(NDBT_Context *ctx, NDBT_Step *step) {
...
static int doCheckSumQuery(NDBT_Context *ctx, NDBT_Step *step) {
  // Insert error.
   ...
   */
  g_info << "Inserting error " << errInsertNo << " in node " << nodeId << endl;
...
static int doInterpretNok6000Query(NDBT_Context *ctx, NDBT_Step *step) {
  // Build query.
  Ndb *const ndb = step->getNdb();
  ...
  // NdbScanOperation* const scanOp = trans->scanTable(record);
  NdbScanOperation *const scanOp = trans->scanTable(
      record, NdbOperation::LM_Read, NULL, &opts, sizeof(opts));
  require(scanOp != NULL);
  require(trans->execute(Commit) == 0);
    ...
    case -1:  // Error
    {
      ...
      if (errCode == 6000) {
        if (rowCount == 0) {
          ...
          g_err << "Got expected error 6000. Query returned " << rowCount
                << " rows out of " << totalRowCount << ". Exepected 0 rows."
                << endl;
          res = NDBT_FAILED;
        }
      } else {
 ...
 */
static int runExtraNextResult(NDBT_Context *ctx, NDBT_Step *step) {
  const NdbDictionary::Table *pTab = ctx->getTab();
  ...
  Uint32 maxTrans = 0;
  NdbConfig conf;
  require(conf.getProperty(conf.getMasterNodeId(), NODE_TYPE_DB,
                           CFG_DB_NO_TRANSACTIONS, &maxTrans));
  require(maxTrans > 0);
    ...
    for (int j = 0; j < scanCount; j++) {
      ...
      if (scan == NULL) {
        g_err << "trans->getNdbScanOperation() gave unexpected error : "
              << trans->getNdbError() << " in the " << i << "th iteration."
              << endl;
        return NDBT_FAILED;
      }
      ...
      if (scan->getNdbError().code != 4120 && result == NDBT_OK) {
 ...
 */
static int populateFragment0(Ndb *ndb, const NdbDictionary::Table *tab,
                             Uint32 rows, Uint32 dbaccBuckets = 0) {
  NdbRestarter restarter;
  ...
  scanOptions.optionsPresent =
      scanOptions.SO_PARALLEL | scanOptions.SO_BATCH | scanOptions.SO_GETVALUE;
  ...
  while (row_count < rows) {
    ...
    require(trans != NULL);
    scanOp = trans->scanTable(record, NdbOperation::LM_Read, NULL, &scanOptions,
                              sizeof(scanOptions));
    require(scanOp != NULL);
    require(trans->execute(Commit) == 0);
    const char *resRow;
    int queryRes = scanOp->nextResult(&resRow, true, false);
    require(queryRes == 0);
    require(fragment == 0);
 ...
 */
static int sizeFragment0DbaccHashTable(Ndb *ndb, const NdbRecord *record,
                                       const char *row, Uint32 bucketCount) {
  NdbRestarter restarter;
 ...
 */
static int runScanDuringShrinkAndExpandBack(NDBT_Context *ctx,
                                            NDBT_Step *step) {
  const NdbDictionary::Table *pTab = ctx->getTab();
  Ndb *const ndb = GETNDB(step);
  ...
  int queryRes = scanOp->nextResult(&anyRow, true, false);
  require(queryRes == 0);
  ...
  if (scanned_rows < fragment_rows || scanned_rows > fragment_rows + 2) {
    /**
 ...
 */
static int runScanDuringExpandAndShrinkBack(NDBT_Context *ctx,
                                            NDBT_Step *step) {
  const NdbDictionary::Table *pTab = ctx->getTab();
  Ndb *const ndb = GETNDB(step);
  const NdbRecord *const record = pTab->getDefaultRecord();
...
int runScanOperation(NDBT_Context *ctx, NDBT_Step *step) {
  Ndb *pNdb = GETNDB(step);
  const NdbDictionary::Table *pTab = ctx->getTab();
  NdbTransaction *pTrans = pNdb->startTransaction();
  if (pTrans == NULL) {
...
int runScanUsingMultipleNdbObjects(NDBT_Context *ctx, NDBT_Step *step) {
  int result = NDBT_OK;
  ...
  for (int i = 0; i < numOfNdbObjects; i++) {
    ...
    if (pNdb->init() != 0 && pNdb->waitUntilReady(30) != 0) {
      NDB_ERR(pNdb->getNdbError());
      result = NDBT_FAILED;
      goto cleanup;
    }
...
int runScanApiDisconnect(NDBT_Context *ctx, NDBT_Step *step) {
  /**
  ...
  while (cases[c]) {
    ...
    Ndb_cluster_connection conn2(
        pCC->get_connectstring(connectString, NDB_ARRAY_SIZE(connectString)));
    conn2.configure_tls(opt_tls_search_path, opt_mgm_tls);
    if (conn2.connect() != 0) {
      g_err << "Failed to connect to cluster : " << conn2.get_latest_error_msg()
            << endl;
      return NDBT_FAILED;
    }

    if (conn2.wait_until_ready(30, 30) != 0) {
      g_err << "Failed to wait until ready : " << conn2.get_latest_error_msg()
            << endl;
      return NDBT_FAILED;
    }
     ...
     * nothing worse
     */
    int rc = hugoTrans.scanReadRecords(&ndb2, ctx->getNumRecords());
...
static int defineScan(HugoTransactions &hugoTrans,
                      const NdbDictionary::Table *pTab,
                      NdbScanOperation::LockMode lm) {
  NdbTransaction *trans = hugoTrans.getTransaction();
  CHECKE(trans != nullptr, hugoTrans);
  NdbScanOperation *pScanOp = trans->getNdbScanOperation(pTab->getName());
  CHECKE(pScanOp != nullptr, (*trans));

  CHECKE(pScanOp->readTuples(lm) == 0, (*pScanOp));
  for (int a = 0; a < pTab->getNoOfColumns(); a++) {
    CHECKE(pScanOp->getValue(pTab->getColumn(a)) != nullptr, (*pScanOp));
  }

  return NDBT_OK;
}

int runScanLateUnlock(NDBT_Context *ctx, NDBT_Step *step) {
  /**
...
TESTCASE(
    ...
    "Verify scan requirement: It should be possible to scan read committed "
    "with "
    "parallelism, test with parallelism 240(240 would automatically be "
    "downgraded to the maximum parallelism value for the current config)") {
  INITIALIZER(runLoadTable);
...
TESTCASE("ScanReadAbort240",
         "Scan requirement: A scan may be aborted by the application "
         "at any time. This can be performed even if there are more "
         "tuples to scan. Use parallelism 240(it will be downgraded to max "
         "para for this config)") {
  INITIALIZER(runLoadTable);
  TC_PROPERTY("Parallelism", 240);
  TC_PROPERTY("AbortProb", 90);
  STEPS(runScanRead, 3);
...
TESTCASE("CheckInactivityTimeOut",
         "Check behaviour when the api sleeps for a long time before "
         "continuing scan ") {
  INITIALIZER(runLoadTable);
...
TESTCASE("CheckInactivityBeforeClose",
         "Check behaviour when the api sleeps for a long time before calling "
         "close scan ") {
  INITIALIZER(runLoadTable);
...
TESTCASE("ScanReadError5023", "Scan and insert error 5023") {
  INITIALIZER(runLoadTable);
  TC_PROPERTY("ErrorCode", 5023);
  STEP(runScanReadError);
  FINALIZER(runClearTable);
}
TESTCASE("ScanReadError5024", "Scan and insert error 5024") {
  INITIALIZER(runLoadTable);
  TC_PROPERTY("ErrorCode", 5024);
  STEP(runScanReadError);
  FINALIZER(runClearTable);
}
TESTCASE("ScanReadError5025", "Scan and insert error 5025") {
  INITIALIZER(runLoadTable);
...
TESTCASE("ScanParallelism", "Test scan with different parallelism") {
  INITIALIZER(runLoadTable);
  STEP(runScanParallelism);
  FINALIZER(runClearTable);
}
TESTCASE("ScanVariants", "Test different scan variants") {
  INITIALIZER(runLoadTable);
...
TESTCASE("Bug36124", "Old interpreted Api usage") {
  INITIALIZER(runLoadTable);
  STEP(runBug36124);
  FINALIZER(runClearTable);
}
...
TESTCASE("CloseRefresh", "") { INITIALIZER(runCloseRefresh); }
...
TESTCASE("extraNextResultBug11748194", "Regression test for bug #11748194") {
