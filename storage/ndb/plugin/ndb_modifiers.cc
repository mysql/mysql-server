...
int
NDB_Modifiers::parse_modifier(struct NDB_Modifier* m,
                              const char * str)
{
  if (m->m_found)
  {
...
int
NDB_Modifiers::parseModifierListString(const char* string)
{
...
int
NDB_Modifiers::loadComment(const char* str,
                           size_t len)
{
  ...
  {
    DBUG_PRINT("info", ("parseModifierListString (%s) returned %d", 
                        pos,
                        mod_len));
    /* Parse error */
    return -1;
...
NDB_Modifier* 
NDB_Modifiers::find(const char* name) const
{
  for (uint i = 0; i < m_len; i++)
  {
...
bool 
NDB_Modifiers::set(const char* name, bool value)
{
...
bool 
NDB_Modifiers::set(const char* name, const char* string)
{
  NDB_Modifier* mod = find(name);
  if (mod != NULL)
  {
...
uint32
NDB_Modifiers::generateModifierListString(char* buf, size_t buflen) const
{
  size_t length = 0;
  bool first = true;
...
const char*
NDB_Modifiers::generateCommentString()
{
  assert(m_comment_len >= m_mod_start_offset + m_mod_len);
  const uint32 postCommentLen = m_comment_len - 
  ...
  const uint32 newCommentLen = (m_comment_len - m_mod_len) + 
    newModLen;
  ...
  DBUG_PRINT("info", ("getCommentString : old comment %s len %d "
                      ...
                      m_mod_len,
                      postCommentLen));
  DBUG_PRINT("info", ("getCommentString : newModListStringLen : %u newModLen : %u "
                      "newCommentLen : %u",
                      newModListStringLen,
                      newModLen,
                      newCommentLen));
  ...
  if (newModListStringLen > 0)
  {
    ...
    /* Add modifier list */
    generateModifierListString(insertPos,
                               remain);
  ...
  if (postCommentLen)
  {
    ...
    memcpy(insertPos,
           m_comment_buf + m_mod_start_offset + m_mod_len,
           postCommentLen);
