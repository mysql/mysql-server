...
int Ndb_move_data::init(const NdbDictionary::Table *source,
...
void Ndb_move_data::set_type(Attr &attr, const NdbDictionary::Column *c) {
  attr.column = c;
  attr.name = c->getName();
...
Uint32 Ndb_move_data::calc_str_len_truncated(CHARSET_INFO *cs, const char *data,
                                             Uint32 maxlen) {
  if (!cs) return maxlen;  // no charset found, do not truncate
...
int Ndb_move_data::check_nopk(const Attr &attr1, const Attr &attr2) {
  int ret = 0;
...
int Ndb_move_data::check_promotion(const Attr &attr1, const Attr &attr2) {
  (void)attr2;
  int ret = 0;
...
int Ndb_move_data::check_demotion(const Attr &attr1, const Attr &attr2) {
  (void)attr2;
  int ret = 0;
  const Opts &opts = m_opts;
  do {
...
int Ndb_move_data::check_sizes(const Attr &attr1, const Attr &attr2) {
  int ret = 0;
  do {
...
int Ndb_move_data::check_unsupported(const Attr &attr1, const Attr &attr2) {
  (void)attr2;
  int ret = 0;
  do {
    CHK2(false, (Error::UnsupportedConversion,
                 "cannot convert column #%d '%s'"
...
int Ndb_move_data::check_tables() {
  int ret = 0;
  const Opts &opts = m_opts;
  do {
    ...
    for (int i1 = 0; i1 < attrcount1; i1++) {
      ...
      if (attr1.type == Attr::TypeArray && attr2.type == Attr::TypeBlob) {
        CHK1(check_nopk(attr1, attr2) == 0);
        CHK1(check_promotion(attr1, attr2) == 0);
        continue;
      }

      if (attr1.type == Attr::TypeBlob && attr2.type == Attr::TypeArray) {
        CHK1(check_nopk(attr1, attr2) == 0);
        CHK1(check_demotion(attr1, attr2) == 0);
        continue;
      }

      if (attr1.type == Attr::TypeArray && attr2.type == Attr::TypeArray) {
        CHK1(check_sizes(attr1, attr2) == 0);
        continue;
      }

      if (attr1.type == Attr::TypeBlob && attr2.type == Attr::TypeBlob) {
...
char *Ndb_move_data::alloc_data(Uint32 n) {
  Data *d = new Data;
  d->data = new char[n];
  d->next = m_data;
  m_data = d;
...
void Ndb_move_data::release_data() {
  while (m_data != 0) {
    Data *d = m_data;
    m_data = d->next;
    delete d;
...
int Ndb_move_data::copy_other_to_other(const Attr &attr1, const Attr &attr2) {
  int ret = 0;
  Op &op = m_op;
  ...
  do {
    ...
    if (ra1->isNULL()) {
      const char *value = 0;
      CHK2(op.updateop->setValue(i2, value) == 0, (op.updateop->getNdbError()));
    } else {
...
int Ndb_move_data::copy_data_to_array(const char *data1, const Attr &attr2,
  ...
  do {
    if (data1 == 0) {
      CHK2(op.updateop->setValue(i2, data1) == 0, (op.updateop->getNdbError()));
    } else {
      /*
      ...
      if (length1x > attr2.data_size) {
        ...
        length1 = calc_str_len_truncated(attr2.column->getCharset(), data1,
                                         attr2.data_size);
      }
...
int Ndb_move_data::copy_array_to_array(const Attr &attr1, const Attr &attr2) {
  int ret = 0;
  Op &op = m_op;
  const int i1 = attr1.id;
  do {
    ...
    if (ra1->isNULL()) {
      ...
      if (attr1.length_bytes == 0)
        while (length1 != 0 && data1[length1 - 1] == attr1.pad_char) length1--;
...
int Ndb_move_data::copy_array_to_blob(const Attr &attr1, const Attr &attr2) {
  int ret = 0;
  Op &op = m_op;
  const int i1 = attr1.id;
  const int i2 = attr2.id;
...
int Ndb_move_data::copy_blob_to_array(const Attr &attr1, const Attr &attr2) {
  int ret = 0;
  Op &op = m_op;
  const int i1 = attr1.id;
  do {
...
int Ndb_move_data::copy_blob_to_blob(const Attr &attr1, const Attr &attr2) {
  int ret = 0;
  Op &op = m_op;
  const int i1 = attr1.id;
  const int i2 = attr2.id;
    ...
    if (isNull) {
      ...
      if (attr2.tiny_bytes) {
        if (bytes > attr2.tiny_bytes) {
          require(m_opts.flags & Opts::MD_ATTRIBUTE_DEMOTION);
          op.truncated_in_batch++;
          data_length = calc_str_len_truncated(attr2.column->getCharset(), data,
                                               attr2.tiny_bytes);
        }
      }
      CHK2(bh2->setValue(data, data_length) == 0, (bh2->getNdbError()));
    }
  } while (0);
  return ret;
}

int Ndb_move_data::copy_attr(const Attr &attr1, const Attr &attr2) {
  int ret = 0;
  ...
  do {
    ...
    for (int j = 0; j <= 1; j++) {
      for (int i1 = 0; i1 < attrcount1; i1++) {
        const Attr &attr1 = m_sourceattr[i1];
        int i2 = attr1.map_id;
        if (unlikely(i2 == -1)) continue;
        const Attr &attr2 = m_targetattr[i2];
        const NdbDictionary::Column *c = attr2.column;
        if (j == 0 && !c->getPrimaryKey()) continue;
        if (j == 1 && c->getPrimaryKey()) continue;
...
int Ndb_move_data::move_batch() {
  ...
  do {
    int res;
    CHK2((res = op.scanop->nextResult(true)) != -1, (op.scanop->getNdbError()));
    require(res == 0 || res == 1);
    if (res == 1) {
      op.end_of_scan = true;
...
int Ndb_move_data::move_data(Ndb *ndb) {
  int ret = 0;
  Op &op = m_op;
  Stat &stat = m_stat;
...
void Ndb_move_data::close_op(Ndb *ndb, int ret) {
  Op &op = m_op;
  if (ret == 0) {
    require(op.scantrans == 0);
    require(op.updatetrans == 0);
...
Ndb_move_data::Opts::Opts() { flags = 0; }

void Ndb_move_data::set_opts_flags(int flags) {
  Opts &opts = m_opts;
  opts.flags = flags;
}

void Ndb_move_data::unparse_opts_tries(char *opt, const Opts::Tries &ot) {
...
static int parse_opts_tries_field(const char *&t, int &out) {
  ...
  if (*u == 0) return 0;     // last one
  if (*u != ',') return -1;  // bad char
  t = u + 1;
  return 1;  // more after this one
}

int Ndb_move_data::parse_opts_tries(const char *s, Opts::Tries &ot) {
  ...
  if ((ret = parse_opts_tries_field(t, out.maxtries)) > 0)
    if ((ret = parse_opts_tries_field(t, out.mindelay)) > 0)
      if ((ret = parse_opts_tries_field(t, out.maxdelay)) > 0)
        return -1;  // too many
  if (ret < 0) return -1;
  if (out.mindelay > out.maxdelay) return -1;
  ot = out;
  return 0;
}
...
const Ndb_move_data::Stat &Ndb_move_data::get_stat() { return m_stat; }
...
bool Ndb_move_data::Error::is_temporary() const {
  return code > 0 && ndberror.status == NdbError::TemporaryError;
}

const Ndb_move_data::Error &Ndb_move_data::get_error() { return m_error; }

void Ndb_move_data::set_error_line(int line) {
  Error &e = m_error;
  e.line = line;
}

void Ndb_move_data::set_error_code(int code, const char *fmt, ...) {
  va_list ap;
  va_start(ap, fmt);
  require(code != 0);
  Error &e = m_error;
...
void Ndb_move_data::set_error_code(const NdbError &ndberror) {
  Error &e = m_error;
  set_error_code(ndberror.code, "%s", ndberror.message);
  e.ndberror = ndberror;
}

void Ndb_move_data::reset_error() { new (&m_error) Error; }

NdbOut &operator<<(NdbOut &out, const Ndb_move_data::Error &error) {
  if (error.code < 0)
    out << "move data error " << error.code << ": " << error.message;
...
void Ndb_move_data::error_insert() { m_error_insert = true; }

void Ndb_move_data::invoke_error_insert() {
  NdbError ndberror;
  ndberror.code = 9999;
...
void Ndb_move_data::abort_on_error() {
  const Opts &opts = m_opts;
