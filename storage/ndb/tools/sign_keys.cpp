...
enum {
  SIGN_LOCAL=0, SIGN_SSH_SIGN_KEYS=1, SIGN_SSH_OPENSSL=2, SIGN_CO_PROCESS=3
...
static struct my_option sign_keys_options[] =
{
  ...
  { "no-config", 'l', "Do not obtain cluster configuration; "
    "create a single certificate",
    &opt_noconfig, nullptr, nullptr, GET_BOOL, NO_ARG,
    0, 0, 0, nullptr, 0, nullptr },
  { "CA-cert", 'C', "Cluster CA Certificate file name",
    &opt_ca_cert, nullptr, nullptr, GET_STR, REQUIRED_ARG,
    0, 0, 0, nullptr, 0, nullptr },
  { "CA-key", 'K', "Cluster CA Private Key file name",
    &opt_ca_key, nullptr, nullptr, GET_STR, REQUIRED_ARG,
    0, 0, 0, nullptr, 0, nullptr },
  { "CA-search-path", 'P', "Cluster CA file search path",
    &opt_ca_search_path, nullptr, nullptr, GET_STR, REQUIRED_ARG,
    0, 0, 0, nullptr, 0, nullptr },
  { "remote-CA-host", NDB_OPT_NOSHORT, "address of remote CA host",
    &opt_ca_host, nullptr, nullptr, GET_STR, REQUIRED_ARG,
    0, 0, 0, nullptr, 0, nullptr },
  { "CA-tool", 'X', "Path to local executable helper tool",
    &opt_ca_tool, nullptr, nullptr, GET_STR, REQUIRED_ARG,
    0, 0, 0, nullptr, 0, nullptr },
  { "create-CA", NDB_OPT_NOSHORT, "Create Cluster CA",
    &opt_create_ca, nullptr, nullptr, GET_BOOL, NO_ARG,
    0, 0, 0, nullptr, 0, nullptr },
  { "rotate-CA", NDB_OPT_NOSHORT, "Rotate Cluster CA",
    &opt_rotate_ca, nullptr, nullptr, GET_BOOL, NO_ARG,
    0, 0, 0, nullptr, 0, nullptr },
  { "CA-ordinal", NDB_OPT_NOSHORT, "Ordinal CA name; "
    "defaults to \"First\" for --create-CA and \"Second\" for --rotate-CA",
    &opt_ca_ordinal, nullptr, nullptr, GET_STR, REQUIRED_ARG,
    0, 0, 0, nullptr, 0, nullptr },
  { "passphrase", NDB_OPT_NOSHORT, "Cluster CA Key Pass Phrase",
    &opt_cluster_key_pass, nullptr, nullptr, GET_STR, REQUIRED_ARG,
    0, 0, 0, nullptr, 0, nullptr },
  ...
  { "curve", NDB_OPT_NOSHORT, "Named curve to use for node keys",
    ...
    0, 0, 0, nullptr, 0, nullptr },
  { "node-id", 'n', "Create or sign key for node n",
    &opt_node_id, nullptr, nullptr, GET_INT, REQUIRED_ARG,
    0, 0, MAX_NODES_ID, nullptr, 0, nullptr },
  { "node-type", 't', "Create or sign keys for certain node types, "
    "from set (mgmd,db,api)",
    &opt_node_types, nullptr, &node_types_lib, GET_SET, REQUIRED_ARG,
    7, 0, 7, nullptr, 0, nullptr }, // default 7 = all node types
  { "create-key", NDB_OPT_NOSHORT, "Create (or replace) private keys",
    &opt_create_key, nullptr, nullptr, GET_BOOL, NO_ARG,
    0, 0, 0, nullptr, 0, nullptr },
  { "pending", NDB_OPT_NOSHORT,
    "Save keys and certificates as pending, rather than active",
    &opt_pending, nullptr, nullptr, GET_BOOL, NO_ARG,
    0, 0, 0, nullptr, 0, nullptr },
  { "promote", NDB_OPT_NOSHORT, "Promote pending files to active, then exit",
    &opt_promote, nullptr, nullptr, GET_BOOL, NO_ARG,
    0, 0, 0, nullptr, 0, nullptr },
  { "sign", NDB_OPT_NOSHORT, "Create signed certificates (with --skip-sign, "
    "create certificate signing requests)",
    &opt_sign, nullptr, nullptr, GET_BOOL, NO_ARG,
    1, 0, 0, nullptr, 0, nullptr },
  { "check", NDB_OPT_NOSHORT, "Run periodic check of certificate expiry dates",
    &opt_periodic, nullptr, nullptr, GET_BOOL, NO_ARG,
    0, 0, 0, nullptr, 0, nullptr },
  { "replace-by", NDB_OPT_NOSHORT,
    "Suggested certificate replacement date for periodic checks",
    &opt_replace_by, nullptr, nullptr, GET_INT, REQUIRED_ARG,
    opt_replace_by, -128, 127, nullptr, 0, nullptr },
  { "schedule", NDB_OPT_NOSHORT, "set certificate expiration schedule",
    &opt_schedule, nullptr, nullptr, GET_STR, REQUIRED_ARG,
    0, 0, 0, nullptr, 0, nullptr },
  { "duration", NDB_OPT_NOSHORT, "Set exact lifetime for CSR in seconds",
    &opt_duration, nullptr, nullptr, GET_INT, REQUIRED_ARG,
    0, -500000, 0, nullptr, 0, nullptr },
  { "bound-hostname", NDB_OPT_NOSHORT, "Create certificate bound to hostname",
    &opt_bound_host, nullptr, 0, GET_STR, REQUIRED_ARG,
    0, 0, 0, nullptr, 0, nullptr },
  { "bind-host", NDB_OPT_NOSHORT,
    "list of node types that should have certificate hostname bindings, "
    "from set (mgmd,db,api)",
    &opt_bind_host, nullptr, &node_types_lib, GET_SET, REQUIRED_ARG,
    5, 0, 7, nullptr, 0, nullptr }, // default 5 = true for MGMD and API
  { "to-dir", NDB_OPT_NOSHORT, "Specify output directory for created files",
    &opt_dest_dir, nullptr, 0, GET_STR, REQUIRED_ARG,
    0, 0, 0, nullptr, 0, nullptr },
  { "keys-to-dir", NDB_OPT_NOSHORT,
    "Specify output directory only for private keys (overrides --to-dir)",
    &opt_key_dest_dir, nullptr, 0, GET_STR, REQUIRED_ARG,
    0, 0, 0, nullptr, 0, nullptr },
  { "stdio", NDB_OPT_NOSHORT, "Read CSR on stdin and write X.509 on stdout",
    & opt_stdio, nullptr, nullptr, GET_BOOL, NO_ARG,
    0, 0, 0, nullptr, 0, nullptr },
...
bool parse_schedule() {
  int i = sscanf(opt_schedule, "%hd,%hd,%hd,%hd,%hd,%hd",
                 & exp_schedule[0], & exp_schedule[1], & exp_schedule[2],
                 & exp_schedule[3], & exp_schedule[4], & exp_schedule[5]);
  return (i == 6);
}

bool check_options() {
  if(! parse_schedule())
    return message("Error: Invalid schedule string.\n");

  if(opt_create_ca && opt_ca_host)
    return message("Error: Cannot create remote CA.\n");

  if(opt_create_ca && opt_rotate_ca)
    return message("Error: Incompatible options: --rotate-CA and --create-CA\n");
  ...
  if(opt_ca_tool) signing_method = SIGN_CO_PROCESS;
  else if(opt_rs_openssl) signing_method = SIGN_SSH_OPENSSL;
  else if(opt_ca_host) signing_method = SIGN_SSH_SIGN_KEYS;
  ...
  if(opt_create_ca) mode = "create CA";
  else if(opt_rotate_ca) mode = "rotate CA";
  else if(opt_promote) mode = "promote files";
  else if(opt_periodic) mode = "check expiration dates";
  else if(! opt_sign) mode = opt_create_key ?
    "create key and signing request" :
    "create signing request for existing key";
  else if(opt_pending) mode = opt_create_key ?
    "create pending keys and certificates" : "create pending certificates";
  else if(opt_create_key) mode = "create active keys and certificates";
  else mode = "create active certificates";
...
static void sign_keys_usage_extra()
{
  ...
  puts("");
  puts("    ndb_sign_keys ... --CA-search-path=/remote/dir"
       " --remote-CA-host=name");
  puts("");
...
struct {
  ...
  void print() {
    if(nodes) fprintf(stderr, "Read %d node%s from custer configuration.\n",
                      nodes, plural(nodes));
    if(matched) {
      fprintf(stderr, "Found %d node%s configured to run on this host",
    ...
    if(promoted) fprintf(stderr, "Promoted %d file%s.\n", promoted,
                         plural(promoted));
  }
} stats;
...
Config * fetch_configuration(SSL_CTX * ctx) {
  ...
  if(conf == nullptr) {
    ndberr << "Could not get configuration, error code: "
           << ndb_mgm_get_latest_error(mgm) << ", error msg: "
           << ndb_mgm_get_latest_error_msg(mgm) << endl;
  }
...
static void print_creating(const char * t1, const char * t2, const char * dir) {
  ...
  if(hasDir) fprintf(stderr, " in directory %s.\n", dir);
...
int ClusterCredentialFiles::read_CA_key(const TlsSearchPath * searchPath,
                                        EVP_PKEY * & key,
                                        PkiFile::PathName & path, char * pass) {
...
int ClusterCredentialFiles::create(const char * key_dir,
                                   const char * cert_dir) {
  fputs("This utility will create a cluster CA private key and "
...
int create_CA(TlsSearchPath * CA_path) {
 /* Determine destination paths */
  ...
  if(! cert_dir)
    cert_dir = CA_path->first_writable();
  if(! cert_dir)
    return TlsKeyError::no_writable_dir;
...
int rotate_CA(EVP_PKEY * ca_key, const PkiFile::PathName & ca_key_path,
              ...
              const PkiFile::PathName & ca_cert_path) {
  ...
  if(! ClusterCertAuthority::sign(sk_X509_value(ca_certs, 0),
                                  ca_key, new_cert))
...
bool set_lifetime(SigningRequest * csr) {
  if(opt_duration)
    return csr->set_exact_duration(opt_duration);
...
int do_promote_files(const TlsSearchPath * tlsPath, int id, Node::Type type) {
  ...
  int result = 0;
  if(! (k || c)) {
    result = c ? TlsKeyError::cannot_promote_key :
                 TlsKeyError::cannot_promote_cert;
  ...
  if(bind_host && opt_bound_host) {
    if(strlen(hostname)) {
      if(strcmp(opt_bound_host, hostname))
        fprintf(stderr, "WARNING: Using host name '%s' where NDB configuration "
                        "requires '%s'.\n", opt_bound_host, hostname);
    }
    else hostname = opt_bound_host;
  }
...
int get_csr(SigningRequest *& csr, PkiFile::PathName & csr_file,
  ...
  if(Node::And(type, opt_bind_host) && ! hostname) {
    require(opt_noconfig);
    message("Missing hostname. In no-config mode, either set bind-host=0 "
  ...
  if(opt_create_key) {
    ...
    if(opt_key_dest_dir) dir = opt_key_dest_dir;
    else if(opt_dest_dir) dir = opt_dest_dir;
    else dir = tlsPath->first_writable();
...
int main(int argc, char** argv) {
  ...
  */
  ...
  if(opt_sign) {
    ...
    if(rs) return fatal(rs);
    if(opt_periodic && do_periodic_check(ca_certs, ca_cert_file))
      return 1;
  }
  ...
  if(opt_stdio) {
    ...
    rs = ClusterCredentialFiles::read_CA_key(CA_path, ca_key,
                                             ca_key_file, passphrase);
    if(rs) return rs;
  ...
  if(opt_noconfig) {
    ...
    if(opt_promote)
      return do_promote_files(search_path, opt_node_id, ntypes);

    if(opt_periodic)
      return do_periodic_check(search_path);
  ...
  for(iter.first(); iter.valid(); iter.next())
  {
    ...
    if(opt_node_id == 0 || opt_node_id == node_id)
    {
      ...
      if(check_group_key_exists(cfg_node_type))
        continue;
      register_group_key_exists(cfg_node_type);

      if(opt_periodic) {
        if(do_periodic_check(search_path, node_type))
          return 1;
        continue;
      }
...
int remote_key_signing(const SigningRequest * csr, EVP_PKEY * key,
  ...
  if(! (rfp && wfp)) { perror("Failed to open streams"); return 134; }
  if(!csr->write(wfp)) { perror("Failed writing to pipe"); return 135; }
