 ...
 public:
  ...
  struct Print {
   private:
    ...
    void print(const char *frm, ...) ATTRIBUTE_FORMAT(printf, 2, 3);
    char *m_buf;
    Uint32 m_bufsz;
    Uint32 m_sz;
  };
...
inline const NdbSqlUtil::Type &NdbPack::getSqlType(Uint32 typeId) {
  return NdbSqlUtil::m_typeList[typeId];
}

// NdbPack::Error

inline NdbPack::Error::Error() {
  m_error_code = 0;
  m_error_line = 0;
}
...
inline NdbPack::Endian::Value NdbPack::Endian::get_endian() {
...
inline NdbPack::Type::Type(int typeId, Uint32 byteSize, bool nullable,
                           Uint32 csNumber) {
  set(typeId, byteSize, nullable, csNumber);
}

inline void NdbPack::Type::set(Uint32 typeId, Uint32 byteSize, bool nullable,
                               Uint32 csNumber) {
  m_typeId = typeId;
  m_byteSize = byteSize;
  m_nullable = nullable;
...
inline Uint32 NdbPack::Type::get_type_id() const { return m_typeId; }

inline Uint32 NdbPack::Type::get_byte_size() const { return m_byteSize; }

inline bool NdbPack::Type::get_nullable() const { return (bool)m_nullable; }

inline Uint32 NdbPack::Type::get_cs_number() const { return m_csNumber; }

inline Uint32 NdbPack::Type::get_array_type() const { return m_arrayType; }

// NdbPack::Spec

inline NdbPack::Spec::Spec() {
  reset();
  m_buf = nullptr;
  m_bufMaxCnt = 0;
}

inline void NdbPack::Spec::set_buf(Type *buf, Uint32 bufMaxCnt) {
  reset();
  m_buf = buf;
  m_bufMaxCnt = bufMaxCnt;
}

inline void NdbPack::Spec::set_buf(Type *buf) { m_buf = buf; }

inline void NdbPack::Spec::reset() {
  m_cnt = 0;
  m_nullableCnt = 0;
  m_varsizeCnt = 0;
  m_maxByteSize = 0;
...
inline const NdbPack::Type &NdbPack::Spec::get_type(Uint32 i) const {
  assert(i < m_cnt);
  return m_buf[i];
}
...
inline Uint32 NdbPack::Spec::get_max_data_len(bool allNullable) const {
  return get_nullmask_len(allNullable) + m_maxByteSize;
}

inline Uint32 NdbPack::Spec::get_min_var_bytes(bool allNullable) const {
...
inline NdbPack::Iter::Iter(const DataC &data)
    : m_spec(data.m_spec), m_allNullable(data.m_allNullable) {
  reset();
}

inline void NdbPack::Iter::reset() {
...
inline NdbPack::DataC::DataC(const Spec &spec, bool allNullable)
    : m_spec(spec), m_allNullable(allNullable) {
  m_buf = nullptr;
  m_bufMaxLen = 0;
  m_cnt = 0;
}

inline void NdbPack::DataC::set_buf(const void *buf, Uint32 bufMaxLen,
                                    Uint32 cnt) {
...
inline const NdbPack::Spec &NdbPack::DataC::get_spec() const { return m_spec; }

inline const void *NdbPack::DataC::get_data_buf() const { return &m_buf[0]; }

inline Uint32 NdbPack::DataC::get_cnt() const { return m_cnt; }

inline bool NdbPack::DataC::is_empty() const { return m_cnt == 0; }

inline bool NdbPack::DataC::is_full() const { return m_cnt == m_spec.m_cnt; }

// NdbPack::Data

inline NdbPack::Data::Data(const Spec &spec, bool allNullable, Uint32 varBytes)
    : DataC(spec, allNullable), m_varBytes(varBytes), m_iter(*this) {
  m_buf = nullptr;
  m_bufMaxLen = 0;
  m_endian = Endian::get_endian();
}

inline void NdbPack::Data::set_buf(void *buf, Uint32 bufMaxLen) {
...
inline void NdbPack::Data::reset() {
  m_cnt = 0;  // in DataC
...
inline int NdbPack::Data::finalize() {
  if (likely(m_varBytes == 0 || finalize_impl() == 0)) return 0;
  return -1;
}

inline int NdbPack::Data::convert(Endian::Value to_endian) {
  if (to_endian == Endian::Native) to_endian = Endian::get_endian();
  if (m_endian == to_endian) return 0;
  if (convert_impl() == 0) {
    m_endian = to_endian;
    return 0;
  }
...
inline Uint32 NdbPack::Data::get_max_len() const {
  return m_varBytes + m_spec.get_max_data_len(m_allNullable);
}

inline Uint32 NdbPack::Data::get_max_len4() const {
...
inline Uint32 NdbPack::Data::get_var_bytes() const { return m_varBytes; }

inline const void *NdbPack::Data::get_full_buf() const { return m_buf; }

inline void *NdbPack::Data::get_full_buf() { return m_buf; }

inline Uint32 NdbPack::Data::get_full_len() const {
  return m_varBytes + m_iter.m_itemPos + m_iter.m_itemLen;
}

inline Uint32 NdbPack::Data::get_data_len() const {
  return m_iter.m_itemPos + m_iter.m_itemLen;
}

inline Uint32 NdbPack::Data::get_null_cnt() const { return m_iter.m_nullCnt; }

inline NdbPack::Endian::Value NdbPack::Data::get_endian() const {
  return m_endian;
}

// NdbPack::BoundC

inline NdbPack::BoundC::BoundC(DataC &data) : m_data(data) { m_side = 0; }

inline int NdbPack::BoundC::cmp(const DataC &d2, Uint32 cnt,
                                Uint32 &num_eq) const {
  const BoundC &b1 = *this;
  const DataC &d1 = b1.m_data;
  int res = d1.cmp(d2, cnt, num_eq);
  if (res == 0 && d1.m_cnt <= d2.m_cnt) res = b1.m_side;
  return res;
}

inline NdbPack::DataC &NdbPack::BoundC::get_data() const { return m_data; }

inline int NdbPack::BoundC::get_side() const { return m_side; }

// NdbPack::Bound

inline NdbPack::Bound::Bound(Data &data) : BoundC(data), m_data(data) {}

inline void NdbPack::Bound::reset() {
  m_data.reset();
  m_side = 0;
}

inline int NdbPack::Bound::finalize(int side) {
  if (unlikely(m_data.finalize() == -1)) {
    set_error(m_data);
    return -1;
  }
  if (unlikely(BoundC::finalize(side) == -1)) return -1;
  return 0;
}

inline NdbPack::Data &NdbPack::Bound::get_data() const { return m_data; }

inline Uint32 NdbPack::DataArray::cnt() const { return m_cnt; }

inline Uint32 NdbPack::DataArray::get_null_cnt() const { return m_null_cnt; }

inline Uint32 NdbPack::DataArray::get_data_len() const {
  Uint32 cnt = m_cnt;
  Uint32 len = 0;
...
inline NdbPack::BoundArray::BoundArray()
    : m_spec(nullptr), m_data_array(nullptr), m_side(0) {}

inline NdbPack::BoundArray::BoundArray(const Spec *spec,
                                       const DataArray *data_array,
                                       const int side)
    : m_spec(spec), m_data_array(data_array), m_side(side) {}

inline int NdbPack::BoundArray::cmp(const DataArray *d2, const Uint32 cnt,
...
inline Uint32 NdbPack::BoundArray::cnt() const { return m_data_array->cnt(); }
#endif  // NDB_PACK_HPP
