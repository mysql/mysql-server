...
void ThreadConfig::scanZeroTimeQueue() { globalTimeQueue.scanZeroTimeQueue(); }
/**
 ...
 */
inline void ThreadConfig::scanTimeQueue() {
...
void ThreadConfig::ipControlLoop(NdbThread *pThis) {
  Uint32 thread_index =
      globalEmulatorData.theConfiguration->addThread(pThis, BlockThread);
  ...
  while (1) {
    ...
    globalData.incrementWatchDogCounter(6);
    {
      ...
      globalData.theMicrosSend += NdbTick_Elapsed(before, after).microSec();
    }
    ...
    if (i++ >= 20) {
      execute_loop_constant =
          globalEmulatorData.theConfiguration->schedulerExecutionTimer();
      min_spin_time = globalEmulatorData.theConfiguration->schedulerSpinTimer();
    ...
    do {
      ...
      if (LEVEL_IDLE == globalData.highestAvailablePrio) {
        ...
        if (min_spin_time && !yield_flag) {
          ...
          if (micros_passed < min_spin_time) timeOutMillis = 0;
        }
      }
      ...
      globalData.incrementWatchDogCounter(7);
      {
        ...
        NDB_TICKS after = NdbTick_getCurrentTicks();
        yield_ticks = after;
        globalData.theMicrosSleep += NdbTick_Elapsed(before, after).microSec();
        if (poll_flag) {
    ...
    do {
      ...
      if (unlikely(globalData.theRestartFlag == perform_stop)) goto out;
      //--------------------------------------------------------------------
      ...
      //--------------------------------------------------------------------
      if (!execute_loop_constant && !min_spin_time) break;
      ...
      if (no_exec_loops++ >= 8192) {
        Uint32 expired_time =
            (Uint32)NdbTick_Elapsed(statistics_start_ticks, end_ticks)
                .microSec();
        statistics_start_ticks = end_ticks;
        globalScheduler.reportThreadConfigLoop(
            expired_time, execute_loop_constant, &no_exec_loops, &tot_exec_time,
            &no_extra_loops, &tot_extra_time);
      }
      /*
      ...
      */
      if (micros_passed >= execute_loop_constant || (exec_again > 1)) break;
      exec_again++;
      //--------------------------------------------------------------------
      ...
      //--------------------------------------------------------------------
      globalData.incrementWatchDogCounter(7);
      if (!globalTransporterRegistry.pollReceive(0)) break;
