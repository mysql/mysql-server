...
inline DLHashTable2<P, T>::DLHashTable2(P &_pool) : thePool(_pool) {
  mask = 0;
  hashValues = 0;
}

template <class P, class T>
inline DLHashTable2<P, T>::~DLHashTable2() {
  if (hashValues != 0) delete[] hashValues;
}

template <class P, class T>
inline bool DLHashTable2<P, T>::setSize(Uint32 size) {
  Uint32 i = 1;
  while (i < size) i *= 2;
  ...
  for (Uint32 j = 0; j < i; j++) hashValues[j] = RNIL;

  return true;
}

template <class P, class T>
inline void DLHashTable2<P, T>::add(Ptr<T> &obj) {
 ...
 */
template <class P, class T>
inline bool DLHashTable2<P, T>::first(Iterator &iter) const {
  Uint32 i = 0;
...
template <class P, class T>
inline bool DLHashTable2<P, T>::next(Iterator &iter) const {
...
template <class P, class T>
inline void DLHashTable2<P, T>::remove(Ptr<T> &ptr, const T &key) {
...
template <class P, class T>
inline void DLHashTable2<P, T>::release(Ptr<T> &ptr, const T &key) {
  const Uint32 hv = key.hashValue() & mask;
...
template <class P, class T>
inline void DLHashTable2<P, T>::remove(Uint32 i) {
  Ptr<T> tmp;
  tmp.i = i;
...
template <class P, class T>
inline void DLHashTable2<P, T>::release(Uint32 i) {
  Ptr<T> tmp;
  tmp.i = i;
  tmp.p = (T *)thePool.getPtr(i);  // cast
  release(tmp);
...
template <class P, class T>
inline void DLHashTable2<P, T>::remove(Ptr<T> &ptr) {
...
template <class P, class T>
inline void DLHashTable2<P, T>::release(Ptr<T> &ptr) {
  const Uint32 next = ptr.p->nextHash;
  const Uint32 prev = ptr.p->prevHash;
...
template <class P, class T>
inline void DLHashTable2<P, T>::removeAll() {
  for (Uint32 i = 0; i <= mask; i++) hashValues[i] = RNIL;
}

template <class P, class T>
inline bool DLHashTable2<P, T>::next(Uint32 bucket, Iterator &iter) const {
  while (bucket <= mask && hashValues[bucket] == RNIL) bucket++;
...
template <class P, class T>
inline bool DLHashTable2<P, T>::seize(Ptr<T> &ptr) {
  Ptr<U> ptr2;
  if (!thePool.seize(ptr2)) return false;
...
template <class P, class T>
inline void DLHashTable2<P, T>::getPtr(Ptr<T> &ptr, Uint32 i) const {
  ptr.i = i;
  ptr.p = (T *)thePool.getPtr(i);  // cast
}

template <class P, class T>
inline void DLHashTable2<P, T>::getPtr(Ptr<T> &ptr) const {
  Ptr<U> ptr2;
  thePool.getPtr(ptr2);
  ptr.i = ptr2.i;
  ptr.p = (T *)ptr2.p;  // cast
...
template <class P, class T>
inline T *DLHashTable2<P, T>::getPtr(Uint32 i) const {
  return (T *)thePool.getPtr(i);  // cast
}

template <class P, class T>
inline bool DLHashTable2<P, T>::find(Ptr<T> &ptr, const T &key) const {
  const Uint32 hv = key.hashValue() & mask;
