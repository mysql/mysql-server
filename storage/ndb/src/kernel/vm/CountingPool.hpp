...
template<typename T, class P> class CountingPool: public P
{
  Uint32 m_seized;
  Uint32 m_inuse;
  Uint32 m_inuse_high;
...
protected:
public:
  CountingPool(): m_seized(0), m_inuse(0), m_inuse_high(0), m_max_allowed(UINT32_MAX) { }
  bool seize(Ptr<T>& ptr)
  {
    if (m_seized >= m_max_allowed) return false;
    bool ok = P::seize(ptr);
    if (!ok) return false;
    m_seized++;
    m_inuse++;
    if (m_inuse_high < m_inuse) m_inuse_high++;
    return true;
  }
  bool release(Ptr<T> ptr)
  {
    (void) P::release(ptr);
    m_seized--;
    m_inuse--;
