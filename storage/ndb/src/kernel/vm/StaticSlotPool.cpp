...
void StaticSlotPool::init(Uint32 type_id, Uint32 slot_size, Uint32 *min_recs,
  ...
  if (likely(page_count != 0)) {
    mem_manager->alloc_pages(type_id, &page_number, &page_count, 1,
...
bool StaticSlotPool::startup(Uint32 *initialized_slots_ptr, Uint32 slot_size) {
  ...
  for (Uint32 i = 0; i < slots_per_page; i++, index++) {
    void *pv = static_cast<void *>(
        &page.p->m_data[(slots_per_page - i - 1) * slot_size]);
