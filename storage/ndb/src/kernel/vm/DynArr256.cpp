...
inline bool DA256Page::is_empty() const {
  return !(0x7fff & (m_header[0].m_magic | m_header[1].m_magic));
}

inline bool DA256Page::is_full() const {
  return !(0x7fff & ~(m_header[0].m_magic & m_header[1].m_magic));
}

inline Uint32 DA256Page::first_free() const {
  Uint32 node = BitmaskImpl::ctz(~m_header[0].m_magic | 0x8000);
  if (node == 15) node = 15 + BitmaskImpl::ctz(~m_header[1].m_magic | 0x8000);
  return node;
}

inline Uint32 DA256Page::last_free() const {
...
static inline Uint32 div15(Uint32 x) {
...
inline bool DA256Page::get(Uint32 node, Uint32 idx, Uint32 type_id,
                           Uint32 *&val_ptr) const {
  Uint32 *magic_ptr, p;
  if (idx != 255) {
 ...
 * sz = 4     = 256^4 - 4 level
 */
Uint32 *DynArr256::get_dirty(Uint32 pos) const {
...
#ifdef DA256_USE_PX
  Uint32 px[4] = {(pos >> 24) & 255, (pos >> 16) & 255, (pos >> 8) & 255,
                  (pos >> 0) & 255};
#endif
  ...
  for (; sz--;) {
    ...
    if (unlikely(!page->get(page_idx, p0, type_id, retVal))) goto err;
...
Uint32 *DynArr256::set(Uint32 pos) {
  ...
  if (pos > m_head.m_high_pos) m_head.m_high_pos = pos;
#endif
...
static inline void initpage(DA256Page *p, Uint32 page_no, Uint32 type_id) {
  ...
  for (i = 0; i < 30; i++) {
    ...
    for (j = 0; j < 17; j++) node->m_lines[j].m_magic = type_id;
#endif
  }
}

bool DynArr256::expand(Uint32 pos) {
  Uint32 i;
  Uint32 idx = 0;
  Uint32 alloc[5];
  Uint32 sz = m_head.m_sz;
  ...
  for (; pos >= g_max_sizes[sz]; sz++) {
    ...
    if (unlikely(ptrI == RNIL)) goto err;
    m_head.m_no_of_nodes++;
    alloc[idx++] = ptrI;
  }
...
err:
  for (i = 0; i < idx; i++) m_pool->release(alloc[i]);
...
void DynArr256::init(ReleaseIterator &iter) {
  iter.m_sz = 1;
 ...
 */
Uint32 DynArr256::truncate(Uint32 trunc_pos, ReleaseIterator &iter,
                           Uint32 *ptrVal) {
  Uint32 type_id = (~m_pool->m_type_id) & 0xFFFF;
  DA256Page *memroot = m_pool->m_memroot;

  for (;;) {
    if (iter.m_sz == 0 || iter.m_pos < trunc_pos || m_head.m_sz == 0 ||
    ...
    if (iter.m_sz == 1 && node_addr == 0) {
    ...
    } else if (is_value || iter.m_ptr_i[iter.m_sz + 1] == *refPtr) {  // sz--
      Uint32 ptrI = *refPtr;
      if (!is_value) {
        if (ptrI != RNIL) {
          m_pool->release(ptrI);
      ...
      if (node_index == 0) {
        ...
        assert((~iter.m_pos &
                ~(0xffffffff << (8 * (m_head.m_sz - iter.m_sz)))) == 0);
...
static inline bool seizenode(DA256Page *page, Uint32 idx, Uint32 type_id) {
  Uint32 i;
  Uint32 b = (idx + 1) >> 4;
  Uint32 p = idx - (b << 4) + b;
  ...
  for (i = 0; i < 17; i++) {
    ...
    for (Uint32 j = 0; j < 15; j++) line->m_data[j] = RNIL;
  }
...
static bool releasenode(DA256Page *page, Uint32 idx, Uint32 type_id) {
  Uint32 i;
  Uint32 b = (idx + 1) >> 4;
  Uint32 p = idx - (b << 4) + b;
    ...
    if (likely(seizenode(page, idx, type_id))) {
      ...
      if (page->is_full()) {
        ...
        if (m_first_free == RNIL) {
          ...
          ((DA256Free *)(page->m_nodes + page->last_free()))->m_prev_free =
              RNIL;
        }
      }

      m_used++;
      if (m_used < m_usedHi) m_usedHi = m_used;
...
void DynArr256Pool::release(Uint32 ptrI) {
  Uint32 type_id = m_type_id;
  ...
  if (likely(releasenode(page, page_idx, type_id))) {
    m_inuse_nodes--;
    ptr->m_magic = type_id;
    if (last_free > 29) {  // Add last to page free list
      Uint32 lf = m_last_free;
      ptr->m_prev_free = lf;
      ptr->m_next_free = RNIL;
      m_last_free = page_no;
      if (m_first_free == RNIL) m_first_free = page_no;
      ...
      if (prevpage != RNIL) {
        ...
        ((DA256Free *)(page->m_nodes + page->last_free()))->m_next_free =
            nextpage;
      }
      if (m_first_free == page_no) {
        m_first_free = nextpage;
      }
      if (m_last_free == page_no) m_last_free = prevpage;
    } else if (page_idx > last_free) {  // last free node in page tracks free
                                        // page list links
      ptr->m_next_free =
          ((DA256Free *)(page->m_nodes + last_free))->m_next_free;
      ptr->m_prev_free =
          ((DA256Free *)(page->m_nodes + last_free))->m_prev_free;
    }
    assert(m_used);
    m_used--;
    return;
...
static bool release(DynArr256 &arr) {
  ...
  if (verbose > 2)
    ndbout_c(
        "allocatedpages: %d (max %d) releasedpages: %d allocatednodes: %d (max "
        "%d) releasednodes: %d",
        allocatedpages, maxallocatedpages, releasedpages, allocatednodes,
        maxallocatednodes, releasednodes);
  ...
  if (verbose > 1)
    ndbout_c(
        "allocatedpages: %d (max %d) releasedpages: %d allocatednodes: %d (max "
        "%d) releasednodes: %d (%llu us)"
        " releasecnt: %d",
        allocatedpages, maxallocatedpages, releasedpages, allocatednodes,
        maxallocatednodes, releasednodes, micros, cnt);
  return true;
}

static bool simple(DynArr256 &arr, int argc, char *argv[]) {
...
static bool basic(DynArr256 &arr, int argc, char *argv[]) {
#define MAXLEN 65536
  ...
  for (Uint32 i = 0; i < MAXLEN; i++) {
    ...
    switch (op) {
      ...
      case 1: {  // set
        Uint32 item = len << 1;
        Uint32 idx = i;  // rand() & 0xFFFFF; // & 0xFFFFF; //rand();
                         // //(65536*i) / 10000;
        Uint32 val = rand();
#if 0
...
static bool read(DynArr256 &arr, int argc, char **argv) {
  Uint32 cnt = 100000;
  ...
  if (verbose)
    ndbout_c("%lldmb data -> %d entries (%dkb)", mbytes, maxidx, 32 * pages);
  ...
  for (Uint32 i = 0; i < 10; i++) {
    ...
    uspg /= cnt;
    if (verbose)
      ndbout_c("Elapsed %lldus diff: %d -> %f us/get", micros, sum0 - sum1,
               uspg);
  }
  return true;
}

static bool write(DynArr256 &arr, int argc, char **argv) {
  Uint32 seq = 0, seqmask = 0;
  Uint32 cnt = 100000;
  Uint64 mbytes = 16 * 1024;
  Uint32 seed = (Uint32)time(0);
  ...
  for (Uint32 i = 0; i < 10; i++) {
    ...
    float uspg = (float)micros;
    uspg /= cnt;
    if (verbose) ndbout_c("Elapsed %lldus -> %f us/set", micros, uspg);
    if (!release(arr)) return false;
  }
  return true;
}

static void usage(FILE *f, int argc, char **argv) {
  fprintf(stderr, "Usage:\n");
  fprintf(stderr, "\t%s --simple <index1> <index2> ... <indexN>\n", argv[0]);
  fprintf(stderr, "\t%s --basic\n", argv[0]);
  fprintf(stderr,
          "\t%s { --read | --write } [ --mbytes=<megabytes> | "
          "--mbytes=<gigabytes>[gG] ] [ --cnt=<count> ] [ --seq ]\n",
          argv[0]);
...
#ifdef TEST_DYNARR256
static char *flatten(int argc, char **argv) /* NOT MT-SAFE */
{
...
int main(int argc, char **argv) {
  ...
  if (argc == 1) {
    ...
    plan(2);
    ok(simple(arr, argc - 1, argv + 1), "simple %s",
       flatten(argc - 1, argv + 1));
...
#ifdef TEST_DYNARR256
  ok(allocatednodes == releasednodes && allocatedpages == releasedpages,
     "release");
  return exit_status();
#else
  return 0;
