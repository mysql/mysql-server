...
template <Uint32 sz>
void compare(DataBuffer<sz, ArrayPool<DataBufferSegment<sz>>> &db,
             Buffer &buf) {
  typename DataBuffer<sz, ArrayPool<DataBufferSegment<sz>>>::DataBufferIterator
      it;
...
template <Uint32 sz>
void test(Uint32 loops, Uint32 iter) {
  ndbout_c("DataBuffer<%d> loops=%d iter=%d", sz, loops, iter);

  while (loops-- > 0) {
    Uint32 size = sz * ((10 + (rand() % (10 * sz)) + sz - 1) / sz);

    typename DataBuffer<sz, ArrayPool<DataBufferSegment<sz>>>::DataBufferPool
        thePool;
    typename DataBuffer<
        sz, ArrayPool<DataBufferSegment<sz>>>::DataBufferIterator it;
    ...
    for (Uint32 i = 0; i < iter; i++) {
      ...
      switch (c) {
        case 0:  // Release
          db.first(it);
          for (; !it.curr.isNull(); db.next(it)) *it.data = 0;
