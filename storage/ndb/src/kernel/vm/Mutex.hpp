...
class Mutex {
 public:
  ...
  bool lock(SimulatedBlock::Callback &callback, bool exclusive = true,
            bool notify = false);
 ...
 public:
  static void release(SimulatedBlock::MutexManager &, Uint32 activePtrI,
                      Uint32 mutexId);
};

inline MutexHandle::MutexHandle(Uint32 id) : m_mutexId(id) {
  m_activeMutexPtrI = RNIL;
}

inline bool MutexHandle::isNull() const { return m_activeMutexPtrI == RNIL; }

inline void MutexHandle::release(SimulatedBlock::MutexManager &mgr) {
  if (!isNull()) {
...
template <Uint32 MutexId>
inline MutexHandle2<MutexId>::MutexHandle2() {
  m_activeMutexPtrI = RNIL;
}

template <Uint32 MutexId>
inline bool MutexHandle2<MutexId>::isNull() const {
  return m_activeMutexPtrI == RNIL;
}

template <Uint32 MutexId>
inline void MutexHandle2<MutexId>::release(SimulatedBlock::MutexManager &mgr) {
  if (!isNull()) {
...
template <Uint32 MutexId>
inline Uint32 MutexHandle2<MutexId>::getHandle() const {
  return m_activeMutexPtrI;
}

template <Uint32 MutexId>
inline void MutexHandle2<MutexId>::clear() {
  m_activeMutexPtrI = RNIL;
}

template <Uint32 MutexId>
inline void MutexHandle2<MutexId>::setHandle(Uint32 val) {
  if (m_activeMutexPtrI == RNIL) {
    m_activeMutexPtrI = val;
    return;
  }
  ErrorReporter::handleAssert("Mutex::setHandle mutex alreay inuse", __FILE__,
                              __LINE__);
}

inline Mutex::Mutex(Signal *signal, SimulatedBlock::MutexManager &mgr,
                    MutexHandle &mh)
    : m_signal(signal),
      m_mgr(mgr),
...
inline Mutex::~Mutex() { m_srcPtrI = m_ptr.i; }
...
inline bool Mutex::isNull() const { return m_ptr.isNull(); }

inline bool Mutex::lock(SimulatedBlock::Callback &callback, bool exclusive,
                        bool notify) {
  if (m_ptr.isNull()) {
  ...
  ErrorReporter::handleAssert("Mutex::lock mutex alreay inuse", __FILE__,
                              __LINE__);
  return false;
}

inline bool Mutex::trylock(SimulatedBlock::Callback &callback, bool exclusive) {
  if (m_ptr.isNull()) {
    if (m_mgr.seize(m_ptr)) {
      m_ptr.p->m_mutexId = m_mutexId;
      m_ptr.p->m_callback = callback;
  ...
  ErrorReporter::handleAssert("Mutex::trylock mutex alreay inuse", __FILE__,
                              __LINE__);
  return false;
}

inline void Mutex::unlock(SimulatedBlock::Callback &callback) {
  if (!m_ptr.isNull()) {
    m_mgr.getPtr(m_ptr);
  ...
  ErrorReporter::handleAssert("Mutex::unlock invalid mutex", __FILE__,
                              __LINE__);
}

inline bool Mutex::create(SimulatedBlock::Callback &callback) {
  if (m_ptr.isNull()) {
    if (m_mgr.seize(m_ptr)) {
      m_ptr.p->m_mutexId = m_mutexId;
      m_ptr.p->m_callback = callback;
  ...
  ErrorReporter::handleAssert("Mutex::create mutex alreay inuse", __FILE__,
                              __LINE__);
  return false;
}

inline bool Mutex::destroy(SimulatedBlock::Callback &callback) {
  if (m_ptr.isNull()) {
    if (m_mgr.seize(m_ptr)) {
      m_ptr.p->m_mutexId = m_mutexId;
      m_ptr.p->m_callback = callback;
  ...
  ErrorReporter::handleAssert("Mutex::destroy mutex alreay inuse", __FILE__,
                              __LINE__);
  return false;
}
