...
NdbOut &operator<<(NdbOut &out, const Record &rec) {
...
static bool cmp(const Uint32 *p1, const Uint32 *p2, Uint32 words) {
  if (memcmp(p1, p2, 4 * words) == 0) return true;

  for (Uint32 i = 0; i < words; i++) printf(" %.8x", p1[i]);
  printf("\n");

  for (Uint32 i = 0; i < words; i++) printf(" %.8x", p2[i]);
  printf("\n");

  return false;
}

static void do_test(int loops, int dist[5]) {
  ...
  loop:
    ...
    switch (op) {
      case 0:  // Alloc
      {
        ...
        if (rnd < 30) {
          ...
          if (free.size()) {
            ...
            if (page.alloc_record(rec.idx, rec.size, &tmp) != rec.idx) {
              if (rec.size + 1 != page.free_space) abort();
              delete[] rec.data;
              ndbout_c(" FAIL");
              break;
            }
      ...
      case 3: {
        ...
        int no = rand() % allocated;
        Record rec = records[no];
        ndbout << "Expand no: " << no << " idx: " << rec.idx << " add: " << free
               << " reorg: " << !page.is_space_behind_entry(rec.idx, free)
               << endl;
