...
Dbtup::scanFirst(Signal*, ScanOpPtr scanPtr)
{
  ...
  if (bits & ScanOp::SCAN_LCP)
  {
    ...
    c_backup->init_lcp_scan(frag.m_max_page_cnt,
                            scan.m_scanGCI,
                            skip_flag,
        ...
        if (key.m_page_idx + size <= Fix_page::DATA_WORDS) 
	{
	  ...
	  if (likely((! ((bits & ScanOp::SCAN_NR) ||
                      ...
                      !pos.m_lcp_scan_changed_rows_page)))
          {
            ...
            {
              jam();
              DEB_LCP(("(%u)LCP_DELETE on page: %u, page_idx: %u ALL ROWS"
                       " page, header: %x",
                      instance(),
                      key.m_page_no,
                      key.m_page_idx,
          ...
          else
          {
            ...
            if ((foundGCI = *th->get_mm_gci(tablePtr.p)) > scanGCI)
            {
              if (unlikely(thbits & Tuple_header::LCP_DELETE))
              {
                ...
                DEB_LCP(("(%u)Reset LCP_DELETE on page: %u, idx: %u,"
                         " header: %x",
                         instance(),
                         key.m_page_no,
                         key.m_page_idx,
...
void
Dbtup::prepare_lcp_scan(Uint32 tableId, Uint32 fragId)
{
  jamEntry();
  TablerecPtr tablePtr;
  tablePtr.i = tableId;
