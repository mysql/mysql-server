...
Uint32 *Dbtup::alloc_var_rec(Uint32 *err, Fragrecord *fragPtr, Tablerec *tabPtr,
                             Uint32 alloc_size, Local_key *key,
                             Uint32 *out_frag_page_id) {
  /**
   * TODO alloc fix+var part
   */
  Uint32 *ptr =
      alloc_fix_rec(jamBuffer(), err, fragPtr, tabPtr, key, out_frag_page_id);
  if (unlikely(ptr == 0)) {
    return 0;
  }
  ...
  if (alloc_size) {
    if (likely(alloc_var_part(err, fragPtr, tabPtr, alloc_size, &varref) !=
               0)) {
      dst->assign(&varref);
      return ptr;
    }
...
Uint32 *Dbtup::alloc_var_part(Uint32 *err, Fragrecord *fragPtr,
                              Tablerec *tabPtr, Uint32 alloc_size,
                              Local_key *key) {
  PagePtr pagePtr;
...
Uint32 *Dbtup::realloc_var_part(Uint32 *err, Fragrecord *fragPtr,
  ...
  if (oldsz && pageP->free_space >= add) {
    ...
    if (unlikely(new_var_ptr == 0)) return NULL;
...
/* ------------------------------------------------------------------------ */
Uint32 Dbtup::get_alloc_page(Fragrecord *fragPtr, Uint32 alloc_size) {
  Uint32 start_index;
  PagePtr pagePtr;
...
Uint32 Dbtup::get_empty_var_page(Fragrecord *fragPtr) {
  PagePtr ptr;
  Uint32 cnt;
...
/* ------------------------------------------------------------------------ */
void Dbtup::update_free_page_list(Fragrecord *fragPtr, Ptr<Page> pagePtr) {
  ...
  if ((free_space < c_min_list_size[list_index]) ||
...

    {
       ...
       */
      Local_Page_list list(c_page_pool,
                           fragPtr->free_var_page_array[list_index]);
      list.remove(pagePtr);
    }
...
Uint64 Dbtup::calculate_used_var_words(Fragrecord *fragPtr) {
  ...
  for (Uint32 freeList = 0; freeList <= MAX_FREE_LIST; freeList++) {
    Local_Page_list list(c_page_pool, fragPtr->free_var_page_array[freeList]);
    Ptr<Page> pagePtr;
...
Uint32 *Dbtup::alloc_var_rowid(Uint32 *err, Fragrecord *fragPtr,
                               Tablerec *tabPtr, Uint32 alloc_size,
                               Local_key *key, Uint32 *out_frag_page_id) {
