...
int Dbtup::tuxReadAttrsCommon(KeyReqStruct &req_struct, const Uint32 *attrIds,
  ...
  // do it
  int ret = readAttributes(&req_struct, attrIds, numAttrs, dataOut, ZNIL);
  // done
  return ret;
}

int Dbtup::tuxReadPk(Uint32 *fragPtrP_input, Uint32 *tablePtrP_input,
                     Uint32 pageId, Uint32 pageIndex, Uint32 *dataOut,
                     bool xfrmFlag) {
  jamEntryDebug();
  Fragrecord *fragPtrP = (Fragrecord *)fragPtrP_input;
  Tablerec *tablePtrP = (Tablerec *)tablePtrP_input;
 ...
 *
 */
bool Dbtup::tuxQueryTh(Uint32 opPtrI, Uint32 tupVersion, Uint32 transId1,
  ...
  const bool sameTrans = c_lqh->m_ldm_instance_used->is_same_trans(
      currOpPtr.p->userpointer, transId1, transId2);
  ...
  if (!sameTrans) {
    ...
    if (!dirty) {
      ...
      m_ldm_instance_used->find_savepoint(loopOpPtr, 0, jamBuffer());
    ...
    if (m_ldm_instance_used->find_savepoint(loopOpPtr, savepointId,
                                            jamBuffer())) {
      jamDebug();
      const Uint32 op_type = loopOpPtr.p->op_type;
...
void Dbtup::buildIndex(Signal *signal, Uint32 buildPtrI) {
  // get build record
  BuildIndexPtr buildPtr;
  ...
  next_tuple:
    ...
    if (pageOperPtr.i == RNIL) {
      EXECUTE_DIRECT(buildPtr.p->m_buildRef, GSN_TUX_MAINT_REQ, signal,
                     TuxMaintReq::SignalLength + 2);
    } else {
      /*
    ...
    if (time_passed < 1000) {
      ...
      if (time_events == number_events) {
        Uint64 mean_time_passed = tot_time_passed / (Uint64)number_events;
...
void Dbtup::buildIndexOffline(Signal *signal, Uint32 buildPtrI) {
  jam();
  ndbassert(!m_is_query_block);
  /**
...
int Dbtup::mt_scan_next(Uint32 tableId, Uint32 fragPtrI, Local_key *pos,
  ...
  while (1) {
    ...
    if (fragPageId == fragPtr.p->m_max_page_cnt) break;
...
void Dbtup::execBUILD_INDX_IMPL_REF(Signal *signal) {
  jamEntry();
...
void Dbtup::execBUILD_INDX_IMPL_CONF(Signal *signal) {
  jamEntry();
