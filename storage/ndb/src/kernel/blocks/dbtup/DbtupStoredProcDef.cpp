...
void Dbtup::execSTORED_PROCREQ(Signal *signal) {
  ...
  switch (requestInfo) {
    case ZSCAN_PROCEDURE: {
      ...
      scanProcedure(signal, regOperPtr.p, &handle,
                    false);  // Not copy
      break;
    }
...
void Dbtup::scanProcedure(Signal *signal, Operationrec *regOperPtr,
  ...
  if (unlikely(!c_storedProcPool.seize(storedPtr))) {
    jam();
    handle->clear();
    storedProcBufferSeizeErrorLab(signal, regOperPtr, RNIL,
...
void Dbtup::prepareCopyProcedure(Uint32 numAttrs, Uint16 tableBits) {
...
void Dbtup::releaseCopyProcedure() {
  ...
  if (cCopyOverwriteLen) {
    ...
    ndbrequire(
        writeToSection(first.i, cCopyOverwrite, attrids, cCopyOverwriteLen));
    cCopyOverwriteLen = 0;
    cCopyOverwrite = 0;
  }
...
void Dbtup::copyProcedure(Signal *signal, TablerecPtr regTabPtr,
   ...
   * as well.
   */
  prepareCopyProcedure(regTabPtr.p->m_no_of_attributes, regTabPtr.p->m_bits);
  ...
  scanProcedure(signal, regOperPtr, &handle,
                true);  // isCopy
  Ptr<SectionSegment> first;
  ndbrequire(g_sectionSegmentPool.getPtr(first, cCopyProcedure));
