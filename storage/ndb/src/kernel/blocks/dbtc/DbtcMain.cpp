...
Dbtc::updateBuddyTimer(ApiConnectRecordPtr apiPtr)
{
  if (apiPtr.p->buddyPtr != RNIL) {
    ...
    if (getApiConTimer(buddyApiPtr) != 0) {
                                ...
                                Uint32 senderRef)
{
  ...
  for (api_ptr = start_api_ptr;
       ...
       api_ptr++, loop_count++)
  {
    ApiConnectRecord * const regApiPtr = c_apiConnectRecordPool.getPtr(api_ptr);
                          ...
                          UintR TapiConnectPtr)
{
  ...
  do {
    ...
    c_apiConnectRecordPool.getPtr(apiConnectptr, false);
    ...
    TapiConnectPtr++;
    if (TapiConnectPtr > ((capiConnectFilesize / 3) - 1)) {
      jam();
      /**
...
void Dbtc::execATTRINFO(Signal* signal) 
{
  ...
  ApiConnectRecord * const regApiPtr = c_apiConnectRecordPool.getPtr(Tdata1);
...
void Dbtc::execTCKEYREQ(Signal* signal) 
{
  ...
  if (regApiPtr->apiCopyRecord == RNIL)
  {
    ...
    if (cfirstfreeApiConnectCopy == RNIL)
    {
      jam();
      releaseSections(handle);
...
void Dbtc::seizeApiConnectCopy(Signal* signal, ApiConnectRecord* const regApiPtr)
{
  ...
  ndbrequire(locApiConnectptr.p->apiConnectkind == ApiConnectRecord::CK_FREE);
...
void Dbtc::releaseApiConCopy(Signal* signal, ApiConnectRecordPtr const apiConnectptr)
{
  ...
  ndbrequire(regApiPtr->commitAckMarker == RNIL);
  regApiPtr->apiConnectkind = ApiConnectRecord::CK_COPY;
				...
				Uint32 failedNodeId)
{
  ...
  for (; transPtrI < endPtrI; transPtrI++)
  {
    ...
    c_apiConnectRecordPool.getPtr(transPtr, false);
    if (transPtr.i != RNIL && transPtr.p->m_transaction_nodes.get(failedNodeId))
    {
      jam();
...
void Dbtc::toCommitHandlingLab(Signal* signal, ApiConnectRecordPtr apiConnectptr)
{
  do {
    if (tcurrentReplicaNo != (Uint8)Z8NIL) {
      jam();
...
void Dbtc::execDBINFO_SCANREQ(Signal *signal)
{
  ...
  case Ndbinfo::TRANSACTIONS_TABLEID:
  {
    ...
    for (Uint32 i = 0; i < maxloop; i++)
    {
      ...
      ptr.i = api_ptr;
      c_apiConnectRecordPool.getPtr(ptr, false);
      if (ptr.i != RNIL)
      {
      Ndbinfo::Row row(signal, req);
...
void Dbtc::execTCINDXREQ(Signal* signal)
{
  ...
  c_apiConnectRecordPool.getPtr(transPtr); // TODO YYY fail if invalid or null
  ApiConnectRecord * const regApiPtr = transPtr.p;  
