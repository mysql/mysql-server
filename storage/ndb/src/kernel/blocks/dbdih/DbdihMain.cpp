...
void Dbdih::execREAD_CONFIG_REQ(Signal *signal) {
  ...
  ndbrequireErr(
      !ndb_mgm_get_int_parameter(p, CFG_DIH_FRAG_CONNECT, &cfragstoreFileSize),
      NDBD_EXIT_INVALID_CONFIG);
  ndbrequireErr(
      !ndb_mgm_get_int_parameter(p, CFG_DIH_REPLICAS, &creplicaFileSize),
      NDBD_EXIT_INVALID_CONFIG);
  ...
  if (!m_use_classic_fragmentation) {
    ...
    if ((c_fragments_per_node_ * cnoReplicas) / globalData.ndbMtLqhWorkers >
        MAX_FRAG_PER_LQH) {
      progError(
          __LINE__, NDBD_EXIT_INVALID_CONFIG,
...
void Dbdih::execUPDATE_FRAG_STATEREQ(Signal *signal) {
  ...
  make_table_use_new_replica(tabPtr, fragPtr, frReplicaPtr, replicaType,
                             tdestNodeid);
...
void Dbdih::nr_start_logging(Signal *signal, TakeOverRecordPtr takeOverPtr) {
  ...
  while (loopCount++ < 100) {
    ...
    if (!check_takeover_thread(takeOverPtr, fragPtr, instanceKey)) {
      jam();
      /**
...
void Dbdih::execCREATE_FRAGMENTATION_REQ(Signal *signal) {
  ...
  if (flags != CreateFragmentationReq::RI_GET_FRAGMENTATION) {
    D("CREATE_FRAGMENTATION_REQ: "
      ...
      << " partitionBalance: " << getPartitionBalanceString(partitionBalance)
      << " fragType: " << fragType << " noOfFragments: " << noOfFragments);
  }
  ...
  do {
    ...
    if (primaryTableId == RNIL && !m_use_classic_fragmentation &&
      ...
      switch ((DictTabInfo::FragmentType)fragType) {
        ...
        case DictTabInfo::HashMapPartition: {
          ...
          if (noOfFragments == 0 || partitionCount != ptr.p->m_fragments ||
      ...
      for (Uint32 fragNo = 0; fragNo < noOfFragments; fragNo++) {
        ...
        add_nodes_to_fragment(fragments, node_index, count, NGPtr,
                              noOfReplicas);
        (*next_replica_node)[NGPtr.i][0] = node_index;
        inc_ng(fragNo, noOfFragments, partitionCount, default_node_group,
               cnoOfNodeGroups);
      }
      if (err) break;
      ...
      switch ((DictTabInfo::FragmentType)fragType) {
        ...
        case DictTabInfo::AllNodesMediumTable:
          ...
          if (noOfFragments > maxFragments) noOfFragments = maxFragments;
          partitionCount = noOfFragments;
          set_default_node_groups(signal, noOfFragments);
          break;
      ...
      */
      ...
      for (Uint32 fragNo = 0; fragNo < noOfFragments; fragNo++) {
        ...
        if (use_specific_fragment_count) {
          ...
          jam();
          if (NGPtr.i == 0 || (noOfFragments != partitionCount)) {
      ...
      memcpy(tmp_next_replica_node, c_next_replica_node,
      ...
      for (Uint32 fragNo = 0; fragNo < primTabPtr.p->totalfragments; fragNo++) {
        ...
        for (Uint32 replicaNo = 0; replicaNo < noOfReplicas; replicaNo++) {
          jam();
          if (fragPtr.p->preferredPrimary == NGPtr.p->nodesInGroup[replicaNo]) {
            Uint32 node_index = replicaNo;
            inc_node_or_group(node_index, NGPtr.p->nodeCount);
            ndbrequire(node_index < noOfReplicas);
            Uint32 tmp_log_part_id =
                (m_use_classic_fragmentation) ? log_part_id : 0;
      ...
      if (flags == CreateFragmentationReq::RI_GET_FRAGMENTATION) {
        ...
        Uint32 first_new_node =
            find_min_index(tmp_fragments_per_node, m_max_node_id + 1, 1, 1);
        ...
        for (Uint32 i = primTabPtr.p->totalfragments; i < noOfFragments; i++) {
          ...
          if (!m_use_classic_fragmentation) {
            ...
            logPart = find_min_index(&tmp_fragments_per_ldm[node][0],
                                     globalData.ndbLogParts, logPart, 0);
    ...
    if (flags == CreateFragmentationReq::RI_ADD_FRAGMENTS ||
      ...
      if (!verify_fragmentation(fragments, partitionCount, partitionBalance,
                                getFragmentsPerNode())) {
        err = CreateFragmentationRef::InvalidFragmentationType;
        break;
      }
    ...
    if (senderRef != 0) {
      ...
      ptr[0].sz = (count + 1) / 2;
      sendSignal(senderRef, GSN_CREATE_FRAGMENTATION_CONF, signal,
                 CreateFragmentationConf::SignalLength, JBB, ptr, 1);
    }
...
bool Dbdih::verify_fragmentation(Uint16 *fragments, Uint32 partition_count,
  ...
  for (Uint32 node = 1; node <= m_max_node_id; node++) {
    jam();
    if (balance_for_ra_by_node_count != 0 && fragments_per_node[node] != 0 &&
...
void Dbdih::insertCopyFragmentList(TabRecord *tabPtr, Fragmentstore *fragPtr,
                                   Uint32 my_fragid) {
...
void Dbdih::addTable_closeConf(Signal *signal, Uint32 tabPtrI) {
  TabRecordPtr tabPtr;
  tabPtr.i = tabPtrI;
  ptrCheckGuard(tabPtr, ctabFileSize, tabRecord);
...
void Dbdih::release_connect(ConnectRecordPtr ptr) {
  TabRecordPtr tabPtr;
...
void Dbdih::execADD_FRAGCONF(Signal *signal) {
  jamEntry();
...
/*
  ...
  *********************
  */
void Dbdih::addtabrefuseLab(Signal *signal, ConnectRecordPtr connectPtr,
                            Uint32 errorCode) {
...
/***********              DELETE TABLE  MODULE                   *************/
/*****************************************************************************/
void Dbdih::execDROP_TAB_REQ(Signal *signal) {
  jamEntry();
  ...
  if (isMaster()) {
    ...
    for (nodePtr.i = 1; nodePtr.i <= m_max_node_id; nodePtr.i++) {
      ...
      if (c_lcpState.m_participatingLQH.get(nodePtr.i)) {
        ...
        DEB_LCP(
            ("DROP_TAB_REQ: nodePtr(%u)->noOfQueuedChkpt = %u"
             ", nodePtr->noOfStartedChkpt = %u"
             ", tab: %u",
             nodePtr.i, nodePtr.p->noOfQueuedChkpt, nodePtr.p->noOfStartedChkpt,
             tabPtr.i));
      }
    }
  }
...

  {
    ...
    switch (tabPtr.p->tabLcpStatus) {
      ...
      case TabRecord::TLS_WRITING_TO_FILE:
        ok = true;
        jam();
        g_eventLogger->info("DROP_TAB_REQ: tab: %u, tabLcpStatus: %u", tabPtr.i,
                            tabPtr.p->tabLcpStatus);
        break;
        return;
      case TabRecord::TLS_ACTIVE:
        ...
        g_eventLogger->info("DROP_TAB_REQ: tab: %u, tabLcpStatus set to %u",
                            tabPtr.i, tabPtr.p->tabLcpStatus);
        /**
...
void Dbdih::tableDeleteLab(Signal *signal, FileRecordPtr filePtr) {
  ...
  sendSignal(tabPtr.p->m_dropTab.tabUserRef, GSN_DROP_TAB_CONF, signal,
...
void Dbdih::execALTER_TAB_REQ(Signal *signal) {
  ...
  if (AlterTableReq::getAddFragFlag(req->changeMask)) {
    ...
    copy(_align, ptr);
    releaseSections(handle);
    start_add_fragments_in_new_table(tabPtr, connectPtr, buf, ptr.sz * 2,
                                     signal);
    return;
  }
...
Uint32 Dbdih::add_fragments_to_table(Ptr<TabRecord> tabPtr, const Uint16 buf[],
                                     const Uint32 bufLen) {
  ...
  for (i = 0; i < cnt; i++) {
    ...
    if ((err = add_fragment_to_table(tabPtr, fragId, fragPtr))) goto error;
    ...
    for (Uint32 j = 0; j < replicas; j++) {
      ...
      ReplicaRecordPtr replicaPtr;
      allocStoredReplica(fragPtr, replicaPtr, nodeId, current + i, tabPtr.i);
      if (getNodeStatus(nodeId) == NodeRecord::ALIVE) {
        jam();
        ndbrequire(activeIndex < MAX_REPLICAS);
        fragPtr.p->activeNodes[activeIndex] = nodeId;
...
void Dbdih::wait_old_scan(Signal *signal) {
  jam();
...
Uint32 Dbdih::add_fragment_to_table(Ptr<TabRecord> tabPtr, Uint32 fragId,
...
void Dbdih::send_alter_tab_ref(Signal *signal, Ptr<TabRecord> tabPtr,
                               Ptr<ConnectRecord> connectPtr, Uint32 errCode) {
...
void Dbdih::send_alter_tab_conf(Signal *signal, Ptr<ConnectRecord> connectPtr) {
  AlterTabConf *conf = (AlterTabConf *)signal->getDataPtrSend();
  conf->senderRef = reference();
  conf->senderData = connectPtr.p->userpointer;
...
void Dbdih::saveTableFile(Signal *signal, Ptr<ConnectRecord> connectPtr,
                          Ptr<TabRecord> tabPtr,
                          TabRecord::CopyStatus expectedStatus, Callback &cb) {
...
void Dbdih::alter_table_writeTable_conf(Signal *signal, Uint32 ptrI,
                                        Uint32 err) {
  jamEntry();
  ndbrequire(err == 0);
...
void Dbdih::drop_fragments(Signal *signal, Ptr<ConnectRecord> connectPtr,
                           Uint32 curr) {
  ...
  if (curr == connectPtr.p->m_alter.m_org_totalfragments) {
    ...
    switch (connectPtr.p->connectState) {
      ...
      case ConnectRecord::ALTER_TABLE_REVERT: {
        ...
        cb.m_callbackData = connectPtr.i;
        cb.m_callbackFunction = safe_cast(&Dbdih::alter_table_writeTable_conf);
        saveTableFile(signal, connectPtr, tabPtr, TabRecord::CS_ALTER_TABLE,
                      cb);
        return;
      }
      default:
  ...
  sendSignal(DBLQH_REF, GSN_DROP_FRAG_REQ, signal, DropFragReq::SignalLength,
             JBB);
}

void Dbdih::execDROP_FRAG_REF(Signal *signal) { ndbabort(); }

void Dbdih::execDROP_FRAG_CONF(Signal *signal) {
...
loop:
  ...
  sig2 = (nodeCount - 1) + (fragPtr.p->distributionKey << 16);
  ...
  if (unlikely(newFragId != RNIL)) {
    ...
    nodeCount = extractNodeInfo(jambuf, fragPtr.p,
                                conf->nodes + 3 + MAX_REPLICAS, false);
    if (unlikely(nodeCount == 0)) {
      thrjam(jambuf);
      goto crash_check_exit;
    }
    conf->nodes[MAX_REPLICAS] = newFragId;
    conf->nodes[MAX_REPLICAS + 1] =
        (nodeCount - 1) + (fragPtr.p->distributionKey << 16);
    conf->nodes[MAX_REPLICAS + 2] = dihGetInstanceKey(fragPtr);
  }

check_exit:
  if (unlikely(!tabPtr.p->m_lock.read_unlock(tab_val))) goto loop;
  if (unlikely(!m_node_view_lock.read_unlock(node_val))) goto loop;
  return;
...
void Dbdih::make_node_usable(NodeRecord *nodePtr) {
  /**
...
void Dbdih::make_node_not_usable(NodeRecord *nodePtr) {
  /**
...
Uint32 Dbdih::findPartitionOrder(const TabRecord *tabPtrP,
                                 FragmentstorePtr fragPtr) {
  Uint32 order = 0;
...
Uint32 Dbdih::findFirstNewFragment(const TabRecord *tabPtrP,
                                   FragmentstorePtr &fragPtr, Uint32 fragId,
...
Uint32 Dbdih::findLocalFragment(const TabRecord *tabPtrP,
                                FragmentstorePtr &fragPtr,
                                EmulatedJamBuffer *jambuf) {
  /**
...
bool Dbdih::check_if_local_fragment(EmulatedJamBuffer *jambuf,
...
void Dbdih::complete_scan_on_table(TabRecordPtr tabPtr, Uint32 scanCookie,
                                   EmulatedJamBuffer *jambuf) {
  /**
...
bool Dbdih::prepare_add_table(TabRecordPtr tabPtr, ConnectRecordPtr connectPtr,
                              Signal *signal) {
  DiAddTabReq *const req = (DiAddTabReq *)signal->getDataPtr();
  ...
  if (req->fullyReplicated) {
    ...
    D("fully replicated, partitionCount = " << tabPtr.p->partitionCount);
    ...
    ptrCheckGuard(primTabPtr, ctabFileSize, tabRecord);
    tabPtr.p->m_flags |=
        (primTabPtr.p->m_flags & TabRecord::TF_FULLY_REPLICATED);
    tabPtr.p->partitionCount = primTabPtr.p->partitionCount;
    D("Non-primary, m_flags: " << tabPtr.p->m_flags << " partitionCount: "
...
void Dbdih::commit_new_table(TabRecordPtr tabPtr) {
  /**
 ...
 * distribution.
 */
void Dbdih::start_add_fragments_in_new_table(TabRecordPtr tabPtr,
                                             ConnectRecordPtr connectPtr,
                                             const Uint16 buf[],
                                             const Uint32 bufLen,
                                             Signal *signal) {
  ...
  sendAddFragreq(signal, connectPtr, tabPtr,
                 connectPtr.p->m_alter.m_org_totalfragments, true);
 ...
 */
void Dbdih::make_new_table_writeable(TabRecordPtr tabPtr,
                                     ConnectRecordPtr connectPtr,
                                     bool rcu_lock_held) {
  ...
  if ((tabPtr.p->m_flags & TabRecord::TF_FULLY_REPLICATED) != 0 &&
    ...
    for (Uint32 i = tabPtr.p->totalfragments;
         i < connectPtr.p->m_alter.m_totalfragments; i++) {
      jam();
      FragmentstorePtr fragPtr;
 ...
 */
void Dbdih::make_new_table_read_and_writeable(TabRecordPtr tabPtr,
                                              ConnectRecordPtr connectPtr,
                                              Signal *signal) {
  jam();
 ...
 */
bool Dbdih::make_old_table_non_writeable(TabRecordPtr tabPtr,
                                         ConnectRecordPtr connectPtr) {
 ...
 */
void Dbdih::make_table_use_new_replica(TabRecordPtr tabPtr,
                                       FragmentstorePtr fragPtr,
                                       ReplicaRecordPtr replicaPtr,
                                       Uint32 replicaType, Uint32 destNodeId) {
  D("make_table_use_new_replica: tableId: "
    << tabPtr.i << " fragId = " << fragPtr.p->fragId
    << " replicaType = " << replicaType << " destNodeId = " << destNodeId);
 ...
 */
void Dbdih::make_table_use_new_node_order(TabRecordPtr tabPtr,
                                          FragmentstorePtr fragPtr,
                                          Uint32 numReplicas,
                                          Uint32 *newNodeOrder) {
  D("make_table_use_new_node_order: tableId = " << tabPtr.i << " fragId = "
                                                << fragPtr.p->fragId);
 ...
 */
void Dbdih::drop_fragments_from_new_table_view(TabRecordPtr tabPtr,
                                               ConnectRecordPtr connectPtr) {
...
void Dbdih::getFragstore(const TabRecord *tab,       // In parameter
...
void Dbdih::getFragstoreCanFail(const TabRecord *tab,       // In parameter
                                Uint32 fragNo,              // In parameter
                                FragmentstorePtr &fragptr)  // Out parameter
{
  FragmentstorePtr fragPtr;
...
void Dbdih::allocFragments(Uint32 noOfFragments, TabRecordPtr tabPtr) {
  FragmentstorePtr fragPtr;
  Uint32 noOfChunks = (noOfFragments + (NO_OF_FRAGS_PER_CHUNK - 1)) >>
...
inline void Dbdih::dequeue(DIVERIFY_queue &q) {
  Uint32 first = q.cfirstVerifyQueue;
...
loop:
  ...
  if (blocked == 0) {
    ...
    signal->theData[3] = 0;
    if (unlikely(!m_micro_gcp.m_lock.read_unlock(val))) goto loop;
    return;
  }  // if
  /*-------------------------------------------------------------------------*/
...
void Dbdih::execDIH_SCAN_TAB_COMPLETE_REP(Signal *signal) {
...
bool Dbdih::check_enable_micro_gcp(Signal *signal, bool broadcast) {
...
void Dbdih::execUPGRADE_PROTOCOL_ORD(Signal *signal) {
...
void Dbdih::checkGCI(Uint32 save_gci) {
  if (save_gci >= next_warn_save_gci) {
    ...
    warningEvent("GCI (%u) is approaching the maximum value (%u).", save_gci,
                 MAX_SAFE_GCI_VALUE);
    warningEvent("GCI projected to reach max in %u days.", days_left);
    warningEvent(
        "Reset system GCI using Backup and Restore with "
        "Cluster Initial Restart.");

    g_eventLogger->warning(
        "Current Global checkpoint id (%u) "
...
void Dbdih::startGcpLab(Signal *signal) {
  ...
  emptyWaitGCPMasterQueue(signal, m_micro_gcp.m_current_gci,
  ...
  const bool need_gcp_save =
      (!NdbTick_IsValid(start) ||  // First or forced GCP
       NdbTick_Elapsed(start, now).milliSec() >=
           delaySave) &&       // Reached time limit
  ...
  if (ERROR_INSERTED(7186)) {
    sendToRandomNodes("GCP_PREPARE", signal, &c_GCP_PREPARE_Counter,
                      &Dbdih::sendGCP_PREPARE);
    signal->theData[0] = 9999;
    ...
    signal->theData[0] = 9999;
    sendSignal(numberToRef(CMVMI, victim), GSN_NDB_TAMPER, signal, 1, JBA);
...
void Dbdih::execGCP_PREPARECONF(Signal *signal) {
  ...
  DEB_NODE_STOP(
      ("Recv GCP_PREPARECONF(%u,%u) from %u", gci_hi, gci_lo, senderNodeId));
...
void Dbdih::gcpcommitreqLab(Signal *signal) {
  ...
  if (ERROR_INSERTED(7187)) {
    sendToRandomNodes("GCP_COMMIT", signal, &c_GCP_COMMIT_Counter,
                      &Dbdih::sendGCP_COMMIT);
    signal->theData[0] = 9999;
    sendSignalWithDelay(CMVMI_REF, GSN_NDB_TAMPER, signal, 1000, 1);
    return;
...
void Dbdih::execGCP_NODEFINISH(Signal *signal) {
  ...
  DEB_NODE_STOP(
      ("Recv GCP_NODEFINISH(%u,%u) from %u", gci_hi, gci_lo, senderNodeId));

  /* Check that there has not been a node failure since TC
   * reported this GCP complete...
   */
  if ((senderNodeId == getOwnNodeId()) && (tcFailNo < cMinTcFailNo)) {
    jam();
     ...
     */
    m_micro_gcp.m_state =
        MicroGcp::M_GCP_COMMIT; /* Reset DIH Slave GCP state */
  ...
  if (ERROR_INSERTED(7188)) {
    sendToRandomNodes("GCP_SAVE", signal, &c_GCP_SAVEREQ_Counter,
                      &Dbdih::sendGCP_SAVEREQ);
    signal->theData[0] = 9999;
    sendSignalWithDelay(CMVMI_REF, GSN_NDB_TAMPER, signal, 1000, 1);
    return;
...
void Dbdih::execSUB_GCP_COMPLETE_ACK(Signal *signal) {
  jamEntry();
  SubGcpCompleteAck ack =
      *CAST_CONSTPTR(SubGcpCompleteAck, signal->getDataPtr());
...
void Dbdih::execGCP_SAVEREQ(Signal *signal) {
  jamEntry();
  ...
  if (ERROR_INSERTED(7237)) {
    ...
    sendSignalWithDelay(reference(), GSN_GCP_SAVEREQ, signal, 5000,
                        signal->getLength());
    return;
  }
...
void Dbdih::execGCP_SAVEREF(Signal *signal) {
  ...
  if (refToBlock(signal->getSendersBlockRef()) == DBLQH) {
    ...
    sendSignal(m_gcp_save.m_master_ref, GSN_GCP_SAVEREF, signal,
               signal->getLength(), JBA);
    return;
  }
...
void Dbdih::execGCP_PREPARE(Signal *signal) {
  ...
  if (ERROR_INSERTED(7030) || ERROR_INSERTED(7238)) {
  ...
  DEB_NODE_STOP(
      ("Recv GCP_PREPARE(%u,%u) from %u", gci_hi, gci_lo, masterNodeId));
...
reply:
  ...
  conf->gci_hi = gci_hi;
  conf->gci_lo = gci_lo;
  DEB_NODE_STOP(("Send GCP_PREPARECONF(%u,%u) to %u", req->gci_hi, req->gci_lo,
                 refToNode(retRef)));
  sendSignal(retRef, GSN_GCP_PREPARECONF, signal, GCPPrepareConf::SignalLength,
             JBA);
  return;
}
...
void Dbdih::execGCP_COMMIT(Signal *signal) {
  ...
  DEB_NODE_STOP(
      ("Recv GCP_COMMIT(%u,%u) from %u", gci_hi, gci_lo, masterNodeId));
  ...
  if (m_micro_gcp.m_current_gci == gci) {
    ...
    DEB_NODE_STOP(("Send GCP_NODEFINISH(%u,%u) to %u", conf->gci_hi,
                   conf->gci_lo, refToNode(masterRef)));
...
void Dbdih::execGCP_TCFINISHED(Signal *signal) {
  ...
  if (ERROR_INSERTED(7181) || ERROR_INSERTED(7182)) {
    ...
    signal->theData[0] = 9999;
    sendSignal(numberToRef(CMVMI, refToNode(cmasterdihref)), GSN_NDB_TAMPER,
               signal, 1, JBB);
    return;
  }
  ...
  if (ERROR_INSERTED(7214)) {
    ...
    Uint32 save = signal->theData[0];
    signal->theData[0] = 9999;
    sendSignal(numberToRef(CMVMI, c_error_insert_extra), GSN_NDB_TAMPER, signal,
               1, JBB);
    signal->theData[0] = save;
    CLEAR_ERROR_INSERT_VALUE;
  }
  ...
  Callback cb;
  cb.m_callbackData =
      tcFailNo; /* Pass fail-no triggering TC_FINISHED to callback */
...
void Dbdih::execGCP_TCFINISHED_sync_conf(Signal *signal, Uint32 cb,
                                         Uint32 err) {
  ndbrequire(m_micro_gcp.m_state == MicroGcp::M_GCP_COMMIT);
  ...
  DEB_NODE_STOP(("2:Send GCP_NODEFINISH(%u,%u) to %u", conf2->gci_hi,
                 conf2->gci_lo, refToNode(retRef)));

  sendSignal(retRef, GSN_GCP_NODEFINISH, signal, GCPNodeFinished::SignalLength,
             JBB);
}

void Dbdih::execSUB_GCP_COMPLETE_REP(Signal *signal) {
  jamEntry();
  ...
  */
  ...
  if (gci > m_micro_gcp.m_last_sent_gci) {
     ...
     */
    sendSignal(DBLQH_REF, GSN_SUB_GCP_COMPLETE_REP, signal, signal->length(),
               JBB);
...
reply:
  ...
  sendSignal(masterRef, GSN_SUB_GCP_COMPLETE_ACK, signal,
             SubGcpCompleteAck::SignalLength, JBA);
}
  ...
  switch (tcgcpblocked) {
    ...
    case 5:
      ...
      if (tuserpointer >= 30000 && tuserpointer < 40000) {
        ...
        /*--------------------------------------------------------------------*/
        tuserpointer -= 40000;
        for (localNodeptr.i = 1; localNodeptr.i <= m_max_node_id;
             localNodeptr.i++) {
          jam();
...
void Dbdih::execCOPY_GCICONF(Signal *signal) {
  ...
  switch (current) {
    case CopyGCIReq::RESTART: {
      ...
      req->senderRef = reference();
      sendSignal(cdictblockref, GSN_DICTSTARTREQ, signal,
                 DictStartReq::SignalLength, JBB);
      break;
    }
    case CopyGCIReq::LOCAL_CHECKPOINT: {
      ok = true;
    ...
    case CopyGCIReq::GLOBAL_CHECKPOINT: {
      ...
      emptyWaitGCPMasterQueue(signal, Uint64(m_gcp_save.m_gci) << 32,
                              c_waitGCPMasterList);
      break;
    }
    case CopyGCIReq::INITIAL_START_COMPLETED:
...
void Dbdih::check_node_in_restart(Signal *signal, BlockReference ref,
                                  Uint32 nodeId) {
  NodeRecordPtr nodePtr;
  if (m_max_node_id == 0) {
    jam();
...
void Dbdih::sendCHECK_NODE_RESTARTCONF(Signal *signal, BlockReference ref,
                                       Uint32 node_restart) {
  signal->theData[0] =
      (m_local_lcp_state.m_state == LocalLCPState::LS_RUNNING) ? 1 : 0;
...
void Dbdih::writingCopyGciLab(Signal *signal, FileRecordPtr filePtr) {
  ...
  if (reason == CopyGCIReq::GLOBAL_CHECKPOINT) {
    ...
    // this is last timer point so we send local report here
    {
      ...
      const Uint32 ms_commit =
          NdbTick_Elapsed(g.gcp_timer_commit[0], g.gcp_timer_commit[1])
              .milliSec();
      const Uint32 ms_save =
          NdbTick_Elapsed(g.gcp_timer_save[0], g.gcp_timer_save[1]).milliSec();
      const Uint32 ms_copygci =
          NdbTick_Elapsed(g.gcp_timer_copygci[0], g.gcp_timer_copygci[1])
              .milliSec();

      const Uint32 ms_total = ms_commit + ms_save + ms_copygci;

      // random formula to report excessive duration
      bool report = g.gcp_timer_limit != 0
                        ? (ms_total > g.gcp_timer_limit)
                        : (ms_total > 3000 * (1 + cgcpDelay / 1000));
      if (report)
        infoEvent("GCP %u ms: total:%u commit:%u save:%u copygci:%u", coldgcp,
                  ms_total, ms_commit, ms_save, ms_copygci);
    }
#endif
  }
...
void Dbdih::execSTART_LCP_REQ(Signal *signal) {
  ...
  if ((req->pauseStart == StartLcpReq::NormalLcpStart) && ownNodeIdSet) {
    jam();
...
void Dbdih::LocalLCPState::reset() {
  m_state = LS_INITIAL;
  m_keep_gci = RNIL;
  m_stop_gci = RNIL;
}

void Dbdih::LocalLCPState::init(const StartLcpReq *req) {
  m_state = LS_RUNNING;
  m_start_lcp_req = *req;
  m_keep_gci = ~(Uint32)0;
  m_stop_gci = 0;
...
void Dbdih::LocalLCPState::init_master_take_over_idle_to_tab_saved() {
...
void Dbdih::LocalLCPState::lcp_frag_rep(const LcpFragRep *rep) {
...
void Dbdih::LocalLCPState::lcp_complete_rep(Uint32 gci) {
  if (m_state == LS_RUNNING) {
    m_state = LS_COMPLETE;
    if (gci > m_stop_gci) m_stop_gci = gci;
...
bool Dbdih::LocalLCPState::check_cut_log_tail(Uint32 gci) const {
  if (m_state == LS_COMPLETE) {
    if (gci >= m_stop_gci) return true;
  }
  return false;
}
...
void Dbdih::execDIH_GET_TABINFO_REQ(Signal *signal) {
  jamEntry();
...
void Dbdih::getTabInfo(Signal *signal) {
  TabRecordPtr tabPtr;
  tabPtr.i = signal->theData[1];
  ptrCheckGuard(tabPtr, ctabFileSize, tabRecord);
  ...
  if (tabPtr.p->tabCopyStatus != TabRecord::CS_IDLE) {
    ...
    signal->theData[1] = tabPtr.i;
    sendSignalWithDelay(reference(), GSN_CONTINUEB, signal,
                        WaitTableStateChangeMillis, signal->length());
    return;
  }
...
int Dbdih::getTabInfo_copyTableToSection(SegmentedSectionPtr &ptr,
                                         CopyTableNode ctn) {
  PageRecordPtr pagePtr;
...
int Dbdih::getTabInfo_copySectionToPages(TabRecordPtr tabPtr,
                                         SegmentedSectionPtr ptr) {
  jam();
  Uint32 sz = ptr.sz;
...
void Dbdih::getTabInfo_send(Signal *signal, TabRecordPtr tabPtr) {
  ...
  conf->senderData = connectPtr.p->userpointer;
  conf->senderRef = reference();
  sendFragmentedSignal(connectPtr.p->userblockref, GSN_DIH_GET_TABINFO_CONF,
                       signal, DihGetTabInfoConf::SignalLength, JBB, &handle,
                       cb);
}

void Dbdih::getTabInfo_sendComplete(Signal *signal, Uint32 senderData,
                                    Uint32 retVal) {
  ndbrequire(retVal == 0);
  ...
  for (Uint32 i = 0; i < tabPtr.p->totalfragments; i++) {
      ...
      if (nodePtr.p->nodeStatus == NodeRecord::ALIVE) {
        ...
        switch (nodePtr.p->activeStatus) {
          ...
          case Sysfile::NS_ActiveMissed_2: {
            ...
            /* ---------------------------------------------------------------
             */
            arrGuardErr(noCrashedReplicas, MAX_CRASHED_REPLICAS,
             ...
             */
            {
              ...
              if (tabPtr.p->tabStorage != TabRecord::ST_NORMAL ||
                  setup_create_replica(fragPtr, &createReplica, replicaPtr)) {
                jam();
                removeOldStoredReplica(fragPtr, replicaPtr);
                linkStoredReplica(fragPtr, replicaPtr);
              } else {
                jam();
                g_eventLogger->info(
                    "Forcing take-over of node %d due to insufficient REDO"
                    " for tab(%u,%u)",
    ...
    if (fragPtr.p->storedReplicas == RNIL) {
      ...
      while (replicaPtr.i != RNIL) {
        ...
        g_eventLogger->info(
            "[2/3] frag %u, replica %u, node %u, replicaLastGci %u,%u",
            fragPtr.i, replicaPtr.i, replicaPtr.p->procNode,
            replicaPtr.p->replicaLastGci[0], replicaPtr.p->replicaLastGci[1]);
        ndbrequire(replicaPtr.p->procNode < MAX_NDB_NODES) g_eventLogger->info(
            "[3/3] frag %u, replica %u, node %u, lastCompletedGCI %u",
            fragPtr.i, replicaPtr.i, replicaPtr.p->procNode,
            SYSFILE->lastCompletedGCI[replicaPtr.p->procNode]);
        replicaPtr.i = replicaPtr.p->nextPool;
      }
...
void Dbdih::resetReplicaLcp(ReplicaRecord *replicaP, Uint32 stopGci) {
...
void Dbdih::execCOPY_TABREQ(Signal *signal) {
  ...
  MEMCOPY_NO_WORDS(&pagePtr.p->word[tabPtr.p->noOfWords], &signal->theData[5],
                   16);
...
void Dbdih::copyTabReq_complete(Signal *signal, TabRecordPtr tabPtr) {
  if (!isMaster()) {
    jam();
    //----------------------------------------------------------------------------
    // In this particular case we do not release table pages if we are master.
    // The reason is that the master could still be sending the table info to
    // another node.
    //----------------------------------------------------------------------------
  ...
  conf->nodeId = getOwnNodeId();
  conf->tableId = tabPtr.i;
  sendSignal(cmasterdihref, GSN_COPY_TABCONF, signal, CopyTabConf::SignalLength,
             JBB);
}
...
void Dbdih::readPagesIntoTableLab(Signal *signal, Uint32 tableId) {
  ...
  ndbrequire((noOfFrags * (rf.rwfTabPtr.p->noOfBackups + 1)) <=
             cnoFreeReplicaRec);
...
void Dbdih::packFragIntoPagesLab(Signal *signal, RWFragment *wf) {
  ...
  if (wf->fragId == wf->totalfragments) {
    ...
    pagePtr.p->word[34] =
        ((wf->rwfTabPtr.p->noPages - 1) * 2048) + wf->wordIndex;
...
/*****************************************************************************/
void Dbdih::dump_replica_info() {
  TabRecordPtr tabPtr;
  FragmentstorePtr fragPtr;

  for (tabPtr.i = 0; tabPtr.i < ctabFileSize; tabPtr.i++) {
    ptrCheckGuard(tabPtr, ctabFileSize, tabRecord);
    if (tabPtr.p->tabStatus != TabRecord::TS_ACTIVE) continue;
...
void Dbdih::dump_replica_info(const Fragmentstore *fragPtrP) {
...
void Dbdih::startFragment(Signal *signal, Uint32 tableId, Uint32 fragId) {
  ...
  for (createReplicaPtr.i = 0; createReplicaPtr.i < cnoOfCreateReplicas;
       createReplicaPtr.i++) {
    jam();
...
void Dbdih::execSTART_RECCONF(Signal *signal) {
  ...
  if (senderData != RNIL) {
    ...
    infoEvent("Restore Database Off-line Completed on node %u", senderNodeId);
  ...
  infoEvent("Restore Database from disk Completed on node %u", senderNodeId);
...
/* ------------------------------------------------------------------------- */
void Dbdih::breakCopyTableLab(Signal *signal, TabRecordPtr tabPtr,
                              Uint32 nodeId) {
  NodeRecordPtr nodePtr;
  nodePtr.i = nodeId;
  while (nodePtr.i != RNIL) {
...
/* ------------------------------------------------------------------------- */
void Dbdih::copyTableNode(Signal *signal, CopyTableNode *ctn,
                          NodeRecordPtr nodePtr) {
  if (getNodeState().startLevel >= NodeState::SL_STARTED) {
    /* --------------------------------------------------------------------- */
    ...
    if (ctn->noOfWords <= 16) {
      ...
      switch (ctn->ctnTabPtr.p->tabCopyStatus) {
        case TabRecord::CS_SR_PHASE3_COPY_TABLE:
          ...
          // We will now proceed
          // with sending the table description to the next node in the node
          // list.
          /* ------------------------------------------------------------------
           */
          jam();
...
void Dbdih::sendCopyTable(Signal *signal, CopyTableNode *ctn,
  ...
  MEMCOPY_NO_WORDS(&req->tableWords[0],
                   &ctn->ctnPageptr.p->word[ctn->wordIndex], 16);
...
void Dbdih::checkTcCounterLab(Signal *signal) {
  ...
  if (c_lcpState.lcpStatus != LCP_STATUS_IDLE) {
    g_eventLogger->error(
        ...
        (Uint32)c_lcpState.lcpStatus, c_lcpState.lcpStatusUpdatedPlace);
    ndbabort();
    return;
  }  // if
...
void Dbdih::execTCGETOPSIZECONF(Signal *signal) {
  ...
  if (unlikely(c_lcpState.lcpManualStallStart)) {
    ...
    g_eventLogger->warning(
        ...
        "/ %llu)",
        c_lcpState.immediateLcpStart, Uint64(c_lcpState.ctcCounter),
...
void Dbdih::lcpFragmentMutex_locked(Signal *signal, Uint32 senderData,
                                    Uint32 retVal) {
  jamEntry();
...
void Dbdih::startLcpMutex_locked(Signal *signal, Uint32 senderData,
                                 Uint32 retVal) {
  jamEntry();
  ndbrequire(retVal == 0);
...
void Dbdih::sendSTART_LCP_REQ(Signal *signal, Uint32 nodeId, Uint32 extra) {
  BlockReference ref = calcDihBlockRef(nodeId);
  Uint32 packed_length1 =
      c_lcpState.m_participatingLQH.getPackedLengthInWords();
  Uint32 packed_length2 =
...
void Dbdih::execSTART_LCP_CONF(Signal *signal) {
...
void Dbdih::startLcpMutex_unlocked(Signal *signal, Uint32 data, Uint32 retVal) {
  jamEntry();
  ndbrequire(retVal == 0);
...
void Dbdih::master_lcp_fragmentMutex_locked(Signal *signal,
                                            Uint32 failedNodePtrI,
                                            Uint32 retVal) {
  jamEntry();
  ndbrequire(retVal == 0);
...
void Dbdih::startLcpRoundLoopLab(Signal *signal, Uint32 startTableId,
                                 Uint32 startFragId) {
  NodeRecordPtr nodePtr;
  for (nodePtr.i = 1; nodePtr.i <= m_max_node_id; nodePtr.i++) {
    ptrAss(nodePtr, nodeRecord);
...
void Dbdih::startNextChkpt(Signal *signal) {
  ...
  const bool allReplicaCheckpointsQueued =
      c_lcpState.m_allReplicasQueuedLQH.contains(c_lcpState.m_participatingLQH);
  ...
  while (curr.tableId < ctabFileSize) {
    ...
    if (started + queued > 256 || (!save && examined > 128)) {
      /**
...
void Dbdih::sendLastLCP_FRAG_ORD(Signal *signal) {
  ...
  for (nodePtr.i = 1; nodePtr.i <= m_max_node_id; nodePtr.i++) {
    ...
    if (nodePtr.p->noOfQueuedChkpt == 0 && nodePtr.p->noOfStartedChkpt == 0 &&
      ...
      if (c_lcpState.m_LAST_LCP_FRAG_ORD.isWaitingFor(nodePtr.i)) {
        DEB_LCP(
            ("Still waiting for sending last LCP_FRAG_ORD to node %u,"
...
void Dbdih::execLCP_FRAG_REP(Signal *signal) {
  ...
  if (ERROR_INSERTED(7178) && (nodeId == ERROR_INSERT_EXTRA)) {
    ...
    signal->theData[0] = 9999;
    sendSignal(numberToRef(CMVMI, nodeId), GSN_NDB_TAMPER, signal, 1, JBA);
    return;
  }
  ...
  bool fromTimeQueue = (signal->length() == LcpFragRep::SignalLengthTQ &&
                        lcpReport->fromTQ == Uint32(1) && !broadcast_req);
  ...
  if (tableDone) {
    ...
    if (tabPtr.p->tabStatus == TabRecord::TS_IDLE ||
      ...
      if (ret && ERROR_INSERTED(7209)) {
        ...
        CLEAR_ERROR_INSERT_VALUE;
        signal->theData[0] = 9999;
        sendSignal(numberToRef(CMVMI, cmasterNodeId), GSN_NDB_TAMPER, signal, 1,
                   JBB);
      }
    }
  }
  ...
  if (isMaster()) {
    ...
    if (outstanding > 0) {
      ...
      for (Uint32 i = 0; i < outstanding; i++) {
        ...
        jam();
        memmove(&nodePtr.p->startedChkpt[i], &nodePtr.p->startedChkpt[i + 1],
    ...
    if (outstanding_queued > 0) {
      ...
      for (Uint32 i = 0; i < outstanding_queued; i++) {
        ...
        memmove(
            ...
            (outstanding_queued - (i + 1)) * sizeof(nodePtr.p->queuedChkpt[0]));
        found = true;
      }
      if (found) {
        jam();
        ...
        DEB_LCP(
            ("LCP_FRAG_REP: nodePtr(%u)->noOfQueuedChkpt = %u"
             ", nodePtr->noOfStartedChkpt = %u"
             ", tab(%u,%u)",
             nodePtr.i, nodePtr.p->noOfQueuedChkpt, nodePtr.p->noOfStartedChkpt,
             tableId, fragId));
...
bool Dbdih::checkLcpAllTablesDoneInLqh(Uint32 line) {
  TabRecordPtr tabPtr;
...
void Dbdih::findReplica(ReplicaRecordPtr &replicaPtr, Fragmentstore *fragPtrP,
                        Uint32 nodeId, bool old) {
...
int Dbdih::handle_invalid_lcp_no(const LcpFragRep *rep,
                                 ReplicaRecordPtr replicaPtr) {
  ndbrequire(!isMaster());
  Uint32 lcpNo = rep->lcpNo;
  Uint32 lcpId = rep->lcpId;

  warningEvent("Detected previous node failure of %d during lcp", rep->nodeId);
 ...
 *        false otherwise
 */
bool Dbdih::reportLcpCompletion(const LcpFragRep *lcpReport) {
  ...
  if (lcpNo != replicaPtr.p->nextLcp) {
    if (handle_invalid_lcp_no(lcpReport, replicaPtr)) {
      g_eventLogger->error("lcpNo = %d replicaPtr.p->nextLcp = %d", lcpNo,
                           replicaPtr.p->nextLcp);
      ndbabort();
    }
  }
...
bool Dbdih::checkStartMoreLcp(Signal *signal, Uint32 nodeId, bool startNext) {
  ...
  if (nodePtr.p->noOfQueuedChkpt > 0) {
    ...
    nodePtr.p->noOfQueuedChkpt--;
    nodePtr.p->noOfStartedChkpt++;
    memmove(&nodePtr.p->queuedChkpt[0], &nodePtr.p->queuedChkpt[1],
            nodePtr.p->noOfQueuedChkpt * sizeof(nodePtr.p->queuedChkpt[0]));
    //-------------------------------------------------------------------
...
void Dbdih::sendLCP_FRAG_ORD(Signal *signal,
  ...
  if (replicaPtr.p->nextLcp >= MAX_LCP_USED) {
    jam();
    infoEvent("Updating nextLcp from %u to %u tab: %u", replicaPtr.p->nextLcp,
              0, info.tableId);
    replicaPtr.p->nextLcp = 0;
  }
...
void Dbdih::sendLCP_COMPLETE_REP(Signal *signal) {
  jam();
...
void Dbdih::execLCP_COMPLETE_REP(Signal *signal) {
  ...
  if (blockNo == DBLQH && signal->length() == LcpCompleteRep::SignalLengthTQ &&
      rep->fromTQ == Uint32(0)) {
    /* Handle case 2) above */
  ...
  switch (blockNo) {
    case DBLQH:
      ...
      DEB_LCP_COMP(("LCP_COMPLETE_REP(LQH)(%u), LCP: %u", nodeId, lcpId));
      break;
    case DBDIH:
      jam();
      ndbrequire(isMaster());
      c_lcpState.m_LCP_COMPLETE_REP_Counter_DIH.clearWaitingFor(nodeId);
      DEB_LCP_COMP(("LCP_COMPLETE_REP(DIH)(%u), LCP: %u", nodeId, lcpId));
      break;
    case 0:
      jam();
      ndbrequire(!isMaster());
      ...
      DEB_LCP_COMP(("LCP_COMPLETE_REP(0)(%u), LCP: %u", nodeId, lcpId));
      break;
    default:
      ndbabort();
  }
...
void Dbdih::allNodesLcpCompletedLab(Signal *signal) {
  ...
  if (isMaster()) {
     ...
     */
    TakeOverRecordPtr takeOverPtr;
    for (c_masterActiveTakeOverList.first(takeOverPtr);
         !takeOverPtr.isNull();) {
      jam();
  ...
  switch (tabPtr.p->tabUpdateState) {
    case TabRecord::US_LOCAL_CHECKPOINT:
      ...
      if (c_lcpTabDefWritesControl.releaseMustStartQueued()) {
        ...
        g_eventLogger->warning(
            "DIH : Error in queued table writes : inUse %u"
            " queued %u total %u",
...
void Dbdih::checkGcpStopLab(Signal *signal) {
  ...
  if (elapsed_ms > GCPCheckPeriodMillis) elapsed_ms = GCPCheckPeriodMillis;
  ...
  if (m_gcp_monitor.m_gcp_save.m_gci == m_gcp_save.m_gci) {
    ...
    if (lag0 > 0 && (lag0 % report_period_ms) < elapsed_ms) {
      if (m_gcp_monitor.m_gcp_save.m_max_lag_ms) {
        warningEvent(
            "GCP Monitor: GCP_SAVE lag %u sec"
            " (max lag: %us), %s",
  ...
  if (m_gcp_monitor.m_micro_gcp.m_gci == m_micro_gcp.m_current_gci) {
    ...
    if (lag1 > 0 && (lag1 % report_period_ms) < elapsed_ms) {
      if (m_gcp_monitor.m_micro_gcp.m_max_lag_ms) {
        warningEvent(
            "GCP Monitor: GCP_COMMIT lag %u sec"
            " (max lag: %us), %s",
    ...
    if (m_gcp_monitor.m_micro_gcp.m_need_max_lag_recalc) {
      setGCPStopTimeouts(signal,
                         false);  // set_micro_gcp_max_lag is true by default
      m_gcp_monitor.m_micro_gcp.m_need_max_lag_recalc = false;
    }
  }

  signal->theData[0] = DihContinueB::ZCHECK_GCP_STOP;
  sendSignalWithDelay(reference(), GSN_CONTINUEB, signal, GCPCheckPeriodMillis,
                      1);
  return;
}  // Dbdih::checkGcpStopLab()

void Dbdih::dumpGcpStop() {
...
void Dbdih::crashSystemAtGcpStop(Signal *signal, bool local) {
  ...
  if (c_nodeStartMaster.blockGcp == 2) {
...

    {
      ...
      isolateNodes(signal, NodeIsolationTimeoutMillis, victims);
    }
    return;
  }
    ...
    switch (m_gcp_save.m_master.m_state) {
      ...
      case GcpSave::GCP_SAVE_COPY_GCI: {
...

        {
          ...
          sysErr->data[1] = cgcpOrderBlocked;
          sysErr->data[2] = m_micro_gcp.m_master.m_state;
          sendSignal(rg, GSN_SYSTEM_ERROR, signal, SystemError::SignalLength,
                     JBA);
        }
...
dolocal:
  ...
  if (ERROR_INSERTED(7238) || ERROR_INSERTED(7239) || ERROR_INSERTED(7244) ||
      ERROR_INSERTED(7237) || ERROR_INSERTED(7241) || ERROR_INSERTED(7242) ||
      ERROR_INSERTED(7243)) {
    jam();
  ...
  sysErr->data[1] = cgcpOrderBlocked;
  sysErr->data[2] = m_micro_gcp.m_master.m_state;
  EXECUTE_DIRECT(NDBCNTR, GSN_SYSTEM_ERROR, signal, SystemError::SignalLength);
  ndbabort();
...
void Dbdih::emptyverificbuffer(Signal *signal, Uint32 q, bool aContinueB) {
...
void Dbdih::emptyverificbuffer_check(Signal *signal, Uint32 q, Uint32 retVal) {
  ndbrequire(retVal == 0);
  if (!isEmpty(c_diverify_queue[q])) {
    jam();
    signal->theData[0] = DihContinueB::ZEMPTY_VERIFY_QUEUE;
...
bool Dbdih::findLogNodes(CreateReplicaRecord *createReplica,
                         FragmentstorePtr fragPtr, Uint32 startGci,
                         Uint32 stopGci) {
  ...
  g_eventLogger->info(
      "startGci: %u, stopGci: %u, noCrashed: %u"
      "newestRestorableGci: %u, createGci: %u,"
      " replicaLastGci: %u, lastCompletedGci: %u"
      ", node: %u",
      startGci, stopGci, noCrashed, SYSFILE->newestRestorableGCI,
  ...
  do {
    Uint32 fblStopGci;
    jam();
    if (!findBestLogNode(createReplica, fragPtr, startGci, stopGci, logNode,
                         fblStopGci)) {
      jam();
      return false;
    }
...
/*************************************************************************/
bool Dbdih::findBestLogNode(CreateReplicaRecord *createReplica,
                            FragmentstorePtr fragPtr, Uint32 startGci,
...
/*************************************************************************/
void Dbdih::findMinGci(ReplicaRecordPtr fmgReplicaPtr, Uint32 &keepGci,
                       Uint32 &oldestRestorableGci) {
  keepGci = (Uint32)-1;
  ...
  for (Uint32 i = 0; i < MAX_LCP_STORED; i++) {
    ...
    if (fmgReplicaPtr.p->lcpStatus[i] == ZVALID) {
      if ((fmgReplicaPtr.p->lcpId[i] + MAX_LCP_STORED) <=
          SYSFILE->latestLCP_ID) {
        jam();
        /*-----------------------------------------------------------------*/
...
bool Dbdih::findStartGci(Ptr<ReplicaRecord> replicaPtr, Uint32 stopGci,
                         Uint32 &startGci, Uint32 &lcpNo) {
  Uint32 cnt = 0;
 ...
 */
Uint32 Dbdih::compute_max_failure_time() {
  jam();
...
*/
void Dbdih::setGCPStopTimeouts(Signal *signal, bool set_gcp_save_max_lag,
  ...
  */
  ...
  if (micro_GCP_timeout != 0) {
    ...
    if (set_gcp_save_max_lag) {
      m_gcp_monitor.m_gcp_save.m_max_lag_ms =
          ...
          MAX(gcp_timeout, micro_GCP_timeout) + max_failure_time;
    }
  } else {
    jam();
  ...
  if (old_micro_GCP_max_lag != m_gcp_monitor.m_micro_gcp.m_max_lag_ms) {
    if (m_gcp_monitor.m_micro_gcp.m_max_lag_ms > 0) {
      ...
      // Log locallly.
      g_eventLogger->info(
          "GCP Monitor: Computed max GCP_COMMIT lag to %u"
          " seconds",
          m_gcp_monitor.m_micro_gcp.m_max_lag_ms / 1000);
    } else {
      jam();
  ...
  if (old_GCP_save_max_lag != m_gcp_monitor.m_gcp_save.m_max_lag_ms) {
    if (m_gcp_monitor.m_gcp_save.m_max_lag_ms > 0) {
      ...
      // Log locallly.
      g_eventLogger->info(
          "GCP Monitor: Computed max GCP_SAVE lag to %u"
          " seconds",
          m_gcp_monitor.m_gcp_save.m_max_lag_ms / 1000);
    } else {
      jam();
...
void Dbdih::initCommonData() {
  ...
  ndb_mgm_get_int_parameter(p, CFG_DB_PARALLEL_COPY_THREADS,
...
void Dbdih::initFragstore(FragmentstorePtr fragPtr, Uint32 fragId) {
  ...
  for (Uint32 i = 0; i < MAX_REPLICAS; i++) fragPtr.p->activeNodes[i] = 0;
...
void Dbdih::initRestartInfo(Signal *signal) {
  ...
  {
    char envBuf[256];
    const char *v = NdbEnv_GetEnv("NDB_START_GCI", envBuf, 256);
    if (v && *v != 0) {
...
void Dbdih::initialiseRecordsLab(Signal *signal, Uint32 stepNo, Uint32 retRef,
                                 Uint32 retData) {
  switch (stepNo) {
    case 0:
      jam();
    ...
    case 5: {
      ...
      for (loopNGPtr.i = 0; loopNGPtr.i < MAX_NDB_NODE_GROUPS; loopNGPtr.i++) {
        ...
        memset(loopNGPtr.p->m_used_log_parts, 0,
...
/*************************************************************************/
void Dbdih::add_nodegroup(NodeGroupRecordPtr NGPtr) {
  if (NGPtr.p->nodegroupIndex == RNIL) {
    jam();
...
void Dbdih::inc_ng_refcount(Uint32 i) {
  NodeGroupRecordPtr NGPtr;
  NGPtr.i = i;
  ptrCheckGuard(NGPtr, MAX_NDB_NODE_GROUPS, nodeGroupRecord);
  NGPtr.p->m_ref_count++;
...
void Dbdih::dec_ng_refcount(Uint32 i) {
  NodeGroupRecordPtr NGPtr;
  NGPtr.i = i;
  ptrCheckGuard(NGPtr, MAX_NDB_NODE_GROUPS, nodeGroupRecord);
...
void Dbdih::makeNodeGroups(Uint32 nodeArray[]) {
  ...
  for (Uint32 i = 0; nodeArray[i] != RNIL; i++) {
    ...
    if (mngNodeptr.p->nodeGroup == NDB_NO_NODEGROUP) {
      ...
      g_eventLogger->info("setting nodeGroup = ZNIL for node %u", mngNodeptr.i);
  ...
  for (; NGPtr.i < MAX_NDB_NODE_GROUPS; NGPtr.i++) {
    jam();
    ptrAss(NGPtr, nodeGroupRecord);
    if (NGPtr.p->nodeCount < cnoReplicas) break;
  }
...
void Dbdih::execCHECKNODEGROUPSREQ(Signal *signal) {
  ...
  if (sd->requestType & CheckNodeGroups::UseBeforeFailMask) {
    jamNoBlock();
    ndbrequire(signal->length() >= CheckNodeGroups::SignalLengthArbitCheckLong);
  ...
  switch (sd->requestType & ~CheckNodeGroups::Direct) {
    case CheckNodeGroups::ArbitCheck: {
      ...
      for (Uint32 i = 0; i < cnoOfNodeGroups; i++) {
        ...
        for (Uint32 j = 0; j < ngPtr.p->nodeCount; j++) {
          ...
          if (use_before_fail_mask && sd->before_fail_mask.get(nodeId) &&
              !sd->mask.get(nodeId)) {
    ...
    case CheckNodeGroups::GetNodeGroup: {
      ...
      if (ng == NO_NODE_GROUP_ID) ng = RNIL;
      sd->output = ng;
      break;
    }
    case CheckNodeGroups::GetNodeGroupMembers: {
      ...
      if (ngPtr.i != RNIL) {
        ...
        for (Uint32 j = 0; j < ngPtr.p->nodeCount; j++) {
          jamNoBlock();
          DEB_MULTI_TRP(
              ("Node %u is in same node group", ngPtr.p->nodesInGroup[j]));
    ...
    case CheckNodeGroups::GetDefaultFragments:
      ...
      sd->output = getFragmentCount(sd->partitionBalance,
                                    ...
                                    cnoReplicas, getFragmentsPerNode());
      break;
    case CheckNodeGroups::GetDefaultFragmentsFullyReplicated:
      jamNoBlock();
      ok = true;
      sd->output = getFragmentCount(sd->partitionBalance, 1, cnoReplicas,
                                    getFragmentsPerNode());
      break;
  }
  ndbrequire(ok);
...
Uint32 Dbdih::getFragmentCount(Uint32 partitionBalance, Uint32 numOfNodeGroups,
...
void Dbdih::mergeCrashedReplicas(ReplicaRecordPtr replicaPtr) {
  /**
 ...
 * will be ignored.
 */
void Dbdih::updateLcpInfo(TabRecord *regTabPtr, Fragmentstore *regFragPtr,
...
void Dbdih::readReplicas(RWFragment *rf, TabRecord *regTabPtr,
                         FragmentstorePtr fragPtr) {
  Uint32 i;
...
/*---------------------------------------------------------------*/
void Dbdih::removeNodeFromStored(Uint32 nodeId, FragmentstorePtr fragPtr,
                                 ReplicaRecordPtr replicatePtr,
                                 bool temporary) {
  if (!temporary) {
    jam();
...
/*************************************************************************/
bool Dbdih::setup_create_replica(FragmentstorePtr fragPtr,
  ...
  bool result = findStartGci(replicaPtr, nodeStopGci, startGci, startLcpNo);
  if (!result) {
    jam();
    /* --------------------------------------------------------------- */
...
/*************************************************************************/
void Dbdih::sendStartFragreq(Signal *signal, TabRecordPtr tabPtr,
                             Uint32 fragId) {
...
void Dbdih::setNodeGroups() {
   ...
   */
  if (c_nextNodeGroup >= cnoOfNodeGroups) c_nextNodeGroup = 0;
...
void Dbdih::setNodeRestartInfoBits(Signal *signal) {
  ...
  for (nodePtr.i = 1; nodePtr.i <= m_max_node_id; nodePtr.i++) {
    ...
    if (c_lcpState.m_participatingLQH.get(nodePtr.i)) {
      ...
      jam();
      if (nodePtr.p->activeStatus == Sysfile::NS_Active) tmp.set(nodePtr.i);
    }
#endif
  }  // for
...
void Dbdih::startGcpMonitor(Signal *signal) {
  jam();
  m_gcp_monitor.m_gcp_save.m_gci = m_gcp_save.m_gci;
  m_gcp_monitor.m_gcp_save.m_elapsed_ms = 0;
  m_gcp_monitor.m_gcp_save.m_need_max_lag_recalc = true;
...
void Dbdih::writeRestorableGci(Signal *signal, FileRecordPtr filePtr) {
  ...
  memset(&sysfileDataToFile[cdata_size_in_words], 0,
  ...
  if (ERROR_INSERTED(7224) && filePtr.i == crestartInfoFile[1]) {
    ...
    sendSignalWithDelay(NDBFS_REF, GSN_FSWRITEREQ, signal, 2000,
...
void Dbdih::writeTabfile(Signal *signal, TabRecord *tab,
                         FileRecordPtr filePtr) {
  FsReadWriteReq *req = (FsReadWriteReq *)signal->getDataPtrSend();
  req->filePointer = filePtr.p->fileRef;
  req->userReference = reference();
...
void Dbdih::execDUMP_STATE_ORD(Signal *signal) {
  ...
  if (arg == DumpStateOrd::DihDumpNodeRestartInfo) {
    ...
    for (Uint32 i = 0; i < c_diverify_queue_cnt; i++) {
      ...
      rmb();
      infoEvent("[ %u : cfirstVerifyQueue = %u clastVerifyQueue = %u]", i,
  ...
  if (arg == DumpStateOrd::DihDumpNodeStatusInfo) {
    ...
    for (localNodePtr.i = 1; localNodePtr.i <= m_max_node_id;
      ...
      if (localNodePtr.p->nodeStatus != NodeRecord::NOT_IN_CLUSTER) {
        infoEvent("Node = %d has status = %d", localNodePtr.i,
                  localNodePtr.p->nodeStatus);
      }  // if
    }    // for
  }      // if
  ...
  if (arg == DumpStateOrd::DihPrintFragmentation) {
    ...
    if (signal->getLength() == 1) {
      ...
      for (Uint32 i = 0; i < cnoOfNodeGroups; i++) {
        ...
        infoEvent("NG %u(%u) ref: %u [ cnt: %u : %u %u %u %u ]", NGPtr.i,
                  NGPtr.p->nodegroupIndex, NGPtr.p->m_ref_count,
                  NGPtr.p->nodeCount, NGPtr.p->nodesInGroup[0],
                  NGPtr.p->nodesInGroup[1], NGPtr.p->nodesInGroup[2],
                  NGPtr.p->nodesInGroup[3]);
      }
  ...
  if (arg == DumpStateOrd::DihPrintOneFragmentation) {
    ...
    const Uint32 noOfReplicas =
        extractNodeInfo(jamBuffer(), fragPtr.p, nodeOrder);
    char buf[100];
    BaseString::snprintf(
        buf, sizeof(buf), " Table %d Fragment %d(%u) LP: %u - ", tabPtr.i,
        fragid, dihGetInstanceKey(fragPtr), fragPtr.p->m_log_part_id);
  ...
  if (signal->theData[0] == 7000) {
    infoEvent("ctimer = %d", c_lcpState.ctimer);
    infoEvent("cmasterState = %d", cmasterState);
    infoEvent("cmasterTakeOverNode = %d, ctcCounter = %d", cmasterTakeOverNode,
              c_lcpState.ctcCounter);
  }  // if
  if (signal->theData[0] == 7001) {
    infoEvent("c_lcpState.keepGci = %d", c_lcpState.keepGci);
    infoEvent("c_lcpState.lcpStatus = %d, clcpStopGcp = %d",
              c_lcpState.lcpStatus, c_lcpState.lcpStopGcp);
    infoEvent("cimmediateLcpStart = %d", c_lcpState.immediateLcpStart);
  }  // if
  if (signal->theData[0] == 7002) {
    infoEvent("cnoOfActiveTables = %d", cnoOfActiveTables);
    infoEvent("cdictblockref = %d, cfailurenr = %d", cdictblockref, cfailurenr);
    infoEvent("con_lineNodes = %d, reference() = %d, creceivedfrag = %d",
              con_lineNodes, reference(), creceivedfrag);
  }  // if
  if (signal->theData[0] == 7003) {
    infoEvent("cfirstAliveNode = %d, cgckptflag = %d", cfirstAliveNode,
              cgckptflag);
    infoEvent(
        "clocallqhblockref = %d, clocaltcblockref = %d, cgcpOrderBlocked = %d",
        clocallqhblockref, clocaltcblockref, cgcpOrderBlocked);
    infoEvent("cstarttype = %d, csystemnodes = %d", cstarttype, csystemnodes);
  }  // if
  if (signal->theData[0] == 7004) {
    infoEvent("cmasterdihref = %d, cownNodeId = %d", cmasterdihref, cownNodeId);
    infoEvent("cndbStartReqBlockref = %d, cremainingfrags = %d",
              cndbStartReqBlockref, cremainingfrags);
  }  // if
  if (signal->theData[0] == 7005) {
    infoEvent("crestartGci = %d", crestartGci);
  }  // if
  if (signal->theData[0] == 7006) {
    infoEvent("clcpDelay = %d", c_lcpState.clcpDelay);
  ...
  if (signal->theData[0] == 7007) {
    ...
    infoEvent("c_nodeStartMaster.blockGcp = %d", c_nodeStartMaster.blockGcp);
  }  // if
  ...
  if (signal->theData[0] == 7010) {
    ...
    infoEvent("c_blockCommit = %d, c_blockCommitNo = %d", c_blockCommit,
              c_blockCommitNo);
  }  // if
  if (signal->theData[0] == 7011) {
    infoEvent("c_COPY_GCIREQ_Counter = %s", c_COPY_GCIREQ_Counter.getText());
    infoEvent("c_COPY_TABREQ_Counter = %s", c_COPY_TABREQ_Counter.getText());
  ...
  if (signal->theData[0] == 7012) {
    ...
    infoEvent("ParticipatingDIH = %s",
              c_lcpState.m_participatingDIH.getText(buf));
    infoEvent("ParticipatingLQH = %s",
              c_lcpState.m_participatingLQH.getText(buf));
      ...
      if (nodePtr.p->nodeStatus == NodeRecord::ALIVE) {
        ...
        for (i = 0; i < nodePtr.p->noOfQueuedChkpt; i++) {
          infoEvent("Node %d: queued: table=%d fragment=%d replica=%d",
                    nodePtr.i, nodePtr.p->queuedChkpt[i].tableId,
                    nodePtr.p->queuedChkpt[i].fragId,
                    nodePtr.p->queuedChkpt[i].replicaPtr);
        }
      } else {
#ifdef DEBUG_LCP
        infoEvent("Node(%u)->nodeStatus = %u", nodePtr.i,
                  nodePtr.p->nodeStatus);
#endif
      }
    }
  ...
  if (arg == DumpStateOrd::DihTcSumaNodeFailCompleted &&
    ...
    ptrCheckGuard(nodePtr, MAX_NDB_NODES, nodeRecord);
    infoEvent(
        "NF Node %d tc: %d lqh: %d qlqh: %d dih: %d dict: %d"
        " recNODE_FAILREP: %d",
        nodePtr.i, nodePtr.p->dbtcFailCompleted, nodePtr.p->dblqhFailCompleted,
        nodePtr.p->dbqlqhFailCompleted, nodePtr.p->dbdihFailCompleted,
        nodePtr.p->dbdictFailCompleted, nodePtr.p->recNODE_FAILREP);
  ...
  if (arg == DumpStateOrd::DihDumpLCPState) {
    ...
    infoEvent("lcpStatus = %d (update place = %d) ", c_lcpState.lcpStatus,
              c_lcpState.lcpStatusUpdatedPlace);
    infoEvent(
        "lcpStart = %d lcpStopGcp = %d keepGci = %d oldestRestorable = %d",
        c_lcpState.lcpStart, c_lcpState.lcpStopGcp, c_lcpState.keepGci,
        c_lcpState.oldestRestorableGci);

    infoEvent("immediateLcpStart = %d masterLcpNodeId = %d",
  ...
  if (arg == DumpStateOrd::DihDumpLCPMasterTakeOver) {
    ...
    infoEvent(
        "c_lcpMasterTakeOverState.state = %d updatePlace = %d failedNodeId = "
        "%d",
        c_lcpMasterTakeOverState.state, c_lcpMasterTakeOverState.updatePlace,
  ...
  if (signal->theData[0] == DumpStateOrd::DihAllAllowNodeStart) {
    for (Uint32 i = 1; i <= m_max_node_id; i++) setAllowNodeStart(i, true);
    return;
  }  // if
  ...
  if (arg == 7021 && signal->getLength() == 2) {
    TabRecordPtr tabPtr;
    tabPtr.i = signal->theData[1];
    if (tabPtr.i >= ctabFileSize) return;

    ptrCheckGuard(tabPtr, ctabFileSize, tabRecord);

    if (tabPtr.p->tabStatus != TabRecord::TS_ACTIVE) return;

    infoEvent(
        "Table %d: TabCopyStatus: %d TabUpdateStatus: %d TabLcpStatus: %d",
        tabPtr.i, tabPtr.p->tabCopyStatus, tabPtr.p->tabUpdateState,
        tabPtr.p->tabLcpStatus);
    ...
    for (Uint32 fid = 0; fid < tabPtr.p->totalfragments; fid++) {
      ...
      do {
        c_replicaRecordPool.getPtr(replicaPtr);
        BaseString::snprintf(buf2, sizeof(buf2), "%s %d(on %d)=%d(%s)", buf,
  ...
  if (arg == 7023) {
    ...
    infoEvent("TakeOverPtr(%u) starting: %u flags: 0x%x ref: 0x%x, data: %u",
              ptr.i, ptr.p->toStartingNode, ptr.p->m_flags, ptr.p->m_senderRef,
              ptr.p->m_senderData);
    infoEvent("slaveState: %u masterState: %u", ptr.p->toSlaveStatus,
  ...
  if (arg == DumpStateOrd::SchemaResourceSnapshot) {
    g_eventLogger->info(
        ...
        ", free replicas: %u",
        cremainingfrags, cnoFreeReplicaRec);
  ...
  if (arg == DumpStateOrd::SchemaResourceCheckLeak) {
    g_eventLogger->info(
        "SchemaResourceCheckLeak: remainingfrags: %u"
        ", free replicas: %u",
        cremainingfrags, cnoFreeReplicaRec);
  ...
  if (arg == DumpStateOrd::DihDisplayPauseState) {
    infoEvent(
        "Pause LCP ref: %x, is_lcp_paused %u,"
        " c_dequeue_lcp_rep_ongoing %u",
        cmasterdihref, is_lcp_paused(), c_dequeue_lcp_rep_ongoing);
    infoEvent("c_pause_lcp_master_state: %u,",
              Uint32(c_pause_lcp_master_state));
    infoEvent(
        "c_queued_lcp_complete_rep: %u,"
        " c_lcp_id_paused: %u",
        c_queued_lcp_complete_rep, c_lcp_id_paused);
    infoEvent(
    ...
    if (isMaster()) {
      ...
      infoEvent("c_lcpState.m_participatingLQH: %s",
                c_lcpState.m_participatingLQH.getText(buf));
      infoEvent("c_pause_participants: %s", c_pause_participants.getText(buf));
    }
  }
  ...
  if (arg == DumpStateOrd::DihStallLcpStart) {
    ...
    if (signal->getLength() != 2) {
      g_eventLogger->warning(
          "Malformed DihStallLcpStart(%u) received, ignoring",
    ...
    if (key == 91919191) {
      jam();
      g_eventLogger->warning(
          "DihStallLcpStart(%u) received, stalling subsequent LCP starts",
          DumpStateOrd::DihStallLcpStart);
      c_lcpState.lcpManualStallStart = true;
    } else {
      jam();
      g_eventLogger->warning(
          "DihStallLcpStart(%u) received, clearing LCP stall state (%u)",
          DumpStateOrd::DihStallLcpStart, c_lcpState.lcpManualStallStart);
      c_lcpState.lcpManualStallStart = false;
    }
    return;
  }
  ...
  if (arg == DumpStateOrd::DihSaveGcpCommitLag) {
    jam();
    m_gcp_monitor.m_savedMaxCommitLag = m_gcp_monitor.m_micro_gcp.m_max_lag_ms;
...
void Dbdih::execPREP_DROP_TAB_REQ(Signal *signal) {
  jamEntry();
...
void Dbdih::waitDropTabWritingToFile(Signal *signal, TabRecordPtr tabPtr) {
  if (tabPtr.p->tabLcpStatus == TabRecord::TLS_WRITING_TO_FILE) {
    jam();
...
void Dbdih::checkDropTabComplete(Signal *signal, TabRecordPtr tabPtr) {
  startDeleteFile(signal, tabPtr);
}

void Dbdih::execNDB_TAMPER(Signal *signal) {
  if ((ERROR_INSERTED(7011)) && (signal->theData[0] == 7012)) {
    CLEAR_ERROR_INSERT_VALUE;
...
void Dbdih::switch_primary_stop_node(Signal *signal, Uint32 node_id,
                                     Uint32 ret_val) {
...
void Dbdih::execDIH_SWITCH_REPLICA_REQ(Signal *signal) {
  ...
  const Uint32 noOfReplicas = extractNodeInfo(jamBuffer(), fragPtr.p, oldOrder);
  ...
  make_table_use_new_node_order(tabPtr, fragPtr, noOfReplicas,
                                &req->newNodeOrder[0]);

  /**
   * Reply
   */
  DihSwitchReplicaConf *const conf =
      (DihSwitchReplicaConf *)&signal->theData[0];
...
void Dbdih::switchReplicaReply(Signal *signal, NodeId nodeId) {
  jam();
...
void Dbdih::switchReplica(Signal *signal, Uint32 nodeId, Uint32 tableId,
                          Uint32 fragNo) {
  jam();
  DihSwitchReplicaReq *const req = (DihSwitchReplicaReq *)&signal->theData[0];
  ...
  for (Uint32 i = 0; i < RT_BREAK; i++) {
    ...
    if (tableId >= ctabFileSize) {
      ...
      if (c_stopPermMaster.returnValue == 0) {
        ...
        sendSignal(c_stopPermMaster.clientRef, GSN_STOP_PERM_CONF, signal, 1,
                   JBB);
      } else {
        jam();
    ...
    Uint32 oldOrder[MAX_REPLICAS];
    const Uint32 noOfReplicas =
        extractNodeInfo(jamBuffer(), fragPtr.p, oldOrder);
...
void Dbdih::execSTOP_ME_REQ(Signal *signal) {
  ...
  if (nodeId != getOwnNodeId()) {
    ...
    sendSignal(senderRef, GSN_STOP_ME_CONF, signal, StopMeConf::SignalLength,
               JBB);
    return;
  }  // if
...
void Dbdih::execSTOP_ME_REF(Signal *signal) { ndbabort(); }
...
void Dbdih::sendREDO_STATE_REP_to_all(Signal *signal, Uint32 block,
                                      bool send_to_all) {
  NodeRecordPtr nodePtr;
  nodePtr.i = cfirstAliveNode;
  jam();
...
RedoStateRep::RedoAlertState Dbdih::get_global_redo_alert_state() {
  ...
  if (isMaster()) {
    ...
    switch (requestType) {
      ...
      case WaitGCPReq::CompleteForceStart: {
        ...
        DEB_NODE_STOP(
            ("waitGCI = %u", Uint32(m_micro_gcp.m_current_gci >> 32)));
...
error:
  ...
  sendSignal(senderRef, GSN_WAIT_GCP_REF, signal, WaitGCPRef::SignalLength,
             JBB);
...
void Dbdih::checkWaitGCPProxy(Signal *signal, NodeId failedNodeId) {
  ...
  while (ptr.i != RNIL) {
    ...
    if (refToNode(masterRef) == failedNodeId) {
      ...
      sendSignal(clientRef, GSN_WAIT_GCP_REF, signal, WaitGCPRef::SignalLength,
                 JBB);
    }  // if
  }    // while
 ...
 *
 */
void Dbdih::getNodeBitmap(NdbNodeBitmask &map, Uint32 listHead,
...
void Dbdih::checkShutdownSync() {
  ...
  getNodeBitmap(allDataNodes, cfirstAliveNode,
...
void Dbdih::emptyWaitGCPMasterQueue(Signal *signal, Uint64 gci,
                                    WaitGCPList &list) {
  jam();
  WaitGCPConf *const conf = (WaitGCPConf *)&signal->theData[0];
  ...
  while (ptr.i != RNIL) {
    ...
    conf->blockStatus = cgcpOrderBlocked;
    sendSignal(clientRef, GSN_WAIT_GCP_CONF, signal, WaitGCPConf::SignalLength,
               JBB);
...
Sysfile::ActiveStatus Dbdih::getNodeActiveStatus(Uint32 nodeId) {
  NodeRecordPtr nodePtr;
  nodePtr.i = nodeId;
  ptrCheckGuard(nodePtr, MAX_NDB_NODES, nodeRecord);
...
void Dbdih::setNodeActiveStatus(Uint32 nodeId,
                                Sysfile::ActiveStatus newStatus) {
  NodeRecordPtr nodePtr;
  nodePtr.i = nodeId;
  ptrCheckGuard(nodePtr, MAX_NDB_NODES, nodeRecord);
...
Uint32 Dbdih::getNodeGroup(Uint32 nodeId) const {
  NodeRecordPtr nodePtr;
  nodePtr.i = nodeId;
  ptrCheckGuard(nodePtr, MAX_NDB_NODES, nodeRecord);
...
void Dbdih::getNodeGroupPtr(Uint32 nodeId, NodeGroupRecordPtr &NGPtr) {
...
void Dbdih::sendDictLockReq(Signal *signal, Uint32 lockType, Callback c) {
...
void Dbdih::recvDictLockConf(Signal *signal) {
...
void Dbdih::sendDictUnlockOrd(Signal *signal, Uint32 lockSlavePtrI) {
  ...
  if (unlikely(getNodeInfo(cmasterNodeId).m_version <
               NDB_MAKE_VERSION(6, 3, 0))) {
    jam();
    len = 2;
  }
...
#ifdef ERROR_INSERT
void Dbdih::sendToRandomNodes(const char *msg, Signal *signal,
                              SignalCounter *counter, SendFunction fun,
                              Uint32 extra, Uint32 block, Uint32 gsn,
                              Uint32 len, JobBufferLevel level) {
  if (counter) counter->clearWaitingFor();
...
do_send:
  ...
  do {
    jam();
    ptrCheckGuard(nodePtr, MAX_NDB_NODES, nodeRecord);
    if (counter) counter->setWaitingFor(nodePtr.i);
    if (!masked.get(nodePtr.i)) {
      ...
      BaseString::snprintf(buf + bufpos, sizeof(buf) - bufpos, "%u ",
                           nodePtr.i);
    } else {
      BaseString::snprintf(buf + bufpos, sizeof(buf) - bufpos, "[%u] ",
                           nodePtr.i);
 ...
 *
 */
void Dbdih::execCREATE_NODEGROUP_IMPL_REQ(Signal *signal) {
  jamEntry();
  ...
  switch (rt) {
    ...
    case CreateNodegroupImplReq::RT_COMMIT: {
      ...
      if (rt == CreateNodegroupImplReq::RT_PARSE ||
          rt == CreateNodegroupImplReq::RT_PREPARE) {
        /**
 ...
 *
 */
void Dbdih::execDROP_NODEGROUP_IMPL_REQ(Signal *signal) {
  jamEntry();
...
Uint32 Dbdih::getMinVersion() const {
...
Uint8 Dbdih::getMaxStartedFragCheckpointsForNode(Uint32 nodeId) const {
