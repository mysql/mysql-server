...
inline int pageSize(const NewVARIABLE *baseAddrRef) {
  int log_psize;
  int log_qsize = baseAddrRef->bits.q;
  int log_vsize = baseAddrRef->bits.v;
  if (log_vsize < 3) log_vsize = 3;
...
Ndbfs::~Ndbfs() {
  ...
  if (theRequestPool) delete theRequestPool;
}

static bool do_mkdir(const char *path) {
  return NdbDir::create(path, NdbDir::u_rwx() | NdbDir::g_r() | NdbDir::g_x(),
                        true /* ignore_existing */);
}

static void add_path(BaseString &dst, const char *add) {
...
static bool validate_path(BaseString &dst, const char *path) {
  char buf2[PATH_MAX];
  ...
  if (!GetFullPathName(path, sizeof(buf2), buf2, &szFilePart) ||
    ...
    return false;
#else
  if (::realpath(path, buf2) == NULL || ::access(buf2, W_OK) != 0) return false;
#endif
  dst.assign(buf2);
  add_path(dst, "");
  return true;
...
void Ndbfs::execREAD_CONFIG_REQ(Signal *signal) {
  ...
  m_base_path[FsOpenReq::BP_FS].assfmt("%s%s", m_ctx.m_config.fileSystemPath(),
                                       tmp.c_str());
  ...
  if (noIdleFiles > m_maxFiles && m_maxFiles != 0) {
    ...
    g_eventLogger->info("Resetting MaxNoOfOpenFiles %u to %u", m_maxFiles,
                        newMax);
    m_maxFiles = newMax;
  }
 ...
 * DR5  : SignalKey
 */
void Ndbfs::execSTTOR(Signal *signal) {
  jamEntry();
  ...
  if (signal->theData[1] == 0) {  // StartPhase 0
    ...
    if (ERROR_INSERTED(2000) || ERROR_INSERTED(2001)) {
      ...
      if (NdbDir::remove_recursive(to_dir.c_str(), !only_contents)) {
        ...
        g_eventLogger->warning("Failed cleaning file system at %s",
                               to_dir.c_str());
      }
      if (access(to_dir.c_str(), F_OK) == 0 || errno != ENOENT) {
        g_eventLogger->error(
            "Destination file system at %s should not be there (errno %d)!",
            to_dir.c_str(), errno);
      ...
      if (rename(from_dir.c_str(), to_dir.c_str()) == -1) {
        g_eventLogger->error(
            "Failed renaming %s file system to %s while %s (errno %d)",
            from_dir.c_str(), to_dir.c_str(),
            (ERROR_INSERTED(2000) ? "saving" : "restoring"), errno);
...
int Ndbfs::forward(AsyncFile *file, Request *request) {
  jam();
...
void Ndbfs::execFSOPENREQ(Signal *signal) {
  ...
  const Uint64 file_size =
      (Uint64{fsOpenReq->file_size_hi} << 32) | fsOpenReq->file_size_lo;
  const Uint32 auto_sync_size = fsOpenReq->auto_sync_size;
#if defined(NAME_BASED_DISABLING_COMPRESS_ENCRYPT_ODIRECT)
  const int name_hash =
      crc32(0, ((const unsigned char *)file->theFileName.c_str()),
 ...
 * remove file
 */
void Ndbfs::execFSCLOSEREQ(Signal *signal) {
  jamEntry();
  ...
  if (format != FsReadWriteReq::fsFormatGlobalPage &&
    ...
    switch (format) {
      ...
      case FsReadWriteReq::fsFormatArrayOfPages: {
        if ((fsRWReq->numberOfPages + fsRWReq->data.arrayOfPages.varIndex) >
            tNRR) {
          jam();
          errorCode = FsRef::fsErrInvalidParameters;
          goto error;
        ...
        request->par.readWrite.pages[0].size =
            tPageSize * fsRWReq->numberOfPages;
...
error:
  // Do not use request, it is only partially filled in
  ...
  switch (action) {
    ...
    case Request::writeSync: {
      jam();
      log_file_error(GSN_FSWRITEREF, openFile, nullptr, fsRef);
    ...
    case Request::read: {
      jam();
      log_file_error(GSN_FSREADREF, openFile, nullptr, fsRef);
...
*/
void Ndbfs::execFSWRITEREQ(Signal *signal) {
  jamEntry();
  const FsReadWriteReq *const fsWriteReq =
      (FsReadWriteReq *)&signal->theData[0];
 ...
 */
void Ndbfs::execFSSYNCREQ(Signal *signal) {
  jamEntry();
 ...
 * PR0: File Pointer DR0: User reference DR1: User Pointer
 */
void Ndbfs::execFSSUSPENDORD(Signal *signal) {
  jamEntry();
  Uint16 filePointer = (Uint16)signal->theData[0];
...
error:
  ...
  jam();
  log_file_error(GSN_FSAPPENDREF, openFile, nullptr, fsRef);
  sendSignal(userRef, GSN_FSAPPENDREF, signal, 3, JBB);
  return;
}

void Ndbfs::execALLOC_MEM_REQ(Signal *signal) {
  jamEntry();
...
void Ndbfs::execBUILD_INDX_IMPL_REQ(Signal *signal) {
  jamEntry();
...
AsyncFile *Ndbfs::createAsyncFile() {
  ...
  if (m_maxFiles != 0 && theFiles.size() == m_maxFiles) {
    ...
    g_eventLogger->info("m_maxFiles: %u, theFiles.size() = %u", m_maxFiles,
                        theFiles.size());
    ERROR_SET(fatal, NDBD_EXIT_AFS_MAXOPEN, "",
              " Ndbfs::createAsyncFile: creating more than MaxNoOfOpenFiles");
  }
  ...
  int err = file->init();
  if (err == -1) {
    ERROR_SET(fatal, NDBD_EXIT_AFS_ZLIB_INIT_FAIL, "",
              " Ndbfs::createAsyncFile: Zlib init failure");
  } else if (err) {
...
void Ndbfs::pushIdleFile(AsyncFile *file) {
...
AsyncIoThread *Ndbfs::createIoThread(bool bound) {
  ...
  if (thr) {
    ...
    thr->set_real_time(globalEmulatorData.theConfiguration->get_io_real_time());
...
AsyncFile *Ndbfs::getIdleFile(bool bound) {
  AsyncFile *file = 0;
...
void Ndbfs::report(Request *request, Signal *signal) {
  ...
  if (request->error.code != 0) {
    ...
    if (request->error.code & FsRef::FS_ERR_BIT) {
      ...
      fsRef->setErrorCode(fsRef->errorCode,
                          translateErrno(request->error.code));
    ...
    switch (request->action) {
      ...
      case Request::allocmem: {
        ...
        sendSignal(ref, GSN_ALLOC_MEM_REF, signal, AllocMemRef::SignalLength,
                   JBB);
        pushIdleFile(request->file);
        break;
      }
    ...
    switch (request->action) {
      ...
      case Request::allocmem: {
        ...
        sendSignal(ref, GSN_ALLOC_MEM_CONF, signal, AllocMemConf::SignalLength,
                   JBB);
        pushIdleFile(request->file);
        break;
      }
...
bool Ndbfs::scanIPC(Signal *signal) {
...
void Ndbfs::execSEND_PACKED(Signal *signal) {
  /**
...
void Ndbfs::execDUMP_STATE_ORD(Signal *signal) {
  ...
  if (signal->theData[0] == DumpStateOrd::NdbfsDumpFileStat) {
    infoEvent("NDBFS: Files: %d Open files: %d", theFiles.size(),
              theOpenFiles.size());
    infoEvent(" Idle files: %u Max opened files: %d", theIdleFiles.size(),
              m_maxOpenedFiles);
    infoEvent(" Bound Threads: %u (active %u) Unbound threads: %u",
              m_bound_threads_cnt, m_active_bound_threads_cnt,
              m_unbounds_threads_cnt);
    infoEvent(" Max files: %d", m_maxFiles);
    infoEvent(" Requests: %d", theRequestPool->size());
  ...
  if (signal->theData[0] == DumpStateOrd::NdbfsDumpOpenFiles) {
    ...
    for (unsigned i = 0; i < theOpenFiles.size(); i++) {
      ...
      infoEvent("%2d (%p): %s thr: %p", i, file, file->theFileName.c_str(),
                file->getThread());
    }
    return;
  }
  ...
  if (signal->theData[0] == DumpStateOrd::NdbfsDumpIdleFiles) {
    infoEvent("NDBFS: Dump idle files: %u", theIdleFiles.size());
  ...
  if (signal->theData[0] == DumpStateOrd::NdbfsDumpRequests) {
    g_eventLogger->info("NDBFS: Dump requests: %u", theRequestPool->inuse());
    ...
    while ((req = theRequestPool->getNextInUseItem(req))) {
      ...
      if (NdbTick_IsValid(req->m_startTime)) {
        duration =
            NdbTick_Elapsed(req->m_startTime, getHighResTimer()).microSec();
      }

      g_eventLogger->info(
          "Request action %u %s userRef 0x%x "
          "userPtr %u filePtr %u bind %u "
          "duration(us) %llu filename %s",
          req->action, Request::actionName(req->action), req->theUserReference,
          req->theUserPointer, req->theFilePointer, req->m_do_bind, duration,
          (req->file ? req->file->theFileName.c_str() : "NO FILE"));
    }
    return;
  }
...
const char *Ndbfs::get_filename(Uint32 fd) const {
  jamNoBlock();
  const AsyncFile *openFile = theOpenFiles.find(fd);
  if (openFile) return openFile->theFileName.get_base_name();
  return "";
}
...
void Ndbfs::log_file_error(GlobalSignalNumber gsn, AsyncFile *file,
  ...
  require((file == nullptr) || (request == nullptr));
