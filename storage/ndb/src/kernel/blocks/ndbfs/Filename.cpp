...
static const char *fileExtension[] = {".Data",     ".FragLog",   ".LocLog",
                                      ".FragList", ".TableList", ".SchemaLog",
                                      ".sysfile",  ".log",       ".ctl"};
  ...
  switch (version) {
    case 1: {
...

      {
        ...
        BaseString::snprintf(buf, sizeof(buf), "%s%s", blockName,
                             DIR_SEPARATOR);
        strcat(theName, buf);
      }
    ...
    case 2: {
      ...
      if (partNum == 0)
        ...
        BaseString::snprintf(buf, sizeof(buf),
                             "BACKUP%sBACKUP-%u%sBACKUP-%u-PART-%u-OF-%u%s",
                             DIR_SEPARATOR, seq, DIR_SEPARATOR, seq, partNum,
                             totalParts, DIR_SEPARATOR);

      strcat(theName, buf);
      if (count == 0xffff) {
        BaseString::snprintf(buf, sizeof(buf), "BACKUP-%u.%u", seq, nodeId);
        strcat(theName, buf);
      } else {
        BaseString::snprintf(buf, sizeof(buf), "BACKUP-%u-%u.%u", seq, count,
                             nodeId);
        strcat(theName, buf);
      }
      break;
    ...
    case 4: {
      ...
      if (ptr_sz_bytes == 0) {
        ERROR_SET(ecError, NDBD_EXIT_AFS_PARAMETER, "", "File name is empty");
        return;
      }
      ...
      const bool nul_terminated =
          ((buf[ptr_sz_bytes - 1] == '\0') || (buf[ptr_sz_bytes - 2] == '\0') ||
           (buf[ptr_sz_bytes - 3] == '\0') || (buf[ptr_sz_bytes - 4] == '\0'));
      if (!nul_terminated) {
        ERROR_SET(ecError, NDBD_EXIT_AFS_PARAMETER, "",
      ...
      if (buf[0] == DIR_SEPARATOR[0]) {
        ...
        const size_t concat_sz = BaseString::snprintf(
            theName, theName_sz, "%s%s", fs->get_base_path(bp).c_str(), buf);
    ...
    case 5: {
      ...
      BaseString::snprintf(buf, sizeof(buf), "LCP%s%d%sT%dF%d", DIR_SEPARATOR,
                           lcpNo, DIR_SEPARATOR, tableId, fragId);
      strcat(theName, buf);
      break;
    }
    case 6: {
