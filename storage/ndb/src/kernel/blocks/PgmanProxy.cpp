...
PgmanProxy::PgmanProxy(Block_context &ctx) : LocalProxy(PGMAN, ctx) {
...
SimulatedBlock *PgmanProxy::newWorker(Uint32 instanceNo) {
  return new Pgman(m_ctx, instanceNo);
}

// GSN_SYNC_EXTENT_PAGES_REQ
void PgmanProxy::execSYNC_EXTENT_PAGES_REQ(Signal *signal) {
...
void PgmanProxy::execEND_LCPREQ(Signal *signal) {
  ...
  ndbrequire(sb == LGMAN);
  {
    ...
    req->requestData = 0;
    // Extra worker
    sendSignal(workerRef(c_workers - 1), GSN_RELEASE_PAGES_REQ, signal,
               ReleasePagesReq::SignalLength, JBB);
    return;
  }
}

void PgmanProxy::execRELEASE_PAGES_CONF(Signal *signal) {
  jam();
...
void PgmanProxy::sendEND_LCPREQ(Signal *signal, Uint32 ssId,
  ...
  *req = ss.m_req;
  req->senderData = ssId;
  req->senderRef = reference();
  sendSignalNoRelease(workerRef(ss.m_worker), GSN_END_LCPREQ, signal,
                      EndLcpReq::SignalLength, JBB, handle);
}

void PgmanProxy::execEND_LCPCONF(Signal *signal) {
  jam();
...
void PgmanProxy::sendEND_LCPCONF(Signal *signal, Uint32 ssId) {
  jam();
  Ss_END_LCPREQ &ss = ssFind<Ss_END_LCPREQ>(ssId);
...
void PgmanProxy::get_extent_page(Page_cache_client &caller, Signal *signal,
...
int PgmanProxy::get_page(Page_cache_client &caller, Signal *signal,
...
void PgmanProxy::set_lsn(Page_cache_client &caller, Local_key key, Uint64 lsn) {
  ndbrequire(blockToInstance(caller.m_block) == 0);
  SimulatedBlock *block = globalData.getBlock(caller.m_block);
  Pgman *worker = (Pgman *)workerBlock(c_workers - 1);  // extraWorkerBlock();
  Page_cache_client pgman(block, worker);
...
void PgmanProxy::update_lsn(Signal *signal, Page_cache_client &caller,
...
int PgmanProxy::drop_page(Page_cache_client &caller, Local_key key,
                          Uint32 page_id) {
  ndbrequire(blockToInstance(caller.m_block) == 0);
  SimulatedBlock *block = globalData.getBlock(caller.m_block);
  Pgman *worker = (Pgman *)workerBlock(c_workers - 1);  // extraWorkerBlock();
...
Uint32 PgmanProxy::create_data_file(Signal *signal, Uint32 version) {
  Pgman *worker = (Pgman *)workerBlock(c_workers - 1);  // extraWorkerBlock();
  Uint32 ret = worker->create_data_file(version);
  Uint32 i;
  for (i = 0; i < c_workers - 1; i++) {
    jam();
    send_data_file_ord(signal, i, ret, version, DataFileOrd::CreateDataFile);
  }
  return ret;
}

Uint32 PgmanProxy::alloc_data_file(Signal *signal, Uint32 file_no,
                                   Uint32 version) {
  Pgman *worker = (Pgman *)workerBlock(c_workers - 1);  // extraWorkerBlock();
  ...
  for (i = 0; i < c_workers - 1; i++) {
    jam();
    send_data_file_ord(signal, i, ret, version, DataFileOrd::AllocDataFile,
                       file_no);
  }
  return ret;
}

void PgmanProxy::map_file_no(Signal *signal, Uint32 file_no, Uint32 fd) {
  Pgman *worker = (Pgman *)workerBlock(c_workers - 1);  // extraWorkerBlock();
  worker->map_file_no(file_no, fd);
  Uint32 i;
  for (i = 0; i < c_workers - 1; i++) {
    jam();
    send_data_file_ord(signal, i, ~(Uint32)0, 0, DataFileOrd::MapFileNo,
                       file_no, fd);
  }
}

void PgmanProxy::free_data_file(Signal *signal, Uint32 file_no, Uint32 fd) {
  Pgman *worker = (Pgman *)workerBlock(c_workers - 1);  // extraWorkerBlock();
  worker->free_data_file(file_no, fd);
  Uint32 i;
  for (i = 0; i < c_workers - 1; i++) {
    jam();
    send_data_file_ord(signal, i, ~(Uint32)0, 0, DataFileOrd::FreeDataFile,
                       file_no, fd);
  }
}

void PgmanProxy::send_data_file_ord(Signal *signal, Uint32 i, Uint32 ret,
                                    Uint32 version, Uint32 cmd, Uint32 file_no,
                                    Uint32 fd) {
  ...
  ord->file_no = file_no;
  ord->fd = fd;
  sendSignal(workerRef(i), GSN_DATA_FILE_ORD, signal, DataFileOrd::SignalLength,
             JBB);
}

bool PgmanProxy::extent_pages_available(Uint32 pages_needed,
