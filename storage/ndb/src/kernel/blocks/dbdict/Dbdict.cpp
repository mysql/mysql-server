...
#define EVENT_TRACE \
  // g_eventLogger->info("Event debug trace: File: %s Line: %u", __FILE__,
  // __LINE__)
...
static Uint32 alter_obj_inc_schema_version(Uint32 old) {
  return (old & 0x00FFFFFF) + ((old + 0x1000000) & 0xFF000000);
}

static Uint32 alter_obj_dec_schema_version(Uint32 old) {
  return (old & 0x00FFFFFF) + ((old - 0x1000000) & 0xFF000000);
}

static Uint32 create_obj_inc_schema_version(Uint32 old) {
  return (old + 0x00000001) & 0x00FFFFFF;
}

static void do_swap(Uint32 &v0, Uint32 &v1) {
  Uint32 save = v0;
  v0 = v1;
  v1 = save;
}
...
// block by command.
/* ---------------------------------------------------------------- */
void Dbdict::execDUMP_STATE_ORD(Signal *signal) {
  jamEntry();
...
#define MEMINFO(x, y) infoEvent(x ": %d %d", y.getSize(), y.getNoOfFree())
  ...
  if (signal->theData[0] == DumpStateOrd::DictDumpLockQueue) {
    ...
    infoEvent("DICT : c_sub_startstop _outstanding %u _lock %s",
              c_outstanding_sub_startstop, c_sub_startstop_lock.getText(buf));
  }

  if (signal->theData[0] == DumpStateOrd::DictDumpGetTabInfoQueue &&
      g_trace >= 2) {
    jam();
    ...
    g_eventLogger->info(
        "c_retrieveRecord totalRequests :  %llu "
        "busyState : %u",
        c_retrieveRecord.totalRequests, c_retrieveRecord.busyState);
    g_eventLogger->info(
        "totalWaiters : %llu totalBusy : %llu "
        "longestWaitMillis : %llu",
        c_retrieveRecord.totalWaiters, c_retrieveRecord.totalBusy,
    ...
    if (c_retrieveRecord.busyState) {
      jam();
      g_eventLogger->info(
          "Busy : requestType : %u  tableId : %u, "
          "type : %u, ref : %x",
          c_retrieveRecord.requestType, c_retrieveRecord.tableId,
          c_retrieveRecord.m_table_type, c_retrieveRecord.blockRef);
      currElapsed = NdbTick_Elapsed(c_retrieveRecord.startProcessTime,
                                    NdbTick_getCurrentTicks())
                        .milliSec();
      g_eventLogger->info("Elapsed processing millis : %llu", currElapsed);
    }
    ...
    if (signal->getLength() == 2) {
      ...
      if (currElapsed > SuspectGETTABINFOREQMillis) {
        ...
        sendSignalWithDelay(reference(), GSN_DUMP_STATE_ORD, signal, 2000,
                            signal->getLength());
      } else {
        jam();
  ...
  if (signal->theData[0] == 8004) {
    ...
    infoEvent("DICT: c_counterMgr size: %u free: %u", c_counterMgr.getSize(),
  ...
  if (signal->theData[0] == 8005) {
    ...
    g_eventLogger->info(
        "DICT: Setting fake empty generic SafeCounter pool to %u", val);
    g_eventLogger->info("DICT: c_counterMgr size: %u free: %u",
                        c_counterMgr.getSize(), c_counterMgr.getNoOfFree());
      ...
      Ndbinfo::pool_entry pools[] = {
          ...
          {"DictObject",
           ...
           {CFG_DB_NO_TABLES, CFG_DB_NO_ORDERED_INDEXES,
            CFG_DB_NO_UNIQUE_HASH_INDEXES, CFG_DB_NO_TRIGGERS},
           0},
          {"Transaction Handle",
      ...
      while (!done) {
        ...
        if (req.tableId == Ndbinfo::DICT_OBJ_INFO_TABLEID) {
          ...
          if (DictTabInfo::isTable(ltd.getTableType()) ||
            ...
            row.write_uint32(ltd.getTableId());
            row.write_uint32(
                (tabPtr.p->m_bits & TableRecord::TR_Logged) == 0 ? 0 : 1);
            row.write_uint32(
                (tabPtr.p->m_bits & TableRecord::TR_RowGCI) == 0 ? 0 : 1);
            row.write_uint32(
                (tabPtr.p->m_bits & TableRecord::TR_RowChecksum) == 0 ? 0 : 1);
            row.write_uint32(
                (tabPtr.p->m_bits & TableRecord::TR_Temporary) == 0 ? 0 : 1);
            row.write_uint32(
                (tabPtr.p->m_bits & TableRecord::TR_ForceVarPart) == 0 ? 0 : 1);
            row.write_uint32(
                (tabPtr.p->m_bits & TableRecord::TR_ReadBackup) == 0 ? 0 : 1);
            row.write_uint32(
                (tabPtr.p->m_bits & TableRecord::TR_FullyReplicated) == 0 ? 0
                                                                          : 1);
            ...
            row.write_uint32(tabPtr.p->partitionBalance);
  ...
  switch (signal->theData[0]) {
    case ZPACK_TABLE_INTO_PAGES:
      ...
      if (ERROR_INSERTED(6800)) {
        jam();
        g_eventLogger->info(
            "Stall serialisation of a table record "
            "until error insert cleared");
        sendSignalWithDelay(reference(), GSN_CONTINUEB, signal, 1000,
                            signal->getLength());
        return;
      }
...
void Dbdict::packTableIntoPages(Signal *signal) {
...

  {
    ...
    if (objEntry == 0) {
      jam();
      sendGET_TABINFOREF(signal, &req_copy, GetTabInfoRef::TableNotDefined,
                         __LINE__);
    ...
    if (transId != 0 && transId == objEntry->m_transId) {
      ...
      if ((err = check_read_obj(objEntry))) {
        ...
        sendGET_TABINFOREF(signal, &req_copy, (GetTabInfoRef::ErrorCode)err,
                           __LINE__);
        initRetrieveRecord(signal, 0, 0);
        return;
      }
...
bool packRopeData(SimpleProperties::Writer &w, ConstRope &rope) {
...
bool unpackDataToRope(SimpleProperties::Reader &it, LocalRope &rope) {
  const int bufsize = ROPE_COPY_BUFFER_SIZE;
  char buffer[bufsize];
  int nread;
  ...
  while ((nread = it.getBuffered(buffer, bufsize)) > 0)
    if (!rope.appendBuffer(buffer, nread)) return false;
  return true;
}

void Dbdict::packTableIntoPages(SimpleProperties::Writer &w,
                                TableRecordPtr tablePtr, Signal *signal) {
  union {
  ...
  if (signal) {
    ...
    for (Uint32 i = 0; i < count; i++) data[i] = htons(data[i]);
  ...
  for (list.first(attrPtr); !attrPtr.isNull(); list.next(attrPtr)) {
    ...
    if (def.size()) {
      ...
      if (remainBytes)
        NdbSqlUtil::convertByteOrder(attrType, attrSize, arrayType, arraySize,
...
void Dbdict::packFileIntoPages(SimpleProperties::Writer &w, FilePtr f_ptr,
  ...
  f.FilegroupId = f_ptr.p->m_filegroup_id;
  ;  // group.m_id;
...
void Dbdict::execCREATE_FRAGMENTATION_REQ(Signal *signal) {
...
void Dbdict::execFSCLOSECONF(Signal *signal) {
  ...
  jamEntry();
  ndbrequire(c_fsConnectRecordPool.getPtr(fsPtr, fsConf->userPointer));
...
void Dbdict::execFSOPENREF(Signal *signal) {
  ...
  FsConnectRecordPtr fsPtr;
  ndbrequire(c_fsConnectRecordPool.getPtr(fsPtr, fsRef->userPointer));
  switch (fsPtr.p->fsState) {
    case FsConnectRecord::OPEN_READ_SCHEMA1:
      jam();
...
/* ---------------------------------------------------------------- */
void Dbdict::writeTableFile(Signal *signal, Uint32 tableId,
  ...
  pageRecPtr.p->word[ZPOS_CHECKSUM] =
      computeChecksum(&pageRecPtr.p->word[0], pages * ZSIZE_OF_PAGES_IN_WORDS);

  startWriteTableFile(signal, tableId);
}

// SchemaTrans variant
void Dbdict::writeTableFile(Signal *signal, SchemaOpPtr op_ptr, Uint32 tableId,
...

  {
    ...
    std::memset(&pageRecPtr.p->word[0], 0, 4 * ZPAGE_HEADER_SIZE);
    pageRecPtr.p->word[ZPOS_CHECKSUM] = computeChecksum(
        &pageRecPtr.p->word[0], pages * ZSIZE_OF_PAGES_IN_WORDS);
  }
...
void Dbdict::openTableFile(Signal *signal, Uint32 fileNo, Uint32 fsConPtr,
  ...
  if (writeFlag) {
    jam();
    fsOpenReq->fileFlags = FsOpenReq::OM_WRITEONLY | FsOpenReq::OM_TRUNCATE |
                           FsOpenReq::OM_CREATE | FsOpenReq::OM_SYNC;
  } else {
    jam();
...
void Dbdict::writeTableConf(Signal *signal, FsConnectRecordPtr fsPtr) {
  fsPtr.p->fsState = FsConnectRecord::CLOSE_WRITE_TAB_FILE;
  closeFile(signal, fsPtr.p->filePtr, fsPtr.i);
  return;
}  // Dbdict::writeTableConf()

void Dbdict::closeWriteTableConf(Signal *signal, FsConnectRecordPtr fsPtr) {
...
void Dbdict::openReadTableRef(Signal *signal, FsConnectRecordPtr fsPtr) {
...
void Dbdict::readTableConf(Signal *signal, FsConnectRecordPtr fsPtr) {
  /* ---------------------------------------------------------------- */
...
void Dbdict::readTableRef(Signal *signal, FsConnectRecordPtr fsPtr) {
  /**
...
void Dbdict::closeReadTableConf(Signal *signal, FsConnectRecordPtr fsPtr) {
  c_fsConnectRecordPool.release(fsPtr);
...
void Dbdict::openSchemaFile(Signal *signal, Uint32 fileNo, Uint32 fsConPtr,
  ...
  if (writeFlag) {
    jam();
    fsOpenReq->fileFlags = FsOpenReq::OM_WRITEONLY | FsOpenReq::OM_SYNC;
    if (newFile)
      fsOpenReq->fileFlags |= FsOpenReq::OM_TRUNCATE | FsOpenReq::OM_CREATE;
  } else {
    jam();
    fsOpenReq->fileFlags = FsOpenReq::OM_READONLY;
  }                              // if
...
void Dbdict::writeSchemaConf(Signal *signal, FsConnectRecordPtr fsPtr) {
...
void Dbdict::closeWriteSchemaConf(Signal *signal, FsConnectRecordPtr fsPtr) {
...
void Dbdict::openReadSchemaRef(Signal *signal, FsConnectRecordPtr fsPtr) {
...
void Dbdict::readSchemaConf(Signal *signal, FsConnectRecordPtr fsPtr) {
  /* ---------------------------------------------------------------- */
  // Verify the data read from disk
  /* ---------------------------------------------------------------- */
  bool crashInd;
  ...
  for (Uint32 n = 0; n < xsf->noOfPages; n++) {
    ...
    if (!ok) {
      ...
      BaseString::snprintf(reason_msg, sizeof(reason_msg),
                           ...
                           "sz=%u sz0=%u pn=%u)",
                           n, reason, sf->FileSize, sf0->FileSize,
                           sf->PageNumber);
      if (crashInd) progError(__LINE__, NDBD_EXIT_SR_SCHEMAFILE, reason_msg);
...
void Dbdict::readSchemaRef(Signal *signal, FsConnectRecordPtr fsPtr) {
  /**
   * First close corrupt file
   */
  fsPtr.p->fsState = FsConnectRecord::OPEN_READ_SCHEMA2;
...
void Dbdict::closeReadSchemaConf(Signal *signal, FsConnectRecordPtr fsPtr) {
  c_fsConnectRecordPool.release(fsPtr);
  ...
  switch (state) {
    case ReadSchemaRecord::INITIAL_READ:
      jam();
      {
        ...
        Uint32 noOfPages =
            (c_noOfMetaTables + NDB_SF_PAGE_ENTRIES - 1) / NDB_SF_PAGE_ENTRIES;
...
Dbdict::Dbdict(Block_context &ctx)
  ...
  addRecSignal(GSN_CREATE_FRAGMENTATION_REQ,
               &Dbdict::execCREATE_FRAGMENTATION_REQ);
  addRecSignal(GSN_CREATE_FRAGMENTATION_REF,
               &Dbdict::execCREATE_FRAGMENTATION_REF);
  addRecSignal(GSN_CREATE_FRAGMENTATION_CONF,
               &Dbdict::execCREATE_FRAGMENTATION_CONF);
  ...
  addRecSignal(GSN_SCHEMA_TRANS_BEGIN_CONF,
               &Dbdict::execSCHEMA_TRANS_BEGIN_CONF);
  ...
  addRecSignal(GSN_CREATE_NODEGROUP_IMPL_REF,
               &Dbdict::execCREATE_NODEGROUP_IMPL_REF);
  addRecSignal(GSN_CREATE_NODEGROUP_IMPL_CONF,
               &Dbdict::execCREATE_NODEGROUP_IMPL_CONF);
  ...
  addRecSignal(GSN_DROP_NODEGROUP_IMPL_REF,
               &Dbdict::execDROP_NODEGROUP_IMPL_REF);
  addRecSignal(GSN_DROP_NODEGROUP_IMPL_CONF,
               &Dbdict::execDROP_NODEGROUP_IMPL_CONF);
...
bool Dbdict::getParam(const char *name, Uint32 *count) {
  if (name != 0 && count != 0) {
...
void Dbdict::initialiseTableRecord(TableRecordPtr tablePtr, Uint32 tableId) {
  ...
  tablePtr.p->partitionBalance = NDB_DEFAULT_PARTITION_BALANCE;
...
Uint32 Dbdict::getFsConnRecord() {
  FsConnectRecordPtr fsPtr;
  ndbrequire(c_fsConnectRecordPool.seize(fsPtr));
...
bool Dbdict::seizeTableRecord(TableRecordPtr &tablePtr, Uint32 &schemaFileId) {
  ...
  if (!c_tableRecordPool_.seize(tablePtr)) {
    jam();
    return false;
  }
...
bool Dbdict::seizeTriggerRecord(TriggerRecordPtr &triggerPtr,
                                Uint32 triggerId) {
  if (triggerId == RNIL) {
    ...
    if (ok) {  // triggerId already in use
      jam();
      return false;
    }
  }
  ...
  if (!c_triggerRecordPool_.seize(triggerPtr)) {
    jam();
    return false;
  }
...
Uint32 Dbdict::check_read_obj(Uint32 objId, Uint32 transId) {
  XSchemaFile *xsf = &c_schemaFile[SchemaRecord::NEW_SCHEMA_FILE];
...
Uint32 Dbdict::check_write_obj(Uint32 objId, Uint32 transId,
  ...
  if (objId < (NDB_SF_PAGE_ENTRIES * xsf->noOfPages)) {
    ...
    if (te->m_tableState == SchemaFile::SF_DROP && op == SchemaFile::SF_DROP) {
      jam();
      return DropTableRef::ActiveSchemaTrans;
    }
...
Uint32 Dbdict::check_write_obj(Uint32 objId, Uint32 transId,
                               SchemaFile::EntryState op, ErrorInfo &error) {
...
void Dbdict::execREAD_CONFIG_REQ(Signal *signal) {
  ...
  ndb_mgm_get_int_parameter(p, CFG_DEFAULT_HASHMAP_SIZE,
  ...
  */
   ...
   */
  c_createTableRecPool.arena_pool_init(&c_arenaAllocator,
                                       RT_DBDICT_CREATE_TABLE, pc);
  c_dropTableRecPool.arena_pool_init(&c_arenaAllocator, RT_DBDICT_DROP_TABLE,
                                     pc);
  c_alterTableRecPool.arena_pool_init(&c_arenaAllocator, RT_DBDICT_ALTER_TABLE,
                                      pc);
  c_createTriggerRecPool.arena_pool_init(&c_arenaAllocator,
                                         RT_DBDICT_CREATE_TRIGGER, pc);
  c_dropTriggerRecPool.arena_pool_init(&c_arenaAllocator,
                                       RT_DBDICT_DROP_TRIGGER, pc);
  c_createIndexRecPool.arena_pool_init(&c_arenaAllocator,
                                       RT_DBDICT_CREATE_INDEX, pc);
  c_dropIndexRecPool.arena_pool_init(&c_arenaAllocator, RT_DBDICT_DROP_INDEX,
                                     pc);
  c_alterIndexRecPool.arena_pool_init(&c_arenaAllocator, RT_DBDICT_ALTER_INDEX,
                                      pc);
  c_buildIndexRecPool.arena_pool_init(&c_arenaAllocator, RT_DBDICT_BUILD_INDEX,
                                      pc);
  c_indexStatRecPool.arena_pool_init(&c_arenaAllocator, RT_DBDICT_INDEX_STAT,
                                     pc);
  c_createFilegroupRecPool.arena_pool_init(&c_arenaAllocator,
                                           RT_DBDICT_CREATE_FILEGROUP, pc);
  c_createFileRecPool.arena_pool_init(&c_arenaAllocator, RT_DBDICT_CREATE_FILE,
                                      pc);
  c_dropFilegroupRecPool.arena_pool_init(&c_arenaAllocator,
                                         RT_DBDICT_DROP_FILEGROUP, pc);
  c_dropFileRecPool.arena_pool_init(&c_arenaAllocator, RT_DBDICT_DROP_FILE, pc);
  c_createHashMapRecPool.arena_pool_init(&c_arenaAllocator,
                                         RT_DBDICT_CREATE_HASH_MAP, pc);
  c_copyDataRecPool.arena_pool_init(&c_arenaAllocator, RT_DBDICT_COPY_DATA, pc);
  c_schemaOpPool.arena_pool_init(&c_arenaAllocator, RT_DBDICT_SCHEMA_OPERATION,
                                 pc);

  c_hash_map_pool.setSize(32);
  g_hash_map.setSize(32);

  c_createNodegroupRecPool.arena_pool_init(&c_arenaAllocator,
                                           RT_DBDICT_CREATE_NODEGROUP, pc);
  c_dropNodegroupRecPool.arena_pool_init(&c_arenaAllocator,
                                         RT_DBDICT_DROP_NODEGROUP, pc);
...
#define RT_DBDICT_DROP_FK RT_DBDICT_DROP_NODEGROUP
  ...
  c_createFKRecPool.arena_pool_init(&c_arenaAllocator, RT_DBDICT_CREATE_FK, pc);
  c_buildFKRecPool.arena_pool_init(&c_arenaAllocator, RT_DBDICT_BUILD_FK, pc);
  c_dropFKRecPool.arena_pool_init(&c_arenaAllocator, RT_DBDICT_DROP_FK, pc);
...

  {
    ...
    while (objs.seizeFirst(ptr)) new (ptr.p) DictObject();
...
void Dbdict::initSchemaFile_conf(Signal *signal, Uint32 callbackData,
                                 Uint32 rv) {
  jam();
...
void Dbdict::activateIndexes(Signal *signal, Uint32 id) {
  if (id == 0) D("activateIndexes start");
  ...
  for (; id < c_noOfMetaTables; id++) {
    ...
    Callback c = {safe_cast(&Dbdict::activateIndex_fromBeginTrans),
                  tx_ptr.p->tx_key};
...
void Dbdict::activateIndex_fromBeginTrans(Signal *signal, Uint32 tx_key,
                                          Uint32 ret) {
  ...
  TableRecordPtr indexPtr;
  ndbrequire(c_tableRecordPool_.getPtr(indexPtr, tx_ptr.p->m_userData));
  DictObjectPtr index_obj_ptr;
  ndbrequire(c_obj_pool.getPtr(index_obj_ptr, indexPtr.p->m_obj_ptr_i));
  ...
  Callback c = {safe_cast(&Dbdict::activateIndex_fromAlterIndex),
                tx_ptr.p->tx_key};
  tx_ptr.p->m_callback = c;
...
void Dbdict::activateIndex_fromAlterIndex(Signal *signal, Uint32 tx_key,
                                          Uint32 ret) {
  ...
  if (ret != 0) setError(tx_ptr.p->m_error, ret, __LINE__);

  Callback c = {safe_cast(&Dbdict::activateIndex_fromEndTrans),
                tx_ptr.p->tx_key};
  tx_ptr.p->m_callback = c;

  Uint32 flags = 0;
  if (hasError(tx_ptr.p->m_error)) flags |= SchemaTransEndReq::SchemaTransAbort;
  endSchemaTrans(signal, tx_ptr, flags);
}

void Dbdict::activateIndex_fromEndTrans(Signal *signal, Uint32 tx_key,
                                        Uint32 ret) {
  ...
  if (!hasError(error)) {
    jam();
    infoEvent("DICT: activate index %u done (%s)", index_obj_ptr.p->m_id,
              indexName);
  } else {
    jam();
    warningEvent("DICT: activate index %u error: code=%u line=%u node=%u (%s)",
                 index_obj_ptr.p->m_id, error.errorCode, error.errorLine,
                 error.errorNodeId, indexName);
  }
...
void Dbdict::rebuildIndexes(Signal *signal, Uint32 id) {
  if (id == 0) D("rebuildIndexes start");
  ...
  for (; id < c_noOfMetaTables; id++) {
    ...
    if (check_read_obj(id)) continue;
    if (!indexPtr.p->isIndex()) continue;
    ...
    Callback c = {safe_cast(&Dbdict::rebuildIndex_fromBeginTrans),
                  tx_ptr.p->tx_key};
    tx_ptr.p->m_callback = c;
    beginSchemaTrans(signal, tx_ptr);
    return;
...
void Dbdict::rebuildIndex_fromBeginTrans(Signal *signal, Uint32 tx_key,
                                         Uint32 ret) {
  ...
  Callback c = {safe_cast(&Dbdict::rebuildIndex_fromBuildIndex),
                tx_ptr.p->tx_key};
  tx_ptr.p->m_callback = c;

  sendSignal(reference(), GSN_BUILDINDXREQ, signal, BuildIndxReq::SignalLength,
             JBB);
}

void Dbdict::rebuildIndex_fromBuildIndex(Signal *signal, Uint32 tx_key,
                                         Uint32 ret) {
  ...
  Callback c = {safe_cast(&Dbdict::rebuildIndex_fromEndTrans),
                tx_ptr.p->tx_key};
  tx_ptr.p->m_callback = c;
...
void Dbdict::rebuildIndex_fromEndTrans(Signal *signal, Uint32 tx_key,
                                       Uint32 ret) {
  ...
  DictObjectPtr obj_ptr;
  ndbrequire(c_obj_pool.getPtr(obj_ptr, indexPtr.p->m_obj_ptr_i));
  ...
  if (!hasError(error)) {
    jam();
    infoEvent("DICT: %s index %u done (%s)", actionName, obj_ptr.p->m_id,
              indexName);
  } else {
    jam();
    warningEvent("DICT: %s index %u error: code=%u line=%u node=%u (%s)",
                 actionName, obj_ptr.p->m_id, error.errorCode, error.errorLine,
                 error.errorNodeId, indexName);
  }
...
void Dbdict::checkFkTriggerIds(Signal *signal) {
  jam();
  ...
  sendSignal(dictMasterRef, GSN_LIST_TABLES_REQ, signal,
             ListTablesReq::SignalLength, JBB);
}

void Dbdict::execLIST_TABLES_CONF(Signal *signal) {
  jam();
  ...
  bool finished = (signal->header.m_fragmentInfo == 0) |
  ...
  while (numObjectsInSignal--) {
    ...
    switch (ltd.getTableType()) {
      case DictTabInfo::FKParentTrigger: {
        ...
        g_eventLogger->info(
            "DICT : Setting FK %u parent trigger id to %u (%u) ", fkID,
            ltd.getTableId(), fk_ptr.p->m_parentTriggerId);
      ...
      case DictTabInfo::FKChildTrigger: {
        ...
        g_eventLogger->info("DICT : Setting FK %u child trigger id to %u (%u)",
                            fkID, ltd.getTableId(), fk_ptr.p->m_childTriggerId);
...
void Dbdict::enableFKs(Signal *signal, Uint32 id) {
  if (id == 0) {
    ...
    if (/* TODO add version check if/when fixed  */
        ...
        (c_initialNodeRestart || c_nodeRestart) && !c_nr_upgrade_fks_done) {
      jam();
  ...
  for (; id < c_noOfMetaTables; id++) {
    ...
    Callback c = {safe_cast(&Dbdict::enableFK_fromBeginTrans),
                  tx_ptr.p->tx_key};
    tx_ptr.p->m_callback = c;
    beginSchemaTrans(signal, tx_ptr);
    return;
...
void Dbdict::enableFK_fromBeginTrans(Signal *signal, Uint32 tx_key,
                                     Uint32 ret) {
  ...
  Ptr<ForeignKeyRec> fk_ptr;
  ndbrequire(c_fk_pool.getPtr(fk_ptr, tx_ptr.p->m_userData));
  DictObjectPtr fk_obj_ptr;
  ndbrequire(c_obj_pool.getPtr(fk_obj_ptr, fk_ptr.p->m_obj_ptr_i));
  ...
  Callback c = {safe_cast(&Dbdict::enableFK_fromCreateFK), tx_ptr.p->tx_key};
  tx_ptr.p->m_callback = c;

  sendSignal(reference(), GSN_CREATE_FK_REQ, signal, CreateFKReq::SignalLength,
             JBB, lsPtr, 1);
}

void Dbdict::enableFK_fromCreateFK(Signal *signal, Uint32 tx_key, Uint32 ret) {
  ...
  Callback c = {safe_cast(&Dbdict::enableFK_fromEndTrans), tx_ptr.p->tx_key};
  tx_ptr.p->m_callback = c;
...
void Dbdict::enableFK_fromEndTrans(Signal *signal, Uint32 tx_key, Uint32 ret) {
  ...
  if (!hasError(error)) {
    jam();
    infoEvent("DICT: enable FK %u done (%s)", fk_obj_ptr.p->m_id, fk_name);
  } else {
    jam();
    warningEvent("DICT: enable FK %u error: code=%u line=%u node=%u (%s)",
                 fk_obj_ptr.p->m_id, error.errorCode, error.errorLine,
                 error.errorNodeId, fk_name);
  }
...
void Dbdict::execDICTSTARTREQ(Signal *signal) {
  ...
  Callback cb = {safe_cast(&Dbdict::masterRestart_checkSchemaStatusComplete),
                 0};
  startRestoreSchema(signal, cb);
}  // execDICTSTARTREQ()

void Dbdict::masterRestart_checkSchemaStatusComplete(Signal *signal,
                                                     Uint32 callbackData,
                                                     Uint32 returnCode) {
  XSchemaFile *oldxsf = &c_schemaFile[SchemaRecord::OLD_SCHEMA_FILE];
  ...
  Callback c = {0, 0};
  sendFragmentedSignal(rg, GSN_SCHEMA_INFO, signal,
...
void Dbdict::execGET_SCHEMA_INFOREQ(Signal *signal) {
  ...
  sendFragmentedSignal(ref, GSN_SCHEMA_INFO, signal,
                       1,  // GetSchemaInfoConf::SignalLength,
                       JBB, ptr, 1, c);
}  // Dbdict::execGET_SCHEMA_INFOREQ()

void Dbdict::sendSchemaComplete(Signal *signal, Uint32 callbackData,
                                Uint32 returnCode) {
...
void Dbdict::execSCHEMA_INFO(Signal *signal) {
  ...
  /* ---------------------------------------------------------------- */
  Callback cb = {safe_cast(&Dbdict::restart_checkSchemaStatusComplete), 0};
  startRestoreSchema(signal, cb);
}  // execSCHEMA_INFO()

void Dbdict::restart_checkSchemaStatusComplete(Signal *signal,
                                               Uint32 callbackData,
                                               Uint32 returnCode) {
  jam();
  ...
  if (c_systemRestart) {
    jam();
    signal->theData[0] = getOwnNodeId();
    sendSignal(calcDictBlockRef(c_masterNodeId), GSN_SCHEMA_INFOCONF, signal, 1,
               JBB);
    return;
  }
...
static bool checkSchemaStatus(Uint32 tableType, Uint32 pass) {
  switch (tableType) {
    case DictTabInfo::UndefTableType:
      return true;
    case DictTabInfo::HashIndexTrigger:
  ...
  for (; c_restartRecord.activeTable < noOfEntries;
    ...
    if (c_restartRecord.activeTable >= c_noOfMetaTables) {
      ...
      if (masterState != SchemaFile::SF_UNUSED ||
        ...
        progError(__LINE__, NDBD_EXIT_RESTORE_SCHEMA,
...
//#define PRINT_SCHEMA_RESTART
    ...
    if (c_restartRecord.m_pass <= CREATE_NEW_PASS) {
      if (!::checkSchemaStatus(masterEntry->m_tableType,
  ...
  if (c_restartRecord.m_op_cnt == 0) {
    ...
    Callback c = {safe_cast(&Dbdict::restartEndPass_fromEndTrans),
                  tx_ptr.p->tx_key};
    tx_ptr.p->m_callback = c;
...
void Dbdict::startRestoreSchema(Signal *signal, Callback cb) {
  jam();
  ...
  Callback c = {safe_cast(&Dbdict::restart_fromBeginTrans), tx_ptr.p->tx_key};
  tx_ptr.p->m_callback = c;
  beginSchemaTrans(signal, tx_ptr);
...
void Dbdict::restart_nextOp(Signal *signal, bool commit) {
  ...
  free_words =
      (rl.m_min - rl.m_curr) * GLOBAL_PAGE_SIZE_WORDS;  // underestimate
  ...
  if (commit || c_restartRecord.m_op_cnt >= ZRESTART_OPS_PER_TRANS) {
    ...
    Callback c = {safe_cast(&Dbdict::restart_fromEndTrans), tx_ptr.p->tx_key};
    tx_ptr.p->m_callback = c;
...
void Dbdict::restart_fromEndTrans(Signal *signal, Uint32 tx_key, Uint32 ret) {
  TxHandlePtr tx_ptr;
  findTxHandle(tx_ptr, tx_key);
  ndbrequire(!tx_ptr.isNull());
...
void Dbdict::restartEndPass_fromEndTrans(Signal *signal, Uint32 tx_key,
                                         Uint32 ret) {
  TxHandlePtr tx_ptr;
  findTxHandle(tx_ptr, tx_key);
  ndbrequire(!tx_ptr.isNull());
...
void Dbdict::restartNextPass(Signal *signal) {
  c_restartRecord.m_pass++;
  c_restartRecord.activeTable = 0;
  ...
  if (c_restartRecord.m_pass <= c_restartRecord.m_end_pass) {
    ...
    if (c_restartRecord.m_tx_ptr_i == RNIL) {
      ...
      Callback c = {safe_cast(&Dbdict::restart_fromBeginTrans),
                    tx_ptr.p->tx_key};
      tx_ptr.p->m_callback = c;
      beginSchemaTrans(signal, tx_ptr);
      return;
...
void Dbdict::restart_fromWriteSchemaFile(Signal *signal, Uint32 senderData,
                                         Uint32 retCode) {
...
void Dbdict::execGET_TABINFOREF(Signal *signal) {
  jamEntry();
...
void Dbdict::execGET_TABINFO_CONF(Signal *signal) {
  jamEntry();
 ...
 */
void Dbdict::restartCreateObj(Signal *signal, Uint32 tableId,
...
void Dbdict::restartCreateObj_readConf(Signal *signal, Uint32 callbackData,
                                       Uint32 returnCode) {
  jam();
...
void Dbdict::restartCreateObj_parse(Signal *signal, SegmentedSectionPtr ptr,
                                    bool file) {
  jam();
  SchemaOpPtr op_ptr;
  ...
  if (unlikely(hasError(error))) {
    ...
    */
    ...
    BaseString::snprintf(msg, sizeof(msg),
                         ...
                         " error %u.",
                         c_restartRecord.activeTable, error.errorCode);
    progError(__LINE__, NDBD_EXIT_RESTORE_SCHEMA, msg);
  }
 ...
 */
void Dbdict::restartDropObj(Signal *signal, Uint32 tableId,
...
void Dbdict::handleApiFailureCallback(Signal *signal, Uint32 failedNodeId,
                                      Uint32 ignoredRc) {
  jamEntry();
...
void Dbdict::handleNdbdFailureCallback(Signal *signal, Uint32 failedNodeId,
                                       Uint32 ignoredRc) {
  jamEntry();
  ...
  sendSignal(DBDIH_REF, GSN_NF_COMPLETEREP, signal, NFCompleteRep::SignalLength,
             JBB);
}
...
void Dbdict::send_nf_complete_rep(Signal *signal, const NodeFailRep *nodeFail) {
  ...
  for (unsigned i = 1; i < MAX_NDB_NODES; i++) {
    ...
    if (tmp.get(i)) {
      ...
      Callback cb = {safe_cast(&Dbdict::handleNdbdFailureCallback), i};
...
void Dbdict::handle_master_takeover(Signal *signal) {
  ...
  */
  ...
  sendSignal(rg, GSN_DICT_TAKEOVER_REQ, signal, DictTakeoverReq::SignalLength,
             JBB);
}
...
bool Dbdict::get_object(DictObjectPtr &obj_ptr, const char *name, Uint32 len,
                        Uint32 hash) {
  DictObject key;
...
void Dbdict::release_object(Uint32 obj_ptr_i, DictObject *obj_ptr_p) {
  jam();
...
void Dbdict::increase_ref_count(Uint32 obj_ptr_i) {
  DictObject *ptr = c_obj_pool.getPtr(obj_ptr_i);
  ptr->m_ref_count++;
}

void Dbdict::decrease_ref_count(Uint32 obj_ptr_i) {
  DictObject *ptr = c_obj_pool.getPtr(obj_ptr_i);
...
void TabInfoLongData::IndirectReader(SimpleProperties::Reader &it, void *dest) {
...
void Dbdict::handleTabInfoInit(Signal *signal, SchemaTransPtr &trans_ptr,
  ...
  /* Create a scope over the LocalRope in TabInfoLongData */
  {
    ...
    status = SimpleProperties::unpack(
        it, &c_tableDesc, DictTabInfo::TableMapping,
        DictTabInfo::TableMappingSize, TabInfoLongData::IndirectReader,
        &tabInfoLongData);
  ...
  const Uint32 name_hash =
      LocalRope::hash(c_tableDesc.TableName, tableNameLength);
  ...
  if (parseP->requestType != DictTabInfo::AlterTableFromAPI) {
    ...
    if (g_trace) {
      ...
      {
        const char *pb_str = NULL;
        switch (c_tableDesc.PartitionBalance) {
      ...
      if (c_tableDesc.FullyReplicatedFlag) {
        const char *fr_str = " FR";
        strcpy(buf_ptr, fr_str);
        buf_ptr += strlen(fr_str);
      }
      ...
      g_eventLogger->info(
          "Dbdict: %u: create name=%s,id=%u,obj_ptr_i=%d%s frm=%d", __LINE__,
          c_tableDesc.TableName, schemaFileId, tablePtr.p->m_obj_ptr_i, buf,
          frmData.m_length);
    }
    send_event(signal, trans_ptr, NDB_LE_CreateSchemaObject, schemaFileId,
               tablePtr.p->tableVersion, c_tableDesc.TableType);
  } else {
    /**
     * When we perform an inplace ALTER TABLE (AlterTableFrom API) we will copy
     * all data to the new table object, but this is a bit misleading since this
     * new object will not replace the old table object, rather in
     * AlterTable_commit the changed parts of the new table object will be
     * copied over to the old table object.
     */
    jam();
  }
  ...
  tabRequire(c_tableDesc.TableLoggedFlag || c_tableDesc.TablespaceId == RNIL,
             CreateTableRef::NoLoggingDiskTable);
  ...
  tablePtr.p->m_bits |= (c_tableDesc.RowGCIFlag ? TableRecord::TR_RowGCI : 0);
  ...
  D("handleTabInfoInit: tableId = "
    ...
    << tablePtr.p->tableVersion << " m_bits = " << hex << tablePtr.p->m_bits);
  ...
  tablePtr.p->partitionBalance = c_tableDesc.PartitionBalance;
   ...
   * calculated basically as number of LDMs.
   */
  ...
  if (!tablePtr.p->defaultNoPartFlag) {
     ...
     */
    if (tablePtr.p->partitionBalance != NDB_PARTITION_BALANCE_FOR_RP_BY_LDM) {
      ndbrequire(tablePtr.p->partitionBalance ==
                 NDB_PARTITION_BALANCE_SPECIFIC);
    } else {
      /**
       * This will happen when an old table created before partitionBalance
  ...
  if (tablePtr.p->fragmentType == DictTabInfo::HashMapPartition &&
    ...
    if (tablePtr.p->fragmentCount == 0) {
       ...
       */
      assert(tablePtr.p->partitionBalance != NDB_PARTITION_BALANCE_SPECIFIC);
      tabRequire(tablePtr.p->partitionBalance != NDB_PARTITION_BALANCE_SPECIFIC,
                 CreateTableRef::NonDefaultPartitioningWithNoPartitions);
      tablePtr.p->fragmentCount =
          get_default_fragments(signal, tablePtr.p->partitionBalance, 0);
    }
    ...
    if (tablePtr.p->partitionCount == 0) {
      if ((tablePtr.p->m_bits & TableRecord::TR_FullyReplicated) == 0) {
        ...
        jam();
        tablePtr.p->partitionCount = get_default_partitions_fully_replicated(
            signal, tablePtr.p->partitionBalance);
      }
    }
    ...
    BaseString::snprintf(buf, sizeof(buf), "DEFAULT-HASHMAP-%u-%u", buckets,
                         partitions);
  ...
  memcpy(c_fragData, c_tableDesc.FragmentData, c_tableDesc.FragmentDataLen);
  ...
  if (c_tableDesc.PrimaryTableId != RNIL) {
    ...
    if (getNodeState().getSystemRestartInProgress()) {
      ...
      if (c_tableDesc.InsertTriggerId != RNIL ||
        ...
        infoEvent("table: %u UPGRADE saving (%u/%u/%u)", schemaFileId,
                  c_tableDesc.InsertTriggerId, c_tableDesc.UpdateTriggerId,
                  c_tableDesc.DeleteTriggerId);
        tablePtr.p->triggerId = c_tableDesc.InsertTriggerId;
        tablePtr.p->m_upgrade_trigger_handling.m_upgrade = true;
        tablePtr.p->m_upgrade_trigger_handling.insertTriggerId =
            c_tableDesc.InsertTriggerId;
        tablePtr.p->m_upgrade_trigger_handling.updateTriggerId =
            c_tableDesc.UpdateTriggerId;
        tablePtr.p->m_upgrade_trigger_handling.deleteTriggerId =
            c_tableDesc.DeleteTriggerId;

        upgrade_seizeTrigger(tablePtr, c_tableDesc.InsertTriggerId,
                             c_tableDesc.UpdateTriggerId,
                             c_tableDesc.DeleteTriggerId);
      }
    }
  ...
  if (parseP->errorCode == 0) {
    if ((tablePtr.p->m_bits & TableRecord::TR_FullyReplicated) != 0) {
      switch (tablePtr.p->partitionBalance) {
        ...
        case NDB_PARTITION_BALANCE_FOR_RA_BY_LDM_X_4: {
          ...
          D("Fully replicated table, partitionCount: "
            << tablePtr.p->partitionCount << " partitionBalance = "
            << getPartitionBalanceString(tablePtr.p->partitionBalance));
          break;
        }
        default: {
          jam();
          ...
          D("Creating fully replicated table failed. "
            ...
            << " Line: " << parseP->errorLine << " partitionCount: "
            << tablePtr.p->partitionCount << " partitionBalance = "
            << getPartitionBalanceString(tablePtr.p->partitionBalance));
          break;
        }
      ...
      jam();
      /**
       * Tables with explicit partitioning could still have fragmentCount set
       * to 0 here. So we will not check for fragmentCount != 0 until after
       * we've also received the fragmentation parts.
       *
       * Explicit partitioning can for the moment not use
  ...
  D("handleTabInfoInit: tableId = "
    << tablePtr.i << "fragmentCount = " << tablePtr.p->fragmentCount
...
void Dbdict::upgrade_seizeTrigger(TableRecordPtr tabPtr, Uint32 insertTriggerId,
  ...
  ndbrequire(c_obj_pool.getPtr(tab_obj_ptr, tabPtr.p->m_obj_ptr_i));
  ...
  if (updateTriggerId != RNIL) {
    ...
    if (!ok) {
      ...
      char buf[256];
      BaseString::snprintf(buf, sizeof(buf), "UPG_UPD_NDB$INDEX_%u_UI",
                           tab_obj_ptr.p->m_id);
      {
      ...
      ndbrequire(link_object(obj_ptr, triggerPtr));
      c_obj_name_hash.add(obj_ptr);
      c_obj_id_hash.add(obj_ptr);
    }
  }
  ...
  if (deleteTriggerId != RNIL) {
    jam();
    bool ok = find_object(
        triggerPtr, deleteTriggerId);  // TODO: msundell seizeTriggerRecord
    if (!ok) {
      jam();
      ...
      TriggerInfo::packTriggerInfo(triggerPtr.p->triggerInfo,
                                   g_hashIndexTriggerTmpl[0].triggerInfo);
      char buf[256];
      BaseString::snprintf(buf, sizeof(buf), "UPG_DEL_NDB$INDEX_%u_UI",
                           tab_obj_ptr.p->m_id);
...
void Dbdict::handleTabInfo(SimpleProperties::Reader &it,
  ...
  for (Uint32 i = 0; i < attrCount; i++) {
    ...
    attrDesc.init();
    status =
        SimpleProperties::unpack(it, &attrDesc, DictTabInfo::AttributeMapping,
    ...
    AttributeDescriptor::setDiskBased(
        desc, attrDesc.AttributeStorageType == NDB_STORAGETYPE_DISK);
      ...
      if (attrDesc.AttributeDefaultValueLen) {
        ...
        if (remainBytes) {
          ...
          NdbSqlUtil::convertByteOrder(
              ...
              (uchar *)defaultValueBuf + sizeof(Uint32), remainBytes);
        }
      }

      LocalRope defaultValue(c_rope_pool, attrPtr.p->defaultValue);
      defaultValue.assign(defaultValueBuf, attrDesc.AttributeDefaultValueLen);
    }
...
void Dbdict::block_substartstop(Signal *signal, SchemaOpPtr op_ptr) {
...
void Dbdict::unblock_substartstop() {
  ndbrequire(c_sub_startstop_lock.get(getOwnNodeId()));
  c_sub_startstop_lock.clear(getOwnNodeId());
}

void Dbdict::wait_substartstop(Signal *signal, Uint32 opkey) {
...
/* ---------------------------------------------------------------- */
void Dbdict::wait_gcp(Signal *signal, SchemaOpPtr op_ptr, Uint32 flags) {
...
const Dbdict::OpInfo Dbdict::CreateTableRec::g_opInfo = {
    {'C', 'T', 'a', 0},
...
bool Dbdict::createTable_seize(SchemaOpPtr op_ptr) {
  return seizeOpRec<CreateTableRec>(op_ptr);
  D("createTable_seize");
}

void Dbdict::createTable_release(SchemaOpPtr op_ptr) {
  CreateTableRecPtr createTabPtr;
  D("createTable_release");
...
void Dbdict::execCREATE_TABLE_REQ(Signal *signal) {
  jamEntry();
  if (!assembleFragments(signal)) {
    jam();
    return;
  ...
  const CreateTableReq req_copy = *(const CreateTableReq *)signal->getDataPtr();
...
Uint32 Dbdict::get_fragmentation(Signal *signal, Uint32 tableId) {
  ...
  req->requestInfo = CreateFragmentationReq::RI_GET_FRAGMENTATION;
  req->partitionBalance = 0; /* Ignored for get_fragmentation */
...
Uint32 Dbdict::create_fragmentation(Signal *signal, TableRecordPtr tabPtr,
                                    const Uint16 *src, Uint32 cnt,
                                    Uint32 flags) {
  ...
  frag_req->partitionBalance = tabPtr.p->partitionBalance;
...
void Dbdict::createTable_parse(Signal *signal, bool master, SchemaOpPtr op_ptr,
  ...
  D("createTable_parse: " << copyRope<MAX_TAB_NAME_SIZE>(tabPtr.p->tableName)
                          << V(tabPtr.p->tableVersion));
}

bool Dbdict::createTable_subOps(Signal *signal, SchemaOpPtr op_ptr) {
  ...
  if ((tabPtr.p->m_bits & TableRecord::TR_FullyReplicated) != 0) {
    jam();
    if (createTabPtr.p->m_fully_replicated_trigger == false) {
      jam();
      Callback c = {safe_cast(&Dbdict::createTable_fromCreateTrigger),
                    op_ptr.p->op_key};
      op_ptr.p->m_callback = c;

      createTabPtr.p->m_fully_replicated_trigger = true;
      createTable_toCreateTrigger(signal, op_ptr);
      return true;
    }
  }
...
void Dbdict::createTable_toCreateTrigger(Signal *signal, SchemaOpPtr op_ptr) {
  D("createTable_toCreateTrigger");
  ...
  const TriggerTmpl &triggerTmpl = g_fullyReplicatedTriggerTmp[0];

  CreateTrigReq *req = CAST_PTR(CreateTrigReq, signal->getDataPtrSend());
  Uint32 requestInfo = 0;
  if (op_ptr.p->m_restart) {
    jam();
    requestInfo |= DictSignal::RF_LOCAL_TRANS;
  }
  ...
  req->triggerNo = 0;
  Uint32 forceTriggerId = tabPtr.p->fullyReplicatedTriggerId;
...
void Dbdict::createTable_fromCreateTrigger(Signal *signal, Uint32 op_key,
                                           Uint32 ret) {
  jamEntry();
...
void Dbdict::createTable_reply(Signal *signal, SchemaOpPtr op_ptr,
                               ErrorInfo error) {
  jam();
...
void Dbdict::createTable_prepare(Signal *signal, SchemaOpPtr op_ptr) {
  jam();
  CreateTableRecPtr createTabPtr;
  getOpRec(op_ptr, createTabPtr);
  ...
  if (createTabPtr.p->m_modified_by_subOps) {
    jam();
    /**
     * The table object was modified by subop,
     *   i.e we need to regenerate DICT_TAB_INFO
     */
    SimplePropertiesSectionWriter w(*this);
    packTableIntoPages(w, tabPtr);
    ...
    SegmentedSectionPtr ss_ptr;
    w.getPtr(ss_ptr);
    bool ok = replaceOpSection(op_ptr, CreateTabReq::DICT_TAB_INFO, ss_ptr);
    releaseSection(ss_ptr.i);

    if (!ok) {
      jam();
      setError(op_ptr, SchemaTransBeginRef::OutOfSchemaTransMemory, __LINE__);
    }
  } else if (ZRESTART_NO_WRITE_AFTER_READ && op_ptr.p->m_restart == 1) {
    jam();
    /**
...
void Dbdict::createTab_writeTableConf(Signal *signal, Uint32 op_key,
                                      Uint32 ret) {
  jam();
  ...
  callback.m_callbackFunction = safe_cast(&Dbdict::createTab_localComplete);

  createTab_local(signal, op_ptr, fragSec, &callback);
}

void Dbdict::createTab_local(Signal *signal, SchemaOpPtr op_ptr,
                             OpSection afragSec, Callback *c) {
  jam();
  CreateTableRecPtr createTabPtr;
  getOpRec(op_ptr, createTabPtr);
...
void Dbdict::execCREATE_TAB_REF(Signal *signal) {
  jamEntry();

  D("execCREATE_TAB_REF");
...
void Dbdict::execCREATE_TAB_CONF(Signal *signal) {
  jamEntry();

  D("execCREATE_TAB_CONF");
...
void Dbdict::sendLQHADDATTRREQ(Signal *signal, SchemaOpPtr op_ptr,
  ...
  for (i = 0; i < LqhAddAttrReq::MAX_ATTRIBUTES && attributePtrI != RNIL; i++) {
    jam();
    AttributeRecordPtr attrPtr;
    ndbrequire(c_attributeRecordPool.getPtr(attrPtr, attributePtrI));
  ...
  if (defVal_length != 0) {
    ...
    sendSignal(DBLQH_REF, GSN_LQHADDATTREQ, signal,
               LqhAddAttrReq::HeaderLength + LqhAddAttrReq::EntryLength * i,
               JBB, ptr, 1);
  } else
    sendSignal(DBLQH_REF, GSN_LQHADDATTREQ, signal,
               LqhAddAttrReq::HeaderLength + LqhAddAttrReq::EntryLength * i,
...
void Dbdict::execLQHADDATTCONF(Signal *signal) {
  jamEntry();
...
void Dbdict::execLQHADDATTREF(Signal *signal) {
  jamEntry();
...
void Dbdict::createTab_dih(Signal *signal, SchemaOpPtr op_ptr) {
  CreateTableRecPtr createTabPtr;
  getOpRec(op_ptr, createTabPtr);
  SchemaTransPtr trans_ptr = op_ptr.p->m_trans_ptr;
  ...
  // fragmentation in long signal section
  {
    ...
    sendSignal(DBDIH_REF, GSN_DIADDTABREQ, signal, DiAddTabReq::SignalLength,
               JBB, ptr, noOfSections);
  }
}

static void calcLHbits(Uint32 *lhPageBits, Uint32 *lhDistrBits, Uint32 fid,
                       Uint32 totalFragments, Dbdict *dict) {
  Uint32 distrBits = 0;
  Uint32 pageBits = 0;
...
void Dbdict::execADD_FRAGREQ(Signal *signal) {
  jamEntry();
  ...
  Uint64 maxRows =
      tabPtr.p->maxRowsLow + (((Uint64)tabPtr.p->maxRowsHigh) << 32);
  Uint64 minRows =
      tabPtr.p->minRowsLow + (((Uint64)tabPtr.p->minRowsHigh) << 32);
...

  {
    ...
    sendSignal(DBLQH_REF, GSN_LQHFRAGREQ, signal, LqhFragReq::SignalLength,
               JBB);
  }
}

void Dbdict::execLQHFRAGCONF(Signal *signal) {
  jamEntry();
...

  {
    ...
    conf->dihPtr = dihPtr;
    conf->fragId = fragId;
    sendSignal(DBDIH_REF, GSN_ADD_FRAGCONF, signal, AddFragConf::SignalLength,
               JBB);
  }
}

void Dbdict::execLQHFRAGREF(Signal *signal) {
  jamEntry();
...

  {
    ...
    ref->dihPtr = dihPtr;
    ref->fragId = fragId;
    sendSignal(DBDIH_REF, GSN_ADD_FRAGREF, signal, AddFragRef::SignalLength,
               JBB);
  }
}

void Dbdict::execDIADDTABCONF(Signal *signal) {
  jam();
...
void Dbdict::execDIADDTABREF(Signal *signal) {
  jam();
...
void Dbdict::execTAB_COMMITCONF(Signal *signal) {
  jamEntry();
  ...
  if (refToBlock(signal->getSendersBlockRef()) == DBLQH) {
    ...
    req->userDefinedPartition =
        (tabPtr.p->fragmentType == DictTabInfo::UserDefined);
...
void Dbdict::execTAB_COMMITREF(Signal *signal) {
  jamEntry();
  ndbabort();
}  // execTAB_COMMITREF()

void Dbdict::createTab_localComplete(Signal *signal, Uint32 op_key,
                                     Uint32 ret) {
  jam();
...
void Dbdict::createTable_commit(Signal *signal, SchemaOpPtr op_ptr) {
  jam();
...
void Dbdict::createTab_activate(Signal *signal, SchemaOpPtr op_ptr,
                                Callback *c) {
  CreateTableRecPtr createTabPtr;
  getOpRec(op_ptr, createTabPtr);
  createTabPtr.p->m_callback = *c;
...
void Dbdict::execTC_SCHVERCONF(Signal *signal) {
  jamEntry();
  ...
  if (refToBlock(signal->getSendersBlockRef()) == DBSPJ) {
    ...
    sendSignal(DBTC_REF, GSN_TC_SCHVERREQ, signal, TcSchVerReq::SignalLength,
               JBB);
    return;
  }
...
void Dbdict::createTab_alterComplete(Signal *signal, Uint32 op_key,
                                     Uint32 ret) {
  jam();
...
void Dbdict::createTable_complete(Signal *signal, SchemaOpPtr op_ptr) {
  jam();
  sendTransConf(signal, op_ptr);
}

void Dbdict::createTable_abortParse(Signal *signal, SchemaOpPtr op_ptr) {
  CreateTableRecPtr createTabPtr;
  getOpRec(op_ptr, createTabPtr);
  const CreateTabReq *impl_req = &createTabPtr.p->m_request;
  const Uint32 tableId = impl_req->tableId;
...
void Dbdict::createTable_abortPrepare(Signal *signal, SchemaOpPtr op_ptr) {
  CreateTableRecPtr createTabPtr;
  getOpRec(op_ptr, createTabPtr);
  const CreateTabReq *impl_req = &createTabPtr.p->m_request;
  // const Uint32 tableId = impl_req->tableId;
  ...
  dropTabPtr.p->m_callback.m_callbackData = oplnk_ptr.p->op_key;
...
void Dbdict::createTable_abortLocalConf(Signal *signal, Uint32 oplnk_key,
                                        Uint32 ret) {
  jam();
...
void Dbdict::execCREATE_FRAGMENTATION_REF(Signal *signal) {
  // currently not received
  ndbabort();
}

void Dbdict::execCREATE_FRAGMENTATION_CONF(Signal *signal) {
  // currently not received
  ndbabort();
}

void Dbdict::execCREATE_TAB_REQ(Signal *signal) {
  // no longer received
  ndbabort();
}
...
void Dbdict::releaseTableObject(Uint32 table_ptr_i, bool removeFromHash) {
  TableRecordPtr tablePtr;
  ndbrequire(c_tableRecordPool_.getPtr(tablePtr, table_ptr_i));
  if (removeFromHash) {
    jam();
...
const Dbdict::OpInfo Dbdict::DropTableRec::g_opInfo = {
    {'D', 'T', 'a', 0},
...
bool Dbdict::dropTable_seize(SchemaOpPtr op_ptr) {
  return seizeOpRec<DropTableRec>(op_ptr);
}

void Dbdict::dropTable_release(SchemaOpPtr op_ptr) {
...
void Dbdict::execDROP_TABLE_REQ(Signal *signal) {
  ...
  const DropTableReq req_copy = *(const DropTableReq *)signal->getDataPtr();
...
void Dbdict::dropTable_parse(Signal *signal, bool master, SchemaOpPtr op_ptr,
                             SectionHandle &handle, ErrorInfo &error) {
...
bool Dbdict::dropTable_subOps(Signal *signal, SchemaOpPtr op_ptr) {
  ...
  if (dropTabPtr.p->m_fully_replicated_trigger == false) {
    jam();
    dropTabPtr.p->m_fully_replicated_trigger = true;
    if (((tabPtr.p->m_bits & TableRecord::TR_FullyReplicated) != 0) &&
        tabPtr.p->fullyReplicatedTriggerId != RNIL) {
      jam();
      Callback c = {safe_cast(&Dbdict::dropTable_fromDropTrigger),
                    op_ptr.p->op_key};
      op_ptr.p->m_callback = c;

      dropTable_toDropTrigger(signal, op_ptr);
      return true;
    }
  }
...
void Dbdict::dropTable_toDropTrigger(Signal *signal, SchemaOpPtr op_ptr) {
  D("dropTable_toDropTrigger");

  SchemaTransPtr trans_ptr = op_ptr.p->m_trans_ptr;
  DropTableRecPtr dropTablePtr;
  getOpRec(op_ptr, dropTablePtr);
  const DropTabReq *impl_req = &dropTablePtr.p->m_request;
  ...
  req->indexVersion = RNIL;
  req->triggerNo = 0;
  req->triggerId = tablePtr.p->fullyReplicatedTriggerId;

  sendSignal(reference(), GSN_DROP_TRIG_REQ, signal, DropTrigReq::SignalLength,
             JBB);
}

void Dbdict::dropTable_fromDropTrigger(Signal *signal, Uint32 op_key,
                                       Uint32 ret) {
  jam();
...
void Dbdict::dropTable_reply(Signal *signal, SchemaOpPtr op_ptr,
                             ErrorInfo error) {
  jam();
...
void Dbdict::dropTable_prepare(Signal *signal, SchemaOpPtr op_ptr) {
  DropTableRecPtr dropTabPtr;
  getOpRec(op_ptr, dropTabPtr);
...
void Dbdict::dropTable_backup_mutex_locked(Signal *signal, Uint32 op_key,
                                           Uint32 ret) {
  jamEntry();
...
void Dbdict::dropTable_commit(Signal *signal, SchemaOpPtr op_ptr) {
  SchemaTransPtr trans_ptr = op_ptr.p->m_trans_ptr;
...
void Dbdict::dropTable_commit_nextStep(Signal *signal, SchemaOpPtr op_ptr) {
  DropTableRecPtr dropTabPtr;
  getOpRec(op_ptr, dropTabPtr);
  const DropTabReq *impl_req = &dropTabPtr.p->m_request;
  ...
  if (ERROR_INSERTED(6131) && blockNo == DBDIH) {
    jam();
    CLEAR_ERROR_INSERT_VALUE;
  ...
  sendSignal(ref, GSN_PREP_DROP_TAB_REQ, signal, PrepDropTabReq::SignalLength,
             JBB);
}

void Dbdict::execPREP_DROP_TAB_REQ(Signal *signal) {
  // no longer received
  ndbabort();
}

void Dbdict::execPREP_DROP_TAB_CONF(Signal *signal) {
  jamEntry();
  D("execPREP_DROP_TAB_CONF");
...
void Dbdict::execPREP_DROP_TAB_REF(Signal *signal) {
  jamEntry();
  D("execPREP_DROP_TAB_REF");
...
void Dbdict::dropTable_commit_fromLocal(Signal *signal, Uint32 op_key,
                                        Uint32 errorCode) {
  SchemaOpPtr op_ptr;
  DropTableRecPtr dropTabPtr;
  ndbrequire(findSchemaOp(op_ptr, dropTabPtr, op_key));
...
void Dbdict::dropTable_commit_done(Signal *signal, SchemaOpPtr op_ptr) {
  jam();
...
void Dbdict::dropTable_complete(Signal *signal, SchemaOpPtr op_ptr) {
  jam();
...
void Dbdict::dropTable_complete_nextStep(Signal *signal, SchemaOpPtr op_ptr) {
  DropTableRecPtr dropTabPtr;
  getOpRec(op_ptr, dropTabPtr);
  const DropTabReq *impl_req = &dropTabPtr.p->m_request;
...
void Dbdict::execDROP_TAB_CONF(Signal *signal) {
  jamEntry();
  D("execDROP_TAB_CONF");
...
void Dbdict::execDROP_TAB_REF(Signal *signal) {
  jamEntry();
  D("execDROP_TAB_REF");
...
void Dbdict::dropTable_complete_fromLocal(Signal *signal, Uint32 op_key) {
  jamEntry();
...
void Dbdict::dropTable_complete_done(Signal *signal, Uint32 op_key,
                                     Uint32 ret) {
  jam();
  ...
  // inform SUMA
  {
    ...
    sendSignal(SUMA_REF, GSN_DROP_TAB_CONF, signal, DropTabConf::SignalLength,
               JBB);
  }
...
void Dbdict::dropTable_abortParse(Signal *signal, SchemaOpPtr op_ptr) {
  D("dropTable_abortParse" << *op_ptr.p);

  sendTransConf(signal, op_ptr);
}

void Dbdict::dropTable_abortPrepare(Signal *signal, SchemaOpPtr op_ptr) {
...
const Dbdict::OpInfo Dbdict::AlterTableRec::g_opInfo = {
    {'A', 'T', 'a', 0},
...
bool Dbdict::alterTable_seize(SchemaOpPtr op_ptr) {
  return seizeOpRec<AlterTableRec>(op_ptr);
}

void Dbdict::alterTable_release(SchemaOpPtr op_ptr) {
  AlterTableRecPtr alterTabPtr;
...
bool Dbdict::check_ndb_versions() const {
  Uint32 node = 0;
...
int Dbdict::check_sender_version(const Signal *signal, Uint32 version) const {
...
void Dbdict::execALTER_TABLE_REQ(Signal *signal) {
  jamEntry();
  ...
  const AlterTableReq req_copy = *(const AlterTableReq *)signal->getDataPtr();
...
void Dbdict::alterTable_parse(Signal *signal, bool master, SchemaOpPtr op_ptr,
                              SectionHandle &handle, ErrorInfo &error) {
  D("alterTable_parse");
  ...
  TableRecordPtr newTablePtr;
  {
    ...
    newTablePtr.p->tableId =
        impl_req->tableId;  // set correct table id...(not the temporary)
  ...
  impl_req->newTableVersion = newTablePtr.p->tableVersion =
  ...
  // changed attribute stuff
  {
    if (newTablePtr.p->noOfAttributes == tablePtr.p->noOfAttributes) {
      ...
      LocalAttributeRecord_list newlist(c_attributeRecordPool,
  ...
  // add attribute stuff
  {
    ...
    list.first(attrPtr);
    Uint32 i = 0;
    LocalArenaPool<OpSectionSegment> op_sec_pool(trans_ptr.p->m_arena,
                                                 c_opSectionBufferPool);
    for (i = 0; i < newTablePtr.p->noOfAttributes; i++) {
      if (i >= tablePtr.p->noOfAttributes) {
        ...
        if (!copyIn(op_sec_pool, alterTabPtr.p->m_newAttrData, attrData, 2)) {
          jam();
          setError(error, SchemaTransBeginRef::OutOfSchemaTransMemory,
                   __LINE__);
          return;
        }
      }
  ...
  if (tablePtr.p->partitionBalance != newTablePtr.p->partitionBalance &&
      ((newTablePtr.p->m_bits & TableRecord::TR_FullyReplicated) != 0)) {
    jam();
    /**
     * We don't supporting changing the number of real fragments for fully
     * replicated tables.
     *
     * The issue is that when we created the table we created a number of
     * "real" fragments, e.g. 0-3, we also created a number of copy
     * fragments, e.g. 4-7. So when we want to increase the number of
     * real fragments to e.g. 8 we cannot use fragment id 4-7 for those.
     * Rather we will have to use 8-11 and 12-15 for new copy fragments.
     *
     * This is not a huge issue, but we need to maintain a real fragment
     * map so that we can find the fragment id given the "real"
     * fragment id calculated by using the hash value and the hash map.
     *
     * This fragment map has to be persistent as well and we have to handle
     * it properly, so there is some amount of work needed that we push
  ...
  if (AlterTableReq::getAddFragFlag(impl_req->changeMask)) {
    ...
    if (master) {
      ...
      err = create_fragmentation(signal, newTablePtr, c_fragData,
                                 c_fragDataLen / 2, flags);
  ...
  if (tablePtr.p->hashMapObjectId != newTablePtr.p->hashMapObjectId &&
      !AlterTableReq::getReorgFragFlag(
          impl_req->changeMask)) {  // Change in hashmap without reorg is not
                                    // supported
    jam();
    setError(error, AlterTableRef::UnsupportedChange, __LINE__);
    return;
  ...
  D("alterTable_parse " << V(newTablePtr.i) << hex
                        << V(newTablePtr.p->tableVersion));
...
Uint32 Dbdict::check_supported_reorg(Uint32 org_map_id, Uint32 new_map_id) {
  ...
  ndbrequire(g_hash_map.getPtr(orgptr, orgmap_ptr.p->m_map_ptr_i));

  Ptr<Hash2FragmentMap> newptr;
  ndbrequire(g_hash_map.getPtr(newptr, newmap_ptr.p->m_map_ptr_i));
  ...
  for (Uint32 i = 0; i < period; i++) {
    if (orgptr.p->m_map[i % orgptr.p->m_cnt] !=
            newptr.p->m_map[i % newptr.p->m_cnt] &&
...
Uint32 Dbdict::check_supported_add_fragment(Uint16 *newdata,
  ...
  memmove(newdata + 2, newdata + 2 + (1 + replicas) * oldFragments,
...
bool Dbdict::alterTable_subOps(Signal *signal, SchemaOpPtr op_ptr) {
  ...
  if (AlterTableReq::getReorgFragFlag(impl_req->changeMask)) {
    ...
    if (alterTabPtr.p->m_sub_reorg_trigger == false) {
      jam();
      Callback c = {safe_cast(&Dbdict::alterTable_fromCreateTrigger),
                    op_ptr.p->op_key};
      TableRecordPtr tabPtr;
      bool ok = find_object(tabPtr, impl_req->tableId);
      ndbrequire(ok);
      op_ptr.p->m_callback = c;
      alterTabPtr.p->m_sub_reorg_trigger = true;

      if ((tabPtr.p->m_bits & TableRecord::TR_FullyReplicated) == 0) {
        /**
         * Normal tables without full replication will install a
         * reorg trigger. When creating a reorg trigger it is defined
         * to be dropped as part of the schema transaction.
         *
         * Fully replicated tables need no reorg trigger since data is
         * only replicated to a new set of fragments on new node groups.
         * In some future version when we support adding new main
         * fragments to fully replicated we need a reorg trigger again
         * since this actually performs a reorg of the table.
         */
        jam();
    ...
    if (alterTabPtr.p->m_sub_copy_data == false) {
      jam();
      Callback c = {safe_cast(&Dbdict::alterTable_fromCopyData),
                    op_ptr.p->op_key};
      op_ptr.p->m_callback = c;
...
void Dbdict::alterTable_toAlterOrderedIndex(Signal *signal,
                                            SchemaOpPtr op_ptr) {
  jam();
  AlterTableRecPtr alterTabPtr;
  getOpRec(op_ptr, alterTabPtr);
  ...
  TableRecordPtr indexPtr;
  ndbrequire(c_tableRecordPool_.getPtr(
      indexPtr, alterTabPtr.p->m_sub_add_frag_index_ptr));
...
void Dbdict::alterTable_toReadBackup(Signal *signal, SchemaOpPtr op_ptr,
                                     TableRecordPtr indexPtr,
                                     TableRecordPtr tablePtr) {
  jam();
  SchemaTransPtr trans_ptr = op_ptr.p->m_trans_ptr;
...
void Dbdict::alterTable_toAlterUniqueIndex(Signal *signal, SchemaOpPtr op_ptr,
                                           TableRecordPtr indexPtr,
                                           TableRecordPtr tablePtr) {
  jam();
  /**
  ...
  D("fragCount: " << tablePtr.p->fragmentCount << " partCount: "
                  << tablePtr.p->partitionCount << " partBal: "
                  << getPartitionBalanceString(tablePtr.p->partitionBalance)
...
void Dbdict::alterTable_fromAlterIndex(Signal *signal, Uint32 op_key,
                                       Uint32 ret) {
  jamEntry();
  D("alterTable_fromAlterIndex");
...
void Dbdict::alterTable_toReorgTable(Signal *signal, SchemaOpPtr op_ptr,
                                     Uint32 step) {
  jam();
  AlterTableRecPtr alterTabPtr;
  getOpRec(op_ptr, alterTabPtr);
  ...
  if (step == 0) {
    ...
    D("alterTable_toReorgTable(0)");
  } else if (step == 1) {
    jam();
    AlterTableReq::setReorgCompleteFlag(req->changeMask, 1);
    D("alterTable_toReorgTable(1)");
  } else {
    jamLine(step);
    ndbabort();
  }
...
void Dbdict::alterTable_fromReorgTable(Signal *signal, Uint32 op_key,
                                       Uint32 ret) {
  jamEntry();
...
void Dbdict::alterTable_toCreateTrigger(Signal *signal, SchemaOpPtr op_ptr) {
  jam();
...
void Dbdict::alterTable_fromCreateTrigger(Signal *signal, Uint32 op_key,
                                          Uint32 ret) {
  D("alterTable_fromCreateReorgTrigger");
  alterTable_fromAlterIndex(signal, op_key, ret);
}

void Dbdict::alterTable_toCopyData(Signal *signal, SchemaOpPtr op_ptr) {
  jam();

  D("alterTable_toCopyData");
  SchemaTransPtr trans_ptr = op_ptr.p->m_trans_ptr;
  AlterTableRecPtr alterTablePtr;
  getOpRec(op_ptr, alterTablePtr);
  const AlterTabReq *impl_req = &alterTablePtr.p->m_request;
  ...
  D("COPY_DATA_REQ: srcTableId: " << req->srcTableId
                                  ...
                                  << " fragmentCount: " << req->srcFragments);

  sendSignal(reference(), GSN_COPY_DATA_REQ, signal, CopyDataReq::SignalLength,
             JBB);
}

void Dbdict::alterTable_fromCopyData(Signal *signal, Uint32 op_key,
                                     Uint32 ret) {
  D("alterTable_fromCopyData");
  alterTable_fromAlterIndex(signal, op_key, ret);
}

void Dbdict::alterTable_reply(Signal *signal, SchemaOpPtr op_ptr,
                              ErrorInfo error) {
  jam();
...
void Dbdict::alterTable_toSumaSync(Signal *signal, SchemaOpPtr op_ptr,
                                   Uint32 step) {
  jam();
  AlterTableRecPtr alterTabPtr;
  getOpRec(op_ptr, alterTabPtr);
  ...
  if (step == 0) {
    ...
    D("alterTable_toSumaSync(Enable)");
  } else if (step == 1) {
    AlterTableReq::setReorgSumaFilterFlag(req->changeMask, 1);
    D("alterTable_toSumaSync(Filter)");
  } else {
    jamLine(step);
    ndbabort();
  }
...
void Dbdict::alterTable_prepare(Signal *signal, SchemaOpPtr op_ptr) {
  jam();
  SchemaTransPtr trans_ptr = op_ptr.p->m_trans_ptr;
...
void Dbdict::alterTable_backup_mutex_locked(Signal *signal, Uint32 op_key,
                                            Uint32 ret) {
  jamEntry();
   ...
   */
  Callback callback = {safe_cast(&Dbdict::alterTab_writeTableConf),
                       callback.m_callbackData = op_ptr.p->op_key};
...
void Dbdict::alterTab_writeTableConf(Signal *signal, Uint32 op_key,
                                     Uint32 ret) {
  jam();
  ndbrequire(ret == 0);
  D("alterTab_writeTableConf");
...
void Dbdict::alterTable_toLocal(Signal *signal, SchemaOpPtr op_ptr) {
  AlterTableRecPtr alterTabPtr;
  getOpRec(op_ptr, alterTabPtr);
  const AlterTabReq *impl_req = &alterTabPtr.p->m_request;
  ...
  if (ERROR_INSERTED(6131) && blockIndex + 1 == AlterTableRec::BlockCount) {
    jam();
    CLEAR_ERROR_INSERT_VALUE;
  ...
  if (blockNo == DBLQH && req->noOfNewAttr > 0) {
    ...
    ndbrequire(2 * impl_req->noOfNewAttr ==
               alterTabPtr.p->m_newAttrData.getSize());
    LocalArenaPool<OpSectionSegment> op_sec_pool(
        op_ptr.p->m_trans_ptr.p->m_arena, c_opSectionBufferPool);
    bool ok = copyOut(op_sec_pool, alterTabPtr.p->m_newAttrData, newAttrData,
                      2 * impl_req->noOfNewAttr);
    ndbrequire(ok);
...
void Dbdict::alterTable_fromLocal(Signal *signal, Uint32 op_key, Uint32 ret) {
...
void Dbdict::alterTable_commit(Signal *signal, SchemaOpPtr op_ptr) {
  AlterTableRecPtr alterTabPtr;
  getOpRec(op_ptr, alterTabPtr);
  const AlterTabReq *impl_req = &alterTabPtr.p->m_request;
  ...
  if (op_ptr.p->m_sections) {
    ...
    if (AlterTableReq::getNameFlag(changeMask)) {
      ...
      DictObjectPtr obj_ptr;
      ndbrequire(c_obj_pool.getPtr(obj_ptr, tablePtr.p->m_obj_ptr_i));
    ...
    if (AlterTableReq::getAddAttrFlag(changeMask)) {
      ...
      D("alterTable_commit: getAddAttrFlag set");
    ...
    if (AlterTableReq::getAddFragFlag(changeMask)) {
      jam();
      D("alterTable_commit: getAddFragFlag set");
  ...
  if (AlterTableReq::getReorgFragFlag(impl_req->changeMask)) {
    ...
    jam();
    D("alterTable_commit: getReorgCommitFlag set");
    /**
...
void Dbdict::alterTable_toCommitComplete(Signal *signal, SchemaOpPtr op_ptr,
                                         Uint32 type) {
  ...
  Callback c = {safe_cast(&Dbdict::alterTable_fromCommitComplete),
                op_ptr.p->op_key};
  op_ptr.p->m_callback = c;

  sendSignal(numberToRef(blockNo, getOwnNodeId()), GSN_ALTER_TAB_REQ, signal,
             AlterTabReq::SignalLength, JBB);
}

void Dbdict::alterTable_fromCommitComplete(Signal *signal, Uint32 op_key,
                                           Uint32 ret) {
  ...
  // older way to notify  wl3600_todo disable to find SUMA problems
  {
    ...
    {
      const RopeHandle &rh = AlterTableReq::getNameFlag(impl_req->changeMask)
    ...
    sendSignal(QMGR_REF, GSN_API_BROADCAST_REP, signal,
               ApiBroadcastRep::SignalLength + AlterTableRep::SignalLength, JBB,
               ptr, 1);
  }
...
void Dbdict::alterTable_complete(Signal *signal, SchemaOpPtr op_ptr) {
  jam();
  ...
  if (AlterTableReq::getReorgCommitFlag(impl_req->changeMask)) {
    jam();
    D("alterTable_complete getReorgCommitFlag set");
    ...
    D("alterTable_complete getReorgCompleteFlag set");
    /**
    ...
    jam();
    D("alterTable_complete getReorgSumaEnableFlag set");
    alterTabPtr.p->m_blockNo[0] = DBLQH;
    alterTable_toCommitComplete(signal, op_ptr,
    ...
    jam();
    D("alterTable_complete getReorgSumaFilterFlag set");
    alterTabPtr.p->m_blockNo[0] = DBLQH;
    alterTable_toCommitComplete(signal, op_ptr,
...
void Dbdict::alterTable_abortParse(Signal *signal, SchemaOpPtr op_ptr) {
...
void Dbdict::alterTable_abortPrepare(Signal *signal, SchemaOpPtr op_ptr) {
...
void Dbdict::alterTable_abortToLocal(Signal *signal, SchemaOpPtr op_ptr) {
  AlterTableRecPtr alterTabPtr;
  getOpRec(op_ptr, alterTabPtr);
  const AlterTabReq *impl_req = &alterTabPtr.p->m_request;
...
void Dbdict::alterTable_abortFromLocal(Signal *signal, Uint32 op_key,
                                       Uint32 ret) {
  SchemaOpPtr op_ptr;
  AlterTableRecPtr alterTabPtr;
  ndbrequire(findSchemaOp(op_ptr, alterTabPtr, op_key));
...
void Dbdict::execALTER_TAB_CONF(Signal *signal) {
  jamEntry();
  D("execALTER_TAB_CONF");
  const AlterTabConf *conf = (const AlterTabConf *)signal->getDataPtr();
...
void Dbdict::execALTER_TABLE_CONF(Signal *signal) {
  jamEntry();
  D("execALTER_TABLE_CONF");
  const AlterTableConf *conf = (const AlterTableConf *)signal->getDataPtr();
  ndbrequire(refToNode(conf->senderRef) == getOwnNodeId());
  handleDictConf(signal, conf);
}

void Dbdict::execALTER_TABLE_REF(Signal *signal) {
  jamEntry();
  D("execALTER_TABLE_REF");
  const AlterTableRef *ref = (const AlterTableRef *)signal->getDataPtr();
  ndbrequire(refToNode(ref->senderRef) == getOwnNodeId());
  handleDictRef(signal, ref);
}
...
void Dbdict::execGET_TABINFOREQ(Signal *signal) {
  ...
  if (c_retrieveRecord.busyState) {
     ...
     */
    if (!c_gettabinforeq_q.tryEnqReq(internalReq, signal)) {
      jam();
      /**
    ...
    if (!c_retrieveRecord.monitorRunning) {
      if (NdbTick_IsValid(c_retrieveRecord.startProcessTime) &&
          NdbTick_Elapsed(c_retrieveRecord.startProcessTime,
                          NdbTick_getCurrentTicks())
                  .milliSec() > SuspectGETTABINFOREQMillis) {
        jam();
...
void Dbdict::doGET_TABINFOREQ(Signal *signal) {
  jam();
  GetTabInfoReq *const req = (GetTabInfoReq *)&signal->theData[0];
  SectionHandle handle(this, signal);
...
void Dbdict::sendGetTabResponse(Signal *signal) {
  ...
  if (c_retrieveRecord.m_useLongSig) {
    ...
    sendFragmentedSignal(c_retrieveRecord.blockRef, GSN_GET_TABINFO_CONF,
                         signal, GetTabInfoConf::SignalLength, JBB, ptr, 1, c);
    return;
  }
...
void Dbdict::sendGET_TABINFOREF(Signal *signal, GetTabInfoReq *req,
  ...
  sendSignal(retRef, GSN_GET_TABINFOREF, signal, GetTabInfoRef::SignalLength,
             JBB);
}

void Dbdict::execLIST_TABLES_REQ(Signal *signal) {
  jamEntry();
...
bool Dbdict::buildListTablesData(const DictObject &dictObject,
                                 Uint32 parentTableId, ListTablesData &ltd,
  ...
  if (DictTabInfo::isTable(type) || DictTabInfo::isIndex(type)) {
    jam();
    TableRecordPtr tablePtr;
    ndbrequire(c_tableRecordPool_.getPtr(tablePtr, dictObject.m_object_ptr_i));
  ...
  if (DictTabInfo::isTrigger(type)) {
    ...
    if (parentTableId != RNIL && parentTableId != triggerPtr.p->tableId) {
      jam();
      return false;
    }
    ...
    switch (TriggerInfo::getTriggerType(triggerPtr.p->triggerInfo)) {
      ...
      default: {
        ...
        const Uint32 parentId =
            (triggerPtr.p->indexId == RNIL ? triggerPtr.p->tableId
  ...
  if (DictTabInfo::isFilegroup(type)) {
    ...
    if (parentTableId != RNIL) return false;
  ...
  if (DictTabInfo::isForeignKey(type)) {
    ...
    if (parentTableId != RNIL && parentTableId != fk_ptr.p->m_parentTableId &&
...
void Dbdict::sendLIST_TABLES_CONF(Signal *signal, ListTablesReq *req) {
  ...
  if (done) {
    ...
    conf->senderData = senderData;
    conf->noOfTables = 0;
    sendSignal(rg, GSN_LIST_TABLES_CONF, signal, ListTablesConf::SignalLength,
               JBB);
    return;
  }
  ...
  while (true) {
    ...
    if ((reqTableType != (Uint32)0) && (reqTableType != type)) goto flush;
    ...
    if (reqListDependent || reqListIndexes) {
      ...
      if (!buildListTablesData(*iter.curr.p, reqTableId, ltd, objectVersion,
      ...
      const bool found =
          buildListTablesData(*iter.curr.p, RNIL, ltd, objectVersion,
                              parentObjectType, parentObjectId);
      (void)found;
      ndbassert(found);
    }
  ...
  flush:
    ...
    if ((tableDataWords + tableNameWords) > fragSize || done) {
      ...
      sendSignal(rg, GSN_LIST_TABLES_CONF, signal, sigLen, JBB, &handle);
...
const Dbdict::OpInfo Dbdict::CreateIndexRec::g_opInfo = {
    {'C', 'I', 'n', 0},
...
bool Dbdict::createIndex_seize(SchemaOpPtr op_ptr) {
  return seizeOpRec<CreateIndexRec>(op_ptr);
}

void Dbdict::createIndex_release(SchemaOpPtr op_ptr) {
  releaseOpRec<CreateIndexRec>(op_ptr);
}

void Dbdict::execCREATE_INDX_REQ(Signal *signal) {
  jamEntry();
  if (!assembleFragments(signal)) {
    jam();
    return;
  ...
  const CreateIndxReq req_copy = *(const CreateIndxReq *)signal->getDataPtr();
...
void Dbdict::createIndex_parse(Signal *signal, bool master, SchemaOpPtr op_ptr,
                               SectionHandle &handle, ErrorInfo &error) {
  D("createIndex_parse");
  ...
  IndexAttributeList &attrList = createIndexPtr.p->m_attrList;
  {
    ...
    if (!r.getWord(&attrList.sz) || r.getSize() != 1 + attrList.sz ||
        attrList.sz == 0 || attrList.sz > MAX_ATTRIBUTES_IN_INDEX ||
  ...
  bits = 0;
  {
    ...
    tableDesc.init();
    SimpleProperties::UnpackStatus status =
        SimpleProperties::unpack(r, &tableDesc, DictTabInfo::TableMapping,
                                 DictTabInfo::TableMappingSize);
    if (status != SimpleProperties::Eof || tableDesc.TableName[0] == 0) {
      ndbassert(false);
  ...
  // check that the temporary status of index is compatible with table
  {
    ...
    if ((bits & TableRecord::TR_Temporary) && (bits & TableRecord::TR_Logged)) {
      jam();
...
bool Dbdict::createIndex_subOps(Signal *signal, SchemaOpPtr op_ptr) {
  ...
  if (!createIndexPtr.p->m_sub_create_table) {
    jam();
    Callback c = {safe_cast(&Dbdict::createIndex_fromCreateTable),
                  op_ptr.p->op_key};
    op_ptr.p->m_callback = c;
  ...
  if (!createIndexPtr.p->m_sub_alter_index) {
    jam();
    Callback c = {safe_cast(&Dbdict::createIndex_fromAlterIndex),
                  op_ptr.p->op_key};
    op_ptr.p->m_callback = c;
...
void Dbdict::createIndex_toCreateTable(Signal *signal, SchemaOpPtr op_ptr) {
  ...
  // Also inherit partition balance of main table to ensure that it
  ...
  w.add(DictTabInfo::NoOfKeyAttr,
        createIndexPtr.p->m_attrList.sz);  // XXX ordered??
  ...
  for (k = 0; k < createIndexPtr.p->m_attrList.sz; k++) {
    ...
    AttributeRecordPtr attrPtr;
    ndbrequire(c_attributeRecordPool.getPtr(attrPtr, attrMap[k].attr_ptr_i));
...
void Dbdict::createIndex_fromCreateTable(Signal *signal, Uint32 op_key,
                                         Uint32 ret) {
  jam();
...
void Dbdict::createIndex_toAlterIndex(Signal *signal, SchemaOpPtr op_ptr) {
...
void Dbdict::createIndex_fromAlterIndex(Signal *signal, Uint32 op_key,
                                        Uint32 ret) {
  jam();
...
void Dbdict::createIndex_reply(Signal *signal, SchemaOpPtr op_ptr,
                               ErrorInfo error) {
  jam();
  D("createIndex_reply");
...
void Dbdict::createIndex_prepare(Signal *signal, SchemaOpPtr op_ptr) {
  jam();
...
void Dbdict::createIndex_commit(Signal *signal, SchemaOpPtr op_ptr) {
  jam();
...
void Dbdict::createIndex_complete(Signal *signal, SchemaOpPtr op_ptr) {
  jam();
  sendTransConf(signal, op_ptr);
}
...
void Dbdict::createIndex_abortParse(Signal *signal, SchemaOpPtr op_ptr) {
...
void Dbdict::createIndex_abortPrepare(Signal *signal, SchemaOpPtr op_ptr) {
...
void Dbdict::execCREATE_INDX_IMPL_CONF(Signal *signal) {
  jamEntry();
  const CreateIndxImplConf *conf =
      (const CreateIndxImplConf *)signal->getDataPtr();
  ndbrequire(refToNode(conf->senderRef) == getOwnNodeId());
  D("execCREATE_INDX_IMPL_CONF");
  handleDictConf(signal, conf);
}

void Dbdict::execCREATE_INDX_IMPL_REF(Signal *signal) {
  jamEntry();
  const CreateIndxImplRef *ref =
      (const CreateIndxImplRef *)signal->getDataPtr();
  ndbrequire(refToNode(ref->senderRef) == getOwnNodeId());
  D("execCREATE_INDX_IMPL_REF");
  handleDictRef(signal, ref);
}
...
const Dbdict::OpInfo Dbdict::DropIndexRec::g_opInfo = {
    {'D', 'I', 'n', 0},
...
bool Dbdict::dropIndex_seize(SchemaOpPtr op_ptr) {
  return seizeOpRec<DropIndexRec>(op_ptr);
}

void Dbdict::dropIndex_release(SchemaOpPtr op_ptr) {
  releaseOpRec<DropIndexRec>(op_ptr);
}

void Dbdict::execDROP_INDX_REQ(Signal *signal) {
  jamEntry();
  if (!assembleFragments(signal)) {
    jam();
    return;
  ...
  const DropIndxReq req_copy = *(const DropIndxReq *)signal->getDataPtr();
...
void Dbdict::dropIndex_parse(Signal *signal, bool master, SchemaOpPtr op_ptr,
                             SectionHandle &handle, ErrorInfo &error) {
...
bool Dbdict::dropIndex_subOps(Signal *signal, SchemaOpPtr op_ptr) {
  ...
  if (!dropIndexPtr.p->m_sub_alter_index) {
    jam();
    Callback c = {safe_cast(&Dbdict::dropIndex_fromAlterIndex),
                  op_ptr.p->op_key};
    op_ptr.p->m_callback = c;
  ...
  if (!dropIndexPtr.p->m_sub_drop_table) {
    jam();
    Callback c = {safe_cast(&Dbdict::dropIndex_fromDropTable),
                  op_ptr.p->op_key};
    op_ptr.p->m_callback = c;
...
void Dbdict::dropIndex_reply(Signal *signal, SchemaOpPtr op_ptr,
                             ErrorInfo error) {
  jam();
  ...
  if (!hasError(error)) {
    ...
    Uint32 clientRef = op_ptr.p->m_clientRef;
    sendSignal(clientRef, GSN_DROP_INDX_REF, signal, DropIndxRef::SignalLength,
               JBB);
  }
}
...
void Dbdict::dropIndex_prepare(Signal *signal, SchemaOpPtr op_ptr) {
  DropIndexRecPtr dropIndexPtr;
  getOpRec(op_ptr, dropIndexPtr);
...
void Dbdict::dropIndex_commit(Signal *signal, SchemaOpPtr op_ptr) {
  DropIndexRecPtr dropIndexPtr;
  getOpRec(op_ptr, dropIndexPtr);
...
void Dbdict::dropIndex_complete(Signal *signal, SchemaOpPtr op_ptr) {
  jam();
  sendTransConf(signal, op_ptr);
}
...
void Dbdict::dropIndex_abortParse(Signal *signal, SchemaOpPtr op_ptr) {
  D("dropIndex_abortParse" << *op_ptr.p);
  // wl3600_todo
  sendTransConf(signal, op_ptr);
}

void Dbdict::dropIndex_abortPrepare(Signal *signal, SchemaOpPtr op_ptr) {
  D("dropIndex_abortPrepare" << *op_ptr.p);
  ndbabort();
  sendTransConf(signal, op_ptr);
}

void Dbdict::dropIndex_toAlterIndex(Signal *signal, SchemaOpPtr op_ptr) {
...
void Dbdict::dropIndex_fromAlterIndex(Signal *signal, Uint32 op_key,
                                      Uint32 ret) {
  jam();
...
void Dbdict::dropIndex_toDropTable(Signal *signal, SchemaOpPtr op_ptr) {
...
void Dbdict::dropIndex_fromDropTable(Signal *signal, Uint32 op_key,
                                     Uint32 ret) {
  jam();
...
void Dbdict::execDROP_INDX_IMPL_CONF(Signal *signal) {
  jamEntry();
  const DropIndxImplConf *conf = (const DropIndxImplConf *)signal->getDataPtr();
  ndbrequire(refToNode(conf->senderRef) == getOwnNodeId());
  D("execDROP_INDX_IMPL_CONF");
  handleDictConf(signal, conf);
}

void Dbdict::execDROP_INDX_IMPL_REF(Signal *signal) {
  jamEntry();
  const DropIndxImplRef *ref = (const DropIndxImplRef *)signal->getDataPtr();
  ndbrequire(refToNode(ref->senderRef) == getOwnNodeId());
  D("execDROP_INDX_IMPL_REF");
  handleDictRef(signal, ref);
}
...
const Dbdict::OpInfo Dbdict::AlterIndexRec::g_opInfo = {
    {'A', 'I', 'n', 0},
...
bool Dbdict::alterIndex_seize(SchemaOpPtr op_ptr) {
  return seizeOpRec<AlterIndexRec>(op_ptr);
}

void Dbdict::alterIndex_release(SchemaOpPtr op_ptr) {
  releaseOpRec<AlterIndexRec>(op_ptr);
}

void Dbdict::execALTER_INDX_REQ(Signal *signal) {
  jamEntry();
  if (!assembleFragments(signal)) {
    jam();
    return;
  ...
  SectionHandle handle(this, signal);
  D("execALTER_INDX_REQ");

  const AlterIndxReq req_copy = *(const AlterIndxReq *)signal->getDataPtr();
...
const Dbdict::TriggerTmpl Dbdict::g_hashIndexTriggerTmpl[1] = {
    {"NDB$INDEX_%u_UI",
     {
         TriggerType::SECONDARY_INDEX, TriggerActionTime::TA_AFTER,
         TriggerEvent::TE_CUSTOM, false, false,
         true  // monitor replicas, monitor all, report all
     }}};

const Dbdict::TriggerTmpl Dbdict::g_orderedIndexTriggerTmpl[1] = {
    {"NDB$INDEX_%u_CUSTOM",
     {TriggerType::ORDERED_INDEX, TriggerActionTime::TA_CUSTOM,
      TriggerEvent::TE_CUSTOM, true, false, true}}};

const Dbdict::TriggerTmpl Dbdict::g_buildIndexConstraintTmpl[1] = {
    {"NDB$INDEX_%u_BUILD",
     {TriggerType::READ_ONLY_CONSTRAINT, TriggerActionTime::TA_AFTER,
      TriggerEvent::TE_UPDATE, false, true, false}}};

const Dbdict::TriggerTmpl Dbdict::g_reorgTriggerTmpl[1] = {
    {"NDB$REORG_%u",
     {TriggerType::REORG_TRIGGER, TriggerActionTime::TA_AFTER,
      TriggerEvent::TE_CUSTOM, false, true, false}}};

const Dbdict::TriggerTmpl Dbdict::g_fkTriggerTmpl[2] = {
    {"NDB$FK_%u_PARENT_%u",
     {
         TriggerType::FK_PARENT, TriggerActionTime::TA_AFTER,
         TriggerEvent::TE_CUSTOM, false, false,
         true  // monitor replicas, monitor all, report all
     }},
    ...
    {"NDB$FK_%u_CHILD_%u",
     {
         TriggerType::FK_CHILD, TriggerActionTime::TA_AFTER,
         TriggerEvent::TE_CUSTOM, false, false,
         true  // monitor replicas, monitor all, report all
     }}};

const Dbdict::TriggerTmpl Dbdict::g_fullyReplicatedTriggerTmp[1] = {
    {"NDB$FULLY_REPLICATED_%u",
     {TriggerType::FULLY_REPLICATED_TRIGGER, TriggerActionTime::TA_AFTER,
      TriggerEvent::TE_CUSTOM, false, true, false}}};

// AlterIndex: PARSE

void Dbdict::alterIndex_parse(Signal *signal, bool master, SchemaOpPtr op_ptr,
                              SectionHandle &handle, ErrorInfo &error) {
  ...
  if (check_read_obj(impl_req->indexId, trans_ptr.p->m_transId) ==
...
void Dbdict::set_index_stat_frag(Signal *signal, TableRecordPtr indexPtr) {
  jam();
  DictObjectPtr index_obj_ptr;
  ndbrequire(c_obj_pool.getPtr(index_obj_ptr, indexPtr.p->m_obj_ptr_i));
...
bool Dbdict::alterIndex_subOps(Signal *signal, SchemaOpPtr op_ptr) {
  ...
  if (alterIndexPtr.p->m_sub_trigger == false) {
    ...
    switch (requestType) {
      case AlterIndxImplReq::AlterIndexOnline:
        jam();
        {
          Callback c = {safe_cast(&Dbdict::alterIndex_fromCreateTrigger),
                        op_ptr.p->op_key};
          op_ptr.p->m_callback = c;
      ...
      case AlterIndxImplReq::AlterIndexOffline:
        jam();
        {
          Callback c = {safe_cast(&Dbdict::alterIndex_fromDropTrigger),
                        op_ptr.p->op_key};
          op_ptr.p->m_callback = c;
  ...
  if (requestType == AlterIndxImplReq::AlterIndexOnline &&
    ...
    jam();
    Callback c = {safe_cast(&Dbdict::alterIndex_fromBuildIndex),
                  op_ptr.p->op_key};
    op_ptr.p->m_callback = c;
  ...
  if (ok && indexPtr.p->isOrderedIndex() &&
    ...
    jam();
    Callback c = {safe_cast(&Dbdict::alterIndex_fromIndexStat),
                  op_ptr.p->op_key};
    op_ptr.p->m_callback = c;
...
void Dbdict::alterIndex_toCreateTrigger(Signal *signal, SchemaOpPtr op_ptr) {
...
void Dbdict::alterIndex_fromCreateTrigger(Signal *signal, Uint32 op_key,
                                          Uint32 ret) {
  jam();
...
void Dbdict::alterIndex_toDropTrigger(Signal *signal, SchemaOpPtr op_ptr) {
...
void Dbdict::alterIndex_fromDropTrigger(Signal *signal, Uint32 op_key,
                                        Uint32 ret) {
  jam();
...
void Dbdict::alterIndex_toBuildIndex(Signal *signal, SchemaOpPtr op_ptr) {
...
void Dbdict::alterIndex_fromBuildIndex(Signal *signal, Uint32 op_key,
                                       Uint32 ret) {
  jam();
...
void Dbdict::alterIndex_toIndexStat(Signal *signal, SchemaOpPtr op_ptr) {
  ...
  sendSignal(reference(), GSN_INDEX_STAT_REQ, signal,
             IndexStatReq::SignalLength, JBB);
}

void Dbdict::alterIndex_fromIndexStat(Signal *signal, Uint32 op_key,
                                      Uint32 ret) {
  jam();
...
void Dbdict::alterIndex_reply(Signal *signal, SchemaOpPtr op_ptr,
                              ErrorInfo error) {
  ...
  if (!hasError(error)) {
    ...
    Uint32 clientRef = op_ptr.p->m_clientRef;
    sendSignal(clientRef, GSN_ALTER_INDX_CONF, signal,
               AlterIndxConf::SignalLength, JBB);
  } else {
    jam();
    AlterIndxRef *ref = (AlterIndxRef *)signal->getDataPtrSend();
    ref->senderRef = reference();
    ...
    Uint32 clientRef = op_ptr.p->m_clientRef;
    sendSignal(clientRef, GSN_ALTER_INDX_REF, signal,
               AlterIndxRef::SignalLength, JBB);
  }
}
...
void Dbdict::alterIndex_prepare(Signal *signal, SchemaOpPtr op_ptr) {
  AlterIndexRecPtr alterIndexPtr;
  getOpRec(op_ptr, alterIndexPtr);
  const AlterIndxImplReq *impl_req = &alterIndexPtr.p->m_request;
  Uint32 requestType = impl_req->requestType;
  ...
  switch (indexPtr.p->tableType) {
    ...
    case DictTabInfo::OrderedIndex:
      if (requestType == AlterIndxImplReq::AlterIndexAddPartition) {
        jam();
        Callback c = {safe_cast(&Dbdict::alterIndex_fromAddPartitions),
                      op_ptr.p->op_key};
        op_ptr.p->m_callback = c;
...
void Dbdict::alterIndex_toCreateLocal(Signal *signal, SchemaOpPtr op_ptr) {
  AlterIndexRecPtr alterIndexPtr;
  getOpRec(op_ptr, alterIndexPtr);
  const AlterIndxImplReq *impl_req = &alterIndexPtr.p->m_request;
...
void Dbdict::alterIndex_toDropLocal(Signal *signal, SchemaOpPtr op_ptr) {
  AlterIndexRecPtr alterIndexPtr;
  getOpRec(op_ptr, alterIndexPtr);
  const AlterIndxImplReq *impl_req = &alterIndexPtr.p->m_request;
  ...
  BlockReference ref = DBTC_REF;
  sendSignal(ref, GSN_DROP_INDX_IMPL_REQ, signal, DropIndxImplReq::SignalLength,
             JBB);
}

void Dbdict::alterIndex_fromLocal(Signal *signal, Uint32 op_key, Uint32 ret) {
  SchemaOpPtr op_ptr;
  AlterIndexRecPtr alterIndexPtr;
  ndbrequire(findSchemaOp(op_ptr, alterIndexPtr, op_key));
...
void Dbdict::alterIndex_toAddPartitions(Signal *signal, SchemaOpPtr op_ptr) {
  AlterIndexRecPtr alterIndexPtr;
  getOpRec(op_ptr, alterIndexPtr);
  const AlterIndxImplReq *impl_req = &alterIndexPtr.p->m_request;
...
void Dbdict::alterIndex_complete(Signal *signal, SchemaOpPtr op_ptr) {
  jam();
  AlterIndexRecPtr alterIndexPtr;
  getOpRec(op_ptr, alterIndexPtr);
  const AlterIndxImplReq *impl_req = &alterIndexPtr.p->m_request;
...
void Dbdict::alterIndex_abortParse(Signal *signal, SchemaOpPtr op_ptr) {
  AlterIndexRecPtr alterIndexPtr;
  getOpRec(op_ptr, alterIndexPtr);
  const AlterIndxImplReq *impl_req = &alterIndexPtr.p->m_request;
  Uint32 requestType = impl_req->requestType;
...
void Dbdict::alterIndex_abortPrepare(Signal *signal, SchemaOpPtr op_ptr) {
  AlterIndexRecPtr alterIndexPtr;
  getOpRec(op_ptr, alterIndexPtr);
  const AlterIndxImplReq *impl_req = &alterIndexPtr.p->m_request;
  Uint32 requestType = impl_req->requestType;
  ...
  Callback c = {safe_cast(&Dbdict::alterIndex_abortFromLocal),
                op_ptr.p->op_key};
  op_ptr.p->m_callback = c;
...
void Dbdict::alterIndex_abortFromLocal(Signal *signal, Uint32 op_key,
                                       Uint32 ret) {
  SchemaOpPtr op_ptr;
  AlterIndexRecPtr alterIndexPtr;
  ndbrequire(findSchemaOp(op_ptr, alterIndexPtr, op_key));
...
void Dbdict::execALTER_INDX_CONF(Signal *signal) {
  jamEntry();
  const AlterIndxConf *conf = (const AlterIndxConf *)signal->getDataPtr();
  D("execALTER_INDX_CONF");
  handleDictConf(signal, conf);
}

void Dbdict::execALTER_INDX_REF(Signal *signal) {
  jamEntry();
  const AlterIndxRef *ref = (const AlterIndxRef *)signal->getDataPtr();
  D("execALTER_INDX_REF");
  handleDictRef(signal, ref);
}

void Dbdict::execALTER_INDX_IMPL_CONF(Signal *signal) {
  jamEntry();
  const AlterIndxImplConf *conf =
      (const AlterIndxImplConf *)signal->getDataPtr();
  D("execALTER_INDX_IMPL_CONF");
  handleDictConf(signal, conf);
}

void Dbdict::execALTER_INDX_IMPL_REF(Signal *signal) {
  jamEntry();
  const AlterIndxImplRef *ref = (const AlterIndxImplRef *)signal->getDataPtr();
  D("execALTER_INDX_IMPL_REF");
  handleDictRef(signal, ref);
}
...
const Dbdict::OpInfo Dbdict::BuildIndexRec::g_opInfo = {
    {'B', 'I', 'n', 0},
...
bool Dbdict::buildIndex_seize(SchemaOpPtr op_ptr) {
  return seizeOpRec<BuildIndexRec>(op_ptr);
}

void Dbdict::buildIndex_release(SchemaOpPtr op_ptr) {
  releaseOpRec<BuildIndexRec>(op_ptr);
}

void Dbdict::execBUILDINDXREQ(Signal *signal) {
  jamEntry();
  if (!assembleFragments(signal)) {
    jam();
    return;
  ...
  const BuildIndxReq req_copy = *(const BuildIndxReq *)signal->getDataPtr();
...
void Dbdict::buildIndex_parse(Signal *signal, bool master, SchemaOpPtr op_ptr,
                              SectionHandle &handle, ErrorInfo &error) {
  D("buildIndex_parse");
...
bool Dbdict::buildIndex_subOps(Signal *signal, SchemaOpPtr op_ptr) {
  ...
  if (buildIndexPtr.p->m_subOpIndex < buildIndexPtr.p->m_subOpCount) {
    ...
    switch (buildIndexPtr.p->m_subOpIndex) {
      case 0:
        jam();
        {
          Callback c = {safe_cast(&Dbdict::buildIndex_fromCreateConstraint),
                        op_ptr.p->op_key};
          op_ptr.p->m_callback = c;
      ...
      case 1:
        ...
        {
          Callback c = {safe_cast(&Dbdict::buildIndex_fromBuildIndex),
                        op_ptr.p->op_key};
          op_ptr.p->m_callback = c;
      ...
      case 2:
        jam();
        {
          Callback c = {safe_cast(&Dbdict::buildIndex_fromDropConstraint),
                        op_ptr.p->op_key};
          op_ptr.p->m_callback = c;
...
void Dbdict::buildIndex_toCreateConstraint(Signal *signal, SchemaOpPtr op_ptr) {
...
void Dbdict::buildIndex_fromCreateConstraint(Signal *signal, Uint32 op_key,
                                             Uint32 ret) {
  jam();
...
void Dbdict::buildIndex_toBuildIndex(Signal *signal, SchemaOpPtr op_ptr) {
...
void Dbdict::buildIndex_fromBuildIndex(Signal *signal, Uint32 op_key,
                                       Uint32 ret) {
  jam();
...
void Dbdict::buildIndex_toDropConstraint(Signal *signal, SchemaOpPtr op_ptr) {
...
void Dbdict::buildIndex_fromDropConstraint(Signal *signal, Uint32 op_key,
                                           Uint32 ret) {
  jam();
...
void Dbdict::buildIndex_reply(Signal *signal, SchemaOpPtr op_ptr,
                              ErrorInfo error) {
  jam();
  ...
  if (!hasError(error)) {
    ...
    Uint32 clientRef = op_ptr.p->m_clientRef;
    sendSignal(clientRef, GSN_BUILDINDXREF, signal, BuildIndxRef::SignalLength,
               JBB);
  }
}
...
void Dbdict::buildIndex_prepare(Signal *signal, SchemaOpPtr op_ptr) {
  jam();
  BuildIndexRecPtr buildIndexPtr;
  getOpRec(op_ptr, buildIndexPtr);
  // const BuildIndxImplReq* impl_req = &buildIndexPtr.p->m_request;
...
void Dbdict::buildIndex_toLocalBuild(Signal *signal, SchemaOpPtr op_ptr) {
  BuildIndexRecPtr buildIndexPtr;
  getOpRec(op_ptr, buildIndexPtr);
  const BuildIndxImplReq *impl_req = &buildIndexPtr.p->m_request;
  SchemaTransPtr trans_ptr = op_ptr.p->m_trans_ptr;
  ...
  Callback c = {safe_cast(&Dbdict::buildIndex_fromLocalBuild),
                op_ptr.p->op_key};
  op_ptr.p->m_callback = c;
...
void Dbdict::buildIndex_fromLocalBuild(Signal *signal, Uint32 op_key,
                                       Uint32 ret) {
  SchemaOpPtr op_ptr;
  BuildIndexRecPtr buildIndexPtr;
  ndbrequire(findSchemaOp(op_ptr, buildIndexPtr, op_key));
...
void Dbdict::buildIndex_commit(Signal *signal, SchemaOpPtr op_ptr) {
  jam();
  BuildIndexRecPtr buildIndexPtr;
  getOpRec(op_ptr, buildIndexPtr);
...
void Dbdict::buildIndex_toLocalOnline(Signal *signal, SchemaOpPtr op_ptr) {
  BuildIndexRecPtr buildIndexPtr;
  getOpRec(op_ptr, buildIndexPtr);
  const BuildIndxImplReq *impl_req = &buildIndexPtr.p->m_request;
  ...
  req->indexId = impl_req->indexId;
  req->indexVersion = 0;  // not used
  req->indexType = 0;     // not used

  Callback c = {safe_cast(&Dbdict::buildIndex_fromLocalOnline),
                op_ptr.p->op_key};
  op_ptr.p->m_callback = c;
...
void Dbdict::buildIndex_fromLocalOnline(Signal *signal, Uint32 op_key,
                                        Uint32 ret) {
  SchemaOpPtr op_ptr;
  BuildIndexRecPtr buildIndexPtr;
  ndbrequire(findSchemaOp(op_ptr, buildIndexPtr, op_key));
...
void Dbdict::buildIndex_complete(Signal *signal, SchemaOpPtr op_ptr) {
  jam();
  sendTransConf(signal, op_ptr);
}
...
void Dbdict::buildIndex_abortParse(Signal *signal, SchemaOpPtr op_ptr) {
  D("buildIndex_abortParse" << *op_ptr.p);
  // wl3600_todo
  sendTransConf(signal, op_ptr);
}

void Dbdict::buildIndex_abortPrepare(Signal *signal, SchemaOpPtr op_ptr) {
...
void Dbdict::execBUILDINDXCONF(Signal *signal) {
  jamEntry();
  const BuildIndxConf *conf = (const BuildIndxConf *)signal->getDataPtr();
  D("execBUILD_INDX_CONF");
  handleDictConf(signal, conf);
}

void Dbdict::execBUILDINDXREF(Signal *signal) {
  jamEntry();
  const BuildIndxRef *ref = (const BuildIndxRef *)signal->getDataPtr();
  D("execBUILD_INDX_REF");
  handleDictRef(signal, ref);
}

void Dbdict::execBUILD_INDX_IMPL_CONF(Signal *signal) {
  jamEntry();
  const BuildIndxImplConf *conf =
      (const BuildIndxImplConf *)signal->getDataPtr();
  D("execBUILD_INDX_IMPL_CONF");
  handleDictConf(signal, conf);
}

void Dbdict::execBUILD_INDX_IMPL_REF(Signal *signal) {
  jamEntry();
  const BuildIndxImplRef *ref = (const BuildIndxImplRef *)signal->getDataPtr();
  D("execBUILD_INDX_IMPL_REF");
  handleDictRef(signal, ref);
}
...
const Dbdict::OpInfo Dbdict::IndexStatRec::g_opInfo = {
    {'S', 'I', 'n', 0},
...
bool Dbdict::indexStat_seize(SchemaOpPtr op_ptr) {
  return seizeOpRec<IndexStatRec>(op_ptr);
}

void Dbdict::indexStat_release(SchemaOpPtr op_ptr) {
  releaseOpRec<IndexStatRec>(op_ptr);
}

void Dbdict::execINDEX_STAT_REQ(Signal *signal) {
  jamEntry();
  if (!assembleFragments(signal)) {
    jam();
    return;
  ...
  D("execINDEX_STAT_REQ");

  const IndexStatReq req_copy = *(const IndexStatReq *)signal->getDataPtr();
...
void Dbdict::indexStat_parse(Signal *signal, bool master, SchemaOpPtr op_ptr,
                             SectionHandle &handle, ErrorInfo &error) {
  D("indexStat_parse");
...
bool Dbdict::indexStat_subOps(Signal *signal, SchemaOpPtr op_ptr) {
  ...
  Callback c = {safe_cast(&Dbdict::indexStat_fromIndexStat), op_ptr.p->op_key};
  op_ptr.p->m_callback = c;
  indexStat_toIndexStat(signal, op_ptr, requestType);
  return true;
}

void Dbdict::indexStat_toIndexStat(Signal *signal, SchemaOpPtr op_ptr,
                                   Uint32 requestType) {
...
void Dbdict::indexStat_fromIndexStat(Signal *signal, Uint32 op_key,
                                     Uint32 ret) {
  jam();
...
void Dbdict::indexStat_reply(Signal *signal, SchemaOpPtr op_ptr,
                             ErrorInfo error) {
  jam();
  ...
  D("indexStat_reply" << V(indexStatPtr.p->m_request.indexId));
...
void Dbdict::indexStat_prepare(Signal *signal, SchemaOpPtr op_ptr) {
  jam();
  IndexStatRecPtr indexStatPtr;
  getOpRec(op_ptr, indexStatPtr);
  const IndexStatImplReq *impl_req = &indexStatPtr.p->m_request;
...
static bool do_action(const NdbNodeBitmask &mask, const Uint16 list[],
                      Uint16 ownId) {
  for (Uint32 i = 0; i < MAX_REPLICAS; i++) {
    if (mask.get(list[i])) {
      if (list[i] == ownId) return true;
      return false;
    }
  }
  return false;
...
void Dbdict::indexStat_toLocalStat(Signal *signal, SchemaOpPtr op_ptr) {
  IndexStatRecPtr indexStatPtr;
  getOpRec(op_ptr, indexStatPtr);
  const IndexStatImplReq *impl_req = &indexStatPtr.p->m_request;
  SchemaTransPtr trans_ptr = op_ptr.p->m_trans_ptr;
  ...
  Callback c = {safe_cast(&Dbdict::indexStat_fromLocalStat), op_ptr.p->op_key};
  op_ptr.p->m_callback = c;
  ...
  *req = *impl_req;
  ...
  sendSignal(ref, GSN_INDEX_STAT_IMPL_REQ, signal,
             IndexStatImplReq::SignalLength, JBB);
}

void Dbdict::indexStat_fromLocalStat(Signal *signal, Uint32 op_key,
                                     Uint32 ret) {
  SchemaOpPtr op_ptr;
  IndexStatRecPtr indexStatPtr;
  ndbrequire(findSchemaOp(op_ptr, indexStatPtr, op_key));
...
void Dbdict::indexStat_commit(Signal *signal, SchemaOpPtr op_ptr) {
  jam();
  IndexStatRecPtr indexStatPtr;
  getOpRec(op_ptr, indexStatPtr);
...
void Dbdict::indexStat_complete(Signal *signal, SchemaOpPtr op_ptr) {
  jam();
  IndexStatRecPtr indexStatPtr;
  getOpRec(op_ptr, indexStatPtr);
...
void Dbdict::indexStat_abortParse(Signal *signal, SchemaOpPtr op_ptr) {
  D("indexStat_abortParse" << *op_ptr.p);
  // wl3600_todo
  sendTransConf(signal, op_ptr);
}

void Dbdict::indexStat_abortPrepare(Signal *signal, SchemaOpPtr op_ptr) {
...
void Dbdict::execINDEX_STAT_CONF(Signal *signal) {
  jamEntry();
  const IndexStatConf *conf = (const IndexStatConf *)signal->getDataPtr();
  D("execINDEX_STAT_CONF");
  handleDictConf(signal, conf);
}

void Dbdict::execINDEX_STAT_REF(Signal *signal) {
  jamEntry();
  const IndexStatRef *ref = (const IndexStatRef *)signal->getDataPtr();
  D("execINDEX_STAT_REF");
  handleDictRef(signal, ref);
}

void Dbdict::execINDEX_STAT_IMPL_CONF(Signal *signal) {
  jamEntry();
  const IndexStatImplConf *conf =
      (const IndexStatImplConf *)signal->getDataPtr();
  D("execINDEX_STAT_IMPL_CONF");
  handleDictConf(signal, conf);
}

void Dbdict::execINDEX_STAT_IMPL_REF(Signal *signal) {
  jamEntry();
  const IndexStatImplRef *ref = (const IndexStatImplRef *)signal->getDataPtr();
  D("execINDEX_STAT_IMPL_REF");
  handleDictRef(signal, ref);
}
 ...
 */
void Dbdict::execINDEX_STAT_REP(Signal *signal) {
  const IndexStatRep *rep = (const IndexStatRep *)signal->getDataPtr();
  D("execINDEX_STAT_REP");

  // non-master
  if (c_masterNodeId != getOwnNodeId()) {
    jam();
    BlockReference dictRef = calcDictBlockRef(c_masterNodeId);
    sendSignal(dictRef, GSN_INDEX_STAT_REP, signal, IndexStatRep::SignalLength,
               JBB);
    return;
  }
  ...
  if (rep->indexVersion != 0 && rep->indexVersion != indexPtr.p->tableVersion) {
    jam();
    return;
  }
...
void Dbdict::indexStatBg_process(Signal *signal) {
  if (!c_indexStatAutoUpdate || c_masterNodeId != getOwnNodeId() ||
  ...
  for (loop = 0; loop < maxloop; loop++, c_indexStatBgId++) {
    ...
    Callback c = {safe_cast(&Dbdict::indexStatBg_fromBeginTrans),
                  tx_ptr.p->tx_key};
    tx_ptr.p->m_callback = c;
...
void Dbdict::indexStatBg_fromBeginTrans(Signal *signal, Uint32 tx_key,
                                        Uint32 ret) {
  ...
  Callback c = {safe_cast(&Dbdict::indexStatBg_fromIndexStat),
                tx_ptr.p->tx_key};
  tx_ptr.p->m_callback = c;
...
void Dbdict::indexStatBg_fromIndexStat(Signal *signal, Uint32 tx_key,
                                       Uint32 ret) {
  ...
  if (ret != 0) {
    ...
    warningEvent("DICT: index %u stats auto-update error: %d", c_indexStatBgId,
                 ret);
  }

  Callback c = {safe_cast(&Dbdict::indexStatBg_fromEndTrans), tx_ptr.p->tx_key};
  tx_ptr.p->m_callback = c;
...
void Dbdict::indexStatBg_fromEndTrans(Signal *signal, Uint32 tx_key,
                                      Uint32 ret) {
...
void Dbdict::indexStatBg_sendContinueB(Signal *signal) {
...
const Dbdict::OpInfo Dbdict::CopyDataRec::g_opInfo = {
    {'D', 'C', 'D', 0},
    ~RT_DBDICT_COPY_DATA,
...
bool Dbdict::copyData_seize(SchemaOpPtr op_ptr) {
  return seizeOpRec<CopyDataRec>(op_ptr);
}

void Dbdict::copyData_release(SchemaOpPtr op_ptr) {
  releaseOpRec<CopyDataRec>(op_ptr);
}

void Dbdict::execCOPY_DATA_REQ(Signal *signal) {
  jamEntry();
  if (!assembleFragments(signal)) {
    jam();
    return;
  ...
  const CopyDataReq req_copy = *(const CopyDataReq *)signal->getDataPtr();
...
void Dbdict::copyData_parse(Signal *signal, bool master, SchemaOpPtr op_ptr,
                            SectionHandle &handle, ErrorInfo &error) {
  D("copyData_parse");
...
bool Dbdict::copyData_subOps(Signal *signal, SchemaOpPtr op_ptr) {
  D("copyData_subOps" << V(op_ptr.i) << *op_ptr.p);

  return false;
}

void Dbdict::copyData_reply(Signal *signal, SchemaOpPtr op_ptr,
                            ErrorInfo error) {
  jam();
  ...
  if (!hasError(error)) {
    ...
    Uint32 clientRef = op_ptr.p->m_clientRef;
    sendSignal(clientRef, GSN_COPY_DATA_REF, signal, CopyDataRef::SignalLength,
               JBB);
  }
}
...
void Dbdict::copyData_prepare(Signal *signal, SchemaOpPtr op_ptr) {
  jam();
  ...
  *req = *impl_req;
  ...
  Callback c = {safe_cast(&Dbdict::copyData_fromLocal), op_ptr.p->op_key};
  op_ptr.p->m_callback = c;
  ...
  if ((tabPtr.p->m_bits & TableRecord::TR_FullyReplicated) != 0) {
    jam();
    req->requestInfo |= CopyDataReq::NoScanTakeOver;
  }
...
void Dbdict::copyData_fromLocal(Signal *signal, Uint32 op_key, Uint32 ret) {
  SchemaOpPtr op_ptr;
  CopyDataRecPtr copyDataPtr;
...
void Dbdict::copyData_commit(Signal *signal, SchemaOpPtr op_ptr) {
  jam();
...
void Dbdict::copyData_complete(Signal *signal, SchemaOpPtr op_ptr) {
  jam();
...
void Dbdict::copyData_abortParse(Signal *signal, SchemaOpPtr op_ptr) {
  D("copyData_abortParse" << *op_ptr.p);
  // wl3600_todo
  sendTransConf(signal, op_ptr);
}

void Dbdict::copyData_abortPrepare(Signal *signal, SchemaOpPtr op_ptr) {
...
void Dbdict::execCOPY_DATA_CONF(Signal *signal) {
  jamEntry();
  const CopyDataConf *conf = (const CopyDataConf *)signal->getDataPtr();
  D("execCOPY_DATA_CONF");
  handleDictConf(signal, conf);
}

void Dbdict::execCOPY_DATA_REF(Signal *signal) {
  jamEntry();
...
void Dbdict::execCOPY_DATA_IMPL_CONF(Signal *signal) {
  jamEntry();
  const CopyDataImplConf *conf = (const CopyDataImplConf *)signal->getDataPtr();
  D("execCOPY_DATA_IMPL_CONF");
  ndbrequire(refToNode(conf->senderRef) == getOwnNodeId());
  handleDictConf(signal, conf);
}

void Dbdict::execCOPY_DATA_IMPL_REF(Signal *signal) {
  jamEntry();
  const CopyDataImplRef *ref = (const CopyDataImplRef *)signal->getDataPtr();
  D("execCOPY_DATA_IMPL_REF");
  ndbrequire(refToNode(ref->senderRef) == getOwnNodeId());
  handleDictRef(signal, ref);
}
...
int Dbdict::sendSignalUtilReq(Callback *pcallback, BlockReference ref,
...
int Dbdict::recvSignalUtilReq(Signal *signal, Uint32 returnCode) {
  jam();
  EVENT_TRACE;
...
void Dbdict::execUTIL_PREPARE_REF(Signal *signal) {
  jamEntry();
  const UtilPrepareRef *ref =
      CAST_CONSTPTR(UtilPrepareRef, signal->getDataPtr());
  Uint32 code = ref->errorCode;
  if (code == UtilPrepareRef::DICT_TAB_INFO_ERROR) code = ref->dictErrCode;
  EVENT_TRACE;
...
static const UintPtr sysTab_NDBEVENTS_0_offsets[] = {
...
void Dbdict::prepareTransactionEventSysTable(
    Callback *pcallback, Signal *signal, Uint32 senderData,
    UtilPrepareReq::OperationTypeValue prepReq) {
  // find table id for event system table
  DictObject *opj_ptr_p =
      get_object(EVENT_SYSTEM_TABLE_NAME, sizeof(EVENT_SYSTEM_TABLE_NAME));

  ndbrequire(opj_ptr_p != 0);
  TableRecordPtr tablePtr;
  ndbrequire(c_tableRecordPool_.getPtr(tablePtr, opj_ptr_p->m_object_ptr_i));
  ...
  prepareUtilTransaction(pcallback, signal, senderData, tableId, NULL, prepReq,
                         noAttr, NULL, NULL);
}

void Dbdict::prepareUtilTransaction(Callback *pcallback, Signal *signal,
                                    Uint32 senderData, Uint32 tableId,
  ...
  UtilPrepareReq *utilPrepareReq = (UtilPrepareReq *)signal->getDataPtrSend();
  ...
  sendSignalUtilReq(pcallback, DBUTIL_REF, GSN_UTIL_PREPARE_REQ, signal,
                    UtilPrepareReq::SignalLength, JBB, sectionsPtr,
                    UtilPrepareReq::NoOfSections);
}
...
void Dbdict::execCREATE_EVNT_REQ(Signal *signal) {
  jamEntry();
  D("execCREATE_EVNT_REQ");
...
void Dbdict::createEvent_RT_USER_CREATE(Signal *signal,
                                        OpCreateEventPtr evntRecPtr,
...
#endif
  // event name
  if ((!r0.first()) || (r0.getValueType() != SimpleProperties::StringValue) ||
      (r0.getValueLen() <= 0)) {
    jam();
    releaseSections(handle);
  ...
  // table name
  if ((!r0.next()) || (r0.getValueType() != SimpleProperties::StringValue) ||
      (r0.getValueLen() <= 0)) {
    jam();
  ...
  if (handle.m_cnt >= CreateEvntReq::ATTRIBUTE_MASK) {
    ...
    std::memset(evntRecPtr.p->m_eventRec.ATTRIBUTE_MASK2, 0,
    ...
    ndbrequire(sz1 == sz0);
    std::memset(evntRecPtr.p->m_eventRec.ATTRIBUTE_MASK, 0,
                sizeof(evntRecPtr.p->m_eventRec.ATTRIBUTE_MASK));
    std::memset(evntRecPtr.p->m_eventRec.ATTRIBUTE_MASK2, 0,
                sizeof(evntRecPtr.p->m_eventRec.ATTRIBUTE_MASK2));
...
void Dbdict::createEventUTIL_PREPARE(Signal *signal, Uint32 callbackData,
                                     Uint32 returnCode) {
  jam();
  EVENT_TRACE;
  if (returnCode == 0) {
    ...
    switch (evntRecPtr.p->m_requestType) {
      case CreateEvntReq::RT_USER_GET:
        jam();
        executeTransEventSysTable(&c, signal, evntRecPtr.i,
                                  evntRecPtr.p->m_eventRec, prepareId,
                                  UtilPrepareReq::Read);
        break;
      case CreateEvntReq::RT_USER_CREATE: {
        ...
        evntRecPtr.p->m_eventRec.TABLEVERSION =
            evntRecPtr.p->m_request.getTableVersion();
        ...
        DBUG_PRINT("info", ("CREATE: event name: %s table name: %s table id: "
                            "%u table version: %u",
...
static Uint32 countPrefixBytes(Uint32 len, const Uint8 *mask) {
  while (len && mask[len - 1] == 0) len--;
  return len;
}

void Dbdict::executeTransEventSysTable(
    Callback *pcallback, Signal *signal, const Uint32 ptrI,
    sysTab_NDBEVENTS_0 &m_eventRec, const Uint32 prepareId,
    UtilPrepareReq::OperationTypeValue prepReq) {
  jam();
  ...
  switch (prepReq) {
    case UtilPrepareReq::Read:
      ...
      std::memset(m_eventRec.ATTRIBUTE_MASK2, 0,
                  sizeof(m_eventRec.ATTRIBUTE_MASK2));

      // no more
      while (id < noAttr) AttributeHeader::init(attrPtr++, id++, 0);
    ...
    case UtilPrepareReq::Insert: {
      ...
      while (id < noAttr) {
        if (id != EVENT_SYSTEM_TABLE_ATTRIBUTE_MASK2_ID) {
          ...
          AttributeHeader::init(attrPtr, id, sz);
          const Uint32 *src =
              (const Uint32 *)(base + sysTab_NDBEVENTS_0_offsets[id]);
          memcpy(dataPtr, src, sz);
          dataPtr += (sz / 4);
          ...
          jam();
          Uint32 szBytes =
              countPrefixBytes(sizeof(m_eventRec.ATTRIBUTE_MASK2) - 4,
  ...
  executeTransaction(pcallback, signal, ptrI, prepareId, id, headerPtr,
                     lsdataPtr);
}

void Dbdict::executeTransaction(Callback *pcallback, Signal *signal,
                                Uint32 senderData, Uint32 prepareId,
                                Uint32 noAttr, LinearSectionPtr headerPtr,
                                LinearSectionPtr dataPtr) {
  jam();
  EVENT_TRACE;

  UtilExecuteReq *utilExecuteReq = (UtilExecuteReq *)signal->getDataPtrSend();
  ...
  sendSignalUtilReq(pcallback, DBUTIL_REF, GSN_UTIL_EXECUTE_REQ, signal,
                    UtilExecuteReq::SignalLength, JBB, sectionsPtr,
...
void Dbdict::parseReadEventSys(Signal *signal, sysTab_NDBEVENTS_0 &m_eventRec) {
  ...
  for (Uint32 i = 0; i < noAttr; i++) {
    ...
    for (Uint32 j = 0; j < sz; j++) dataReader.getWord(dst++);
  }
  ...
  if (noAttr < EVENT_SYSTEM_TABLE_LENGTH) {
    ...
    std::memset(lenbytes + szBytes, 0,
                sizeof(m_eventRec.ATTRIBUTE_MASK2) - szBytes);
  }
}

void Dbdict::createEventUTIL_EXECUTE(Signal *signal, Uint32 callbackData,
                                     Uint32 returnCode) {
  jam();
  EVENT_TRACE;
  if (returnCode == 0) {
...
void Dbdict::createEvent_RT_USER_GET(Signal *signal,
                                     OpCreateEventPtr evntRecPtr,
                                     SectionHandle &handle) {
  jam();
  EVENT_TRACE;
...
void Dbdict::createEventComplete_RT_USER_GET(Signal *signal,
                                             OpCreateEventPtr evntRecPtr) {
  jam();
...
void Dbdict::createEvent_nodeFailCallback(Signal *signal, Uint32 eventRecPtrI,
                                          Uint32 returnCode) {
  OpCreateEventPtr evntRecPtr;
...
void Dbdict::execCREATE_EVNT_CONF(Signal *signal) {
  ...
  evntRecPtr.p->m_reqTracker.reportConf(c_counterMgr,
...
void Dbdict::createEvent_RT_DICT_AFTER_GET(Signal *signal,
  ...
  sendSignal(SUMA_REF, GSN_SUB_CREATE_REQ, signal, SubCreateReq::SignalLength,
             JBB);
  DBUG_VOID_RETURN;
}
...
void Dbdict::createEvent_sendReply(Signal *signal, OpCreateEventPtr evntRecPtr,
...
void Dbdict::execSUB_START_REQ(Signal *signal) {
  ...
  if (refToBlock(origSenderRef) != DBDICT) {
    ...
    if (ERROR_INSERTED(6011)) {
      ...
      sendSignalWithDelay(reference(), GSN_SUB_START_REQ, signal, 5000,
                          SubStartReq::SignalLength);
    } else {
...

  {
    ...
    g_eventLogger->info(
        ...
        subbPtr.i);
#endif
    sendSignal(SUMA_REF, GSN_SUB_START_REQ, signal, SubStartReq::SignalLength,
               JBB);
  }
}
...
void Dbdict::execSUB_START_REF(Signal *signal) {
  ...
  if (refToBlock(senderRef) == SUMA) {
    ...
    ref->errorCode = err;
    sendSignal(subbPtr.p->m_senderRef, GSN_SUB_START_REF, signal,
               SubStartRef::SignalLength2, JBB);
    c_opSubEvent.release(subbPtr);
    return;
  }
 ...
 * Coordinator
 */
void Dbdict::completeSubStartReq(Signal *signal, Uint32 ptrI,
                                 Uint32 returnCode) {
  jam();
...
void Dbdict::execSUB_STOP_REQ(Signal *signal) {
...

  {
    ...
    subbPtr.p->m_subscriberRef = req->subscriberRef;
    subbPtr.p->m_subscriberData = req->subscriberData;
    std::memset(&subbPtr.p->m_sub_stop_conf, 0,
...

  {
    ...
    sendSignal(SUMA_REF, GSN_SUB_STOP_REQ, signal, SubStopReq::SignalLength,
               JBB);
  }
}
...
void Dbdict::execSUB_STOP_REF(Signal *signal) {
  ...
  if (refToBlock(senderRef) == SUMA) {
    ...
    ref->senderData = subbPtr.p->m_senderData;
    ref->errorCode = err;
    sendSignal(subbPtr.p->m_senderRef, GSN_SUB_STOP_REF, signal,
 ...
 * Coordinator
 */
void Dbdict::completeSubStopReq(Signal *signal, Uint32 ptrI,
                                Uint32 returnCode) {
  OpSubEventPtr subbPtr;
  c_opSubEvent.getPtr(subbPtr, ptrI);
...
void Dbdict::execDROP_EVNT_REQ(Signal *signal) {
  jamEntry();
  D("execDROP_EVNT_REQ");
  ...
  if (refToBlock(senderRef) != DBDICT && getOwnNodeId() != c_masterNodeId) {
    jam();
    releaseSections(handle);
    ...
    ref->setErrorNode(reference());
    ref->setMasterNode(c_masterNodeId);
    sendSignal(senderRef, GSN_DROP_EVNT_REF, signal, DropEvntRef::SignalLength2,
               JBB);
    DBUG_VOID_RETURN;
  }
  ...
  if (!c_opDropEvent.seize(evntRecPtr)) {
    ...
    ret->setErrorLine(__LINE__);
    ret->setErrorNode(reference());
    sendSignal(senderRef, GSN_DROP_EVNT_REF, signal, DropEvntRef::SignalLength,
               JBB);
    DBUG_VOID_RETURN;
  }
...
void Dbdict::dropEventUTIL_PREPARE_READ(Signal *signal, Uint32 callbackData,
                                        Uint32 returnCode) {
  jam();
  EVENT_TRACE;
  if (returnCode != 0) {
  ...
  executeTransEventSysTable(&c, signal, evntRecPtr.i, evntRecPtr.p->m_eventRec,
                            prepareId, UtilPrepareReq::Read);
}

void Dbdict::dropEventUTIL_EXECUTE_READ(Signal *signal, Uint32 callbackData,
                                        Uint32 returnCode) {
  jam();
  EVENT_TRACE;
  if (returnCode != 0) {
...
void Dbdict::execSUB_REMOVE_REQ(Signal *signal) {
  jamEntry();
  D("execSUB_REMOVE_REQ");
  ...
  sendSignal(SUMA_REF, GSN_SUB_REMOVE_REQ, signal, SubRemoveReq::SignalLength,
             JBB);
  DBUG_VOID_RETURN;
}
...
void Dbdict::execSUB_REMOVE_REF(Signal *signal) {
  jamEntry();
  D("execSUB_REMOVE_REF");
  ...
  if (refToBlock(senderRef) == SUMA) {
    ...
    if (err == SubRemoveRef::NoSuchSubscription ||
      ...
      conf->senderRef = reference();
      conf->senderData = subbPtr.p->m_senderData;
      sendSignal(subbPtr.p->m_senderRef, GSN_SUB_REMOVE_CONF, signal,
      ...
      ref->senderData = subbPtr.p->m_senderData;
      ref->errorCode = err;
      sendSignal(subbPtr.p->m_senderRef, GSN_SUB_REMOVE_REF, signal,
                 SubRemoveRef::SignalLength, JBB);
    }
    c_opSubEvent.release(subbPtr);
    DBUG_VOID_RETURN;
...
void Dbdict::execSUB_REMOVE_CONF(Signal *signal) {
  jamEntry();
  const SubRemoveConf *conf = (SubRemoveConf *)signal->getDataPtr();
  Uint32 senderRef = conf->senderRef;
  D("execSUB_REMOVE_CONF");
...
void Dbdict::completeSubRemoveReq(Signal *signal, Uint32 ptrI, Uint32 xxx) {
  OpDropEventPtr evntRecPtr;
...
void Dbdict::dropEventUTIL_PREPARE_DELETE(Signal *signal, Uint32 callbackData,
                                          Uint32 returnCode) {
  jam();
  EVENT_TRACE;
  if (returnCode != 0) {
...
void Dbdict::dropEventUTIL_EXECUTE_DELETE(Signal *signal, Uint32 callbackData,
                                          Uint32 returnCode) {
  jam();
  EVENT_TRACE;
  if (returnCode != 0) {
...
void Dbdict::dropEventUtilPrepareRef(Signal *signal, Uint32 callbackData,
                                     Uint32 returnCode) {
  jam();
  EVENT_TRACE;
  UtilPrepareRef *const ref = (UtilPrepareRef *)signal->getDataPtr();
...
void Dbdict::dropEventUtilExecuteRef(Signal *signal, Uint32 callbackData,
                                     Uint32 returnCode) {
  jam();
  EVENT_TRACE;
  OpDropEventPtr evntRecPtr;
...
void Dbdict::dropEvent_sendReply(Signal *signal, OpDropEventPtr evntRecPtr) {
  jam();
  EVENT_TRACE;
  Uint32 senderRef = evntRecPtr.p->m_request.getUserRef();
...
const Dbdict::OpInfo Dbdict::CreateTriggerRec::g_opInfo = {
    {'C', 'T', 'r', 0},
...
bool Dbdict::createTrigger_seize(SchemaOpPtr op_ptr) {
  return seizeOpRec<CreateTriggerRec>(op_ptr);
}

void Dbdict::createTrigger_release(SchemaOpPtr op_ptr) {
  releaseOpRec<CreateTriggerRec>(op_ptr);
}

void Dbdict::execCREATE_TRIG_REQ(Signal *signal) {
  jamEntry();
  if (!assembleFragments(signal)) {
    jam();
    return;
  ...
  SectionHandle handle(this, signal);
  D("execCREATE_TRIG_REQ");

  const CreateTrigReq req_copy = *(const CreateTrigReq *)signal->getDataPtr();
...
void Dbdict::createTrigger_parse(Signal *signal, bool master,
                                 SchemaOpPtr op_ptr, SectionHandle &handle,
                                 ErrorInfo &error) {
  ...
  SimpleProperties::UnpackStatus status = SimpleProperties::unpack(
      r, &tableDesc, DictTabInfo::TableMapping, DictTabInfo::TableMappingSize);
  ...
  if (handle.m_cnt >= 2) {
    ...
    if (mask_ptr.sz < triggerPtr.p->attributeMask.getSizeInWords()) {
      ...
      std::memset(triggerPtr.p->attributeMask.rep.data + mask_ptr.sz, 0,
                  4 * len);
    }
  } else {
    jam();
  ...
  {
    DictObjectPtr obj_ptr;
    seizeDictObject(op_ptr, obj_ptr,
                    triggerPtr.p->triggerName);  // added to c_obj_name_hash
  ...
  if (TriggerInfo::getTriggerType(triggerPtr.p->triggerInfo) ==
      TriggerType::FULLY_REPLICATED_TRIGGER) {
    jam();
    TableRecordPtr tablePtr;
    bool ok = find_object(tablePtr, impl_req->tableId);
    ndbrequire(ok);
    ...
    /**
     * during NR it might be that fullyReplicatedTriggerId is already set
     *   and in that case we don't actually modify the table...
     */
    bool modified = tablePtr.p->fullyReplicatedTriggerId != impl_req->triggerId;
    tablePtr.p->fullyReplicatedTriggerId = impl_req->triggerId;

    if (modified) {
      jam();
      /**
       * Mark table modified by subOp
       */
      markTableModifiedBySubOp(op_ptr, impl_req->tableId);
    }
  }
}

void Dbdict::markTableModifiedBySubOp(SchemaOpPtr op_ptr, Uint32 tableId) {
  SchemaTransPtr trans_ptr = op_ptr.p->m_trans_ptr;
  LocalSchemaOp_list list(c_schemaOpPool, trans_ptr.p->m_op_list);
  ndbrequire(list.prev(op_ptr));
  do {
    jam();
    Uint32 sz = sizeof(op_ptr.p->m_oprec_ptr.p->m_opType);
    const char *opType = op_ptr.p->m_oprec_ptr.p->m_opType;
    if (memcmp(opType, CreateTableRec::g_opInfo.m_opType, sz) == 0) {
      CreateTableRecPtr createTabPtr;
      getOpRec(op_ptr, createTabPtr);
      if (createTabPtr.p->m_request.tableId == tableId) {
        jam();
        createTabPtr.p->m_modified_by_subOps = true;
        return;
      }
    }
  } while (list.prev(op_ptr));
...
void Dbdict::createTrigger_parse_endpoint(Signal *signal, SchemaOpPtr op_ptr,
                                          ErrorInfo &error) {
  jam();
...
void Dbdict::createTrigger_create_drop_trigger_operation(Signal *signal,
                                                         SchemaOpPtr op_ptr,
                                                         ErrorInfo &error) {
  jam();
...
bool Dbdict::createTrigger_subOps(Signal *signal, SchemaOpPtr op_ptr) {
  ...
  if (!createTriggerPtr.p->m_sub_dst) {
    ...
    Callback c = {safe_cast(&Dbdict::createTrigger_fromCreateEndpoint),
                  op_ptr.p->op_key};
    op_ptr.p->m_callback = c;
    createTrigger_toCreateEndpoint(signal, op_ptr, CreateTrigReq::TriggerDst);
    return true;
  }
  ...
  if (!createTriggerPtr.p->m_sub_src) {
    ...
    Callback c = {safe_cast(&Dbdict::createTrigger_fromCreateEndpoint),
                  op_ptr.p->op_key};
    op_ptr.p->m_callback = c;
    createTrigger_toCreateEndpoint(signal, op_ptr, CreateTrigReq::TriggerSrc);
    return true;
  }
...
void Dbdict::createTrigger_toCreateEndpoint(
    Signal *signal, SchemaOpPtr op_ptr, CreateTrigReq::EndpointFlag endpoint) {
  D("alterIndex_toCreateTrigger");
...
void Dbdict::createTrigger_fromCreateEndpoint(Signal *signal, Uint32 op_key,
                                              Uint32 ret) {
  SchemaOpPtr op_ptr;
  CreateTriggerRecPtr createTriggerPtr;
...
void Dbdict::createTrigger_reply(Signal *signal, SchemaOpPtr op_ptr,
                                 ErrorInfo error) {
  jam();
...
void Dbdict::createTrigger_prepare(Signal *signal, SchemaOpPtr op_ptr) {
  jam();
  CreateTriggerRecPtr createTriggerPtr;
  getOpRec(op_ptr, createTriggerPtr);
...
void Dbdict::createTrigger_prepare_fromLocal(Signal *signal, Uint32 op_key,
                                             Uint32 ret) {
  SchemaOpPtr op_ptr;
  CreateTriggerRecPtr createTriggerPtr;
  ndbrequire(findSchemaOp(op_ptr, createTriggerPtr, op_key));
...
void Dbdict::createTrigger_commit(Signal *signal, SchemaOpPtr op_ptr) {
  jam();
...
void Dbdict::createTrigger_complete(Signal *signal, SchemaOpPtr op_ptr) {
  jam();
...
void Dbdict::createTrigger_abortParse(Signal *signal, SchemaOpPtr op_ptr) {
...
void Dbdict::createTrigger_abortPrepare(Signal *signal, SchemaOpPtr op_ptr) {
  ...
  sendSignal(ref, GSN_DROP_TRIG_IMPL_REQ, signal, DropTrigImplReq::SignalLength,
             JBB);
}

void Dbdict::createTrigger_abortPrepare_fromLocal(Signal *signal, Uint32 op_key,
                                                  Uint32 ret) {
  SchemaOpPtr op_ptr;
  CreateTriggerRecPtr createTriggerPtr;
  ndbrequire(findSchemaOp(op_ptr, createTriggerPtr, op_key));
...
void Dbdict::send_create_trig_req(Signal *signal, SchemaOpPtr op_ptr) {
  CreateTriggerRecPtr createTriggerPtr;
  getOpRec(op_ptr, createTriggerPtr);
  const CreateTrigImplReq *impl_req = &createTriggerPtr.p->m_request;
...
void Dbdict::execCREATE_TRIG_CONF(Signal *signal) {
  jamEntry();
  const CreateTrigConf *conf = (const CreateTrigConf *)signal->getDataPtr();
  D("execCREATE_TRIG_CONF");
  handleDictConf(signal, conf);
}

void Dbdict::execCREATE_TRIG_REF(Signal *signal) {
  jamEntry();
  const CreateTrigRef *ref = (const CreateTrigRef *)signal->getDataPtr();
  D("execCREATE_TRIG_REF");
  handleDictRef(signal, ref);
}

void Dbdict::execCREATE_TRIG_IMPL_CONF(Signal *signal) {
  jamEntry();
  const CreateTrigImplConf *conf =
      (const CreateTrigImplConf *)signal->getDataPtr();
  D("execCREATE_TRIG_IMPL_CONF");
  ndbrequire(refToNode(conf->senderRef) == getOwnNodeId());
  handleDictConf(signal, conf);
}

void Dbdict::execCREATE_TRIG_IMPL_REF(Signal *signal) {
  jamEntry();
  const CreateTrigImplRef *ref =
      (const CreateTrigImplRef *)signal->getDataPtr();
  D("execCREATE_TRIG_IMPL_REF");
  ndbrequire(refToNode(ref->senderRef) == getOwnNodeId());
  handleDictRef(signal, ref);
}
...
const Dbdict::OpInfo Dbdict::DropTriggerRec::g_opInfo = {
    {'D', 'T', 'r', 0},
...
bool Dbdict::dropTrigger_seize(SchemaOpPtr op_ptr) {
  return seizeOpRec<DropTriggerRec>(op_ptr);
}

void Dbdict::dropTrigger_release(SchemaOpPtr op_ptr) {
  releaseOpRec<DropTriggerRec>(op_ptr);
}

void Dbdict::execDROP_TRIG_REQ(Signal *signal) {
  jamEntry();
  if (!assembleFragments(signal)) {
    jam();
    return;
  ...
  const DropTrigReq req_copy = *(const DropTrigReq *)signal->getDataPtr();
...
void Dbdict::dropTrigger_parse(Signal *signal, bool master, SchemaOpPtr op_ptr,
                               SectionHandle &handle, ErrorInfo &error) {
  ...
  if (reqByName) {
    ...
    if (impl_req->triggerId != RNIL && impl_req->triggerId != obj_ptr.p->m_id) {
      jam();
...
void Dbdict::dropTrigger_parse_endpoint(Signal *signal, SchemaOpPtr op_ptr,
                                        ErrorInfo &error) {
  DropTriggerRecPtr dropTriggerPtr;
  getOpRec(op_ptr, dropTriggerPtr);
  DropTrigImplReq *impl_req = &dropTriggerPtr.p->m_request;
...
bool Dbdict::dropTrigger_subOps(Signal *signal, SchemaOpPtr op_ptr) {
  ...
  if (!dropTriggerPtr.p->m_sub_src) {
    ...
    Callback c = {safe_cast(&Dbdict::dropTrigger_fromDropEndpoint),
                  op_ptr.p->op_key};
    op_ptr.p->m_callback = c;
    dropTrigger_toDropEndpoint(signal, op_ptr, DropTrigReq::TriggerSrc);
    return true;
  }
  ...
  if (!dropTriggerPtr.p->m_sub_dst) {
    ...
    Callback c = {safe_cast(&Dbdict::dropTrigger_fromDropEndpoint),
                  op_ptr.p->op_key};
    op_ptr.p->m_callback = c;
    dropTrigger_toDropEndpoint(signal, op_ptr, DropTrigReq::TriggerDst);
    return true;
  }
...
void Dbdict::dropTrigger_toDropEndpoint(Signal *signal, SchemaOpPtr op_ptr,
...
void Dbdict::dropTrigger_fromDropEndpoint(Signal *signal, Uint32 op_key,
                                          Uint32 ret) {
  SchemaOpPtr op_ptr;
  DropTriggerRecPtr dropTriggerPtr;
...
void Dbdict::dropTrigger_reply(Signal *signal, SchemaOpPtr op_ptr,
                               ErrorInfo error) {
  jam();
  SchemaTransPtr &trans_ptr = op_ptr.p->m_trans_ptr;
  DropTriggerRecPtr dropTriggerPtr;
  ...
  if (!hasError(error)) {
    ...
    Uint32 clientRef = op_ptr.p->m_clientRef;
    sendSignal(clientRef, GSN_DROP_TRIG_REF, signal, DropTrigRef::SignalLength,
               JBB);
  }
}
...
void Dbdict::dropTrigger_prepare(Signal *signal, SchemaOpPtr op_ptr) {
  jam();
...
void Dbdict::dropTrigger_commit(Signal *signal, SchemaOpPtr op_ptr) {
  jam();
  DropTriggerRecPtr dropTriggerPtr;
  getOpRec(op_ptr, dropTriggerPtr);
...
void Dbdict::dropTrigger_commit_fromLocal(Signal *signal, Uint32 op_key,
                                          Uint32 ret) {
  SchemaOpPtr op_ptr;
  DropTriggerRecPtr dropTriggerPtr;
  ndbrequire(findSchemaOp(op_ptr, dropTriggerPtr, op_key));
  ...
  if (ret) {
    jam();
    warningEvent("Got error code %u from %x (DROP_TRIG_IMPL_REQ)", ret,
...
void Dbdict::send_drop_trig_req(Signal *signal, SchemaOpPtr op_ptr) {
  DropTriggerRecPtr dropTriggerPtr;
  getOpRec(op_ptr, dropTriggerPtr);
  const DropTrigImplReq *impl_req = &dropTriggerPtr.p->m_request;
...
void Dbdict::dropTrigger_complete(Signal *signal, SchemaOpPtr op_ptr) {
  jam();
  sendTransConf(signal, op_ptr);
}
...
void Dbdict::dropTrigger_abortParse(Signal *signal, SchemaOpPtr op_ptr) {
  DropTriggerRecPtr dropTriggerPtr;
  getOpRec(op_ptr, dropTriggerPtr);
...
void Dbdict::dropTrigger_abortPrepare(Signal *signal, SchemaOpPtr op_ptr) {
...
void Dbdict::execDROP_TRIG_CONF(Signal *signal) {
  jamEntry();
  const DropTrigConf *conf = (const DropTrigConf *)signal->getDataPtr();
  D("execDROP_TRIG_CONF");
  handleDictConf(signal, conf);
}

void Dbdict::execDROP_TRIG_REF(Signal *signal) {
  jamEntry();
  const DropTrigRef *ref = (const DropTrigRef *)signal->getDataPtr();
  D("execDROP_TRIG_REF");
  handleDictRef(signal, ref);
...
void Dbdict::execDROP_TRIG_IMPL_CONF(Signal *signal) {
  jamEntry();
  const DropTrigImplConf *conf = (const DropTrigImplConf *)signal->getDataPtr();
  D("execDROP_TRIG_IMPL_CONF");
  ndbrequire(refToNode(conf->senderRef) == getOwnNodeId());
  handleDictConf(signal, conf);
}

void Dbdict::execDROP_TRIG_IMPL_REF(Signal *signal) {
  jamEntry();
  const DropTrigImplRef *ref = (const DropTrigImplRef *)signal->getDataPtr();
  D("execDROP_TRIG_IMPL_REF");
  ndbrequire(refToNode(ref->senderRef) == getOwnNodeId());
  handleDictRef(signal, ref);
}
...
void Dbdict::getTableKeyList(TableRecordPtr tablePtr,
...
void Dbdict::getIndexAttr(TableRecordPtr indexPtr, Uint32 itAttr, Uint32 *id) {
  jam();
...
void Dbdict::getIndexAttrList(TableRecordPtr indexPtr,
...
void Dbdict::getIndexAttrMask(TableRecordPtr indexPtr, AttributeMask &mask) {
  jam();
  mask.clear();
  ndbrequire(indexPtr.p->noOfAttributes >= 2);
...
const Dbdict::DictLockType *Dbdict::getDictLockType(Uint32 lockType) {
  ...
  for (unsigned int i = 0; i < sizeof(lt) / sizeof(lt[0]); i++) {
    if ((Uint32)lt[i].lockType == lockType) return &lt[i];
  }
  return NULL;
}

void Dbdict::debugLockInfo(Signal *signal, const char *text, Uint32 rc) {
  if (!g_trace) return;
  ...
  infoEvent("DICT : %s %u %s", text, rc, rctext);
}

void Dbdict::sendDictLockInfoEvent(Signal *, const UtilLockReq *req,
                                   const char *text) {
  const Dbdict::DictLockType *lt = getDictLockType(req->extra);

  infoEvent("DICT: %s %u for %s", text, (unsigned)refToNode(req->senderRef),
            lt->text);
}

void Dbdict::execDICT_LOCK_REQ(Signal *signal) {
  jamEntry();
  ...
  debugLockInfo(signal, "DICT_LOCK_REQ lock", res);
  switch (res) {
    case 0:
      jam();
...
void Dbdict::execDICT_UNLOCK_ORD(Signal *signal) {
  jamEntry();
  const DictUnlockOrd *ord = (const DictUnlockOrd *)&signal->theData[0];
  D("execDICT_UNLOCK_ORD");
  ...
  debugLockInfo(signal, "DICT_UNLOCK_ORD unlock", res);
...
void Dbdict::execDICT_TAKEOVER_REQ(Signal *signal) {
  jamEntry();
  ...
  while (pending_trans) {
    ...
    if (pending_op && trans_ptr.p->m_state == SchemaTrans::TS_ENDING) {
      jam();
      /*
      ...
      */
      switch (op_ptr.p->m_state) {
        ...
        case SchemaOp::OS_COMPLETING: {
          ...
          sendSignalWithDelay(reference(), GSN_CONTINUEB, signal, 100,
                              1 + DictTakeoverReq::SignalLength);
          return;
        }
        default:
          break;
...
void Dbdict::execDICT_TAKEOVER_REF(Signal *signal) {
...
void Dbdict::execDICT_TAKEOVER_CONF(Signal *signal) {
  jamEntry();
...
void Dbdict::check_takeover_replies(Signal *signal) {
  ...
  while (pending_trans) {
    ...
    debugLockInfo(signal, "check_takeover_replies trylock 1", lockError);
    if (lockError != 0) {
      jam();
#ifdef VM_TRACE
      ...
      c_nodes.getPtr(nodePtr, i);
      {
        ...
        if (!findSchemaTrans(trans_ptr, trans_key)) {
          ...
          */
          ...
          lockReq.lockType = DictLockReq::SchemaTransLock;
          int lockError = dict_lock_trylock(&lockReq);
          debugLockInfo(signal, "check_takeover_replies trylock 2", lockError);
          if (lockError != 0) {
            jam();
#ifdef VM_TRACE
            g_eventLogger->info(
        ...
        if ((SchemaOp::weight(rollforward_op_state) <
            ...
            ((rollforward_op_state == trans_ptr.p->m_rollforward_op_state &&
        ...
        if ((SchemaOp::weight(rollback_op_state) >
            ...
            ((rollback_op_state == trans_ptr.p->m_rollback_op_state &&
  ...
  */
  ...
  while (pending_trans) {
    ...
    if (trans_ptr.p->m_master_recovery_state == SchemaTrans::TRS_ROLLFORWARD) {
       ...
       */
      jam();
      infoEvent("Pending schema transaction %u will be rolled forward",
                trans_ptr.p->trans_key);
       ...
       */
      jam();
      infoEvent("Pending schema transaction %u will be rolled back",
                trans_ptr.p->trans_key);
  ...
  while (pending_trans) {
          ...
          if (SchemaTrans::weight(trans_ptr.p->m_state) <
            ...
            if (masterNodePtr.p->recoveryState !=
              ...
              */
              ...
              if (seizeSchemaOp(trans_ptr, missing_op_ptr,
                                nodePtr.p->takeOverConf.highest_op, info)) {
                jam();
#ifdef VM_TRACE
                g_eventLogger->info(
                ...
                missing_op_ptr.p->m_state =
                    nodePtr.p->takeOverConf.highest_op_state;
                masterNodePtr.p->recoveryState =
                    NodeRecord::RS_PARTIAL_ROLLBACK;
                masterNodePtr.p->start_op =
                    masterNodePtr.p->takeOverConf.highest_op;
                masterNodePtr.p->start_op_state =
                    masterNodePtr.p->takeOverConf.highest_op_state;
              } else {
                jam();
                ndbassert(false);
              }
            ...
            */
            ...
            if (seizeSchemaOp(trans_ptr, missing_op_ptr, op_key, info)) {
              jam();
#ifdef VM_TRACE
              g_eventLogger->info(
    ...
    */
    for (unsigned i = 1; i < MAX_NDB_NODES; i++) {
      ...
      if (trans_ptr.p->m_nodes.get(i)) {
        ...
        if (trans_ptr.p->m_master_recovery_state ==
          ...
          if (trans_ptr.p->m_lowest_trans_state == SchemaTrans::TS_PREPARING &&
            ...
            g_eventLogger->info(
                ...
                nodePtr.i);
#endif
          } else if (SchemaOp::weight(
                         nodePtr.p->takeOverConf.rollforward_op_state) >
                         SchemaOp::weight(
            ...
            */
            ...
            if (i == c_masterNodeId) {
              ...
              jam();
              SchemaOpPtr op_ptr;
              ndbrequire(findSchemaOp(op_ptr, trans_ptr.p->m_rollforward_op));
#ifdef VM_TRACE
          ...
          } else if (trans_ptr.p->m_lowest_trans_state ==
                         SchemaTrans::TS_COMMITTING &&
                     nodePtr.p->takeOverConf.trans_state >=
                         SchemaTrans::TS_FLUSH_COMPLETE) {
            /*
        ...
        } else  // if (trans_ptr.p->m_master_recovery_state ==
                // SchemaTrans::TRS_ROLLBACK)
        {
          jam();
          if (SchemaOp::weight(nodePtr.p->takeOverConf.rollback_op_state) <
            ...
            */
            ...
            if (i == c_masterNodeId &&
              ...
              jam();
              SchemaOpPtr op_ptr;
              ndbrequire(findSchemaOp(op_ptr, trans_ptr.p->m_rollback_op));
#ifdef VM_TRACE
              g_eventLogger->info("Changed op %u from state %u to %u",
    ...
    */
    if (trans_ptr.p->m_master_recovery_state == SchemaTrans::TRS_ROLLFORWARD) {
      ...
      ndbrequire(
          findSchemaOp(rollforward_op_ptr, trans_ptr.p->m_rollforward_op));
...
#endif
    } else  // if (trans_ptr.p->master_recovery_state ==
            // SchemaTrans::TRS_ROLLBACK)
    {
      jam();
...
void Dbdict::removeStaleDictLocks(Signal *signal,
    ...
    do {
      if (NodeBitmask::get(theFailedNodes,
        ...
        if (iter.m_curr.p->m_req.requestInfo & UtilLockReq::Granted) {
          jam();
          infoEvent("Removed lock for node %u",
                    refToNode(iter.m_curr.p->m_req.senderRef));
          sendDictLockInfoEvent(signal, &iter.m_curr.p->m_req,
                                "remove lock by failed node");
        } else {
          jam();
          infoEvent("Removed lock request for node %u",
                    refToNode(iter.m_curr.p->m_req.senderRef));
          sendDictLockInfoEvent(signal, &iter.m_curr.p->m_req,
...
Uint32 Dbdict::dict_lock_trylock(const DictLockReq *_req) {
  UtilLockReq req;
...
#endif
  if (g_trace) m_dict_lock.dump_queue(m_dict_lock_pool, this);

  return SchemaTransBeginRef::Busy;
}

Uint32 Dbdict::dict_lock_unlock(Signal *signal, const DictLockReq *_req,
  ...
  UtilLockReq lockReq;
  Uint32 res = m_dict_lock.unlock(this, m_dict_lock_pool, &req, &lockReq);
  switch (res) {
    case UtilUnlockRef::OK:
      if (type) {
  ...
  if (m_dict_lock.first(this, m_dict_lock_pool, iter)) {
    ...
    while ((res = m_dict_lock.checkLockGrant(iter, &lockReq)) > 0) {
      ...
      if (!m_dict_lock.next(iter)) break;
    }
  }
...
void Dbdict::execBACKUP_LOCK_TAB_REQ(Signal *signal) {
  jamEntry();
...
void Dbdict::initSchemaFile(XSchemaFile *xsf, Uint32 firstPage, Uint32 lastPage,
                            bool initEntries) {
...
void Dbdict::modifySchemaFileAtRestart(XSchemaFile *xsf) {
  D("modifySchemaFileAtRestart" << V(c_at_restart_skip_indexes)
                                << V(c_at_restart_skip_fks));
  ...
  for (Uint32 i = 0; i < noOfEntries; i++) {
    ...
    if (c_at_restart_skip_fks)
      if (entry->m_tableType == DictTabInfo::ForeignKey) zap = true;
    if (zap) {
      D("zap entry " << i << " " << *entry);
      entry->init();
    }
  }
  for (Uint32 n = 0; n < xsf->noOfPages; n++) computeChecksum(xsf, n);
}

void Dbdict::computeChecksum(XSchemaFile *xsf, Uint32 pageNo) {
  SchemaFile *sf = &xsf->schemaPage[pageNo];
  sf->CheckSum = 0;
  sf->CheckSum = computeChecksum((Uint32 *)sf, NDB_SF_PAGE_SIZE_IN_WORDS);
}

bool Dbdict::validateChecksum(const XSchemaFile *xsf) {
  for (Uint32 n = 0; n < xsf->noOfPages; n++) {
    SchemaFile *sf = &xsf->schemaPage[n];
...
Uint32 Dbdict::computeChecksum(const Uint32 *src, Uint32 len) {
...
const SchemaFile::TableEntry *Dbdict::getTableEntry(const XSchemaFile *xsf,
                                                    Uint32 tableId) {
  Uint32 n = tableId / NDB_SF_PAGE_ENTRIES;
  Uint32 i = tableId % NDB_SF_PAGE_ENTRIES;
  ndbrequire(n < xsf->noOfPages);
...
const Dbdict::OpInfo Dbdict::CreateFileRec::g_opInfo = {
    {'C', 'F', 'l', 0},
...
void Dbdict::execCREATE_FILE_REQ(Signal *signal) {
  jamEntry();
  if (!assembleFragments(signal)) {
    jam();
    return;
  ...
  D("execCREATE_FILE_REQ");

  const CreateFileReq req_copy = *(const CreateFileReq *)signal->getDataPtr();
...
bool Dbdict::createFile_seize(SchemaOpPtr op_ptr) {
  return seizeOpRec<CreateFileRec>(op_ptr);
}

void Dbdict::createFile_release(SchemaOpPtr op_ptr) {
  releaseOpRec<CreateFileRec>(op_ptr);
}

// CreateFile: PARSE

void Dbdict::createFile_parse(Signal *signal, bool master, SchemaOpPtr op_ptr,
                              SectionHandle &handle, ErrorInfo &error) {
  jam();
  ...
  if (fg_ptr.p->m_type == DictTabInfo::Tablespace && f.FileSizeHi == 0 &&
  ...
  if (fg_ptr.p->m_type == DictTabInfo::Tablespace) {
    ...
    if (filePtr.p->m_file_size % page_size != 0 && !ERROR_INSERTED(6030)) {
      jam();
      filePtr.p->m_file_size /= page_size;
      filePtr.p->m_file_size *= page_size;
      createFilePtr.p->m_warningFlags |= CreateFileConf::WarnDatafileRoundDown;
    }
    if (filePtr.p->m_file_size % extent_size != 0 && !ERROR_INSERTED(6030)) {
      jam();
  ...
  send_event(signal, trans_ptr, NDB_LE_CreateSchemaObject, impl_req->file_id,
             impl_req->file_version, f.FileType);
...
void Dbdict::createFile_abortParse(Signal *signal, SchemaOpPtr op_ptr) {
  CreateFileRecPtr createFilePtr;
  getOpRec(op_ptr, createFilePtr);
  CreateFileImplReq *impl_req = &createFilePtr.p->m_request;
...
bool Dbdict::createFile_subOps(Signal *signal, SchemaOpPtr op_ptr) {
  return false;
}

void Dbdict::createFile_reply(Signal *signal, SchemaOpPtr op_ptr,
                              ErrorInfo error) {
  SchemaTransPtr trans_ptr = op_ptr.p->m_trans_ptr;
...
void Dbdict::createFile_prepare(Signal *signal, SchemaOpPtr op_ptr) {
  jam();
  ...
  Callback cb = {safe_cast(&Dbdict::createFile_fromWriteObjInfo),
                 op_ptr.p->op_key};
...
void Dbdict::createFile_fromWriteObjInfo(Signal *signal, Uint32 op_key,
                                         Uint32 ret) {
  SchemaOpPtr op_ptr;
  CreateFileRecPtr createFileRecPtr;
...
void Dbdict::createFile_abortPrepare(Signal *signal, SchemaOpPtr op_ptr) {
  CreateFileImplReq *req = (CreateFileImplReq *)signal->getDataPtrSend();
  FilePtr f_ptr;
  FilegroupPtr fg_ptr;
...
void Dbdict::createFile_commit(Signal *signal, SchemaOpPtr op_ptr) {
  jam();
...
void Dbdict::createFile_complete(Signal *signal, SchemaOpPtr op_ptr) {
  jam();
  sendTransConf(signal, op_ptr);
}

void Dbdict::createFile_fromLocal(Signal *signal, Uint32 op_key, Uint32 ret) {
  jam();
  SchemaOpPtr op_ptr;
  CreateFileRecPtr createFilePtr;
...
void Dbdict::execCREATE_FILE_IMPL_REF(Signal *signal) {
  jamEntry();
  CreateFileImplRef *ref = (CreateFileImplRef *)signal->getDataPtr();
  D("execCREATE_FILE_IMPL_REF");
  handleDictRef(signal, ref);
}

void Dbdict::execCREATE_FILE_IMPL_CONF(Signal *signal) {
  jamEntry();
  CreateFileImplConf *conf = (CreateFileImplConf *)signal->getDataPtr();
  D("execCREATE_FILE_IMPL_CONF");
  handleDictConf(signal, conf);
}
...
const Dbdict::OpInfo Dbdict::CreateFilegroupRec::g_opInfo = {
    {'C', 'F', 'G', 0},
...
bool Dbdict::createFilegroup_seize(SchemaOpPtr op_ptr) {
  return seizeOpRec<CreateFilegroupRec>(op_ptr);
}

void Dbdict::createFilegroup_release(SchemaOpPtr op_ptr) {
  releaseOpRec<CreateFilegroupRec>(op_ptr);
}

// CreateFilegroup: PARSE

void Dbdict::createFilegroup_parse(Signal *signal, bool master,
                                   SchemaOpPtr op_ptr, SectionHandle &handle,
                                   ErrorInfo &error) {
  jam();
  ...
  fg.init();
  SimpleProperties::UnpackStatus status = SimpleProperties::unpack(
      it, &fg, DictFilegroupInfo::Mapping, DictFilegroupInfo::MappingSize);
  ...
  switch (fg.FilegroupType) {
    case DictTabInfo::Tablespace: {
      ...
      if (fg_ptr.p->m_tablespace.m_extent_size % page_size != 0 &&
        ...
        createFilegroupPtr.p->m_warningFlags |=
            CreateFilegroupConf::WarnExtentRoundUp;
      }
#if defined VM_TRACE || defined ERROR_INSERT
    ...
    case DictTabInfo::LogfileGroup: {
      ...
      if (fg_ptr.p->m_logfilegroup.m_undo_buffer_size % page_size != 0 &&
        ...
        createFilegroupPtr.p->m_warningFlags |=
            CreateFilegroupConf::WarnUndobufferRoundUp;
      }
#if defined VM_TRACE || defined ERROR_INSERT
...
void Dbdict::createFilegroup_abortParse(Signal *signal, SchemaOpPtr op_ptr) {
  CreateFilegroupRecPtr createFilegroupPtr;
  getOpRec(op_ptr, createFilegroupPtr);
...
bool Dbdict::createFilegroup_subOps(Signal *signal, SchemaOpPtr op_ptr) {
  return false;
}

void Dbdict::createFilegroup_reply(Signal *signal, SchemaOpPtr op_ptr,
                                   ErrorInfo error) {
  jam();
...
void Dbdict::createFilegroup_prepare(Signal *signal, SchemaOpPtr op_ptr) {
  jam();
  ...
  Callback cb = {safe_cast(&Dbdict::createFilegroup_fromWriteObjInfo),
                 op_ptr.p->op_key};
...
void Dbdict::createFilegroup_fromWriteObjInfo(Signal *signal, Uint32 op_key,
                                              Uint32 ret) {
  SchemaOpPtr op_ptr;
  CreateFilegroupRecPtr createFilegroupRecPtr;
  ...
  Callback c = {safe_cast(&Dbdict::createFilegroup_fromLocal),
                op_ptr.p->op_key};
  op_ptr.p->m_callback = c;
}
...
void Dbdict::createFilegroup_commit(Signal *signal, SchemaOpPtr op_ptr) {
  jam();
...
void Dbdict::createFilegroup_complete(Signal *signal, SchemaOpPtr op_ptr) {
  jam();
  sendTransConf(signal, op_ptr);
}

void Dbdict::createFilegroup_abortPrepare(Signal *signal, SchemaOpPtr op_ptr) {
  CreateFilegroupRecPtr createFilegroupRecPtr;
  getOpRec(op_ptr, createFilegroupRecPtr);
  CreateFilegroupImplReq *impl_req = &createFilegroupRecPtr.p->m_request;
...
void Dbdict::createFilegroup_fromLocal(Signal *signal, Uint32 op_key,
                                       Uint32 ret) {
  jam();
  SchemaOpPtr op_ptr;
  CreateFilegroupRecPtr createFilegroupPtr;
...
void Dbdict::execCREATE_FILEGROUP_IMPL_REF(Signal *signal) {
  jamEntry();
  CreateFilegroupImplRef *ref = (CreateFilegroupImplRef *)signal->getDataPtr();
  D("execCREATE_FILEGROUP_IMPL_REF");
  handleDictRef(signal, ref);
}

void Dbdict::execCREATE_FILEGROUP_IMPL_CONF(Signal *signal) {
  jamEntry();
  CreateFilegroupImplConf *conf =
      (CreateFilegroupImplConf *)signal->getDataPtr();
  D("execCREATE_FILEGROUP_IMPL_CONF");
  handleDictConf(signal, conf);
}
...
const Dbdict::OpInfo Dbdict::DropFileRec::g_opInfo = {
    {'D', 'F', 'l', 0},
    ~RT_DBDICT_DROP_FILE,
...
void Dbdict::execDROP_FILE_REQ(Signal *signal) {
  jamEntry();
  if (!assembleFragments(signal)) {
    jam();
    return;
  ...
  const DropFileReq req_copy = *(const DropFileReq *)signal->getDataPtr();
...
bool Dbdict::dropFile_seize(SchemaOpPtr op_ptr) {
  return seizeOpRec<DropFileRec>(op_ptr);
}

void Dbdict::dropFile_release(SchemaOpPtr op_ptr) {
  releaseOpRec<DropFileRec>(op_ptr);
}

// DropFile: PARSE

void Dbdict::dropFile_parse(Signal *signal, bool master, SchemaOpPtr op_ptr,
                            SectionHandle &handle, ErrorInfo &error) {
  SchemaTransPtr trans_ptr = op_ptr.p->m_trans_ptr;
  ...
  if (check_write_obj(impl_req->file_id, trans_ptr.p->m_transId,
                      SchemaFile::SF_DROP, error)) {
    jam();
    return;
  }
...
void Dbdict::dropFile_abortParse(Signal *signal, SchemaOpPtr op_ptr) {
  jam();
  sendTransConf(signal, op_ptr);
}

bool Dbdict::dropFile_subOps(Signal *signal, SchemaOpPtr op_ptr) {
  return false;
}

void Dbdict::dropFile_reply(Signal *signal, SchemaOpPtr op_ptr,
                            ErrorInfo error) {
  jam();
  SchemaTransPtr trans_ptr = op_ptr.p->m_trans_ptr;
  DropFileRecPtr dropFileRecPtr;
  ...
  if (!hasError(error)) {
    ...
    Uint32 clientRef = op_ptr.p->m_clientRef;
    sendSignal(clientRef, GSN_DROP_FILE_REF, signal, DropFileRef::SignalLength,
               JBB);
  }
}
...
void Dbdict::dropFile_prepare(Signal *signal, SchemaOpPtr op_ptr) {
  jam();
  ...
  Callback c = {safe_cast(&Dbdict::dropFile_fromLocal), op_ptr.p->op_key};
  op_ptr.p->m_callback = c;
...
void Dbdict::dropFile_abortPrepare(Signal *signal, SchemaOpPtr op_ptr) {
  DropFileRecPtr dropFilePtr;
...
void Dbdict::dropFile_commit(Signal *signal, SchemaOpPtr op_ptr) {
  jam();
...
void Dbdict::dropFile_complete(Signal *signal, SchemaOpPtr op_ptr) {
  jam();
...
void Dbdict::dropFile_fromLocal(Signal *signal, Uint32 op_key, Uint32 ret) {
  jam();
  SchemaOpPtr op_ptr;
  DropFileRecPtr dropFilePtr;
...
void Dbdict::execDROP_FILE_IMPL_REF(Signal *signal) {
  jamEntry();
  DropFileImplRef *ref = (DropFileImplRef *)signal->getDataPtr();
  D("execDROP_FILE_IMPL_REF");
  handleDictRef(signal, ref);
}

void Dbdict::execDROP_FILE_IMPL_CONF(Signal *signal) {
  jamEntry();
  DropFileImplConf *conf = (DropFileImplConf *)signal->getDataPtr();
  D("execDROP_FILE_IMPL_CONF");
  handleDictConf(signal, conf);
}

void Dbdict::send_drop_file(Signal *signal, Uint32 op_key, Uint32 fileId,
  ...
  sendSignal(ref, GSN_DROP_FILE_IMPL_REQ, signal, DropFileImplReq::SignalLength,
             JBB);
}
...
const Dbdict::OpInfo Dbdict::DropFilegroupRec::g_opInfo = {
    {'D', 'F', 'g', 0},
...
void Dbdict::execDROP_FILEGROUP_REQ(Signal *signal) {
  jamEntry();
  if (!assembleFragments(signal)) {
    jam();
    return;
...
bool Dbdict::dropFilegroup_seize(SchemaOpPtr op_ptr) {
  return seizeOpRec<DropFilegroupRec>(op_ptr);
}

void Dbdict::dropFilegroup_release(SchemaOpPtr op_ptr) {
  releaseOpRec<DropFilegroupRec>(op_ptr);
}

// DropFilegroup: PARSE

void Dbdict::dropFilegroup_parse(Signal *signal, bool master,
                                 SchemaOpPtr op_ptr, SectionHandle &handle,
                                 ErrorInfo &error) {
  SchemaTransPtr trans_ptr = op_ptr.p->m_trans_ptr;
  ...
  if (check_write_obj(impl_req->filegroup_id, trans_ptr.p->m_transId,
                      SchemaFile::SF_DROP, error)) {
    jam();
    return;
  }
...
void Dbdict::dropFilegroup_abortParse(Signal *signal, SchemaOpPtr op_ptr) {
  jam();
  sendTransConf(signal, op_ptr);
}

bool Dbdict::dropFilegroup_subOps(Signal *signal, SchemaOpPtr op_ptr) {
  return false;
}

void Dbdict::dropFilegroup_reply(Signal *signal, SchemaOpPtr op_ptr,
                                 ErrorInfo error) {
  jam();
  SchemaTransPtr trans_ptr = op_ptr.p->m_trans_ptr;
  DropFilegroupRecPtr dropFilegroupRecPtr;
...
void Dbdict::dropFilegroup_prepare(Signal *signal, SchemaOpPtr op_ptr) {
  jam();
  SchemaTransPtr trans_ptr = op_ptr.p->m_trans_ptr;
  DropFilegroupRecPtr dropFilegroupRecPtr;
  getOpRec(op_ptr, dropFilegroupRecPtr);
  ...
  Callback c = {safe_cast(&Dbdict::dropFilegroup_fromLocal), op_ptr.p->op_key};
  op_ptr.p->m_callback = c;
  ...
  if (fg_ptr.p->m_type == DictTabInfo::LogfileGroup) {
    ...
    for (list.first(filePtr); !filePtr.isNull(); list.next(filePtr)) {
      ...
      ndbrequire(c_obj_pool.getPtr(objPtr, filePtr.p->m_obj_ptr_i));
...
void Dbdict::dropFilegroup_abortPrepare(Signal *signal, SchemaOpPtr op_ptr) {
  ndbabort();
  DropFilegroupRecPtr dropFilegroupPtr;
...
void Dbdict::dropFilegroup_commit(Signal *signal, SchemaOpPtr op_ptr) {
  jam();
  DropFilegroupRecPtr dropFilegroupRecPtr;
  getOpRec(op_ptr, dropFilegroupRecPtr);
  DropFilegroupImplReq *impl_req = &dropFilegroupRecPtr.p->m_request;
  ...
  if (fg_ptr.p->m_type == DictTabInfo::LogfileGroup) {
    ...
    FilegroupPtr lg_ptr;
    ndbrequire(
        find_object(lg_ptr, fg_ptr.p->m_tablespace.m_default_logfile_group_id));
...
void Dbdict::dropFilegroup_complete(Signal *signal, SchemaOpPtr op_ptr) {
  jam();
...
void Dbdict::dropFilegroup_fromLocal(Signal *signal, Uint32 op_key,
                                     Uint32 ret) {
  jam();
  SchemaOpPtr op_ptr;
  DropFilegroupRecPtr dropFilegroupPtr;
...
void Dbdict::execDROP_FILEGROUP_IMPL_REF(Signal *signal) {
  jamEntry();
  DropFilegroupImplRef *ref = (DropFilegroupImplRef *)signal->getDataPtr();
  D("execDROP_FILEGROUP_IMPL_REF");
  handleDictRef(signal, ref);
}

void Dbdict::execDROP_FILEGROUP_IMPL_CONF(Signal *signal) {
  jamEntry();
  DropFilegroupImplConf *conf = (DropFilegroupImplConf *)signal->getDataPtr();
  D("execDROP_FILEGROUP_IMPL_CONF");
  handleDictConf(signal, conf);
}

void Dbdict::send_drop_fg(Signal *signal, Uint32 op_key, Uint32 filegroupId,
...
const Dbdict::OpInfo Dbdict::CreateNodegroupRec::g_opInfo = {
    {'C', 'N', 'G', 0},
...
void Dbdict::execCREATE_NODEGROUP_REQ(Signal *signal) {
  jamEntry();
  if (!assembleFragments(signal)) {
    jam();
    return;
  ...
  do {
    ...
    for (Uint32 i = 0;
         i < NDB_ARRAY_SIZE(req->nodes) && i < NDB_ARRAY_SIZE(impl_req->nodes);
         i++) {
...
bool Dbdict::createNodegroup_seize(SchemaOpPtr op_ptr) {
  return seizeOpRec<CreateNodegroupRec>(op_ptr);
}

void Dbdict::createNodegroup_release(SchemaOpPtr op_ptr) {
  releaseOpRec<CreateNodegroupRec>(op_ptr);
}

// CreateNodegroup: PARSE

void Dbdict::createNodegroup_parse(Signal *signal, bool master,
                                   SchemaOpPtr op_ptr, SectionHandle &handle,
                                   ErrorInfo &error) {
  SchemaTransPtr trans_ptr = op_ptr.p->m_trans_ptr;
  CreateNodegroupRecPtr createNodegroupRecPtr;
...
void Dbdict::createNodegroup_abortParse(Signal *signal, SchemaOpPtr op_ptr) {
  jam();
  sendTransConf(signal, op_ptr);
}

bool Dbdict::createNodegroup_subOps(Signal *signal, SchemaOpPtr op_ptr) {
  SchemaTransPtr trans_ptr = op_ptr.p->m_trans_ptr;
  CreateNodegroupRecPtr createNodegroupRecPtr;
  getOpRec(op_ptr, createNodegroupRecPtr);
  // CreateNodegroupImplReq* impl_req = &createNodegroupRecPtr.p->m_request;
  ...
  if (createNodegroupRecPtr.p->m_map_created == false) {
    ...
    Uint32 buckets = c_default_hashmap_size;
    Uint32 fragments =
        get_default_fragments(signal, NDB_DEFAULT_PARTITION_BALANCE, 1);
    char buf[MAX_TAB_NAME_SIZE + 1];
    ...
    Callback c = {safe_cast(&Dbdict::createNodegroup_fromCreateHashMap),
                  op_ptr.p->op_key};
    op_ptr.p->m_callback = c;
...
void Dbdict::createNodegroup_fromCreateHashMap(Signal *signal, Uint32 op_key,
                                               Uint32 ret) {
  SchemaOpPtr op_ptr;
  CreateNodegroupRecPtr createNodegroupRecPtr;
  ...
  if (ret == 0) {
    ...
    const CreateHashMapRef *ref =
        (const CreateHashMapRef *)signal->getDataPtr();
...
void Dbdict::createNodegroup_reply(Signal *signal, SchemaOpPtr op_ptr,
                                   ErrorInfo error) {
  jam();
  SchemaTransPtr trans_ptr = op_ptr.p->m_trans_ptr;
  CreateNodegroupRecPtr createNodegroupRecPtr;
...
void Dbdict::createNodegroup_prepare(Signal *signal, SchemaOpPtr op_ptr) {
  jam();
  CreateNodegroupRecPtr createNodegroupRecPtr;
  getOpRec(op_ptr, createNodegroupRecPtr);
  CreateNodegroupImplReq *impl_req = &createNodegroupRecPtr.p->m_request;
  ...
  Callback c = {safe_cast(&Dbdict::createNodegroup_fromBlockSubStartStop),
                op_ptr.p->op_key};
  op_ptr.p->m_callback = c;
...
void Dbdict::createNodegroup_fromBlockSubStartStop(Signal *signal,
...
void Dbdict::createNodegroup_toLocal(Signal *signal, SchemaOpPtr op_ptr) {
  CreateNodegroupRecPtr createNodegroupRecPtr;
  getOpRec(op_ptr, createNodegroupRecPtr);
  CreateNodegroupImplReq *impl_req = &createNodegroupRecPtr.p->m_request;
  ...
  if (blockIndex == createNodegroupRecPtr.p->m_blockCnt) {
    ...
    if (op_ptr.p->m_state == SchemaOp::OS_PREPARING) {
       ...
       */
      Callback c = {safe_cast(&Dbdict::createNodegroup_fromWaitGCP),
                    op_ptr.p->op_key};
      op_ptr.p->m_callback = c;
  ...
  Callback c = {safe_cast(&Dbdict::createNodegroup_fromLocal),
                op_ptr.p->op_key};
  op_ptr.p->m_callback = c;

  Uint32 ref = createNodegroupRecPtr.p->m_blockNo[blockIndex];
  CreateNodegroupImplReq *req =
      (CreateNodegroupImplReq *)signal->getDataPtrSend();
...
void Dbdict::createNodegroup_fromLocal(Signal *signal, Uint32 op_key,
                                       Uint32 ret) {
  SchemaOpPtr op_ptr;
  CreateNodegroupRecPtr createNodegroupRecPtr;
  ndbrequire(findSchemaOp(op_ptr, createNodegroupRecPtr, op_key));
...
void Dbdict::createNodegroup_abortPrepare(Signal *signal, SchemaOpPtr op_ptr) {
  CreateNodegroupRecPtr createNodegroupRecPtr;
  getOpRec(op_ptr, createNodegroupRecPtr);
  // CreateNodegroupImplReq* impl_req = &createNodegroupRecPtr.p->m_request;
...
void Dbdict::createNodegroup_commit(Signal *signal, SchemaOpPtr op_ptr) {
  jam();
  CreateNodegroupRecPtr createNodegroupRecPtr;
  getOpRec(op_ptr, createNodegroupRecPtr);
  CreateNodegroupImplReq *impl_req = &createNodegroupRecPtr.p->m_request;
...
void Dbdict::createNodegroup_fromWaitGCP(Signal *signal, Uint32 op_key,
                                         Uint32 ret) {
  jam();
  SchemaOpPtr op_ptr;
  CreateNodegroupRecPtr createNodegroupRecPtr;
...
void Dbdict::createNodegroup_complete(Signal *signal, SchemaOpPtr op_ptr) {
  jam();
  CreateNodegroupRecPtr createNodegroupRecPtr;
  getOpRec(op_ptr, createNodegroupRecPtr);
  CreateNodegroupImplReq *impl_req = &createNodegroupRecPtr.p->m_request;
...
void Dbdict::execCREATE_NODEGROUP_IMPL_REF(Signal *signal) {
  jamEntry();
  CreateNodegroupImplRef *ref = (CreateNodegroupImplRef *)signal->getDataPtr();
  D("execCREATE_NODEGROUP_IMPL_REF");
  handleDictRef(signal, ref);
}

void Dbdict::execCREATE_NODEGROUP_IMPL_CONF(Signal *signal) {
  jamEntry();
  CreateNodegroupImplConf *conf =
      (CreateNodegroupImplConf *)signal->getDataPtr();
  D("execCREATE_NODEGROUP_IMPL_CONF");
  handleDictConf(signal, conf);
}

void Dbdict::execCREATE_HASH_MAP_REF(Signal *signal) {
  jamEntry();
  CreateHashMapRef *ref = (CreateHashMapRef *)signal->getDataPtr();
  D("execCREATE_HASH_MAP_REF");
  handleDictRef(signal, ref);
}

void Dbdict::execCREATE_HASH_MAP_CONF(Signal *signal) {
  jamEntry();
  CreateHashMapConf *conf = (CreateHashMapConf *)signal->getDataPtr();
  D("execCREATE_HASH_MAP_CONF");
  handleDictConf(signal, conf);
}
...
const Dbdict::OpInfo Dbdict::DropNodegroupRec::g_opInfo = {
    {'D', 'N', 'G', 0},
...
void Dbdict::execDROP_NODEGROUP_REQ(Signal *signal) {
  jamEntry();
  if (!assembleFragments(signal)) {
    jam();
    return;
...
bool Dbdict::dropNodegroup_seize(SchemaOpPtr op_ptr) {
  return seizeOpRec<DropNodegroupRec>(op_ptr);
}

void Dbdict::dropNodegroup_release(SchemaOpPtr op_ptr) {
  releaseOpRec<DropNodegroupRec>(op_ptr);
}

// DropNodegroup: PARSE

void Dbdict::dropNodegroup_parse(Signal *signal, bool master,
                                 SchemaOpPtr op_ptr, SectionHandle &handle,
                                 ErrorInfo &error) {
  SchemaTransPtr trans_ptr = op_ptr.p->m_trans_ptr;
  DropNodegroupRecPtr dropNodegroupRecPtr;
...
void Dbdict::dropNodegroup_abortParse(Signal *signal, SchemaOpPtr op_ptr) {
  jam();
  sendTransConf(signal, op_ptr);
}

bool Dbdict::dropNodegroup_subOps(Signal *signal, SchemaOpPtr op_ptr) {
  return false;
}

void Dbdict::dropNodegroup_reply(Signal *signal, SchemaOpPtr op_ptr,
                                 ErrorInfo error) {
  jam();
  SchemaTransPtr trans_ptr = op_ptr.p->m_trans_ptr;
  DropNodegroupRecPtr dropNodegroupRecPtr;
...
void Dbdict::dropNodegroup_prepare(Signal *signal, SchemaOpPtr op_ptr) {
  jam();
  DropNodegroupRecPtr dropNodegroupRecPtr;
  getOpRec(op_ptr, dropNodegroupRecPtr);
  DropNodegroupImplReq *impl_req = &dropNodegroupRecPtr.p->m_request;
  ...
  Callback c = {safe_cast(&Dbdict::dropNodegroup_fromBlockSubStartStop),
                op_ptr.p->op_key};
  op_ptr.p->m_callback = c;
...
void Dbdict::dropNodegroup_fromBlockSubStartStop(Signal *signal, Uint32 op_key,
                                                 Uint32 ret) {
  SchemaOpPtr op_ptr;
  DropNodegroupRecPtr dropNodegroupRecPtr;
...
void Dbdict::dropNodegroup_toLocal(Signal *signal, SchemaOpPtr op_ptr) {
  DropNodegroupRecPtr dropNodegroupRecPtr;
  getOpRec(op_ptr, dropNodegroupRecPtr);
  DropNodegroupImplReq *impl_req = &dropNodegroupRecPtr.p->m_request;
  ...
  if (blockIndex == dropNodegroupRecPtr.p->m_blockCnt) {
    ...
    if (op_ptr.p->m_state == SchemaOp::OS_PREPARING) {
       ...
       * Block GCP as last part of prepare
       */
      Callback c = {safe_cast(&Dbdict::dropNodegroup_fromWaitGCP),
                    op_ptr.p->op_key};
      op_ptr.p->m_callback = c;
  ...
  Callback c = {safe_cast(&Dbdict::dropNodegroup_fromLocal), op_ptr.p->op_key};
  op_ptr.p->m_callback = c;
...
void Dbdict::dropNodegroup_fromLocal(Signal *signal, Uint32 op_key,
                                     Uint32 ret) {
  SchemaOpPtr op_ptr;
  DropNodegroupRecPtr dropNodegroupRecPtr;
  ndbrequire(findSchemaOp(op_ptr, dropNodegroupRecPtr, op_key));
...
void Dbdict::dropNodegroup_abortPrepare(Signal *signal, SchemaOpPtr op_ptr) {
  DropNodegroupRecPtr dropNodegroupRecPtr;
  getOpRec(op_ptr, dropNodegroupRecPtr);
  // DropNodegroupImplReq* impl_req = &dropNodegroupRecPtr.p->m_request;
...
void Dbdict::dropNodegroup_commit(Signal *signal, SchemaOpPtr op_ptr) {
  jam();
  DropNodegroupRecPtr dropNodegroupRecPtr;
  getOpRec(op_ptr, dropNodegroupRecPtr);
  DropNodegroupImplReq *impl_req = &dropNodegroupRecPtr.p->m_request;
...
void Dbdict::dropNodegroup_fromWaitGCP(Signal *signal, Uint32 op_key,
                                       Uint32 ret) {
  jam();
  SchemaOpPtr op_ptr;
  DropNodegroupRecPtr dropNodegroupRecPtr;
...
void Dbdict::dropNodegroup_complete(Signal *signal, SchemaOpPtr op_ptr) {
  jam();
...
void Dbdict::execDROP_NODEGROUP_IMPL_REF(Signal *signal) {
  jamEntry();
  DropNodegroupImplRef *ref = (DropNodegroupImplRef *)signal->getDataPtr();
  D("execDROP_NODEGROUP_IMPL_REF");
  handleDictRef(signal, ref);
}

void Dbdict::execDROP_NODEGROUP_IMPL_CONF(Signal *signal) {
  jamEntry();
  DropNodegroupImplConf *conf = (DropNodegroupImplConf *)signal->getDataPtr();
  D("execDROP_NODEGROUP_IMPL_CONF");
  handleDictConf(signal, conf);
}
...
const Dbdict::OpInfo Dbdict::CreateFKRec::g_opInfo = {
    {'C', 'F', 'K', 0},
    ~RT_DBDICT_CREATE_FK,
...
void Dbdict::execCREATE_FK_REQ(Signal *signal) {
  jamEntry();
  if (!assembleFragments(signal)) {
    jam();
    return;
  ...
  const CreateFKReq req_copy = *(const CreateFKReq *)signal->getDataPtr();
...
bool Dbdict::createFK_seize(SchemaOpPtr op_ptr) {
  return seizeOpRec<CreateFKRec>(op_ptr);
}

void Dbdict::createFK_release(SchemaOpPtr op_ptr) {
  releaseOpRec<CreateFKRec>(op_ptr);
}

// CreateFK: PARSE

void Dbdict::createFK_parse(Signal *signal, bool master, SchemaOpPtr op_ptr,
                            SectionHandle &handle, ErrorInfo &error) {
  D("createFK_parse");
  SchemaTransPtr trans_ptr = op_ptr.p->m_trans_ptr;
  CreateFKRecPtr createFKRecPtr;
  ...
  fk.init();
  SimpleProperties::UnpackStatus status = SimpleProperties::unpack(
      it, &fk, DictForeignKeyInfo::Mapping, DictForeignKeyInfo::MappingSize);
   ...
   * child has Blobs
   */
  if (fk.OnDeleteAction == NDB_FK_CASCADE && !op_ptr.p->m_restart) {
    jam();
    TableRecordPtr childTablePtr;
    ndbrequire(find_object(childTablePtr, fk.ChildTableId));
    ...
    for (list.first(attrPtr); !attrPtr.isNull(); list.next(attrPtr)) {
      ...
      if (attrType == NDB_TYPE_BLOB || attrType == NDB_TYPE_TEXT) {
        jam();

        setError(error, CreateFKRef::ChildTableHasBlobsAndCascadeDelete,
                 __LINE__);
        return;
      }
    }
...
void Dbdict::packFKIntoPages(SimpleProperties::Writer &w,
                             Ptr<ForeignKeyRec> fk_ptr) {
  D("packFKIntoPages");
  DictForeignKeyInfo::ForeignKey fk;
  fk.init();
...
void Dbdict::createFK_abortParse(Signal *signal, SchemaOpPtr op_ptr) {
...
bool Dbdict::createFK_subOps(Signal *signal, SchemaOpPtr op_ptr) {
  D("createFK_subOps");
  SchemaTransPtr trans_ptr = op_ptr.p->m_trans_ptr;
  CreateFKRecPtr createFKRecPtr;
  getOpRec(op_ptr, createFKRecPtr);
  ...
  if (createFKRecPtr.p->m_sub_create_trigger < 2) {
     ...
     */
    Callback c = {safe_cast(&Dbdict::createFK_fromCreateTrigger),
                  op_ptr.p->op_key};
    op_ptr.p->m_callback = c;
  ...
  if (createFKRecPtr.p->m_sub_build_fk == false) {
    jam();
    Callback c = {safe_cast(&Dbdict::createFK_fromBuildFK), op_ptr.p->op_key};
    op_ptr.p->m_callback = c;
    ...
    sendSignal(reference(), GSN_BUILD_FK_REQ, signal, BuildFKReq::SignalLength,
               JBB);
    return true;
  }
...
void Dbdict::createFK_toCreateTrigger(Signal *signal, SchemaOpPtr op_ptr) {
...
void Dbdict::createFK_fromCreateTrigger(Signal *signal, Uint32 op_key,
                                        Uint32 ret) {
  ...
  if (ret == 0) {
    ...
    const CreateTrigConf *conf =
        CAST_CONSTPTR(CreateTrigConf, signal->getDataPtr());
    Ptr<ForeignKeyRec> fk_ptr;
    ndbrequire(find_object(fk_ptr, impl_req->fkId));
    ...
    const CreateTrigRef *ref =
        CAST_CONSTPTR(CreateTrigRef, signal->getDataPtr());
    ErrorInfo error;
    setError(error, ref);
    abortSubOps(signal, op_ptr, error);
  }
...
void Dbdict::createFK_fromBuildFK(Signal *signal, Uint32 op_key, Uint32 ret) {
  ...
  if (ret == 0) {
    ...
    jam();
    const BuildFKRef *ref = CAST_CONSTPTR(BuildFKRef, signal->getDataPtr());
    ErrorInfo error;
    setError(error, ref);
    abortSubOps(signal, op_ptr, error);
  }
...
void Dbdict::createFK_reply(Signal *signal, SchemaOpPtr op_ptr,
                            ErrorInfo error) {
  D("createFK_reply");
  jam();
  SchemaTransPtr trans_ptr = op_ptr.p->m_trans_ptr;
  ...
  if (!hasError(error)) {
    ...
    Uint32 clientRef = op_ptr.p->m_clientRef;
    sendSignal(clientRef, GSN_CREATE_FK_REF, signal, CreateFKRef::SignalLength,
               JBB);
  }
}
...
void Dbdict::createFK_writeTableConf(Signal *signal, Uint32 op_key,
  ...
  Callback c = {safe_cast(&Dbdict::createFK_prepareFromLocal),
                op_ptr.p->op_key};
  op_ptr.p->m_callback = c;
...
void Dbdict::createFK_abortPrepare(Signal *signal, SchemaOpPtr op_ptr) {
  ...
  if (createFKRecPtr.p->m_prepared) {
    Callback c = {safe_cast(&Dbdict::createFK_abortPrepareFromLocal),
                  op_ptr.p->op_key};
    op_ptr.p->m_callback = c;
...
void Dbdict::createFK_abortPrepareFromLocal(Signal *signal, Uint32 op_key,
                                            Uint32 ret) {
...
void Dbdict::createFK_commit(Signal *signal, SchemaOpPtr op_ptr) {
  D("createFK_commit");
  jam();
  CreateFKRecPtr createFKRecPtr;
  getOpRec(op_ptr, createFKRecPtr);
...
void Dbdict::createFK_complete(Signal *signal, SchemaOpPtr op_ptr) {
  D("createFK_complete");
  jam();
  CreateFKRecPtr createFKRecPtr;
  getOpRec(op_ptr, createFKRecPtr);
...
void Dbdict::execCREATE_FK_REF(Signal *signal) {
  jamEntry();
...
void Dbdict::execCREATE_FK_CONF(Signal *signal) {
  jamEntry();
  CreateFKConf *conf = (CreateFKConf *)signal->getDataPtr();
  D("execCREATE_FK_IMPL_CONF");
  handleDictConf(signal, conf);
}

void Dbdict::execCREATE_FK_IMPL_REF(Signal *signal) {
  jamEntry();
  CreateFKImplRef *ref = (CreateFKImplRef *)signal->getDataPtr();
  D("execCREATE_FK_IMPL_REF");
  handleDictRef(signal, ref);
}

void Dbdict::execCREATE_FK_IMPL_CONF(Signal *signal) {
  jamEntry();
  D("execCREATE_FK_IMPL_CONF");
...
const Dbdict::OpInfo Dbdict::BuildFKRec::g_opInfo = {
    {'B', 'F', 'K', 0},
...
void Dbdict::execBUILD_FK_REQ(Signal *signal) {
  jamEntry();
  if (!assembleFragments(signal)) {
    jam();
    return;
  ...
  const BuildFKReq req_copy = *(const BuildFKReq *)signal->getDataPtr();
  ...
  sendSignal(req->senderRef, GSN_BUILD_FK_REF, signal, BuildFKRef::SignalLength,
             JBB);
}

bool Dbdict::buildFK_seize(SchemaOpPtr op_ptr) {
  return seizeOpRec<BuildFKRec>(op_ptr);
}

void Dbdict::buildFK_release(SchemaOpPtr op_ptr) {
  releaseOpRec<BuildFKRec>(op_ptr);
}

// BuildFK: PARSE

void Dbdict::buildFK_parse(Signal *signal, bool master, SchemaOpPtr op_ptr,
                           SectionHandle &handle, ErrorInfo &error) {
  D("buildFK_parse");
  BuildFKRecPtr buildFKRecPtr;
...
void Dbdict::buildFK_abortParse(Signal *signal, SchemaOpPtr op_ptr) {
  D("buildFK_abortParse");
  jam();
  sendTransConf(signal, op_ptr);
}

bool Dbdict::buildFK_subOps(Signal *signal, SchemaOpPtr op_ptr) {
  D("buildFK_subOps");
  BuildFKRecPtr buildFKRecPtr;
  getOpRec(op_ptr, buildFKRecPtr);
  // BuildFKImplReq* impl_req = &buildFKRecPtr.p->m_request;
...
void Dbdict::buildFK_reply(Signal *signal, SchemaOpPtr op_ptr,
                           ErrorInfo error) {
  D("buildFK_reply");
  jam();
  SchemaTransPtr trans_ptr = op_ptr.p->m_trans_ptr;
  ...
  if (!hasError(error)) {
    ...
    conf->transId = trans_ptr.p->m_transId;
    Uint32 clientRef = op_ptr.p->m_clientRef;
    sendSignal(clientRef, GSN_BUILD_FK_CONF, signal, BuildFKConf::SignalLength,
               JBB);
  } else {
    jam();
    BuildFKRef *ref = (BuildFKRef *)signal->getDataPtrSend();
    ...
    Uint32 clientRef = op_ptr.p->m_clientRef;
    sendSignal(clientRef, GSN_BUILD_FK_REF, signal, BuildFKRef::SignalLength,
               JBB);
  }
}
...
void Dbdict::buildFK_prepare(Signal *signal, SchemaOpPtr op_ptr) {
  D("buildFK_prepare");
  jam();
  SchemaTransPtr trans_ptr = op_ptr.p->m_trans_ptr;
  BuildFKRecPtr buildFKRecPtr;
  ...
  *req = *impl_req;
  ...
  Callback c = {safe_cast(&Dbdict::buildFK_fromLocal), op_ptr.p->op_key};
  op_ptr.p->m_callback = c;
...
void Dbdict::buildFK_fromLocal(Signal *signal, Uint32 op_key, Uint32 ret) {
  D("buildFK_fromLocal");
  jam();
  SchemaOpPtr op_ptr;
...
void Dbdict::buildFK_abortPrepare(Signal *signal, SchemaOpPtr op_ptr) {
...
void Dbdict::buildFK_commit(Signal *signal, SchemaOpPtr op_ptr) {
  D("buildFK_commit");
  jam();
  sendTransConf(signal, op_ptr);
}
...
void Dbdict::buildFK_complete(Signal *signal, SchemaOpPtr op_ptr) {
  D("buildFK_complete");
  jam();
  sendTransConf(signal, op_ptr);
}

void Dbdict::execBUILD_FK_REF(Signal *signal) {
  jamEntry();
...
void Dbdict::execBUILD_FK_CONF(Signal *signal) {
  jamEntry();
...
void Dbdict::execBUILD_FK_IMPL_REF(Signal *signal) {
  jamEntry();
  BuildFKImplRef *ref = (BuildFKImplRef *)signal->getDataPtr();
  D("execBUILD_FK_IMPL_REF");
  handleDictRef(signal, ref);
}

void Dbdict::execBUILD_FK_IMPL_CONF(Signal *signal) {
  jamEntry();
  BuildFKImplConf *conf = (BuildFKImplConf *)signal->getDataPtr();
  D("execBUILD_FK_IMPL_CONF");
  handleDictConf(signal, conf);
}
...
const Dbdict::OpInfo Dbdict::DropFKRec::g_opInfo = {
    {'D', 'F', 'K', 0},
    ~RT_DBDICT_DROP_FK,
...
void Dbdict::execDROP_FK_REQ(Signal *signal) {
  jamEntry();
  if (!assembleFragments(signal)) {
    jam();
    return;
  ...
  const DropFKReq req_copy = *(const DropFKReq *)signal->getDataPtr();
  ...
  sendSignal(req->senderRef, GSN_DROP_FK_REF, signal, DropFKRef::SignalLength,
             JBB);
}

bool Dbdict::dropFK_seize(SchemaOpPtr op_ptr) {
  return seizeOpRec<DropFKRec>(op_ptr);
}

void Dbdict::dropFK_release(SchemaOpPtr op_ptr) {
  releaseOpRec<DropFKRec>(op_ptr);
}

// DropFK: PARSE

void Dbdict::dropFK_parse(Signal *signal, bool master, SchemaOpPtr op_ptr,
                          SectionHandle &handle, ErrorInfo &error) {
  D("dropFK_parse");
  SchemaTransPtr trans_ptr = op_ptr.p->m_trans_ptr;
  DropFKRecPtr dropFKRecPtr;
...
void Dbdict::dropFK_abortParse(Signal *signal, SchemaOpPtr op_ptr) {
  D("dropFK_abortParse");
  jam();
  sendTransConf(signal, op_ptr);
}

bool Dbdict::dropFK_subOps(Signal *signal, SchemaOpPtr op_ptr) {
  D("dropFK_subOps");
  DropFKRecPtr dropFKRecPtr;
  getOpRec(op_ptr, dropFKRecPtr);
  ...
  if (dropFKRecPtr.p->m_sub_drop_trigger < 2) {
     ...
     * 1 - trigger on child table
     */
    Callback c = {safe_cast(&Dbdict::dropFK_fromDropTrigger), op_ptr.p->op_key};
    op_ptr.p->m_callback = c;
...
void Dbdict::dropFK_toDropTrigger(Signal *signal, SchemaOpPtr op_ptr,
                                  Uint32 no) {
...
void Dbdict::dropFK_fromDropTrigger(Signal *signal, Uint32 op_key, Uint32 ret) {
  ...
  if (ret == 0) {
    ...
    const DropTrigConf *conf =
        CAST_CONSTPTR(DropTrigConf, signal->getDataPtr());
    (void)conf;
    Ptr<ForeignKeyRec> fk_ptr;
    ndbrequire(find_object(fk_ptr, impl_req->fkId));
    ...
    jam();
    const DropTrigRef *ref = CAST_CONSTPTR(DropTrigRef, signal->getDataPtr());
    ErrorInfo error;
    setError(error, ref);
    abortSubOps(signal, op_ptr, error);
  }
...
void Dbdict::dropFK_reply(Signal *signal, SchemaOpPtr op_ptr, ErrorInfo error) {
  D("dropFK_reply");
  jam();
  SchemaTransPtr trans_ptr = op_ptr.p->m_trans_ptr;
  DropFKRecPtr dropFKRecPtr;
  ...
  if (!hasError(error)) {
    ...
    conf->transId = trans_ptr.p->m_transId;
    Uint32 clientRef = op_ptr.p->m_clientRef;
    sendSignal(clientRef, GSN_DROP_FK_CONF, signal, DropFKConf::SignalLength,
               JBB);
  } else {
    jam();
    DropFKRef *ref = (DropFKRef *)signal->getDataPtrSend();
    ...
    Uint32 clientRef = op_ptr.p->m_clientRef;
    sendSignal(clientRef, GSN_DROP_FK_REF, signal, DropFKRef::SignalLength,
               JBB);
  }
}
...
void Dbdict::dropFK_prepare(Signal *signal, SchemaOpPtr op_ptr) {
  D("dropFK_prepare");
  jam();
  DropFKRecPtr dropFKRecPtr;
  getOpRec(op_ptr, dropFKRecPtr);
...
void Dbdict::dropFK_abortPrepare(Signal *signal, SchemaOpPtr op_ptr) {
...
void Dbdict::dropFK_fromLocal(Signal *signal, Uint32 op_key, Uint32 ret) {
  D("dropFK_fromLocal");
  jamEntry();
...
void Dbdict::dropFK_complete(Signal *signal, SchemaOpPtr op_ptr) {
  D("dropFK_complete");
  jam();
...
void Dbdict::execDROP_FK_IMPL_REF(Signal *signal) {
  jamEntry();
  DropFKImplRef *ref = (DropFKImplRef *)signal->getDataPtr();
  D("execDROP_FK_IMPL_REF");
  handleDictRef(signal, ref);
}

void Dbdict::execDROP_FK_IMPL_CONF(Signal *signal) {
  jamEntry();
  DropFKImplConf *conf = (DropFKImplConf *)signal->getDataPtr();
  D("execDROP_FK_IMPL_CONF");
  handleDictConf(signal, conf);
}
...
int Dbdict::checkSingleUserMode(Uint32 senderRef) {
  Uint32 nodeId = refToNode(senderRef);
  return getNodeState().getSingleUserMode() &&
...
void Dbdict::setError(ErrorInfo &e, Uint32 code, Uint32 line, Uint32 nodeId,
                      Uint32 status, Uint32 key, const char *name) {
...
void Dbdict::setError(ErrorInfo &e, Uint32 code, Uint32 line,
                      const char *name) {
  setError(e, code, line, 0, 0, 0, name);
}

void Dbdict::setError(ErrorInfo &e, const ErrorInfo &e2) {
  setError(e, e2.errorCode, e2.errorLine, e2.errorNodeId);
}

void Dbdict::setError(ErrorInfo &e, const ParseDictTabInfoRecord &e2) {
  setError(e, e2.errorCode, e2.errorLine, 0, e2.status, e2.errorKey);
}

void Dbdict::setError(SchemaOpPtr op_ptr, Uint32 code, Uint32 line,
                      Uint32 nodeId) {
...
void Dbdict::setError(SchemaOpPtr op_ptr, const ErrorInfo &e2) {
  setError(op_ptr, e2.errorCode, e2.errorLine, e2.errorNodeId);
}

void Dbdict::setError(SchemaTransPtr trans_ptr, Uint32 code, Uint32 line,
                      Uint32 nodeId) {
  D("setError" << *trans_ptr.p << V(code) << V(line) << V(nodeId));
  setError(trans_ptr.p->m_error, code, line, nodeId);
}

void Dbdict::setError(SchemaTransPtr trans_ptr, const ErrorInfo &e2) {
  setError(trans_ptr, e2.errorCode, e2.errorLine, e2.errorNodeId);
}

void Dbdict::setError(TxHandlePtr tx_ptr, Uint32 code, Uint32 line,
                      Uint32 nodeId) {
  D("setError" << *tx_ptr.p << V(code) << V(line) << V(nodeId));
  setError(tx_ptr.p->m_error, code, line, nodeId);
}

void Dbdict::setError(TxHandlePtr tx_ptr, const ErrorInfo &e2) {
  setError(tx_ptr, e2.errorCode, e2.errorLine, e2.errorNodeId);
}

bool Dbdict::hasError(const ErrorInfo &e) { return e.errorCount != 0; }

void Dbdict::resetError(ErrorInfo &e) { new (&e) ErrorInfo(); }

void Dbdict::resetError(SchemaOpPtr op_ptr) {
  if (hasError(op_ptr.p->m_error)) {
    jam();
...
void Dbdict::resetError(SchemaTransPtr trans_ptr) {
...
void Dbdict::resetError(TxHandlePtr tx_ptr) {
...
const Dbdict::OpInfo *Dbdict::g_opInfoList[] = {
    &Dbdict::CreateTableRec::g_opInfo,     &Dbdict::DropTableRec::g_opInfo,
    &Dbdict::AlterTableRec::g_opInfo,      &Dbdict::CreateTriggerRec::g_opInfo,
    &Dbdict::DropTriggerRec::g_opInfo,     &Dbdict::CreateIndexRec::g_opInfo,
    &Dbdict::DropIndexRec::g_opInfo,       &Dbdict::AlterIndexRec::g_opInfo,
    &Dbdict::BuildIndexRec::g_opInfo,      &Dbdict::IndexStatRec::g_opInfo,
    &Dbdict::CreateFilegroupRec::g_opInfo, &Dbdict::CreateFileRec::g_opInfo,
    &Dbdict::DropFilegroupRec::g_opInfo,   &Dbdict::DropFileRec::g_opInfo,
    &Dbdict::CreateHashMapRec::g_opInfo,   &Dbdict::CopyDataRec::g_opInfo,
    &Dbdict::CreateNodegroupRec::g_opInfo, &Dbdict::DropNodegroupRec::g_opInfo,
    &Dbdict::CreateFKRec::g_opInfo,        &Dbdict::DropFKRec::g_opInfo,
    &Dbdict::BuildFKRec::g_opInfo,         0};

const Dbdict::OpInfo *Dbdict::findOpInfo(Uint32 gsn) {
  Uint32 i = 0;
  while (g_opInfoList[i]) {
    const OpInfo *info = g_opInfoList[i];
    if (info->m_impl_req_gsn == 0) break;
    if (info->m_impl_req_gsn == gsn) return info;
    i++;
  }
  ndbabort();
  return 0;
...
bool Dbdict::copyIn(OpSectionBufferPool &pool, OpSection &op_sec,
                    const SegmentedSectionPtr &ss_ptr) {
...
bool Dbdict::copyOut(Dbdict::OpSectionBuffer &buffer,
                     Dbdict::OpSectionBufferConstIterator &iter, Uint32 *dst,
                     Uint32 len) {
  Uint32 n = 0;
...
bool Dbdict::copyOut(OpSectionBufferPool &pool, const OpSection &op_sec,
                     SegmentedSectionPtr &ss_ptr) {
  const Uint32 size = 1024;
  Uint32 buf[size];
...
void Dbdict::release(OpSectionBufferPool &pool, OpSection &op_sec) {
  OpSectionBuffer buffer(pool, op_sec.m_head);
  buffer.release();
}
...
const Dbdict::OpInfo &Dbdict::getOpInfo(SchemaOpPtr op_ptr) {
  ndbrequire(!op_ptr.isNull());
  OpRecPtr oprec_ptr = op_ptr.p->m_oprec_ptr;
...
bool Dbdict::seizeSchemaOp(SchemaTransPtr trans_ptr, SchemaOpPtr &op_ptr,
                           Uint32 op_key, const OpInfo &info, bool linked) {
  if ((ERROR_INSERTED(6111) && (info.m_impl_req_gsn == GSN_CREATE_TAB_REQ ||
...
bool Dbdict::findSchemaOp(SchemaOpPtr &op_ptr, Uint32 op_key) {
...
void Dbdict::releaseSchemaOp(SchemaOpPtr &op_ptr) {
  D("releaseSchemaOp" << V(op_ptr.p->op_key));

  const OpInfo &info = getOpInfo(op_ptr);

  /* Release first any sections connected to this Schema Operation */
  ...
  /* Now unlink any DICT objects connected to this schema operation */
  ...
  /**
   * Now that we have released sections and objects connected to the
   * operation record specific for this schema operation, we're ready
   * to release the specific schema operation and unlink it from the
   * schema operation.
   */
  (this->*(info.m_release))(op_ptr);

  /**
   * Now we will release all schema operations and their connected records
   * associated with this schema operation. (Recursive call!)
   */
  if (!op_ptr.p->m_oplnk_ptr.isNull()) {
    jam();
  ...
  /**
   * Now we only have remaining the schema operation, so we release this,
   * but first we remove it from the hash table. Finally we ensure that
   * we don't reuse the operation pointer.
   */
  ndbrequire(op_ptr.p->m_magic == SchemaOp::DICT_MAGIC);
...
const Dbdict::OpSection &Dbdict::getOpSection(SchemaOpPtr op_ptr,
                                              Uint32 ss_no) {
  ndbrequire(ss_no < op_ptr.p->m_sections);
  return op_ptr.p->m_section[ss_no];
}

bool Dbdict::saveOpSection(SchemaOpPtr op_ptr, SectionHandle &handle,
                           Uint32 ss_no) {
  SegmentedSectionPtr ss_ptr;
...
bool Dbdict::saveOpSection(SchemaOpPtr op_ptr, SegmentedSectionPtr ss_ptr,
                           Uint32 ss_no) {
...
bool Dbdict::replaceOpSection(SchemaOpPtr op_ptr, Uint32 ss_no,
                              SegmentedSectionPtr ss_ptr) {
  ndbrequire(ss_no <= 2);
  OpSection save = op_ptr.p->m_section[ss_no];
...
void Dbdict::releaseOpSection(SchemaOpPtr op_ptr, Uint32 ss_no) {
...
void Dbdict::addSchemaOp(SchemaOpPtr op_ptr) {
  SchemaTransPtr trans_ptr = op_ptr.p->m_trans_ptr;
  LocalSchemaOp_list list(c_schemaOpPool, trans_ptr.p->m_op_list);
  list.addLast(op_ptr);
...
bool Dbdict::hasDictObject(SchemaOpPtr op_ptr) {
  OpRecPtr oprec_ptr = op_ptr.p->m_oprec_ptr;
  return oprec_ptr.p->m_obj_ptr_i != RNIL;
}

// get dict object for existing link
void Dbdict::getDictObject(SchemaOpPtr op_ptr, DictObjectPtr &obj_ptr) {
  OpRecPtr oprec_ptr = op_ptr.p->m_oprec_ptr;
  ndbrequire(oprec_ptr.p->m_obj_ptr_i != RNIL);
  ndbrequire(c_obj_pool.getPtr(obj_ptr, oprec_ptr.p->m_obj_ptr_i));
}

// create link from schema op to dict object
void Dbdict::linkDictObject(SchemaOpPtr op_ptr, DictObjectPtr obj_ptr) {
...
void Dbdict::unlinkDictObject(SchemaOpPtr op_ptr) {
  DictObjectPtr obj_ptr;
  OpRecPtr oprec_ptr = op_ptr.p->m_oprec_ptr;
  ndbrequire(oprec_ptr.p->m_obj_ptr_i != RNIL);
  ndbrequire(c_obj_pool.getPtr(obj_ptr, oprec_ptr.p->m_obj_ptr_i));
...
void Dbdict::seizeDictObject(SchemaOpPtr op_ptr, DictObjectPtr &obj_ptr,
...
bool Dbdict::findDictObject(SchemaOpPtr op_ptr, DictObjectPtr &obj_ptr,
                            const char *name) {
...
bool Dbdict::findDictObject(SchemaOpPtr op_ptr, DictObjectPtr &obj_ptr,
  ...
  if (obj_ptr_i != RNIL) {
    jam();
    ndbrequire(c_obj_pool.getPtr(obj_ptr, obj_ptr_i));
    linkDictObject(op_ptr, obj_ptr);
    return true;
  }
  return false;
...
void Dbdict::releaseDictObject(SchemaOpPtr op_ptr) {
  DictObjectPtr obj_ptr;
  getDictObject(op_ptr, obj_ptr);
...
void Dbdict::findDictObjectOp(SchemaOpPtr &op_ptr, DictObjectPtr obj_ptr) {
...
bool Dbdict::seizeSchemaTrans(SchemaTransPtr &trans_ptr, Uint32 trans_key) {
...
bool Dbdict::seizeSchemaTrans(SchemaTransPtr &trans_ptr) {
...
bool Dbdict::findSchemaTrans(SchemaTransPtr &trans_ptr, Uint32 trans_key) {
...
void Dbdict::releaseSchemaTrans(SchemaTransPtr &trans_ptr) {
  ...
  if (c_schemaTransCount == 0) {
    ...
    Resource_limit rl;
    m_ctx.m_mm.get_resource_limit(RG_SCHEMA_TRANS_MEMORY, rl);
    ndbrequire(rl.m_curr <=
               1);  // ArenaAllocator can keep one page for empty pool
#ifdef VM_TRACE
    if (getNodeState().startLevel == NodeState::SL_STARTED) check_consistency();
#endif
  }
}
...
void Dbdict::execSCHEMA_TRANS_BEGIN_REQ(Signal *signal) {
  jamEntry();
  ...
  do {
    ...
    lockReq.lockType = DictLockReq::SchemaTransLock;
    int lockError = dict_lock_trylock(&lockReq);
    debugLockInfo(signal, "SCHEMA_TRANS_BEGIN_REQ trylock", lockError);
    if (lockError != 0) {
      // remove the trans
...
void Dbdict::trans_start_recv_reply(Signal *signal, SchemaTransPtr trans_ptr) {
  jam();
...
void Dbdict::execSCHEMA_TRANS_END_REQ(Signal *signal) {
  jamEntry();
...
void Dbdict::handleClientReq(Signal *signal, SchemaOpPtr op_ptr,
                             SectionHandle &handle) {
  ...
  if (trans_ptr.p->m_state == SchemaTrans::TS_SUBOP) {
    jam();
    SchemaOpPtr baseOp;
    ndbrequire(c_schemaOpPool.getPtr(baseOp, trans_ptr.p->m_curr_op_ptr_i));
    op_ptr.p->m_base_op_ptr_i = baseOp.i;
  ...
  Uint32 i;
  for (i = 0; i < extra_length; i++) data[skip + i] = src[i];
...
void Dbdict::trans_parse_recv_reply(Signal *signal, SchemaTransPtr trans_ptr) {
  SchemaOpPtr op_ptr;
...
void Dbdict::execSCHEMA_TRANS_IMPL_CONF(Signal *signal) {
  jamEntry();
  ndbrequire(signal->getNoOfSections() == 0);
  D("execSCHEMA_TRANS_IMPL_CONF");
...
void Dbdict::execSCHEMA_TRANS_IMPL_REF(Signal *signal) {
  jamEntry();
  ndbrequire(signal->getNoOfSections() == 0);
  D("execSCHEMA_TRANS_IMPL_REF");
  ...
  SchemaTransImplRef refCopy = *(SchemaTransImplRef *)signal->getDataPtr();
...
void Dbdict::createSubOps(Signal *signal, SchemaOpPtr op_ptr, bool first) {
  ...
  if (c_restartRecord.m_complete == false) {
    jam();
    restart_nextOp(signal);
    return;
  }
...
void Dbdict::abortSubOps(Signal *signal, SchemaOpPtr op_ptr, ErrorInfo error) {
  D("abortSubOps" << *op_ptr.p << error);
  ndbrequire(c_restartRecord.m_complete);
  SchemaTransPtr trans_ptr = op_ptr.p->m_trans_ptr;
  setError(trans_ptr, error);
  trans_rollback_sp_start(signal, trans_ptr);
}

void Dbdict::trans_prepare_start(Signal *signal, SchemaTransPtr trans_ptr) {
...
void Dbdict::trans_prepare_first(Signal *signal, SchemaTransPtr trans_ptr) {
...
void Dbdict::trans_prepare_next(Signal *signal, SchemaTransPtr trans_ptr,
                                SchemaOpPtr op_ptr) {
...
void Dbdict::trans_prepare_recv_reply(Signal *signal,
                                      SchemaTransPtr trans_ptr) {
  jam();
...
void Dbdict::trans_prepare_done(Signal *signal, SchemaTransPtr trans_ptr) {
  ndbrequire(trans_ptr.p->m_state == SchemaTrans::TS_PREPARING);
...
void Dbdict::trans_abort_parse_start(Signal *signal, SchemaTransPtr trans_ptr) {
...
void Dbdict::trans_abort_parse_recv_reply(Signal *signal,
                                          SchemaTransPtr trans_ptr) {
  SchemaOpPtr op_ptr;
  ndbrequire(c_schemaOpPool.getPtr(op_ptr, trans_ptr.p->m_curr_op_ptr_i));
...
void Dbdict::check_partial_trans_abort_parse_next(SchemaTransPtr trans_ptr,
...
void Dbdict::trans_abort_parse_next(Signal *signal, SchemaTransPtr trans_ptr,
                                    SchemaOpPtr op_ptr) {
  jam();
...
void Dbdict::trans_abort_parse_done(Signal *signal, SchemaTransPtr trans_ptr) {
  ndbrequire(trans_ptr.p->m_state == SchemaTrans::TS_ABORTING_PARSE);

  trans_end_start(signal, trans_ptr);
}

void Dbdict::trans_abort_prepare_start(Signal *signal,
                                       SchemaTransPtr trans_ptr) {
  trans_ptr.p->m_state = SchemaTrans::TS_ABORTING_PREPARE;
...
void Dbdict::trans_abort_prepare_recv_reply(Signal *signal,
                                            SchemaTransPtr trans_ptr) {
  // XXX error states
...
void Dbdict::check_partial_trans_abort_prepare_next(SchemaTransPtr trans_ptr,
                                                    NdbNodeBitmask &nodes,
                                                    SchemaOpPtr op_ptr) {
  jam();
  NodeRecordPtr ownNodePtr;
...
void Dbdict::trans_abort_prepare_next(Signal *signal, SchemaTransPtr trans_ptr,
                                      SchemaOpPtr op_ptr) {
  jam();
...
void Dbdict::trans_abort_prepare_done(Signal *signal,
                                      SchemaTransPtr trans_ptr) {
  jam();
  ndbrequire(trans_ptr.p->m_state == SchemaTrans::TS_ABORTING_PREPARE);
#ifdef MARTIN
 ...
 */
void Dbdict::trans_rollback_sp_start(Signal *signal, SchemaTransPtr trans_ptr) {
  SchemaOpPtr op_ptr;
...
void Dbdict::trans_rollback_sp_recv_reply(Signal *signal,
                                          SchemaTransPtr trans_ptr) {
...
void Dbdict::trans_rollback_sp_next(Signal *signal, SchemaTransPtr trans_ptr,
                                    SchemaOpPtr op_ptr) {
  trans_ptr.p->m_curr_op_ptr_i = op_ptr.i;
...
void Dbdict::trans_rollback_sp_done(Signal *signal, SchemaTransPtr trans_ptr,
                                    SchemaOpPtr op_ptr) {
  jam();
...
void Dbdict::trans_commit_start(Signal *signal, SchemaTransPtr trans_ptr) {
...
void Dbdict::trans_commit_first(Signal *signal, SchemaTransPtr trans_ptr) {
  jam();
...
void Dbdict::trans_commit_wait_gci(Signal *signal) {
  jam();
  SchemaTransPtr trans_ptr;
  ndbrequire(c_schemaTransPool.getPtr(trans_ptr, signal->theData[1]));
...
void Dbdict::trans_commit_mutex_locked(Signal *signal, Uint32 transPtrI,
                                       Uint32 ret) {
  jamEntry();
#ifdef MARTIN
...
#endif
  SchemaTransPtr trans_ptr;
  ndbrequire(c_schemaTransPool.getPtr(trans_ptr, transPtrI));
...
void Dbdict::trans_commit_next(Signal *signal, SchemaTransPtr trans_ptr,
                               SchemaOpPtr op_ptr) {
  jam();
#ifdef MARTIN
...
void Dbdict::trans_commit_recv_reply(Signal *signal, SchemaTransPtr trans_ptr) {
  if (hasError(trans_ptr.p->m_error)) {
    jam();
...
void Dbdict::trans_commit_done(Signal *signal, SchemaTransPtr trans_ptr) {
#ifdef MARTIN
...
void Dbdict::trans_commit_mutex_unlocked(Signal *signal, Uint32 transPtrI,
                                         Uint32 ret) {
  jamEntry();
#ifdef MARTIN
...
void Dbdict::check_partial_trans_complete_start(SchemaTransPtr trans_ptr,
                                                NdbNodeBitmask &nodes) {
  jam();
  NodeRecordPtr ownNodePtr;
  c_nodes.getPtr(ownNodePtr, getOwnNodeId());
    ...
    for (unsigned i = 1; i < MAX_NDB_NODES; i++) {
      ...
      if (trans_ptr.p->m_nodes.get(i)) {
        ...
        g_eventLogger->info("Checking node %u(%u,%u)", nodePtr.i,
                            ...
                            nodePtr.p->takeOverConf.trans_state);
#endif
        if (nodePtr.p->takeOverConf.trans_state >=
            SchemaTrans::TS_FLUSH_COMPLETE) {
          jam();
#ifdef VM_TRACE
...
void Dbdict::trans_complete_start(Signal *signal, SchemaTransPtr trans_ptr) {
  jam();
...
void Dbdict::trans_complete_first(Signal *signal, SchemaTransPtr trans_ptr) {
  jam();
...
void Dbdict::trans_complete_next(Signal *signal, SchemaTransPtr trans_ptr,
                                 SchemaOpPtr op_ptr) {
...
void Dbdict::trans_complete_recv_reply(Signal *signal,
                                       SchemaTransPtr trans_ptr) {
  if (hasError(trans_ptr.p->m_error)) {
    jam();
    // kill nodes that failed COMMIT
...
void Dbdict::trans_complete_done(Signal *signal, SchemaTransPtr trans_ptr) {
  jam();
  trans_end_start(signal, trans_ptr);
}

void Dbdict::trans_end_start(Signal *signal, SchemaTransPtr trans_ptr) {
  ...
  if (ERROR_INSERTED(6050)) {
     ...
     */
    infoEvent("Simulating only node %u getting RT_END", ERROR_INSERT_EXTRA);
    rg.m_nodes.clear();
...
void Dbdict::trans_end_recv_reply(Signal *signal, SchemaTransPtr trans_ptr) {
  // unlock
  const DictLockReq &lockReq = trans_ptr.p->m_lockReq;
  Uint32 rc = dict_lock_unlock(signal, &lockReq);
  debugLockInfo(signal, "trans_end_recv_reply unlock", rc);
    ...
    case SchemaTrans::TS_STARTED:
      ...
      if (trans_ptr.p->m_rollback_op == 0) {
        ...
        g_eventLogger->info(
            ...
            trans_ptr.i, (uint)trans_ptr.p->trans_key, trans_ptr.p->m_state);
#endif
        setError(trans_ptr.p->m_error, SchemaTransEndRep::TransAborted,
                 __LINE__);
        trans_end_start(signal, trans_ptr);
        return;
      }
...
void Dbdict::execSCHEMA_TRANS_IMPL_REQ(Signal *signal) {
  jamEntry();
  if (!assembleFragments(signal)) {
    jam();
    return;
  }
  D("execSCHEMA_TRANS_IMPL_REQ");
...
void Dbdict::slave_run_start(Signal *signal, const SchemaTransImplReq *req) {
  ErrorInfo error;
  SchemaTransPtr trans_ptr;
  const Uint32 trans_key = req->transKey;

  Uint32 objId = req->start.objectId;
  if (check_read_obj(objId, 0) == 0) { /* schema file id already in use */
    jam();
    setError(error, CreateTableRef::NoMoreTableRecords, __LINE__);
    goto err;
  }
...
void Dbdict::slave_run_flush(Signal *signal, SchemaTransPtr trans_ptr,
                             const SchemaTransImplReq *req) {
  bool do_flush = false;
  const Uint32 rt = DictSignal::getRequestType(req->requestInfo);
...
void Dbdict::slave_writeSchema_conf(Signal *signal, Uint32 trans_key,
                                    Uint32 ret) {
  jamEntry();
  ndbrequire(ret == 0);
  SchemaTransPtr trans_ptr;
  ...
  if (!trans_ptr.p->m_isMaster) {
    switch (trans_ptr.p->m_state) {
      ...
      case SchemaTrans::TS_FLUSH_COMMIT: {
        ...
        // Take commit lock
        Mutex mutex(signal, c_mutexMgr, trans_ptr.p->m_commit_mutex);
        Callback c = {safe_cast(&Dbdict::slave_commit_mutex_locked),
                      trans_ptr.i};
        bool ok = mutex.lock(c);
        ndbrequire(ok);
        return;
      ...
      case SchemaTrans::TS_FLUSH_COMPLETE: {
        ...
        // Release commit lock
        Mutex mutex(signal, c_mutexMgr, trans_ptr.p->m_commit_mutex);
        Callback c = {safe_cast(&Dbdict::slave_commit_mutex_unlocked),
                      trans_ptr.i};
        mutex.unlock(c);
        return;
      }
...
void Dbdict::slave_commit_mutex_locked(Signal *signal, Uint32 transPtrI,
                                       Uint32 ret) {
  jamEntry();
#ifdef MARTIN
...
void Dbdict::slave_commit_mutex_unlocked(Signal *signal, Uint32 transPtrI,
                                         Uint32 ret) {
  jamEntry();
#ifdef MARTIN
...
void Dbdict::update_op_state(SchemaOpPtr op_ptr) {
  switch (op_ptr.p->m_state) {
    case SchemaOp::OS_PARSE_MASTER:
      jam();
      break;
...
void Dbdict::sendTransConf(Signal *signal, SchemaOpPtr op_ptr) {
  jam();
  SchemaTransPtr trans_ptr = op_ptr.p->m_trans_ptr;
...
void Dbdict::sendTransConf(Signal *signal, SchemaTransPtr trans_ptr) {
  ...
  SchemaTransImplConf *conf = (SchemaTransImplConf *)signal->getDataPtrSend();
  conf->senderRef = reference();
  ...
  if (ERROR_INSERTED(6103)) {
    ...
    data[0] = 6103;
    data[1] = masterRef;
    sendSignalWithDelay(reference(), GSN_CONTINUEB, signal, 5000,
                        2 + SchemaTransImplConf::SignalLength);
    return;
  }
...
void Dbdict::sendTransRef(Signal *signal, SchemaOpPtr op_ptr) {
  SchemaTransPtr trans_ptr = op_ptr.p->m_trans_ptr;
 ...
 */
Uint32 Dbdict::trans_log_schema_op(SchemaOpPtr op_ptr, Uint32 objectId,
...
void Dbdict::trans_log_schema_op_abort(SchemaOpPtr op_ptr) {
...
void Dbdict::trans_log_schema_op_complete(SchemaOpPtr op_ptr) {
  Uint32 objectId = op_ptr.p->m_orig_entry_id;
  if (objectId != RNIL) {
    jam();
    op_ptr.p->m_orig_entry_id = RNIL;
...
void Dbdict::sendTransClientReply(Signal *signal, SchemaTransPtr trans_ptr) {
  ...
  if (trans_ptr.p->m_clientState == TransClient::EndReq) {
    if (!hasError(trans_ptr.p->m_error)) {
      ...
      jam();
      SchemaTransEndRef *ref = (SchemaTransEndRef *)signal->getDataPtrSend();
      ref->senderRef = reference();
      ref->transId = transId;
      getError(trans_ptr.p->m_error, ref);
  ...
  if (ownNodePtr.p->nodeState == NodeRecord::NDB_MASTER_TAKEOVER) {
    ...
    jam();
    {
      SchemaTransEndRep *rep = (SchemaTransEndRep *)signal->getDataPtrSend();
...
bool Dbdict::seizeTxHandle(TxHandlePtr &tx_ptr) {
...
bool Dbdict::findTxHandle(TxHandlePtr &tx_ptr, Uint32 tx_key) {
...
void Dbdict::releaseTxHandle(TxHandlePtr &tx_ptr) {
...
void Dbdict::beginSchemaTrans(Signal *signal, TxHandlePtr tx_ptr) {
  D("beginSchemaTrans");
...
void Dbdict::endSchemaTrans(Signal *signal, TxHandlePtr tx_ptr, Uint32 flags) {
...
void Dbdict::execSCHEMA_TRANS_BEGIN_CONF(Signal *signal) {
  jamEntry();
  const SchemaTransBeginConf *conf =
      (const SchemaTransBeginConf *)signal->getDataPtr();
  D("execSCHEMA_TRANS_BEGIN_CONF");
...
void Dbdict::execSCHEMA_TRANS_BEGIN_REF(Signal *signal) {
  jamEntry();
  const SchemaTransBeginRef *ref =
      (const SchemaTransBeginRef *)signal->getDataPtr();

  D("execSCHEMA_TRANS_BEGIN_REF");
  TxHandlePtr tx_ptr;
...
void Dbdict::execSCHEMA_TRANS_END_CONF(Signal *signal) {
  jamEntry();
  const SchemaTransEndConf *conf =
      (const SchemaTransEndConf *)signal->getDataPtr();

  D("execSCHEMA_TRANS_END_CONF");
  TxHandlePtr tx_ptr;
  findTxHandle(tx_ptr, conf->transId);
  ndbrequire(!tx_ptr.isNull());
...
void Dbdict::execSCHEMA_TRANS_END_REF(Signal *signal) {
  jamEntry();
  const SchemaTransEndRef *ref =
      (const SchemaTransEndRef *)signal->getDataPtr();

  D("execSCHEMA_TRANS_END_REF");
  TxHandlePtr tx_ptr;
  findTxHandle(tx_ptr, ref->transId);
  ndbrequire(!tx_ptr.isNull());
...
void Dbdict::execSCHEMA_TRANS_END_REP(Signal *signal) {
  jamEntry();
  const SchemaTransEndRep *rep =
      (const SchemaTransEndRep *)signal->getDataPtr();

  D("execSCHEMA_TRANS_END_REP");
  TxHandlePtr tx_ptr;
 ...
 * in finishApiFail.
 */
void Dbdict::handleApiFail(Signal *signal, Uint32 failedApiNode) {
 ...
 */
void Dbdict::takeOverTransClient(Signal *signal, SchemaTransPtr trans_ptr) {
 ...
 */
void Dbdict::runTransClientTakeOver(Signal *signal, Uint32 tx_key, Uint32 ret) {
  TxHandlePtr tx_ptr;
  ...
  if (!at_end) {
    jam();
    Callback c = {safe_cast(&Dbdict::runTransClientTakeOver), tx_ptr.p->tx_key};
    tx_ptr.p->m_callback = c;
...
void Dbdict::finishApiFail(Signal *signal, TxHandlePtr tx_ptr) {
...
void Dbdict::apiFailBlockHandling(Signal *signal, Uint32 failedApiNode) {
  Callback cb = {safe_cast(&Dbdict::handleApiFailureCallback), failedApiNode};
  simBlockNodeFailure(signal, failedApiNode, cb);
}

// find callback for any key

bool Dbdict::findCallback(Callback &callback, Uint32 any_key) {
  SchemaOpPtr op_ptr;
  SchemaTransPtr trans_ptr;
  TxHandlePtr tx_ptr;
...
const Dbdict::OpInfo Dbdict::CreateHashMapRec::g_opInfo = {
    {'C', 'H', 'M', 0},
...
bool Dbdict::createHashMap_seize(SchemaOpPtr op_ptr) {
  return seizeOpRec<CreateHashMapRec>(op_ptr);
}

void Dbdict::createHashMap_release(SchemaOpPtr op_ptr) {
  releaseOpRec<CreateHashMapRec>(op_ptr);
}

void Dbdict::execCREATE_HASH_MAP_REQ(Signal *signal) {
  jamEntry();
  if (!assembleFragments(signal)) {
    jam();
    return;
...
Uint32 Dbdict::get_default_fragments(Signal *signal, Uint32 partitionBalance,
  ...
  sd->requestType =
      CheckNodeGroups::Direct | CheckNodeGroups::GetDefaultFragments;
  sd->partitionBalance = partitionBalance;
...
Uint32 Dbdict::get_default_partitions_fully_replicated(
    Signal *signal, Uint32 partitionBalance) {
  jam();

  CheckNodeGroups *sd = CAST_PTR(CheckNodeGroups, signal->getDataPtrSend());
  sd->extraNodeGroups = 0;
  sd->requestType = CheckNodeGroups::Direct |
                    CheckNodeGroups::GetDefaultFragmentsFullyReplicated;
  sd->partitionBalance = partitionBalance;
  EXECUTE_DIRECT(DBDIH, GSN_CHECKNODEGROUPSREQ, signal,
                 CheckNodeGroups::SignalLength);
  jamEntry();
...
void Dbdict::createHashMap_parse(Signal *signal, bool master,
                                 SchemaOpPtr op_ptr, SectionHandle &handle,
                                 ErrorInfo &error) {
  SchemaTransPtr trans_ptr = op_ptr.p->m_trans_ptr;
  CreateHashMapRecPtr createHashMapRecordPtr;
  ...
  if (handle.m_cnt) {
    ...
    /**
     * impl_req->fragments contains either partitionBalance or
    ...
    switch (partitionBalance) {
      ...
      case NDB_PARTITION_BALANCE_FOR_RA_BY_NODE:
        ...
        if (impl_req->requestType & CreateHashMapReq::CreateForOneNodegroup) {
          partitions =
              get_default_partitions_fully_replicated(signal, partitionBalance);
        } else {
      ...
      default:
        if (impl_req->requestType & CreateHashMapReq::CreateForOneNodegroup) {
          jam();
          /**
           * PartitionBalance_Specific is not supported for fully replicated.
           * And if it was fragment count is unknown.
           */
          setError(error, CreateTableRef::WrongPartitionBalanceFullyReplicated,
                   __LINE__);
          return;
        }
        partitions = fragments = impl_req->fragments;
        partitionBalance = NDB_PARTITION_BALANCE_SPECIFIC;
        break;
    }
    if (g_trace) {
      g_eventLogger->info("HashMap: fragments: %u, partitionBalance: %s",
                          fragments,
    ...
    BaseString::snprintf(hm.HashMapName, sizeof(hm.HashMapName),
                         "DEFAULT-HASHMAP-%u-%u", buckets, partitions);
  ...
  if (objptr != 0) {
    ...
    if (check_write_obj(objptr->m_id, trans_ptr.p->m_transId,
  ...
  ndbrequire(link_object(obj_ptr, hm_ptr));
  ...
  map_ptr.p->m_object_id = objId;
  {
    ...
    for (Uint32 i = 0; i < hm.HashMapBuckets; i++) {
      ...
      if (hm.HashMapValues[i] > tmp) tmp = hm.HashMapValues[i];
    }
...
void Dbdict::createHashMap_abortParse(Signal *signal, SchemaOpPtr op_ptr) {
...
bool Dbdict::createHashMap_subOps(Signal *signal, SchemaOpPtr op_ptr) {
  jam();
  return false;
}

void Dbdict::createHashMap_reply(Signal *signal, SchemaOpPtr op_ptr,
                                 ErrorInfo error) {
  jam();
...
void Dbdict::createHashMap_prepare(Signal *signal, SchemaOpPtr op_ptr) {
  jam();
...
void Dbdict::createHashMap_writeObjConf(Signal *signal, Uint32 op_key,
                                        Uint32 ret) {
  jam();
  SchemaOpPtr op_ptr;
  CreateHashMapRecPtr createHashMapRecordPtr;
...
void Dbdict::createHashMap_commit(Signal *signal, SchemaOpPtr op_ptr) {
  jam();
...
void Dbdict::createHashMap_complete(Signal *signal, SchemaOpPtr op_ptr) {
  jam();
  sendTransConf(signal, op_ptr);
}
...
void Dbdict::createHashMap_abortPrepare(Signal *signal, SchemaOpPtr op_ptr) {
  D("createHashMap_abortPrepare" << *op_ptr.p);
  // wl3600_todo
  sendTransConf(signal, op_ptr);
}

void Dbdict::packHashMapIntoPages(SimpleProperties::Writer &w,
                                  HashMapRecordPtr hm_ptr) {
  DictHashMapInfo::HashMap hm;
  hm.init();

  Ptr<Hash2FragmentMap> map_ptr;
  ndbrequire(g_hash_map.getPtr(map_ptr, hm_ptr.p->m_map_ptr_i));
...
NdbOut &operator<<(NdbOut &out, const Dbdict::ErrorInfo &a) {
  a.print(out);
  return out;
}

void Dbdict::ErrorInfo::print(NdbOut &out) const {
  out << "[";
  out << " code: " << errorCode;
  out << " line: " << errorLine;
...
void Dbdict::ErrorInfo::print(EventLogger *logger) const {
  logger->error(
...
NdbOut &operator<<(NdbOut &out, const Dbdict::DictObject &a) {
  a.print(out);
  return out;
}

void Dbdict::DictObject::print(NdbOut &out) const {
...
NdbOut &operator<<(NdbOut &out, const Dbdict::SchemaOp &a) {
  a.print(out);
  return out;
}

void Dbdict::SchemaOp::print(NdbOut &out) const {
  // Dbdict* dict = (Dbdict*)globalData.getBlock(DBDICT);
  const Dbdict::OpInfo &info = m_oprec_ptr.p->m_opInfo;
  out << " (SchemaOp";
  out << " " << info.m_opType;
  out << dec << V(op_key);
  if (m_error.errorCode != 0) out << m_error;
  if (m_sections != 0) out << dec << V(m_sections);
...
NdbOut &operator<<(NdbOut &out, const Dbdict::SchemaTrans &a) {
  a.print(out);
  return out;
}

void Dbdict::SchemaTrans::print(NdbOut &out) const {
  out << " (SchemaTrans";
  out << dec << V(m_state);
  out << dec << V(trans_key);
...
NdbOut &operator<<(NdbOut &out, const Dbdict::TxHandle &a) {
  a.print(out);
  return out;
}

void Dbdict::TxHandle::print(NdbOut &out) const {
  out << " (TxHandle";
  out << dec << V(tx_key);
  out << hex << V(m_transId);
...
void Dbdict::check_consistency() {
  ...
  TriggerRecordPtr triggerPtr;
  for (Uint32 id = 0; id < c_triggerRecordPool_.getSize(); id++) {
    bool ok = find_object(triggerPtr, id);
    if (!ok) continue;
    switch (triggerPtr.p->triggerState) {
...
void Dbdict::check_consistency_entry(TableRecordPtr tablePtr) {
...
void Dbdict::check_consistency_trigger(TriggerRecordPtr triggerPtr) {
  ...
  if (triggerPtr.p->indexId != RNIL) {
    ...
    ndbrequire(ti.triggerType == TriggerType::REORG_TRIGGER ||
               ti.triggerType == TriggerType::FULLY_REPLICATED_TRIGGER);
  }
}

void Dbdict::check_consistency_object(DictObjectPtr obj_ptr) {
  ndbrequire(obj_ptr.p->m_trans_key == 0);
  ndbrequire(obj_ptr.p->m_op_ref_count == 0);
}
...
void Dbdict::send_event(Signal *signal, SchemaTransPtr &trans_ptr, Uint32 ev,
                        Uint32 id, Uint32 version, Uint32 type) {
  if (!trans_ptr.p->m_isMaster) {
    return;
  }
...
void Dbdict::startNextGetTabInfoReq(Signal *signal) {
  jam();
