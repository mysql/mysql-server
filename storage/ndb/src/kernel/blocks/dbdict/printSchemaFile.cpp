...
static void usage() {
  ndbout << "Usage: " << progname << " [-aceq]"
         ...
         << "Example: " << progname
         << " -ceq ndb_*_fs/D[12]/DBDICT/P0.SchemaLog" << endl;
}
...
static int print_head(const char *filename, const SchemaFile *sf) {
  ...
  if (!checkonly) {
    ...
    ndbout_c("Magic: %.*s ByteOrder: %.8x NdbVersion: %s FileSize: %d",
             (int)sizeof(sf->Magic), sf->Magic, sf->ByteOrder,
             version(sf->NdbVersion), sf->FileSize);
  }
  ...
  if ((sf->NdbVersion >> 16) < 4 || (sf->NdbVersion >> 16) > 9) {
    ndbout << filename << ": impossible version " << hex << sf->NdbVersion
           << endl;
    retcode = 1;
  }
...
static int print_old(const char *filename, const SchemaFile *sf, Uint32 sz) {
  ...
  for (Uint32 i = 0; i < sf->NoOfTableEntries; i++) {
    ...
    if (allflag || (te.m_tableState != SchemaFile::SF_UNUSED)) {
      if (!checkonly)
        ndbout << "Table " << i << ":"
               ...
               << " type = " << te.m_tableType
               << " noOfPages = " << te.m_noOfPages << " gcp: " << te.m_gcp
               << endl;
    }
  }
  return retcode;
...
static int print(const char *filename, const SchemaFile *xsf, Uint32 sz) {
  ...
  if (print_head(filename, xsf) != 0) retcode = 1;
  ...
  for (Uint32 n = 0; n < noOfPages; n++) {
    ...
    if (sf->FileSize != xsf->FileSize) {
      ndbout << filename << ": page " << n << " FileSize changed to "
             << sf->FileSize << "!=" << xsf->FileSize << endl;
      retcode = 1;
    }
    Uint32 cs = 0;
    ...
    if (sf->NoOfTableEntries != NDB_SF_PAGE_ENTRIES) {
      ndbout << filename << ": page " << n << " invalid NoOfTableEntries "
             << sf->NoOfTableEntries << endl;
      retcode = 1;
    }
    for (Uint32 i = 0; i < NDB_SF_PAGE_ENTRIES; i++) {
      ...
      if (!transok && te.m_transId != 0) {
        ndbout << filename << ": entry " << j
               << ": active transaction, transId: " << hex << te.m_transId
               << endl;
        entryerr = true;
        retcode = 1;
      }
      ...
      for (Uint32 k = 0; k < num_unused; k++) {
        if (te.m_unused[k] != 0) {
          ndbout << filename << ": entry " << j << ": garbage in unused word "
                 << k << ": " << te.m_unused[k] << endl;
          entryerr = true;
          retcode = 1;
        }
      }
      if (allflag || (te.m_tableState != SchemaFile::SF_UNUSED) || entryerr) {
        if (!checkonly || entryerr)
          ndbout << "Table " << j << ":"
                 ...
                 << "(" << table_version_minor(te.m_tableVersion) << ")"
                 << " type = " << te.m_tableType
                 << " noOfWords = " << te.m_info_words << " gcp: " << te.m_gcp
