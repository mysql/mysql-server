...
extern int ndb_mgm_listen_event_internal(NdbMgmHandle, const int filter[], int,
                                         ndb_socket_t *);
...
extern "C" NdbLogEventHandle ndb_mgm_create_logevent_handle(
  ...
  auto h = new (std::nothrow)
      ndb_logevent_handle(ndb_mgm_listen_event_internal(mh, filter, 1));
...
extern "C" socket_t ndb_logevent_get_fd(const NdbLogEventHandle h) {
...
static int ref_to_node(int ref) { return ref & 0xFFFF; }
...
struct Ndb_logevent_body_row ndb_logevent_body[] = {
    ...
    ROW(EventBufferStatus2, "latest_consumed_epoch_l", 4,
        latest_consumed_epoch_l),
    ROW(EventBufferStatus2, "latest_consumed_epoch_h", 5,
        latest_consumed_epoch_h),
    ROW(EventBufferStatus2, "latest_buffered_epoch_l", 6,
        latest_buffered_epoch_l),
    ROW(EventBufferStatus2, "latest_buffered_epoch_h", 7,
        latest_buffered_epoch_h),
    ...
    ROW(EventBufferStatus3, "latest_consumed_epoch_l", 4,
        latest_consumed_epoch_l),
    ROW(EventBufferStatus3, "latest_consumed_epoch_h", 5,
        latest_consumed_epoch_h),
    ROW(EventBufferStatus3, "latest_buffered_epoch_l", 6,
        latest_buffered_epoch_l),
    ROW(EventBufferStatus3, "latest_buffered_epoch_h", 7,
        latest_buffered_epoch_h),
...
#define ROW2(a, b) \
  { a, offsetof(struct ndb_logevent, b), sizeof(((struct ndb_logevent *)0)->b) }
...
extern "C" int ndb_logevent_get_next(const NdbLogEventHandle h,
                                     struct ndb_logevent *dst,
                                     unsigned timeout_in_milliseconds) {
  int rc = ndb_logevent_get_next2(h, dst, timeout_in_milliseconds);
  if (rc == 1) {
    /**
...
extern "C" int ndb_logevent_get_next2(const NdbLogEventHandle h,
                                      struct ndb_logevent *dst,
                                      unsigned timeout_in_milliseconds) {
  ...
  */
  ...
  if (EventLoggerBase::event_lookup(dst->type, category, level, severity,
                                    text_fn)) {
  ...
  for (i = 0; ndb_logevent_body[i].token; i++) {
    if (ndb_logevent_body[i].type == dst->type) break;
  }
...
extern "C" int ndb_logevent_get_latest_error(const NdbLogEventHandle h) {
  return h->m_error;
}

extern "C" const char *ndb_logevent_get_latest_error_msg(
    const NdbLogEventHandle h) {
