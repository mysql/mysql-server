...
/* Static functions */
Uint32
ConfigSection::loc_mod4_v1(Uint32 len)
{
  return len + (4 - (len % 4));
}

Uint32
ConfigSection::loc_mod4_v2(Uint32 len)
{
  return len + ((4 - (len & 3)) & 3);
}

void
ConfigSection::set_checksum(Uint32 *packed_ptr, Uint32 len_words)
{
  Uint32 checksum = 0;
...
void
ConfigSection::create_int_value(Uint32 **v1_ptr, Uint32 val)
{
  val = htonl(val);
...
void
ConfigSection::create_v1_entry_key(Uint32 **v1_ptr,
                                   Uint32 type,
                                   Uint32 key,
                                   Uint32 section_id)
{
...
void
ConfigSection::create_v2_entry_key(Uint32 **v2_ptr,
                                   Uint32 type,
                                   Uint32 key)
{
...
Uint32
ConfigSection::read_v2_int_value(const Uint32 **src)
{
...
Uint32
ConfigSection::Entry::get_v2_length() const
{
  Uint32 len = 0;
  switch (m_type)
  {
...
Uint32
ConfigSection::Entry::get_v1_length() const
{
  Uint32 len = 0;
  switch (m_type)
  {
...
void
ConfigSection::Entry::create_v1_entry(Uint32 **v1_ptr,
...
void
ConfigSection::Entry::create_v2_entry(Uint32 **v2_ptr) const
{
  switch (m_type)
  {
    case IntTypeId:
    {
      create_v2_entry_key(v2_ptr,
                          IntTypeId,
                          m_key);
...
Uint32
ConfigSection::Entry::unpack_entry(const Uint32 **data)
{
...
bool
ConfigSection::Entry::equal(Entry *cmp) const
{
  if (m_type != cmp->m_type ||
      m_key != cmp->m_key)
  {
    return false;
  }
...
Uint32
ConfigSection::get_v2_length() const
{
  Uint32 len = 3;
  for (Uint32 i = 0; i < m_num_entries; i++)
  {
...
Uint32
ConfigSection::get_section_type_value()
{
  Uint32 val = 0;
  switch (m_section_type)
  {
...
void
ConfigSection::create_v2_section(Uint32 **v2_ptr) const
{
  check_magic();
...
ConfigSection::Entry*
ConfigSection::find_key(Uint32 key) const
{
  for (Uint32 i = 0; i < m_num_entries; i++)
  {
    Entry* curr_entry = m_entry_array[i];
    if (curr_entry->m_key == key)
      return curr_entry;
  }
  return nullptr;
}
...
bool
ConfigSection::get(Uint32 key, Entry& entry)
{
  check_magic();
...
void
ConfigSection::set_config_section_type(ConfigSectionType config_section_type)
{
  m_config_section_type = config_section_type;
}

void
ConfigSection::set_section_type(SectionType section_type)
{
  m_section_type = section_type;
}

bool
ConfigSection::set_section_type(Entry& entry)
{
  /**
...
bool ConfigSection::set_string(Entry *update_entry,
                               Entry &input_entry,
                               bool free_string)
{
...
bool
ConfigSection::set(Entry &entry, bool free_string)
{
  check_magic();
  ...
  {
    if (unlikely(found_entry->m_type !=
                 entry.m_type))
    {
...
void
ConfigSection::set_node_ids(ConfigSection::Entry *entry)
{
  if (entry->m_key == CONFIG_NODE_ID &&
  ...
  switch (get_section_type())
  {
    ...
    case ConfigSection::ShmTypeId:
    {
      ...
      require(entry1 != nullptr &&
              ...
              m_node1 > 0 &&
              m_node2 > 0 &&
              entry1->m_type == IntTypeId &&
              entry2->m_type == IntTypeId &&
...
void
ConfigSection::set_node_id_from_keys()
{
  switch (get_section_type())
  {
    case ConfigSection::DataNodeTypeId:
...
void
ConfigSection::handle_default_section(ConfigSection *default_section)
{
  /**
  ...
  for (Uint32 i = 0; i < m_num_entries; i++)
  {
    ...
    if (default_entry != nullptr &&
        curr_entry->equal(default_entry))
    {
      /**
