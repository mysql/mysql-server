...
{
  NdbStdOpt::help,
  { "port", 'p', "server port number", & opt_port, nullptr, nullptr,
    GET_INT, REQUIRED_ARG, opt_port, 0, 0, nullptr, 0, nullptr },
...
public:
  ...
  ~Cluster() {
    for(int i = 1 ; i < 256 ; i++) if(nc[i]) delete nc[i];
  }
...
void Session::runSession() {
  ...
  if(m_socket.associate(ssl)) {
    if(m_socket.do_tls_handshake()) {
      ...
      if(auth)
        authResult = TlsKeyManager::perform_client_host_auth(auth);
      if(authResult == 0)
        m_socket.send("M", 1);
    }
  } else NdbSocket::free_ssl(ssl);
...
public:
  ...
  bool test_connect_ok(int id) {
    return (test_connection_from(id) == 1);
  }
...
bool test_2nd_auth_unbound_name(TestTlsKeyManager & t) {
  ...
  int r1 = TlsKeyManager::check_server_host_auth(t.cert(2),
                                                 "dominique.mysql.fr");
  return (r1 == 0);
}
...
void test_secondary_auth(Test::CertAuthority & ca) {
  ...
  ok(test_2nd_auth_localhost(t),
     "    2nd auth should succeed (localhost)");
}
...
static void test_iterate(TlsKeyManager & keyMgr, int expect, int last_row) {
  ...
  while(keyMgr.iterate_cert_table(node_id, & row)) {
    ...
    printf("Node: %3d   Expires:  %d    Name: %s \n",
           node_id, (int) row.expires, row.name);
    last = node_id;
  }
