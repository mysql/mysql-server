...
struct IntIntKVStaticMethods {
  static Uint32 hashValue(const IntIntKVPod *obj) { return obj->a * 31; }

  static bool equal(const IntIntKVPod *objA, const IntIntKVPod *objB) {
    return objA->a == objB->a;
  }

  static void setNext(IntIntKVPod *from, IntIntKVPod *to) { from->next = to; }

  static IntIntKVPod *getNext(const IntIntKVPod *from) { return from->next; }
};
...
struct IntIntKVObj {
  ...
  Uint32 hashValue() const { return a * 31; }

  bool equal(const IntIntKVObj *other) const { return a == other->a; }

  void setNext(IntIntKVObj *_next) { next = _next; }

  IntIntKVObj *getNext() const { return next; }
};
...
TAPTEST(HashMap2) {
  ...
  for (int j = 1; j < 150; j++) {
    ...
    /* Test iterator Api */
    HashMap2<IntIntKVPod, true, TestHeapAllocator,
             IntIntKVStaticMethods>::Iterator it(hash1);
  ...
  printf("int -> int (Static, !unique) \n");
  for (int j = 1; j < 150; j++) {
    HashMap2<IntIntKVPod, false, TestHeapAllocator, IntIntKVStaticMethods>
        hash1;
