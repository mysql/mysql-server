/*
<<<<<<< HEAD:storage/ndb/src/common/util/ndbzio.cpp
   Copyright (c) 2007, 2017, Oracle and/or its affiliates. All rights reserved.
=======
   Copyright (c) 2007, 2021, Oracle and/or its affiliates.
>>>>>>> upstream/cluster-7.6:storage/ndb/src/common/util/ndbzio.c
...
int ndbz_open (ndbzio_stream *s, const char *path, int Flags, File fd)
{
  ...
  if(!s->inbuf)
  {
    ...
    if(err==ENOTSUP)
      s->inbuf= (Byte *)malloc(AZ_BUFSIZE_READ);
    else if(err)
      return -err;
    err= posix_memalign((void**)&(s->outbuf),512,AZ_BUFSIZE_WRITE);
    if(err==ENOTSUP)
      s->outbuf= (Byte *)malloc(AZ_BUFSIZE_WRITE);
    else if(err)
      return -err;
    s->bufalloced = 1;
  }
