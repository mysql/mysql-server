...
public:
  SSL_CTX * ctx {nullptr};
  ~SSL_CTX_owner() { if(ctx) SSL_CTX_free(ctx); }
};
...
void TlsKeyManager::initialize_context() {
  ...
  */
  ...
  /* Set the security level to level 2, requiring 112-bit security.
     ...
     256 bits.
  */
  if(SSL_CTX_get_security_level(ctx) < 2)
  ...
  */
  ...
  if(SSL_CTX_up_ref(g.ctx) != 1)
    return log_openssl_errors(-9);
  m_ctx = g.ctx;
...
int TlsKeyManager::check_server_host_auth(const NodeCertificate & nc,
  ...
  for(int n = 0; n < n_bound_hosts ; n++)
    if(nc.bound_hostname(n) == hostname)
      return 0;
...
int TlsKeyManager::check_socket_for_auth(const NdbSocket & socket,
  ...
  *pAuth = nullptr;
  ...
  if(! cert)
    return TlsKeyError::auth2_no_cert;
...
ClientAuthorization *
TlsKeyManager::test_client_auth(X509 * cert, const struct addrinfo * ai) {
...
int ClientAuthorization::run_check_name(int n) {
  ...
  if(r) {
    g_eventLogger->error(
        "TLS Authorization failure checking host name '%s': '%s'",
...
bool ClientAuthorization::compare_list(const addrinfo * ai_list) const {
  for(const addrinfo *ai = ai_list; ai != nullptr; ai = ai->ai_next) {
    ...
    if(m_sockaddr.has_same_addr(addr))
      return true;
  }
  return false;
}
...
bool TlsKeyManager::open_active_cert() {
  if(ActivePrivateKey::find(m_search_path, m_node_id, m_type, m_key_file)) {
    ...
    if(key) {
      if(ActiveCertificate::find(m_search_path, m_node_id, m_type, m_cert_file))
      {
        ...
        if(certs) {
          if(EVP_PKEY_eq(key, X509_get0_pubkey(sk_X509_value(certs, 0)))) {
            ...
            if(m_node_cert.is_signed()) {
              if(check_replace_date(1.0))
                return true;
              log_error(TlsKeyError::active_cert_expired);
            } else log_error(TlsKeyError::active_cert_invalid);
          } else log_error(TlsKeyError::active_cert_mismatch);
        } else log_error(TlsKeyError::cannot_read_active_cert);
      } else log_error(TlsKeyError::active_cert_not_found);
    } else log_error(TlsKeyError::cannot_read_active_key);
  }
  return false;
}
