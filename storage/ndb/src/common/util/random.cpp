...
static void localRandom48(DRand48Data *buffer, long int *result) {
  ...
  if (sizeof(unsigned short int) == 2) {
    X = (Uint64)buffer->x[2] << 32 | (Uint64)buffer->x[1] << 16 | buffer->x[0];
    a = ((Uint64)buffer->a[2] << 32 | (Uint64)buffer->a[1] << 16 |
         buffer->a[0]);
    ...
    X = (Uint64)buffer->x[2] << 16 | (Uint64)buffer->x[1] >> 16;
    a = (Uint64)buffer->a[2] << 16 | (Uint64)buffer->a[1] >> 16;
...
void printSequence(RandomSequence *seq, unsigned int numPerRow) {
  ...
  for (i = 0; i < seq->length; i++) {
    ...
    if ((i + 1) % numPerRow == 0) ndbout_c("%s", "");
  }

  if (i % numPerRow != 0) ndbout_c("%s", "");
}
