...
int PkiFile::assign(PathName & path, const char * dir, const char * file) {
  path.clear();
  if(dir == nullptr || dir[0] == '\0') path.append(file);
  else {
    path.append(dir);
...
int TlsSearchPath::find(const char * name) const {
  ...
  for(size_t i = 0 ; i < m_path.size() ; i++) {
    ...
    if(m_path[i].length())
      file_buf.append(DIR_SEPARATOR);
    file_buf.append(name);

    if(! file_buf.is_truncated())
      if(stat(file_buf.c_str(), &s) == 0)
        return i;
...
public:
  ...
  static int data_node_file(PkiFile::Type, PkiFile::FileName &); // ndb-data-...
...
private:
  ...
  PkiFile::FileName & current(int score) { m_score[m_size] = score;
...
int PkiFilenames::data_node_file(PkiFile::Type file_type, PkiFile::FileName &buf)
{
  buf.clear();
  const char * file_suffix = type_names[(int) file_type];
...
PkiFilenames::PkiFilenames(int, Node::Type node_type, PkiFile::Type file_type) {
  if(Node::And(node_type, Node::Type::MGMD))
    mgmd_file(file_type, current(3));
...
short PkiFilenames::find_file(const TlsSearchPath * path,
  ...
  for(int i = 0 ; i < m_size ; i++)
    if(path->find(m_list[i].c_str(), path_buffer))
      return m_score[i];
  return 0;
}

static bool promote_file(const char * pending,
                         const char * active,
                         const char * retired) {
#ifdef _WIN32
  ...
  if(link(active, retired)) { }   // this may fail if active does not exist
#endif
...
bool PrivateKey::store(EVP_PKEY * key, const PkiFile::PathName & path,
  ...
  if(PEM_write_PKCS8PrivateKey(fp, key, enc, nullptr, 0,
                               nullptr, passphrase)) {
    fclose(fp);
    return true;
  }
...
void PrivateKey::free(EVP_PKEY * key) {
  EVP_PKEY_free(key);
}
 ...
 */
short PendingPrivateKey::find(const TlsSearchPath * searchPath,
                              int node_id, Node::Type type,
 ...
 */
short ActivePrivateKey::find(const TlsSearchPath * searchPath,
                             int node_id, Node::Type type,
                             PkiFile::PathName & path_buffer) {
  ...
  CertSubject(), CertLifetime(), m_req(req)
{
  ...
  if(x) {
   STACK_OF(GENERAL_NAME) * gn =
      static_cast<struct stack_st_GENERAL_NAME *>
...
int SigningRequest::finalise(EVP_PKEY * key) {
  ...
  if(bound_hostnames()) {
    ...
    int r = X509V3_add1_i2d(&x, NID_subject_alt_name, m_bound_hostnames,
                            1, X509V3_ADD_DEFAULT);
    if(r == 0) return -20;
...
bool SigningRequest::find(const TlsSearchPath * searchPath, int node_id,
                          Node::Type node_type, PkiFile::PathName & path_buffer) {
...
void SerialNumber::free(ASN1_STRING * serial) {
...
bool Certificate::store(STACK_OF(X509) * certs,
...
void Certificate::free(X509 * c) {
  X509_free(c);
}

void Certificate::free(STACK_OF(X509) * s) {
...
static bool initClusterCertAuthority(X509 * cert, const char * ordinal) {
  ...
  int r1;
  snprintf((char *) subject, sizeof(subject),
           ClusterCertAuthority::Subject, ordinal);
...
static X509 * create_unsigned_CA(EVP_PKEY *key, const char * ordinal) {
  X509 * cert = Certificate::create(key);
  if(cert) {
    if(initClusterCertAuthority(cert, ordinal))
      return cert;
...
X509 * ClusterCertAuthority::create(EVP_PKEY *key,
                                    const char * ordinal,
                                    bool sign) {
  X509 * cert = create_unsigned_CA(key, ordinal);
  if(cert) {
    if(!sign || ClusterCertAuthority::sign(cert, key, cert))
      return cert;
    Certificate::free(cert);
  }
  return nullptr;
...
int ClusterCertAuthority::sign(X509 * issuer, EVP_PKEY * key, X509 * cert) {
  if(X509_set_issuer_name(cert, X509_get_subject_name(issuer)) == 0)
    return 0;
 ...
 */
short PendingCertificate::find(const TlsSearchPath * searchPath,
                               int node_id, Node::Type type,
                               PkiFile::PathName & path_buffer) {
 ...
 */
short ActiveCertificate::find(const TlsSearchPath * searchPath,
                              int node_id, Node::Type type,
                              PkiFile::PathName & path_buffer) {
...
CertSubject::CertSubject(const CertSubject & other) :
  ...
  m_type(other.m_type), m_cluster_id(other.m_cluster_id)
{
  m_names_owner = false;
}
...
int CertSubject::bound_hostnames() const {
  if(m_bound_hostnames)
    return sk_GENERAL_NAME_num(m_bound_hostnames);
  return 0;
}
...
bool CertSubject::parse_name(const ASN1_STRING * str) {
  ...
  if(find("Data Node", 9)) m_type = Node::Type::DB;
  else if(find("Management Node", 15)) m_type = Node::Type::MGMD;
  else if(find("Node", 4)) m_type = Node::Type::Client;
  else return false;
...
bool CertLifetime::set_lifetime(X509 * cert) {
  ...
  if(ASN1_TIME_to_tm(X509_get0_notBefore(cert), & m_notBefore) != 1)
    return false;
  if(ASN1_TIME_to_tm(X509_get0_notAfter(cert), & m_notAfter) != 1)
    return false;
...
bool CertLifetime::set_lifetime(int expire_days, int extra_days) {
  ...
  time_t duration =  (expire_days * SecondsPerDay) +
                     (extra_hours * SecondsPerHour);
...
void NodeCertificate::init_from_x509(X509 * cert)
{
  ...
  m_x509_names_set = true;
  m_signed = (bool) X509_get_signature_info(cert, nullptr, nullptr,
                                            nullptr, nullptr);
  m_final = m_signed;
}
...
const NodeCertificate * NodeCertificate::from_credentials(
    STACK_OF(X509) * certs, EVP_PKEY * key)
{
...
int NodeCertificate::finalise(X509 * CA_cert, EVP_PKEY * CA_key) {
  ...
  if(! m_x509_names_set) {
    ...
    if(bound_hostnames())
      if(!X509_add1_ext_i2d(m_x509, NID_subject_alt_name, m_bound_hostnames,
...
bool NodeCertificate::verify_signature(EVP_PKEY * CA_key) const {
  ...
  fprintf(stderr, "signed = %d, mdnid = %d, pknid = %d, secbits = %d, "
