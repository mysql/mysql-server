...
time_t File_class::mtime(const char *aFileName) {
  struct stat s;
  if (stat(aFileName, &s) != 0) return 0;
  return s.st_mtime;
}
...
bool File_class::open() { return open(m_fileName, m_fileMode); }

bool File_class::open(const char *aFileName, const char *mode) {
...
bool File_class::is_open() { return (m_file != nullptr); }

File_class::~File_class() { close(); }

bool File_class::remove() {
  // Close the file first!
  close();
  return File_class::remove(m_fileName);
}

bool File_class::close() {
  bool rc = true;
  int retval = 0;
...
int File_class::read(void *buf, size_t itemSize, size_t nitems) const {
  return (int)::fread(buf, itemSize, nitems, m_file);
}

int File_class::readChar(char *buf, long start, long length) const {
...
int File_class::write(const void *buf, size_t size_arg, size_t nitems) {
  return (int)::fwrite(buf, size_arg, nitems, m_file);
}

int File_class::writeChar(const char *buf, long start, long length) {
...
ndb_off_t File_class::size() const { return File_class::size(m_file); }

const char *File_class::getName() const { return m_fileName; }
