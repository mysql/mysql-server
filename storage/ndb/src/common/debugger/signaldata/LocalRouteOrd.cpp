...
bool printLOCAL_ROUTE_ORD(FILE *output, const Uint32 *theData, Uint32 len,
  ...
  fprintf(output, " gsn: %u(%s) prio: %u\n", sig->gsn, getSignalName(sig->gsn),
          sig->prio);

  const Uint32 *ptr = sig->path;
  fprintf(output, " path:");
  for (Uint32 i = 0; i < pathcnt; i++) {
    fprintf(output, " [ hop: 0x%x(%s) prio: %u ]", ptr[0],
            getBlockName(refToMain(ptr[0])), ptr[1]);
    ptr += 2;
  }

  fprintf(output, "\n dst:");
  for (Uint32 i = 0; i < dstcnt; i++) {
    fprintf(output, " [ 0x%x(%s) ]", ptr[0], getBlockName(refToMain(ptr[0])));
  }
  fprintf(output, "\n");
