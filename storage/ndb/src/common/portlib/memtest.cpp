...
main(int argc, char **argv) {
  ...
  switch (atoi(argv[1])) {
    case 1:
      malloctest(loopcount, memsize, 0);
      break;
    case 2:
      mmaptest(loopcount, memsize, 0);
      break;
    case 3:
      malloctest(loopcount, memsize, 1);
      break;
    case 4:
      mmaptest(loopcount, memsize, 1);
      break;
    case 5:
      freetest(loopcount, memsize);
      break;
    case 6:
      unmaptest(loopcount, memsize);
      break;
    default:
      break;
  }
...
long long getMilli() {
  ...
  return ((long long)tick_time.tv_sec) * ((long long)1000) +
...
void mmaptest(int loopcount, int memsize, int touch) {
  ...
  start = getMicro();
  for (i = 0; i < loopcount; i++) {
    ptr = mmap(0, size, PROT_READ | PROT_WRITE, MAP_PRIVATE | MAP_ANONYMOUS, 0,
               0);
    if (ptr < 0) {
