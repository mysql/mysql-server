...
void NdbOperation::initInterpreter() {
...
bool NdbOperation::isNdbRecordOperation() {
...
int NdbOperation::write_attrCheck(const NdbColumnImpl *tNdbColumnImpl) {
  if (isNdbRecordOperation()) {
    /* Wrong API.  Use NdbInterpretedCode for NdbRecord operations */
    setErrorCodeAbort(4537);
    return -1;
...
int NdbOperation::read_attrCheck(const NdbColumnImpl *tNdbColumnImpl) {
  if (isNdbRecordOperation()) {
    /* Wrong API.  Use NdbInterpretedCode for NdbRecord operations */
    setErrorCodeAbort(4537);
    return -1;
...
int NdbOperation::initial_interpreterCheck() {
  if (isNdbRecordOperation()) {
    /* Wrong API.  Use NdbInterpretedCode for NdbRecord operations */
    setErrorCodeAbort(4537);
    return -1;
...
int NdbOperation::intermediate_interpreterCheck() {
  if (isNdbRecordOperation()) {
    /* Wrong API.  Use NdbInterpretedCode for NdbRecord operations */
    setErrorCodeAbort(4537);
    return -1;
 ...
 *****************************************************************************/
int NdbOperation::incValue(const NdbColumnImpl *tNdbColumnImpl, Uint32 aValue) {
  INT_DEBUG(("incValue32 %d %u", tNdbColumnImpl->m_attrId, aValue));
  int tAttrId;

  tAttrId = incCheck(tNdbColumnImpl);
  if (tAttrId == -1) goto incValue_error1;

  // Load Attribute into register 6

  if (insertATTRINFO(Interpreter::Read(tAttrId, 6)) == -1) goto incValue_error1;
  // Load aValue into register 7
  if (aValue < 65536) {
    if (insertATTRINFO(Interpreter::LoadConst16(7, aValue)) == -1)
      goto incValue_error1;
  } else {
    if (insertATTRINFO(Interpreter::LoadConst32(7)) == -1) goto incValue_error1;
    if (insertATTRINFO(aValue) == -1) goto incValue_error1;
  }
  // Add register 6 and 7 and put result in register 7

  if (insertATTRINFO(Interpreter::Add(7, 6, 7)) == -1) goto incValue_error1;
 ...
 *                aValue : The constant to increment the attribute value with.
 ******************************************************************************/
int NdbOperation::subValue(const NdbColumnImpl *tNdbColumnImpl, Uint32 aValue) {
  INT_DEBUG(("subValue32 %d %u", tNdbColumnImpl->m_attrId, aValue));
  int tAttrId;

  tAttrId = incCheck(tNdbColumnImpl);
  if (tAttrId == -1) goto subValue_error1;

  // Load Attribute into register 6

  if (insertATTRINFO(Interpreter::Read(tAttrId, 6)) == -1) goto subValue_error1;
  // Load aValue into register 7
  if (aValue < 65536) {
    if (insertATTRINFO(Interpreter::LoadConst16(7, aValue)) == -1)
      goto subValue_error1;
  } else {
    if (insertATTRINFO(Interpreter::LoadConst32(7)) == -1) goto subValue_error1;
    if (insertATTRINFO(aValue) == -1) goto subValue_error1;
  }
  // Subtract register 6 and 7 and put result in register 7

  if (insertATTRINFO(Interpreter::Sub(7, 6, 7)) == -1) goto subValue_error1;
  if (insertATTRINFO(Interpreter::Write(tAttrId, 7)) == -1)
    goto subValue_error1;
 ...
 *                aValue : The constant to increment the attribute value with.
 *****************************************************************************/
int NdbOperation::incValue(const NdbColumnImpl *tNdbColumnImpl, Uint64 aValue) {
  ...
  if (insertATTRINFO(Interpreter::Read(tAttrId, 6)) == -1) goto incValue_error1;
  // Load aValue into register 7
  if (insertATTRINFO(Interpreter::LoadConst64(7)) == -1) goto incValue_error1;
  if (insertATTRINFOloop((Uint32 *)&aValue, 2) == -1) goto incValue_error1;
  // Add register 6 and 7 and put result in register 7
  if (insertATTRINFO(Interpreter::Add(7, 6, 7)) == -1) goto incValue_error1;
  if (insertATTRINFO(Interpreter::Write(tAttrId, 7)) == -1)
    goto incValue_error1;
 ...
 *                aValue : The constant to increment the attribute value with.
 ******************************************************************************/
int NdbOperation::subValue(const NdbColumnImpl *tNdbColumnImpl, Uint64 aValue) {
  ...
  if (insertATTRINFO(Interpreter::Read(6, tAttrId)) == -1) goto subValue_error1;
  // Load aValue into register 7
  if (insertATTRINFO(Interpreter::LoadConst64(7)) == -1) goto subValue_error1;
  if (insertATTRINFOloop((Uint32 *)&aValue, 2) == -1) goto subValue_error1;
  // Subtract register 6 and 7 and put result in register 7
  if (insertATTRINFO(Interpreter::Sub(7, 6, 7)) == -1) goto subValue_error1;
  if (insertATTRINFO(Interpreter::Write(tAttrId, 7)) == -1)
    goto subValue_error1;
 ...
 *****************************************************************************/
int NdbOperation::def_label(int tLabelNo) {
  INT_DEBUG(("def_label %d", tLabelNo));
  Uint32 tLabelIndex;
  if (labelCheck() == -1) return -1;
  ...
  if (theNoOfSubroutines > 0) {
     ...
     */
    initialOffset +=
        (theInterpretedSize + theFinalUpdateSize + theFinalReadSize);
  }

  theLastLabel->theLabelNo[tLabelIndex] = tLabelNo;
  theLastLabel->theLabelAddress[tLabelIndex] =
      (theTotalCurrAI_Len + 1) - initialOffset;
...
************************************************************************************************/
int NdbOperation::def_subroutine(int tSubNo) {
  ...
  if (theStatus == FinalGetValue) {
    ...
    if (insertATTRINFO(Interpreter::EXIT_OK) == -1) return -1;
    ...
    theFinalUpdateSize =
        theTotalCurrAI_Len - (theInitialReadSize + theInterpretedSize +
  ...
  theLastSubroutine->theSubroutineAddress[tSubroutineIndex] =
      theTotalCurrAI_Len -
      (AttrInfo::SectionSizeInfoLength + theInitialReadSize +
       theInterpretedSize + theFinalUpdateSize +
       theFinalReadSize);  // Preceding sections + sizes array
...
************************************************************************************************/
int NdbOperation::add_reg(Uint32 RegSource1, Uint32 RegSource2,
                          Uint32 RegDest) {
  INT_DEBUG(("add_reg %u %u %u", RegSource1, RegSource2, RegDest));
  if (intermediate_interpreterCheck() == -1) return -1;
...
************************************************************************************************/
int NdbOperation::sub_reg(Uint32 RegSource1, Uint32 RegSource2,
                          Uint32 RegDest) {
...
int NdbOperation::load_const_u32(Uint32 RegDest, Uint32 Constant) {
  ...
  if (initial_interpreterCheck() == -1) goto l_u32_error1;
  if (RegDest >= 8) goto l_u32_error2;
  if (insertATTRINFO(Interpreter::LoadConst32(RegDest)) == -1)
    goto l_u32_error1;
  if (insertATTRINFO(Constant) == -1) goto l_u32_error1;
  theErrorLine++;
  return 0;
...
int NdbOperation::load_const_u64(Uint32 RegDest, Uint64 Constant) {
  ...
  if (initial_interpreterCheck() == -1) return -1;
  if (RegDest >= 8) {
    setErrorCodeAbort(4229);
    return -1;
  }
  ...
  // 64 bit value
  if (insertATTRINFO(Interpreter::LoadConst64(RegDest)) == -1) return -1;
  if (insertATTRINFOloop((Uint32 *)&Constant, 2) == -1) return -1;
  theErrorLine++;
  return 0;
}
...
int NdbOperation::load_const_null(Uint32 RegDest) {
  ...
  if (insertATTRINFO(Interpreter::LOAD_CONST_NULL) == -1) return -1;
  theErrorLine++;
  return 0;
}
...
************************************************************************************************/
int NdbOperation::read_attr(const NdbColumnImpl *anAttrObject, Uint32 RegDest) {
  INT_DEBUG(("read_attr %d %u", anAttrObject->m_attrId, RegDest));
  if (initial_interpreterCheck() == -1) return -1;

  int tAttrId = read_attrCheck(anAttrObject);
  if (tAttrId == -1) goto read_attr_error1;
  if (RegDest >= 8) goto read_attr_error2;
...
int NdbOperation::write_attr(const NdbColumnImpl *anAttrObject,
  ...
  if (tAttrId == -1) return -1;
  if (insertATTRINFO(Interpreter::Write(tAttrId, RegSource)) == -1) return -1;
  theErrorLine++;
  return 0;
}

int NdbOperation::branch_reg_reg(Uint32 type, Uint32 RegLvalue,
                                 Uint32 RegRvalue, Uint32 Label) {
  if (intermediate_interpreterCheck() == -1) return -1;
  if (insertATTRINFO(Interpreter::Branch(type, RegLvalue, RegRvalue)) == -1)
    return -1;
  if (insertBranch(Label) == -1) return -1;
  theErrorLine++;
  return 0;
}

int NdbOperation::branch_ge(Uint32 RegLvalue, Uint32 RegRvalue, Uint32 Label) {
  INT_DEBUG(("branch_ge %u %u %u", RegLvalue, RegRvalue, Label));
  return branch_reg_reg(Interpreter::BRANCH_GE_REG_REG, RegLvalue, RegRvalue,
                        Label);
}

int NdbOperation::branch_gt(Uint32 RegLvalue, Uint32 RegRvalue, Uint32 Label) {
  INT_DEBUG(("branch_gt %u %u %u", RegLvalue, RegRvalue, Label));
  return branch_reg_reg(Interpreter::BRANCH_GT_REG_REG, RegLvalue, RegRvalue,
                        Label);
}

int NdbOperation::branch_le(Uint32 RegLvalue, Uint32 RegRvalue, Uint32 Label) {
  INT_DEBUG(("branch_le %u %u %u", RegLvalue, RegRvalue, Label));
  return branch_reg_reg(Interpreter::BRANCH_LE_REG_REG, RegLvalue, RegRvalue,
                        Label);
}

int NdbOperation::branch_lt(Uint32 RegLvalue, Uint32 RegRvalue, Uint32 Label) {
  INT_DEBUG(("branch_lt %u %u %u", RegLvalue, RegRvalue, Label));
  return branch_reg_reg(Interpreter::BRANCH_LT_REG_REG, RegLvalue, RegRvalue,
                        Label);
}

int NdbOperation::branch_eq(Uint32 RegLvalue, Uint32 RegRvalue, Uint32 Label) {
  INT_DEBUG(("branch_eq %u %u %u", RegLvalue, RegRvalue, Label));
  return branch_reg_reg(Interpreter::BRANCH_EQ_REG_REG, RegLvalue, RegRvalue,
                        Label);
}

int NdbOperation::branch_ne(Uint32 RegLvalue, Uint32 RegRvalue, Uint32 Label) {
  INT_DEBUG(("branch_ne %u %u %u", RegLvalue, RegRvalue, Label));
  return branch_reg_reg(Interpreter::BRANCH_NE_REG_REG, RegLvalue, RegRvalue,
                        Label);
}

int NdbOperation::branch_ne_null(Uint32 RegLvalue, Uint32 Label) {
...
int NdbOperation::branch_eq_null(Uint32 RegLvalue, Uint32 Label) {
...
int NdbOperation::branch_label(Uint32 Label) {
  ...
  if (initial_interpreterCheck() == -1) return -1;
  if (insertATTRINFO(Interpreter::BRANCH) == -1) return -1;
  if (insertBranch(Label) == -1) return -1;
  theErrorLine++;
  return 0;
}
...
int NdbOperation::interpret_exit_last_row() {
  ...
  if (initial_interpreterCheck() == -1) return -1;
  if (insertATTRINFO(Interpreter::EXIT_OK_LAST) == -1) return -1;
  theErrorLine++;
  return 0;
}
...
int NdbOperation::interpret_exit_nok() {
...
int NdbOperation::call_sub(Uint32 Subroutine) {
  ...
  if (initial_interpreterCheck() == -1) return -1;
  if (insertATTRINFO((Subroutine << 16) + Interpreter::CALL) == -1) return -1;
  if (insertCall(Subroutine) == -1) return -1;
  theErrorLine++;
  return 0;
}
...
int NdbOperation::ret_sub() {
  ...
  if (insertATTRINFO(Interpreter::RETURN) == -1) return -1;
  theStatus = SubroutineEnd;
  theErrorLine++;
  return 0;
}

int NdbOperation::insertBranch(Uint32 aLabel) {
  Uint32 tAddress;
  NdbBranch *tBranch = theNdb->getNdbBranch();
  if (tBranch == nullptr) goto insertBranch_error1;
  ...
  if (theNoOfSubroutines == 0)
    ...
    tAddress = theTotalCurrAI_Len -
               (theInitialReadSize + theInterpretedSize + theFinalUpdateSize +
                theFinalReadSize + AttrInfo::SectionSizeInfoLength);

  tBranch->theBranchAddress = tAddress;
  tBranch->theSignal = theCurrentATTRINFO;
  tBranch->theSignalAddress =
      theAI_LenInCurrAI;  // + 1;  theAI_LenInCurrAI has already been updated in
  tBranch->theSubroutine =
      theNoOfSubroutines;  //       insertATTRINFO which was done before
                           //       insertBranch!!
...
int NdbOperation::insertCall(Uint32 aCall) {
...
int NdbOperation::branch_col(Uint32 type, Uint32 ColId, const void *val,
                             Uint32 len, Uint32 Label) {
  DBUG_ENTER("NdbOperation::branch_col");
  DBUG_PRINT("enter", ("type: %u  col:%u  val: %p  len: %u  label: %u", type,
                       ColId, val, len, Label));
  ...
  if (val == nullptr)
    ...
    if (!col->getStringType()) {
      ...
      if (col->getType() == NDB_TYPE_BIT) {
        ...
        if (lastWordBits) lastWordMask = (1 << lastWordBits) - 1;
      }
       ...
       */
      if ((type != Interpreter::LIKE) && (type != Interpreter::NOT_LIKE)) {
  ...
  if (insertATTRINFO(Interpreter::BranchCol(c, nulls)) == -1) DBUG_RETURN(-1);

  if (insertBranch(Label) == -1) DBUG_RETURN(-1);
...
int NdbOperation::branch_col_eq(Uint32 ColId, const void *val, Uint32 len, bool,
                                Uint32 Label) {
  INT_DEBUG(
      ("branch_col_eq %u %.*s(%u) -> %u", ColId, len, (char *)val, len, Label));
...
int NdbOperation::branch_col_ne(Uint32 ColId, const void *val, Uint32 len, bool,
                                Uint32 Label) {
  INT_DEBUG(
      ("branch_col_ne %u %.*s(%u) -> %u", ColId, len, (char *)val, len, Label));
...
int NdbOperation::branch_col_lt(Uint32 ColId, const void *val, Uint32 len, bool,
                                Uint32 Label) {
  INT_DEBUG(
      ("branch_col_lt %u %.*s(%u) -> %u", ColId, len, (char *)val, len, Label));
...
int NdbOperation::branch_col_le(Uint32 ColId, const void *val, Uint32 len, bool,
                                Uint32 Label) {
  INT_DEBUG(
      ("branch_col_le %u %.*s(%u) -> %u", ColId, len, (char *)val, len, Label));
...
int NdbOperation::branch_col_gt(Uint32 ColId, const void *val, Uint32 len, bool,
                                Uint32 Label) {
  INT_DEBUG(
      ("branch_col_gt %u %.*s(%u) -> %u", ColId, len, (char *)val, len, Label));
...
int NdbOperation::branch_col_ge(Uint32 ColId, const void *val, Uint32 len, bool,
                                Uint32 Label) {
  INT_DEBUG(
      ("branch_col_ge %u %.*s(%u) -> %u", ColId, len, (char *)val, len, Label));
...
int NdbOperation::branch_col_null(Uint32 type, Uint32 ColId, Uint32 Label) {
  if (initial_interpreterCheck() == -1) return -1;

  if (insertATTRINFO(type) == -1) return -1;
  ...
  if (insertATTRINFO(Interpreter::BranchCol_2(attrId))) return -1;

  theErrorLine++;
  return 0;
}

int NdbOperation::branch_col_eq_null(Uint32 ColId, Uint32 Label) {
  INT_DEBUG(("branch_col_eq_null %u -> %u", ColId, Label));
  return branch_col_null(Interpreter::BRANCH_ATTR_EQ_NULL, ColId, Label);
}

int NdbOperation::branch_col_ne_null(Uint32 ColId, Uint32 Label) {
