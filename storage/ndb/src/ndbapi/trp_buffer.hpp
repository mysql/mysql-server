...
struct TFPage {
  ...
  static constexpr inline Uint32 max_data_bytes() { return SIZE; }

  inline Uint32 get_free_bytes() const { return SIZE - m_bytes; }

  inline bool is_full() const { return m_bytes == SIZE; }
...
class TFPool {
 ...
 public:
  ...
  bool init(size_t total_memory, size_t reserved_memory = 0,
            size_t page_sz = SENDBUFFER_DEFAULT_PAGE_SIZE);
  bool inited() const { return m_alloc_ptr != nullptr; }

  TFPage *try_alloc(
      Uint32 N, bool reserved = false);  // Return linked list of most N pages
  ...
  Uint64 get_total_send_buffer_size() const {
    ...
    return Uint64(m_tot_send_buffer_pages - m_reserved_send_buffer_pages) *
           m_pagesize;
  }
  Uint64 get_total_used_send_buffer_size() const {
    return Uint64(m_tot_send_buffer_pages - m_free_send_buffer_pages) *
           m_pagesize;
  }
  Uint32 get_page_size() const { return m_pagesize; }
...
class TFMTPool : private TFPool {
 ...
 public:
  ...
  bool init(size_t total_memory, size_t reserved_memory = 0,
            size_t page_sz = SENDBUFFER_DEFAULT_PAGE_SIZE) {
    return TFPool::init(total_memory, reserved_memory, page_sz);
  }
  bool inited() const { return TFPool::inited(); }
  ...
  Uint32 get_page_size() const { return TFPool::get_page_size(); }
};

inline TFPage *TFPool::try_alloc(Uint32 n, bool reserved) {
  ...
  if (unlikely(m_free_send_buffer_pages < m_reserved_send_buffer_pages + n)) {
    ...
    if (!reserved) {
      ...
      if (m_free_send_buffer_pages > m_reserved_send_buffer_pages) {
        ...
        avail_pages = m_free_send_buffer_pages - m_reserved_send_buffer_pages;
      } else {
        /* No pages available */
        avail_pages = 0;
      }
...
inline Uint32 TFPool::try_alloc(struct iovec tmp[], Uint32 cnt) {
...
inline void TFPool::release(TFPage *first, TFPage *last, Uint32 page_count) {
...
inline void TFPool::release_list(TFPage *head) {
  TFPage *tail = head;
  Uint32 page_count = 1;
  while (tail->m_next != nullptr) {
    tail = tail->m_next;
