...
static const ErrorBundle ErrorCodes[] = {
    ...
    {250, DMEC, NR,
     "Node where lock was held crashed, restart scan transaction"},
    ...
    {20011, DMEC, IE,
     "Query aborted due to both tree and parameters contain interpreted "
     "program"},
    {20012, DMEC, IE,
     "Query aborted due to invalid tree parameter specification: Key parameter "
     "bits mismatch"},
    {20013, DMEC, IE,
     "Query aborted due to invalid tree parameter specification: Incorrect key "
     "parameter count"},
     ...
     */
    {21000, HA_ERR_CANNOT_ADD_FOREIGN, AE,
     "Create foreign key failed - parent key is primary key and "
     "on-update-cascade is not allowed"},
    /* CreateFKRef + CreateFKImplRef */
    {21020, DMEC, TR,
     "Create foreign key failed in NDB - no more object records"},
    {21021, DMEC, IE, "Create foreign key failed in NDB - invalid request"},
    {21022, DMEC, SE,
     "Create foreign key failed in NDB - parent table is not table"},
    {21023, DMEC, SE,
     "Create foreign key failed in NDB - invalid parent table version"},
    {21024, DMEC, SE,
     "Create foreign key failed in NDB - child table is not table"},
    {21025, DMEC, SE,
     "Create foreign key failed in NDB - invalid child table version"},
    {21026, HA_ERR_CANNOT_ADD_FOREIGN, AE,
     "Create foreign key failed in NDB - parent index is not unique index"},
    {21027, DMEC, SE,
     "Create foreign key failed in NDB - invalid parent index version"},
    {21028, DMEC, SE,
     "Create foreign key failed in NDB - child index is not index"},
    {21029, DMEC, SE,
     "Create foreign key failed in NDB - invalid child index version"},
    {21030, DMEC, IE,
     "Create foreign key failed in NDB - object already exists in TC"},
    {21031, DMEC, IE,
     "Create foreign key failed in NDB - no more object records in TC"},
    {21032, DMEC, IE,
     "Create foreign key failed in NDB - invalid request to TC"},
    {21033, HA_ERR_CANNOT_ADD_FOREIGN, AE,
     "Create foreign key failed in NDB - No parent row found"},
    {21034, HA_ERR_CANNOT_ADD_FOREIGN, AE,
     "Create foreign key failed - child table has Blob or Text column and "
    ...
    {21041, DMEC, SE,
     "Drop foreign key failed in NDB - invalid foreign key version"},
    {21042, DMEC, SE,
     "Drop foreign key failed in NDB - foreign key not found in TC"},
    /* BuildFKRef + BuildFKImplRef */
    {21060, DMEC, AE,
     "Build foreign key failed in NDB - foreign key not found"},
    {21061, DMEC, SE,
     "Build foreign key failed in NDB - invalid foreign key version"},
    /* Referential integrity */
    {21080, HA_ERR_ROW_IS_REFERENCED, AE,
     "Drop table not allowed in NDB - referenced by foreign key on another "
     "table"},
    /* Drop index */
    {21081, HA_ERR_DROP_INDEX_FK, AE,
     "Drop index not allowed in NDB - used as parent index of a foreign key"},
    {21082, HA_ERR_DROP_INDEX_FK, AE,
     "Drop index not allowed in NDB - used as child index of a foreign key"},
    /* Misc */
    {21090, HA_ERR_CANNOT_ADD_FOREIGN, AE,
    ...
    {251, DMEC, TR,
     "Out of frag location records in TC (increase SharedGlobalMemory)"},
    {275, DMEC, TR,
     "Out of transaction records for complete phase (increase "
     "SharedGlobalMemory)"},
    {273, DMEC, TR,
     "Out of transaction markers databuffer in TC, "
     "increase SharedGlobalMemory"},
    {279, DMEC, TR,
     "Out of transaction markers in TC, increase SharedGlobalMemory"},
    {293, DMEC, TR,
     "Out of attribute buffers in TC block, increase SharedGlobalMemory"},
    {312, DMEC, TR, "Out of LongMessageBuffer"},
    {414, DMEC, TR, "414"},
    {418, DMEC, TR,
     "Out of transaction buffers in LQH, increase LongMessageBuffer"},
    ...
    {805, DMEC, TR,
     "Out of attrinfo records in tuple manager, increase LongMessageBuffer"},
    {830, DMEC, TR, "Out of add fragment operation records"},
    {873, DMEC, TR,
     "Out of transaction memory in local data manager, ordered index data "
     "(increase SharedGlobalMemory)"},
    {899, DMEC, TR, "Rowid already allocated"},
    {909, DMEC, TR,
     "Out of transaction memory in local data manager, ordered scan operation "
     "(increase SharedGlobalMemory)"},
    {921, DMEC, TR,
     "Out of transaction memory in local data manager, copy tuples (increase "
     "SharedGlobalMemory)"},
    {923, DMEC, TR, "Out of UNDO buffer memory (increase UNDO_BUFFER_SIZE)"},
    {924, DMEC, TR,
     "Out of transaction memory in local data manager, stored procedure record "
     "(increase SharedGlobalMemory)"},
    {925, DMEC, TR,
     "Out of transaction memory in local data manager, tup scan operation "
     "(increase SharedGlobalMemory)"},
    {926, DMEC, TR,
     "Out of transaction memory in local data manager, acc scan operation "
     "(increase SharedGlobalMemory)"},
    {1217, DMEC, TR,
     "Out of operation records in local data manager (increase "
     "SharedGlobalMemory)"},
    ...
    {1222, DMEC, TR,
     "Out of transaction markers in LQH, increase SharedGlobalMemory"},
    ...
    {288, DMEC, TR,
     "Out of index operations in transaction coordinator (increase "
     "SharedGlobalMemory)"},
    {289, DMEC, TR,
     "Out of transaction buffer memory in TC (increase SharedGlobalMemory)"},
    ...
    {291, DMEC, TR,
     "Out of scanfrag records in TC (increase SharedGlobalMemory)"},
    ...
    {826, HA_ERR_RECORD_FILE_FULL, IS,
     "Too many tables and attributes (increase MaxNoOfAttributes or "
     "MaxNoOfTables)"},
    {827, HA_ERR_RECORD_FILE_FULL, IS,
     "Out of memory in Ndb Kernel, table data (increase DataMemory)"},
    {889, HA_ERR_RECORD_FILE_FULL, IS,
     "Table fragment fixed data reference has reached maximum possible value "
     "(specify MAXROWS or increase no of partitions)"},
    {902, HA_ERR_RECORD_FILE_FULL, IS,
     "Out of memory in Ndb Kernel, ordered index data (increase DataMemory)"},
    {903, HA_ERR_INDEX_FILE_FULL, IS,
     "Too many ordered indexes (increase MaxNoOfOrderedIndexes)"},
    {904, HA_ERR_INDEX_FILE_FULL, IS,
    ...
    {1605, HA_ERR_RECORD_FILE_FULL, IS,
     "Out of page request records when allocating disk record"},
    {1606, HA_ERR_RECORD_FILE_FULL, IS,
     "Out of extent records when allocating disk record"},

    /**
     * TimeoutExpired
     */
    {266, HA_ERR_LOCK_WAIT_TIMEOUT, TO,
     "Time-out in NDB, probably caused by deadlock"},
    {274, HA_ERR_LOCK_WAIT_TIMEOUT, TO,
     "Time-out in NDB, probably caused by deadlock"}, /* Scan trans timeout */
    {296, HA_ERR_LOCK_WAIT_TIMEOUT, TO,
     "Time-out in NDB, probably caused by deadlock"}, /* Scan trans timeout */
    {297, HA_ERR_LOCK_WAIT_TIMEOUT, TO,
     "Time-out in NDB, probably caused by deadlock"}, /* Scan trans timeout,
                                                         temporary!! */
    {237, HA_ERR_LOCK_WAIT_TIMEOUT, TO,
     "Transaction had timed out when trying to commit it"},
    {635, HA_ERR_LOCK_WAIT_TIMEOUT, TO,
     "Lock already taken, not waiting"},  // HA_ERR_NO_WAIT_LOCK
    ...
    {410, DMEC, OL,
     "REDO log files overloaded (decrease TimeBetweenLocalCheckpoints or "
    ...
    {238, DMEC, IE, "238"},
    {240, DMEC, IE,
     "Invalid data encountered during foreign key trigger execution"},
    ...
    {788, DMEC, TR,
     "Missing schema operation at takeover of schema transaction"},
    {809, DMEC, IE, "809"},
    {812, DMEC, IE, "812"},
    {833, DMEC, IE, "833"},
    {871, DMEC, IE, "871"},
    ...
    {4345, DMEC, IE,
     "TRIX block is not available yet, probably due to node failure"},
    ...
    {851, DMEC, AE,
     "Fixed-size column offset exceeded max."
    ...
    {324, DMEC, AE,
     "Invalid node(s) specified for new nodegroup, no node in nodegroup is "
     "started"},
    {325, DMEC, AE,
     "Invalid node(s) specified for new nodegroup, node ID invalid or "
     "undefined"},
    ...
    {4604, DMEC, AE,
     "takeOverScanOp, to take over a scanned row one must explicitly request "
     "keyinfo on readTuples call"},
    ...
    {721, HA_ERR_TABLE_EXIST, OE,
     "Schema object with given name already exists"},
    ...
    {743, HA_WRONG_CREATE_OPTION, SE,
     "Unsupported character set in table or index"},
    {744, DMEC, SE, "Character string is invalid for given character set"},
    {745, HA_WRONG_CREATE_OPTION, SE,
     "Distribution key not supported for char attribute (use binary "
     "attribute)"},
    {771, HA_WRONG_CREATE_OPTION, AE,
    ...
    {911, DMEC, SE,
     "Index stat scan requested on index with unsupported key size"},
    ...
    {1504, DMEC, SE,
     "Out of logbuffer memory(specify smaller undo_buffer_size or increase "
     "SharedGlobalMemory)"},
    ...
    {1517, DMEC, SE,
     "Insufficient disk page buffer memory. Increase DiskPageBufferMemory or "
    ...
    {799, HA_WRONG_CREATE_OPTION, SE,
     "Non default partitioning without partitions"},
    ...
    {1308, DMEC, CE,
     "Data node/s configured to have encryption but password not provided"},
    {1309, DMEC, CE,
     "Encryption password has bad character/s (see 'HELP START BACKUP')"},
    {1310, DMEC, CE,
     "Encryption password is too long (see 'HELP START BACKUP')"},
    ...
    {1350, DMEC, TR,
     "Backup failed: file already exists (use 'START BACKUP <backup id>')"},
    ...
    {1428, DMEC, IE,
     "No replica to scan on this node (internal index stats error)"},
    {1429, DMEC, IE,
     "Subscriber node undefined in SubStartReq (config change?)"},
    ...
    {4315, DMEC, AE,
     "No more key attributes allowed after defining variable length key "
     "attribute"},
    ...
    {4340, DMEC, AE,
     "Result or attribute record must be a base table ndbrecord, not an index "
     "ndbrecord"},
    ...
    {4515, DMEC, AE,
     "Method not allowed for NdbRecord, use OperationOptions or ScanOptions "
     "structure instead"},
    ...
    {4536, DMEC, AE,
     "NdbScanFilter constructor taking NdbOperation is not supported for "
     "NdbRecord"},
    {4537, DMEC, AE,
    ...
    {4540, DMEC, AE,
     "Attempt to pass an Index column to createRecord.  Use base table columns "
     "only"},
    ...
    {4233, DMEC, AE,
     "Calling execute (synchronous) when already prepared asynchronous "
     "transaction exists"},
    ...
    {4251, HA_ERR_FOUND_DUPP_UNIQUE, AE,
    ...
    {4255, DMEC, AE,
     "Hash index attributes must be specified in same order as table "
     "attributes"},
    {4258, DMEC, AE,
    ...
    {4335, DMEC, AE,
     "Only one autoincrement column allowed per table. Having a table without "
     "primary key uses an autoincremented hidden key, i.e. a table without a "
    ...
    {4299, DMEC, AE,
     "Incorrect combination of ScanOption flags, extraGetValues ptr and "
     "numExtraGetValues"},
    ...
    {5008 /* INVALID_TRACE_NUMBER */, DMEC, AE, "Invalid trace number."},
    {5010 /* INVALID_BLOCK_NAME */, DMEC, AE, "Invalid block name"},
