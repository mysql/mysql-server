...
Ndb_local_table_info *Ndb_local_table_info::create(NdbTableImpl *table_impl,
                                                   Uint32 sz) {
...
LocalDictCache::LocalDictCache() { m_tableHash.createHashTable(); }

LocalDictCache::~LocalDictCache() { m_tableHash.releaseHashTable(); }

Ndb_local_table_info *LocalDictCache::get(const BaseString &name) {
  ASSERT_NOT_MYSQLD;
  return m_tableHash.getData(name.c_str(), name.length());
}

void LocalDictCache::put(const BaseString &name,
                         Ndb_local_table_info *tab_info) {
  ASSERT_NOT_MYSQLD;
  const Uint32 id = tab_info->m_table_impl->m_id;
  m_tableHash.insertKey(name.c_str(), name.length(), id, tab_info);
}

void LocalDictCache::drop(const BaseString &name) {
  ASSERT_NOT_MYSQLD;
...
GlobalDictCache::GlobalDictCache() {
  ...
  if (f_invalid_table == nullptr) f_invalid_table = new NdbTableImpl();
  if (f_altered_table == nullptr) f_altered_table = new NdbTableImpl();
...
GlobalDictCache::~GlobalDictCache() {
  ...
  while (curr != nullptr) {
    ...
    for (unsigned i = 0; i < sz; i++) {
      ...
      DBUG_PRINT("  ", ("vers[%d]: ver: %d, refCount: %d, status: %d", i,
                        tv.m_version, tv.m_refCount, tv.m_status));
...
void GlobalDictCache::printCache() {
  ...
  NdbElement_t<Vector<TableVersion>> *curr = m_tableHash.getNext(nullptr);
  while (curr != nullptr) {
    DBUG_PRINT("curr", ("len: %d, hash: %d, lk: %d, str: %s", curr->len,
                        curr->hash, curr->localkey1, (char *)curr->str));
    if (curr->theData) {
      Vector<TableVersion> *vers = curr->theData;
      const unsigned sz = vers->size();
      for (unsigned i = 0; i < sz; i++) {
...
NdbTableImpl *GlobalDictCache::get(const BaseString &name, int *error) {
    ...
    switch (ver->m_status) {
      case OK:
        ...
        DBUG_PRINT("info", ("Table OK tab: %p  version=%x.%x refCount=%u",
                            ver->m_impl, ver->m_impl->m_version & 0xFFFFFF,
                            ver->m_impl->m_version >> 24, ver->m_refCount));
...
NdbTableImpl *GlobalDictCache::put(const BaseString &name, NdbTableImpl *tab) {
  ...
  DBUG_PRINT(
      "enter",
      ("tab: %p  name: %s, internal_name: %s version: %x.%x", tab, name.c_str(),
       tab ? tab->m_internalName.c_str() : "tab NULL",
       tab ? tab->m_version & 0xFFFFFF : 0, tab ? tab->m_version >> 24 : 0));

  Vector<TableVersion> *vers = m_tableHash.getData(name.c_str(), name.length());
  if (vers == nullptr) {
...
unsigned GlobalDictCache::get_size() {
  NdbElement_t<Vector<TableVersion>> *curr = m_tableHash.getNext(nullptr);
  int sz = 0;
  while (curr != nullptr) {
...
void GlobalDictCache::invalidate_all() {
...
void GlobalDictCache::invalidateDb(const char *name, size_t len) {
...
void GlobalDictCache::release(const NdbTableImpl *tab, int invalidate) {
  ...
  DBUG_PRINT("enter",
             ("tab: %p  internal_name: %s", tab, tab->m_internalName.c_str()));

  unsigned i;
  Vector<TableVersion> *vers = m_tableHash.getData(
      tab->m_internalName.c_str(), tab->m_internalName.length());
  if (vers == nullptr) {
    // Should always tried to retrieve it first
    // and thus there should be a record
    abort();
...
void GlobalDictCache::alter_table_rep(const BaseString &name, Uint32 tableId,
  ...
  for (Uint32 i = 0; i < sz; i++) {
    ...
    if (ver.m_version == tableVersion && ver.m_impl &&
      ...
      ver.m_status = DROPPED;
      ver.m_impl->m_status = altered ? NdbDictionary::Object::Altered
...
int GlobalDictCache::chg_ref_count(const NdbTableImpl *impl, int value) {
  DBUG_ENTER("GlobalDictCache::chg_ref_count");

  Vector<TableVersion> *vers = m_tableHash.getData(
      impl->m_internalName.c_str(), impl->m_internalName.length());
  if (vers == nullptr) {
    DBUG_RETURN(-1);
  }
