...
private:
  ...
  STATIC_CONST( MAX_PROXIMITY_GROUP = INT32_MAX );
  STATIC_CONST( INVALID_PROXIMITY_GROUP = INT32_MIN );
  STATIC_CONST( DATA_NODE_NEIGHBOUR_PROXIMITY_ADJUSTMENT = -50 );
  STATIC_CONST( HINT_COUNT_BITS = 10 );
  STATIC_CONST( HINT_COUNT_HALF = (1 << (HINT_COUNT_BITS - 1)) );
  STATIC_CONST( HINT_COUNT_MASK = (HINT_COUNT_HALF | (HINT_COUNT_HALF - 1)) );
