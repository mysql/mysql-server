...
inline NdbIndexStatImpl::Bound::Bound(const NdbPack::Spec &spec)
    : m_data(spec, true, 0), m_bound(m_data) {
  m_type = -1;
  m_strict = -1;
}

inline NdbIndexStatImpl::Range::Range(Bound &bound1, Bound &bound2)
    : m_bound1(bound1), m_bound2(bound2) {
  bound1.m_type = 0;
  bound2.m_type = 1;
}

inline int NdbIndexStatImpl::finalize_range(Range &range) {
  if (finalize_bound(range.m_bound1) == -1) return -1;
  if (finalize_bound(range.m_bound2) == -1) return -1;
  return 0;
}

inline NdbIndexStatImpl::StatValue::StatValue()
    : m_num_fragments(0), m_num_rows(0), m_empty(false) {}

inline NdbIndexStatImpl::StatBound::StatBound() {
  m_pos = 0;
  m_numEqL = 0;
  m_numEqH = 0;
}

inline NdbIndexStatImpl::Stat::Stat() {
  m_rule[0] = nullptr;
  m_rule[1] = nullptr;
  m_rule[2] = nullptr;
}
