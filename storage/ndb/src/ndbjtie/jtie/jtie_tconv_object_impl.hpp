...
struct ObjectParam<_jtie_Object *, C *> {
  static C *convert(cstatus &s, _jtie_Object *j, JNIEnv *env) {
  ...
  static void release(C *c, _jtie_Object *j, JNIEnv *env) {
...
struct ObjectResult<J *, C *> {
 ...
 private:
  ...
  static J *wrapAsJavaObject(jclass cls, jmethodID cid, C *c, JNIEnv *env);
};
...
inline void detachWrapper(_jtie_Object *jo, JNIEnv *env) {
  // get a (local or global) class object reference
...
// Constructs a Wrapper object of user-defined type.
template <typename J, typename C>
inline J *ObjectResult<J *, C *>::wrapAsJavaObject(jclass cls, jmethodID cid,
                                                   C *c, JNIEnv *env) {
  J *j = NULL;
