 ...
 */
template <typename JA, typename J, typename C,
          J *(JNIEnv::*GET)(JA, jboolean *), void (JNIEnv::*REL)(JA, J *, jint),
          JA (JNIEnv::*NEW)(jsize),
...
struct PrimArrayConvImpl {
  ...
  static void releaseArrayElements(JNIEnv *env, JA j, const C *c, jint mode) {
    TRACE(
        "void PrimArrayConvImpl.releaseArrayElements(JNIEnv *, JA, const C *, "
        "jint)");
    // XXX currently, only exact-width base type conversions supported
    assert(sizeof(J) == sizeof(C));
    assert(j != NULL);
...
struct ObjectArrayConvImpl {
  static C *getArrayElements(JNIEnv *env, jobjectArray j, jboolean *isCopy) {
    TRACE(
        "C * ObjectArrayConvImpl.getArrayElements(JNIEnv *, jobjectArray, "
        "jboolean *)");
    assert(j != NULL);
    ...
    if (env->ExceptionCheck() != JNI_OK) {
      ...
      if (s != 0) {
        ...
        if (isCopy != NULL) {
          *isCopy = ResultBasicT<jboolean, bool>::convert(true, env);
        }
  ...
  static void releaseArrayElements(JNIEnv *env, jobjectArray j, const C *c,
                                   jint mode) {
    TRACE(
        "void ObjectArrayConvImpl.releaseArrayElements(JNIEnv *, jobjectArray, "
        "const C *, jint)");
    assert(j != NULL);
    assert(c != NULL);
    delete[] c;
 ...
 private:
  ...
  static J *newJavaObjectArray(jclass cls, jsize n, JNIEnv *env);
  ...
  static cstatus copyToCObjectArray(C *c, jobjectArray j, jsize n, JNIEnv *env);

  // Initializes a Java Object array with references from a C object array;
  // a non-zero return value indicates failure with an exception pending.
  static cstatus copyToJavaObjectArray(jobjectArray j, C *c, jsize n,
                                       JNIEnv *env);
};

template <typename J, typename C>
inline J *ObjectArrayConvImpl<J, C>::newJavaObjectArray(jclass cls, jsize n,
                                                        JNIEnv *env) {
  assert(cls);
...
template <typename J, typename C>
inline cstatus ObjectArrayConvImpl<J, C>::copyToCObjectArray(C *c,
                                                             jobjectArray j,
                                                             jsize n,
                                                             JNIEnv *env) {
  assert(j != NULL);
...
template <typename J, typename C>
inline cstatus ObjectArrayConvImpl<J, C>::copyToJavaObjectArray(jobjectArray j,
                                                                C *c, jsize n,
                                                                JNIEnv *env) {
  assert(c != NULL);
  assert(j != NULL);
...
#define JTIE_SPECIALIZE_ARRAY_TYPE_HELPER(JA, J, JN, C)              \
  ...
  struct ArrayConv<JA, C *>                                          \
      : PrimArrayConvImpl<JA, J, C, &JNIEnv::Get##JN##ArrayElements, \
                          &JNIEnv::Release##JN##ArrayElements,       \
                          &JNIEnv::New##JN##Array,                   \
                          &JNIEnv::Set##JN##ArrayRegion> {};         \
  template <>                                                        \
  struct ArrayConv<JA, const C *> : ArrayConv<JA, C *> {};
