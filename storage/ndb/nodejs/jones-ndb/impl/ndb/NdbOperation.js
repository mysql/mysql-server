...
var path = require("path"), assert = require("assert"),
    conf = require("./path_config"), adapter = require(conf.binary).ndb,
    jones = require("database-jones"), doc = require(jones.spi_doc.DBOperation),
    ...
    ROLLBACK = adapter.ndbapi.Rollback, constants = adapter.impl,
    OpHelper = constants.OpHelper, ScanHelper = constants.Scan.helper,
    BoundHelper = constants.IndexBound.helper, opcodes = doc.OperationCodes,
...
*/
function encodeColumnsInBuffer(
    fields, ncolumns, metadata, record, buffer, definedColumnList) {
...
function encodeKeyBuffer(op) {
  ...
  return encodeColumnsInBuffer(
      op.keys, op.indexHandler.getNumberOfColumns(),
      op.indexHandler.getAllColumnMetadata(), op.index.record, op.buffers.key,
      []);
}
...
function encodeRowBuffer(op) {
  ...
  return encodeColumnsInBuffer(
      valuesArray, ncolumns, columnMetadata, op.tableHandler.resultRecord,
      op.buffers.row, op.columnMask);
}
...
BoundHelperSpec.prototype.buildPartialSpec = function(
    base, bound, dbIndexHandler, buffer) {
  ...
  */
  ...
  if (nparts > 0) {
    err = encodeColumnsInBuffer(
        bound.key, nparts, columns, dbIndexHandler.dbIndex.record, buffer, []);
  }
...
BoundHelperSpec.prototype.setLow = function(bound, dbIndexHandler, buffer) {
  return this.buildPartialSpec(
      BoundHelper.low_key, bound.low, dbIndexHandler, buffer);
};

BoundHelperSpec.prototype.setHigh = function(bound, dbIndexHandler, buffer) {
  return this.buildPartialSpec(
      BoundHelper.high_key, bound.high, dbIndexHandler, buffer);
};
...
DBOperation.prototype.buildBoundHelpers = function(indexBounds) {
  ...
  if (sz && n) {
    ...
    for (i = 0; i < n; i++) {
      ...
      helper.setLow(
          bound, dbIndexHandler, mainBuffer.slice(offset, offset + sz));
      offset += sz;
      helper.setHigh(
          bound, dbIndexHandler, mainBuffer.slice(offset, offset + sz));
      offset += sz;
...
function prepareOperations(
    dbTransactionContext, dbOperationList, recycleWrapper) {
  ...
  return adapter.impl.DBOperationHelper(
      length, specs, dbTransactionContext, recycleWrapper);
}
  ...
  if (VOC) {
    ...
    for (i = 0; i < tableHandler.getNumberOfColumns(); i++) {
      if (tableHandler.columnHasConverter(i)) {
        tableHandler.set(
            value, i, adapter.impl.getValueObjectFieldByNumber(value, i));
      }
    }
...
function getResultValue(op, tableHandler, buffer, blobs) {
  ...
  var use_nro =
      (op.connProperties.use_mapped_ndb_record && tableHandler.is1to1 &&
...
function getScanResults(scanop, userCallback) {
  var buffer, results, dbSession, postScanCallback, nSkip, maxRow, i,
      recordSize, gather;
  dbSession = scanop.transaction.dbSession;
  postScanCallback = {fn: userCallback, arg0: null, arg1: null};
  i = 0;
  nSkip = 0;
  maxRow = 100000000000;
  if (scanop.params) {
    if (scanop.params.skip > 0) {
      nSkip = scanop.params.skip;
    }
    if (scanop.params.limit >= 0) {
      maxRow = nSkip + scanop.params.limit;
    }
  }
  if (udebug.is_debug()) {
  ...
  op.scanOp.fetchAllResults(function(err, nresults) {
    ...
    if (err) {
      ...
      for (i = 0; i < nresults; i++) {
        ...
        level = wrapper.level;
        if (level > 0) {
          parentLevel = sectors[level].parent.serial;
        }
        if (udebug.is_detail) {
          udebug.log(
              "TABLE", level, sectors[level].tableHandler.dbTable.name,
              "PARENT TABLE", parentLevel);
        }
        if (wrapper.tag) {
          assembleSpecial(wrapper.tag);
        } else {
          resultObject = getResultValue(
              op, sectors[level].tableHandler, wrapper.data, null);
          assemble();
        }
      }
  ...
  if (execMode !== ROLLBACK) {
    if (op.result.success) {
      if (transactionHandler.error) {
        ...
        op.result.success = false;
        op.result.error =
            new DBOperationError().cascading(transactionHandler.error);
      }
    } else {
    ...
    if (op.result.success && op.opcode === opcodes.OP_READ) {
      op.result.value =
          getResultValue(op, op.tableHandler, op.buffers.row, op.blobs);
    }
  }
  if (udebug.is_detail()) {
    udebug.log("buildOperationResult finished:", op.result);
  }
}
...
function completeExecutedOps(dbTxHandler, execMode, operations) {
  ...
  */
  if (udebug.is_debug()) {
    udebug.log(
        "completeExecutedOps mode:", execMode,
...
function newProjectionOperation(
    sessionImpl, tx, indexHandler, keys, projection) {
  ...
  if (op.query.error) { /* TODO: Report this error back to the user
    ...
    op.scanOp = adapter.impl.QueryOperation.create(
        op.query, op.buffers.key, op.query.size, sessionImpl);
  }
  return op;
}
...
function newInsertOperation(tx, tableHandler, row) {
  ...
  if ((op.tableHandler.autoIncFieldName) &&
    ...
    // we need autoincrement services because the user did not supply the value
    // for the autoincrement column
    op.needAutoInc = true;
  }
  return op;
}
...
function newScanOperation(tx, QueryTree, properties) {
  ...
  var op = new DBOperation(
      opcodes.OP_SCAN, tx, queryHandler.dbIndexHandler,
