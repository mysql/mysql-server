...
int createEmployeeDb(MYSQL &mysql) {
  if (true) {
    ...
    if (mysql_query(&mysql,
                    "Insert into dept_manager(dept_no,emp_no,my_key) values "
                    "('d005',110567,110567)") != 0)
      MYSQLERROR(mysql);
    mysql_commit(&mysql);
    if (mysql_query(&mysql,
                    "Insert into dept_manager(dept_no,emp_no,my_key) values "
    ...
    if (mysql_query(&mysql,
                    "Insert into dept_manager(dept_no,emp_no,my_key) values "
                    "('e005',210567,210567)") != 0)
      MYSQLERROR(mysql);
    mysql_commit(&mysql);

    if (mysql_query(&mysql,
                    "Insert into dept_manager(dept_no,emp_no,my_key) values "
                    "('f005',210568,210568)") != 0)
      MYSQLERROR(mysql);
    mysql_commit(&mysql);
    if (mysql_query(&mysql,
                    "Insert into dept_manager(dept_no,emp_no,my_key) values "
                    "('g005',210569,210569)") != 0)
      MYSQLERROR(mysql);
    mysql_commit(&mysql);
    if (mysql_query(&mysql,
                    "Insert into dept_manager(dept_no,emp_no,my_key) values "
                    "('h005',210560,210560)") != 0)
      MYSQLERROR(mysql);
    mysql_commit(&mysql);
    if (mysql_query(&mysql,
                    "Insert into dept_manager(dept_no,emp_no,my_key) values "
                    "('i005',210561,210561)") != 0)
      MYSQLERROR(mysql);
    mysql_commit(&mysql);

    if (mysql_query(
            &mysql,
            "Insert into employees(emp_no,dept_no) values (110567,'d005')") !=
        0)
      MYSQLERROR(mysql);
    mysql_commit(&mysql);
  }
...
int testQueryBuilder(Ndb &myNdb) {
  ...
  if (!employee || !manager || !salary) APIERROR(myDict->getNdbError());
  ...
  const NdbQueryDef *q1 = 0;
  {
    ...
    //  NdbQueryLookupOperationDef *illegalCopy1 = new
    //  NdbQueryLookupOperationDef(*readManager); NdbQueryLookupOperationDef
    //  illegalCopy2(*readManager);
  }
...
/**** UNFINISHED...
  ...
  const NdbQueryDef* q3 = 0;
  {
    ...
    const NdbQueryIndexBound* managerBound =       // Manager is indexed om
{"dept_no", "emp_no"} {  ....
    };
    // Lookup on a single tuple with key define by 'managerKey' param. tuple
...
*****/
...
#if 1
  {
    ...
    const NdbQueryDef *q4 = 0;
    {
      ...
      const NdbQueryOperand
          *constManagerKey[] =  // Manager is indexed om {"dept_no", "emp_no"}
          {qb->constValue("d005"),  // dept_no = "d005"
           qb->constValue(110567),  // emp_no  = 110567
           0};
      const NdbQueryOperand
          *paramManagerKey[] =  // Manager is indexed om {"dept_no", "emp_no"}
          {qb->paramValue(),    // dept_no parameter,
           qb->paramValue(
               "emp"),  // emp_no parameter - param naming is optional
           0};
      // Lookup a single tuple with key define by 'managerKey' param. tuple
      const NdbQueryLookupOperationDef *readManager =
          qb->readTuple(manager, paramManagerKey);
      // const NdbQueryLookupOperationDef *readManager = qb->readTuple(manager,
      // constManagerKey);
      if (readManager == NULL) APIERROR(qb->getNdbError());
      ...
      const NdbQueryOperand
          *joinEmployeeKey[] =  // Employee is indexed om {"emp_no"}
          {qb->linkedValue(readManager,
                           "emp_no"),  // where '= readManger.emp_no'
           0};
      const NdbQueryLookupOperationDef *readEmployee =
          qb->readTuple(employee, joinEmployeeKey);
    ...
    if (myQuery == NULL) APIERROR(myTransaction->getNdbError());
    ...
    if (res == NdbQuery::NextResult_error) APIERROR(myQuery->getNdbError());
    ...
    const NdbQueryDef *q4_1 = 0;
    {
      ...
      const NdbQueryOperand *empKey[] =  // Employee is indexed om {"emp_no"}
          {// qb->constValue(110567),   // emp_no  = 110567
           qb->paramValue(), 0};
      const NdbQueryLookupOperationDef *readEmployee =
          qb->readTuple(employee, empKey);
      if (readEmployee == NULL) APIERROR(qb->getNdbError());

      const NdbQueryOperand
          *joinManagerKey[] =  // Manager is indexed om {"dept_no", "emp_no"}
          {qb->paramValue(),
      ...
      const NdbQueryLookupOperationDef *readManager =
          qb->readTuple(manager, joinManagerKey);
      if (readManager == NULL) APIERROR(qb->getNdbError());
...
#if 1
  {
    ...
    const NdbQueryDef *q5 = 0;
    {
      ...
      const NdbDictionary::Index *myPIndex =
          myDict->getIndex("MYINDEXNAME$unique", manager->getName());
      if (myPIndex == NULL) APIERROR(myDict->getNdbError());

      // Manager index-key defined as parameter, NB: Reversed order compared to
      // hash key
      const NdbQueryOperand
          *managerKey[] =  // Manager PK index is {"emp_no","dept_no", }
          {                // qb->constValue(110567),   // emp_no  = 110567
           qb->paramValue(), 0};
      // Lookup on a single tuple with key define by 'managerKey' param. tuple
      const NdbQueryLookupOperationDef *readManager =
          qb->readTuple(myPIndex, manager, managerKey);
      if (readManager == NULL) APIERROR(qb->getNdbError());
...
#else
    {
      ...
      // Specify result handling NdbRecord style - need the (single)
      // NdbQueryOperation:
      NdbQueryOperation *op = myQuery->getQueryOperation(0U);
...
#if 1
  {
    ...
    const NdbQueryDef *q6 = 0;
    {
      ...
      /****
          // Lookup Primary key for manager table
          const NdbDictionary::Index *myPIndex= myDict->getIndex("PRIMARY",
      manager->getName()); if (myPIndex == NULL)
            APIERROR(myDict->getNdbError());

          const NdbQueryOperand* low[] =  // Manager PK index is
      {"emp_no","dept_no", } {  qb->constValue(110567),      // emp_no  = 110567
             0
          };
          const NdbQueryOperand* high[] =  // Manager PK index is
      {"emp_no","dept_no", } {  qb->constValue("illegal key"),
             0
          };
          const NdbQueryIndexBound  bound        (low, NULL);   // emp_no =
      [110567, oo] const NdbQueryIndexBound  bound_illegal(low, high);   //
      'high' is char type -> illegal const NdbQueryIndexBound  boundEq(low);
      ****/
      // Lookup on a single tuple with key define by 'managerKey' param. tuple
      //  const NdbQueryScanOperationDef* scanManager = qb->scanIndex(myPIndex,
      //  manager, &boundEq);
      ...
      const NdbQueryOperand
          *empJoinKey[] =  // Employee is indexed om {"emp_no"}
          {qb->linkedValue(scanManager,
                           "emp_no"),  // where '= readManger.emp_no'
           0};
      const NdbQueryLookupOperationDef *readEmployee =
          qb->readTuple(employee, empJoinKey);
      if (readEmployee == NULL) APIERROR(qb->getNdbError());
    ...
    while (true) {
      ...
      // All NdbQuery operations are handled as scans with cursor placed
      // 'before' first record: Fetch next to retrieve result:
      ...
      printf("manager  emp_no: %d, NULL:%d\n", value_q6[0][1]->u_32_value(),
             myQuery->getQueryOperation(0U)->isRowNULL());
      printf("employee emp_no: %d, NULL:%d\n", value_q6[1][0]->u_32_value(),
             myQuery->getQueryOperation(1U)->isRowNULL());
#else
...
#if 1
  {
    ...
    const NdbQueryDef *q6_1 = 0;
    {
      ...
      // Lookup Primary key for manager table
      const NdbDictionary::Index *myPIndex =
          myDict->getIndex("PRIMARY", manager->getName());
      if (myPIndex == NULL) APIERROR(myDict->getNdbError());

      const NdbQueryOperand
          *low[] =  // Manager PK index is {"emp_no","dept_no", }
          {qb->paramValue(),
           //     qb->constValue(110567),      // emp_no  = 110567
           qb->constValue("d005"),  // dept_no = "d005"
           0};
      const NdbQueryOperand
          *high[] =  // Manager PK index is {"emp_no","dept_no", }
          {qb->constValue(110567),  // emp_no  = 110567
           qb->constValue("d005"),  // dept_no = "d005"
           0};
      ...
      // Lookup on a single tuple with key define by 'managerKey' param. tuple
      const NdbQueryScanOperationDef *scanManager =
          qb->scanIndex(myPIndex, manager, &bound);
      if (scanManager == NULL) APIERROR(qb->getNdbError());
...
#else
    {
      ...
      assert(myQuery->getNoOfOperations() == 2);
      NdbQueryOperation *op0 = myQuery->getQueryOperation(0U);
      err = op0->setResultRowBuf(
          rowManagerRecord,
          (char *)&managerRow /*, (const unsigned char*)&mask*/);
      assert(err == 0);
      ...
      NdbQueryOperation *op1 = myQuery->getQueryOperation(1U);
      err = op1->setResultRowBuf(
          rowEmployeeRecord,
          (char *)&employeeRow /*, (const unsigned char*)&mask*/);
      assert(err == 0);
      if (err) APIERROR(myQuery->getNdbError());
    }
#endif
    ...
    while (true) {
      ...
      printf("manager  emp_no: %d, NULL:%d\n", managerRow.emp_no,
             myQuery->getQueryOperation(0U)->isRowNULL());
      printf("employee emp_no: %d, NULL:%d\n", employeeRow.emp_no,
             myQuery->getQueryOperation(1)->isRowNULL());
#endif
      cnt++;
    };
...
int main(int argc, const char **argv) {
   ...
   **************************************************************/
  {
    ...
    if (true) {
      ...
      if (!has_tables) {
        std::cout << "Table(s) was missing from the 'employees' DB"
                  << std::endl;
        exit(-1);
      }
