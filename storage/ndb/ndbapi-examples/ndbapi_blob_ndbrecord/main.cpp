...
void drop_table(MYSQL &mysql) {
  if (mysql_query(&mysql, "DROP TABLE api_blob_ndbrecord")) MYSQLERROR(mysql);
}
...
int populate(Ndb *myNdb) {
  ...
  const NdbOperation *myNdbOperation =
      myTrans->insertTuple(full_record, (const char *)&row);
...
int update_key(Ndb *myNdb) {
  ...
  */
  ...
  const NdbOperation *myNdbOperation = myTrans->updateTuple(
      key_record, (const char *)&row, blob_record, (const char *)&row);
  if (myNdbOperation == NULL) APIERROR(myTrans->getNdbError());
...
#define CHUNK_SIZE 100
  ...
  for (chunk = (length - 1) / CHUNK_SIZE; chunk >= 0; chunk--) {
    ...
    if (-1 == myBlobHandle->setPos(pos)) APIERROR(myBlobHandle->getNdbError());
...
int update_scan(Ndb *myNdb) {
  ...
  */
  ...
  for (;;) {
     ...
     */
    const NdbOperation *myUpdateOp = myScanOp->updateCurrentTuple(
        myTrans, blob_record, (const char *)out_row);
...
int fetch_key(Ndb *myNdb) {
  ...
  */
  ...
  key_row.myId = 1;
  out_row.myText = NULL;
  const NdbOperation *myNdbOperation = myTrans->readTuple(
      key_record, (const char *)&key_row, blob_record, (char *)&out_row);
  if (myNdbOperation == NULL) APIERROR(myTrans->getNdbError());
...
int update2_key(Ndb *myNdb) {
  ...
  row.myId = 1;
  const NdbOperation *myNdbOperation = myTrans->updateTuple(
      key_record, (const char *)&row, blob_record, (char *)&row);
  if (myNdbOperation == NULL) APIERROR(myTrans->getNdbError());
  NdbBlob *myBlobHandle = myNdbOperation->getBlobHandle("my_text");
  if (myBlobHandle == NULL) APIERROR(myNdbOperation->getNdbError());
...
int delete_key(Ndb *myNdb) {
  ...
  row.myId = 1;
  const NdbOperation *myNdbOperation =
      myTrans->deleteTuple(key_record, (const char *)&row, full_record);
  if (myNdbOperation == NULL) APIERROR(myTrans->getNdbError());
...
void ndb_run_ndbrecord_blob_operations(const char *connectstring) {
  ...
  if (update_key(&myNdb) > 0) std::cout << "update_key: Success!" << std::endl;

  if (update_scan(&myNdb) > 0)
    std::cout << "update_scan: Success!" << std::endl;

  if (fetch_key(&myNdb) > 0) std::cout << "fetch_key: Success!" << std::endl;

  if (update2_key(&myNdb) > 0)
    std::cout << "update2_key: Success!" << std::endl;

  if (delete_key(&myNdb) > 0) std::cout << "delete_key: Success!" << std::endl;
}
