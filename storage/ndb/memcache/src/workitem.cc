...
bool workitem_allocate_rowbuffer_1(workitem *i, size_t buffersize) {  
  ...
  i->row_buffer_1 = (char *)pipeline_alloc(i->pipeline, i->rowbuf1_cls);
...
bool workitem_allocate_rowbuffer_2(workitem *i, size_t buffersize) {  
  ...
  i->row_buffer_2 = (char *)pipeline_alloc(i->pipeline, i->rowbuf2_cls);
                          ...
                          int nkey, const char *key)
{
  ...
  if(sz > workitem_actual_inline_buffer_size) {
    ...
    item->ndb_key_buffer = (char *)pipeline_alloc(pipeline, item->keybuf1_cls);
  }
  else {
...
{
  workitem *newitem;
  newitem = (workitem *)pipeline_alloc(pipeline, workitem_class_id);
                                  ...
                                  int nkey, const char *key)
{
  ...
  if((workitem_actual_inline_buffer_size - 3) > (2 * nkey)) {    
   /* use space at the end of the inline buffer */
    ...
    newitem->key_buffer_2 = (char *)pipeline_alloc(newitem->pipeline, newitem->keybuf2_cls);
  }
