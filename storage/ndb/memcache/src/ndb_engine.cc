...
ENGINE_ERROR_CODE create_instance(uint64_t interface, 
  ...
  ndb_eng = (ndb_engine *)malloc(sizeof(struct ndb_engine));
  if(ndb_eng == NULL) {
    return ENGINE_ENOMEM;
  }
 
  logger = (EXTENSION_LOGGER_DESCRIPTOR *)api->extension->get_extension(EXTENSION_LOGGER);
...
{   
  ...
  */  
  ndb_eng->pipelines  = (void **)malloc(nthreads * sizeof(void *));
  for(i = 0 ; i < nthreads ; i++) {
    ndb_eng->pipelines[i] = (ndb_pipeline *)get_request_pipeline(i, ndb_eng);
    if(! scheduler_initialize((ndb_pipeline *)ndb_eng->pipelines[i], & sched_opts)) {
...
static void ndb_destroy(ENGINE_HANDLE* handle, bool force) 
{
  ...
  for(atomic_int32_t i = 0 ; i < ndb_eng->npipelines; i ++) {
    ndb_pipeline *p = (ndb_pipeline *)ndb_eng->pipelines[i];
    if(p) {
                                         ...
                                         uint16_t vbucket  __attribute__((unused)))
{
  ...
  wqitem = (workitem *)ndb_eng->server.cookie->get_engine_specific(cookie);
  if(wqitem) {
  ...
  prefix = get_prefix_info_for_key(nkey, (const char *)key);
  ...
  */
  ...
  if(prefix.do_db_delete) {                        /* Database Delete */
    wqitem = (workitem *)new_workitem_for_delete_op(pipeline, prefix, cookie, nkey, (const char *)key, & cas);
                        ...
                        item* item)
{
  ...
  workitem *wqitem = (workitem *)ndb_eng->server.cookie->get_engine_specific(cookie);  
  if(wqitem) {
                                 ...
                                 uint16_t vbucket __attribute__((unused)))
{
  ...
  if(prefix.do_db_read) {
    wqitem = (workitem *)new_workitem_for_get_op(wqitem, pipeline, prefix, cookie, nkey, (const char *)key);
    DEBUG_PRINT("creating workitem %d.%d", pipeline->id, wqitem->id);
    return_status = scheduler_schedule(pipeline, wqitem);
                                   ...
                                   uint16_t vbucket  __attribute__((unused)))
{
  ...
  prefix = get_prefix_info_for_key(hash_item_get_key_len((hash_item *)item), 
                                   hash_item_get_key((hash_item *)item));
  ...
  */
  if(prefix.do_db_write) {
    wqitem = (workitem *)new_workitem_for_store_op(pipeline, op, prefix, cookie, (hash_item *)item, cas);
    ...
    DEBUG_PRINT("[%s] prefix %d; CAS %llu; use mc/db: %d/%d --  cache-only store.",
                set_ops[op], prefix.prefix_id, cas ? *cas : 0,
                prefix.do_mc_write, prefix.do_db_write);
    return_status = store_item(default_handle(ndb_eng), (hash_item *)item, cas, op, cookie);
  }
                                        ...
                                        uint16_t vbucket)
{
  ...
  wqitem = (workitem *)new_workitem_for_arithmetic(pipeline, prefix, cookie, (const char *)key, nkey,
...
/*
  C++ doesn't have C99's designated initializers until C++20, and we need to
  initialize an union, so create some constructors for config_item as convenience.
*/
config_item Config_item_string(const char *key, char **value)
{
  config_item item;
  item.key= key;
  item.datatype= DT_STRING;
  item.value.dt_string= value;
  return item;
}

config_item Config_item_bool(const char *key, bool *value)
{
  config_item item;
  item.key= key;
  item.datatype= DT_BOOL;
  item.value.dt_bool= value;
  return item;
}

config_item Config_item_size_t(const char *key, size_t *value)
{
  config_item item;
  item.key= key;
  item.datatype= DT_SIZE;
  item.value.dt_size= value;
  return item;
}

config_item Config_item_float(const char *key, float *value)
{
  config_item item;
  item.key= key;
  item.datatype= DT_FLOAT;
  item.value.dt_float= value;
  return item;
}

config_item Config_item_configfile(const char *key)
{
  config_item item;
  item.key= key;
  item.datatype= DT_CONFIGFILE;
  return item;
}
                          ...
                          const char * conf)
{
  ...
  if (conf != NULL) {
    struct config_item items[] = {
      /* NDB OPTIONS */
      Config_item_string("connectstring", (char **) &(ndb->startup_options.connectstring)),
      Config_item_string("role", (char **) &(ndb->startup_options.server_role)),
      Config_item_string("scheduler", (char **) &(ndb->startup_options.scheduler)),
#ifdef DEBUG_OUTPUT
      Config_item_bool("debug", &(ndb->startup_options.debug_enable)),
      Config_item_bool("detail", &(ndb->startup_options.debug_detail)),
#endif
      Config_item_bool("reconf", &(ndb->startup_options.reconf_enable)),
      
      /* DEFAULT ENGINE OPTIONS */
      Config_item_bool("use_cas", &se->config.use_cas),
      Config_item_size_t("verbose", &se->config.verbose),
      Config_item_bool("eviction", &se->config.evict_to_free),
      Config_item_size_t("cache_size", &se->config.maxbytes),
      Config_item_bool("preallocate", &se->config.preallocate),
      Config_item_float("factor", &se->config.factor),
      Config_item_size_t("chunk_size", &se->config.chunk_size),
      Config_item_size_t("item_size_max", &se->config.item_size_max),
      Config_item_configfile("config_file"),
      { nullptr }
    };
...
int fetch_core_settings(struct ndb_engine *engine,
  ...
  struct config_item items[] = {
    Config_item_bool("cas_enabled", &engine->server_options.cas_enabled),
    Config_item_size_t("maxconns", &engine->server_options.maxconns),
    Config_item_size_t("num_threads", &engine->server_options.nthreads),
    Config_item_size_t("verbosity", &engine->server_options.verbose),
    { nullptr }
  };
