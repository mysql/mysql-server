...
void
cleanup_account(void)
{
  global_account_container.cleanup();
}

static const uchar *
account_hash_get_key(const uchar *entry, size_t *length)
{
...
*/
int
init_account_hash(const PFS_global_param *param)
{
  if ((!account_hash_inited) && (param->m_account_sizing != 0))
  {
    lf_hash_init(&account_hash,
                 sizeof(PFS_account *),
                 LF_HASH_UNIQUE,
                 0,
                 0,
...
static LF_PINS *
get_account_hash_pins(PFS_thread *thread)
{
...
static void
set_account_key(PFS_account_key *key,
                const char *user,
                uint user_length,
                const char *host,
                uint host_length)
...
void
PFS_account::aggregate(bool alive, PFS_user *safe_user, PFS_host *safe_host)
{
...
void
PFS_account::aggregate_memory(bool alive,
                              PFS_user *safe_user,
                              PFS_host *safe_host)
{
...
void
PFS_account::aggregate_status(PFS_user *safe_user, PFS_host *safe_host)
{
  if (likely(safe_user != NULL && safe_host != NULL))
  {
    /*
...
void
PFS_account::aggregate_stats(PFS_user *safe_user, PFS_host *safe_host)
{
  if (likely(safe_user != NULL && safe_host != NULL))
  {
...
PFS_account *
sanitize_account(PFS_account *unsafe)
{
  return global_account_container.sanitize(unsafe);
}

static void
purge_account(PFS_thread *thread, PFS_account *account)
{
  LF_PINS *pins = get_account_hash_pins(thread);
  if (unlikely(pins == NULL))
  {
...
void
update_accounts_derived_flags(PFS_thread *thread)
{
