...
int
PFS_account_allocator::alloc_array(PFS_account_array *array)
{
  PFS_account *pfs;
  ...
  if (size > 0)
  {
    array->m_ptr = PFS_MALLOC_ARRAY(&builtin_memory_account,
                                    size,
                                    sizeof(PFS_account),
                                    PFS_account,
                                    MYF(MY_ZEROFILL));
  ...
  if (errors_sizing > 0)
  {
    array->m_instr_class_errors_array =
      PFS_MALLOC_ARRAY(&builtin_memory_account_errors,
                       errors_sizing,
...
void
PFS_account_allocator::free_array(PFS_account_array *array)
{
  size_t index;
  size_t size = array->m_max;
  size_t waits_sizing = size * wait_class_max;
  ...
  PFS_FREE_ARRAY(
    &builtin_memory_account, size, sizeof(PFS_account), array->m_ptr);
  array->m_ptr = NULL;
  ...
  if (array->m_instr_class_errors_array != NULL)
    for (index = 0; index < errors_sizing; index++)
      array->m_instr_class_errors_array[index].cleanup(
        &builtin_memory_account_errors);
  PFS_FREE_ARRAY(&builtin_memory_account_errors,
                 errors_sizing,
                 sizeof(PFS_error_stat),
                 array->m_instr_class_errors_array);
  array->m_instr_class_errors_array = NULL;
...
int
PFS_host_allocator::alloc_array(PFS_host_array *array)
{
  size_t size = array->m_max;
  PFS_host *pfs;
  size_t index;
  ...
  {
    array->m_instr_class_errors_array =
      PFS_MALLOC_ARRAY(&builtin_memory_host_errors,
                       errors_sizing,
                       sizeof(PFS_error_stat),
                       PFS_error_stat,
                       MYF(MY_ZEROFILL));
...
void
PFS_host_allocator::free_array(PFS_host_array *array)
{
  size_t index;
  size_t size = array->m_max;
  size_t waits_sizing = size * wait_class_max;
  ...
  if (array->m_instr_class_errors_array != NULL)
    for (index = 0; index < errors_sizing; index++)
      array->m_instr_class_errors_array[index].cleanup(
        &builtin_memory_host_errors);
  PFS_FREE_ARRAY(&builtin_memory_host_errors,
                 errors_sizing,
                 sizeof(PFS_error_stat),
                 array->m_instr_class_errors_array);
  array->m_instr_class_errors_array = NULL;
...
int
PFS_thread_allocator::alloc_array(PFS_thread_array *array)
{
  size_t size = array->m_max;
  PFS_thread *pfs;
  ...
  if (size > 0)
  {
    array->m_ptr = PFS_MALLOC_ARRAY(&builtin_memory_thread,
                                    size,
                                    sizeof(PFS_thread),
                                    PFS_thread,
                                    MYF(MY_ZEROFILL));
  ...
  {
    array->m_instr_class_errors_array =
      PFS_MALLOC_ARRAY(&builtin_memory_thread_errors,
                       errors_sizing,
                       sizeof(PFS_error_stat),
                       PFS_error_stat,
                       MYF(MY_ZEROFILL));
  ...
  for (index = 0; index < size; index++)
  {
    ...
    pfs->m_statements_history =
      &array->m_statements_history_array[index *
    ...
    pfs->m_transactions_history =
      &array
         ->m_transactions_history_array[index *
                                        events_transactions_history_per_thread];
    pfs->m_session_connect_attrs =
      &array
         ->m_session_connect_attrs_array[index *
  ...
  for (index = 0; index < statements_stack_sizing; index++)
  {
    ...
    pfs_tokens =
      &array->m_current_stmts_digest_token_array[index * pfs_max_digest_length];
  ...
  for (index = 0; index < statements_history_sizing; index++)
  {
    ...
    pfs_tokens =
      &array->m_history_stmts_digest_token_array[index * pfs_max_digest_length];
    pfs_stmt->m_digest_storage.reset(pfs_tokens, pfs_max_digest_length);
  }
...
void
PFS_thread_allocator::free_array(PFS_thread_array *array)
{
  size_t index;
  size_t size = array->m_max;
  size_t waits_sizing = size * wait_class_max;
  ...
  PFS_FREE_ARRAY(
    &builtin_memory_thread, size, sizeof(PFS_thread), array->m_ptr);
  array->m_ptr = NULL;
  ...
  if (array->m_instr_class_errors_array != NULL)
    for (index = 0; index < errors_sizing; index++)
      array->m_instr_class_errors_array[index].cleanup(
        &builtin_memory_thread_errors);
  PFS_FREE_ARRAY(&builtin_memory_thread_errors,
                 errors_sizing,
                 sizeof(PFS_error_stat),
                 array->m_instr_class_errors_array);
  array->m_instr_class_errors_array = NULL;
...
int
PFS_user_allocator::alloc_array(PFS_user_array *array)
{
  size_t size = array->m_max;
  PFS_user *pfs;
  size_t index;
  ...
  {
    array->m_instr_class_errors_array =
      PFS_MALLOC_ARRAY(&builtin_memory_user_errors,
                       errors_sizing,
                       sizeof(PFS_error_stat),
                       PFS_error_stat,
                       MYF(MY_ZEROFILL));
...
void
PFS_user_allocator::free_array(PFS_user_array *array)
{
  size_t index;
  size_t size = array->m_max;
  size_t waits_sizing = size * wait_class_max;
  ...
  if (array->m_instr_class_errors_array != NULL)
    for (index = 0; index < errors_sizing; index++)
      array->m_instr_class_errors_array[index].cleanup(
        &builtin_memory_user_errors);
  PFS_FREE_ARRAY(&builtin_memory_user_errors,
                 errors_sizing,
                 sizeof(PFS_error_stat),
                 array->m_instr_class_errors_array);
  array->m_instr_class_errors_array = NULL;
