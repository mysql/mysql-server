...
bool
PFS_index_host_cache_by_ip::match(const row_host_cache *row)
{
  if (m_fields >= 1)
  {
...
bool
PFS_index_host_cache_by_host::match(const row_host_cache *row)
{
  if (m_fields >= 1)
  {
...
int
table_host_cache::make_row(Host_entry *entry, row_host_cache *row)
{
...
void
table_host_cache::reset_position(void)
{
  m_pos.m_index = 0;
...
int
table_host_cache::rnd_next(void)
{
  int result;
...
int
table_host_cache::rnd_pos(const void *pos)
{
  set_position(pos);
...
int
table_host_cache::index_init(uint idx, bool)
{
...
int
table_host_cache::index_next(void)
{
  int result;
...
int
table_host_cache::read_row_values(TABLE *table,
                                  unsigned char *buf,
                                  Field **fields,
                                  bool read_all)
{
    ...
    if (read_all || bitmap_is_set(table->read_set, f->field_index))
    {
      ...
      case 20: /* COUNT_MAX_USER_CONNECTION_PER_HOUR_ERRORS */
        set_field_ulonglong(f,
                            m_row->m_count_max_user_connection_per_hour_errors);
        break;
