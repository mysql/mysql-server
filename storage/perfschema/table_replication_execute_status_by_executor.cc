...
/**
  ...
  Table replication_execute_status_per_executor (implementation).
*/
...
{
  for (int i= RPL_EXECUTE_THREAD_ID; i <= _RPL_EXECUTE_LAST_FIELD_; i++)
  {
...
void table_replication_execute_status_by_executor::fill_rows(Master_info *mi)
{
  ...
  /*TODO: RPL_EXECUTE_THREAD_ID, RPL_EXECUTE_SERVICE_STATE,RPL_LAST_EXECUTED_TRANSACTION*/
  int_store(RPL_EXECUTE_THREAD_ID, 5);
  enum_store(RPL_EXECUTE_SERVICE_STATE, PS_RPL_YES);
  str_store(RPL_LAST_EXECUTED_TRANSACTION, "need to modify --shiv");
  int_store(RPL_EXECUTE_LAST_ERROR_NUMBER, (long int) mi->rli->last_error().number);
  str_store(RPL_EXECUTE_LAST_ERROR_MESSAGE, mi->rli->last_error().message);
  str_store(RPL_EXECUTE_LAST_ERROR_TIMESTAMP, mi->rli->last_error().timestamp);
