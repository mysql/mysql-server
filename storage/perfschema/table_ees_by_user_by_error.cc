...
int table_ees_by_user_by_error::rnd_init(bool scan)
{
  return 0;
}
    ...
    if (user != NULL)
    {
      ...
      if (error_class)
      {
        for ( ;
             ...
             m_pos.next_error())
        {
          make_row(user, error_class, m_pos.m_index_3);
...
void table_ees_by_user_by_error
::make_row(PFS_user *user, PFS_error_class *klass,int error_index)
{
