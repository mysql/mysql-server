...
TABLE_FIELD_DEF
table_events_transactions_current::m_field_def = {24, field_types};
...
bool
PFS_index_events_transactions::match(PFS_thread *pfs)
{
  if (m_fields >= 1)
  {
...
bool
PFS_index_events_transactions::match(PFS_events *pfs)
{
  if (m_fields >= 2)
  {
...
*/
int
table_events_transactions_common::make_row(PFS_events_transactions *transaction)
{
  const char *base;
  ...
  m_normalizer->to_pico(transaction->m_timer_start,
                        timer_end,
                        &m_row.m_timer_start,
                        &m_row.m_timer_end,
                        &m_row.m_timer_wait);
...
static size_t
xid_to_hex(
  char *buf, size_t buf_len, PSI_xid *xid, size_t offset, size_t length)
{
  DBUG_ASSERT(buf_len >= XID_BUFFER_SIZE);
  DBUG_ASSERT(offset + length <= XIDDATASIZE);
  *buf++ = '0';
  *buf++ = 'x';
  return bin_to_hex_str(
           buf, buf_len - 2, (char *)(xid->data + offset), length) +
         2;
}
...
*/
static void
xid_store(Field *field, PSI_xid *xid, size_t offset, size_t length)
{
...
static void
xid_store_bqual(Field *field, PSI_xid *xid)
{
  xid_store(field, xid, xid->gtrid_length, xid->bqual_length);
}

static void
xid_store_gtrid(Field *field, PSI_xid *xid)
{
  xid_store(field, xid, 0, xid->gtrid_length);
}

int
table_events_transactions_common::read_row_values(TABLE *table,
                                                  unsigned char *buf,
                                                  Field **fields,
                                                  bool read_all)
{
  ...
  for (; (f = *fields); fields++)
  {
    if (read_all || bitmap_is_set(table->read_set, f->field_index))
    {
      ...
      case 15: /* ACCESS_MODE */
        set_field_enum(
          f, m_row.m_read_only ? TRANS_MODE_READ_ONLY : TRANS_MODE_READ_WRITE);
        break;
...
table_events_transactions_current::table_events_transactions_current()
  : table_events_transactions_common(&m_share, &m_pos), m_pos(0), m_next_pos(0)
{
}

void
table_events_transactions_current::reset_position(void)
{
  m_pos.m_index = 0;
...
int
table_events_transactions_current::rnd_next(void)
{
...
int
table_events_transactions_current::rnd_pos(const void *pos)
{
  PFS_thread *pfs_thread;
  PFS_events_transactions *transaction;
...
int
table_events_transactions_current::index_next(void)
{
  PFS_thread *pfs_thread;
  PFS_events_transactions *transaction;
  bool has_more_thread = true;
...
int
table_events_transactions_current::delete_all_rows(void)
{
...
table_events_transactions_history::table_events_transactions_history()
  : table_events_transactions_common(&m_share, &m_pos), m_pos(), m_next_pos()
{
}

void
table_events_transactions_history::reset_position(void)
{
  m_pos.reset();
  m_next_pos.reset();
}
...
int
table_events_transactions_history::rnd_next(void)
{
  PFS_thread *pfs_thread;
  PFS_events_transactions *transaction;
  bool has_more_thread = true;
...
int
table_events_transactions_history::rnd_pos(const void *pos)
{
  PFS_thread *pfs_thread;
  PFS_events_transactions *transaction;
...
int
table_events_transactions_history::index_next(void)
{
  PFS_thread *pfs_thread;
  PFS_events_transactions *transaction;
  bool has_more_thread = true;
...
int
table_events_transactions_history::delete_all_rows(void)
{
...
void
table_events_transactions_history_long::reset_position(void)
{
  m_pos.m_index = 0;
  m_next_pos.m_index = 0;
}
...
int
table_events_transactions_history_long::rnd_next(void)
{
  PFS_events_transactions *transaction;
  uint limit;
...
int
table_events_transactions_history_long::rnd_pos(const void *pos)
{
  PFS_events_transactions *transaction;
  uint limit;
...
int
table_events_transactions_history_long::delete_all_rows(void)
{
