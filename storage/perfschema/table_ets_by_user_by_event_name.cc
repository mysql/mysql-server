...
Plugin_table table_ets_by_user_by_event_name::m_table_def(
    /* Schema name */
    "performance_schema",
    /* Name */
...
PFS_engine_table_share table_ets_by_user_by_event_name::m_share = {
    &pfs_truncatable_acl,
...
bool PFS_index_ets_by_user_by_event_name::match(PFS_user *pfs) {
  if (m_fields >= 1) {
...
bool PFS_index_ets_by_user_by_event_name::match(PFS_instr_class *instr_class) {
  if (m_fields >= 2) {
...
PFS_engine_table *table_ets_by_user_by_event_name::create(
    PFS_engine_table_share *) {
  return new table_ets_by_user_by_event_name();
}

int table_ets_by_user_by_event_name::delete_all_rows() {
...
ha_rows table_ets_by_user_by_event_name::get_row_count() {
...
void table_ets_by_user_by_event_name::reset_position() {
  m_pos.reset();
  m_next_pos.reset();
}

int table_ets_by_user_by_event_name::rnd_init(bool) { return 0; }

int table_ets_by_user_by_event_name::rnd_next() {
  PFS_user *user;
  ...
  for (m_pos.set_at(&m_next_pos); has_more_user; m_pos.next_user()) {
    ...
    if (user != nullptr) {
      ...
      if (transaction_class) {
        ...
        return make_row(user, transaction_class);
      }
    }
  }
...
int table_ets_by_user_by_event_name::rnd_pos(const void *pos) {
  set_position(pos);
...
int table_ets_by_user_by_event_name::index_init(uint idx [[maybe_unused]],
                                                bool) {
  assert(idx == 0);
  auto *result = PFS_NEW(PFS_index_ets_by_user_by_event_name);
  m_opened_index = result;
  m_index = result;
  return 0;
}

int table_ets_by_user_by_event_name::index_next() {
  PFS_user *user;
  PFS_transaction_class *transaction_class;
  bool has_more_user = true;
        ...
        do {
          ...
          if (transaction_class) {
            if (m_opened_index->match(transaction_class)) {
              if (!make_row(user, transaction_class)) {
                m_next_pos.set_after(&m_pos);
                return 0;
              }
            }
...
int table_ets_by_user_by_event_name::make_row(PFS_user *user,
...
int table_ets_by_user_by_event_name::read_row_values(TABLE *table,
                                                     unsigned char *buf,
                                                     Field **fields,
                                                     bool read_all) {
  Field *f;
