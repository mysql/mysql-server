...
TABLE_FIELD_DEF
table_ets_by_user_by_event_name::m_field_def = {17, field_types};
...
bool
PFS_index_ets_by_user_by_event_name::match(PFS_user *pfs)
{
  if (m_fields >= 1)
  {
...
bool
PFS_index_ets_by_user_by_event_name::match(PFS_instr_class *instr_class)
{
  if (m_fields >= 2)
  {
...
PFS_engine_table *
table_ets_by_user_by_event_name::create(PFS_engine_table_share *)
{
...
void
table_ets_by_user_by_event_name::reset_position(void)
{
  m_pos.reset();
  m_next_pos.reset();
}
...
int
table_ets_by_user_by_event_name::rnd_next(void)
{
  PFS_user *user;
...
int
table_ets_by_user_by_event_name::index_next(void)
{
  PFS_user *user;
  PFS_transaction_class *transaction_class;
  bool has_more_user = true;
...
int
table_ets_by_user_by_event_name::make_row(PFS_user *user,
...
int
table_ets_by_user_by_event_name::read_row_values(TABLE *table,
                                                 unsigned char *buf,
                                                 Field **fields,
                                                 bool read_all)
{
