...
int table_esms_by_account_by_event_name::delete_all_rows() {
  reset_events_statements_by_thread();
  reset_events_statements_by_account();
  return 0;
}

ha_rows table_esms_by_account_by_event_name::get_row_count() {
...
void table_esms_by_account_by_event_name::reset_position() {
  m_pos.reset();
  m_next_pos.reset();
}
...
int table_esms_by_account_by_event_name::rnd_next() {
  PFS_account *account;
...
int table_esms_by_account_by_event_name::index_init(uint idx [[maybe_unused]],
                                                    bool) {
  assert(idx == 0);
  auto *result = PFS_NEW(PFS_index_esms_by_account_by_event_name);
  m_opened_index = result;
  m_index = result;
  return 0;
}

int table_esms_by_account_by_event_name::index_next() {
  PFS_account *account;
  PFS_statement_class *statement_class;
  bool has_more_account = true;
