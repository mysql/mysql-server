...
bool
PFS_index_setup_instruments::match_view(uint view)
{
  if (m_fields >= 1)
  {
...
bool
PFS_index_setup_instruments::match(PFS_instr_class *klass)
{
  if (m_fields >= 1)
  {
...
void
table_setup_instruments::reset_position(void)
{
  m_pos.reset();
  m_next_pos.reset();
}
...
int
table_setup_instruments::rnd_next(void)
{
...
int
table_setup_instruments::rnd_pos(const void *pos)
{
  PFS_instr_class *instr_class = NULL;
  PFS_builtin_memory_class *pfs_builtin;
  bool update_enabled;
...
int
table_setup_instruments::index_next(void)
{
  PFS_instr_class *instr_class = NULL;
  PFS_builtin_memory_class *pfs_builtin;
  bool update_enabled;
...
int
table_setup_instruments::update_row_values(TABLE *table,
                                           const unsigned char *,
                                           unsigned char *,
                                           Field **fields)
{
