...
/**
  @file storage/perfschema/table_replication_execute_status_by_channel.cc
  Table replication_execute_status_by_channel (implementation).
*/
...
THR_LOCK table_replication_execute_status_by_channel::m_table_lock;
...
TABLE_FIELD_DEF
table_replication_execute_status_by_channel::m_field_def=
{ 2, field_types };

PFS_engine_table_share
table_replication_execute_status_by_channel::m_share=
{
  { C_STRING_WITH_LEN("replication_execute_status_by_channel") },
  &pfs_readonly_acl,
  &table_replication_execute_status_by_channel::create,
  NULL, /* write_row */
...
PFS_engine_table* table_replication_execute_status_by_channel::create(void)
{
  return new table_replication_execute_status_by_channel();
}
...
table_replication_execute_status_by_channel::table_replication_execute_status_by_channel()
...
void table_replication_execute_status_by_channel::reset_position(void)
{
  m_pos.m_index= 0;
  m_next_pos.m_index= 0;
}
#ifndef MYSQL_CLIENT
int table_replication_execute_status_by_channel::rnd_next(void)
{
...
#endif
int table_replication_execute_status_by_channel::rnd_pos(const void *pos)
{
  Master_info *mi= active_mi;
  set_position(pos);
...
void table_replication_execute_status_by_channel::drop_null(enum enum_rpl_execution_channel_status_field_names name)
{
  if (slave_field_info[name].can_be_null)
    m_fields[name].is_null= false;
}

void table_replication_execute_status_by_channel::set_null(enum enum_rpl_execution_channel_status_field_names name)
{
  DBUG_ASSERT(slave_field_info[name].can_be_null);
  m_fields[name].is_null= true;
}

void table_replication_execute_status_by_channel::str_store(enum enum_rpl_execution_channel_status_field_names name, const char* val)
{
...
void table_replication_execute_status_by_channel::int_store(enum enum_rpl_execution_channel_status_field_names name, longlong val)
{
  m_fields[name].u.n= val;
  drop_null(name);
}

void table_replication_execute_status_by_channel::fill_rows(Master_info *mi)
{
...
int table_replication_execute_status_by_channel::read_row_values(TABLE *table,
                                       unsigned char *,
                                       Field **fields,
                                       bool read_all)
{
