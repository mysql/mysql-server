...
*/
int
init_instruments(const PFS_global_param *param)
{
  uint index;
...
static const uchar *
filename_hash_get_key(const uchar *entry, size_t *length)
{
...
*/
int
init_file_hash(const PFS_global_param *param)
{
  if ((!filename_hash_inited) && (param->m_file_sizing != 0))
  {
    lf_hash_init(&filename_hash,
                 sizeof(PFS_file *),
                 LF_HASH_UNIQUE,
                 0,
                 0,
...
*/
PFS_mutex *
create_mutex(PFS_mutex_class *klass, const void *identity)
{
  PFS_mutex *pfs;
...
*/
PFS_rwlock *
create_rwlock(PFS_rwlock_class *klass, const void *identity)
{
  PFS_rwlock *pfs;
  pfs_dirty_state dirty_state;
...
*/
PFS_cond *
create_cond(PFS_cond_class *klass, const void *identity)
{
  PFS_cond *pfs;
  pfs_dirty_state dirty_state;
...
*/
PFS_thread *
create_thread(PFS_thread_class *klass,
...
PFS_mutex *
sanitize_mutex(PFS_mutex *unsafe)
{
  return global_mutex_container.sanitize(unsafe);
}

PFS_rwlock *
sanitize_rwlock(PFS_rwlock *unsafe)
{
  return global_rwlock_container.sanitize(unsafe);
}

PFS_cond *
sanitize_cond(PFS_cond *unsafe)
{
...
*/
PFS_thread *
sanitize_thread(PFS_thread *unsafe)
{
  return global_thread_container.sanitize(unsafe);
}

PFS_file *
sanitize_file(PFS_file *unsafe)
{
  return global_file_container.sanitize(unsafe);
}

PFS_socket *
sanitize_socket(PFS_socket *unsafe)
{
  return global_socket_container.sanitize(unsafe);
}

PFS_metadata_lock *
sanitize_metadata_lock(PFS_metadata_lock *unsafe)
{
...
*/
static LF_PINS *
get_filename_hash_pins(PFS_thread *thread)
{
...
*/
PFS_file *
find_or_create_file(PFS_thread *thread,
...
*/
void
destroy_file(PFS_thread *thread, PFS_file *pfs)
{
  DBUG_ASSERT(thread != NULL);
  DBUG_ASSERT(pfs != NULL);
...
*/
PFS_table *
create_table(PFS_table_share *share,
...
void
PFS_table::sanitized_aggregate(void)
{
  /*
...
void
PFS_table::sanitized_aggregate_io(void)
{
  PFS_table_share *safe_share = sanitize_table_share(m_share);
...
void
PFS_table::sanitized_aggregate_lock(void)
{
  PFS_table_share *safe_share = sanitize_table_share(m_share);
...
void
PFS_table::safe_aggregate_io(const TABLE_SHARE *optional_server_share,
  ...
  for (index = 0; index < key_count; index++)
  {
    ...
    if (from_stat->m_has_data)
    {
      if (optional_server_share != NULL)
      {
        ...
        */
        to_stat =
          table_share->find_or_create_index_stat(optional_server_share, index);
      }
      else
      {
        /*
...
void
PFS_table::safe_aggregate_lock(PFS_table_stat *table_stat,
                               PFS_table_share *table_share)
{
  DBUG_ASSERT(table_stat != NULL);
  DBUG_ASSERT(table_share != NULL);
...
*/
PFS_socket *
create_socket(PFS_socket_class *klass,
              const my_socket *fd,
...
PFS_metadata_lock *
create_metadata_lock(void *identity,
...
void
destroy_metadata_lock(PFS_metadata_lock *pfs)
{
  DBUG_ASSERT(pfs != NULL);
...
static void
fct_reset_mutex_waits(PFS_mutex *pfs)
{
  pfs->m_mutex_stat.reset();
}

static void
reset_mutex_waits_by_instance(void)
{
  global_mutex_container.apply_all(fct_reset_mutex_waits);
}

static void
fct_reset_rwlock_waits(PFS_rwlock *pfs)
{
  pfs->m_rwlock_stat.reset();
}

static void
reset_rwlock_waits_by_instance(void)
{
  global_rwlock_container.apply_all(fct_reset_rwlock_waits);
}

static void
fct_reset_cond_waits(PFS_cond *pfs)
{
  pfs->m_cond_stat.reset();
}

static void
reset_cond_waits_by_instance(void)
{
  global_cond_container.apply_all(fct_reset_cond_waits);
}

static void
fct_reset_file_waits(PFS_file *pfs)
{
  pfs->m_file_stat.reset();
}

static void
reset_file_waits_by_instance(void)
{
  global_file_container.apply_all(fct_reset_file_waits);
}

static void
fct_reset_socket_waits(PFS_socket *pfs)
{
  pfs->m_socket_stat.reset();
}

static void
reset_socket_waits_by_instance(void)
{
...
void
reset_events_waits_by_instance(void)
{
...
static void
fct_reset_file_io(PFS_file *pfs)
{
  pfs->m_file_stat.m_io_stat.reset();
}
...
static void
fct_reset_socket_io(PFS_socket *pfs)
{
...
void
aggregate_all_event_names(PFS_single_stat *from_array,
...
void
aggregate_all_stages(PFS_stage_stat *from_array, PFS_stage_stat *to_array)
{
  PFS_stage_stat *from;
...
void
aggregate_all_stages(PFS_stage_stat *from_array,
...
void
aggregate_all_statements(PFS_statement_stat *from_array,
...
void
aggregate_all_transactions(PFS_transaction_stat *from_array,
...
void
aggregate_all_errors(PFS_error_stat *from_array, PFS_error_stat *to_array)
{
  DBUG_ASSERT(from_array != NULL);
  DBUG_ASSERT(to_array != NULL);
...
void
aggregate_all_errors(PFS_error_stat *from_array,
...
void
aggregate_thread_status(PFS_thread *thread,
...
static void
aggregate_thread_stats(PFS_thread *,
                       PFS_account *safe_account,
                       PFS_user *safe_user,
                       PFS_host *safe_host)
{
...
void
aggregate_thread(PFS_thread *thread,
                 PFS_account *safe_account,
                 PFS_user *safe_user,
                 PFS_host *safe_host)
{
...
void
aggregate_thread_waits(PFS_thread *thread,
                       PFS_account *safe_account,
                       PFS_user *safe_user,
                       PFS_host *safe_host)
{
...
void
aggregate_thread_stages(PFS_thread *thread,
                        PFS_account *safe_account,
                        PFS_user *safe_user,
                        PFS_host *safe_host)
{
...
void
aggregate_thread_statements(PFS_thread *thread,
                            PFS_account *safe_account,
                            PFS_user *safe_user,
                            PFS_host *safe_host)
{
...
void
aggregate_thread_transactions(PFS_thread *thread,
                              PFS_account *safe_account,
                              PFS_user *safe_user,
                              PFS_host *safe_host)
{
...
void
aggregate_thread_errors(PFS_thread *thread,
                        PFS_account *safe_account,
                        PFS_user *safe_user,
                        PFS_host *safe_host)
{
...
void
aggregate_thread_memory(bool alive,
                        PFS_thread *thread,
                        PFS_account *safe_account,
                        PFS_user *safe_user,
                        PFS_host *safe_host)
...
void
clear_thread_account(PFS_thread *thread)
{
...
void
set_thread_account(PFS_thread *thread)
{
  ...
  if ((thread->m_account == NULL) && (thread->m_username_length > 0))
    thread->m_user = find_or_create_user(
      thread, thread->m_username, thread->m_username_length);

  if ((thread->m_account == NULL) && (thread->m_hostname_length > 0))
    thread->m_host = find_or_create_host(
      thread, thread->m_hostname, thread->m_hostname_length);
}

static void
fct_update_mutex_derived_flags(PFS_mutex *pfs)
{
...
static void
fct_update_rwlock_derived_flags(PFS_rwlock *pfs)
{
...
static void
fct_update_cond_derived_flags(PFS_cond *pfs)
{
...
static void
fct_update_file_derived_flags(PFS_file *pfs)
{
...
static void
fct_update_table_derived_flags(PFS_table *pfs)
{
...
static void
fct_update_socket_derived_flags(PFS_socket *pfs)
{
...
static void
fct_update_metadata_derived_flags(PFS_metadata_lock *pfs)
{
  pfs->m_enabled =
    global_metadata_class.m_enabled && flag_global_instrumentation;
  pfs->m_timed = global_metadata_class.m_timed;
...
static void
fct_update_thread_derived_flags(PFS_thread *pfs)
{
