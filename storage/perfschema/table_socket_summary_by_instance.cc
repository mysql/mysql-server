...
bool PFS_index_socket_summary_by_instance_by_instance::match(const PFS_socket *pfs)
{
  if (m_fields >= 1)
  {
    if (!m_key.match(pfs))
...
bool PFS_index_socket_summary_by_instance_by_event_name::match(const PFS_socket *pfs)
{
  if (m_fields >= 1)
  {
    if (!m_key.match(pfs))
...
int table_socket_summary_by_instance::index_init(uint idx, bool sorted)
{
