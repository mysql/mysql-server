...
bool
PFS_index_rpl_applier_status_by_worker_by_channel::match(Master_info *mi, Slave_worker *worker)
{
  if (m_fields >= 1)
  {
    st_row_worker row;
...
bool
PFS_index_rpl_applier_status_by_worker_by_thread::match(Master_info *mi, Slave_worker *worker)
{
  if (m_fields >= 1)
  {
    st_row_worker row;
