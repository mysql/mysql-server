...
int table_mems_by_host_by_event_name::delete_all_rows() {
...
ha_rows table_mems_by_host_by_event_name::get_row_count() {
...
void table_mems_by_host_by_event_name::reset_position() {
  m_pos.reset();
  m_next_pos.reset();
}

int table_mems_by_host_by_event_name::rnd_next() {
  PFS_host *host;
...
int table_mems_by_host_by_event_name::index_init(uint idx [[maybe_unused]],
                                                 bool) {
  assert(idx == 0);
  auto *result = PFS_NEW(PFS_index_mems_by_host_by_event_name);
  m_opened_index = result;
  m_index = result;
  return 0;
}

int table_mems_by_host_by_event_name::index_next() {
  PFS_host *host;
  PFS_memory_class *memory_class;
  bool has_more_host = true;
