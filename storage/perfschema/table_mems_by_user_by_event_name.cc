...
bool
PFS_index_mems_by_user_by_event_name::match(PFS_user *pfs)
{
  if (m_fields >= 1)
  {
...
bool
PFS_index_mems_by_user_by_event_name::match(PFS_instr_class *instr_class)
{
  if (m_fields >= 2)
  {
...
PFS_engine_table *
table_mems_by_user_by_event_name::create(PFS_engine_table_share *)
{
...
void
table_mems_by_user_by_event_name::reset_position(void)
{
  m_pos.reset();
  m_next_pos.reset();
}
...
int
table_mems_by_user_by_event_name::rnd_next(void)
{
  PFS_user *user;
...
int
table_mems_by_user_by_event_name::rnd_pos(const void *pos)
{
  PFS_user *user;
  PFS_memory_class *memory_class;
...
int
table_mems_by_user_by_event_name::index_init(uint idx MY_ATTRIBUTE((unused)),
                                             bool)
{
  PFS_index_mems_by_user_by_event_name *result = NULL;
  DBUG_ASSERT(idx == 0);
...
int
table_mems_by_user_by_event_name::index_next(void)
{
  PFS_user *user;
  PFS_memory_class *memory_class;
  bool has_more_user = true;
...
int
table_mems_by_user_by_event_name::make_row(PFS_user *user,
...
int
table_mems_by_user_by_event_name::read_row_values(TABLE *table,
                                                  unsigned char *buf,
                                                  Field **fields,
                                                  bool read_all)
{
