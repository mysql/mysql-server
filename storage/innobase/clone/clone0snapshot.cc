...
bool
Clone_Snapshot::attach(
	Clone_Handle_Type	hdl_type)
{
	if (m_allow_new_clone
	    && hdl_type == m_snapshot_handle_type
...
Clone_File_Meta*
Clone_Snapshot::get_file_by_index(
	uint index)
{
	...
	uint&			data_size)
{
	...
	if (m_snapshot_state == CLONE_SNAPSHOT_PAGE_COPY) {
		...
		err = get_next_page(chunk_num, block_num, file_meta,
		...
		current_file = get_file(m_data_file_vector, m_num_data_files,
		...
		current_file = get_file(m_redo_file_vector, m_num_redo_files,
				        chunk_num, start_index);
	...
	if (chunk_num == current_file->m_end_chunk) {
		...
		size_in_pages = ut_uint64_align_up(current_file->m_file_size,
					   UNIV_PAGE_SIZE);
...
void
Clone_Snapshot::update_block_size(
	uint	buff_size)
{
	mutex_enter(&m_snapshot_mutex);
	...
	uint	temp_buffer_len)
{
	...
	case CLONE_SNAPSHOT_INIT:
		ut_ad(false);
		my_error(ER_INTERNAL_ERROR, MYF(0), "Innodb Clone Snapshot Invalid state");
		err = DB_ERROR;
		break;
	...
	uint&			data_size)
{
	...
	if (bpage->oldest_modification > 0) {
		...
		/* For compressed table, must copy the compressed page. */
		 if (page_size.is_compressed()) {
			...
			buf_flush_init_for_writing(nullptr, block->frame,
				&page_zip, bpage->newest_modification,
	...
	if (request.is_encrypted()) {
		...
		ret_data = encryption.encrypt(request, page_data, data_size,
