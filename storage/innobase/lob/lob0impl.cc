...
buf_block_set_next_page_no(
	...
	mtr_t*		mtr)
{
	mlog_write_ulint(block->frame + FIL_PAGE_NEXT, next_page_no,
			 MLOG_4BYTES, mtr);
}
...
bool
plist_base_node_t::validate() const
{
	ulint len = 0;
	ulint exp = get_len();
...
std::ostream&
z_frag_entry_t::print(std::ostream& out) const
{
	out << "[z_frag_entry_t: prev=" << get_prev() << ", next="
		<< get_next() << ", page_no=" << get_page_no()
		<< ", n_frags=" << get_n_frags() << ", used_len="
		<< get_used_len() << ", total_free_len="
		<< get_total_free_len() << ", big_free_len="
		<< get_big_free_len() << "]";
	return(out);
}

void
z_frag_entry_t::purge(
	flst_base_node_t*	used_lst,
...
void
z_frag_entry_t::update(const z_frag_page_t& frag_page)
{
	ut_ad(m_mtr != nullptr);
	...
	bool		bulk)
{
	...
	if (first.get_data_len() == 0) {
		...
		data_page.alloc(first_page_no + 1, bulk);
		...
		z_frag_entry_t frag_entry = first.find_frag_page(
			bulk, remain, frag_page);
	...
	while (remain > 0 && !z_frag_page_t::can_data_fit(index, remain)) {
		...
		buf_block_set_next_page_no(block, data_page.get_page_no(),
					   mtr);
	...
	if (remain > 0) {
		...
		buf_block_set_next_page_no(block, frag_page.get_page_no(),
					   mtr);
	}
	...
	bool			bulk)
{
	...
	std::pair<ulint, ulint> result = z_insert_strm(
		index, first, trxid, tmpbuf.get(), strm.total_out, mtr, bulk);
	...
	ulint			field_j)
{
	...
	while (remain > 0) {
		...
		err = z_insert_chunk(index, first, trx, ref, ptr, size,
				     &entry, mtr, ctx->is_bulk());
	...
	DBUG_EXECUTE_IF("innodb_zlob_print",
			z_print_info(index, ref, std::cerr););
...
z_frag_entry_t
z_frag_page_t::get_frag_entry_x()
{
...
z_frag_entry_t
z_frag_page_t::get_frag_entry_s()
{
	fil_addr_t node_loc = get_frag_entry_addr();
...
std::ostream&
z_frag_page_t::print_frags_in_order(std::ostream& out) const
{
	if (m_block == nullptr) {
		return(out);
	}
...
ulint
z_frag_page_t::get_total_stored_data() const
{
	ulint len = 0;
...
ulint
z_frag_page_t::get_total_free_len() const
{
	ulint len = 0;
...
ulint
z_frag_page_t::get_big_free_len() const
{
	ulint big = 0;
	...
	byte*		buf)
{
	...
	cached_blocks.insert(
		std::pair<page_no_t, buf_block_t*>(
			page_no, first_page.get_block()));
	...
	while (!fil_addr_is_null(node_loc) && want > 0) {
		...
		if (entry_lob_version > lob_version) {
			...
			while (!fil_addr_is_null(node_versions)) {
				...
				flst_node_t* node_old_version
					= first_page.addr2ptr_s_cache(
						cached_blocks, node_versions);
		...
		if (read_from_page_no != FIL_NULL) {
			...
			if (read_from_page_no == first_page_no) {
				...
				actual_read = first_page.read(
					page_offset, ptr, want);
				ptr += actual_read;
				want -= actual_read;
...
ulint
z_index_page_t::get_n_index_entries() const
{
