...
@param[in]	to_entry	The index entry to which the version base
				node is moved to. */
void
index_entry_t::move_version_base_node(
	index_entry_t&	to_entry)
{
...
fil_addr_t
index_entry_t::make_old_version_current(
	dict_index_t*	index,
	trx_id_t	trxid,
...
std::ostream&
index_entry_t::print(std::ostream& out) const
{
	if (!is_null()) {
		out << "[index_entry_t: node=" << (void*)m_node
			<< ", self=" << get_self()
...
void
index_entry_t::read(index_entry_mem_t& entry_mem) const
{
	if (is_null()) {
		entry_mem.reset();
		return;
...
buf_block_t*
index_entry_t::load_x(const fil_addr_t& addr)
{
	ut_ad(m_mtr != nullptr);
	...
	m_node = fut_get_ptr(space_id, page_size, addr, RW_X_LATCH,
			     m_mtr, &m_block);
...
buf_block_t* index_entry_t::load_s(const fil_addr_t& addr)
{
	...
	m_node = fut_get_ptr(space_id, page_size, addr, RW_S_LATCH,
			     m_mtr, &m_block);
...
std::ostream&
index_entry_mem_t::print(std::ostream& out) const
{
