	...
	mtr_t*		mtr)
{
	...
	DBUG_LOG("lob::first_page_t", "old data="
		 << PrintBuffer(old_ptr - data_to_copy, data_to_copy));
	/* The new data that replaces old data. */
	DBUG_LOG("lob::first_page_t", "new data="
		 << PrintBuffer(new_ptr - data_to_copy, data_to_copy));
#endif /* UNIV_DEBUG */
...
std::ostream&
first_page_t::print_index_entries_cache_s(
...
std::ostream&
first_page_t::print_index_entries(
...
flst_node_t*
first_page_t::alloc_index_entry(
	bool	bulk)
{
...
void
first_page_t::free_all_index_pages()
{
	...
	const page_size_t&	page_size)
{
	...
	case FIL_PAGE_SDI_BLOB:
		...
		break;
	default:
		std::cerr << "Unexpected LOB first page type=" << page_type
			<< std::endl;
...
ulint first_page_t::write(
	trx_id_t	trxid,
	const byte*&	data,
	ulint&		len)
{
	byte* ptr = data_begin();
	ulint written = (len > max_space_available())
		? max_space_available() : len;
...
void
first_page_t::import(trx_id_t trx_id)
{
