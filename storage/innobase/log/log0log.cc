...
static void log_downgrade() {
  ...
  err = fil_redo_io(IORequestLogWrite, page_id_t(group->space_id, page_no),
                    univ_page_size,
...
void log_buffer_extend(ulint len) {
  ulint move_start;
  ulint move_end;
  ...
  while (
      ut_calc_align_down(log_sys->buf_free, OS_FILE_LOG_BLOCK_SIZE) !=
      ut_calc_align_down(log_sys->buf_next_to_write, OS_FILE_LOG_BLOCK_SIZE)) {
  ...
  move_start = ut_calc_align_down(log_sys->buf_free, OS_FILE_LOG_BLOCK_SIZE);
  move_end = log_sys->buf_free;

  /* store the last log block in buffer */
  ut_memcpy(tmp_buf, log_sys->buf + move_start, move_end - move_start);
  ...
  ib::info() << "innodb_log_buffer_size was extended to " << LOG_BUFFER_SIZE
             << ".";
}
...
static inline ulint log_calculate_actual_len(ulint len) {
  ut_ad(log_mutex_own());

  /* actual length stored per block */
  const ulint len_per_blk =
...
void log_margin_checkpoint_age(ulint len) {
  ...
  if (margin > log_sys->log_group_capacity) {
    ...
    if (!log_has_printed_chkp_margine_warning ||
        difftime(time(NULL), log_last_margine_warning_time) > 15) {
  ...
  if (log_sys->lsn - log_sys->last_checkpoint_lsn + margin >
    ...
    if (log_sys->lsn - log_buf_pool_get_oldest_modification() + margin <=
        log_sys->log_group_capacity) {
      flushed_enough = true;
    }
...
loop:
  ...
  len_upper_limit =
      LOG_BUF_WRITE_MARGIN + srv_log_write_ahead_size + (5 * len) / 4;
...
/*===========*/
{
  ...
  if (checkpoint_age >= log->log_group_capacity) {
    DBUG_EXECUTE_IF("print_all_chkp_warnings",
  ...
  if (!oldest_lsn || lsn - oldest_lsn > log->max_modified_age_sync ||
...
/*==========*/
{
  ...
  log_sys->checkpoint_buf_ptr =
      static_cast<byte *>(ut_zalloc_nokey(2 * OS_FILE_LOG_BLOCK_SIZE));
                         ...
                         group */
{
  ...
  group->file_header_bufs_ptr =
      static_cast<byte **>(ut_zalloc_nokey(sizeof(byte *) * n_files));
  ...
  for (i = 0; i < n_files; i++) {
    group->file_header_bufs_ptr[i] = static_cast<byte *>(
        ut_zalloc_nokey(LOG_FILE_HDR_SIZE + OS_FILE_LOG_BLOCK_SIZE));
...
void log_io_complete(log_group_t *group) {
  ...
  DBUG_PRINT("ib_log",
             ("checkpoint info written to group %u", unsigned(group->id)));
}
...
void log_header_fill(byte *buf, lsn_t start_lsn, const char *creator) {
...
void log_group_file_header_flush(log_group_t *group, ulint nth_file,
                                 lsn_t start_lsn) {
  byte *buf;
  lsn_t dest_offset;
  ...
  DBUG_PRINT("ib_log",
             ("write " LSN_PF " group " ULINTPF " file " ULINTPF " header",
...
bool log_write_encryption(byte *key, byte *iv, bool is_boot) {
  ...
  err = fil_redo_io(IORequestLogWrite, page_id, univ_page_size,
                    LOG_CHECKPOINT_1 + OS_FILE_LOG_BLOCK_SIZE,
                    OS_FILE_LOG_BLOCK_SIZE, log_block_buf);
...
loop:
  ...
  if (write_header && next_offset % group->file_size == LOG_FILE_HDR_SIZE) {
    /* We start to write a new log file instance in the group */

    ut_a(next_offset / group->file_size <= ULINT_MAX);

    log_group_file_header_flush(group, (ulint)(next_offset / group->file_size),
                                start_lsn);
  ...
  if ((next_offset % group->file_size) + len > group->file_size) {
    ...
    write_len = (ulint)(group->file_size - (next_offset % group->file_size));
  } else {
    write_len = len;
  }
  ...
  DBUG_PRINT(
      "ib_log",
      ("write " LSN_PF " to " LSN_PF ": group " ULINTPF " len " ULINTPF
       " blocks " ULINTPF ".." ULINTPF,
       start_lsn, next_offset, group->id, write_len, log_block_get_hdr_no(buf),
       log_block_get_hdr_no(buf + write_len - OS_FILE_LOG_BLOCK_SIZE)));

  ut_ad(pad_len >= len ||
        log_block_get_hdr_no(buf) == log_block_convert_lsn_to_no(start_lsn));

  /* Calculate the checksums for each log block and write them to
  the trailer fields of the log blocks */

  for (i = 0; i < write_len / OS_FILE_LOG_BLOCK_SIZE; i++) {
    ut_ad(pad_len >= len || i * OS_FILE_LOG_BLOCK_SIZE >= len - pad_len ||
  ...
  const page_no_t page_no =
      static_cast<page_no_t>(next_offset / univ_page_size.physical());
  ...
  err = fil_redo_io(IORequestLogWrite, page_id_t(group->space_id, page_no),
                    univ_page_size, (ulint)(next_offset % UNIV_PAGE_SIZE),
                    write_len, buf);
...
static inline void log_buffer_switch() {
  ut_ad(log_mutex_own());
  ut_ad(log_write_mutex_own());

  const byte *old_buf = log_sys->buf;
  ulint area_end = ut_calc_align(log_sys->buf_free, OS_FILE_LOG_BLOCK_SIZE);

  if (log_sys->first_in_use) {
    ut_ad(log_sys->buf == ut_align(log_sys->buf_ptr, OS_FILE_LOG_BLOCK_SIZE));
  ...
  ut_memcpy(log_sys->buf, old_buf + area_end - OS_FILE_LOG_BLOCK_SIZE,
            OS_FILE_LOG_BLOCK_SIZE);
...
loop:
  ...
  lsn_t limit_lsn =
      flush_to_disk ? log_sys->flushed_to_disk_lsn : log_sys->write_lsn;
  ...
  DBUG_PRINT("ib_log",
             ("write " LSN_PF " to " LSN_PF, log_sys->write_lsn, log_sys->lsn));
  ...
  if (write_ahead_size > OS_FILE_LOG_BLOCK_SIZE) {
    ...
    end_offset = log_group_calc_lsn_offset(
        ut_uint64_align_up(write_lsn, OS_FILE_LOG_BLOCK_SIZE), group);
  ...
  log_group_write_buf(
      group, write_buf + area_start, area_end - area_start + pad_size,
#ifdef UNIV_DEBUG
      pad_size,
#endif /* UNIV_DEBUG */
      ut_uint64_align_down(log_sys->write_lsn, OS_FILE_LOG_BLOCK_SIZE),
    ...
    bool flush) /*!< in: flush the logs to disk */
{
  ...
  if (flush && log_sys->n_pending_flushes > 0 &&
...
static bool log_preflush_pool_modified_pages(lsn_t new_oldest) {
  ...
  if (new_oldest == LSN_MAX || !buf_page_cleaner_is_active ||
...
static void log_complete_checkpoint() {
  ...
  DBUG_PRINT("ib_log",
             ("checkpoint ended at " LSN_PF ", flushed to " LSN_PF,
              log_sys->last_checkpoint_lsn, log_sys->flushed_to_disk_lsn));
    ...
    log_group_t *group) /*!< in: log group */
{
  ...
  DBUG_PRINT(
      "ib_log",
      ("checkpoint " UINT64PF " at " LSN_PF " written to group " ULINTPF,
       log_sys->next_checkpoint_no, log_sys->next_checkpoint_lsn, group->id));
  ...
  err = fil_io(
      IORequestLogWrite, false, page_id_t(group->space_id, 0), univ_page_size,
...
void log_group_header_read(const log_group_t *group, ulint header) {
  ut_ad(log_mutex_own());
...
void log_write_checkpoint_info(bool sync) {
  log_group_t *group;

  ut_ad(log_mutex_own());

  if (!srv_read_only_mode) {
    for (group = UT_LIST_GET_FIRST(log_sys->log_groups); group;
         group = UT_LIST_GET_NEXT(log_groups, group)) {
...
bool log_checkpoint(bool sync, bool write_always) {
  ...
  DBUG_EXECUTE_IF("using_wa_checkpoint_middle", if (write_always) {
    DEBUG_SYNC_C("wa_checkpoint_middle");

    const bool b = TRUE;
    buf_flush_page_cleaner_disabled_debug_update(NULL, NULL, NULL, &b);

    dict_stats_disabled_debug_update(NULL, NULL, NULL, &b);

    srv_master_thread_disabled_debug_update(NULL, NULL, NULL, &b);
  });
...
loop:
  ...
  if (thread_name != NULL) {
    ...
    if (srv_print_verbose_log && count > 600) {
      ib::info() << "Waiting for " << thread_name << " to exit";
      count = 0;
    }
  ...
  if (pending_io) {
    if (srv_print_verbose_log && count > 600) {
      ib::info() << "Waiting for " << pending_io
                 << " buffer"
  ...
  if (srv_fast_shutdown == 2) {
    if (!srv_read_only_mode) {
      ...
      if (thread_name != NULL) {
        ib::warn() << "Background thread " << thread_name
                   << " woke up during"
                      " shutdown";
        goto loop;
      }
    ...
    FILE *file) /*!< in: file where to print */
{
  ...
  fprintf(file,
          ...
          log_buf_pool_get_oldest_modification(), log_sys->last_checkpoint_lsn);
...
void log_get_last_block(lsn_t &last_lsn, byte *last_block,
                        uint32_t &block_len) {
  byte *src_block;
  ...
  if (block_len == 0) {
    ut_ad(last_lsn == ut_calc_align_down(last_lsn, OS_FILE_LOG_BLOCK_SIZE));
