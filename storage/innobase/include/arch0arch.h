...
const uint MAX_ARCH_LOG_FILE_NAME_LEN =
		sizeof(ARCH_DIR) + 1 + sizeof(ARCH_LOG_DIR)
		+ MAX_LSN_DECIMAL_DIGIT + 1 + sizeof(ARCH_LOG_FILE)
		+ MAX_LSN_DECIMAL_DIGIT + 1;

/** Max string length for archive page file name */
const uint MAX_ARCH_PAGE_FILE_NAME_LEN =
		sizeof(ARCH_DIR) + 1 + sizeof(ARCH_PAGE_DIR)
		+ MAX_LSN_DECIMAL_DIGIT + 1 + sizeof(ARCH_PAGE_FILE)
		+ MAX_LSN_DECIMAL_DIGIT + 1;
...
@startuml
  ...
  ARCH_CLIENT_STATE_INIT -down-> ARCH_CLIENT_STATE_STARTED : Attach and start archiving
  ARCH_CLIENT_STATE_STARTED -right-> ARCH_CLIENT_STATE_STOPPED : Stop archiving
...
public:
	...
	dberr_t init_file_ctx(
		...
		ib_uint64_t	file_size)
	{
		return(m_file_ctx.init(path, base_dir, base_file,
		       num_files, file_size));
	}
...
public:
	...
	bool is_init()
	{
		return(m_state == ARCH_STATE_INIT);
	}
