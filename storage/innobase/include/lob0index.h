...
struct index_entry_t
{
	...
	void
	add_version(index_entry_t& entry) const
	{
		flst_node_t* node = entry.get_node_ptr();
		flst_base_node_t* version_list = get_versions_ptr();
		flst_add_first(version_list, node, m_mtr);
...
private:
	...
	{
		return(m_node + OFFSET_TRX_UNDO_NO);
	}

	byte* get_lob_version_ptr() const
	{
		return(m_node + OFFSET_LOB_VERSION);
	}
	...
	{
		return(m_node + OFFSET_PAGE_NO);
	}

	byte* get_datalen_ptr() const
	{
		return(m_node + OFFSET_DATA_LEN);
	}
