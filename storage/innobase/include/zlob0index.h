...
struct z_index_entry_mem_t
{
	...
	bool is_null()
	{
		return(m_self.is_equal(fil_addr_null));
	}
};
...
struct z_index_entry_t
{
	...
	static const ulint OFFSET_TRXID
		= OFFSET_VERSIONS + FLST_BASE_NODE_SIZE;
		...
		undo_no_t	undo_no)
	{
		...
		return((trxid == get_trx_id())
		       && (get_trx_undo_no() >= undo_no));
	}
	...
	void set_z_page_no(page_no_t page_no) {
		ut_ad(m_mtr != nullptr);
		mlog_write_ulint(m_node + OFFSET_Z_PAGE_NO, page_no,
				 MLOG_4BYTES, m_mtr);
	}
	...
	void set_z_frag_id(frag_id_t id) {
		ut_ad(m_mtr != nullptr);
		mlog_write_ulint(m_node + OFFSET_Z_FRAG_ID, id,
				 MLOG_2BYTES, m_mtr);
	}
	...
	void set_data_len(ulint len) {
		ut_ad(m_mtr != nullptr);
		mlog_write_ulint(m_node + OFFSET_DATA_LEN, len,
				 MLOG_4BYTES, m_mtr);
	}
	...
	{
		ut_ad(m_mtr != nullptr);
		mlog_write_ulint(m_node + OFFSET_LOB_VERSION, version,
				 MLOG_4BYTES, m_mtr);
	}
	...
	void load_s(fil_addr_t& addr)
	{
		...
		const page_size_t page_size = dict_table_page_size(
			m_index->table);
		m_node = fut_get_ptr(space, page_size, addr, RW_S_LATCH,
				     m_mtr, &m_block);
	...
	void load_x(fil_addr_t& addr)
	{
		...
		const page_size_t page_size = dict_table_page_size(
			m_index->table);
		m_node = fut_get_ptr(space, page_size, addr, RW_X_LATCH,
				     m_mtr, &m_block);
		m_page_no = m_block->get_page_no();
	}
