...
struct z_first_page_t
{
	...
	static const ulint OFFSET_INDEX_LIST
		= OFFSET_FREE_LIST + FLST_BASE_NODE_SIZE;
	...
	{
		ut_ad(m_mtr != nullptr);
		mlog_write_ulint(frame() + OFFSET_INDEX_PAGE_NO,
				 page_no, MLOG_4BYTES, m_mtr);
	}
	...
	{
		ut_ad(m_mtr != nullptr);
		mlog_write_ulint(frame() + OFFSET_FRAG_NODES_PAGE_NO,
				 page_no, MLOG_4BYTES, m_mtr);
	}
	...
	void set_page_type_unknown()
	{
		...
		mlog_write_ulint(frame() + FIL_PAGE_TYPE,
				 FIL_PAGE_TYPE_UNKNOWN, MLOG_2BYTES,
				 m_mtr);
	}
	...
	void set_page_type()
	{
		ut_ad(m_mtr != nullptr);
		mlog_write_ulint(frame() + FIL_PAGE_TYPE,
				 FIL_PAGE_TYPE_ZLOB_FIRST, MLOG_2BYTES,
				 m_mtr);
	}
	...
	flst_node_t* addr2ptr_x(fil_addr_t& addr) const
	{
		...
		return(fut_get_ptr(space, page_size, addr, RW_X_LATCH,
				   m_mtr));
	}
	...
	flst_node_t* addr2ptr_s(fil_addr_t& addr)
	{
		...
		const page_size_t page_size
			= dict_table_page_size(m_index->table);
		return (fut_get_ptr(space, page_size, addr, RW_S_LATCH,
				    m_mtr));
	}
	...
	static
	void destroy(dict_index_t* index, page_no_t first_page_no);
