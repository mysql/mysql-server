...
void
recv_recover_page_func(buf_block_t* block);
...
class MetadataRecover {
	...
	using PersistentTables =
		std::map<table_id_t,
			 PersistentTableMetadata*,
			 std::less<table_id_t>,
			 ut_allocator<
				 std::pair<const table_id_t,
					PersistentTableMetadata*>>>;
...
struct recv_sys_t {
	...
	using Pages = std::unordered_map<
		page_no_t, recv_addr_t*,
		std::hash<page_no_t>,
	...
	struct Space {
		...
		explicit Space(mem_heap_t* heap)
			:
			m_heap(heap),
			m_pages()
		{ }
