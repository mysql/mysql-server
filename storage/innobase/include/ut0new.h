...
Has to work recursively due to C++11 constexpr constraints (C++14 is more flexible).
...
constexpr bool
ut_string_begins_with(const char *a, const char *b, size_t b_len, size_t index = 0)
{
	return(index == b_len ||
		(a[index] == b[index] && ut_string_begins_with(a, b, b_len, index + 1)));
}

/** Find the length of the filename without its file extension.
Has to work recursively due to C++11 constexpr constraints (C++14 is more flexible).
@param[in]	file	filename, with extension but without directory
@param[in]	index	character index to start scanning for extension separator at
@return length, in bytes */
constexpr size_t
ut_len_without_extension(const char *file, size_t index = 0)
{
	return((file[index] == '\0' || file[index] == '.') ? index :
...
{
	return((index == n_auto) ? PSI_NOT_INSTRUMENTED :
		(ut_string_begins_with(auto_event_names[index], file, len) ? auto_event_keys[index] :
...
#define THIS_FILE_PSI_KEY ut_new_get_key_by_file(MY_BASENAME)
...
#define UT_NEW(expr, key) \
	...
	::new(ut_allocator<byte>(key).allocate( \
		sizeof expr, NULL, THIS_FILE_PSI_KEY, false, false)) expr
...
#define UT_NEW_ARRAY(type, n_elements, key) \
	ut_allocator<type>(key).new_array(n_elements, THIS_FILE_PSI_KEY)
...
#define ut_malloc(n_bytes, key)		static_cast<void*>( \
	ut_allocator<byte>(key).allocate( \
		n_bytes, NULL, THIS_FILE_PSI_KEY, false, false))

#define ut_zalloc(n_bytes, key)		static_cast<void*>( \
	ut_allocator<byte>(key).allocate( \
		n_bytes, NULL, THIS_FILE_PSI_KEY, true, false))
