...
struct first_page_t : public basic_page_t
{
	...
	uint8_t get_flags()
	{
		return(mach_read_from_1(frame() + OFFSET_FLAGS));
	}
	...
	@return		the file list node.*/
	flst_node_t*
	addr2ptr(const fil_addr_t& addr)
	{
	...
	flst_node_t*
	addr2ptr_s_cache(
		std::map<page_no_t, buf_block_t*>& cache,
		fil_addr_t&	addr) const
	{
		byte* result;
		space_id_t space = dict_index_get_space(m_index);
	...
	void init_lob_version()
	{
		...
		mlog_write_ulint(frame() + OFFSET_LOB_VERSION, 1,
				 MLOG_4BYTES, m_mtr);
	}
	...
	{
		return(frame() + OFFSET_INDEX_LIST);
	}
	...
	{
		const uint16_t index_array_size
			= node_count() * index_entry_t::SIZE;
	...
	std::ostream& print_index_entries_cache_s(
		std::ostream&	out,
	...
	byte* data_begin() const
	{
		...
		constexpr uint16_t index_array_size
			= node_count() * index_entry_t::SIZE;
