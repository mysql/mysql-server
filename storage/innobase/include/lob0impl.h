...
public:
	...
	{
		return(mach_read_from_2(m_node + OFFSET_PREV));
	}
	...
	{
		return(mach_read_from_2(m_node + OFFSET_NEXT));
	}
	...
	{
		out << "[plist_node_t: next=" << get_next() << ", prev="
			<< get_prev() << ", this=" << addr() << ", frame="
			<< (void *) m_frame << ", m_node="
			<< (void *) m_node << "]";
		return(out);
	}
...
struct plist_base_node_t
{
	...
	ulint get_len() const
	{
		return(mach_read_from_4(m_base + OFFSET_LEN));
	}
	...
	{
		return(mach_read_from_2(m_base + OFFSET_LAST));
	}
	...
	void incr_len()
	{
		...
		mlog_write_ulint(m_base + OFFSET_LEN, len + 1,
				 MLOG_4BYTES, m_mtr);
	}
	...
	void set_first(paddr_t addr)
	{
		...
		mlog_write_ulint(m_base + OFFSET_FIRST, addr,
				 MLOG_2BYTES, m_mtr);
	}
	...
	void set_last(paddr_t addr)
	{
		...
		mlog_write_ulint(m_base + OFFSET_LAST, addr,
				 MLOG_2BYTES, m_mtr);
	}
	...
	paddr_t addr() const
	{
		return(m_base - m_frame);
	}

	std::ostream& print(std::ostream& out) const
	{
		out << "[plist_base_node_t: len=" << get_len()
...
public:
	...
	z_frag_entry_t()
	:
	m_node(nullptr), m_mtr(nullptr)
	{}

	/** Constructor. */
	z_frag_entry_t(mtr_t* mtr)
	:
	m_node(nullptr), m_mtr(mtr)
	{}
	...
	void set_page_no(page_no_t page_no) const
	{
		...
		mlog_write_ulint(m_node + OFFSET_PAGE_NO, page_no,
				 MLOG_4BYTES, m_mtr);
	}
	...
	void set_n_frags(ulint frags) const
	{
		...
		mlog_write_ulint(m_node + OFFSET_N_FRAGS, frags,
				 MLOG_2BYTES, m_mtr);
	}
	...
	void set_used_len(ulint used) const
	{
		...
		mlog_write_ulint(m_node + OFFSET_USED_LEN, used,
				 MLOG_2BYTES, m_mtr);
	}
	...
	void set_total_free_len(ulint n)
	{
		...
		mlog_write_ulint(m_node + OFFSET_TOTAL_FREE_LEN, n,
				 MLOG_2BYTES, m_mtr);
	}
	...
	void set_big_free_len(ulint n)
	{
		...
		mlog_write_ulint(m_node + OFFSET_BIG_FREE_LEN, n,
				 MLOG_2BYTES, m_mtr);
	}
...
struct z_index_page_t
{
	...
	explicit z_index_page_t(mtr_t* mtr)
	:
	m_block(nullptr),
	m_mtr(mtr)
	{}
	...
	{
		mlog_write_ulint(frame() + FIL_PAGE_SPACE_ID, space_id,
				 MLOG_4BYTES, nullptr);
	}
	...
	void set_next_page_no(page_no_t page_no)
	{
		...
		mlog_write_ulint(frame() + FIL_PAGE_NEXT, page_no,
				 MLOG_4BYTES, m_mtr);
	}
	...
	ulint payload() const
	{
		...
		return(page_size.physical() - FIL_PAGE_DATA_END
		       - LOB_PAGE_DATA);
	}
...
struct z_data_page_t
{
	...
	{
		page_size_t page_size(dict_table_page_size(m_index->table));
		return(page_size.physical() - OFFSET_DATA_BEGIN
		       - FIL_PAGE_DATA_END);
	}
	...
	{
		return(frame() + OFFSET_DATA_BEGIN);
	}
	...
	void set_data_len(ulint len)
	{
		...
		mlog_write_ulint(frame() + OFFSET_DATA_LEN, len,
				 MLOG_4BYTES, m_mtr);
	}
...
struct z_frag_node_page_t {
	...
	void set_version_0()
	{
		...
		mlog_write_ulint(frame() + OFFSET_VERSION, 0,
				 MLOG_1BYTE, m_mtr);
	}
...
struct frag_node_t {
	...
	m_mtr(mtr)
	{
		...
		mlog_write_ulint(m_node.ptr() + OFFSET_LEN, len,
				 MLOG_2BYTES, mtr);
	}

	byte* frag_begin() const
	{
		return(m_node.ptr() + OFFSET_DATA);
	}
	...
	void incr_length_by_2()
	{
		...
		mlog_write_ulint(m_node.ptr() + OFFSET_LEN,
				 len + SIZE_OF_PAGE_DIR_ENTRY,
				 MLOG_2BYTES, m_mtr);
	}
	...
	void set_frag_id_null()
	{
		...
		mlog_write_ulint(m_node.ptr() + OFFSET_FRAG_ID,
				 FRAG_ID_NULL, MLOG_2BYTES, m_mtr);
	}
	...
	void set_frag_id(ulint id)
	{
		...
		mlog_write_ulint(m_node.ptr() + OFFSET_FRAG_ID, id,
				 MLOG_2BYTES, m_mtr);
	}
	...
	std::ostream& print(std::ostream& out) const
	{
		if (!m_node.is_null()) {
			...
			out << "[frag_node_t: " << m_node << ", len=" << len
				<< "/" << payload() << ", frag_id="
				<< get_frag_id() << "]";
		} else {
	...
	{
		return(get_next_frag());
	}

	frag_node_t get_prev_node()
	{
		return(get_prev_frag());
	}
...
struct z_frag_page_t
{
	...
	static const ulint OFFSET_FREE_LIST
		= OFFSET_FRAG_ENTRY + FIL_ADDR_SIZE;
	...
	{
		return(flst_write_addr(frame() + OFFSET_FRAG_ENTRY,
				       fil_addr_null, m_mtr));
	}

	ulint get_n_dir_entries() const
	{
		byte* ptr = frame() + get_page_size()
	...
	byte* slots_end_ptr() const
	{
		...
		byte* first = frame() + get_page_size()
			- OFFSET_PAGE_DIR_ENTRY_COUNT;
	...
	std::ostream& print_frag_id(std::ostream& out)
	{
		...
		for (ulint frag_id = 0; frag_id < n; frag_id++) {
			out << "id=" << frag_id << ", addr="
				<< frag_id_to_addr(frag_id) << std::endl;
		}
	...
	void split_free_frag(frag_node_t& free_frag, ulint size)
	{
		...
		ut_a(free_frag.get_total_len() + frag2.get_total_len()
		     == old_total_len);
	...
	void set_page_type()
	{
		...
		mlog_write_ulint(ptr, FIL_PAGE_TYPE_ZLOB_FRAG, MLOG_2BYTES,
				 m_mtr);
	}
	...
	std::ostream& print_free_list(std::ostream& out) const
	{
		...
		for (plist_node_t cur = free_lst.get_first_node();
		     !cur.is_null(); cur = cur.get_next_node()) {
	...
	std::ostream& print_frag_list(std::ostream& out) const
	{
		...
		for (plist_node_t cur = frag_lst.get_first_node();
		     !cur.is_null(); cur = cur.get_next_node()) {
