...
public:
	...
	{
		return(m_next_state != CLONE_SNAPSHOT_NONE);
	}
	...
	mem_heap_t* get_heap()
	{
		return(m_clone_snapshot->get_heap());
	}
...
public:
	...
	dberr_t add_task()
	{
		return(m_clone_task_manager.add_task());
	}
	...
	bool is_copy_clone() {
		...
		return(m_clone_handle_type == CLONE_HDL_COPY);
	}
	...
	bool is_active() {
		...
		return(m_clone_handle_state == CLONE_STATE_ACTIVE);
	}
	...
	bool is_init() {
		...
		return(m_clone_handle_state == CLONE_STATE_INIT);
	}
...
public:
	...
	Clone_Snapshot* attach_snapshot(
