	...
	ib_trx_t	ib_trx)		/*!< in: trx handle */
{
	...
	err = trx_rollback_for_mysql(trx);
	...
	trx_t*		trx)		/*!< in: transaction */
{
	...
	if (heap != NULL) {
		...
		cursor->prebuilt = row_create_prebuilt(table);
	...
	ib_crsr_t*	ib_crsr)	/*!< out,own: InnoDB cursor */
{
	...
	if (ib_trx != NULL) {
	       if (!ib_schema_lock_is_exclusive(ib_trx)) {
			...
			/* FIXME: Need to check if we need to acquire
			MySQL metadata lock */
	...
	ib_cursor_t*	cursor)		/*!< in: current cursor */
{
	...
	if (node->upd == NULL) {
		node->upd = row_create_update_node_for_mysql(table, heap);
	}
	...
	ib_srch_mode_t	ib_srch_mode)	/*!< in: search mode */
{
	...
	err = row_search_for_mysql(
		(unsigned char *) &buf, ib_srch_mode,
		prebuilt, cursor->match_mode, 0);
	...
	ib_ulint_t	len)		/*!< in: data value len */
{
	...
	case DATA_FLOAT:
		if (len == sizeof(float)) {
			mach_float_write(dst, *(float*)src);
		} else {
			return(DB_DATA_MISMATCH);
		}
		break;
	...
	case DATA_DOUBLE:
		if (len == sizeof(double)) {
			mach_double_write(dst, *(double*)src);
		} else {
			return(DB_DATA_MISMATCH);
		}
		break;
	...
	ib_ulint_t	len)		/*!< in: max data value len to copy */
{
	...
	if (data_len != UNIV_SQL_NULL) {
		...
		case DATA_INT: {
			...
			usign = dtype_get_prtype(dtype) & DATA_UNSIGNED;
			ret = mach_read_int_type(data, data_len, usign);
	...
	*table_id = 0;
	...
	if (err == DB_SUCCESS) {
		...
		*ib_crsr = NULL;
		...
		on success. */
		err = row_truncate_table_for_mysql(table, trx);
	...
	ib_id_t*	table_id)	/*!< out: new table id */
{
	...
	if (trunc_err == DB_SUCCESS) {
		ut_a(ib_trx_state(ib_trx) == TRX_STATE_NOT_STARTED);
