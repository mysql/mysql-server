...
dberr_t
Page_Arch_Client_Ctx::start()
{
	dberr_t	err;

	err = arch_page_sys->start(&m_group, &m_start_lsn,
				   &m_start_pos, false);
...
@return error code */
dberr_t
Page_Arch_Client_Ctx::get_from_file(
	...
	byte*		read_buff)
{
	...
	/* Build file name */
	file_index = static_cast<uint>(read_pos->m_block_num
				       / ARCH_PAGE_FILE_CAPACITY);

	m_group->get_file_name(file_index, file_name,
...
@return error code */
dberr_t
Page_Arch_Client_Ctx::get_pages(
	Page_Arch_Cbk*	cbk_func,
	void*		cbk_ctx,
	byte*		buff,
	uint		buf_len)
{
	...
	while (true) {
		...
		success = arch_page_sys->get_pages(m_group, &cur_pos,
						   read_len, buff);
...
void
Arch_Block::end_write()
{
	m_state = ARCH_BLOCK_READY_TO_FLUSH;
}
	...
	Arch_Blk_Flush_Type	type)
{
	...
	/* Update block header. */
	mach_write_to_4(m_data + ARCH_PAGE_BLK_HEADER_VERSION, ARCH_PAGE_FILE_VERSION);
...
ArchPageData::init()
{
	...
	for (index = 0; index < m_num_blocks; index++) {
		...
		cur_blk = UT_NEW(Arch_Block(mem_ptr, m_block_size),
				 mem_key_archive);
...
void
Arch_Page_Sys::track_initial_pages()
{
	uint		index;
	buf_pool_t*	buf_pool;
	...
	for (index = 0; index < srv_buf_pool_instances; ++index) {
		...
		while (bpage != NULL) {
			...
			if (buf_page_get_io_fix_unlocked(bpage)
				== BUF_IO_WRITE) {
...
void
Arch_Page_Sys::set_tracking_buf_pool(
	lsn_t	tracking_lsn)
{
	uint		index;
	buf_pool_t*	buf_pool;
	...
	bool		is_durable)
{
	...
	if (m_current_group == nullptr) {
		...
		m_current_group = UT_NEW(Arch_Group(log_sys_lsn,
			ARCH_PAGE_FILE_HDR_SIZE, &m_mutex), mem_key_archive);
		...
		err = m_current_group->init_file_ctx(ARCH_DIR, ARCH_PAGE_DIR,
			ARCH_PAGE_FILE, 0, static_cast<ib_uint64_t>
                        (ARCH_PAGE_BLK_SIZE) * ARCH_PAGE_FILE_CAPACITY);
	...
	m_current_group->attach(m_last_lsn,
		m_last_pos.m_block_num, is_durable);
	...
	bool*	wait)
{
	...
	*wait = (cur_pos.m_block_num == end_pos.m_block_num);
	...
	if (set_idle || is_abort) {
		...
		ref_count = m_current_group->disable(LSN_MAX,
			m_flush_pos.m_block_num);
