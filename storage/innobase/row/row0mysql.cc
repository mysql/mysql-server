	...
	row_prebuilt_t*	prebuilt)
{
	...
	if (err != DB_SUCCESS) {
		...
		for (dict_index_t* index =
			...
			if (clust_index_updated == false && k == 0) {
				continue;
			}
	...
	const index_update_t&	update_index)
{
	...
	for (dict_index_t* index = UT_LIST_GET_FIRST(table->indexes);
	     ...
	     index = UT_LIST_GET_NEXT(indexes, index), ++itr) {
		...
		entry generated by explicit_rollback. Search for original
		entry to delete. */
	...
	const index_update_t&	update_index)
{
	...
	for (dict_index_t* index = UT_LIST_GET_FIRST(table->indexes);
	     index != NULL && err == DB_SUCCESS;
	     index = UT_LIST_GET_NEXT(indexes, index), inserted_upto++) {

		if (!update_index[inserted_upto]) {
			continue;
		}
	...
	/* Explicitly rollback half-cooked insertion if error is encouter while
	inserting entries in one of the index. */
	if (err != DB_SUCCESS) {
		...
		for (dict_index_t* index =
			UT_LIST_GET_FIRST(node->table->indexes);
		     index != NULL && k < inserted_upto;
		     index = UT_LIST_GET_NEXT(indexes, index), k++) {

			entry = row_build_index_entry(
				node->upd_row, node->upd_ext, index, heap);

			row_explicit_rollback(index, entry, thr, &mtr);
		}
		mtr_commit(&mtr);
	...
	row_prebuilt_t*		prebuilt)
{
	...
	if (!dict_index_is_auto_gen_clust(clust_index)) {
		ordering_change = row_upd_changes_some_index_ord_field_binary(
