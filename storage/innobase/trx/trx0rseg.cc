	...
	mtr_t*			mtr)
{
	...
	ut_ad(mtr);
	ut_ad(mtr_memo_contains(mtr, fil_space_get_latch(space_id),
				MTR_MEMO_X_LOCK));
	...
	if (space_id == TRX_SYS_SPACE) {
		...
		trx_rsegsf_set_page_no(
			rsegs_header, rseg_slot, page_no, mtr);
	}
	...
	ulint		rseg_id)
{
	...
	page_no_t	page_no = trx_rsegsf_get_page_no(rsegs_header,
							 rseg_id, &mtr);
	...
	Rsegs*		rsegs)
{
	...
	for (ulint num = 0; num < FSP_MAX_ROLLBACK_SEGMENTS; num++) {
		...
		rseg = trx_rseg_mem_create(
			rseg_id, space_id, page_no, univ_page_size,
	...
	if (n_known < target_rsegs) {
		if (srv_read_only_mode || srv_force_recovery > 0) {
			...
			bool use_and = srv_read_only_mode
				       && srv_force_recovery > 0;

			ib::info() << "Could not create all " << target_rsegs
				<< " rollback segments in " << loc.str()
				<< " because "
				<< (srv_read_only_mode ?
			...
			srv_rollback_segments = ut_min(
				srv_rollback_segments,
				static_cast<ulong>(n_known));
		...
		ib::info() << "Created " << n_created
			<< " and tracked " << n_tracked
			<< " new rollback segment(s) in " << loc.str()
			<< ". " << target_rsegs << " are now active.";

	} else if (n_tracked > 0) {
		ib::info() << "Using " << n_tracked
			<< " more rollback segment(s) in " << loc.str()
			<< ". " << target_rsegs
			<< " are now active.";

	} else if (target_rsegs < n_known) {
		ib::info() << target_rsegs
			<< " rollback segment(s) are now active in "
			<< loc.str() << ".";
	}
	...
	mtr_t*		mtr)
{
	...
	block = fseg_create(space_id, 0,
			    RSEG_ARRAY_HEADER + RSEG_ARRAY_FSEG_HEADER_OFFSET,
			    mtr);
	...
	mlog_write_ulint(page + FIL_PAGE_TYPE, FIL_PAGE_TYPE_RSEG_ARRAY,
			 MLOG_2BYTES, mtr);

	rsegs_header = page + RSEG_ARRAY_HEADER;

	/* Initialize the rseg array version. */
	mach_write_to_4(rsegs_header + RSEG_ARRAY_VERSION_OFFSET,
			RSEG_ARRAY_VERSION);
	...
	mlog_log_string(rsegs_header,
			UNIV_PAGE_SIZE - RSEG_ARRAY_HEADER - FIL_PAGE_DATA_END,
			mtr);
}
