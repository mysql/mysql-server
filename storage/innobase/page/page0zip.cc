                                     ...
                                     externally stored columns */
{
  ...
  for (i = 0; i < rec_offs_n_fields(offsets); i++) {
    ...
    if (UNIV_UNLIKELY(i == trx_id_col)) {
      ...
      src = rec_get_nth_field(rec, offsets, i, nullptr, &len);
      ut_ad(src + DATA_TRX_ID_LEN ==
            rec_get_nth_field(rec, offsets, i + 1, nullptr, &len));
    ...
    mem_heap_t *heap)                /*!< in: temporary memory heap */
{
  ...
  do {
    ...
    if (rec_offs_any_extern(offsets)) {
      ...
      src = rec_get_nth_field(rec, offsets, trx_id_col, nullptr, &len);
      ut_ad(src + DATA_TRX_ID_LEN ==
            rec_get_nth_field(rec, offsets, trx_id_col + 1, nullptr, &len));
      ut_ad(len == DATA_ROLL_PTR_LEN);
      UNIV_MEM_ASSERT_RW(rec, rec_offs_data_size(offsets));
      UNIV_MEM_ASSERT_RW(rec - rec_offs_extra_size(offsets),
                         rec_offs_extra_size(offsets));
                       ...
                       or NULL if no logging is needed */
{
  ...
  field = rec_get_nth_field(rec, offsets, n, nullptr, &len);
    ...
    const ulint *offsets)      /*!< in: rec_get_offsets(rec, index) */
{
  ...
  if (!page_is_leaf(page)) {
    ...
    storage = page_zip_dir_start(page_zip);
    field = const_cast<byte *>(
        rec_get_nth_field(rec, offsets, trx_id_pos, nullptr, &len));
    ut_ad(len == DATA_TRX_ID_LEN);
    ...
    if (rec_offs_any_extern(offsets)) {
      ...
      for (i = rec_offs_n_fields(offsets); i--;) {
        ...
        if (rec_offs_nth_extern(offsets, i)) {
          field = const_cast<byte *>(
              rec_get_nth_field(rec, offsets, i, nullptr, &len));
          ut_ad(len == BTR_EXTERN_FIELD_REF_SIZE);
