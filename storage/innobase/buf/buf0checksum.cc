...
bool
BlockReporter::is_corrupted() const
{
	ulint		checksum_field1;
	ulint		checksum_field2;

	if (!m_page_size.is_compressed()
	    && memcmp(m_read_buf + FIL_PAGE_LSN + 4,
		      m_read_buf + m_page_size.logical()
...
#error "FIL_PAGE_LSN must be 64 bit aligned"
	...
	if (stored == 0
	    && *reinterpret_cast<const ib_uint64_t*>(m_read_buf
		+ FIL_PAGE_LSN) == 0) {
		/* make sure that the page is really empty */
