...
uint _mi_ft_segiterator(FT_SEG_ITERATOR *ftsi) {
  ...
  if (!ftsi->num) DBUG_RETURN(0);

  ftsi->num--;
  if (!ftsi->seg) DBUG_RETURN(1);
...
FT_WORD *_mi_ft_parserecord(MI_INFO *info, uint keynr, const uchar *record,
  ...
  if (!(param = ftparser_call_initializer(info, keynr, 0))) DBUG_RETURN(NULL);
...
static int _mi_ft_store(MI_INFO *info, uint keynr, uchar *keybuf,
  ...
  for (; wlist->pos; wlist++) {
    ...
    if (_mi_ck_write(info, keynr, (uchar *)keybuf, key_length)) DBUG_RETURN(1);
  }
  DBUG_RETURN(0);
}
...
uint _mi_ft_convert_to_ft2(MI_INFO *info, uint keynr, uchar *key) {
  ...
  DBUG_RETURN(_mi_ck_real_write_btree(info, info->s->keyinfo + keynr, key, 0,
