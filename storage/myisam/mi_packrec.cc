...
typedef void (*unpack_function_t)(MI_COLUMNDEF *field, MI_BIT_BUFF *buff,
                                  uchar *to, uchar *end);
...
static void uf_zerofill_normal(MI_COLUMNDEF *rec, MI_BIT_BUFF *bit_buff,
                               uchar *to, uchar *end);
static void uf_constant(MI_COLUMNDEF *rec, MI_BIT_BUFF *bit_buff, uchar *to,
                        uchar *end);
static void uf_intervall(MI_COLUMNDEF *rec, MI_BIT_BUFF *bit_buff, uchar *to,
                         uchar *end);
static void uf_zero(MI_COLUMNDEF *rec, MI_BIT_BUFF *bit_buff, uchar *to,
                    uchar *end);
...
bool _mi_read_pack_info(MI_INFO *info, bool fix_keys) {
  ...
  if (mysql_file_read(file, (uchar *)header, sizeof(header), MYF(MY_NABP))) {
    if (!my_errno()) set_my_errno(HA_ERR_END_OF_FILE);
    goto err0;
  }
  ...
  if (fix_keys) share->rec_reflength = rec_reflength;
  share->base.min_block_length = share->min_pack_length + 1;
  if (share->min_pack_length > 254) share->base.min_block_length += 2;
  ...
  */
  if (!(share->decode_trees = (MI_DECODE_TREE *)my_malloc(
  ...
  */
  if (!(share->decode_tables = (uint16 *)my_malloc(
  ...
  for (i = 0; i < share->base.fields; i++) {
    ...
    share->rec[i].huff_tree =
        share->decode_trees + (uint)get_bits(&bit_buff, huff_tree_bits);
    share->rec[i].unpack = get_unpack_function(share->rec + i);
    DBUG_PRINT("info", ("col: %2u  type: %2u  pack: %u  slbits: %2u", i,
                        share->rec[i].base_type, share->rec[i].pack_type,
  ...
  */
  ...
  decode_table = (uint16 *)my_realloc(
      mi_key_memory_MYISAM_SHARE_decode_tables, (uchar *)share->decode_tables,
  ...
  if (fix_keys) {
    for (i = 0; i < share->base.keys; i++) {
      ...
      keyinfo->seg[keyinfo->flag & HA_FULLTEXT ? FT_SEGS : keyinfo->keysegs]
          .length = (uint16)rec_reflength;
    }
...
static uint read_huff_table(MI_BIT_BUFF *bit_buff, MI_DECODE_TREE *decode_tree,
                            uint16 **decode_table, uchar **intervall_buff,
                            uint16 *tmp_buff) {
  uint min_chr, elements, char_bits, offset_bits, size, intervall_length,
      table_bits, next_free_offset;
  uint16 *ptr, *end;
  ...
  if (!get_bits(bit_buff, 1)) {
    ...
    if (elements > 256) {
      DBUG_PRINT("error",
                 ("ERROR: illegal number of tree elements: %u", elements));
      DBUG_RETURN(1);
    }
  } else {
  ...
  DBUG_PRINT("info",
             ("tree size in bytes:    %u", size * (uint)sizeof(uint16)));
  ...
  if (!intervall_length) {
    ...
    if (table_bits >= OFFSET_TABLE_SIZE) DBUG_RETURN(1);
...
static void make_quick_table(uint16 *to_table, uint16 *decode_table,
  ...
  */
  if (!bits--) {
    ...
    */
    *next_free_offset =
        copy_decode_table(to_table, *next_free_offset, decode_table);
    DBUG_VOID_RETURN;
  }
  ...
  if (!(*decode_table & IS_CHAR)) {
    ...
    make_quick_table(to_table, decode_table + *decode_table, next_free_offset,
                     value, bits, max_bits);
  } else {
    /*
...
static uint copy_decode_table(uint16 *to_pos, uint offset,
  ...
  if (!(*decode_table & IS_CHAR)) {
    ...
    offset =
        copy_decode_table(to_pos, offset + 2, decode_table + *decode_table);
  } else {
    /* Copy the byte value. */
...
int _mi_read_pack_record(MI_INFO *info, my_off_t filepos, uchar *buf) {
  ...
  DBUG_RETURN(_mi_pack_rec_unpack(info, &info->bit_buff, buf, info->rec_buff,
                                  block_info.rec_len));
panic:
  set_my_errno(HA_ERR_WRONG_IN_RECORD);
err:
...
static int _mi_read_mempack_record(MI_INFO *info, my_off_t filepos,
  ...
  if (!(pos = (uchar *)_mi_mempack_get_block_info(
    ...
    DBUG_RETURN(-1);
  DBUG_RETURN(
      _mi_pack_rec_unpack(info, &info->bit_buff, buf, pos, block_info.rec_len));
}
