...
/** @file storage/innmem/src/handler.cc
InnMEM public handler API implementation. */
...
#include "innmem/row.h"
#include "innmem/storage.h" /* innmem::Storage */
#include "innmem/table.h"
#include "innmem/test.h"
#include "my_base.h"
#include "my_dbug.h"
...
int Handler::create(const char* table_name, TABLE* mysql_table, HA_CREATE_INFO*,
                    dd::Table*) {
  DBUG_ENTER("innmem::Handler::create");
  ...
  /* To run this test:
   * - CREATE TABLE t (__innmem_embedded_unit_tests CHAR(120) NOT NULL);
  ...
  if (mysql_table->s->fields == 1 &&
      strcmp(mysql_table->field[0]->field_name,
             "__innmem_embedded_unit_tests") == 0) {
    test(mysql_table);
  }
  ...
  DBUG_PRINT("innmem_api", ("this=%p %s; return=%s", this,
...
int Handler::delete_table(const char* table_name, const dd::Table*) {
  DBUG_ENTER("innmem::Handler::delete_table");
  ...
  DBUG_PRINT("innmem_api", ("this=%p %s; return=%s", this, table_name,
                            result_to_string(ret)));
...
int Handler::open(const char* table_name, int, uint, const dd::Table*) {
  DBUG_ENTER("innmem::Handler::open");
...
int Handler::close() {
  DBUG_ENTER("innmem::Handler::close");
  ...
  DBUG_PRINT("innmem_api", ("this=%p; return=%s", this, result_to_string(ret)));

  DBUG_RET(ret);
}

int Handler::rnd_init(bool) {
  DBUG_ENTER("innmem::Handler::rnd_init");
...
int Handler::rnd_next(uchar* mysql_row) {
  DBUG_ENTER("innmem::Handler::rnd_next");
...
int Handler::rnd_pos(uchar* mysql_row, uchar* position) {
  DBUG_ENTER("innmem::Handler::rnd_pos");
...
int Handler::rnd_end() {
  DBUG_ENTER("innmem::Handler::rnd_end");
...
int Handler::index_init(uint index_no, bool) {
  DBUG_ENTER("innmem::Handler::index_init");
  ...
  DBUG_PRINT("innmem_api", ("this=%p index=%d; return=%s", this, index_no,
                            result_to_string(ret)));
...
int Handler::index_read(uchar* mysql_row, const uchar* mysql_search_cells,
  ...
  DBUG_ENTER("innmem::Handler::index_read");
...
int Handler::index_next(uchar* mysql_row) {
  DBUG_ENTER("innmem::Handler::index_next");
...
int Handler::index_next_same(uchar* mysql_row, const uchar*, uint) {
  DBUG_ENTER("innmem::Handler::index_next_same");
...
Result Handler::index_next_conditional(uchar* mysql_row,
  ...
  DBUG_ENTER("innmem::Handler::index_next_conditional");
...
int Handler::index_read_last(uchar* mysql_row, const uchar* mysql_search_cells,
                             uint mysql_search_cells_len_bytes) {
  DBUG_ENTER("innmem::Handler::index_read_last");
...
int Handler::index_prev(uchar* mysql_row) {
  DBUG_ENTER("innmem::Handler::index_prev");
...
int Handler::index_end() {
  DBUG_ENTER("innmem::Handler::index_end");
...
void Handler::position(const uchar*) {
  DBUG_ENTER("innmem::Handler::position");
  ...
  *reinterpret_cast<Storage::Element**>(handler::ref) = row;
  ...
  DBUG_PRINT("innmem_api", ("this=%p; saved position=%p", this, row));

  DBUG_VOID_RETURN;
}

int Handler::write_row(uchar* mysql_row) {
  DBUG_ENTER("innmem::Handler::write_row");
...
int Handler::update_row(const uchar* mysql_row_old, uchar* mysql_row_new) {
  DBUG_ENTER("innmem::Handler::update_row");
...
int Handler::delete_row(const uchar* mysql_row) {
  DBUG_ENTER("innmem::Handler::delete_row");
...
int Handler::truncate(dd::Table*) {
  DBUG_ENTER("innmem::Handler::truncate");
...
int Handler::delete_all_rows() {
  DBUG_ENTER("innmem::Handler::delete_all_rows");
  DBUG_RETURN(truncate(nullptr));
}

int Handler::info(uint) {
  DBUG_ENTER("innmem::Handler::info");
  ...
  DBUG_PRINT("innmem_api", ("this=%p out=(stats.records=%llu); return=%s", this,
...
longlong Handler::get_memory_buffer_size() const {
  DBUG_ENTER("innmem::Handler::get_memory_buffer_size");

  DBUG_PRINT("innmem_api", ("this=%p; return=%lld", this, innmem_max_ram));

  DBUG_RETURN(innmem_max_ram);
}

const char* Handler::table_type() const {
  DBUG_ENTER("innmem::Handler::table_type");
  DBUG_RETURN("InnMEM");
}

::handler::Table_flags Handler::table_flags() const {
  DBUG_ENTER("innmem::Handler::table_flags");
  ...
  DBUG_PRINT("innmem_api", ("this=%p; return=%lld", this, flags));

  DBUG_RETURN(flags);
}

ulong Handler::index_flags(uint index_no, uint, bool) const {
  DBUG_ENTER("innmem::Handler::index_flags");
...
ha_rows Handler::estimate_rows_upper_bound() {
  DBUG_ENTER("innmem::Handler::estimate_rows_upper_bound");

  DBUG_ASSERT(m_opened_table != nullptr);

  const ha_rows n = m_opened_table->number_of_rows();

  DBUG_PRINT("innmem_api", ("this=%p; return=%llu", this, n));
...
int Handler::disable_indexes(uint mode) {
  DBUG_ENTER("innmem::Handler::disable_indexes");
...
int Handler::enable_indexes(uint mode) {
  DBUG_ENTER("innmem::Handler::enable_indexes");
...
char* Handler::get_foreign_key_create_info() {
  DBUG_ENTER("innmem::Handler::get_foreign_key_create_info");
  DBUG_ABORT();
  DBUG_RETURN(nullptr);
}

void Handler::free_foreign_key_create_info(char*) {
  DBUG_ENTER("innmem::Handler::free_foreign_key_create_info");
  DBUG_ABORT();
  DBUG_VOID_RETURN;
}

int Handler::external_lock(THD*, int) {
  DBUG_ENTER("innmem::Handler::external_lock");
  DBUG_ABORT();
  DBUG_RETURN(0);
}

void Handler::unlock_row() {
  DBUG_ENTER("innmem::Handler::unlock_row");
  DBUG_VOID_RETURN;
}

handler* Handler::clone(const char*, MEM_ROOT*) {
  DBUG_ENTER("innmem::Handler::clone");
  DBUG_ABORT();
  DBUG_RETURN(nullptr);
}

bool Handler::was_semi_consistent_read() {
  DBUG_ENTER("innmem::Handler::was_semi_consistent_read");
  DBUG_ABORT();
  DBUG_RETURN(true);
}

void Handler::try_semi_consistent_read(bool) {
  DBUG_ENTER("innmem::Handler::try_semi_consistent_read");
  DBUG_ABORT();
  DBUG_VOID_RETURN;
}

int Handler::index_first(uchar*) {
  DBUG_ENTER("innmem::Handler::index_first");
  DBUG_ABORT();
  DBUG_RETURN(0);
}

int Handler::index_last(uchar*) {
  DBUG_ENTER("innmem::Handler::index_last");
  DBUG_ABORT();
  DBUG_RETURN(0);
}

int Handler::analyze(THD*, HA_CHECK_OPT*) {
  DBUG_ENTER("innmem::Handler::analyze");
  DBUG_ABORT();
  DBUG_RETURN(0);
}

int Handler::optimize(THD*, HA_CHECK_OPT*) {
  DBUG_ENTER("innmem::Handler::optimize");
  DBUG_ABORT();
  DBUG_RETURN(0);
}

int Handler::check(THD*, HA_CHECK_OPT*) {
  DBUG_ENTER("innmem::Handler::check");
  DBUG_ABORT();
  DBUG_RETURN(0);
}

int Handler::start_stmt(THD*, thr_lock_type) {
  DBUG_ENTER("innmem::Handler::start_stmt");
  DBUG_ABORT();
  DBUG_RETURN(0);
}

int Handler::reset() {
  DBUG_ENTER("innmem::Handler::reset");
  DBUG_ABORT();
  DBUG_RETURN(0);
}

int Handler::records(ha_rows*) {
  DBUG_ENTER("innmem::Handler::records");
  DBUG_ABORT();
  DBUG_RETURN(0);
}

void Handler::update_create_info(HA_CREATE_INFO*) {
  DBUG_ENTER("innmem::Handler::update_create_info");
  DBUG_ABORT();
  DBUG_VOID_RETURN;
}
...
int Handler::rename_table(const char*, const char*, const dd::Table*,
                          dd::Table*) {
  DBUG_ENTER("innmem::Handler::rename_table");
  DBUG_ABORT();
  DBUG_RETURN(0);
}

void Handler::init_table_handle_for_HANDLER() {
  DBUG_ENTER("innmem::Handler::init_table_handle_for_HANDLER");
  DBUG_ABORT();
  DBUG_VOID_RETURN;
}

bool Handler::get_error_message(int, String*) {
  DBUG_ENTER("innmem::Handler::get_error_message");
  DBUG_ABORT();
  DBUG_RETURN(false);
}

uint8 Handler::table_cache_type() {
  DBUG_ENTER("innmem::Handler::table_cache_type");
  DBUG_ABORT();
  DBUG_RETURN(0);
}

bool Handler::primary_key_is_clustered() const {
  DBUG_ENTER("innmem::Handler::primary_key_is_clustered");
  DBUG_ABORT();
  DBUG_RETURN(false);
}

int Handler::cmp_ref(const uchar*, const uchar*) const {
  DBUG_ENTER("innmem::Handler::cmp_ref");
  DBUG_ABORT();
  DBUG_RETURN(0);
}

bool Handler::check_if_incompatible_data(HA_CREATE_INFO*, uint) {
  DBUG_ENTER("innmem::Handler::check_if_incompatible_data");
  DBUG_ABORT();
  DBUG_RETURN(false);
}

ha_rows Handler::records_in_range(uint, key_range*, key_range*) {
  DBUG_ENTER("innmem::Handler::records_in_range");
  DBUG_ABORT();
  DBUG_RETURN(0);
}
