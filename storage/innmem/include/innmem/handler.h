...
/** @file storage/innmem/include/innmem/handler.h
InnMEM public handler API declaration. */

/** @page PAGE_INNMEM InnMEM storage engine
The InnMEM storage engine is designed to be used by the Optimizer for creating
temporary tables internally when dealing with complex queries.

@subpage PAGE_INNMEM_GLOSSARY

@subpage PAGE_INNMEM_ROW_FORMAT

@page PAGE_INNMEM_GLOSSARY Glossary

Following is a list of the terms used in the InnMEM storage engine source code.

This is to avoid confusion because in the surrounding code (outside of
storage/innmem) different terms are used to designate the one thing and in
...
Below are the terms used in InnMEM source code with some explanations.
...
#ifndef INNMEM_HANDLER_H
#define INNMEM_HANDLER_H
...
#include "innmem/storage.h" /* innmem::Storage::Iterator */
#include "innmem/table.h"   /* innmem::Table */
#include "innmem/test.h"    /* INNMEM_CPP_HOOKED_TESTS */
