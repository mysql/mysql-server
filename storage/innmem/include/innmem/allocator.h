...
/** @file storage/innmem/include/innmem/allocator.h
InnMEM custom allocator. */

#ifndef INNMEM_ALLOCATOR_H
#define INNMEM_ALLOCATOR_H
...
#include "sql/mysqld.h"             /* innmem_max_ram */

#ifdef HAVE_LIBNUMA
#define INNMEM_USE_LINUX_NUMA
#endif /* HAVE_LIBNUMA */
...
#define INNMEM_PFS_MEMORY

/* Enabling this causes ~ 4% performance drop in sysbench distinct ranges. */
//#define INNMEM_PFS_MEMORY_COUNT_LOGICAL
#endif /* HAVE_PSI_MEMORY_INTERFACE */

namespace innmem {

#ifdef INNMEM_PFS_MEMORY
#ifdef INNMEM_PFS_MEMORY_COUNT_LOGICAL
...
#endif /* INNMEM_PFS_MEMORY_COUNT_LOGICAL */

/** PFS key to account physical allocations and deallocations from disk. After
 * we have allocated more than `innmem_max_ram` we start taking memory from
 ...
 * we have allocated more than `innmem_max_ram` we take memory from the OS
 ...
 * we have reached the `innmem_max_ram` threshold. */
...
#ifdef INNMEM_PFS_MEMORY
  PSI_MEMORY_CALL(register_memory)("innmem", pfs_info, pfs_info_num_elements);
#endif /* INNMEM_PFS_MEMORY */
...
inline void* Allocator<T>::mem_fetch(size_t bytes) {
  ...
  if (bytes_allocated_in_ram > innmem_max_ram) {
    t = Mem_type::DISK;
  } else {
    ...
    if (new_bytes_allocated_in_ram <= innmem_max_ram) {
      t = Mem_type::RAM;
    } else {
      t = Mem_type::DISK;
...
inline void* Allocator<T>::mem_fetch_from_disk(size_t bytes) {
  ...
  File f =
      create_temp_file(file_path, nullptr, "mysql_innmem.", mode, MYF(MY_WME));
