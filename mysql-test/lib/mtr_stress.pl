...
sub run_stress_test () {
  ...
  if ($::opt_stress_suite ne 'main' && $::opt_stress_suite ne 'default') {
    $stress_suitedir =
      File::Spec->catdir($::glob_mysql_test_dir, "suite", $::opt_stress_suite);
  } else {
  ...
  if (@::opt_cases) {
    $::opt_stress_test_file =
      File::Spec->catfile($stress_basedir, "stress_tests.txt");
    open(STRESS_FILE, ">$::opt_stress_test_file");
    print STRESS_FILE join("\n", @::opt_cases), "\n";
    close(STRESS_FILE);
  } elsif ($::opt_stress_test_file) {
    $::opt_stress_test_file =
      File::Spec->catfile($stress_suitedir, $::opt_stress_test_file);
    if (!-f $::opt_stress_test_file) {
      mtr_error("Specified file $::opt_stress_test_file with list of tests",
                "does not exist.\n",
                "Please ensure that file exists and has proper permissions.");
    }
  } else {
    $::opt_stress_test_file =
      File::Spec->catfile($stress_suitedir, "stress_tests.txt");
    if (!-f $::opt_stress_test_file) {
      mtr_error("Default file $::opt_stress_test_file with list of tests",
                "does not exist.\n",
                "Please use --stress-test-file option to specify custom one",
                "or you can\n",
                "just specify name of test for testing as last argument in",
                "command line");
    }
  }
  ...
  if ($::opt_stress_init_file) {
    $::opt_stress_init_file =
      File::Spec->catfile($stress_suitedir, $::opt_stress_init_file);
    if (!-f $::opt_stress_init_file) {
      mtr_error("Specified file $::opt_stress_init_file with list of tests",
                "does not exist.\n",
                "Please ensure that file exists and has proper permissions");
    }
  } else {
    $::opt_stress_init_file =
      File::Spec->catfile($stress_suitedir, "stress_init.txt");
    if (!-f $::opt_stress_init_file) {
      $::opt_stress_init_file = '';
    }
  }
  ...
  mtr_init_args(\$args);
  mtr_add_arg($args, "$::glob_mysql_test_dir/mysql-stress-test.pl");
  ...
  # Run stress test
  My::SafeProcess->run(name => "stress test",
                       path => $^X,
                       args => \$args,);
