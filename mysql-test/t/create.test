...
--echo # values that are not valid utf8mb3

CREATE TABLE t0(a ENUM('aaa', 'bbb', 'ccc') BYTE);
SHOW CREATE TABLE t0;
SHOW COLUMNS FROM t0;

--echo # Tables t1 and t2 uses a binary value that is legal as utf8mb3, so they
...
--echo # Tables t3 - t6 contains binary values that are not valid utf8mb3
...
--echo #
--echo # Bug #29283055: CREATE TABLE IF NOT EXISTS .. SHOULD NOT ALWAYS
--echo # FAIL ON SQL_REQUIRE_PRIMARY_KEY
--echo #

--echo # Create table without PK
CREATE TABLE t1(i INT);

SET SESSION sql_require_primary_key=1;

--echo # This must not fail when t1 already exists, even if it does not have
--echo # a pk
...
--echo #
--echo # WL#13127: Deprecate integer display width and ZEROFILL option
--echo #
...
--let $count = `SELECT COUNT(*) FROM types`
--let $i = 0
while ($i < $count) {
--inc $i
--let $type=`SELECT name FROM types WHERE id = $i`
--eval CREATE TABLE t1(col $type ZEROFILL)
CREATE TABLE t2 LIKE t1;
CREATE TABLE t3 AS SELECT * FROM t1;
# Should not give warnings.
ALTER TABLE t1 ADD COLUMN col2 VARCHAR(10);
--eval ALTER TABLE t1 MODIFY COLUMN col2 $type ZEROFILL
--eval ALTER TABLE t1 ADD COLUMN col3 $type ZEROFILL
DROP TABLE t1, t2, t3;
--eval CREATE FUNCTION f1(x $type ZEROFILL) RETURNS INT DETERMINISTIC RETURN 1
--eval CREATE FUNCTION f2(x INT) RETURNS $type ZEROFILL DETERMINISTIC RETURN 1
DELIMITER //;
eval CREATE FUNCTION f3() RETURNS INT DETERMINISTIC
BEGIN
  DECLARE x $type ZEROFILL DEFAULT 1;
  RETURN x;
END//
DELIMITER ;//
DROP FUNCTION f1;
...
--echo #
--echo # WL#13528: Remove integer display width from SHOW CREATE output
--echo #
...
--echo #
--echo # Bug#30556657: REVERT I_S/SHOW CREATE CHANGES REGARDING TINYINT(1)
--echo #
...
--echo #
--echo # Bug#32227101: CREATE TABLE PRODUCES CORE
--echo #
--echo # Bug#33114368: 'SHOW TABLE STATUS' WITH HUGE AVG_ROW_LENGTH:
--echo # DD::PROPERTIES::GET: ASSERTION `FALSE' FAILED.
--echo #
--echo # These issues are related and arise from insufficient input checking
--echo # of numeric table options.
--echo # From bug#33114368 we see that even if no overflow occurs on CREATE,
--echo # there can be a risk of overflow when the option value is used
--echo # elsewhere.
--echo # The safest thing to do is to constrain values to the largest
--echo # value permitted by the frm-file format (unless the range of the option
--echo # value has been explicitly changed in 8.0).
--echo #
--echo # key_block_size, parser type: signed_num (int)
--echo # std::uint32_t HA_CREATE_INFO::key_block_size
--echo # 2 bytes in frm
--echo # Only settable for myisam, but also used in Innodb

--echo # key_block_size = 2^31-1 - rejected by explicit check 
--error ER_PARSE_ERROR
CREATE TABLE t1(i INT PRIMARY KEY) KEY_BLOCK_SIZE = 2147483647;

--echo # key_block_size = -(2^31)+1 - rejected by parser (parser switched
--echo # back to using ulong_num now that there is an explicit check).
--error ER_PARSE_ERROR
CREATE TABLE t1(i INT PRIMARY KEY) KEY_BLOCK_SIZE = -2147483647;

--echo # key_block_size = 2^31 - rejected by explicit check
--error ER_PARSE_ERROR
CREATE TABLE t1(i INT PRIMARY KEY) KEY_BLOCK_SIZE = 2147483648;

--echo # key_block_size = -2^31 - rejected by parser (parser switched
--echo # back to using ulong_num now that there is an explicit check).
--error ER_PARSE_ERROR
CREATE TABLE t1(i INT PRIMARY KEY) KEY_BLOCK_SIZE = -2147483648;

--echo # key_block_size = 2^32-1 - rejected by explicit check
--error ER_PARSE_ERROR
CREATE TABLE t2(i INT PRIMARY KEY) KEY_BLOCK_SIZE = 4294967295;

--echo # key_block_size = 2^32 - rejected by explicit check
--error ER_PARSE_ERROR
CREATE TABLE t3(i INT PRIMARY KEY) KEY_BLOCK_SIZE = 4294967296;

--echo # key_block_size = 3927749503218772669 - now rejected by parser
--error ER_PARSE_ERROR
CREATE TABLE t4(
...
--echo # Only values which will fit in the two bytes allocated in the frm file
--echo # format are accepted.
CREATE TABLE t1(a INT) ENGINE=myisam key_block_size=65535;
--replace_column 12 @ts@ 13 @ts@
...
--echo # avg_row_length, parser type: ulong_num (ulong) 
--echo # ulong HA_CREATE_INFO::avg_row_length
--echo # 4 bytes in frm
--error ER_PARSE_ERROR
...
--echo # Only values which will fit in the four bytes allocated in the frm file
--echo # format are accepted.
...
--echo # max_rows, parser type: ulonglong_num (ulonglong)
--echo # ulonglong HA_CREATE_INFO::max_rows
--echo # 4 bytes in frm
--echo # max_rows is capped and a warning produced
CREATE TABLE t1(a INT) max_rows=18446744073709551615;
--replace_column 12 @ts@ 13 @ts@
SHOW TABLE STATUS LIKE 't1';
DROP TABLE t1;

--echo # min_rows, parser type: ulonglong_num (ulonglong)
--echo # ulonglong HA_CREATE_INFO::min_rows
--echo # 4 bytes in frm
--echo # min_rows is capped and a warning produced
CREATE TABLE t1(a INT) min_rows=18446744073709551615;
--replace_column 12 @ts@ 13 @ts@
SHOW TABLE STATUS LIKE 't1';
DROP TABLE t1;

--echo # Bug#34523475: Inconsequent type derivation when using @variable
...
--echo #
--echo # Bug#34861841: Assertion: `false' in CostingReceiver::ProposeAccessPath
--echo #
...
--echo #
--echo # Bug#31402520: SQL_REQUIRE_PRIMARY_KEY should also accept PKEs
--echo #

SET sql_require_primary_key=ON;

--echo # Table with a PK is allowed, of course.
CREATE TABLE pk   (a INT PRIMARY KEY);

--echo # We can't DROP INDEX if that would leave the table without a PK.
--error ER_TABLE_WITHOUT_PK
DROP INDEX `PRIMARY` ON pk;

--echo # Table with a PKE is (newly) also allowed.
CREATE TABLE uknn (a INT UNIQUE KEY NOT NULL);

--echo # We can't DROP INDEX if that would leave the table without a PKE.
--error ER_TABLE_WITHOUT_PK
DROP INDEX `a` ON uknn;

--echo # Table without a PKE (here: UNIQUE, but NULLable) is (still) not allowed.
--error ER_TABLE_WITHOUT_PK
CREATE TABLE ukn  (a INT UNIQUE KEY);

--echo # Table with no keys is not allowed.
--error ER_TABLE_WITHOUT_PK
CREATE TABLE nk   (a INT NOT NULL);

# Clean up.
DROP TABLE IF EXISTS pk;
DROP TABLE IF EXISTS uknn;
