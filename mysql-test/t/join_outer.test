...
# Tests printing of EXPLAIN FORMAT=tree for outer join BKA (in join_outer_bka.test).
...
# In join_outer_bka.test, demonstrates that when combining outer BKA and BNL,
# we revert to simple nested loops.
...
--echo #
--echo # Bug #30659532: WL#13476: DIFFERENT NUMBER OF ROWS WITH NESTED LOOP JOINS
--echo #
...
--echo #
--echo # Bug #30654713: WL#13476: DIFFERENT ROWS WITH BATCHED KEY ACCESS LEFT JOIN
--echo #
...
--echo #
--echo # Bug #31252625: DATA IN WRONG ROW ON LEFT JOIN
--echo #
...
--echo #
--echo # Bug#33639644: sig11 in CreateIteratorFromAccessPath
--echo #
...
--echo #
--echo # Bug#34668756: impossible on not optimized in 8.x like 5.6
--echo #
...
#Inner-joined tables with always false join condition and false WHERE clause
EXPLAIN
SELECT t1.f1
...
#Outer-joined tables with always false join condition and false WHERE clause
EXPLAIN
SELECT t1.f1,t2.f1
...
#Semi join with false WHERE clause
EXPLAIN
SELECT f1,f2
...
#Anti-join tables with always false join condition and false WHERE clause
EXPLAIN
SELECT t2.f1,t2.f2
...
--echo #
--echo # Bug#35788971: Unexpected Result by NO_BNL
--echo #
...
--echo #
--echo # Bug#36418426: Even though there is an index, the index is not being used enough when LEFT JOIN.
--echo #
