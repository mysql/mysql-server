...
--echo #
--echo # Bug#32239578: WRONG METADATA FOR THE TO_DAYS FUNCTION
--echo #
...
--echo #
--echo # Bug #32384355 INSERT,SELECT FROM SAME TABLE WITH AGGREGATE
--echo # AND NOW() COLUMNS PRODUCES ERROR
--echo #
...
--echo #
--echo # BUG#33438883: Assertion `!thd->lex->is_view_context_analysis()
--echo #
...
--echo
--echo Bug#35884337 Mysqld crash from TIME_to_longlong_datetime_packed
--echo              (my_time=...) my_time.cc:1868
--echo
...
--echo #
--echo # Bug#37807230: assertion null_value || current_thd->is_error failed
--echo #

CREATE TABLE t1(ic INTEGER, d DATE);
INSERT INTO t1 VALUES(NULL, DATE'2025-04-11'),
                     (20250412, DATE'2025-04-11'),
                     (41331246, DATE'1998-11-30'),
                     (-711023684, DATE'2028-09-27');
SELECT * FROM t1 WHERE d > TIMEDIFF(ic, NULL);
DROP TABLE t1;

--echo #
--echo # Bug#37818753: Assertion `null_value == (res == nullptr)' failed
--echo #

CREATE TABLE t1(vc varchar(1));
INSERT INTO t1 VALUES('a');
SELECT SUBTIME('0000-00-00 00:00:00', vc) AS c FROM t1;
DROP TABLE t1;

--echo #
--echo # Bug#37864777: Assertion `null_value || current_thd->is_error()
--echo #

CREATE TABLE t1(t time(3));
INSERT INTO t1 VALUES(TIME'05:27:52.019');

SELECT DATEDIFF('2031-08-06',
                TIMEDIFF(SUBTIME(t, '12:59:38'), '1994-01-14')) AS c
FROM t1;

DROP TABLE t1;

--echo #
--echo # Bug#37881114: Assertion `mt.time_type == MYSQL_TIMESTAMP_DATETIME'
--echo #

SELECT SUBTIME(DATE'2005-04-14', TIMESTAMPADD(YEAR, 165, DATE'2025-01-01')) c;

--echo #
--echo # Bug#37900924: Assertion `mt.time_type == MYSQL_TIMESTAMP_DATETIME'
--echo #

CREATE TABLE t1 (t time(3) NOT NULL);

INSERT INTO t1 VALUES (TIME'00:20:01.000'), (TIME'14:46:31.033');

SELECT *
FROM t1
WHERE t IN (FROM_UNIXTIME(2120220672, '%Y-%m-%d'),
            TIMESTAMP('2003-10-18', TIMESTAMPADD(QUARTER, 1, '2003-03-07'))
           );

DROP TABLE t1;

--echo #
--echo # Bug#37923312: Assertion `l_time1.time_type == MYSQL_TIMESTAMP_DATETIME'
--echo #

SELECT SUBTIME(FROM_UNIXTIME(3569025024, CONCAT_WS(':', '%V', '%V', '%m', '%T')
                            ), '00:00:00') AS c;

--echo #
--echo # Bug#37976294: Assertion `hour <= TIME_MAX_HOUR &&
--echo #                          minute <= TIME_MAX_MINUTE'
--echo #

SELECT TIMEDIFF('4617000000', DATE(NULL)) AS c;

--echo #
--echo # Bug#38179658: TIMEDIFF is not consistent with documentation
--echo #

CREATE TABLE t(t2 TIME, t4 DATETIME);
INSERT INTO t VALUES(TIME'01:01:01', TIMESTAMP'2001-02-01 01:01:01');
...
--echo #
--echo # Bug#38177844: TO_DAYS() result is inconsistent
--echo #

SELECT FROM_DAYS(3652499), FROM_DAYS(3652500), FROM_DAYS(3652501);

--echo #
--echo # Bug#38177993: TO_DAYS() result with argument < 366 is inconsistent
--echo #

SELECT FROM_DAYS(1), FROM_DAYS(365);

--echo #
--echo # Bug#38181443: TIMEDIFF returns wrong result when first argument
--echo #               is DATETIME and second argument is DATE
--echo #

SELECT TIMEDIFF(TIMESTAMP'0000-01-01 00:00:00.00000', DATE'2025-08-01');
SELECT TIMEDIFF(TIMESTAMP'0000-01-01 00:00:00.00000', '2025-08-01');
SELECT TIMEDIFF('0000-01-01 00:00:00.00000', DATE'2025-08-01');
SELECT TIMEDIFF('0000-01-01 00:00:00.00000', '2025-08-01');

--echo #
--echo # Bug#38980002: Assertion 'dt.time_type == MYSQL_TIMESTAMP_DATETIME ||
--echo #               dt.time_type == MYSQL_TIMESTAMP_DATETIME_TZ' failed
--echo #

SELECT CEILING(CONVERT_TZ('2025-01-01' ,'-01:00','-08:00')) AS ceil;

--echo #
--echo # Bug#38979962: Assertion 'hour != 0 && minute != 0 && second != 0 &&
--echo #               microsecond != 0' failed'
--echo #

SELECT SEC_TO_TIME(-00.0000004584) AS tv;
