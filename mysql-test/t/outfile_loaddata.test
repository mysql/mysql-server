...
--replace_result $MYSQLTEST_VARDIR MYSQLTEST_VARDIR
--eval LOAD DATA INFILE '$MYSQLTEST_VARDIR/tmp/bug31663.txt' IGNORE INTO TABLE t2 $clauses
...
--replace_result $MYSQLTEST_VARDIR MYSQLTEST_VARDIR
--eval SELECT * INTO OUTFILE '$file' CHARACTER SET utf8mb3 FROM t1
--echo ##################################################
--cat_file $file
--echo ##################################################
TRUNCATE t2;
--replace_result $MYSQLTEST_VARDIR MYSQLTEST_VARDIR
--eval LOAD DATA INFILE '$file' INTO TABLE t2 CHARACTER SET utf8mb3
--remove_file $file
SELECT * FROM t1 UNION SELECT * FROM t2 ORDER BY a, b, c;
...
--echo #
--echo # Bug#24577194 - LOAD DATA ERROR 1262 BEHAVIOUR DIFFERENT
--echo #                 BETWEEN 5.6 AND 5.7 UNDER STRICT SQLMOD
--echo #
...
DROP TABLE t1,t2;
--remove_file $MYSQLTEST_VARDIR/tmp/bug24577194.txt
SET sql_mode=default;

--echo #
--echo # Bug #32888982 EXPORT DATA OF DOUBLE FIELD FAILED TO IMPORT
--echo #               WHEN IT IS DOUBLE'S LOWER BOUNDARY
--echo #
...
--replace_result $MYSQLTEST_VARDIR MYSQLTEST_VARDIR
--eval SELECT * FROM t1 INTO OUTFILE $file;
...
SELECT * from t1;
DROP TABLE t1;
--remove_file $MYSQLTEST_VARDIR/tmp/bug32888982.txt
