...
while($n) {
  ...
  --disable_result_log
  eval EXPLAIN SELECT 1 FROM t1 WHERE a = $x AND TRIM(a) = b;
  --enable_result_log
  --skip_if_hypergraph  # The non-tree EXPLAIN statement is skipped.
  SHOW WARNINGS;
}
...
--echo #
--echo # Bug #27008075: UBSAN: MY_STRNTOLL_8BIT - NEGATION OF XYZ CANNOT BE REPRESENTED IN TYPE
--echo #
DO TRIM(-9223372036854775808) XOR '.D-';
DO OCT(-9223372036854775808);

--echo #
--echo # Bug#27451113 UBSAN: ITEM_FUNC_MOD::INT_OP
--echo #              NEGATION OF -9223372036854775808 !
--echo #

select -9223372036854775808 mod 9223372036854775810 as result;

--echo #
--echo # Bug #31325602 UBSAN: NEGATION OF -9223372036854775808
--echo #                      CANNOT BE REPRESENTED IN TYPE 'LONG LONG INT'
--echo #
select bin(convert(-9223372036854775808 using ucs2));

--echo #
--echo # Bug #22515857	"-1 BETWEEN 9223372036854775808 AND 1" GIVES TRUE
--echo #
