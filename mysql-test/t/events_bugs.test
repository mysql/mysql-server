...
set @@sql_mode= cast(pow(2,34)-1 as unsigned integer) & ~0x1003ff00;
create event e1 on schedule every 1 day do select 1;
...
--echo #
--echo # Bug#27041552 - M_THD->GET_TRANSACTION()->IS_EMPTY(TRANSACTION_CTX::STMT) ETC. ASSERTION
--echo #
...
--echo #
--echo # Test case to verify events with case and accent insensitive names.
--echo #
CREATE EVENT cafe ON SCHEDULE EVERY 2 YEAR DO SELECT 1;
--echo # Event names are case and accent insensitive. So from the data-dictionary
--echo # "cafe" is obtained for the following statement. Since MDL key comparison
--echo # is case and accent sensitive, assert condition to verify expected lock
--echo # with name "test.cafe" fails (lock is obtained on the test.café)
--error ER_EVENT_ALREADY_EXISTS
...
--echo # Event with NAME_LEN size name.
...
--echo #
--echo # Bug#29871530 - MYSQL 8.0 INFORMATION_SCHEMA.EVENTS NOT
--echo # OBSERVING CUSTOM TIMEZONE FOR LAST_EXECU
--echo #
...
--echo # Verify value of 'LAST EXECUTED'.
--echo # We do not check last executed against timezone change, as it
--echo # will always be in the timezone at create event, as per:
--echo # WL#3698: Events: execution in local time zone.
echo $created;
eval SELECT TIMESTAMP('$created') < LAST_EXECUTED
     FROM INFORMATION_SCHEMA.EVENTS WHERE EVENT_NAME='my_event';

--echo # Verify value of 'LAST ALTERED'.
...
let $last_altered=`SELECT LAST_ALTERED FROM INFORMATION_SCHEMA.EVENTS
                   WHERE EVENT_NAME='my_event'`;
eval SELECT TIMESTAMP('$last_altered') = TIMESTAMP('2019-08-01 01:20:50');

--echo # Check that both last altered and created is in the same time zone.
eval SELECT (CREATED - TIMESTAMP('$created')),
            (LAST_ALTERED - TIMESTAMP('$last_altered'))
     FROM INFORMATION_SCHEMA.EVENTS WHERE EVENT_NAME='my_event';
SET TIME_ZONE= '-02:00';
eval SELECT (CREATED - CONVERT_TZ('$created', '+03:00', '-02:00')),
...
--echo #
--echo # Bug#11764690: SOME FUNCTIONS CAUSE ASSERTION WHEN GIVEN A ROW
--echo #               CONSTRUCTOR ARGUMENT
--echo #
