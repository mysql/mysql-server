...
--echo # Should use index scan on PRIMARY.
...
--echo # Should use a index scan on primary with a filter rather than a non-covering index range scan
...
--echo # Should choose a hash join with index scans on PRIMARY.
...
--echo # Narrow table: Filtered index scan should be chosen.
--replace_regex $elide_metrics
