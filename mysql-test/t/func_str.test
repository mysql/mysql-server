...
#
# EXPLICIT coercibility:

# Two equal COLLATE clauses are OK:
...
# Two conflicting COLLATE clauses:
...
# One COLLATE clause takes precedence
...
# Binary string takes precedence:

SELECT c1 = b0 FROM t1;
SELECT CONCAT(c1, b0) FROM t1;

# Binary collation takes precedence:

SELECT c1 = c0 FROM t1;
SELECT CONCAT(c1, c0) FROM t1;

# Same character set, different collations: conflict for compare, OK for result:

--error ER_CANT_AGGREGATE_2COLLATIONS
SELECT c1 = c2 FROM t1;
SELECT CONCAT(c1, c2) FROM t1;

# Superset character set: UNICODE:
...
# Superset character set: UNICODE vs ASCII/LATIN1/2:
...
# Non superset character sets: LATIN1 vs. LATIN2:
...
# SYSCONST coercibility

# Testing is irrelevant since most system constant strings get the same
# character set and collation.

# Testing against COERCIBLE constants:
...
# NUMERIC coercibility, again irrelevant since all numeric constant values
# get the same character set and collation.

# Testing against COERCIBLE constants (only CONCAT() since comparisons would
# be numeric):

SELECT CONCAT(_utf8mb4'æøå', 1);
SELECT CONCAT(_utf8mb4'āđē', 1);

# NULL coercibility, also irrelevant since all NULL values get the same
# character set and collation.
...
# c1 and c2 are different collations on the same character set.
# For CONCAT(), this is assigned NONE coercibility and binary collation is set.
# Equality sees the NONE coercibility and uses collation of c3 as collation
# for the comparison. There is similar logic for the GREATEST() function.
...
# Similar logic as above for CONCAT(), but the comparison operator does not
# allow NONE coercibility, hence the error. But CONCAT allows NONE.
...
--echo #
--echo # Bug#21317406 TRUNCATED DATA WITH SUBQUERY & utf8mb3
--echo #
...
--echo #              SRC, FILE .\STRINGS\CTYPE-utf8mb3.CC, LINE 8686
--echo #
...
--echo Bug#32501472: Concatenation may create invalid utf8mb3 strings
...
--echo # Bug#34962821 space():
--echo #              Item_str_func::push_packet_overflow_warning:
--echo #              Assertion `is_nullable()' failed.
SET @@sql_mode='';
--disable_warnings
DO SPACE(SHA2(UUID_SHORT(),256));
--enable_warnings
SET @@sql_mode = default;

--echo #
--echo # Bug#34961236 [rl]pad() + row_count():
--echo # Item::null_return_str(): Assertion `m_nullable' failed.
--echo #
...
--echo #
--echo # Bug#34781507 random_bytes(): Assertion `is_nullable()' failed.
--echo # and Assertion `!null_value || is_nullable()' failed
--echo #
...
--echo #
--echo # Bug#35129361 my_strnncollsp_utf16 Assertion `(slen % 2) == 0' failed.
--echo #
...
--echo #
--echo # Bug#35148945: String* Item::null_return_str():
--echo #               Assertion `m_nullable' failed
--echo #
...
SET sql_mode = '';
# We don't care about the result, only that it doesn't hit an assertion.
--disable_result_log
...
--echo #
--echo # Bug#35286970 QUOTE function doesn't work as expected
--echo #              on tables that are utf16
...
--echo #
--echo # Bug#35174730 push_packet_overflow_warning(THD*, const char*):
--echo #              Assertion `is_nullable()' failed
...
--echo #
--echo # Bug#35489153 String* Item_str_func::push_packet_overflow_warning
--echo #              (THD*, const char*): Assertion `is_nullable()' failed
--echo #
...
--echo #
--echo # Bug#35499232 The quote function may return binary data
--echo #
...
--echo #
--echo # Bug#36137690 Item_func_conv_charset::eq not correct
--echo #

CREATE TABLE t1 (a CHAR(10));
INSERT INTO t1 VALUES ('你好'), ('中午好');

SELECT a, CONVERT(a USING latin1) FROM t1 ORDER BY CONVERT(a USING utf8mb4);
SELECT a FROM t1 ORDER BY CONVERT(a USING utf8mb4);

DROP TABLE t1;

--echo # Bug#36118590: Inconsistent results caused by JOIN tables and CHAR func

CREATE TABLE t0 (vkey INTEGER);
CREATE TABLE t1 (vkey INTEGER);
INSERT INTO t0 (vkey) VALUES (6);

SELECT 1 AS c_0
FROM t0 LEFT JOIN t1 ON t0.vkey = t1.vkey
WHERE CHAR(CASE WHEN FALSE THEN t1.vkey ELSE t1.vkey END) NOT LIKE 'YVR';

SELECT 1 AS c_0
FROM t0 LEFT JOIN t1 ON t0.vkey = t1.vkey
WHERE CHAR(t1.vkey) NOT LIKE 'YVR';

DROP TABLE t0, t1;

--echo #
--echo # Bug#36277823 Contribution: Fix multibyte bugs in my_instr_mb
--echo #
...
SELECT LOCATE('ß', 'ss');
SELECT LOCATE("", "ÅÅ", 2);
SELECT LOCATE('àà', 'aaa', 2);
SELECT LOCATE('viii', 'Ⅷ', 1);
SELECT LOCATE('', 'å', 3);

SET NAMES DEFAULT;

--echo # Bug#36367313: String::length at include/sql_string.h
...
SELECT REVERSE(TIME_FORMAT('', NULL)) AS field1
FROM t1
GROUP BY ROLLUP(REVERSE(TIME_FORMAT('', NULL)));

DROP TABLE t1;

--echo # Bug#36367776: Assertion failure in Item_func_numhybrid::val_str

CREATE TABLE t(col INTEGER);
  
SELECT DATE_FORMAT(NULL, -112982309.91)
FROM t
GROUP BY DATE_FORMAT(NULL ,-112982309.91);

DROP TABLE t;

--echo # Bug#36364619: Assertion 'src != dst || cs->casedn_multiply == 1'
...
INSERT INTO t1 VALUES (1, '12:16:22', '2018-06-07');

SELECT pk
FROM t1
WHERE CONCAT_WS('string',
                CONCAT(CONCAT('08:50:27', cd COLLATE utf8mb4_turkish_ci),
                       LOWER(ct))) <> 'xyz';

DROP TABLE t1;

--echo # Bug#36457756: Invalid result converting string to numeric
...
SELECT IF(a <> 0, 1, 0) FROM t1;
SELECT a | 2147483647 FROM t1;
SELECT CAST(a AS DECIMAL) FROM t1;

--error ER_TRUNCATED_WRONG_VALUE
INSERT INTO t0 SELECT IF(a, 1, 0) FROM t1;
--error ER_TRUNCATED_WRONG_VALUE
INSERT INTO t0 SELECT IF(a <> 0, 1, 0) FROM t1;
--error ER_TRUNCATED_WRONG_VALUE
INSERT INTO t0 SELECT a | 2147483647 FROM t1;
--error ER_TRUNCATED_WRONG_VALUE
INSERT INTO t0 SELECT CAST(a AS DECIMAL) FROM t1;
...
--echo # Bug#36562972: Character set changing unexpectedly

CREATE TABLE t1 (
  col varchar(36) DEFAULT (REPLACE(UUID(), _utf8mb4'-', _utf8mb4'_'))
);
...
--echo #
--echo # Bug#36812010 crash in Item_func_soundex::val_str
--echo #

--error ER_CANNOT_CONVERT_STRING
DO '' SOUNDS LIKE LEAD(DATABASE(), 1, x'cafe') OVER();

--echo #
--echo # Bug#37086818 mysqld assertion failure `(slen % 4) == 0'
--echo #              in my_strnncollsp_utf32 at ../strings/ctype-ucs2.cc
--echo #
CREATE TABLE t1 (f1 VARCHAR(10) COLLATE UTF32_GENERAL_CI);
SELECT * FROM t1 WHERE LOCATE('Q',f1,1) BETWEEN 'a' AND 'b';
DROP TABLE t1;

--echo #
--echo # Bug#37554688 Collation name aliases should not be case significant
--echo #
...
--echo #
--echo # Bug#37601389 Assertion `!thd->is_error()' failed|sql_resolver.cc
--echo #
...
INSERT INTO customer_demographics VALUES (1,500) ;

--error ER_DATA_OUT_OF_RANGE
SELECT * FROM  customer_demographics
WHERE CEILING( customer_demographics.cd_purchase_estimate )
BETWEEN '2037' AND ( -71 * ( -1552207807 * 440370369 ) )
COLLATE utf8mb4_et_0900_as_cs;

DROP TABLE customer_demographics;

--echo #
--echo # Bug#37285902: Illegal mix of collations NONE vs. COERCIBLE
--echo #
...
--echo #
--echo # Bug37815490 - vector_to_string returns data in non-standard collation
--echo #

select CHARSET(vector_to_string(string_to_vector('[1,2]')));
select COLLATION(vector_to_string(string_to_vector('[1,2]')));
select JSON_OBJECT('a', vector_to_string(string_to_vector('[1,2]')));
...
--error ER_DATA_OUT_OF_RANGE
SELECT UNCOMPRESS(BIN(POW(-9223372036854775807, 0xa5)));
--error ER_DATA_OUT_OF_RANGE
SELECT JSON_TYPE(BIN(EXP(0x5f022b6c81)));
--error ER_DATA_OUT_OF_RANGE
