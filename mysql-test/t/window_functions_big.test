...
--echo # ----------------------------------------------------------------------
--echo # Time consuming tests of window functions.
--echo # ----------------------------------------------------------------------

--echo # ----------------------------------------------------------------------
--echo # Test of SQL window functions with frame buffer in MEM only and with
--echo # spill over to INNODB as tmp file for the frame buffer (slower)
--echo # ----------------------------------------------------------------------

--echo default size is 16777216. Use 10 * 16777216 to fit frame buffer in memory
...
--echo Frame buffer in MEM engine
...
--echo Spill frame buffer to INNODB
INSERT INTO cpy SELECT d, SUM(d) OVER w summ, NTH_VALUE(d, 3) OVER w nth, LAG(d, 20) OVER w lagg FROM td
                WINDOW w AS (ORDER BY d ROWS BETWEEN 10 PRECEDING AND 10 FOLLOWING);
...
--echo Set tmp_table_size and max_heap_table_size back to default
SET tmp_table_size=DEFAULT;
SET max_heap_table_size=DEFAULT;
SET internal_tmp_mem_storage_engine=DEFAULT;

--echo #
--echo # Bug#25894783 WL9603: CAN'T FIND RECORD IN '/TMP/#SQL6E16_3_4'
--echo #
SET SQL_MODE='';
