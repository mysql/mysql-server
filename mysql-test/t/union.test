...
explain format=tree (select * from t1 where a=5) union (select * from t2 where a=1);  # Tests printing of ZeroRowsIterator.
...
--echo #
--echo # Bug#11886060: SELECT .. UNION SELECT .. ORDER BY IS WRONGLY PARSED
--echo #
...
--replace_regex $elide_costs
EXPLAIN FORMAT=tree SELECT a, SUM(a) FROM t2 UNION ALL SELECT a, MIN(a) FROM t1 ;  # Tests printing of ZeroRowsAggregatedIterator.
...
--echo # Bug#18503515 UNION with set function in ORDER BY should be rejected

--echo # UNIONed queries are non-aggregated:
let $query=
SELECT 1 AS foo
UNION
SELECT 2
...
--echo # UNIONed queries are aggregated:
let $query=
SELECT MAX(1) AS foo
UNION
SELECT MAX(2)
...
--echo # Subquery, ORDER BY contains set function
let $query=
SELECT (SELECT 1 AS foo
        UNION
        SELECT 2
...
--echo #
--echo # Bug #19471564 WL#6737:IN-CONSISTENT DATA-TYPE FOR GEOMETRY TYPES WHILE
--echo # PROJECTING DERIVED TBL.
--echo #
...
--echo #
--echo # Bug #21156155 UNION GEOMETRY TYPE: UNITIALIZED VALUE IN
...
(SELECT SQL_CALC_FOUND_ROWS c1 FROM t1) UNION ALL
(SELECT c1 FROM t1) LIMIT 2;
SELECT FOUND_ROWS();
...
(SELECT c1 FROM t1 LIMIT 3) UNION ALL
...
--echo # INSERT SELECT using UNION ALL:
--echo # UNION should create no tmp table if not at the top level.
...
# STATUS reports 1;
# in PS mode, the preparation phase creates one, and the execution
# phase does, too. So SHOW reports 2. To have a single result in both
# modes, we use this complex query instead of SHOW.
...
--echo #
--echo # Bug#11747567: INTEGER CONSTANTS CASTED TO BIGINTS BY UNIONS
--echo #
...
--echo #
--echo # Bug #27068222 CREATE TABLE USING SELECT WITH UNION
...
--echo # we see that with CREATE SELECT (no UNION), defaults are inherited.
...
--echo # With UNION and UNION ALL, defaults are not inherited; every
--echo # column gets, as default, NULL (if nullable) or 0 (if not
--echo # nullable). Exception: if strict mode and a date type,
--echo # 0000-00-00 isn't given as default

# In strict mode, t2.d1 has no DEFAULT, so cannot insert;
# in non-strict mode, no problem.
  if ($c)
  {
...
--echo #
--echo # Bug#28237675: ASSERTION IN UNION QUERY WHEN MIXING CHARACTER SETS
--echo #
...
--echo #
--echo # Bug#27849293 INVALID WRITE IN SYSSCHEMA.PR_DIAGNOSTICS
--echo #
--echo # Note that this bug can only be seen when running with valgrind.
...
--echo # during implicit aggregation, even in UNIONs, even when involving
...
--echo #
--echo # Tests that when executing a UNION query during optimization, lex->current_select
...
--echo #
--echo # Bug#22292995 UNION ALL ... LIMIT 1 EVALUATES TOO MANY ROWS IN 5.5/6 AND TWICE MORE IN 5.7
--echo #
...
--echo #
--echo # Bug #30655712: EMPTY RESULTSET FOR SUBQUERY WITH UNION AND TEXT COLUMN WITH CONSTANT EXPRESSION
--echo #
...
--echo # Bug#29698617: UNION ALL signed and unsigned int display wrong result
...
--echo # Bug#31348202: Wrong result is returned with UNION syntax
...
--echo #
--echo # Bug#33066455: REGRESSION: ACCESSPATH*
--echo #               MOVECOMPOSITEITERATORSFROMTABLEPATH(ACCESSPATH*):
...
--echo #    a) multi-level set operations
--echo #    b) right side DISTINCT restriction lifted
--echo #    c) arbitrary combination with unary, cf. also
--echo #       unary.test
--echo # This is a mix of coverage for refactoring parts of the WL and
--echo # for new capabilities, marked [NEW].
...
--echo # UNION mixed with ORDER BY
...
--echo # UNION mixed with ORDER BY and LIMIT
let $query = SELECT * FROM r UNION DISTINCT
             SELECT * FROM s UNION ALL
...
--echo # UNION ALL with ORDER BY and LIMIT
let $query = SELECT * FROM r UNION ALL
             SELECT * FROM s UNION ALL
             SELECT * FROM t ORDER BY a LIMIT 11;
eval $query;
...
--echo # [NEW] UNION ALL with wrapped in unary query: ORDER BY
...
--echo # [NEW] UNION ALL with wrapped in unary query: ORDER BY + offset
let $query = ( SELECT * FROM r UNION
               SELECT * FROM s ORDER BY a LIMIT 11)
...
--echo # from derived table which is a union
...
--echo # [NEW]
--echo # SQL_CALC_FOUND_ROWS with unary terms. Semantics gets
--echo # a little weird here: it collects number of rows if we strip
--echo # off the outermost LIMIT... but SQL_CALC_FOUND_ROWS is
--echo # deprecated anyways.
--echo #
((SELECT SQL_CALC_FOUND_ROWS * FROM r ORDER BY a LIMIT 5) ORDER BY -a LIMIT 4) ORDER BY a LIMIT 3;
SELECT FOUND_ROWS();

--echo # [NEW] Multi-level set ops
let $query =
...
--echo # [NEW] One union materialized tmp table with dedup over unary sorted tmp tables
let $query =
...
--echo # [NEW] Streams a limited and sorted union materialized tmp
--echo # table *without* dedup over unary sorted tmp tables
let $query =
((((SELECT * FROM t ORDER BY a LIMIT 5) ORDER BY -a LIMIT 3) ORDER BY a LIMIT 2) UNION ALL
...
--echo # [NEW]
--echo # Don't collapse levels if LIMIT is present: streams two limited and sorted
--echo # materialized tmp tables for dedup
...
--echo # [NEW]
--echo # Ok to collapse here: one materialized tmp table with last two tables
--echo # added with dedup disabled
let $query =
...
--echo # [NEW]
--echo # but *not* ok to collapse here
--echo #
let $query =
    (SELECT * FROM r UNION DISTINCT SELECT * FROM t) UNION ALL
...
--echo # [NEW]
--echo # nor here: two materialized tmp tables for dedup (of 2 and 3
--echo # tables respectively), streamed
--echo #
let $query =
    (SELECT * FROM r UNION DISTINCT SELECT * FROM t) UNION ALL
...
--echo # [NEW]
--echo # mix: two materialized tmp tables for dedup, rest is streaming
--echo #
let $query =
...
--echo # [NEW]
--echo # mix: One materialized tmp table w/disable de-dup for final ALL
--echo #
let $query =
...
--echo # [NEW}
--echo # mix: One materialized tmp table for dedup
--echo #
let $query =
    (SELECT * FROM r UNION DISTINCT SELECT * FROM s) UNION ALL
...
--echo # [NEW]
--echo # mix, same result as above, since the last inner ALL
--echo # can be elided
--echo #
let $query =
    (SELECT * FROM r UNION DISTINCT SELECT * FROM s) UNION ALL
...
--echo # [NEW]
--echo # mix, now final DISTINCT can not be elided. Top level streaming of
--echo # three lower materialized tmp tables for dedup.
let $query =
    (SELECT * FROM r UNION DISTINCT SELECT * FROM s) UNION ALL
    (SELECT * FROM r UNION DISTINCT SELECT * FROM t) UNION ALL
    (SELECT * FROM r UNION DISTINCT SELECT * FROM s);
...
--echo # [NEW]
--echo # mix: one materialized tmp table for de-dup, four last tables
--echo # added with dedup disabled
let $query =
    (SELECT * FROM r UNION DISTINCT SELECT * FROM s) UNION ALL
...
--echo # [NEW]
--echo # ALL ALL: every table is streamed
let $query =
...
let $query =
   ((((SELECT a FROM r UNION DISTINCT
     ...
     (VALUES ROW(1), ROW(2) UNION DISTINCT
      SELECT * FROM s ORDER BY column_0 LIMIT 3)) ORDER BY a LIMIT 3) UNION DISTINCT
   (SELECT * FROM r UNION DISTINCT SELECT * FROM t) ORDER BY a LIMIT 5;
eval $query;
--replace_regex $elide_costs
--skip_if_hypergraph
...
--echo #
--echo # Test that LIMIT in nests prohibit merging
--echo #
let $query =
...
--echo # No limit, but ordering does not prohibit merging:
let $query =
...
--echo #
--echo # [NEW] Check maximum depth 63
--echo #
let $query =
     (SELECT * FROM r
...
--echo # This query one exceeds max and should fail
let $query = ($query ORDER BY a LIMIT 1);
--error ER_TOO_HIGH_LEVEL_OF_NESTING_FOR_SELECT
eval $query;

--echo #
--echo # WL#11350 bug: lower level ALL not quenched when merging, giving
--echo # rise to duplicates: upper DISTINCT must be heeded
--echo #
let $query =
...
--echo #
--echo # WL#11350 bug: upper level ALL in nesting context, e.g. ORDER
--echo # BY: non-first lower level operand with DISTINCT incorrectly
--echo # merged.
--echo #
let $query =
...
--echo #
--echo # WL#11350 bug: upper level ALL in nesting context, e.g ORDER
--echo # BY: first operand lower level with DISTINCT can be merged,
--echo # but last_distinct was clobbered if right hand side contains a
--echo # nest that could not be merged.
--echo #
let $query =
...
--echo #
--echo # Bug#35590128 Wrong first-row cost for 'Append'.
--echo #
...
--echo # Bug#35736158: Insert on duplicate assertion error
...
--echo # Bug#36112585: Data type is changed from VAR_STRING to STRING by UNION
--echo #               if all values are empty string

CREATE TABLE t
SELECT 'AAA' AS a, '' AS b, '' AS c
UNION ALL
SELECT 'BBB', 'b', '';
...
--echo #
--echo # Bug#36838373: Failure in Item::convert_charset()
--echo #

CREATE TABLE t (c0 TEXT CHARACTER SET utf8mb4 COLLATE utf8mb4_romanian_ci);
