...
--echo ensure that view definitions are cached in the data-dictionary cache.
--disable_result_log
--disable_query_log
SHOW CREATE VIEW v1;
SHOW CREATE VIEW v2;
...
# Condition f1 < 7 gets pushed down to view v1 here. However when the
# tranformed query is printed in "NOTE", it does not display the pushed
# condition in the view definition. It just chooses to print the original view
# definition only.
EXPLAIN
...
# The query should write more than 1000 rows since it also includes
# rows dumped from memory to disk when the temporary table overflows.
# The hypergraph optimizer creates slightly more compact rows on
# 32-bit platforms, and can write some more rows to the temporary
# table before it overflows.
--replace_result 1031 1021
...
let query=
SELECT SHA2(pk,256) IN (SELECT * FROM (SELECT '' FROM t2) a) FROM t1;
...
--echo # Per MySQL rules, SUM(1) is aggregated in top query
--echo # (can't be in WHERE, in FROM)
...
--echo #
--echo # Bug#26596977: ASSERT `!(USED_TABS & (~READ_TABLES & ~FILTER_FOR_TABLE))'
--echo #               FAILED IN ITEM_FUNC
--echo #
...
--echo #
--echo # Bug #26618455: ASSERT 'KEYPARTS > 0' IN CALC_LENGTH_AND_KEYPARTS()
--echo #                WITHOUT FIRSTMATCH
--echo #
...
--echo # Bug#26798989: Sig 11 in find_table_in_global_list
...
--echo #
--echo # Bug#30381092 WRONG BEHAVIOR OF '=' CONDITION WHEN WORKING WITH COMPLEX SUBQUERIES
--echo #
...
--echo # Bug#30488700: Mysql chooses different execution plan in 5.7
...
--echo # Bug#26808862: Execution of prepared statement with ORDER BY
--echo #               in derived table may fail
...
--echo # Bug#31565009: Simplification of condition when seeing false and
--echo #               <cond> leaks tmp tables opened
...
--echo # Bug#31401637: Asan heap-buffer-overflow Handler::index_only_read_time
...
--echo #
--echo # Bug #32162862: ORDER BY SUB-SELECT COLUMN SETS ALL VALUES TO NULL
--echo #
...
--echo #
--echo # Bug #32164724: HYPERGRAPH: ASSERTION `PATH->COST >= PATH->INIT_COST' FAILED.
--echo #
...
--echo # Bug#34131822: Outer references in INSERT ... VALUES are not recognized
...
--echo #
--echo # Bug#33725542: A bug in sql_derived.cc cause mysql-server crash when
--echo #               input some sql statement..
...
--echo # Bug#36557029: Recent failure in check_column_grant_in_table_ref
...
SELECT a, a FROM cte1 WHERE cte1.a <= r.a;
...
SELECT _utf8mb4 'a' AS a UNION SELECT a FROM t;
SELECT * FROM (SELECT _utf8mb4 'a' AS a) AS dt UNION SELECT a FROM t;

--error ER_CANNOT_CONVERT_STRING
SELECT _utf8mb4 'Â±' AS a UNION SELECT a FROM t;
--error ER_CANNOT_CONVERT_STRING
SELECT * FROM (SELECT _utf8mb4 'Â±' AS a) AS dt UNION SELECT a FROM t;

SELECT _utf8mb4 'a' AS a UNION SELECT b FROM t;
SELECT * FROM (SELECT _utf8mb4 'a' AS a) AS dt UNION SELECT b FROM t;

SELECT _utf8mb4 'Â±' AS a UNION SELECT b FROM t;
SELECT * FROM (SELECT _utf8mb4 'Â±' AS a) AS dt UNION SELECT b FROM t;

SELECT _utf8mb4 'a' AS a UNION SELECT c FROM t;
SELECT * FROM (SELECT _utf8mb4 'a' AS a) AS dt UNION SELECT c FROM t;

--error ER_CANNOT_CONVERT_STRING
SELECT _utf8mb4 'Â±' AS a UNION SELECT c FROM t;
--error ER_CANNOT_CONVERT_STRING
SELECT * FROM (SELECT _utf8mb4 'Â±' AS a) AS dt UNION SELECT c FROM t;

SELECT _utf8mb4 'a' AS a UNION SELECT d FROM t;
SELECT * FROM (SELECT _utf8mb4 'a' AS a) AS dt UNION SELECT d FROM t;

SELECT _latin1 'a' AS a UNION SELECT a FROM t;
SELECT * FROM (SELECT _latin1 'a' AS a) AS dt UNION SELECT a FROM t;

--error ER_CANNOT_CONVERT_STRING
SELECT _latin1 'Ã¥' AS a UNION SELECT a FROM t;
--error ER_CANNOT_CONVERT_STRING
SELECT * FROM (SELECT _latin1 'Ã¥' AS a) AS dt UNION SELECT a FROM t;

SELECT _latin1 'a' AS a UNION SELECT c FROM t;
SELECT * FROM (SELECT _latin1 'a' AS a) AS dt UNION SELECT c FROM t;

--error ER_CANNOT_CONVERT_STRING
SELECT _latin1 'Ã¥' AS a UNION SELECT c FROM t;
--error ER_CANNOT_CONVERT_STRING
SELECT * FROM (SELECT _latin1 'Ã¥' AS a) AS dt UNION SELECT c FROM t;
