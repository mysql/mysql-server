...
execute stmt;
--disable_warnings WARN_DATA_TRUNCATED ONCE
...
delimiter ;|
# WL#5928: prepare of diagnostics statements fails now, cos the standard says it should.
--error ER_UNSUPPORTED_PS
...
--echo #
--echo # Bug#54494 crash with explain and prepared statements
--echo #
CREATE TABLE t1(a INT);
INSERT INTO t1 VALUES (1),(2);
...
--source include/ps_out_params_generated.inc
...
--echo #
--echo # Bug#12582849
--echo # ASSERTION FAILURE IN __CXA_PURE_VIRTUAL/ITEM_COND::FIX_FIELDS
--echo #
...
--echo Resolver errors should be given at prepare time in multi-table update
...
--exec $MYSQL_BINLOG --force-if-open -d client_test_db $MYSQLD_DATADIR/binlog.000001 > $MYSQLTEST_VARDIR/tmp/mysqlbinlog_bug19894382.binlog
--exec $MYSQL --commands -e "source $MYSQLTEST_VARDIR/tmp/mysqlbinlog_bug19894382.binlog"
...
--echo # Test parameter string longer than 65k
...
--echo # Bug#28929657: Assertion !is_temporal() at Item::get_time_from_int()
...
DROP TABLE t1;
--echo #
--echo # Bug#30438038: MAIN.LOCK_MULTI_BUG38499 FAILS ON PB2
--echo #
...
EXECUTE ps;
EXECUTE ps;  # The second execution used to fail.
DROP PREPARE ps;
DROP TABLE t;

--echo # Bug#31437028: SIG11 AT FIELD::HIDDEN() IN SQL/FIELD.H WITH PREPARED SHOW CREATE VIEW
...
--echo # Bug#32077842: "Illegal mix of collations" errors when matching ASCII field and UNICODE
...
--echo #
--echo # Bug #32751973: REGRESSION: HEAP-USE-AFTER-FREE IN ITEM_FIELD::BIND_FIELDS
--echo #
...
--echo #
--echo # Bug #32808790: DON'T ADD CACHES AROUND PREPARED STATEMENT PARAMETERS
--echo #
...
--echo # Bug#32213576: MySQL tries to parse string-bound float as integer
--echo #               and returns wrong error
...
--echo # Bug#32787037: Passed string type binding for integer column
--echo #               incorrectly cast to integer
...
--echo # Bug#34213338: Broken integer value empty string validation with
--echo #               bind vars (8.0.27 to 8.0.28)
...
--echo # Bug#33521861: Server fails to update row via PS but works via query
...
--echo Notice the above statement is implicitly reprepared, as there
--echo is no context available to guess the desired types.
--echo Using explicit CAST avoids the reprepare.
