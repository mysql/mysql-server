...
--echo # Index scans with PRIMARY is picked over a table scan unless we force table scans.
EXPLAIN FORMAT=TREE SELECT /*+ NO_INDEX(t1) */ * FROM t1;

--echo # Scanning along the primary index should be the same cost as table scan.
