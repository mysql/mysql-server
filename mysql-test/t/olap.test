...
--replace_regex /[0-9][0-9]/XX/
SELECT NOW(), (SELECT 1), GROUPING((SELECT 1)) FROM t1 GROUP BY (SELECT 1) WITH ROLLUP;
DROP TABLE t1;

CREATE TABLE t1(a INTEGER) engine=innodb;
SELECT RELEASE_ALL_LOCKS() FROM t1 GROUP BY a WITH ROLLUP;
SELECT RELEASE_ALL_LOCKS(), a, GROUPING(a) FROM t1 GROUP BY a WITH ROLLUP;
DROP TABLE t1;
...
SELECT (((@e:=`b`)) NOT BETWEEN 0x0b5f09 AND (CHAR(sha2(@pub1, 256),
...
SELECT
  REPLACE( IF( t2.a = t2.b, _latin1 'a', 'e'), 'd', 'b' ) AS rep,
  GROUPING(REPLACE( IF( t2.a = t2.b, _latin1 'a', 'e'), 'd', 'b' )) AS grp
FROM
  t1, t1 AS t2
GROUP BY
  REPLACE( IF( t2.a = t2.b, _latin1 'a', 'e'), 'd', 'b' )
...
--echo #
--echo # Bug#33349994 GROUP BY CONST WITH ROLLUP CRASHES WHEN EXECUTE PREPARE STMT TWICE
--echo #
...
(SELECT DISTINCT GROUPING(t3.x) FROM t AS t3, t AS t4
 GROUP BY ROLLUP(t3.x)) AS dt;
DROP TABLE t;

--echo #
--echo # Bug#35856247: Assertion Item_ref::walk at sql/item.h
--echo #

CREATE TABLE t(x TEXT);
INSERT INTO t VALUES ('a'), ('b'), ('b');
PREPARE ps FROM
  'SELECT dt.x, COUNT(*) FROM t, LATERAL (SELECT x) AS dt
   GROUP BY ROLLUP(dt.x)';
--sorted_result
EXECUTE ps;
--sorted_result
EXECUTE ps;
...
--echo #
--echo # Bug#35652424: Mysql server crash:
--echo # Query_block::check_column_privileges at sql/sql_select.cc
--echo #

CREATE TABLE t(i INTEGER);
PREPARE ps FROM 'SELECT 1+2 AS x FROM t GROUP BY ROLLUP(x) HAVING x <= 9';
EXECUTE ps;
EXECUTE ps;
DROP PREPARE ps;
DROP TABLE t;
...
--echo #
--echo # Bug#36257081: Assertion `!OrderItemsReferenceUnavailableTables
...
--error ER_SECONDARY_ENGINE
SELECT 1
FROM t1, LATERAL (SELECT f1 AS f2
                  FROM v1
                  GROUP BY CUBE(f2)) AS dt1
GROUP BY CUBE(f2);
...
--echo # check that we only get one row with multiple rollup levels also
CREATE TABLE t1 (a INT NOT NULL,
                 b INT NOT NULL,
                 c INT NOT NULL);
SELECT a, b, c, COUNT(*) AS count FROM t1 GROUP BY a, b, c WITH ROLLUP;
DROP TABLE t1;
...
--echo #
--echo # Bug#36750571 COUNT as window function can give NULL in conjunction with ROLLUP
--echo #
CREATE TABLE t(a INT);
...
SELECT a, GROUPING(a),
       COUNT(*),
       COUNT(*) OVER(),
       COUNT(a) OVER() # used to give NULL
FROM t GROUP BY a WITH ROLLUP
ORDER BY a;
DROP TABLE t;
...
--echo #
--echo # Bug#38203473: Assertion `is_nullable()' in Item_typecast_signed::val_int
--echo #               at sql/item_func.cc

CREATE TABLE t1 (f1 INTEGER, f2 INTEGER, PRIMARY KEY (f1));
# ROLLUP outputs one row for empty result
let $query = SELECT f1 FROM t1 WHERE f1 = 10 GROUP BY ROLLUP(f1);
--skip_if_hypergraph
--eval EXPLAIN FORMAT=tree $query
--eval $query
let $query = SELECT f1, AVG(f2), COUNT(f2) FROM t1 WHERE f1 = 10 GROUP BY ROLLUP(f1);
--skip_if_hypergraph
--eval EXPLAIN FORMAT=tree $query
--eval $query
let $query = SELECT f1, AVG(f2), COUNT(f2) FROM t1 WHERE f1 = 10 GROUP BY ROLLUP(f1) HAVING FALSE;
--eval EXPLAIN FORMAT=tree $query
--eval $query
...
let $query = SELECT f1 FROM t1 WHERE f1 = 10 GROUP BY ROLLUP(f2) ORDER BY CAST(f1 AS SIGNED);
--replace_regex $elide_costs
--skip_if_hypergraph
--eval EXPLAIN FORMAT=tree $query
--eval $query
let $query = SELECT f1 FROM t1 WHERE f1 = 10 GROUP BY ROLLUP(f2) HAVING f1 IS NOT NULL;
--replace_regex $elide_costs
--skip_if_hypergraph
--eval EXPLAIN FORMAT=tree $query
--eval $query
let $query = SELECT (SELECT 'a' FROM t1 AS t2 WHERE t2.f1 = CAST(t1.f1 AS SIGNED)) = 'aa' FROM t1 GROUP BY ROLLUP(f1);
--eval $query
SET optimizer_switch=@saved_opt_switch;

let $query = SELECT /*+NO_INDEX(t1) */ f1 FROM t1 WHERE f1 = 10 GROUP BY ROLLUP(f2);
--let $elide_differences = $elide_costs /t1\.f2/f2/
--replace_regex $elide_differences
--skip_if_hypergraph
--eval EXPLAIN FORMAT=tree $query
--eval $query
...
CREATE TABLE t1 (f1 INTEGER, f2 INTEGER);
let $query = SELECT f1 FROM t1 GROUP BY ROLLUP(f2);
--replace_regex $elide_differences
--eval EXPLAIN FORMAT=tree $query
--eval $query
let $query = SELECT ANY_VALUE(f1) FROM t1 GROUP BY ROLLUP(f2);
--replace_regex $elide_differences
--eval EXPLAIN FORMAT=tree $query
--eval $query;
...
--echo # GROUPING SET
--echo #
--echo # Empty or single GROUPING SETS can transformed to an implicitly
--echo # grouped query, or a primitive grouped query, respectively.

CREATE TABLE t1 (i int, j INT);
SELECT COUNT(*) FROM t1
GROUP BY GROUPING SETS (());

SELECT GROUPING(i) FROM t1
GROUP BY GROUPING SETS ((i));

INSERT INTO t1 VALUES (1, 2), (2,3),  (1, 2),  (1, 2),  (3, 4), (4,3);
ANALYZE TABLE t1;
...
--echo #
--echo # Bug#38107927 WL16494: Assertion failed in UpdateReferencesToMaterializedItems at finalize_plan.cc
--echo #

CREATE TABLE t1 (`col_date3` date DEFAULT NULL) ENGINE = InnoDB;

CREATE TABLE t2 (`col_date3` date DEFAULT NULL) ENGINE = InnoDB;

--sorted_result
SELECT MAX(table1.col_date3) AS field1
FROM t1 AS table1 STRAIGHT_JOIN t1 AS table2
ON 1
WHERE EXISTS (
    WITH cte1 AS (
      SELECT DISTINCT SUBQUERY1_t1.col_date3 AS SUBQUERY1_field1
      FROM       t1 AS SUBQUERY1_t1
      INNER JOIN t2 AS SUBQUERY1_t2
      RIGHT JOIN t1 AS SUBQUERY1_t3 ON 1
      ON 1
      GROUP BY GROUPING SETS ((SUBQUERY1_field1))
    )
    SELECT * FROM cte1
  )
WINDOW w1 AS ();
...
WHERE EXISTS (
  WITH cte1 AS (
      SELECT DISTINCT SUBQUERY1_t1.col_date3 AS SUBQUERY1_field1
      FROM  t1 AS SUBQUERY1_t1 RIGHT JOIN t1 AS SUBQUERY1_t3 ON 1
      GROUP BY GROUPING SETS ((SUBQUERY1_field1))
            )
  SELECT * FROM cte1
);
...
--echo #
--echo # Bug#38161069 WL16494: mysqld crash - Assertion failure in Field::get_geometry_type at field.h
--echo #
...
FROM t1 AS alias1, ( SELECT * FROM t1 ) AS alias2
WHERE (7, 8) IN ( SELECT SQ2_alias1.pk AS SQ2_field1,
                         MAX(SQ2_alias2.col_int_key) AS SQ2_field2
                   FROM t1 AS SQ2_alias1, t1 AS SQ2_alias2
                   WHERE SQ2_alias2.col_int >= 113
                   GROUP BY GROUPING SETS ((SQ2_field1))
                 );
DROP TABLE t1;


--echo #
--echo # Bug 38168233 - WL16494: mysqld crash - ASAN crash in Field_longlong::store
--echo #
...
SELECT COUNT( pk )
FROM t
WHERE (3,4)  IN ( SELECT SQ1_alias1 . pk AS SQ1_field1,
                         MAX(  SQ1_alias1.pk ) AS SQ1_field2
                  FROM ( t AS  SQ1_alias1, t AS  SQ1_alias2)
                  GROUP BY GROUPING SETS ( ( SQ1_field1 ) ) );
