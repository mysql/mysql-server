...
# check that we see conversion warnings for above queries when we use
# DISTINCT SUBSTRING, but make a smaller table t2 instead of t1 to
# avoid spewing out too many warnings on the canon.
...
--echo
--echo Bug#36035064 sum(distinct substring()) does not give warnings, while sum(substring() does
--echo
SELECT SUM(         SUBSTRING("text", 1, 3)) FROM dual;
--echo # Used to not give warnings:
SELECT SUM(DISTINCT SUBSTRING("text", 1, 3)) FROM dual;

--echo
--echo Bug#36796197: BIT query wrong result
--echo
