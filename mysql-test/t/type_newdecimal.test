...
--replace_regex /at row 1$/at row 5/
-- error 1264
...
--skip_if_hypergraph  # Undefined results.
...
--echo # Bug#32785804: Failure in decimal_intg
...
--echo # Bug#32802251: decimal_bin_size_inline: assertion `scale >= 0 &&
--echo #               precision > 0 && scale <= precision' failed
...
--echo #
--echo # Bug#32345941 SELECT RESULT IS MISMATCH WHEN USE COALESCE AND NOT
--echo #
...
--echo #
--echo # Bug #34479194 Query not returning result for given DECIMAL
--echo #               data type filter condittion
--echo #
...
--echo #
--echo # Bug#39037669: Failing in decimal_cmp
--echo #

SET @b:=1, @d:=1, @f:=1;
--error ER_DATA_OUT_OF_RANGE
SELECT (@b AND @d) NOT BETWEEN
          FLOOR(ST_LATFROMGEOHASH(ST_GEOHASH(POINT(105,84), 164)) OR @f) AND
          ROUND(194266643188592009712552808333820559360.000000);

--echo #
--echo # Bug#39037609: Failing in decimal_sub
--echo #

--error ER_WRONG_ARGUMENTS
SELECT PERIOD_ADD(-15803,-22788) -
       TRUNCATE(261755955496550870014828808043013079040.000000, 11498);

--error ER_INVALID_TYPE_FOR_JSON
SELECT JSON_STORAGE_FREE(34) -
       TRUNCATE(-9223372036854775809,
                0x3afaecdc717e37a8cf8d76a011a0f0d23b950ff4);

--echo #
--echo # Bug#39037556: More decimal failures, my_decimal::operator=
--echo #

SET @a:=1;
--error ER_INVALID_TYPE_FOR_JSON
SELECT ABS(TRUNCATE(SUM(14),
         COMPRESS(JSON_CONTAINS(@a, '{"bb":[3,1,0],"bbccb":true,"ccbc":0}'))));

--echo #
--echo # Bug#38949150: Simplify Item::val_decimal(): Return null
--echo #

--error ER_DATA_OUT_OF_RANGE
SELECT CAST(1000000000000*1000000000000 AS DECIMAL) BETWEEN 1.0 AND 2.0 AS col;

--error ER_DATA_OUT_OF_RANGE
SELECT CAST(1000000000000*1000000000000 AS DECIMAL) - 1.0 AS col;

--error ER_DATA_OUT_OF_RANGE
SELECT 1.0 - CAST(1000000000000*1000000000000 AS DECIMAL) AS col;

--error ER_DATA_OUT_OF_RANGE
SELECT CAST(1000000000000*1000000000000 AS DECIMAL) DIV 1.0 AS col;

--error ER_DATA_OUT_OF_RANGE
SELECT 1.0 DIV CAST(1000000000000*1000000000000 AS DECIMAL) AS col;

--error ER_DATA_OUT_OF_RANGE
SELECT CAST(1000000000000*1000000000000 AS DECIMAL) MOD 1.0 AS col;

--error ER_DATA_OUT_OF_RANGE
SELECT 1.0 MOD CAST(1000000000000*1000000000000 AS DECIMAL) AS col;

--error ER_DATA_OUT_OF_RANGE
SELECT - CAST(1000000000000*1000000000000 AS DECIMAL) AS col;

--error ER_DATA_OUT_OF_RANGE
SELECT ABS(CAST(1000000000000*1000000000000 AS DECIMAL)) AS col;

--error ER_DATA_OUT_OF_RANGE
SELECT CEILING(CAST(1000000000000*1000000000000 AS DECIMAL)) AS col;

--error ER_DATA_OUT_OF_RANGE
SELECT FLOOR(CAST(1000000000000*1000000000000 AS DECIMAL)) AS col;

--error ER_DATA_OUT_OF_RANGE
SELECT GREATEST(CAST(1000000000000*1000000000000 AS DECIMAL), 1.0) AS col;

--error ER_DATA_OUT_OF_RANGE
SELECT LEAST(CAST(1000000000000*1000000000000 AS DECIMAL), 1.0) AS col;

--error ER_DATA_OUT_OF_RANGE
SELECT FIELD(CAST(1000000000000*1000000000000 AS DECIMAL), 1.0, 2.0) AS col;

--error ER_DATA_OUT_OF_RANGE
SELECT EXISTS(SELECT 1
              WHERE CAST(1000000000000*1000000000000 AS DECIMAL) > 0
             ) > 1.0 AS col;

--error ER_DATA_OUT_OF_RANGE
SELECT SEC_TO_TIME(CAST(1000000000000*1000000000000 AS DECIMAL)) AS col;

--echo #
--echo # Bug#39053819: failing in Item_cache_decimal::cache_value()
--echo #

--error ER_WRONG_ARGUMENTS
SELECT AVG(1) = PERIOD_DIFF(1509498847,-26603);
