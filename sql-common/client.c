...
my_bool my_net_is_inited(NET *net) { return net->buff != NULL; }
...
void set_mysql_extended_error(MYSQL *mysql, int errcode, const char *sqlstate,
  ...
  my_vsnprintf(net->last_error, sizeof(net->last_error) - 1, format, args);
  va_end(args);
  my_stpcpy(net->sqlstate, sqlstate);
  ...
  for (i = 0; i < 100; i++) /* Don't retry forever */
  {
    ...
    if (!WaitNamedPipe(pipe_name, connect_timeout)) {
      set_mysql_extended_error(mysql, CR_NAMEDPIPEWAIT_ERROR, unknown_sqlstate,
                               ER(CR_NAMEDPIPEWAIT_ERROR), host, unix_socket,
                               (ulong)GetLastError());
      return INVALID_HANDLE_VALUE;
    }
  }
...
static HANDLE create_shared_memory(MYSQL *mysql, NET *net,
  ...
  */
  ...
  if (!(handle_connect_map = MapViewOfFile(
...
ulong cli_safe_read_with_ok(MYSQL *mysql, my_bool parse_ok,
...
my_bool cli_advanced_command(MYSQL *mysql, enum enum_server_command command,
                             ...
                             const uchar *arg, size_t arg_length,
  ...
  while (mysql->server_status & SERVER_MORE_RESULTS_EXISTS) {
    ...
    */
    ...
    if (!(mysql->server_capabilities & CLIENT_DEPRECATE_EOF)) {
      ...
      if ((mysql->fields = cli_read_metadata(mysql, mysql->net.read_pos[0],
...
static int add_init_command(struct st_mysql_options *options, const char *cmd) {
  ...
  if (!options->init_commands) {
    options->init_commands = (DYNAMIC_ARRAY *)my_malloc(
        key_memory_mysql_options, sizeof(DYNAMIC_ARRAY), MYF(MY_WME));
      ...
      if (option[0][0] == '-' && option[0][1] == '-') {
        ...
        case OPT_report_data_truncation:
          options->report_data_truncation =
              opt_arg ? MY_TEST(atoi(opt_arg)) : 1;
          break;
        case OPT_plugin_dir: {
...
int unpack_field(MYSQL *mysql, MEM_ROOT *alloc, my_bool default_value,
...
MYSQL_EXTENSION *mysql_extension_init(MYSQL *mysql MY_ATTRIBUTE((unused))) {
  ...
  ext = my_malloc(PSI_NOT_INSTRUMENTED, sizeof(MYSQL_EXTENSION),
                  MYF(MY_WME | MY_ZEROFILL));
  return ext;
}
...
static void mysql_ssl_free(MYSQL *mysql MY_ATTRIBUTE((unused))) {
...
static const char *my_os_charset_to_mysql_charset(const char *csname) {
...
static int client_mpvio_write_packet(struct st_plugin_vio *, const uchar *,
                                     int);
...
static uchar *write_length_encoded_string3(uchar *buf, char *string,
                                           size_t length) {
...
int embedded_ssl_check(MYSQL *mysql) {
  if (mysql->options.ssl_key || mysql->options.ssl_cert ||
    ...
    set_mysql_extended_error(
        mysql, CR_SSL_CONNECTION_ERROR, unknown_sqlstate,
    ...
    */
    ...
    if (my_net_write(net, (uchar *)buff, (size_t)(end - buff)) ||
      ...
      set_mysql_extended_error(
          ...
          "sending connection information to server", errno);
      goto error;
    }
    ...
    if (sslconnect(ssl_fd, net->vio, (long)(mysql->options.connect_timeout),
      ...
      buf[511] = 0;
      set_mysql_extended_error(mysql, CR_SSL_CONNECTION_ERROR, unknown_sqlstate,
                               ER(CR_SSL_CONNECTION_ERROR), buf);
      goto error;
    }
...
void mpvio_info(Vio *vio, MYSQL_PLUGIN_VIO_INFO *info) {
  ...
  case VIO_TYPE_SSL: {
    ...
    if (getsockname(vio_fd(vio), &addr, &addrlen))
      return;
    info->protocol =
        addr.sa_family == AF_UNIX ? MYSQL_VIO_SOCKET : MYSQL_VIO_TCP;
    info->socket = (int)vio_fd(vio);
    return;
  }
#ifdef _WIN32
...
MYSQL *STDCALL CLI_MYSQL_REAL_CONNECT(MYSQL *mysql, const char *host,
  ...
  */
  ...
  if (!net->vio &&
    ...
    if (sock == SOCKET_ERROR) {
      set_mysql_extended_error(mysql, CR_SOCKET_CREATE_ERROR, unknown_sqlstate,
                               ER(CR_SOCKET_CREATE_ERROR), socket_errno);
      goto error;
    }
    ...
    if (vio_socket_connect(net->vio, (struct sockaddr *)&UNIXaddr,
      ...
      set_mysql_extended_error(mysql, CR_CONNECTION_ERROR, unknown_sqlstate,
                               ER(CR_CONNECTION_ERROR), unix_socket,
                               socket_errno);
      vio_delete(net->vio);
      net->vio = 0;
      goto error;
  ...
  if (!net->vio && (!mysql->options.protocol ||
    ...
    my_snprintf(host_info = buff, sizeof(buff) - 1, ER(CR_TCP_CONNECTION),
                host);
    ...
    if (mysql->options.ci.bind_address) {
      ...
      bind_gai_errno = getaddrinfo(mysql->options.ci.bind_address, 0, &hints,
                                   &client_bind_ai_lst);
      if (bind_gai_errno) {
        DBUG_PRINT("info",
                   ("client bind getaddrinfo error %d", bind_gai_errno));
        set_mysql_extended_error(
            mysql, CR_UNKNOWN_HOST, unknown_sqlstate, ER(CR_UNKNOWN_HOST),
            mysql->options.ci.bind_address, bind_gai_errno);
  ...
  if (mysql->options.connect_timeout &&
    ...
    set_mysql_extended_error(
        mysql, CR_SERVER_LOST, unknown_sqlstate, ER(CR_SERVER_LOST_EXTENDED),
        "waiting for initial communication packet", socket_errno);
    goto error;
  }
...
int STDCALL mysql_options(MYSQL *mysql, enum mysql_option option,
  ...
  case MYSQL_READ_DEFAULT_FILE:
    ...
    mysql->options.my_cnf_file =
        my_strdup(key_memory_mysql_options, arg, MYF(MY_WME));
    break;
  ...
  case MYSQL_OPT_SSL_CRL:
    ...
    mysql->options.extension->ssl_crl = set_ssl_option_unpack_path(arg);
#elif defined(EMBEDDED_LIBRARY)
    DBUG_RETURN(1);
#endif
    break;
  case MYSQL_OPT_SSL_CRLPATH:
    ...
    if (mysql->options.extension)
      ...
      ALLOCATE_EXTENSIONS(&mysql->options);
    mysql->options.extension->ssl_crlpath = set_ssl_option_unpack_path(arg);
#elif defined(EMBEDDED_LIBRARY)
    DBUG_RETURN(1);
#endif
    break;
    ...
    if (my_hash_inited(&mysql->options.extension->connection_attributes)) {
      ...
      if (len) {
        ...
        if (elt) {
          ...
          my_hash_delete(&mysql->options.extension->connection_attributes, elt);
        }
      }
    }
    break;
...
int STDCALL mysql_get_option(MYSQL *mysql, enum mysql_option option,
  ...
  case MYSQL_OPT_GUESS_CONNECTION:
    *((my_bool *)arg) =
        (mysql->options.methods_to_use == MYSQL_OPT_GUESS_CONNECTION) ? TRUE
                                                                      : FALSE;
    break;
  case MYSQL_SET_CLIENT_IP:
  ...
  case MYSQL_OPT_SSL_VERIFY_SERVER_CERT: /* Deprecated. */
    *((my_bool *)arg) =
        (mysql->options.client_flag & CLIENT_SSL_VERIFY_SERVER_CERT) ? TRUE
                                                                     : FALSE;
    break;
  ...
  case MYSQL_PLUGIN_DIR:
    *((char **)arg) =
        mysql->options.extension ? mysql->options.extension->plugin_dir : NULL;
    break;
  case MYSQL_DEFAULT_AUTH:
  ...
  case MYSQL_OPT_TLS_VERSION:
    *((char **)arg) =
        mysql->options.extension ? mysql->options.extension->tls_version : NULL;
    break;
  case MYSQL_OPT_SSL_CRL:
    *((char **)arg) =
  ...
  case MYSQL_OPT_SSL_CRLPATH:
    *((char **)arg) =
        mysql->options.extension ? mysql->options.extension->ssl_crlpath : NULL;
    break;
  case MYSQL_SERVER_PUBLIC_KEY:
    *((char **)arg) = mysql->options.extension
...
*/
uchar *get_attr_key(LEX_STRING *part, size_t *length,
...
int STDCALL mysql_options4(MYSQL *mysql, enum mysql_option option,
  ...
  case MYSQL_OPT_CONNECT_ATTR_ADD: {
    ...
    */
    ...
    if (!my_multi_malloc(key_memory_mysql_options, MY_WME, &elt,
                         2 * sizeof(LEX_STRING), &key, key_len + 1, &value,
                         value_len + 1, NULL)) {
      set_mysql_error(mysql, CR_OUT_OF_MEMORY, unknown_sqlstate);
      DBUG_RETURN(1);
    }
    elt[0].str = key;
    elt[0].length = key_len;
    elt[1].str = value;
    elt[1].length = value_len;
    if (key_len)
