...
static struct my_option my_long_options[] = {
    ...
    {"symbols-file", 's', "Use specified symbols file.", &sym_fname, &sym_fname,
     0, GET_STR, REQUIRED_ARG, 0, 0, 0, 0, 0, 0},
    {"numeric-dump-file", 'n', "Read the dump from specified file.",
     &dump_fname, &dump_fname, 0, GET_STR, REQUIRED_ARG, 0, 0, 0, 0, 0, 0},
...
static uchar hex_val(char c) {
  uchar l;
  if (my_isdigit(&my_charset_latin1, c)) return c - '0';
  l = my_tolower(&my_charset_latin1, c);
  if (l < 'a' || l > 'f') return HEX_INVALID;
...
static my_long_addr_t read_addr(char **buf) {
  ...
  while ((c = hex_val(*p++)) != HEX_INVALID) addr = (addr << 4) + c;
...
static int init_sym_entry(SYM_ENTRY *se, char *buf) {
  ...
  if (!se->addr) return -1;
  while (my_isspace(&my_charset_latin1, *buf++)) /* empty */
    ;

  while (my_isspace(&my_charset_latin1, *buf++)) /* empty - skip more space */
    ;
  --buf;
...
static void init_sym_table() {
  ...
  */
  ...
  while (fgets(buf, sizeof(buf), fp_sym)) {
    SYM_ENTRY se;
    if (init_sym_entry(&se, buf)) continue;
    try {
...
static void do_resolve() {
  ...
  while (fgets(buf, sizeof(buf), fp_dump)) {
    ...
    /* skip space */
    while (my_isspace(&my_charset_latin1, *p)) ++p;
