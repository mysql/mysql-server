...
TEST_F(Xcl_protocol_impl_error_tests, execute_close) {
  ...
  m_context->m_global_error = XError{
    expected_error_code,
    expected_error_txt
  };
  ...
  ASSERT_EQ(
      ...
      m_sut->send(
          out_cmid,
          *static_cast<XProtocol::Message*>(&reset)).error());

  ASSERT_EQ(
      expected_error_code,
      m_sut->send(out_smid, buffer, 1).error());
  ...
  ASSERT_EQ(
      expected_error_code,
      m_sut->send(Mysqlx::Session::Reset()).error());

  ASSERT_EQ(
      expected_error_code,
      m_sut->send(Mysqlx::Session::Close()).error());
  ...
  ASSERT_EQ(
      expected_error_code,
      m_sut->send(Mysqlx::Crud::Find()).error());

  ASSERT_EQ(
      expected_error_code,
      m_sut->send(Mysqlx::Crud::Insert()).error());

  ASSERT_EQ(
      expected_error_code,
      m_sut->send(Mysqlx::Crud::Update()).error());
  ...
  ASSERT_EQ(
      expected_error_code,
      m_sut->send(Mysqlx::Crud::DropView()).error());

  ASSERT_EQ(
      expected_error_code,
      m_sut->send(Mysqlx::Expect::Open()).error());

  ASSERT_EQ(
      expected_error_code,
      m_sut->send(Mysqlx::Expect::Close()).error());
  ...
  ASSERT_EQ(
      expected_error_code,
      m_sut->execute_close().error());

  ASSERT_EQ(nullptr, m_sut->execute_with_resultset(out_cmid,
                                                   reset,
                                                   &out_error));
  ASSERT_EQ(expected_error_code, out_error.error());
  ASSERT_STREQ(expected_error_txt, out_error.what());

  ASSERT_EQ(nullptr, m_sut->execute_stmt(
      Mysqlx::Sql::StmtExecute(),
      &out_error));
  ASSERT_EQ(expected_error_code, out_error.error());
  ASSERT_STREQ(expected_error_txt, out_error.what());

  ASSERT_EQ(nullptr, m_sut->execute_find(
      Mysqlx::Crud::Find(),
      &out_error));
  ASSERT_EQ(expected_error_code, out_error.error());
  ASSERT_STREQ(expected_error_txt, out_error.what());

  ASSERT_EQ(nullptr, m_sut->execute_update(
      Mysqlx::Crud::Update(),
      &out_error));
  ASSERT_EQ(expected_error_code, out_error.error());
  ASSERT_STREQ(expected_error_txt, out_error.what());

  ASSERT_EQ(nullptr, m_sut->execute_insert(
      Mysqlx::Crud::Insert(),
      &out_error));
  ASSERT_EQ(expected_error_code, out_error.error());
  ASSERT_STREQ(expected_error_txt, out_error.what());

  ASSERT_EQ(nullptr, m_sut->execute_delete(
      Mysqlx::Crud::Delete(),
      &out_error));
  ASSERT_EQ(expected_error_code, out_error.error());
  ASSERT_STREQ(expected_error_txt, out_error.what());

  ASSERT_EQ(nullptr, m_sut->execute_fetch_capabilities(
      &out_error));
  ASSERT_EQ(expected_error_code, out_error.error());
  ASSERT_STREQ(expected_error_txt, out_error.what());

  ASSERT_EQ(expected_error_code, m_sut->execute_set_capability(
      Mysqlx::Connection::CapabilitiesSet()).error());

  ASSERT_EQ(expected_error_code, m_sut->execute_authenticate(
      "user", "pass", "db", "PLAIN").error());

  ASSERT_EQ(expected_error_code, m_sut->execute_authenticate(
      "user", "pass", "db", "MYSQL41").error());
}
