...
class Xcl_session_impl_tests_execute: public Xcl_session_impl_tests {
 ...
 public:
  void SetUp() override {
    m_sut = std::move(make_sut(connected));
  }
};
...
TEST_F(Xcl_session_impl_tests_execute_disconnected,
       execute_sql_not_usable) {
  XError out_error;
...
TEST_F(Xcl_session_impl_tests_execute_connected,
    execute_sql_fails) {
  XError out_error;
  EXPECT_CALL(*m_mock_protocol, execute_stmt_raw(
      Cmp_msg("stmt: \"SELECT 1;\""),
      _)).WillOnce(Invoke(
        [](const StmtExecute &stmt MY_ATTRIBUTE((unused)),
...
TEST_F(Xcl_session_impl_tests_execute_connected,
  ...
  XError out_error;
  EXPECT_CALL(*m_mock_protocol, execute_stmt_raw(
      Cmp_msg("stmt: \"SELECT 1;\" namespace: \"our-namespace\""),
      _)).WillOnce(Invoke(
        [](const StmtExecute &stmt MY_ATTRIBUTE((unused)),
           XError *out_r) -> XQuery_result_ptr {
          *out_r = XError{expected_error_code, ""};
...
TEST_F(Xcl_session_impl_tests_execute_connected,
  ...
  EXPECT_CALL(*m_mock_protocol, execute_stmt_raw(
      Cmp_msg("stmt: \"SELECT 1;\" namespace: \"our-namespace\""),
      _)).WillOnce(Invoke(
        [&dummy_result](const StmtExecute &stmt MY_ATTRIBUTE((unused)),
                        XError *out_r) -> XQuery_result_ptr {
          *out_r = {};
...
TEST_F(Xcl_session_impl_tests_execute_connected,
  ...
  auto query_result = m_sut->execute_stmt(
      ...
      expected_sql,
      {
          Argument_value(argument_value_int),
          Argument_value(argument_value_uint),
