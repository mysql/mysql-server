...
struct Auth_selector {
  std::string m_name;
  ngs::Account_verification_interface::Account_type m_verificator_type;
};
...
class Sasl_challenge_response_auth_test
 ...
 public:
  ...
  ::testing::StrictMock<ngs::test::Mock_authentication_interface>
      mock_authentication;

  using Response = ngs::Authentication_interface::Response;
};
...
TEST_P(Sasl_challenge_response_auth_test, handle_start_call_twice) {
  ...
  ASSERT_RESPONSE(
      Response(ngs::Authentication_interface::Ongoing, ER_SUCCESS, SALT),
      auth->handle_start(GetParam().m_name, AUTH_DATA, EMPTY));
  ASSERT_RESPONSE(Response(ngs::Authentication_interface::Error,
