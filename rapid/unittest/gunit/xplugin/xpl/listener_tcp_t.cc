...
public:
  ...
  void assert_verify_and_reinitailize_rules() {
    ...
    ON_CALL(*m_mock_system, get_socket_error_and_message(_,_)).WillByDefault(DoAll(SetArgReferee<0>(0), SetArgReferee<1>("")));
  }
...
TEST_F(Listener_tcp_testsuite, setup_listener_does_resolved_IP6_and_IP4_localhost_when_asterisk_and_IP6_supported) {
  ...
  EXPECT_CALL(*m_mock_factory, create_socket(
      static_cast<PSI_socket_key>(PSI_NOT_INSTRUMENTED),
      AF_INET6,
      SOCK_STREAM,
      0))
    .WillOnce(Return(m_mock_socket));
...
TEST_F(Listener_tcp_testsuite, setup_listener_does_resolved_IP4_localhost_when_asterisk_and_IP6_not_supported) {
  ...
  EXPECT_CALL(*m_mock_factory, create_socket(
      static_cast<PSI_socket_key>(PSI_NOT_INSTRUMENTED), AF_INET6, SOCK_STREAM, 0))
    .WillOnce(Return(m_mock_socket));
...
TEST_F(Listener_tcp_testsuite, setup_listener_listen_failure) {
  ...
  EXPECT_CALL(*m_mock_socket, set_socket_opt(
      SOL_SOCKET,
      SO_REUSEADDR,
      _,
