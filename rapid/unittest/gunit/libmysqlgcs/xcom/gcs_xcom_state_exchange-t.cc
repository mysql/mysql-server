...
public:
  ...
  MOCK_CONST_METHOD0(get_local_member_identifier, const Gcs_member_identifier ());
...
TEST_F(XComStateExchangeTest, StateExchangeBroadcastJoinerTest)
{
  ...
  EXPECT_CALL(*comm_mock, send_binding_message(_,_,_)).Times(1)
  ...
  bool leaving= state_exchange->state_exchange(configuration_id,
                                               total_members,
                                               left_members,
                                               joined_members,
                                               data_to_exchange,
...
enum_gcs_error
copy_message_content(const Gcs_message &msg)
{
...
TEST_F(XComStateExchangeTest, StateExchangeProcessStatesPhase)
{
  ...
  */
  ...
  ASSERT_TRUE(synode_eq(state_1->get_configuration_id(),
              configuration_id));
  ...
  */
  bool can_install= state_exchange->process_member_state(state_1, *member_id_1);
...
TEST_F(XComStateExchangeTest, StateExchangeDiscardSynodes)
{
  ...
  */
  ...
  /*
    Send a state exchange message on behalf of member 1.
  */
  state_exchange->state_exchange(configuration_id,
                                 total_members,
                                 left_members,
                                 joined_members,
                                 data_to_exchange,
  ...
  */
  ...
  bool can_install= state_exchange->process_member_state(state_1,
                                                         *member_id_1);
  ASSERT_FALSE(can_install);
  ASSERT_EQ(state_exchange->get_member_states()->size(), 0u);
