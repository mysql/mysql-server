...
{
protected:
  LoggingDebuggingSystemTest() : common_sink(NULL), logger(NULL), debugger(NULL)
  {
  }
...
public:
  ...
  {
    ON_CALL(*this, log_event(_)).WillByDefault(
      Invoke(m_sink, static_cast<void (Gcs_file_sink::*)(const std::string &)>
      (&Gcs_file_sink::log_event))
    );
    ON_CALL(*this, log_event(_,_)).WillByDefault(
      Invoke(m_sink, static_cast<void (Gcs_file_sink::*)(const char *, size_t)>
      (&Gcs_file_sink::log_event))
    );
    ON_CALL(*this, initialize()).WillByDefault(Invoke(m_sink, &Gcs_file_sink::initialize));
    ON_CALL(*this, finalize()).WillByDefault(Invoke(m_sink, &Gcs_file_sink::finalize));
    ON_CALL(*this, get_file_name(_)).WillByDefault(Invoke(m_sink, &Gcs_file_sink::get_file_name));
    ON_CALL(*this, get_information()).WillByDefault(Invoke(m_sink, &Gcs_file_sink::get_information));
  }
...
TEST_F(FileOutputSystemTest, SinkFileTest)
{
  ...
  */
  Wrapper_file_sink wrapper_unknown(new Gcs_file_sink("unknown", "/unknown/unknown/"));
