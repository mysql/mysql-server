...
{
  Gcs_internal_message_header gcs_hd(Gcs_internal_message_header::CT_INTERNAL_STATE_EXCHANGE);
  unsigned long long payload_len= SMALL_PAYLOAD_LEN;
  unsigned long long size= payload_len + Gcs_internal_message_header::WIRE_FIXED_HEADER_SIZE;
  // create a packet to send
  Gcs_packet p(size);
  unsigned char *control= (unsigned char*) malloc(payload_len);

  // mess around with the payload
  memset(p.get_buffer() + Gcs_internal_message_header::WIRE_FIXED_HEADER_SIZE, 0x61, payload_len);
  memset(control, 0x61, payload_len);

  // create the header
  gcs_hd.set_msg_length(size+Gcs_internal_message_header::WIRE_FIXED_HEADER_SIZE);
  ...
  ASSERT_EQ(p.get_payload() - p.get_buffer(), Gcs_internal_message_header::WIRE_FIXED_HEADER_SIZE);
  EXPECT_TRUE(strncmp((const char*)p.get_payload(), (const char*)control, SMALL_PAYLOAD_LEN) == 0);
...
TEST_F(XcomStagesTest, CompressMessage)
{
  ...
  ASSERT_EQ(p.get_dyn_headers_length(),
            static_cast<unsigned int>(Gcs_message_stage_lz4::WIRE_HD_LEN_SIZE +
                                      Gcs_message_stage_lz4::WIRE_HD_TYPE_SIZE +
                                      Gcs_message_stage_lz4::WIRE_HD_UNCOMPRESSED_SIZE));
  EXPECT_FALSE(strncmp((const char*)p.get_payload(), (const char*)control, LARGE_PAYLOAD_LEN) == 0);
...
TEST_F(XcomStagesTest, CompressDecompressMessage)
{
  ...
  ASSERT_EQ(before_length, after_length);
  ASSERT_EQ(p.get_dyn_headers_length(), (unsigned int) 0); // all headers processed
  EXPECT_TRUE(strncmp((const char*)p.get_payload(), (const char*)control, LARGE_PAYLOAD_LEN) == 0);
...
TEST_F(XcomStagesTest, CompressDecompressMessageBoundary)
{
  ...
  ASSERT_EQ(before_length, after_length);
  ASSERT_EQ(p.get_dyn_headers_length(), (unsigned int) 0); // all headers processed
  EXPECT_TRUE(strncmp((const char*)p.get_payload(), (const char*)control, payload_len) == 0);
...
{
  std::string error_message("Gcs_packet's payload is too big. Only the packets "
