...
void read_optional_time(pb::io::CodedInputStream *input_stream,
               Hour_type *hour,
               uint8_t *minutes,
               uint8_t *seconds,
               uint32_t *useconds) {
  ...
  if (!input_stream->ReadVarint64(&value))
    return;
...
bool read_required_uint64(
    pb::io::CodedInputStream *input_buffer,
  ...
  if (out_result)
    *out_result = static_cast<Numeric_type>(value);
  return true;
}
...
bool buffer_to_s64(const std::string& buffer,
                   int64_t *out_result) {
...
bool buffer_to_u64(const std::string& buffer,
                     uint64_t *out_result) {
  pb::io::CodedInputStream input_stream(
      reinterpret_cast<const pb::uint8*>(&buffer[0]),
      static_cast<int>(buffer.length()));
...
bool buffer_to_string(
  ...
  if (out_length)
    *out_length = buffer.length() - 1;
  if (out_result)
    *out_result = buffer.c_str();
...
bool buffer_to_string_set(const std::string& buffer,
...
bool buffer_to_float(const std::string& buffer,
                     float *out_result) {
  pb::io::CodedInputStream input_stream(
      reinterpret_cast<const pb::uint8*>(&buffer[0]),
      static_cast<int>(buffer.length()));
...
bool buffer_to_double(const std::string& buffer,
                      double *out_result) {
  pb::io::CodedInputStream input_stream(
      reinterpret_cast<const pb::uint8*>(&buffer[0]),
      static_cast<int>(buffer.length()));
...
bool buffer_to_datetime(const std::string& buffer,
  ...
  if (!read_required_uint64(&input_stream, &year))
    return false;

  if (!read_required_uint64(&input_stream, &month))
    return false;
...
bool buffer_to_decimal(const std::string& buffer,
                       Decimal *out_result) {
  if (out_result)
    *out_result = Decimal::from_bytes(buffer);
