...
void xpl::Update_statement_builder::add_operation(
    const Operation_list &operation, const bool is_relational) const {
  if (operation.size() == 0)
...
void xpl::Update_statement_builder::add_document_operation_item(
    const Operation_item &item, int &opeartion_id) const {
  if (opeartion_id != item.operation()) m_builder.put(")");
  ...
  switch (item.operation()) {
    ...
    case UpdateOperation::ITEM_MERGE: {
      ...
      m_builder.put(",IF(JSON_TYPE(")
          .put(value)
          .put(")='OBJECT',JSON_REMOVE(")
          .put(value)
          .put(",'$._id'),'_ERROR_')");
      break;
    }
...
void xpl::Update_statement_builder::add_document_operation(
...
struct Is_not_equal {
  Is_not_equal(const Operation_item &item) : m_item(item) {}

  bool operator()(const Operation_item &item) const {
    return item.source().name() != m_item.source().name() ||
...
void xpl::Update_statement_builder::add_table_operation(
    const Operation_list &operation) const {
...
void xpl::Update_statement_builder::add_table_operation_items(
...
void xpl::Update_statement_builder::add_member(const Operation_item &item)
    const {
  if (item.source().document_path_size() == 0)
...
void xpl::Update_statement_builder::add_value(const Operation_item &item)
    const {
  m_builder.put(",").put_expr(item.value());
}

void xpl::Update_statement_builder::add_member_with_value(
    const Operation_item &item) const {
  add_member(item);
  add_value(item);
}

void xpl::Update_statement_builder::add_field_with_value(
    const Operation_item &item) const {
