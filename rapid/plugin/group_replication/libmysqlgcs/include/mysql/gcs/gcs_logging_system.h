...
class Gcs_log_event
{
  public:
    ...
    */
    ...
    inline void set_event(bool ready)
    {
      m_ready_flag.store(ready);
    }
    ...
    */
    ...
    inline bool flush_event(Sink_interface &sink)
    {
      /*
        ...
        the number of entries in the buffer is increased before filling the buffer.
        This happens because the copy itself is done outside the critical section.
...
class Gcs_default_debugger
{
  public:
    ...
    */
    ...
    inline Gcs_log_event &get_entry()
    {
      return m_sink->get_entry();
    }
    ...
    */
    ...
    inline void log_event(const char* format, va_list args) MY_ATTRIBUTE((format(printf, 2, 0)))
    {
      Gcs_log_event &event= get_entry();
      char *buffer= event.get_buffer();
      size_t size= append_prefix(buffer);
      size += vsnprintf(buffer + size, event.get_max_buffer_size() - size, format, args);
    ...
    */
    ...
    inline void log_event(const int64_t options, Args ... args)
    {
      if (Gcs_debug_options::test_debug_options(options))
      {
        ...
        char *buffer= event.get_buffer();
        size_t size= append_prefix(buffer);
        size += snprintf(buffer + size, event.get_max_buffer_size() - size, args...);
        if (unlikely(size > event.get_max_buffer_size()))
        {
          fprintf(stderr, "The following message was truncated: %s\n", buffer);
          size = event.get_max_buffer_size();
...
public:
  ...
  */
  ...
  /**
    This static method retrieves the currently set debugging system, allowing the
    debugging macros to invoke its log_event method.
  ...
  */
  ...
  /**
    ...
    It is invoked by the Gcs_debug_manager::finalize method during the GCS interface
    termination procedure, and also by the Gcs_debug_manager::initialize method in
    case a debugging system was set previously.
...
#define MYSQL_GCS_LOG_DEBUG(...) \
  MYSQL_GCS_LOG_DEBUG_WITH_OPTION(GCS_DEBUG_BASIC | GCS_DEBUG_TRACE, __VA_ARGS__)
