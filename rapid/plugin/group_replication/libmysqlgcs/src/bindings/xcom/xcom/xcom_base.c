...
static void	read_missing_values(int n)
{
	...
	while (!synode_gt(find, end) && i < n && !too_far(find)) {
		...
		MAY_DBG(FN; SYCEXP(find); SYCEXP(end); NDBG(recently_active(p), d); NDBG(finished(p), d); NDBG(is_busy_machine(p),
		     d));
...
static u_int is_reincarnation_adding(app_data_ptr a)
{
	...
	for(; i < nodes_len; i++)
	{
		if (nodes_to_change[i].uuid.data.data_len != 0 &&
                       (node_exists(&nodes_to_change[i], &new_site_def->nodes) ||
...
static u_int is_reincarnation_removing(app_data_ptr a)
{
	...
	{
                if (nodes_to_change[i].uuid.data.data_len != 0 &&
                     !node_exists_with_uid(&nodes_to_change[i], &new_site_def->nodes))
		{
			/*
