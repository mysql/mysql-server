...
test_truncate_subdb (int n) {
    ...
    r = db->open(db, 0, "test.db", "a", DB_BTREE, DB_CREATE, S_IRWXU+S_IRWXG+S_IRWXO); assert(r == 0);
    for (i=0; i<1; i++) {
    ...
    r = db_create(&db, env, 0); assert(r == 0);
    r = db->open(db, 0, "test.db", "b", DB_BTREE, DB_CREATE, S_IRWXU+S_IRWXG+S_IRWXO); assert(r == 0);
    ...
    r = db_create(&db, env, 0); assert(r == 0);
    r = db->open(db, 0, "test.db", "b", DB_UNKNOWN, 0, S_IRWXU+S_IRWXG+S_IRWXO); assert(r == 0);
