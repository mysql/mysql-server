...
static void insert(DB_LOADER *loader, int k, int val_size) {
    ...
    r = loader->put(loader, &key, &value); assert(r == 0);
...
int test_main(int argc, char * const argv[]) {
    ...
    r = db_env->set_generate_row_callback_for_put(db_env, my_row_generate); assert(r == 0);
    r = db_env->set_default_bt_compare(db_env, my_bt_compare); assert(r == 0);
    if (!do_txn)
    ...
    r = db_env->create_loader(db_env, create_txn, &loader, db, 1, &db, &db_flags, &dbt_flags, 0); assert(r == 0);

    insert(loader, 1, 8000000);
    insert(loader, 2, 1);

    r = loader->close(loader); assert(r == 0);
