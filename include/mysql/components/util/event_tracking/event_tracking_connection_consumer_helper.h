...
#ifndef EVENT_TRACKING_CONNECTION_CONSUMER_HELPER_H
#define EVENT_TRACKING_CONNECTION_CONSUMER_HELPER_H
...
/**
  @file event_tracking_connection_consumer_helper.h
  Helper file to create connection event consumer
*/

// clang-format off
/**
  @anchor EVENT_TRACKING_CONNECTION_CONSUMER_EXAMPLE
  ...
  IMPLEMENTS_SERVICE_EVENT_TRACKING_CONNECTION(<implementation_name>);

  BEGIN_COMPONENT_PROVIDES(<component_name>)
  PROVIDES_SERVICE_EVENT_TRACKING_CONNECTION(<implementation_name>)
...
#define PROVIDES_SERVICE_EVENT_TRACKING_CONNECTION(component) \
  PROVIDES_SERVICE(component, event_tracking_connection)

#define IMPLEMENTS_SERVICE_EVENT_TRACKING_CONNECTION(component)             \
  BEGIN_SERVICE_IMPLEMENTATION(component, event_tracking_connection)        \
...
class Event_tracking_connection_implementation {
 public:
  ...
  /**
    ...
    @param [in] data  Data related to connection event
  ...
  */
  static DEFINE_BOOL_METHOD(
      notify, (const mysql_event_tracking_connection_data *data)) {
    try {
      if (!data) return true;
      if (data->event_subclass & filtered_sub_events) return false;
...
#endif  // !EVENT_TRACKING_CONNECTION_CONSUMER_HELPER_H
