...
// Workaround for Sun Developer Studio 12.6
#if defined(__SUNPRO_CC)
#define BOOST_RATIONAL_INTCREF_CAST(VARIABLE) static_cast<int_type const&>(VARIABLE)
#else
#define BOOST_RATIONAL_INTCREF_CAST(VARIABLE) VARIABLE
#endif
...
bool rational<IntType>::operator< (const rational<IntType>& r) const
{
    ...
    for ( ;; )
    {
        ...
        if ( BOOST_RATIONAL_INTCREF_CAST(ts.q)
                != BOOST_RATIONAL_INTCREF_CAST(rs.q) )
        {
            // Since reciprocation changes the relative order of two variables,
            // and c.f. use reciprocals, the less/greater-than test reverses
            // after each index.  (Start w/ non-reversed @ whole-number place.)
            return reverse
		 ? (BOOST_RATIONAL_INTCREF_CAST(ts.q) > BOOST_RATIONAL_INTCREF_CAST(rs.q))
		 : (BOOST_RATIONAL_INTCREF_CAST(ts.q) < BOOST_RATIONAL_INTCREF_CAST(rs.q));
        }
        ...
        ts.n = ts.d;         ts.d = ts.r;
        ts.q = BOOST_RATIONAL_INTCREF_CAST(ts.n) / BOOST_RATIONAL_INTCREF_CAST(ts.d);
	ts.r = BOOST_RATIONAL_INTCREF_CAST(ts.n) % BOOST_RATIONAL_INTCREF_CAST(ts.d);
        rs.n = rs.d;         rs.d = rs.r;
        rs.q = BOOST_RATIONAL_INTCREF_CAST(rs.n) / BOOST_RATIONAL_INTCREF_CAST(rs.d);
	rs.r = BOOST_RATIONAL_INTCREF_CAST(rs.n) % BOOST_RATIONAL_INTCREF_CAST(rs.d);
    }

    // Compare infinity-valued components for otherwise equal sequences
    if ( BOOST_RATIONAL_INTCREF_CAST(ts.r)
            == BOOST_RATIONAL_INTCREF_CAST(rs.r) )
    {
