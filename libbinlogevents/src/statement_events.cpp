...
Query_event::Query_event(const char *buf, unsigned int event_len,
  ...
  */
  if (event_len < (unsigned int)(common_header_len + post_header_len)) return;
  ...
  */
  ...
  for (const Log_event_header::Byte *pos = start; pos < end;) {
    switch (*pos++) {
      ...
      case Q_CATALOG_NZ_CODE:
        if ((catalog_len = *pos)) catalog = (const char *)(pos + 1);
      ...
      case Q_TIME_ZONE_CODE: {
        if ((time_zone_len = *pos)) time_zone_str = (const char *)(pos + 1);
        pos += time_zone_len + 1;
        break;
      }
...
#endif
          strncpy(
              mts_accessed_db_names[i], (char *)pos,
  ...
  if (time_zone_len) query_data_written += time_zone_len + 1;
  if (user_len > 0) query_data_written += user_len + 1;
  if (host_len > 0) query_data_written += host_len + 1;
...
User_var_event::User_var_event(
  ...
  */
  ...
  if (is_null) {
    ...
    memcpy(&val_len,
           (buf + UV_VAL_IS_NULL + UV_VAL_TYPE_SIZE + UV_CHARSET_NUMBER_SIZE),
           sizeof(val_len));
