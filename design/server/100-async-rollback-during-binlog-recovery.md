...
Primary GitHub Issue: 779
Design and implementation PR: 711
Contributors: Libing Song, Genze Wu
...
FR1. Binlog recovery MUST preserve the transaction decision: an internal
prepared transaction absent from the recovered binlog commit set must
eventually be rolled back.

## Non-Functional Requirements

NFR1. The time binlog recovery spends rolling back a recovered prepared DML
transaction SHOULD be independent of the transaction's size. Binlog recovery
MUST NOT wait for row-by-row undo of a recovered prepared DML transaction.
The server MUST be able to complete startup while the transaction remains
observable as `ROLLING BACK`. Increasing the number of rows to undo MAY
increase background rollback time, but MUST NOT add those row-undo operations
to the startup critical path.
...
1. **Prepared DML rollback and DDL exclusion (FR1, NFR1):** crash after InnoDB
   ...
   background thread before it applies any undo and verify that server startup
   completes while the uncommitted row remains visible and the transaction is
   still rolling back. Verify that a concurrent `DROP TABLE` waits for metadata
   lock. Crash again before the DDL can execute, restart normally, and verify
   that the uncommitted row disappears. Reusing the affected key verifies that
   no transaction or lock remains.
2. **Crash after a durable handoff (FR1, NFR1):** create a prepared DML
   transaction whose XID is absent from the binlog, then restart and pause its
   background rollback after binlog recovery has persisted the
   prepared-to-active transition. Verify through `INNODB_TRX` that it is
   `ROLLING BACK`, crash again, and verify that a normal restart recovers it as
   active and completes the rollback.
3. **Crash before the handoff begins (FR1):** use the
