...
void get_date(char *to, int flag, time_t date) {
  ...
  if (flag & GETDATE_SHORT_DATE)
    sprintf(to, "%02d%02d%02d", start_time->tm_year % 100,
            start_time->tm_mon + 1, start_time->tm_mday);
  else if (flag & GETDATE_SHORT_DATE_FULL_YEAR)
    sprintf(to, "%4d%02d%02d", start_time->tm_year + 1900,
            start_time->tm_mon + 1, start_time->tm_mday);
  else
    sprintf(to,
            ((flag & GETDATE_FIXEDLENGTH) ? "%4d-%02d-%02d" : "%d-%02d-%02d"),
            start_time->tm_year + 1900, start_time->tm_mon + 1,
            start_time->tm_mday);
  if (flag & GETDATE_DATE_TIME) {
    if (flag & GETDATE_T_DELIMITER) sprintf(strend(to), "T");
    sprintf(
        strend(to),
    ...
    if (flag & GETDATE_T_DELIMITER) sprintf(strend(to), "T");
    sprintf(strend(to), "%02d%02d%02d", start_time->tm_hour, start_time->tm_min,
            start_time->tm_sec);
  }
} /* get_date */
