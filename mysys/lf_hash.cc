...
/*
  Lookup table used by reverse_bits(). The reverse_bits_table[i] contains the
  value of "i" with the bits reversed.
...
static uint32_t reverse_bits(uint32_t key) {
  return (reverse_bits_table[key & 255] << 24) |
         (reverse_bits_table[(key >> 8) & 255] << 16) |
         (reverse_bits_table[(key >> 16) & 255] << 8) |
         reverse_bits_table[(key >> 24)];
}
...
int lf_hash_delete(LF_HASH *hash, LF_PINS *pins, const void *key, uint keylen) {
  ...
  */
  ...
  if (ldelete(el, hash->cmp_function, hash->charset, reverse_bits(hashnr) | 1,
...
void *lf_hash_search(LF_HASH *hash, LF_PINS *pins, const void *key,
  ...
  found = my_lsearch(el, hash->cmp_function, hash->charset,
                     reverse_bits(hashnr) | 1, pointer_cast<const uchar *>(key),
                     keylen, pins);
