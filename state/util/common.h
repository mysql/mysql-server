...
using rwlatch_t = uint64_t;  // latch that has to be modified by RDMA atomic
...
#define STATE_TXN_ACTIVE \
  0x0  // the transaction is in processing (cannot release locks)
#define STATE_TXN_COMMITING \
  0x1  // the transaction is commtting (cannot request new lock)
#define STATE_TXN_ABORTING \
  0x2  // the transaction is aborting (cannot request new lock)
