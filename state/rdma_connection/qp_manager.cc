...
void QPManager::BuildQPConnection(MetaManager* meta_man) {
  for (const auto& remote_node : meta_man->remote_nodes) {
    ...
    RCQP* txn_list_qp = meta_man->global_rdma_ctrl->create_rc_qp(create_rc_idx(remote_node.node_id, (int)global_tid * 2),
                                                             meta_man->opened_rnic,
                                                             &local_mr);
    RCQP* lock_buf_qp = meta_man->global_rdma_ctrl->create_rc_qp(create_rc_idx(remote_node.node_id, (int)global_tid * 2 + 1),
                                                            meta_man->opened_rnic,
                                                            &local_mr);
    RCQP* log_buf_qp = meta_man->global_rdma_ctrl->create_rc_qp(create_rc_idx(remote_node.node_id, (int)global_tid*2 + 2),
                                                            meta_man->opened_rnic,
                                                            &local_mr);
