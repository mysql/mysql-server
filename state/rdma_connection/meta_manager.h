...
class MetaManager {
 public:
  ...
  ALWAYS_INLINE
  offset_t GetTxnListLatchAddr() { return txn_list_latch_addr; }

  ALWAYS_INLINE
  offset_t GetTxnListBitmapAddr() { return txn_list_bitmap_addr; }
  ...
  ALWAYS_INLINE
  size_t GetTxnBitmapSize() { return txn_bitmap_size; }
 ...
 private:
  ...
  // MemoryAttrs for various states in remote StateNodes, there may be multiple
  // StateNodes because we assume that the StateNode can elasticly expand.
  std::unordered_map<node_id_t, MemoryAttr>
      remote_lock_buf_mrs;  // MemoryAttr for lock_buffer in remote StateNodes
  std::unordered_map<node_id_t, MemoryAttr>
      remote_txn_list_mrs;  // MemoryAttr for txn_list in remote StateNodes
  std::unordered_map<node_id_t, MemoryAttr>
      remote_log_buf_mrs;  // MemoryAttr for log_buffer in remote StateNodes
 ...
 public:
  ...
  RdmaCtrlPtr global_rdma_ctrl;  // rdma controller used by QPManager and local
                                 // RDMA Region
